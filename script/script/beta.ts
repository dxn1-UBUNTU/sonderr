...
const model = "sonderr/gpt-5.3-codex"
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with sonderr...`)
  ...
  try {
    await $`sonderr run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  sonderr failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with sonderr")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with sonderr...")
