...
import { createSonderr } from "@sonderr/sdk"
...
`)
  ...
  const sonderr = await createSonderr({ port: 0 })
  ...
  try {
    ...
    const session = await sonderr.client.session.create()
    const result = await sonderr.client.session
      .prompt({
