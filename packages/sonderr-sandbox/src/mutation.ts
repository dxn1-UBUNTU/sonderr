...
declare const SONDERR_SANDBOX_MUTATION_WORKER_PATH: string

function worker() {
  if (typeof SONDERR_SANDBOX_MUTATION_WORKER_PATH === "undefined") {
    return { path: fileURLToPath(new URL("./sonderr-sandbox-mutation-worker.ts", import.meta.url)), environment: {} }
  }
  const path = SONDERR_SANDBOX_MUTATION_WORKER_PATH.startsWith(".")
    ? fileURLToPath(new URL(SONDERR_SANDBOX_MUTATION_WORKER_PATH, import.meta.url))
    : resolve(dirname(process.execPath), SONDERR_SANDBOX_MUTATION_WORKER_PATH)
...
const CurrentRunner = Context.Reference<Runner>("@sonderr/sandbox/CurrentMutationRunner", {
  defaultValue: () => mutate,
})
