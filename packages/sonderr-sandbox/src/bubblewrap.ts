...
declare const SONDERR_BWRAP_SHA256: string | undefined
declare const SONDERR_SANDBOX_NETWORK_RELAY_PATH: string | undefined
declare const SONDERR_SANDBOX_SECCOMP_PATH: string | undefined
...
function relay() {
  if (typeof SONDERR_SANDBOX_NETWORK_RELAY_PATH === "undefined") {
    return { path: fileURLToPath(new URL("./sonderr-sandbox-network-relay.ts", import.meta.url)), environment: {} }
  }
  const target = SONDERR_SANDBOX_NETWORK_RELAY_PATH.startsWith(".")
    ? fileURLToPath(new URL(SONDERR_SANDBOX_NETWORK_RELAY_PATH, import.meta.url))
    : path.resolve(path.dirname(process.execPath), SONDERR_SANDBOX_NETWORK_RELAY_PATH)
  return { path: target, environment: { BUN_BE_BUN: "1" } }
}

function seccomp() {
  if (typeof SONDERR_SANDBOX_SECCOMP_PATH !== "undefined") {
    return path.resolve(path.dirname(process.execPath), SONDERR_SANDBOX_SECCOMP_PATH)
  }
...
function digest() {
  return typeof SONDERR_BWRAP_SHA256 === "undefined" ? undefined : SONDERR_BWRAP_SHA256
}
...
function select(): Selection {
  const override = process.env.SONDERR_BWRAP_PATH
