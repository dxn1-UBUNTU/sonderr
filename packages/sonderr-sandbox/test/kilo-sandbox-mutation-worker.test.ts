...
async function worker(request: Request) {
  const entry = fileURLToPath(new URL("../src/sonderr-sandbox-mutation-worker.ts", import.meta.url))
...
describe("filesystem mutation worker", () => {
  test("executes ordered mutation batches", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "sonderr-mutation-worker-"))
    roots.push(root)
