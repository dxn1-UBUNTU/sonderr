// Individual chat message bubble — renders Sonderr Chat ContentBlock[] content
// with reactions, action approvals, edit/delete/reply controls.
//
// Layout mirrors the web client (cloud/apps/web/src/app/(app)/claw/sonderr-chat/
...
export function MessageBubble(props: MessageBubbleProps) {
  ...
  return (
    <div class={`sonderrclaw-msg sonderrclaw-msg-${variant()}`}>
      <div class="sonderrclaw-msg-column">
        {/* Author label — above the bubble for bot messages only */}
        <Show when={showAuthor()}>
          <span class="sonderrclaw-msg-author">{props.assistantName ?? t("sonderrClaw.message.bot")}</span>
        </Show>

        {/* Reply preview — above the bubble, aligned with the author label */}
        <Show when={props.replyToMessage}>
          {(reply) => (
            <div class="sonderrclaw-msg-reply">
              <span class="sonderrclaw-msg-reply-arrow" aria-hidden="true">
                ↩
              </span>
              <Show
                when={!reply().deleted}
                fallback={<span class="sonderrclaw-msg-reply-deleted">{t("sonderrClaw.message.replyDeleted")}</span>}
              >
                <span class="sonderrclaw-msg-reply-text">
                  {(() => {
        ...
        <div class="sonderrclaw-msg-bubble-wrap">
          <Show when={!props.message.deleted && !isEditing() && !isDeleting() && !isOptimistic()}>
            <div class="sonderrclaw-msg-toolbar">
              <button
                type="button"
          ...
          <Show when={showReactionPick()}>
            <div class="sonderrclaw-msg-reactionpick">
              <For each={["👍", "❤️", "😂", "🎉", "🚀", "👀"]}>
                {(emoji) => (
                  <button
                    type="button"
                    class="sonderrclaw-msg-reactionpick-btn"
                    onClick={() => {
          ...
          <div class="sonderrclaw-msg-bubble">
            <Show
              when={!props.message.deleted}
              fallback={<span class="sonderrclaw-msg-deleted">{t("sonderrClaw.message.deleted")}</span>}
            >
              <Show when={isEditing()}>
                <div class="sonderrclaw-msg-edit">
                  <textarea
                    class="sonderrclaw-msg-edit-input"
                    value={editText()}
                    ...
                    autofocus
                  />
                  <div class="sonderrclaw-msg-edit-actions">
                    <button
                      type="button"
                      class="sonderrclaw-iconbtn-sm"
                      onClick={saveEdit}
              ...
              <Show when={isDeleting()}>
                <div class="sonderrclaw-msg-confirm-delete">
              ...
              <Show when={!isEditing() && !isDeleting()}>
                <div class="sonderrclaw-msg-body">
                  <Show
                    when={!empty()}
                    fallback={<span class="sonderrclaw-msg-thinking">{t("sonderrClaw.message.thinking")}</span>}
                  >
                    <Show when={isBot()} fallback={<span class="sonderrclaw-msg-text">{textContent()}</span>}>
                ...
                {(block) => {
                  ...
                  return (
                    <Show
                      when={!block.resolved}
                      fallback={
                        <div class="sonderrclaw-msg-actions-resolved">
                          <span class="sonderrclaw-msg-actions-resolved-icon">
                    ...
                    >
                      <div class="sonderrclaw-msg-actions">
                        <For each={block.actions}>
                          {(action) => (
                            <button
                              type="button"
                              class={`sonderrclaw-msg-action sonderrclaw-msg-action-${action.style}`}
            ...
            <Show when={!isEditing() && !isDeleting()}>
              <div class="sonderrclaw-msg-footer">
                <Show when={props.message.deliveryFailed}>
                  <span class="sonderrclaw-msg-failed">{t("sonderrClaw.message.notDelivered")}</span>
                </Show>
          ...
          <Show when={!props.message.deleted && props.message.reactions.length > 0}>
            <div class="sonderrclaw-msg-reactions">
              <For each={props.message.reactions}>
                {(r) => (
                  <button
                    type="button"
                    class="sonderrclaw-msg-reaction-pill"
