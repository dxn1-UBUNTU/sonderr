...
import { DataProvider, type OpenDiffFn, type OpenFileFn } from "@sonderr/sonderr-ui/context/data"
...
import type { UiI18nPluralKey } from "@sonderr/sonderr-ui/context"
...
import { SonderrEmbeddingModelsProvider } from "../context/sonderr-embedding-models"
...
const dict: Record<string, string> = { ...appEn, ...amEn, ...uiEn, ...sonderrEn }
...
// ---------------------------------------------------------------------------
// Mock providers — pre-loaded Sonderr Gateway model for stories
// ---------------------------------------------------------------------------
...
const MockProviderProvider: ParentComponent<{ sonderrAuth?: boolean; training?: boolean }> = (props) => {
  ...
  const value = {
    ...
    connected: () => ["sonderr"],
    defaults: () => ({}),
    defaultSelection: () => ({ providerID: "sonderr", modelID: "anthropic/claude-sonnet-4-6" }),
    models,
    findModel: (sel: any) => _findModel(models(), sel),
    authMethods: () => ({}),
    authStates: () => (props.sonderrAuth ? { sonderr: "oauth" } : {}) as Record<string, ProviderAuthState>,
    isModelValid: () => true,
  }
...
function mockNotificationsValue(items: SonderrNotification[] = []) {
  return {
    notifications: () => items,
...
export function mockSessionValue(overrides?: {
  ...
  return {
    ...
    selected: () => ({ providerID: "sonderr", modelID: "anthropic/claude-sonnet-4-6" }),
    modelForAgent: () => ({ providerID: "sonderr", modelID: "anthropic/claude-sonnet-4-6" }),
    selectModel: noop,
    costBreakdown: () => [],
...
interface StoryProvidersProps {
  ...
  questions?: QuestionRequest[]
  suggestions?: SuggestionRequest[]
  notifications?: SonderrNotification[]
  status?: string
  sessionID?: string
    ...
    <VSCodeProvider>
      <ServerProvider>
        <FeedbackProvider>
          ...
          >
            <DisplayProvider>
              <MockProviderProvider sonderrAuth={props.sonderrAuth} training={props.training}>
                <DialogProvider>
