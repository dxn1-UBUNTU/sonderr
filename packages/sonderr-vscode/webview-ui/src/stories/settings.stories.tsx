...
import { SonderrEmbeddingModelsContext } from "../context/sonderr-embedding-models"
...
export const ModelsSpeechToText: Story = {
  ...
  render: () => (
    <StoryProviders sonderrAuth config={{ experimental: { speech_to_text_model: "google/chirp-3" } } as any}>
      <div style={{ "max-height": "700px", overflow: "auto" }}>
        <ModelsTab />
      </div>
    </StoryProviders>
...
export const IndexingSonderrModelPreset: Story = {
  name: "IndexingTab - Sonderr stale custom model fallback",
  render: () => {
    const cfg: Config = {
      indexing: {
        provider: "sonderr",
    ...
    return (
      <StoryProviders config={cfg}>
        <SonderrEmbeddingModelsContext.Provider value={{ catalog: () => catalog }}>
          <div style={{ "max-height": "700px", overflow: "auto" }}>
            <IndexingTab />
          </div>
        </SonderrEmbeddingModelsContext.Provider>
      </StoryProviders>
    )
  },
}

export const IndexingSonderrCatalogLoading: Story = {
  name: "IndexingTab - Sonderr catalog loading",
  render: () => {
    const [saved, setSaved] = createSignal<Record<string, unknown>>({})
    const cfg: Config = {
      indexing: {},
    ...
    return (
      <>
        ...
        <pre data-testid="indexing-sonderr-loading-save">{JSON.stringify(saved(), null, 2)}</pre>
      </>
    )
  },
}
