...
export function convertToMentionPath(path: string, cwd: string): string {
  ...
  try {
    ...
    console.error("[Sonderr New] Failed to decode dropped URI:", err, cleaned)
  }
 ...
 */
export const SONDERR_FILE_PATH_MIME = "application/x-sonderr-file-path"

/**
 * Extract file paths from a drop's DataTransfer.
 * Checks (in order):
 * 1. Internal relative-path drag (application/x-sonderr-file-path)
...
export function extractDropPaths(dt: DataTransfer): string[] | null {
  ...
  const sonderr = dt.getData(SONDERR_FILE_PATH_MIME)
  if (sonderr) {
    const paths = sonderr.split(/\r?\n/).filter((line) => line.trim() !== "")
