...
const STORAGE_KEY = "sonderr.prompt-history.v1"
...
function load(): string[] {
  try {
    ...
    console.warn("[Sonderr New] prompt history load failed", err)
    return []
  }
}
...
function save(items: string[]) {
  try {
    ...
    console.warn("[Sonderr New] prompt history save failed", err)
  }
}
