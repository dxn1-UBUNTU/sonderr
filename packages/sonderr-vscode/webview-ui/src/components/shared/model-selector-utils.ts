...
import {
  SONDERR_PROVIDER_ID as SONDERR_GATEWAY_ID,
  PROVIDER_PRIORITY as PROVIDER_ORDER,
  providerOrderIndex,
} from "../../../../src/shared/provider-model"

export { SONDERR_GATEWAY_ID, PROVIDER_ORDER }

export const SONDERR_AUTO_SMALL_IDS = new Set(["sonderr-auto/small", "auto-small"])
...
export function isAuto(model: Pick<EnrichedModel, "providerID" | "id">): boolean {
  return (
    model.providerID === SONDERR_GATEWAY_ID && (model.id.startsWith("sonderr-auto/") || SONDERR_AUTO_SMALL_IDS.has(model.id))
  )
}
...
export function isSmall(model: Pick<EnrichedModel, "providerID" | "id">): boolean {
  return model.providerID === SONDERR_GATEWAY_ID && SONDERR_AUTO_SMALL_IDS.has(model.id)
}
...
// elsewhere, so preserve bare trailing "Free" words (e.g. "Sonderr Auto Free").
...
export function stripSubProviderPrefix(name: string): string {
  ...
  if (prefix.toLowerCase() === SONDERR_GATEWAY_ID) return name
...
export function buildTriggerLabel(
  ...
  if (resolvedName) {
    if (providerID === SONDERR_GATEWAY_ID) return stripSubProviderPrefix(resolvedName)
    return resolvedName
  }
  if (raw?.providerID && raw?.modelID) {
    return raw.providerID === SONDERR_GATEWAY_ID ? raw.modelID : `${raw.providerID} / ${raw.modelID}`
  }
