...
import { Card } from "@sonderr/sonderr-ui/card"
import { DEFAULT_VECTOR_STORE, isFileExtension, parseFileExtensions } from "@sonderr/sonderr-indexing/config"
import { formatSonderrEmbeddingModelLabel, getSonderrEmbeddingModel } from "@sonderr/sonderr-indexing/embedding-models"
import { Select } from "@sonderr/sonderr-ui/select"
import { Switch } from "@sonderr/sonderr-ui/switch"
import { TextField } from "@sonderr/sonderr-ui/text-field"
import { useConfig } from "../../context/config"
import { formatIndexingLabel, useIndexing } from "../../context/indexing"
import { useSonderrEmbeddingModels } from "../../context/sonderr-embedding-models"
...
const IndexingTab: Component = () => {
  ...
  const embeds = useSonderrEmbeddingModels()
  ...
  const sonderrDefault = () =>
    getSonderrEmbeddingModel(embeds.catalog().defaultModel, embeds.catalog())?.id ?? embeds.catalog().defaultModel
  const sonderrModels = createMemo(() =>
    embeds.catalog().models.map((model) => ({
      value: model.id,
      label: formatSonderrEmbeddingModelLabel(model),
    })),
  )
  const knownSonderrModel = (model: string | null | undefined) =>
    getSonderrEmbeddingModel(model ?? undefined, embeds.catalog())?.id
  const sonderrValue = () => knownSonderrModel(cfg().model) ?? sonderrDefault()
  const sonderrAvailable = () => !!server.profileData() || provider.authStates()[SONDERR_PROVIDER_ID] !== undefined
  const selectedProvider = () => cfg().provider ?? (sonderrAvailable() ? "sonderr" : undefined)
  const staleSonderrModel = () => selectedProvider() === "sonderr" && !!cfg().model && !knownSonderrModel(cfg().model)
  const providers = createMemo(() =>
    allProviders.filter((item) => item.value !== "sonderr" || sonderrAvailable() || selectedProvider() === "sonderr"),
  )
  const fields = createMemo(() => providerFields(selectedProvider()))

  const saveProvider = (next: ProviderId | undefined) => {
    if (next === "sonderr") {
      const model = knownSonderrModel(cfg().model) ?? (sonderrDefault() || null)
      updateIndexing({
        provider: next,
        model,
        dimension: null,
  ...
  const saveEnabled = (next: boolean) => {
    if (next && !cfg().provider && sonderrAvailable()) {
      updateIndexing({
        enabled: next,
        provider: "sonderr",
        model: knownSonderrModel(cfg().model) ?? (sonderrDefault() || null),
        dimension: null,
      })
      return
    }
  ...
  const saveModel = (value: string) => {
    if (selectedProvider() === "sonderr") return
  ...
  const content = (_scope: IndexingScope) => (
    <div style={{ display: "flex", "flex-direction": "column", gap: "16px" }}>
      ...
      <Card>
        ...
        <Show when={selectedProvider() === "sonderr"}>
          <Show when={sonderrModels().length > 0}>
            <SettingsRow
              title={language.t("settings.indexing.sonderrModel.title")}
              description={description(language.t("settings.indexing.sonderrModel.description"), [["model"]])}
              tag={() => tag(scope(), [["model"]])}
            >
              <Select
                options={sonderrModels()}
                current={sonderrModels().find((item) => item.value === sonderrValue())}
                value={(item) => item.value}
                label={(item) => item.label}
                onSelect={(item) => updateIndexing({ model: item?.value ?? sonderrDefault(), dimension: null })}
                variant="secondary"
                size="small"
                triggerVariant="settings"
        ...
        <SettingsRow
          ...
          tag={() => (selectedProvider() === "sonderr" ? undefined : tag(scope(), [["dimension"]]))}
          last={!selectedProvider() || (fields().length === 0 && !(selectedProvider() === "sonderr" && !sonderrAvailable()))}
        >
          <TextField
            value={
              staleSonderrModel() || cfg().dimension === undefined || cfg().dimension === null
                ? ""
                : String(cfg().dimension)
            }
            placeholder={
              selectedProvider() === "sonderr" ? "Provided by Sonderr" : language.t("settings.indexing.dimension.placeholder")
            }
            disabled={selectedProvider() === "sonderr"}
            onChange={(value) => saveNumber("dimension", value, { integer: true, min: 1 })}
          />
        </SettingsRow>
        <Show when={selectedProvider() === "sonderr" && !sonderrAvailable()}>
          <SettingsRow
            title={language.t("settings.indexing.sonderrSignIn.title")}
            description={language.t("settings.indexing.sonderrSignIn.description")}
            last
          >
            <span />
          </SettingsRow>
