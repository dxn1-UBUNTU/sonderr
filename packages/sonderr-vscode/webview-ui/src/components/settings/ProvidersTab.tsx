...
import { Collapsible } from "@sonderr/sonderr-ui/collapsible"
...
import { disabledProviderOptions, providersWithSonderrFallback, visibleConnectedIds } from "./provider-visibility"
import { isCustomProviderPackage, SONDERR_PROVIDER_ID } from "../../../../src/shared/provider-model"
...
const ProvidersTab: Component = () => {
  ...
  const sonderrLoggedIn = createMemo(() => !!provider.authStates()[SONDERR_PROVIDER_ID])

  const connectedProviders = createMemo(() => {
    const ids = visibleConnectedIds(provider.connected(), provider.authStates())
    const all = provider.providers()
    return ids
      .filter((id) => id !== SONDERR_PROVIDER_ID)
      .map((id) => all[id])
  ...
  const popularProviders = createMemo(() => {
    ...
    return sortProviders(
      all.filter(
        (item) =>
          item.id !== SONDERR_PROVIDER_ID && isPopularProvider(item) && !connected.has(item.id) && !disabled.has(item.id),
      ),
    )
  })
  ...
  const providers = createMemo(() => providersWithSonderrFallback(provider.providers()))
  ...
  function connectProvider(item: Provider) {
    if (item.id === SONDERR_PROVIDER_ID) {
      // Route Sonderr Gateway sign-in through the Profile view so the user sees
  ...
  return (
    <div>
      <Show when={!disabledIds().has(SONDERR_PROVIDER_ID)}>
        {/* Sonderr Gateway — always at the top, not editable */}
        <Card>
          <div
            style={{
              display: "flex",
          ...
          >
            <ProviderIcon id={providerIcon(SONDERR_PROVIDER_ID)} width={20} height={20} />
            <span
              style={{
      ...
      {/* Connected providers (excluding Sonderr) */}
