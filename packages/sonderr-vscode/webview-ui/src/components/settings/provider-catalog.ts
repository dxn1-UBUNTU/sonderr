import { iconNames, type IconName } from "@sonderr/ui/icons/provider"
import type { Provider } from "../../types/messages"
import {
  SONDERR_PROVIDER_ID,
  PROVIDER_PRIORITY as FALLBACK_PROVIDER_IDS,
  createSonderrFallbackProvider,
  providerOrderIndex,
...
export function providerIcon(provider: Provider | string): IconName {
  ...
  if (icon) return icon
  if (providerID === SONDERR_PROVIDER_ID) return validIcon("sonderr") ?? "synthetic"
  const fallback = validIcon(providerID)
  if (fallback) return fallback
  return "synthetic"
}

export function sonderrFallbackProvider(): Provider {
  return createSonderrFallbackProvider()
}
...
export function providerNoteKey(provider: Provider | string) {
  ...
  if (provider === SONDERR_PROVIDER_ID) return "settings.providers.note.sonderr"
  return undefined
}
