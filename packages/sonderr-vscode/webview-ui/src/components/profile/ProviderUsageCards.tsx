...
import type { SonderrPassState, ProviderUsageData } from "../../types/messages"
import type { ProviderUsageSnapshot } from "@sonderr/sdk/v2/client"
import { Button } from "@sonderr/sonderr-ui/button"
import { Card, CardActions, CardDescription, CardHeader, CardTitle } from "@sonderr/sonderr-ui/card"
import { SonderrPassMeter } from "@sonderr/sonderr-ui/sonderr-pass-meter"
import { Progress } from "@sonderr/sonderr-ui/progress"
...
import { formatWindow, windowLabel, windowProgress } from "@sonderr/sonderr-gateway/provider-usage"
...
const order = (items: ProviderUsageSnapshot[]) =>
  [...items].sort(
    (left, right) => Number(left.sourceKind !== "sonderr_managed") - Number(right.sourceKind !== "sonderr_managed"),
  )
...
const SonderrPassCard: Component<{
  pass?: SonderrPassState | null
  onGet: () => void
  language: Language
}> = (props) => {
  const renewal = () => {
    ...
    <Card>
      <CardHeader>
        <div>
          ...
          <CardDescription>Sonderr Pass</CardDescription>
        </div>
        <Tag>Sonderr Gateway</Tag>
      </CardHeader>
      <Show
        when={props.pass}
        fallback={
  ...
  return (
    <section class="provider-usage-section" aria-labelledby="provider-usage-title">
      ...
      <div class="provider-usage-list">
        ...
        <Show when={props.showSonderrPass}>
          <SonderrPassCard pass={props.sonderrPass} onGet={props.onGetSonderrPass} language={language} />
        </Show>
        <Show
          when={props.data}
          fallback={
