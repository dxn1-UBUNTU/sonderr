...
export const InstallModal = (props: Props) => {
  ...
  const destination = (target = scope().value) => {
    const base = target === "project" ? ".sonderr" : "~/.config/sonderr"
    if (props.item.type === "mcp") return `${base}/sonderr.json`
    if (props.item.type === "agent") return `${base}/agents/${props.item.id}.md`
    if (target === "project") return `.sonderr/skills/${props.item.id}/`
    return `~/.sonderr/skills/${props.item.id}/`
  }
