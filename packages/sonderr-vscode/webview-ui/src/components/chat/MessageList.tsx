...
import { createAutoScroll } from "@sonderr/sonderr-ui/hooks"
import { useSession } from "../../context/session"
import { useServer } from "../../context/server"
import { useVSCode } from "../../context/vscode"
import { useLanguage } from "../../context/language"
import { useI18n } from "@sonderr/sonderr-ui/context/i18n"
...
import { SonderrNotifications } from "./SonderrNotifications"
...
import { useData } from "@sonderr/sonderr-ui/context/data"
import { getDirectory as getRawDirectory, getFilename } from "@sonderr/core/util/path"
import {
  partitionRows,
  retainTurn,
  transcriptRows,
...
export const MessageList: Component<MessageListProps> = (props) => {
  ...
  // back to sonderr-ui's default hideDetails renderer, which never shows a
  ...
  // Extracts only the text sonderr-ui's tool renderers actually put on screen —
  ...
  //
  // read/glob/grep/list are the one confirmed exception: sonderr-ui always
  ...
  // packages/cli/src/tool/todo.ts sets metadata.todos to the exact
  ...
  // see packages/cli/src/sonderr/todo-view.ts). Recursively collecting
  ...
  // registers for "task", overriding sonderr-ui's default) exactly: title is
  ...
  function taskText(part: Part & { type: "tool" }, state: ToolState): string[] {
    ...
    // sonderr-ui's default hideDetails task card, which never shows result
  ...
  function questionText(part: Part & { type: "tool" }, state: ToolState): string[] {
    ...
    questions.forEach((q, i) => {
      ...
      // values. Once completed, sonderr-ui's question renderer instead shows
    ...
    <div class="message-list-container">
      ...
      <Show when={isEmpty()}>
        <div class="welcome-header">
          ...
          <SonderrNotifications sessionID={props.sessionID} />
        </div>
      </Show>
      <div
        ref={setScrollRef}
        ...
        <div ref={autoScroll.contentRef} class={isEmpty() ? "message-list-content-empty" : "message-list-content"}>
          ...
          <Show when={isEmpty() && props.readonly}>
            <div class="message-list-empty">
              <p class="sonderr-about-text">{language.t("session.messages.initializing")}</p>
            </div>
          </Show>
          <Show when={isEmpty() && !props.readonly}>
            {props.emptyState ? (
