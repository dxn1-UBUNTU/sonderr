...
import { DataProvider } from "@sonderr/sonderr-ui/context/data"
...
import type { Message as SDKMessage, Part as SDKPart } from "@sonderr/sdk/v2"
...
export const DataBridge: Component<{ children: any }> = (props) => {
  ...
  const open = (filePath: string, line?: number, column?: number, sessionID?: string) => {
    const event = new CustomEvent("sonderr:open-file", {
      cancelable: true,
  ...
  return (
    <DataProvider
      data={data}
      directory={directory()}
      // @ts-expect-error — onPermissionRespond/onQuestion* are extension-specific props not yet in sonderr-ui's DataProvider types
...
const AppContent: Component = () => {
  ...
  const handleSonderrModel = (message: { type?: string }) => {
    if (message.type === "selectSonderrModel") setCurrentView("newTask")
  }
  ...
  onMount(() => {
    const handler = (event: MessageEvent) => {
      ...
      if (message?.type === "action" && message.action) {
        console.log("[Sonderr New] App: 🎬 action:", message.action)
        handleViewAction(message.action)
      }
      if (message?.type === "navigate" && message.view && VALID_VIEWS.has(message.view)) {
        console.log("[Sonderr New] App: 🧭 navigate:", message.view, message.tab ? `tab=${message.tab}` : "")
      ...
      if (message?.type === "openCloudSession" && message.sessionId) {
        console.log("[Sonderr New] App: ☁️ openCloudSession:", message.sessionId)
        session.selectCloudSession(message.sessionId)
        setCurrentView("newTask")
      }
      handleSonderrModel(message)
      handleForked(message)
      if (message?.type === "viewSubAgentSession" && message.sessionID) {
        console.log("[Sonderr New] App: 🔍 viewSubAgentSession:", message.sessionID)
      ...
      if (message?.type === "migrationState") {
        console.log("[Sonderr New] App: 🔄 migrationState:", message.needed)
  ...
  // Set synchronously in the webview HTML by SonderrProvider so it's available
  ...
  const host = window as {
    SONDERR_TOP_BAR?: boolean
    SONDERR_TOP_BAR_SURFACE?: string
    SONDERR_AGENT_MANAGER_SETTINGS?: boolean
  }
  const showTopBar = host.SONDERR_TOP_BAR !== false
  const topBarSurface = host.SONDERR_TOP_BAR_SURFACE ?? "sidebar_title"
          ...
          >
            ...
            <Match when={currentView() === "settings"}>
              <Settings
                ...
                agentManagerSettings={host.SONDERR_AGENT_MANAGER_SETTINGS === true}
