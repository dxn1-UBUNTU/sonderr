 ...
 * Sonderr gateway models remain usable whenever the provider catalog exposes them.
 */
export function isModelValid(
  ...
  if (selection.providerID !== "sonderr" && !connected.includes(selection.providerID)) return false
