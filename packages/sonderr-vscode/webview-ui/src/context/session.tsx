...
import { SONDERR_AUTO, SONDERR_PROVIDER_ID, parseModelString } from "../../../src/shared/provider-model"
...
export const SessionProvider: ParentComponent = (props) => {
  ...
  const [pendingSonderrModel, setPendingSonderrModel] = createSignal<{
    modelID?: string
    agent?: string
    after: number
  } | null>(null)
  ...
  function selectSonderrModel(modelID?: string, agent?: string) {
    if (!modelID && !agent) return
    setPendingSonderrModel({ ...(modelID && { modelID }), ...(agent && { agent }), after: catalog() })
    if (modelID) vscode.postMessage({ type: "requestProviders" })
  }

  const unsubSonderrModel = vscode.onMessage((message: ExtensionMessage) => {
    if (message.type === "providersLoaded") {
      setCatalog((value) => value + 1)
      return
    }
    if (message.type === "selectSonderrModel") selectSonderrModel(message.modelID, message.agent)
  })
  onCleanup(unsubSonderrModel)

  createEffect(() => {
    const pending = pendingSonderrModel()
    if (!pending || agents().length === 0 || (pending.modelID && catalog() <= pending.after)) return
    setPendingSonderrModel(null)
    if (pending.modelID && !provider.providers()[SONDERR_PROVIDER_ID]?.models[pending.modelID]) {
      console.warn("[Sonderr New] Ignoring unavailable Sonderr catalog model:", pending.modelID)
      return
    }
    if (pending.agent && !agentNames().has(pending.agent)) {
      console.warn("[Sonderr New] Ignoring unavailable Sonderr agent:", pending.agent)
      return
    }
    if (pending.agent) selectAgent(pending.agent)
    if (pending.modelID) selectModel(SONDERR_PROVIDER_ID, pending.modelID)
  })
  ...
  function handleExtensionMessage(message: ExtensionMessage): void {
    ...
    switch (message.type) {
      ...
      case "cloudSessionImportFailed": {
        ...
        console.error("[Sonderr New] Cloud session import failed:", message.error)
        break
      }
  ...
  function handlePartUpdated(
    ...
    if (!effectiveMessageID) {
      console.warn("[Sonderr New] Part updated without messageID:", part.id, part.type)
      return
    }
  ...
  function sendMessage(
    ...
    if (!server.isConnected()) {
      console.warn("[Sonderr New] Cannot send message: not connected")
      return
    }
  ...
  function sendCommand(
    ...
    if (!server.isConnected()) {
      console.warn("[Sonderr New] Cannot send command: not connected")
      return
    }
  ...
  function abort() {
    ...
    if (!scope) {
      console.warn("[Sonderr New] Cannot abort: no current or pending session")
      return
    }
  ...
  function compact() {
    if (!server.isConnected()) {
      console.warn("[Sonderr New] Cannot compact: not connected")
      return
    }

    const sessionID = currentSessionID()
    if (!sessionID) {
      console.warn("[Sonderr New] Cannot compact: no current session")
      return
    }
  ...
  function createSession() {
    if (!server.isConnected()) {
      console.warn("[Sonderr New] Cannot create session: not connected")
      return
    }
  ...
  function loadSessions() {
    if (!server.isConnected()) {
      console.warn("[Sonderr New] Cannot load sessions: not connected")
      return
    }
  ...
  function selectSession(id: string, options: { focus?: boolean } = {}) {
    ...
    if (id.startsWith("cloud:")) {
      console.warn("[Sonderr New] Cannot select cloud preview session via selectSession")
      return
    }
  ...
  function selectCloudSession(cloudSessionId: string) {
    if (!server.isConnected()) {
      console.warn("[Sonderr New] Cannot select cloud session: not connected")
      return
    }
    const key = `cloud:${cloudSessionId}`
    agentDrafts.prune(draftSessionID())
  ...
  function deleteSession(id: string) {
    if (!server.isConnected()) {
      console.warn("[Sonderr New] Cannot delete session: not connected")
      return
    }
  ...
  function renameSession(id: string, title: string) {
    if (!server.isConnected()) {
      console.warn("[Sonderr New] Cannot rename session: not connected")
      return
    }
  ...
  function exportSessionTranscript(id: string) {
    if (!server.isConnected()) {
      console.warn("[Sonderr New] Cannot export session transcript: not connected")
      return
    }
    if (id.startsWith("cloud:")) {
      console.warn("[Sonderr New] Cannot export cloud session transcript")
      return
    }
