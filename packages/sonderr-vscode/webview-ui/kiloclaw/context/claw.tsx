...
export function ClawProvider(props: { children: JSX.Element }) {
  ...
  const applyState = (s: SonderrClawState) => {
    setPhase(s.phase)
    setLocale(s.locale)
    if (s.phase === "ready") {
      setStatus(s.status)
  ...
  const handleConversationMessage = (msg: SonderrClawOutMessage, active: string | null): boolean => {
    switch (msg.type) {
      case "sonderrclaw.messages":
        if (msg.conversationId === active) {
          setMessages(msg.messages)
          setHasMoreMessages(msg.hasMore)
        }
        return true
      case "sonderrclaw.messageOptimistic":
        if (msg.conversationId === active) {
          setMessages((prev) => (prev.some((m) => m.id === msg.message.id) ? prev : [...prev, msg.message]))
        }
        return true
      case "sonderrclaw.messageReplaced":
        if (msg.conversationId === active) {
  ...
  const handler = (event: MessageEvent) => {
    const msg = event.data as SonderrClawOutMessage
    if (msg?.type === "fontSizeChanged") {
      applyFontSize(msg.fontSize)
      return
    }
    if (!msg?.type?.startsWith("sonderrclaw.")) return
    const active = activeConversationId()
    if (handleConversationMessage(msg, active)) return

    switch (msg.type) {
      case "sonderrclaw.state":
        applyState(msg.state)
        return
      case "sonderrclaw.status":
        setStatus(msg.data)
        return
      case "sonderrclaw.locale":
        setLocale(msg.locale)
        return
      case "sonderrclaw.error":
        showToast({ title: msg.error, variant: "error", duration: 5000 })
        return
      case "sonderrclaw.conversations":
        setConversations(msg.conversations)
        setHasMoreConversations(msg.hasMore)
        return
      case "sonderrclaw.activeConversation":
        setActiveConversationId(msg.conversationId)
        setTypingMembers([])
        return
      case "sonderrclaw.botStatus":
        setBotStatus(msg.status)
        return
      case "sonderrclaw.conversationStatus":
  ...
  onMount(() => {
    vscode.postMessage({ type: "sonderrclaw.ready" })
  })
  ...
  const ctx: ClawCtx = {
    ...
    locale,
    error,
    retry: () => vscode.postMessage({ type: "sonderrclaw.ready" }),
    ...
    selectConversation: (conversationId) => vscode.postMessage({ type: "sonderrclaw.selectConversation", conversationId }),
    createConversation: (title) => vscode.postMessage({ type: "sonderrclaw.createConversation", title }),
    renameConversation: (conversationId, title) =>
      vscode.postMessage({ type: "sonderrclaw.renameConversation", conversationId, title }),
    leaveConversation: (conversationId) => vscode.postMessage({ type: "sonderrclaw.leaveConversation", conversationId }),
    loadMoreConversations: () => vscode.postMessage({ type: "sonderrclaw.loadMoreConversations" }),
    ...
    editMessage: (conversationId, messageId, content) =>
      vscode.postMessage({ type: "sonderrclaw.editMessage", conversationId, messageId, content }),
    deleteMessage: (conversationId, messageId) =>
      vscode.postMessage({ type: "sonderrclaw.deleteMessage", conversationId, messageId }),
    loadMoreMessages: (conversationId, before) =>
      vscode.postMessage({ type: "sonderrclaw.loadMoreMessages", conversationId, before }),

    addReaction: (conversationId, messageId, emoji) =>
      vscode.postMessage({ type: "sonderrclaw.addReaction", conversationId, messageId, emoji }),
    removeReaction: (conversationId, messageId, emoji) =>
      vscode.postMessage({ type: "sonderrclaw.removeReaction", conversationId, messageId, emoji }),

    executeAction: (conversationId, messageId, groupId, value) =>
      vscode.postMessage({ type: "sonderrclaw.executeAction", conversationId, messageId, groupId, value }),

    sendTyping: (conversationId) => vscode.postMessage({ type: "sonderrclaw.sendTyping", conversationId }),
    sendTypingStop: (conversationId) => vscode.postMessage({ type: "sonderrclaw.sendTypingStop", conversationId }),
    ...
    botStatus,
    conversationStatus,
    markConversationRead: (conversationId) => vscode.postMessage({ type: "sonderrclaw.markRead", conversationId }),

    openExternal: (url) => vscode.postMessage({ type: "sonderrclaw.openExternal", url }),
  }
