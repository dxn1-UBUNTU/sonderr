// SonderrClaw active-conversation view — message list + composer.
// Mirrors cloud/apps/web/src/app/(app)/claw/sonderr-chat/components/MessageArea.tsx
...
export function MessageArea() {
  ...
  //
  // Uses @sonderr/sonderr-ui's createAutoScroll (the same one MessageList.tsx
  // uses) because it tracks real user-input events — wheel/pointer/key/touch —
  // via `markUser`. The older @sonderr/ui hook only listens to wheel
  ...
  // `bot:sonderrclaw:{sandboxId}` — the bot is the only non-self member in
  // 1:1 chats, so we resolve bot ids to the user-configured `botName`
  // (falls back to the default "SonderrClaw" label) and render any human
  ...
  return (
    <Show
      ...
      fallback={
        <div class="sonderrclaw-empty-area">
          <p>{t("sonderrClaw.conversations.selectOne")}</p>
        </div>
      }
    >
      <div class="sonderrclaw-area">
        {/* Messages */}
        <div class="sonderrclaw-area-messages-wrap">
          <div
            class="sonderrclaw-area-messages"
            ref={(el) => {
              scrollEl = el
              auto.scrollRef(el)
            }}
        ...
        <Show when={replyingTo()}>
          {(r) => (
            <div class="sonderrclaw-reply-preview">
              <span class="sonderrclaw-reply-preview-label">{t("sonderrClaw.message.replyTo")}</span>
              <span class="sonderrclaw-reply-preview-text">{replyText(r())}</span>
              <button
                type="button"
        ...
        <div class="sonderrclaw-input-wrap">
          <textarea
            ref={input}
            class="sonderrclaw-input"
