// Sonderr-specific replacement for the shared `@sonderr/ui/pierre/worker`
...
import { LINE_DIFF_TYPE } from "@sonderr/sonderr-ui/pierre"
import { WorkerPoolManager } from "@pierre/diffs/worker"
import { ensureSonderrDiffTheme, SONDERR_DIFF_THEME } from "@sonderr/ui/pierre/sonderr-diff-theme"

// Register the "Sonderr" theme before any pool initializes. resolveThemes([theme])
// runs on the main thread during initialize() and throws "resolveTheme: No valid
// loader for Sonderr" if the theme name was never registered. Registering here makes
...
function uri(): string | undefined {
  ...
  return (window as { SONDERR_SHIKI_WORKER_URI?: string }).SONDERR_SHIKI_WORKER_URI
}

export function workerFactory(): Worker {
  const url = uri()
  if (!url) throw new Error("SONDERR_SHIKI_WORKER_URI is not set")
...
function createPool() {
  const pool = new WorkerPoolManager(
    ...
    { theme: SONDERR_DIFF_THEME, lineDiffType: LINE_DIFF_TYPE, preferredHighlighter: ENGINE },
  )
  void pool.initialize().catch((err) => console.warn("[Sonderr New] Failed to initialize Pierre worker pool", err))
  return pool
}
