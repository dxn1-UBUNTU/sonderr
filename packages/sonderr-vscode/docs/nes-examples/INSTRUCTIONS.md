...
1. **`bun run watch`** is already running for the sonderr extension.
2. In the sonderr VSCode window, press **F5** → opens the **Extension Development Host**.
3. In the Dev Host: `File → Open Folder…` → `packages/sonderr-vscode/docs/nes-examples/` inside this repo.
4. Open Settings (`Cmd+,`), confirm:
   - `sonderr-code.new.autocomplete.enableAutoTrigger` → ✓ (default true)
   - `sonderr-code.new.autocomplete.model` → **Mercury Next Edit (Inception)** ← *NOT* "Mercury Edit 2", which is the classic FIM option
   - `sonderr-code.new.autocomplete.nextEdit.apiKey` → your `sk_...` key
   - VSCode global `editor.inlineSuggest.enabled` → ✓
5. To watch the pipeline live: `View → Output` → pick the **"Sonderr · Next Edit"** channel.
...
- **No log lines appearing**: confirm the output channel is "Sonderr · Next Edit". Also confirm you reloaded the Dev Host after rebuilding.
