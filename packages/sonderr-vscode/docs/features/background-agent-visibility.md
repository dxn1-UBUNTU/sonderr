...
The CLI already solves this. It keeps a per-session subagent list with status (`running`, `done`, `cancelled`, `error`), shows a `subagents` hint in the status line, and opens a subagent inspector tab. See `packages/cli/src/cli/cmd/run/subagent-data.ts` and `footer.command.tsx`.
...
- Sidebar and Sonderr tab: a dedicated editor tab, reached with the `openSubAgentViewer` message.
...
- `TaskHeader` is shared through `ChatView`, so the sidebar, the Sonderr tab, and Agent Manager all get it.
...
The webview uses the Sonderr-owned background-job API for authoritative lifecycle state. A task-part fallback keeps running agents visible while the first request is loading:
...
| lifecycle state | `GET /sonderr/background-jobs` |
| per-agent cancellation | `POST /sonderr/background-jobs/:jobID/cancel`, resolved in the owning parent directory and cancelling the child session tree |
...
| child sessions stay tracked while the card is closed | `SonderrProvider` auto-adopts them from task parts |
...
1. `packages/cli/src/sonderr/server/httpapi/`
   Sonderr-owned parent-scoped background-job list and cancellation routes.
