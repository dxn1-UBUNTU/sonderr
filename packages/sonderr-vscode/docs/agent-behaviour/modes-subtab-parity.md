...
| Custom Instructions (per-mode) | `<TextArea>` (10 rows) + link to `.sonderr/rules-{slug}/rules.md` | Not available as separate field (merged into system prompt) |
...
| Override System Prompt | Advanced disclosure with link to `.sonderr/system-prompt-{slug}` | Not available |
...
| Link to global rules file | Link to `.sonderr/rules/rules.md` | Not available |

### Organization Features

| Feature | Legacy | New |
|---|---|---|
| Share modes banner | `SonderrShareModesBanner` component | Not available |
