...
const GLOBALS = "colorScheme:dark;theme:sonderr-vscode;vscodeTheme:dark-modern"
const STORY_ID = "settings--indexing-provider-blur-race"
const SONDERR_STORY_ID = "settings--indexing-sonderr-model-preset"
const SONDERR_LOADING_STORY_ID = "settings--indexing-sonderr-catalog-loading"
...
test("Sonderr exposes only supported embedding model presets", async ({ page }) => {
  await page.setViewportSize({ width: 420, height: 720 })
  await page.goto(storyUrl(SONDERR_STORY_ID), { waitUntil: "load" })
  await disableAnimations(page)
  await page.waitForSelector("#storybook-root *", { state: "attached" })

  await expect(page.getByText("Sonderr model preset", { exact: true })).toBeVisible()
  ...
  const preset = selectIn(page, "Sonderr model preset")
...
test("enabling Sonderr before its catalog loads does not store an empty model", async ({ page }) => {
  const saved = page.getByTestId("indexing-sonderr-loading-save")
  const cfg = async () => JSON.parse(((await saved.textContent()) ?? "{}").trim()) as Saved
  const verify = async () => {
    await expect.poll(async () => (await cfg()).provider).toBe("sonderr")
    expect((await cfg()).model).toBeNull()
    expect((await cfg()).dimension).toBeNull()
  }

  await page.setViewportSize({ width: 420, height: 720 })
  await page.goto(storyUrl(SONDERR_LOADING_STORY_ID), { waitUntil: "load" })
  await disableAnimations(page)
  await page.waitForSelector("#storybook-root *", { state: "attached" })
  await page.getByRole("button", { name: "Local", exact: true }).click()
  await page
