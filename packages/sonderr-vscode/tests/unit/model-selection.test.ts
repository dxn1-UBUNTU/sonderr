...
import { SONDERR_AUTO, parseModelString } from "../../src/shared/provider-model"
...
const providers = {
  sonderr: makeProvider("sonderr", "Sonderr Gateway", ["sonderr-auto/free"]),
...
describe("parseModelString", () => {
  ...
  it("keeps slashes inside sonderr model ids", () => {
    expect(parseModelString("sonderr/sonderr-auto/free")).toEqual({
      providerID: "sonderr",
      modelID: "sonderr-auto/free",
    })
  })
...
describe("resolveModelSelection", () => {
  ...
  it("uses sonderr auto as the explicit final fallback", () => {
    const result = resolveModelSelection({
      providers,
      connected: [],
      fallback: SONDERR_AUTO,
    })
    expect(result).toEqual(SONDERR_AUTO)
  })

  it("keeps the explicit fallback even when sonderr is missing from the loaded catalog", () => {
    const result = resolveModelSelection({
