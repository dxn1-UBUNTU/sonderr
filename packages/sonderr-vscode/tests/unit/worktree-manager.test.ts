...
async function createTempRepo(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "sonderr-wt-"))
  tempDirs.push(dir)
...
async function createTempRepoWithOrigin(): Promise<{ bare: string; clone: string }> {
  const bare = await fs.mkdtemp(path.join(os.tmpdir(), "sonderr-wt-bare-"))
  const clone = await fs.mkdtemp(path.join(os.tmpdir(), "sonderr-wt-clone-"))
...
describe("WorktreeStateManager.updateWorktreeLabel", () => {
  it("persists label on a worktree", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "sonderr-wt-label-"))
    tempDirs.push(dir)
...
describe("WorktreeManager.createWorktree", () => {
  it("uses a configured Git executable for worktree creation", async () => {
    ...
    const fake = await fs.mkdtemp(path.join(os.tmpdir(), "sonderr-wt-no-git-"))
    tempDirs.push(fake)
    ...
    const bin =
      process.platform === "win32"
        ? real
        : path.join(await fs.mkdtemp(path.join(os.tmpdir(), "sonderr-git executable-")), "git")
    ...
    env.SONDERR_TEST_ROOT = root
    env.SONDERR_TEST_GIT = bin
    ...
    const script = `
      ...
      const root = process.env.SONDERR_TEST_ROOT
      const git = process.env.SONDERR_TEST_GIT
    ...
    expect(existsSync(path.join(root, ".sonderr", "worktrees", "configured-git", ".git"))).toBe(true)
  }, 120_000)
  ...
  it("throws when workspace is not a git repo", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "sonderr-wt-nogit-"))
    tempDirs.push(dir)
  ...
  it("creates worktrees directory under .sonderr/worktrees/", async () => {
    const root = await createTempRepo()
    const mgr = createManager(root)

    const result = await mgr.createWorktree({ prompt: "test" })

    expect(result.path).toContain(path.join(".sonderr", "worktrees"))
  })
...
describe("WorktreeManager.removeWorktree", () => {
  ...
  it("does not throw when worktree path does not exist", async () => {
    ...
    // Should not throw
    await mgr.removeWorktree(path.join(root, ".sonderr", "worktrees", "nonexistent"))
  })
  ...
  it("removes orphaned directory that git does not know about", async () => {
    ...
    const orphanPath = path.join(root, ".sonderr", "worktrees", "orphan")
  ...
  it(
    ...
    async () => {
      ...
      const worktreesDir = path.join(root, ".sonderr", "worktrees")
      const deadline = Date.now() + 5000
      while (Date.now() < deadline) {
        const entries = await fs.readdir(worktreesDir)
        if (!entries.some((e) => e.startsWith(".sonderr-delete-"))) break
        await new Promise((r) => setTimeout(r, 100))
      }

      // No .sonderr-delete-* temp dirs should remain
      const entries = await fs.readdir(worktreesDir)
      const orphans = entries.filter((e) => e.startsWith(".sonderr-delete-"))
...
describe("WorktreeManager.discoverWorktrees orphan cleanup", () => {
  it("cleans up .sonderr-delete-* dirs left by interrupted deletions", async () => {
    const root = await createTempRepo()
    const mgr = createManager(root)
    ...
    const orphan = path.join(root, ".sonderr", "worktrees", ".sonderr-delete-fake-uuid")
...
describe("WorktreeManager metadata", () => {
  ...
  it("writes metadata outside the worktree checkout", async () => {
    ...
    expect(existsSync(path.join(result.path, ".sonderr", "session-id"))).toBe(false)
    expect(existsSync(path.join(result.path, ".sonderr", "metadata.json"))).toBe(false)
  ...
  it("reads legacy session-id file when metadata.json is missing", async () => {
    ...
    const dir = path.join(result.path, ".sonderr")
...
describe("WorktreeManager.discoverWorktrees", () => {
  ...
  it("repairs stale gitdir refs when .sonderr/worktrees already exists", async () => {
    const root = await createTempRepo()
    const mgr = createManager(root)

    const worktree = path.join(root, ".sonderr", "worktrees", "partial")
    const gitdir = path.join(root, ".git", "worktrees", "partial", "gitdir")
    await fs.mkdir(worktree, { recursive: true })
    await fs.mkdir(path.dirname(gitdir), { recursive: true })
    await fs.writeFile(gitdir, path.join(root, ".sonderr", "worktrees", "partial", ".git"), "utf-8")

    await mgr.discoverWorktrees()

    const fixed = await fs.readFile(gitdir, "utf-8")
    expect(fixed).toContain(path.join(root, ".sonderr", "worktrees", "partial", ".git"))
  })
})
...
describe("WorktreeManager.ensureGitExclude", () => {
  it("adds Agent Manager state and worktrees to .git/info/exclude", async () => {
    ...
    expect(content).toContain(".sonderr/worktrees/")
    expect(content).toContain(".sonderr/agent-manager.json")
  })
  ...
  it("adds only specific legacy Agent Manager paths", async () => {
    ...
    expect(content).toContain(".sonderr/worktrees/")
    expect(content).toContain(".sonderr/agent-manager.json")
    expect(content).toContain(".sonderr/setup-script")
  ...
  it("is idempotent -- does not duplicate entries", async () => {
    ...
    const content = await fs.readFile(path.join(root, ".git", "info", "exclude"), "utf-8")
    const count = content.split(".sonderr/worktrees/").length - 1
    expect(count).toBe(1)
  })
})
...
describe("WorktreeManager.removeWorktree safety", () => {
  it("refuses to remove paths outside the worktrees directory", async () => {
    ...
    // Create a directory outside .sonderr/worktrees/
...
describe("WorktreeManager.createWorktree advanced", () => {
  ...
  it("fails before creating a worktree for an unavailable PR target", async () => {
    ...
    expect(existsSync(path.join(clone, ".sonderr", "worktrees"))).toBe(false)
  })
})
