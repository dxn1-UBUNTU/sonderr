/**
 * Localization lint: unused translation keys (all Sonderr-owned pools)
 *
 * Ensures every key in every Sonderr-owned english dictionary is either
 ...
 *   sonderr-i18n     shared webview overrides (packages/sonderr-i18n)
 *   agent-manager agent manager webview dict
 *   cli-backend   extension server/remote dict
 *   host          extension host dict (autocomplete)
 *
 * The upstream @sonderr/ui dict (packages/ui/src/i18n) is intentionally
 ...
 *     provider metadata noteKeys in sonderr/src/sonderr/provider), or
...
const VSCODE = path.join(REPO, "packages/sonderr-vscode")
...
const WEBVIEW_ROOTS = [
  VSCODE,
  path.join(REPO, "packages/sonderr-ui"),
  path.join(REPO, "packages/cli"),
  path.join(REPO, "packages/sonderr-gateway"),
]

const pools = [
  { name: "app", dict: appEn, roots: WEBVIEW_ROOTS, runtime: ["settings.providers.note."] },
  // sonderr-i18n also overrides upstream ui.* keys consumed by components in
