...
import type { Event, Session } from "@sonderr/sdk/v2/client"
...
describe("SonderrProvider follow-up sessions", () => {
  it("scopes shared session events to the active project directory", () => {
    const service = connection()
    const provider = new SonderrProvider({} as never, service as never, undefined, {
  ...
  it("starts standalone stats polling and skips it for embedded providers", async () => {
    const standalone = new SonderrProvider({} as never, connection() as never)
    const normal = standalone as unknown as Internals
    normal.startStatsPolling()
    expect(normal.statsPoller).not.toBeNull()
    standalone.dispose()

    const embedded = new SonderrProvider({} as never, git() as never, undefined, {
  ...
  it("ignores subagents before adopting pending follow-up sessions", async () => {
    const service = connection()
    const provider = new SonderrProvider({} as never, service as never)
    const internal = provider as unknown as Internals
    const sent: unknown[] = []
    const loaded: string[] = []
  ...
  it("calls onFollowupAdopted listeners with session and directory", async () => {
    ...
    internal.pendingFollowup = { dir: "/repo/.sonderr/worktrees/feat", time: Date.now() }
    service.emit(created({ id: "ses-wt", directory: "/repo/.sonderr/worktrees/feat" }))
    await Promise.resolve()

    expect(adopted).toEqual([{ id: "ses-wt", dir: "/repo/.sonderr/worktrees/feat" }])
  })
})
