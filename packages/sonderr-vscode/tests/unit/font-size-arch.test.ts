 ...
 *
 * Sonderr's VS Code webviews use the `sonderr-code.new.fontSize` setting, not
 * VS Code editor font-size or raw pixel declarations. This keeps the Sonderr UI
 * independently scalable across sidebar, settings, Agent Manager, SonderrClaw,
 * diff viewers, code blocks, and shared sonderr-ui controls.
 */
...
describe("webview font-size architecture", () => {
  it("does not use raw font-size pixels or VS Code editor font-size variables in runtime webview UI", () => {
    ...
    expect(
      violations,
      `Use the Sonderr webview font-size tokens instead of raw pixels or VS Code editor font-size variables.\n` +
        `Preferred tokens: var(--font-size-base), var(--font-size-small), or var(--sonderr-font-size-N).\n\n` +
  ...
  it("uses scalable line heights in polished tool previews", () => {
    const files = [
      path.join(REPO, "packages/sonderr-ui/src/components/basic-tool.css"),
      path.join(REPO, "packages/sonderr-ui/src/components/message-part.css"),
    ]
  ...
  it("injects and live-broadcasts the webview font-size setting to all webview providers", () => {
    ...
    expect(util, "buildWebviewHtml must define scaled Sonderr font tokens").toContain("--sonderr-font-size-")

    const missing = WATCHED_PROVIDERS.filter((file) => !fs.readFileSync(file, "utf-8").includes("watchFontSizeConfig"))
    expect(
      missing.map(rel),
      `Webview providers that stay open must broadcast fontSizeChanged when sonderr-code.new.fontSize changes.\n` +
