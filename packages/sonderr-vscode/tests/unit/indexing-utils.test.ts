...
import { mapSSEEventToWebviewMessage } from "../../src/sonderr-provider-utils"
import { configFeatures } from "../../src/features"
import type { EventIndexingStatus, IndexingStatus } from "@sonderr/sdk/v2/client"
...
describe("indexing feature detection", () => {
  ...
  it("enables indexing settings when the indexing plugin is present", () => {
    expect(configFeatures({ plugin: ["sonderr-indexing"] }).indexing).toBe(true)
  })

  it("detects supported indexing plugin specifiers", () => {
    expect(configFeatures({ plugin: ["sonderr-indexing"] }).indexing).toBe(true)
    expect(configFeatures({ plugin: ["sonderr-indexing@1.2.3"] }).indexing).toBe(true)
    expect(configFeatures({ plugin: ["@sonderr/sonderr-indexing"] }).indexing).toBe(true)
    expect(configFeatures({ plugin: ["@sonderr/sonderr-indexing@1.2.3"] }).indexing).toBe(true)
    expect(configFeatures({ plugin: ["file:///tmp/.sonderr/plugin/sonderr-indexing.js"] }).indexing).toBe(true)
    expect(configFeatures({ plugin: ["file:///tmp/node_modules/@sonderr/sonderr-indexing/index.js"] }).indexing).toBe(
      true,
    )
  })

  it("ignores unrelated plugin lists", () => {
    expect(configFeatures({ plugin: ["@sonderr/sonderr-gateway"] }).indexing).toBe(false)
    expect(configFeatures({ plugin: ["file:///tmp/.sonderr/plugin/index.js"] }).indexing).toBe(false)
