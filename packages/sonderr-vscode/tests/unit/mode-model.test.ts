...
describe("modelPatch", () => {
  ...
  it("keeps current variant when next model supports it", () => {
    expect(modelPatch("sonderr", "anthropic/claude-sonnet-4-6", ["low", "high"], "high")).toEqual({
      model: "sonderr/anthropic/claude-sonnet-4-6",
    })
  })

  it("keeps the nearest supported effort when the exact variant is unavailable", () => {
    expect(modelPatch("sonderr", "anthropic/claude-sonnet-4-6", ["low", "medium"], "high")).toEqual({
      model: "sonderr/anthropic/claude-sonnet-4-6",
      variant: "medium",
    })
  })
  ...
  it("clears an unknown variant when next model does not support it", () => {
    expect(modelPatch("sonderr", "anthropic/claude-sonnet-4-6", ["low", "medium"], "thinking")).toEqual({
      model: "sonderr/anthropic/claude-sonnet-4-6",
      variant: null,
    })
  })
