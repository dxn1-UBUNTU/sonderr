...
const SCRIPT = `
  ...
  const [config, setConfig] = createSignal({ disabled_providers: ["sonderr"] })
  ...
  if (sent.length !== 0) fail("prewarmed without Sonderr access")
  setAuth({ sonderr: "api" })
  if (sent.length !== 0) fail("prewarmed while Sonderr was disabled")
  setConfig({})
  if (sent.length !== 1 || sent[0]?.type !== "speechToTextPrewarm") {
    fail("did not prewarm after Sonderr access became available")
  }
  setAuth({ sonderr: "oauth" })
...
describe("speech-to-text prewarm", () => {
  it("starts only after Sonderr speech access becomes available", () => {
    const attempts = 3
