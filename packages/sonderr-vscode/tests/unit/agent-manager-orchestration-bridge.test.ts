...
import type { AgentManagerRequest, Session } from "@sonderr/sdk/v2/client"
...
describe("AgentManagerOrchestrationBridge", () => {
  ...
  function harness(
    ...
    const request = (value: AgentManagerRequest, directory = root) =>
      handlers.event?.(
        { id: `event-${value.id}`, type: "sonderr.agent_manager.requested", properties: value } as SSEPayload,
        directory,
      )
    return {
      bridge,
  ...
  it("keeps live-only secondary worktree sessions scoped to their owning project", async () => {
    ...
    fs.mkdirSync(path.join(secondary, ".sonderr"), { recursive: true })
  ...
  it("handles requests for secondary project directories in multi-project mode", async () => {
    ...
    fs.mkdirSync(path.join(secondaryRoot, ".sonderr"), { recursive: true })
