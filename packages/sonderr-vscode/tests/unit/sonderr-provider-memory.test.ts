...
import { SonderrProviderMemory } from "../../src/sonderr-provider/memory"

function subject(client: SonderrClient | undefined) {
  const posts: unknown[] = []
  const memory = new SonderrProviderMemory({
    client: () => client,
    session: () => undefined,
    dir: () => "/repo",
...
describe("SonderrProviderMemory", () => {
  it("shows stored memory and explains empty projects", async () => {
    ...
    try {
      ...
      expect(picker.mock.calls[0]?.[0]).toContainEqual(
        expect.objectContaining({ label: "Storage", detail: "/repo/.sonderr/memory" }),
      )
      expect(picker.mock.calls[0]?.[0]).toContainEqual(
  ...
  it("routes inspect operations to the memory folder", async () => {
    ...
    try {
      ...
      expect(reveal).toHaveBeenCalledWith("revealFileInOS", expect.objectContaining({ fsPath: "/repo/.sonderr/memory" }))
  ...
  it("handles clients without memory endpoints gracefully", async () => {
    const item = subject({} as SonderrClient)
  ...
  it("evicts older cached memory payloads", async () => {
    let client: SonderrClient | undefined
    const posts: unknown[] = []
    const item = new SonderrProviderMemory({
      client: () => client,
      session: () => undefined,
    ...
    expect(posts[1]).toMatchObject({
      type: "memoryLoaded",
      sessionID: "ses_8",
      status: { root: "/repo/ses_8/.sonderr/memory" },
    })
  })
    ...
    const item = subject({
      memory: {
        configure: async (input: unknown) => {
          ...
          return { data: { root: "/repo/.sonderr/memory", state: state.state } }
        },
        purge: async (input: unknown) => {
          calls.push(["purge", input])
          return { data: { root: "/repo/.sonderr/memory", purged: true } }
        },
        status: async () => ({ data: state }),
        show: async () => ({ data: view }),
      },
