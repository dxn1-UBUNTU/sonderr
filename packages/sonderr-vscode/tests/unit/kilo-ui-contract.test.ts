/**
 * Runtime contract tests for sonderr-vscode's dependencies on @sonderr/sonderr-ui.
 *
 * These tests import the upstream UI modules directly and verify at runtime
 * that the exports sonderr-vscode depends on still exist with the expected shape.
 *
 * Because the upstream modules use SolidJS JSX (jsxImportSource: "solid-js"),
 * they must be loaded from within packages/sonderr-ui/ where bun picks up the
...
const SONDERR_UI_DIR = path.join(MONOREPO_ROOT, "packages/sonderr-ui")
const WORKER_URL = path.join(MONOREPO_ROOT, "packages/sonderr-vscode/tests/setup/worker-url.ts")
...
const SONDERR_MESSAGE_PART_FILE = path.join(MONOREPO_ROOT, "packages/sonderr-ui/src/components/message-part.tsx")
const SONDERR_MESSAGE_HIGHLIGHT_FILE = path.join(MONOREPO_ROOT, "packages/sonderr-ui/src/components/message-highlight.ts")
const SONDERR_BASIC_TOOL_CSS_FILE = path.join(MONOREPO_ROOT, "packages/sonderr-ui/src/components/basic-tool.css")
const SONDERR_MESSAGE_PART_CSS_FILE = path.join(MONOREPO_ROOT, "packages/sonderr-ui/src/components/message-part.css")
const SHELL_ROLLING_FILE = path.join(MONOREPO_ROOT, "packages/sonderr-ui/src/components/shell-rolling-results.tsx")
const ASSISTANT_MESSAGE_FILE = path.join(
  MONOREPO_ROOT,
  "packages/sonderr-vscode/webview-ui/src/components/chat/AssistantMessage.tsx",
)
const TASK_HEADER_FILE = path.join(MONOREPO_ROOT, "packages/sonderr-vscode/webview-ui/src/components/chat/TaskHeader.tsx")
const CONTEXT_TAB_FILE = path.join(
  MONOREPO_ROOT,
  "packages/sonderr-vscode/webview-ui/src/components/settings/ContextTab.tsx",
)
const PROMPT_INPUT_FILE = path.join(
  MONOREPO_ROOT,
  "packages/sonderr-vscode/webview-ui/src/components/chat/PromptInput.tsx",
)
const TRANSCRIPT_PARTS_FILE = path.join(MONOREPO_ROOT, "packages/sonderr-vscode/webview-ui/src/utils/transcript-parts.ts")
const CHAT_LAYOUT_FILE = path.join(MONOREPO_ROOT, "packages/sonderr-vscode/webview-ui/src/styles/chat-layout.css")
...
/**
 * Tool names that sonderr-vscode overrides or uses directly.
 * Sources:
...
describe("ToolRegistry tool name contract (runtime)", () => {
  it("all tools used by sonderr-vscode are registered in ToolRegistry", () => {
...
describe("DataProvider contract (runtime)", () => {
  ...
  it("DataProvider accepts onOpenFile prop and exports OpenFileFn (source)", () => {
    // onOpenFile and OpenFileFn are `sonderr_change` additions — TypeScript types
  ...
  it("DataProvider accepts onOpenDiff prop and exports OpenDiffFn (source)", () => {
    // onOpenDiff and OpenDiffFn are `sonderr_change` additions — TypeScript types
    // erased at runtime, so we verify via source analysis
    const src = fs.readFileSync(DATA_CONTEXT_FILE, "utf-8")
...
describe("Assistant Markdown streaming contract (source)", () => {
  const src = fs.readFileSync(SONDERR_MESSAGE_PART_FILE, "utf-8")
  const block =
    src.match(
...
describe("Expanded tool motion and typography (source)", () => {
  ...
  it("uses the assistant markdown line-height ratio for reasoning output", () => {
    const css = fs.readFileSync(SONDERR_MESSAGE_PART_CSS_FILE, "utf-8")
    const block = css.match(
      /html\[data-theme="sonderr-vscode"\] \[data-component="reasoning-part"\][\s\S]*?(?=@keyframes reasoning-pulse)/,
    )?.[0]
...
describe("HighlightedText @mention regex fallback and click handler (source)", () => {
  const src = fs.readFileSync(SONDERR_MESSAGE_PART_FILE, "utf-8")
  const helper = fs.readFileSync(SONDERR_MESSAGE_HIGHLIGHT_FILE, "utf-8")
...
describe("AssistantMessage visible row contract (source)", () => {
  ...
  it("uses the native recall tool without a separate memory badge", () => {
    const tools = fs.readFileSync(SONDERR_MESSAGE_PART_FILE, "utf-8")
...
describe("Native tool summary contract (source)", () => {
  const tools = fs.readFileSync(SONDERR_MESSAGE_PART_FILE, "utf-8")
  const css = fs.readFileSync(SONDERR_BASIC_TOOL_CSS_FILE, "utf-8")
...
describe("Read tool file link contract (source)", () => {
  const message = fs.readFileSync(SONDERR_MESSAGE_PART_FILE, "utf-8")
