...
import { createAutoApproveBridge } from "../../src/sonderr-provider/auto-approve"
import type { Event, SonderrClient } from "@sonderr/sdk/v2/client"
import type { SonderrConnectionService } from "../../src/services/cli-backend/connection-service"
...
function config(initial: boolean, info: Record<string, unknown> = {}) {
  ...
  return {
    ...
    emit(key = "sonderr-code.new.autoApprove.enabled") {
...
function connection(client: SonderrClient | null, dirs = new Map<string, string>()) {
  const svc = {
    getClient: () => {
  ...
  } as unknown as SonderrConnectionService
...
describe("registerToggleAutoApprove", () => {
  ...
  it("uses the SSE directory for worktree permissions before session mappings are available", async () => {
    ...
    await ctrl.approve(asked("perm_worktree", "ses_worktree"), "/workspace/.sonderr/worktrees/feature")
    await ctrl.approve(asked("perm_child", "ses_child"), "/workspace/.sonderr/worktrees/feature")

    expect(replies).toEqual([
      { requestID: "perm_worktree", directory: "/workspace/.sonderr/worktrees/feature", reply: "once" },
      { requestID: "perm_child", directory: "/workspace/.sonderr/worktrees/feature", reply: "once" },
    ])
  })
  ...
  it("uses the shared permission directory before falling back to session mappings", async () => {
    ...
    const conn = connection(
      ...
      new Map([["perm_shared", "/workspace/.sonderr/worktrees/shared"]]),
    )
    const ctrl = registerToggleAutoApprove(
      context(),
      conn.svc,
    ...
    expect(replies).toEqual([
      { requestID: "perm_shared", directory: "/workspace/.sonderr/worktrees/shared", reply: "once" },
    ])
  })
