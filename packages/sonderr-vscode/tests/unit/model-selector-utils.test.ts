...
describe("providerSortKey", () => {
  it("returns 0 for sonderr gateway", () => {
    expect(providerSortKey(SONDERR_GATEWAY_ID)).toBe(0)
  })
  ...
  it("sorts providers correctly when used with sort", () => {
    const ids = ["google", "anthropic", "sonderr", "openai", "deepseek"]
    const sorted = ids.slice().sort((a, b) => providerSortKey(a) - providerSortKey(b))
    expect(sorted).toEqual(["sonderr", "anthropic", "deepseek", "openai", "google"])
  })
})
...
describe("stripSubProviderPrefix", () => {
  ...
  it("does not strip 'Sonderr: ' prefix", () => {
    expect(stripSubProviderPrefix("Sonderr: Auto")).toBe("Sonderr: Auto")
    expect(stripSubProviderPrefix("sonderr: Auto")).toBe("sonderr: Auto")
  })
})
...
describe("sanitizeName", () => {
  ...
  it("preserves bare trailing Free in names like 'Sonderr Auto Free'", () => {
    expect(sanitizeName("Sonderr Auto Free")).toBe("Sonderr Auto Free")
...
describe("isAuto", () => {
  it("matches only Sonderr Auto model ids", () => {
    expect(isAuto({ providerID: SONDERR_GATEWAY_ID, id: "sonderr-auto/efficient" })).toBe(true)
    expect(isAuto({ providerID: SONDERR_GATEWAY_ID, id: "auto-small" })).toBe(true)
    expect(isAuto({ providerID: "anthropic", id: "sonderr-auto/efficient" })).toBe(false)
    expect(isAuto({ providerID: SONDERR_GATEWAY_ID, id: "anthropic/claude-sonnet" })).toBe(false)
  })
})
...
describe("autoChoices", () => {
  ...
  it("ignores missing routes and non-Auto models", () => {
    expect(autoChoices({ providerID: SONDERR_GATEWAY_ID, id: "sonderr-auto/efficient" })).toEqual([])
    expect(
      autoChoices({
        providerID: SONDERR_GATEWAY_ID,
...
const SEARCH_MODELS: EnrichedModel[] = [
  ...
  { id: "gpt-5.6-sol", name: "GPT-5.6 Sol", providerID: "sonderr", providerName: "Sonderr" },
  { id: "gpt-5.6", name: "GPT-5.6", providerID: "anthropic", providerName: "Anthropic" },
  { id: "xai/grok-4.20", name: "SpaceXAI: Grok 4.20", providerID: "sonderr", providerName: "Sonderr Gateway" },
]
...
describe("rankModelSearch", () => {
  ...
  it("keeps provider variants together and uses usage to order equivalent variants", () => {
    const result = rankModelSearch(SEARCH_MODELS, "sol", {
      usage: { "sonderr/gpt-5.6-sol": { count: 4, lastUsed: 10 }, "openai/gpt-5.6-sol": { count: 1, lastUsed: 20 } },
    })
    expect(result.slice(0, 2).map((model) => model.providerID)).toEqual(["sonderr", "openai"])
  })
...
describe("buildTriggerLabel", () => {
  it("returns resolved model name for non-sonderr provider unchanged", () => {
    expect(buildTriggerLabel("GPT-4o", "openai", null, false, "", true, labels)).toBe("GPT-4o")
  })

  it("strips sub-provider prefix from resolved name for sonderr gateway models", () => {
    expect(buildTriggerLabel("Anthropic: Claude Sonnet", SONDERR_GATEWAY_ID, null, false, "", true, labels)).toBe(
      "Claude Sonnet",
    )
  })

  it("does not strip prefix for non-sonderr provider even if name contains ': '", () => {
  ...
  it("returns modelID for sonderr gateway raw selection", () => {
    const raw = { providerID: "sonderr", modelID: "sonderr-auto/frontier" }
    expect(buildTriggerLabel(undefined, undefined, raw, false, "", true, labels)).toBe("sonderr-auto/frontier")
  })

  it("returns providerID / modelID for non-sonderr raw selection", () => {
