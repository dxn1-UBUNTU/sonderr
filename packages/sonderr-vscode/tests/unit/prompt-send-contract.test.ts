...
const CLOUD_SESSION_FILE = path.join(ROOT, "src/sonderr-provider/handlers/cloud-session.ts")
...
describe("static command completion contract", () => {
  ...
  it("acknowledges aliases after direct and cloud command confirmation", () => {
    const provider = readFile(SONDERRPROVIDER_FILE)
...
describe("SonderrProvider pruneDeletedSession contract", () => {
...
describe("SonderrConnectionService pruneSession contract", () => {
  const source = readFile(CONNECTION_SERVICE_FILE)

  it("drops the deleted session from attached and visible Maps", () => {
    // SonderrProvider's pruneDeletedSession calls connectionService.pruneSession.
