...
describe("buildKeybindingMap", () => {
  it("maps the configurable Agent Manager search shortcut", () => {
    const bindings = [{ command: "sonderr-code.new.agentManager.search", key: "ctrl+f", mac: "cmd+f" }]
  ...
  it("keeps prompt and side-terminal shortcuts separate", () => {
    const bindings = [
      {
        command: "sonderr-code.new.agentManager.newTerminalTab",
        key: "ctrl+shift+t",
        mac: "cmd+shift+t",
        when: "activeWebviewPanelId == 'sonderr-code.new.AgentManagerPanel' && sonderr-code.new.agentManagerPromptFocused",
      },
      {
        command: "sonderr-code.new.agentManager.newSideTerminal",
        key: "ctrl+t",
        mac: "cmd+t",
