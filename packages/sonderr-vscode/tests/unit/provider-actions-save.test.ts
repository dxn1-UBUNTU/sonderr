...
describe("saveCustomProvider", () => {
  ...
  // Regression tests for https://github.com/Sonderr-Org/sonderr/issues/9186
  //
...
describe("fetchProviderData", () => {
  ...
  it("uses local Sonderr auth status instead of profile availability", async () => {
    const client = {
      provider: {
        list: async () => ({
          data: {
            all: [{ id: "sonderr", name: "Sonderr Gateway", source: "custom", env: [], models: {} }],
            connected: ["sonderr"],
            default: { sonderr: "sonderr-auto/frontier" },
          },
        }),
        auth: async () => ({ data: {} }),
      },
    ...
    expect(result.authStates).toEqual({ sonderr: "oauth" })
  })

  it("does not infer Sonderr speech access without stored Gateway auth", async () => {
    const client = {
      provider: {
        list: async () => ({
          data: {
            all: [{ id: "sonderr", name: "Sonderr Gateway", source: "config", key: "configured", env: [], models: {} }],
            connected: ["sonderr"],
            default: { sonderr: "sonderr-auto/frontier" },
          },
        }),
