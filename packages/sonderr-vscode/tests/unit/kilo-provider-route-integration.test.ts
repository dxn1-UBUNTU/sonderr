 ...
 * queried. Mirrors the shape used by sonderr-provider-session-refresh.test.ts.
 */
function mockConnection(getImpl?: (p: SessionGetParams) => Promise<unknown>, vcs = "git") {
  ...
  return {
    ...
    } as unknown as ConstructorParameters<typeof SonderrProvider>[1],
  }
}

async function withNestedRepo(run: (root: string) => Promise<void>): Promise<void> {
  const base = await fs.mkdtemp(path.join(os.tmpdir(), "sonderr-nested-repo-"))
...
describe("SonderrProvider route integration", () => {
  it("finds a nested Git root when the workspace parent is not a repo", async () => {
    await withNestedRepo(async (root) => {
      ...
      const provider = new SonderrProvider({} as never, connection, undefined, {
  ...
  it("resolves a unique worktree session route to its exact worktree directory", async () => {
    const routes = new ProjectRouteService()
    routes.registerProject("a", "/repo/a", 1)
    routes.registerWorktree({ projectId: "a", worktreeId: "wt" }, "/repo/a/.sonderr/wt", 1)
    routes.registerSession({ projectId: "a", sessionId: "ses-wt" }, "/repo/a/.sonderr/wt", 1)
    const { connection, calls } = mockConnection()
    const provider = new SonderrProvider({} as never, connection, undefined, {
      routeService: routes,
      rootDirectory: () => "/active/root",
    ...
    expect(calls).toHaveLength(1)
    expect(calls[0]!.directory).toBe("/repo/a/.sonderr/wt")
  })
  ...
  it("exposes route registration helpers that forward to the route service", () => {
    ...
    provider.registerWorktreeRoute({ projectId: "a", worktreeId: "wt" }, "/repo/a/.sonderr/wt", 1)
    provider.registerSessionRoute({ projectId: "a", sessionId: "s" }, "/repo/a/.sonderr/wt", 1)

    expect(routes.sessionDirectory({ projectId: "a", sessionId: "s" })).toBe("/repo/a/.sonderr/wt")
    expect(routes.worktreeDirectory({ projectId: "a", worktreeId: "wt" })).toBe("/repo/a/.sonderr/wt")
    expect(provider.isSessionRouteAmbiguous("s")).toBe(false)
    expect(provider.routeSessionDirectoryFor({ projectId: "a", sessionId: "s" })).toBe("/repo/a/.sonderr/wt")
  ...
  it("runs a share command on a unique session route against its exact directory", async () => {
    ...
    expect(commandCalls[0]!.directory).toBe("/repo/a/.sonderr/wt")
