...
describe("WorktreeStateManager", () => {
  ...
  beforeEach(() => {
    ...
    // Pre-create .sonderr dir so fire-and-forget saves don't race on mkdir
    fs.mkdirSync(path.join(root, ".sonderr"), { recursive: true })
    logs.length = 0
  ...
  describe("session CRUD", () => {
    ...
    it("drops obsolete session prefs while loading state", async () => {
      const file = path.join(root, ".sonderr", "agent-manager.json")
      fs.writeFileSync(
        file,
        JSON.stringify({
          worktrees: {},
  ...
  describe("persistence", () => {
    ...
    it("creates .sonderr directory if missing", async () => {
      ...
      expect(fs.existsSync(path.join(fresh, ".sonderr", "agent-manager.json"))).toBe(true)
    })
    ...
    it("backs up a corrupt state file before recovery saves", async () => {
      ...
      const files = fs.readdirSync(path.join(root, ".sonderr"))
      expect(result.status).toBe("failed")
  ...
  describe("tab order", () => {
    ...
    it("does not persist empty tab order", async () => {
      ...
      const content = fs.readFileSync(path.join(root, ".sonderr", "agent-manager.json"), "utf-8")
  ...
  describe("validate", () => {
    ...
    it("resolves relative paths against root", async () => {
      const relative = ".sonderr/worktrees/test-branch"
  ...
  describe("legacy .sonderr migration", () => {
    it("migrates and loads state from .sonderr when .sonderr is absent", async () => {
      // Remove the .sonderr dir created in beforeEach
      fs.rmSync(path.join(root, ".sonderr"), { recursive: true, force: true })

      // Write state to legacy .sonderr dir (migration will move it to .sonderr)
      const legacyDir = path.join(root, ".sonderr")
    ...
    it("skips migration when .sonderr state already exists", async () => {
      // .sonderr state already present — migration should skip agent-manager.json
      fs.writeFileSync(
        path.join(root, ".sonderr", "agent-manager.json"),
        JSON.stringify({
          worktrees: {
            "wt-new": {
              branch: "new-branch",
    ...
    it("rewrites stale .sonderr paths in worktree entries (unix)", async () => {
      fs.writeFileSync(
        path.join(root, ".sonderr", "agent-manager.json"),
        JSON.stringify({
          worktrees: {
            "wt-stale": {
              branch: "fix",
              path: "/repo/.sonderr/worktrees/fix",
              parentBranch: "main",
              createdAt: new Date().toISOString(),
            },
          },
      ...
      expect(manager.getWorktrees()[0].path).toBe(`/repo/.sonderr/worktrees/fix`)
    })

    it("rewrites stale .sonderr paths with backslashes (windows)", async () => {
      fs.writeFileSync(
        path.join(root, ".sonderr", "agent-manager.json"),
        JSON.stringify({
          worktrees: {
      ...
      expect(manager.getWorktrees()[0].path).toBe("C:\\.sonderr\\worktrees\\fix")
    })
  })
})
