...
describe("RunScriptService", () => {
  ...
  it("builds commands for shell, PowerShell, and CMD scripts", () => {
    ...
    expect(buildRunTaskCommand({ path: "C:\\repo\\.sonderr\\run-script.ps1", kind: "powershell" })).toEqual({
      command: "powershell.exe",
      args: ["-NoLogo", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "C:\\repo\\.sonderr\\run-script.ps1"],
    })
    expect(buildRunTaskCommand({ path: "C:\\repo path\\.sonderr\\run-script.cmd", kind: "cmd" })).toEqual({
      command: "cmd.exe",
      args: ["/d", "/s", "/c", '"C:\\repo path\\.sonderr\\run-script.cmd"'],
    })
  })
  ...
  it("rejects symlinks pointing outside the .sonderr directory", () => {
  ...
  it("accepts symlinks pointing inside the .sonderr directory", () => {
