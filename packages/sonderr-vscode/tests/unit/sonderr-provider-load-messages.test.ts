...
import type { SessionStatus } from "@sonderr/sdk/v2/client"
import * as vscode from "vscode"
import type { PartUpdate } from "../../src/shared/stream-messages"

// vscode mock is provided by the shared preload (tests/setup/vscode-mock.ts)
const { SonderrProvider, unwrapSyncEvent } = await import("../../src/SonderrProvider")
...
function makeProvider(client: ReturnType<typeof createClient> | null) {
  ...
  const provider = new SonderrProvider({} as never, connection as never)
...
describe("SonderrProvider.handleAbort", () => {
...
describe("SonderrProvider session status reconciliation", () => {
...
describe("SonderrProvider sandbox status", () => {
...
describe("SonderrProvider sandbox toggle", () => {
  ...
  it("resolves a blank worktree default against the routed directory", async () => {
    ...
    await internal.fetchAndSendSandboxDefault("/repo/.sonderr/worktrees/wt-1")

    expect(client.configReads).toEqual([{ directory: "/repo/.sonderr/worktrees/wt-1" }])
    expect(client.sandboxSupport).toEqual([{ directory: "/repo/.sonderr/worktrees/wt-1" }])
  })
  ...
  it("waits for a blank toggle before creating the first prompt session", async () => {
    ...
    expect(client.created).toEqual([
      expect.objectContaining({ metadata: { "sonderr.sandbox": { enabled: true, version: 0 } } }),
    ])
  ...
  it("snapshots the remembered default before sending the first prompt", async () => {
    ...
    expect(client.created).toEqual([
      expect.objectContaining({
        directory: "/repo",
        metadata: { "sonderr.sandbox": { enabled: true, version: 0 } },
      }),
    ])
    expect(client.sandboxed).toHaveLength(0)
    expect(client.prompted).toHaveLength(1)
...
describe("SonderrProvider sidebar tabs", () => {
...
describe("SonderrProvider revert ordering", () => {
...
describe("SonderrProvider.handleLoadMessages / focus mode freshness", () => {
...
describe("SonderrProvider.handleDeleteSession / background processes", () => {
...
describe("SonderrProvider.handleDeleteMessage", () => {
...
describe("SonderrProvider.handleLoadMessages / slim payload", () => {
...
describe("SonderrProvider.loadMessages / sub-agent viewer", () => {
...
describe("SonderrProvider.handleLoadMessages / prepend into deleted session", () => {
