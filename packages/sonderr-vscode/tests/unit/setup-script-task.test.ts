...
const config = {
  command: "sh",
  args: ["/repo/.sonderr/setup-script"],
  cwd: "/repo/worktree",
...
describe("runWorktreeSetupScript", () => {
  function root(script: boolean): string {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "sonderr-setup-flow-"))
    if (!script) return dir
    fs.mkdirSync(path.join(dir, ".sonderr"), { recursive: true })
    fs.writeFileSync(path.join(dir, ".sonderr", "setup-script"), "#!/bin/sh\nexit 0\n")
    return dir
  }
