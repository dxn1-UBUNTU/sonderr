...
import type { QuestionRequest } from "@sonderr/sdk/v2/client"
import {
  fetchAndSendPendingQuestions,
  handleQuestionReject,
  handleQuestionReply,
  type QuestionContext,
} from "../../src/sonderr-provider/handlers/question"
...
describe("question handlers", () => {
  it("routes replies through the recorded request directory", async () => {
    const { fake, replies, questionDirs } = ctx({
      ...
      dirs: new Map([["ses-worktree", "/workspace/.sonderr/worktrees/current"]]),
    })
    questionDirs.set("req-1", "/workspace/.sonderr/worktrees/origin")
    ...
    expect(replies).toEqual([
      {
        requestID: "req-1",
        answers: [["Continue"]],
        directory: "/workspace/.sonderr/worktrees/origin",
      },
    ])
  ...
  it("routes rejects through the recorded request directory", async () => {
    ...
    questionDirs.set("req-2", "/workspace/.sonderr/worktrees/origin")

    const ok = await handleQuestionReject(fake, "req-2", "ses-worktree")

    expect(ok).toBe(true)
    expect(rejects).toEqual([{ requestID: "req-2", directory: "/workspace/.sonderr/worktrees/origin" }])
    ...
    expect(replies).toEqual([
      {
        requestID: "req-3",
        answers: [["Continue"]],
        directory: "/workspace/.sonderr/worktrees/current",
      },
    ])
  })
  ...
  it("removes a stale question when the backend reports it missing", async () => {
    ...
    questionDirs.set("req-stale", "/workspace/.sonderr/worktrees/origin")
  ...
  it("retries a reply through the recovered request directory", async () => {
    ...
    const dir = "/workspace/.sonderr/worktrees/origin"
    ...
    questionDirs.set("req-misrouted", "/workspace/.sonderr/worktrees/stale")
    ...
    expect(replies).toEqual([
      {
        ...
        answers: [["Continue"]],
        directory: "/workspace/.sonderr/worktrees/stale",
      },
      {
        requestID: "req-misrouted",
        answers: [["Continue"]],
  ...
  it("retries even when an unrelated directory fails to list", async () => {
    ...
    const dir = "/workspace/.sonderr/worktrees/origin"
    const failing = "/workspace/.sonderr/worktrees/failing"
    const { fake, messages, replies, questionDirs } = ctx({
      tracked: ["ses-root"],
      extra: [dir, failing],
      pending: { [dir]: [pending("req-misrouted", "ses-root")] },
    ...
    expect(replies.map((args) => (args as { directory: string }).directory)).toEqual([
      "/workspace/.sonderr/worktrees/stale",
      dir,
    ])
    expect(messages).not.toContainEqual({ type: "questionError", requestID: "req-misrouted" })
    expect(questionDirs.has("req-misrouted")).toBe(false)
  ...
  it("retries a reject through the recovered request directory", async () => {
    ...
    expect(rejects).toEqual([
      { requestID: "req-misrouted", directory: "/workspace/.sonderr/worktrees/stale" },
  ...
  it("keeps fallback questions retryable when recovery is incomplete", async () => {
    ...
    const dir = "/workspace/.sonderr/worktrees/failing"
  ...
  it("keeps non-404 failures retryable", async () => {
    ...
    const spy = spyOn(console, "error").mockImplementation(() => {})
    questionDirs.set("req-error", "/workspace/.sonderr/worktrees/origin")

    const ok = await handleQuestionReject(fake, "req-error", "ses-root")
    spy.mockRestore()

    expect(ok).toBe(false)
    expect(questionDirs.get("req-error")).toBe("/workspace/.sonderr/worktrees/origin")
...
describe("question recovery", () => {
  it("records the directory that owns each recovered question", async () => {
    const dir = "/workspace/.sonderr/worktrees/late"
  ...
  it("deduplicates recovered questions across directories", async () => {
    ...
    const dir = "/workspace/.sonderr/worktrees/feature"
    const { fake, messages } = ctx({
      tracked: ["ses-worktree"],
