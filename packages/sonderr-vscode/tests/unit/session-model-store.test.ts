...
const SONDERR_AUTO: ModelSelection = { providerID: "sonderr", modelID: "sonderr-auto/free" }

const providers: Record<string, Provider> = {
  sonderr: makeProvider("sonderr", ["sonderr-auto/free"]),
...
function env(): ResolveEnv {
  return {
    providers,
    connected: ["sonderr", "anthropic", "openai"],
...
describe("per-session model selection", () => {
  it("selecting a model in session A does not write per-mode globally", () => {
    ...
    expect(sessionB).toEqual(SONDERR_AUTO)
  })
...
describe("per-mode model memory", () => {
  ...
  it("switching modes falls back to default after session override is cleared", () => {
    ...
    expect(getSelected(cleared, e, "session-a", "code")).toEqual(SONDERR_AUTO)
  })
