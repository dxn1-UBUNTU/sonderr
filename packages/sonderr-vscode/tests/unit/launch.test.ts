...
async function fixture() {
  const repo = realpathSync(mkdtempSync(join(tmpdir(), "sonderr-launch-")))
  dirs.push(repo)
  for (const path of ["", "packages/sonderr-vscode", "packages/cli", "packages/sdk/js"]) {
    const dir = join(repo, path)
    mkdirSync(dir, { recursive: true })
    symlinkSync(join(source, path, "node_modules"), join(dir, "node_modules"), "junction")
  }
  const root = join(repo, "packages/sonderr-vscode")
  mkdirSync(join(root, "script"))
  cpSync(join(source, "packages/sonderr-vscode/script/launch.ts"), join(root, "script/launch.ts"))
