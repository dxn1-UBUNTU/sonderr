...
const tmpDir = path.join(os.tmpdir(), `sonderr-test-${Date.now()}`)

class TestPaths extends MarketplacePaths {
  override configPath(scope: "project" | "global", workspace?: string): string {
    if (scope === "global") return path.join(tmpDir, "global", "sonderr.json")
    return path.join(tmpDir, "project", ".sonderr", "sonderr.json")
  }
