...
describe("parseServerPort", () => {
  it("parses port from standard CLI startup message", () => {
    expect(parseServerPort("sonderr server listening on http://127.0.0.1:12345")).toBe(12345)
  })
  ...
  it("parses port when embedded in longer output", () => {
    const output = "[INFO] 2024-01-01 sonderr server listening on http://127.0.0.1:54321\n[INFO] ready"
  ...
  it("waits for the complete startup line before resolving a split port", () => {
    const first = "sonderr server listening on http://127.0.0.1:43"
  ...
  it("detects a startup announcement split across stdout chunks", () => {
    const first = "sonderr server listening on http://127.0."
  ...
  it("accepts complete Windows startup lines", () => {
    expect(parseServerPort("sonderr server listening on http://127.0.0.1:43123\r\n", true)).toBe(43123)
  })
})

describe("scanServerPort", () => {
  it("detects startup announcements split across stdout chunks", () => {
    const first = scanServerPort("", "sonderr server listening on http://127.0.", 1024)
  ...
  it("waits for split port digits before resolving startup", () => {
    const first = scanServerPort("", "sonderr server listening on http://127.0.0.1:43", 1024)
  ...
  it("preserves startup announcements followed by oversized stdout chunks", () => {
    const chunk = `sonderr server listening on http://127.0.0.1:43123\n${"x".repeat(1024)}`
...
describe("cli tree-sitter resources", () => {
  it("resolves resources next to the VS Code bundled CLI", () => {
    const root = "/Users/test/.vscode/extensions/sonderr.sonderr-code-7.2.50-darwin-arm64"
    const bin = `${root}/bin/sonderr`
    ...
    expect(sonderrSandboxWorkerForBinary(bin)).toBe(`${root}/bin/sonderr-sandbox-mutation-worker.js`)
    expect(resolveTreeSitterEnv(root)).toEqual({ SONDERR_TREE_SITTER_WASM_DIR: `${root}/bin/tree-sitter` })
  })

  it("resolves resources next to a Windows packaged CLI", () => {
    const root = String.raw`C:\Users\test\.vscode\extensions\sonderr.sonderr-code-7.2.50-win32-x64`
    const bin = String.raw`${root}\bin\sonderr.exe`
    ...
    expect(sonderrSandboxWorkerForBinary(bin)).toBe(String.raw`${root}\bin\sonderr-sandbox-mutation-worker.js`)
    expect(resolveTreeSitterEnv(root)).toEqual({
      SONDERR_TREE_SITTER_WASM_DIR: String.raw`${root}\bin\tree-sitter`,
    })
  })

  it("copies the Sonderr sandbox worker with the packaged CLI binary", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "sonderr-vscode-sandbox-worker-"))
    try {
      const source = path.join(root, "dist", "@sonderr", "cli-darwin-arm64", "bin", "sonderr")
      const target = path.join(root, "extension", "bin", "sonderr")
      await fs.mkdir(path.dirname(source), { recursive: true })
      await fs.mkdir(path.dirname(target), { recursive: true })
      await fs.writeFile(sonderrSandboxWorkerForBinary(source), "worker")

      await copySonderrSandboxWorker(source, target)

      expect(await fs.readFile(sonderrSandboxWorkerForBinary(target), "utf8")).toBe("worker")
    } finally {
  ...
  it("copies runtime and language WASMs with the packaged CLI binary", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "sonderr-vscode-tree-sitter-"))
    try {
      const source = path.join(root, "dist", "@sonderr", "cli-darwin-arm64", "bin", "sonderr")
      const target = path.join(root, "extension", "bin", "sonderr")
  ...
  it("copies the Linux sandbox helper and license resources", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "sonderr-vscode-sandbox-"))
    try {
      const source = path.join(root, "dist", "bin", "sonderr")
      const target = path.join(root, "extension", "bin", "sonderr")
      ...
      const relay = path.join(path.dirname(source), "sonderr-sandbox-network-relay.js")
      const seccomp = path.join(path.dirname(source), "sonderr-sandbox-seccomp")
      ...
      expect(await fs.readFile(path.join(path.dirname(target), "sonderr-sandbox-network-relay.js"), "utf8")).toBe("relay")
      const copiedSeccomp = path.join(path.dirname(target), "sonderr-sandbox-seccomp")
  ...
  it("copies seccomp licensing when bundled Bubblewrap is omitted", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "sonderr-vscode-seccomp-license-"))
    try {
      const source = path.join(root, "dist", "bin", "sonderr")
      const target = path.join(root, "extension", "bin", "sonderr")
      const relay = path.join(path.dirname(source), "sonderr-sandbox-network-relay.js")
      ...
      expect(await fs.readFile(path.join(path.dirname(target), "sonderr-sandbox-network-relay.js"), "utf8")).toBe("relay")
      expect(await fs.readFile(path.join(path.dirname(target), "sonderr-sandbox-seccomp"), "utf8")).toBe("seccomp")
      expect(await fs.readFile(path.join(path.dirname(target), "licenses", "sandbox-runtime", "LICENSE"), "utf8")).toBe(
        "Apache-2.0",
      )
    } finally {
  ...
  it("removes stale sandbox resources when the source has none", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "sonderr-vscode-sandbox-stale-"))
    try {
      const source = path.join(root, "dist", "bin", "sonderr")
      const target = path.join(root, "extension", "bin", "sonderr")
...
describe("toErrorMessage", () => {
  ...
  it("strips ANSI codes before matching Error:", () => {
    const ansiError = "\x1b[91m\x1b[1mError: \x1b[0mConfig file at /path/sonderr.json is not valid JSON(C):"
    const result = toErrorMessage("startup failed", [ansiError])
    expect(result.userMessage).toBe("Config file at /path/sonderr.json is not valid JSON(C):")
  })
  ...
  it("includes CLI path in userDetails when provided", () => {
    const result = toErrorMessage("startup failed", [], "/usr/local/bin/sonderr")
    expect(result.userDetails).toContain("CLI path: /usr/local/bin/sonderr")
  })
  ...
  it("formats structured spawn error details with syscall, errno, and args", () => {
    const spawnLines = [
      ...
      "Errno: -86",
      "Syscall: spawn",
      "Path: /path/to/bin/sonderr",
      'Spawn args: ["serve","--port","0"]',
    ]
    const result = toErrorMessage("Failed to spawn CLI binary (UNKNOWN)", spawnLines, "/path/to/bin/sonderr")
    expect(result.userMessage).toBe("spawn UNKNOWN")
    expect(result.userDetails).toContain("CLI path: /path/to/bin/sonderr")
...
describe("server workspace helpers", () => {
  ...
  it("disables codebase indexing only when no workspace folder is open", () => {
    expect(resolveIndexingEnv(undefined)).toEqual({ SONDERR_DISABLE_CODEBASE_INDEXING: "vscode-no-workspace" })
    expect(resolveIndexingEnv([])).toEqual({ SONDERR_DISABLE_CODEBASE_INDEXING: "vscode-no-workspace" })
  ...
  it("disables unused managed-backend services while preserving the environment", () => {
    expect(
      resolveManagedServerEnv({
        PATH: "/usr/bin",
        SONDERR_DISABLE_CHANNEL_DB: "false",
        SONDERR_EXPERIMENTAL_DISABLE_FILEWATCHER: "false",
      }),
    ).toEqual({
      PATH: "/usr/bin",
      SONDERR_DISABLE_CHANNEL_DB: "true",
      SONDERR_EXPERIMENTAL_DISABLE_FILEWATCHER: "true",
    })
  })
})
