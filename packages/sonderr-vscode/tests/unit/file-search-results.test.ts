...
import { mergeFileSearchResults } from "../../src/sonderr-provider/file-search-results"
...
describe("mergeFileSearchResults", () => {
  ...
  it("preserves camel-case scoring for acronym-style queries", () => {
    const result = mergeFileSearchResults({
      query: "amprov",
      backend: [
        "packages/sonderr-docs/pages/contributing/architecture/onboarding-improvements.md",
        "packages/sonderr-vscode/src/agent-manager/AgentManagerProvider.ts",
      ],
      open: new Set(),
    })
    expect(result[0]).toBe("packages/sonderr-vscode/src/agent-manager/AgentManagerProvider.ts")
  })
})
