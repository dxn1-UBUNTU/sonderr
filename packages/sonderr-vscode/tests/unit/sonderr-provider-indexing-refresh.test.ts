...
import type { Config } from "@sonderr/sdk/v2/client"
import { fetchSnapshot } from "../../src/sonderr-provider/config-snapshot"
...
function binding(internal: Internals, scope: "global" | "project") {
  return internal.configBindings.create({
    ...
    target: {
      scope,
      path: scope === "global" ? "/config/sonderr.jsonc" : "/repo/.sonderr/sonderr.jsonc",
...
function createConnection() {
  ...
  const snapshot = {
    ...
    targets: {
      ...
      project: {
        scope: "project",
        path: "/repo/.sonderr/sonderr.jsonc",
...
describe("SonderrProvider indexing refresh", () => {
  it("shares snapshot payloads across load, SSE refresh, and post-save refresh", async () => {
    ...
    const provider = new SonderrProvider({} as never, conn.service as never)
  ...
  it("reloadAfterAuthChange fetches config first, then indexing status", async () => {
    const provider = new SonderrProvider({} as never, {} as never)
    const internal = provider as unknown as Internals
    const calls: string[] = []
  ...
  it("passes scoped unset paths to the config overlay endpoint", async () => {
    ...
    expect(conn.patches()).toEqual([
      expect.objectContaining({
        scope: "global",
        expected: { path: "/config/sonderr.jsonc", revision: "global-revision" },
        set: { indexing: { qdrant: { apiKey: undefined } } },
        unset: [["indexing", "qdrant", "apiKey"]],
      }),
      expect.objectContaining({
        scope: "project",
        expected: { path: "/repo/.sonderr/sonderr.jsonc", revision: "project-revision" },
  ...
  it("fetchAndSendIndexingStatus writes project consent through the dedicated endpoint", async () => {
    const worktree = "/repo/.sonderr/.sonderr/worktrees/feature"
    ...
    try {
      ...
      const auth = Buffer.from("sonderr:secret").toString("base64")
      expect(headers.get("Authorization")).toBe(`Basic ${auth}`)
      expect(headers.get("x-sonderr-directory")).toBe(worktree)
