...
import { globalFiles, localFiles } from "../../src/sonderr-provider/config-file"
import { openConfig } from "../../src/sonderr-provider/open-config"
...
const labels = {
  ...
  sourceHomeSonderr: "Home Sonderr",
  sourceHomeSonderr: "Home Sonderr",
  sourceHomeSonderr: "Home Sonderr",
  sourceProjectSonderr: "Project Sonderr",
  sourceProjectSonderr: "Project Sonderr",
  sourceProjectSonderr: "Project Sonderr",
...
async function temp() {
  const dir = await mkdtemp(path.join(os.tmpdir(), "sonderr-config-"))
  dirs.push(dir)
  return dir
}
...
describe("config file discovery", () => {
  it("discovers global, env, legacy, and virtual config sources", async () => {
    ...
    process.env.SONDERR_CONFIG_DIR = extra
    process.env.SONDERR_CONFIG_CONTENT = "{}"
    await file(path.join(xdg, "sonderr", "sonderr.json"))
    await file(path.join(home, ".sonderr", "sonderr.json"))
    await file(path.join(home, ".sonderr", "sonderr.jsonc"))
    await file(envfile)
    ...
    expect(sources).toContain("sourceHomeSonderr")
    expect(sources).toContain("sourceHomeSonderr")
  ...
  it("marks project files unloaded when project config is disabled", async () => {
    ...
    await file(path.join(root, "sonderr.json"))
    await file(path.join(root, ".sonderr", "sonderr.json"))
    ...
    expect(list.some((item) => item.source === "sourceProjectSonderr" && item.legacy)).toBe(true)
    expect(list.find((item) => item.recommended)?.file).toBe(path.join(root, ".sonderr", "sonderr.jsonc"))
  })
})
...
describe("openConfig", () => {
  ...
  it("opens the only editable config without showing the picker", async () => {
    reset()
    const root = await temp()
    const cfg = path.join(root, ".sonderr", "sonderr.jsonc")
    await file(cfg)
