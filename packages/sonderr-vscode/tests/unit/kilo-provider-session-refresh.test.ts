...
import { loadSessions, flushPendingSessionRefresh, type SessionRefreshContext } from "../../src/sonderr-provider-utils"
...
describe("SonderrProvider pending session refresh", () => {
  it("does not let a late listing restore the previous project's identity", async () => {
    ...
    let active = "a"
    const provider = new SonderrProvider({} as never, connection as never, undefined, {
