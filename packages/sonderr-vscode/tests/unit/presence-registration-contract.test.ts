 ...
 *
 * Static analysis — reads SonderrProvider.ts, AgentManagerProvider.ts,
 ...
 *
 * Protects against accidental removal during Sonderr development.
 */
...
const SONDERRPROVIDER_FILE = path.join(ROOT, "src/SonderrProvider.ts")
...
describe("SonderrProvider editor-panel visible registration contract", () => {
  const source = readFile(SONDERRPROVIDER_FILE)
...
describe("SonderrProvider disableViewedRegistration contract", () => {
  const sonderrProvider = readFile(SONDERRPROVIDER_FILE)
  const vscodeHost = readFile(VSCODE_HOST_FILE)

  it("registerPresence skips viewed registration when the option is set", () => {
    const match = sonderrProvider.match(/private registerPresence\(\): void \{([\s\S]*?)\n {2}\}/)
    expect(match).not.toBeNull()
    const body = match![1]
  ...
  it("focusSession and trackOpenSessions report through registerPresence", () => {
    ...
    const focus = sonderrProvider.match(/private focusSession\(id\?: string\): void \{([\s\S]*?)\n {2}\}/)
    expect(focus).not.toBeNull()
    expect(focus![1]).toContain("this.registerPresence()")
    const track = sonderrProvider.match(/private trackOpenSessions\(ids: string\[\]\): void \{([\s\S]*?)\n {2}\}/)
  ...
  it("the editor-panel view-state callback honors the same option", () => {
    const match = sonderrProvider.match(
      /this\.viewStateDisposable = this\.visibleTaskStreams\.bindPanel\(panel, \(\) => \{([\s\S]*?)\n {4}\}\)/,
    )
    expect(match).not.toBeNull()
  ...
  it("embedded Agent Manager providers disable generic viewed registration", () => {
    // Each Agent Manager panel hosts a full SonderrProvider; the "agent-manager"
...
describe("SonderrConnectionService connection snapshot contract", () => {
