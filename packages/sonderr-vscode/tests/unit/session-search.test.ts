...
import { handleSessionSearch } from "../../src/sonderr-provider/session-search"
...
describe("handleSessionSearch", () => {
  it("lists root sessions across the worktree family for the resolved directory", async () => {
    ...
    await handleSessionSearch({
      ...
      dir: (id) => (id === "ses_current" ? "/repo/.sonderr/worktrees/wt-1" : "/repo"),
      post: (msg) => posted.push(msg),
    })

    expect(calls).toEqual([{ worktrees: true, roots: true, directory: "/repo/.sonderr/worktrees/wt-1", limit: 5_000 }])
  ...
  it.each(["/repo", "/repo/.sonderr/worktrees/branch"])("loads older chats in one request from %s", async (dir) => {
