...
import type { NotebookRequest } from "@sonderr/sdk/v2/client"
import * as vscode from "vscode"
import { SonderrConnectionService } from "../../src/services/cli-backend/connection-service"
...
function harness(context: NotebookBridgeContext, dirs = ["/repo"]) {
  ...
  const request = (value: NotebookRequest = read, directory = "/repo") =>
    handlers.event?.(
      { id: `event-${value.id}`, type: "sonderr.notebook.requested", properties: value } as SSEPayload,
      directory,
    )
...
describe("SonderrConnectionService notebook directories", () => {
  it("tracks the workspace root, current request directory, and provider directories", async () => {
    ...
    const service = new SonderrConnectionService({} as vscode.ExtensionContext)
