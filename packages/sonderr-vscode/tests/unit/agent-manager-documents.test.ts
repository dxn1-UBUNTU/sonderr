...
describe("Agent Manager document state", () => {
  ...
  it("keeps Markdown in the document inspector and opens source files in VS Code", () => {
    expect(isMarkdownPath(".sonderr/plans/feature.md")).toBe(true)
    expect(isMarkdownPath("docs/architecture.MDX")).toBe(true)
    expect(isMarkdownPath("src/index.ts")).toBe(false)

    const opened: unknown[] = []
    const native: unknown[] = []
    const markdown = new CustomEvent("sonderr:open-file", {
      cancelable: true,
      detail: { filePath: ".sonderr/plans/feature.md", sessionID: "wt-a", line: 4, column: 2 },
    })
    ...
    expect(opened).toEqual([[".sonderr/plans/feature.md", "wt-a", 4, 2]])

    const source = new CustomEvent("sonderr:open-file", {
      cancelable: true,
    ...
    const missing = new CustomEvent("sonderr:open-file", {
      cancelable: true,
