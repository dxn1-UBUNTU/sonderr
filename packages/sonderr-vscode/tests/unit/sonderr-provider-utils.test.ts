...
import {
  ...
  type ProviderInfo,
} from "../../src/sonderr-provider-utils"
...
describe("getErrorMessage", () => {
  ...
  it("reads the first Zod issue message from ConfigInvalidError", () => {
    const err = {
      ...
      data: {
        path: "/Users/me/.config/sonderr/sonderr.json",
        issues: [
...
describe("getConfigErrorDetails", () => {
  ...
  it("formats a single-issue ConfigInvalidError", () => {
    const err = {
      data: {
        path: "/home/me/.config/sonderr/sonderr.json",
        issues: [{ code: "unrecognized_keys", keys: ["indexing"], path: [], message: 'Unrecognized key: "indexing"' }],
      },
    }
    expect(getConfigErrorDetails(err)).toBe('File: /home/me/.config/sonderr/sonderr.json\n\n✖ Unrecognized key: "indexing"')
  })
