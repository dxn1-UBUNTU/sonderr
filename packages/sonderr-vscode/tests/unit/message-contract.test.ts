 ...
 * 2. Every WebviewMessage type handled in SonderrProvider has a corresponding member in the WebviewMessage union
...
const SONDERR_PROVIDER_UTILS_FILE = path.join(ROOT, "src/sonderr-provider-utils.ts")
...
describe("SonderrProvider message handler coverage", () => {
  it("all WebviewMessage switch cases in SonderrProvider exist in WebviewMessage union", () => {
    const providerContent = readFile(SONDERR_PROVIDER_FILE)
    ...
    expect(
      unrecognized,
      `SonderrProvider switch cases not found in any message type definition: ${unrecognized.join(", ")}`,
    ).toEqual([])
  })
})
...
describe("mapSSEEventToWebviewMessage output types", () => {
  it("all output types from mapSSEEventToWebviewMessage exist in ExtensionMessage", () => {
    const utilsContent = readFile(SONDERR_PROVIDER_UTILS_FILE)
    const messagesContent = readMessageTypeSources()
