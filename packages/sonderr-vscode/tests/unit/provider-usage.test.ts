...
import type { ProviderUsage, ProviderUsageWindow } from "@sonderr/sdk/v2/client"
...
// response, so tests never have to mirror SonderrProvider's internal fetcher list.
...
function bridge(usage: UsageClient) {
  ...
  const provider = new SonderrProvider(
    {} as never,
    { getClient: () => benign({ sonderr: { providerUsage: usage } }) } as never,
    undefined,
...
describe("SonderrProvider provider usage bridge", () => {
