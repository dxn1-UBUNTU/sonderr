...
import type { TuiAttentionSoundName } from "@sonderr/plugin/tui"
...
describe("attention defaults", () => {
  it("keeps attention sounds opt-in", async () => {
    ...
    expect(properties["sonderr-code.new.attention.enabled"]?.default).toBe(false)
    expect(properties["sonderr-code.new.attention.sound"]?.default).toBe("default")
    expect(properties["sonderr-code.new.attention.sound"]?.enum).toEqual(["default", "system", ...CustomSoundIDs])
    expect(properties["sonderr-code.new.sounds.agentEnabled"]).toBeUndefined()
    expect(properties["sonderr-code.new.sounds.permissionsEnabled"]).toBeUndefined()
    expect(properties["sonderr-code.new.sounds.errorsEnabled"]).toBeUndefined()
  })
