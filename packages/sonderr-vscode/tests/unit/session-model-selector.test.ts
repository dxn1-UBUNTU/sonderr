...
describe("model selector", () => {
  it("carries the active session variant for the selected model", () => {
    const selected = { providerID: "sonderr", modelID: "old" }
    ...
    selector.select("sonderr", "new")

    const model = { providerID: "sonderr", modelID: "new" }
  ...
  it("retains a session variant without persisting a global model selection", () => {
    ...
    selector.session("session", "sonderr", "new")
