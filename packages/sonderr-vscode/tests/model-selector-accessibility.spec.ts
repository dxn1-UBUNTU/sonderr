...
test("model selector exposes combobox relationships and active option movement", async ({ page }) => {
  ...
  await expect(alpha.locator(".model-selector-item-provider-tag")).toHaveText("Sonderr")
  await expect(bravo.locator(".model-selector-item-provider-tag")).toHaveText("Sonderr")
  ...
  const auto = page.getByRole("treeitem", { name: /Sonderr Auto Efficient/ })
...
test("auto efficient details show server description and model choices", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Review model: Alpha" }).click()
  await page.getByRole("treeitem", { name: /Sonderr Auto Efficient/ }).click()
...
test("auto frontier details show model choices when routes are present", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Review model: Alpha" }).click()
  await page.getByRole("treeitem", { name: /Sonderr Auto Frontier/ }).click()
...
test("provider groups collapse, expand, and skip their model rows", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Review model: Alpha" }).click()
  const combobox = page.getByRole("combobox", { name: "Review model: Alpha. Search models" })
  const sonderr = page.getByRole("treeitem", { name: "Sonderr", exact: true })
  const nvidia = page.getByRole("treeitem", { name: "NVIDIA", exact: true })

  await combobox.press("ArrowDown")
  await combobox.press("ArrowLeft")
  await expect(combobox).toHaveAttribute("aria-activedescendant", await sonderr.getAttribute("id"))
  await combobox.press("ArrowLeft")
  await expect(sonderr).toHaveAttribute("aria-expanded", "false")
