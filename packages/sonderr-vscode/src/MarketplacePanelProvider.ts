...
import type { GlobalEvent, SessionStatus } from "@sonderr/sdk/v2/client"
import { buildWebviewHtml, getWebviewFontSize } from "./utils"
import { watchFontSizeConfig } from "./sonderr-provider/font-size"
import { mapSSEEventToWebviewMessage } from "./sonderr-provider-utils"
...
export class MarketplacePanelProvider implements vscode.Disposable {
  public static readonly viewType = "sonderr-code.new.marketplacePanel"
  ...
  private async sync(reconcile: boolean): Promise<void> {
    ...
    try {
      ...
      console.warn("[Sonderr New] Marketplace session status sync failed:", err)
    }
  }
  ...
  private async handle(msg: MarketplaceMessage): Promise<void> {
    switch (msg.type) {
      ...
      case "dismissAgentMigrationBanner":
        await this.context.globalState.update("sonderr.agentMigrationBannerDismissed", true)
        return
      case "openExternal":
  ...
  private async fetchData(): Promise<void> {
    ...
    try {
      ...
      const dismissed = this.context.globalState.get<boolean>("sonderr.agentMigrationBannerDismissed") ?? false
      this.post({ type: "marketplaceData", ...data, showAgentMigrationBanner: !dismissed })
    } catch (err) {
      if (generation !== this.generation) return
      const error = err instanceof Error ? err.message : String(err)
      console.warn("[Sonderr New] Marketplace data fetch failed:", err)
      this.post({
  ...
  private post(msg: unknown): void {
    ...
    void this.panel.webview.postMessage(msg).then(undefined, (err) => {
      console.warn("[Sonderr New] Marketplace panel postMessage failed:", err)
    })
  }
