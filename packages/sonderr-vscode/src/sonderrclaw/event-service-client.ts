 ...
 *
 * Minimal inline port of `@sonderr/event-service` (cloud monorepo). Connects
 * to the sonderr events Cloudflare Worker using a two-step ticket flow:
...
import type { SonderrChatEventMap, SonderrChatEventName } from "./types"

const WS_SUBPROTOCOL = "sonderr.events.v1"
...
export class EventServiceClient {
  ...
  on<N extends SonderrChatEventName>(event: N, handler: (ctx: string, payload: SonderrChatEventMap[N]) => void): () => void {
    ...
    // sonderr-chat worker edge before broadcast.
    const wrapped: EventHandler = (ctx, payload) => handler(ctx, payload as SonderrChatEventMap[N])
    set.add(wrapped)
  ...
  private handleMessage(data: string): void {
    ...
    if (m.type === "error") {
      console.warn("[Sonderr New] event-service server error", m)
    }
  }
