...
export class BrowserAutomationService implements vscode.Disposable {
  ...
  private static readonly MCP_SERVER_NAME = "sonderr-playwright"

  constructor(private readonly connectionService: SonderrConnectionService) {
    // Listen for settings changes
    this.disposables.push(
      vscode.workspace.onDidChangeConfiguration((e) => {
        if (e.affectsConfiguration("sonderr-code.new.browserAutomation")) {
  ...
  private async register(): Promise<void> {
    ...
    if (!client) {
      console.error("[Sonderr New] BrowserAutomationService: No SDK client available")
    ...
    try {
      ...
      if (serverStatus?.status === "connected") {
        ...
        console.error(
          "[Sonderr New] BrowserAutomationService: MCP server failed:",
      ...
      console.error("[Sonderr New] BrowserAutomationService: Failed to register MCP server:", error)
      this.setState("failed")
    }
  }
  ...
  private async unregister(): Promise<void> {
    ...
    if (client) {
      try {
        ...
        console.error("[Sonderr New] BrowserAutomationService: Failed to disconnect MCP server:", error)
      }
    }
  ...
  private getClient(): SonderrClient | null {
    try {
  ...
  private setState(state: BrowserAutomationState): void {
    ...
    console.log(`[Sonderr New] BrowserAutomationService: State ${this.state} → ${state}`)
    this.state = state
  }
