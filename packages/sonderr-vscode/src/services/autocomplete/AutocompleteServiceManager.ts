...
import type { SonderrConnectionService } from "../cli-backend"
...
const CONFIG_SECTION = "sonderr-code.new.autocomplete"
...
export class AutocompleteServiceManager {
  ...
  constructor(context: vscode.ExtensionContext, connectionService: SonderrConnectionService) {
    ...
    this.config = vscode.workspace.onDidChangeConfiguration((event) => {
      if (event.affectsConfiguration("sonderr-code.new.language")) {
  ...
  private async updateGlobalContext() {
    await vscode.commands.executeCommand(
      "setContext",
      "sonderr.autocomplete.enableSmartInlineTaskKeybinding",
  ...
  private handleFatalAutocompleteError(status: number | null): void {
    const msg =
      status === 402
        ? t("sonderr:autocomplete.creditsExhausted.message")
        : t("sonderr:autocomplete.authError.message")

    if (status === 402) {
      vscode.window.showWarningMessage(msg, t("sonderr:autocomplete.creditsExhausted.addCredits")).then((choice) => {
        if (choice === t("sonderr:autocomplete.creditsExhausted.addCredits")) {
  ...
  public async showIncompatibilityExtensionPopup() {
    const message = t("sonderr:autocomplete.incompatibilityExtensionPopup.message")
    const disableCopilot = t("sonderr:autocomplete.incompatibilityExtensionPopup.disableCopilot")
    const disableInlineAssist = t("sonderr:autocomplete.incompatibilityExtensionPopup.disableInlineAssist")
    ...
    if (response === disableCopilot) {
      ...
      await vscode.commands.executeCommand("sonderr-code.new.autocomplete.disable")
    }
  }
