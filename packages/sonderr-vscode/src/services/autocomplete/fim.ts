 ...
 * Uses the SDK's sonderr.fim() SSE endpoint which handles auth and streaming.
 *
...
export async function generateFim(
  ...
  console.info(`[FIM] request provider=${info.providerID} model=${info.requestModel} url=/sonderr/fim`)

  const { stream } = await client.sonderr.fim(
    {
      prefix,
      suffix,
 ...
 */
export function hasValidCredentials(connectionService: SonderrConnectionService): boolean {
