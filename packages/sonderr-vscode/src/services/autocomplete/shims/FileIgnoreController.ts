...
const SONDERRIGNORE = ".sonderrignore"
const GITIGNORE = ".gitignore"

/**
 * Patterns for sensitive environment files, applied only when no .sonderrignore exists.
 */
...
export class FileIgnoreController {
  ...
  async initialize(): Promise<void> {
    ...
    // Try .sonderrignore first — if it exists, use only that.
    ...
    const sonderrignorePath = path.join(this.workspacePath, SONDERRIGNORE)
    if (fs.existsSync(sonderrignorePath)) {
      const sonderrignoreContent = fs.readFileSync(sonderrignorePath, "utf-8")
      if (sonderrignoreContent.trim()) {
        this.ignoreInstance.add(sonderrignoreContent)
        this.ignoreInstance.add(SONDERRIGNORE)
        return
      }
    }
