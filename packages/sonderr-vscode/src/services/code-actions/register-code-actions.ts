...
import type { SonderrProvider } from "../../SonderrProvider"
...
export function registerCodeActions(
  ...
  provider: SonderrProvider,
  agentManager?: AgentManagerProvider,
  activeTabProvider?: () => SonderrProvider | undefined,
): void {
  ...
  // settles.
  const revealTarget = async (view: SonderrProvider | AgentManagerProvider): Promise<boolean> => {
    if (view === provider) {
      await reveal()
      return true
    }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("sonderr-code.new.explainCode", async () => {
    ...
    vscode.commands.registerCommand("sonderr-code.new.fixCode", async () => {
      const ctx = getEditorContext()
      if (!ctx) return
    ...
    vscode.commands.registerCommand("sonderr-code.new.improveCode", async () => {
      const ctx = getEditorContext()
      if (!ctx) return
    ...
    vscode.commands.registerCommand("sonderr-code.new.addToContext", async () => {
      const ctx = getEditorContext()
      if (!ctx) return
    ...
    vscode.commands.registerCommand("sonderr-code.new.focusChatInput", async () => {
      const view = target()
      if (!(await revealTarget(view))) return
    ...
    vscode.commands.registerCommand("sonderr-code.new.toggleChatSearch", async () => {
      const view = target()
      if (!(await revealTarget(view))) return
