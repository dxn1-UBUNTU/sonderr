...
import type { ConnectionState, SonderrConnectionService } from "../cli-backend/connection-service"
...
export class NotebookBridge {
  ...
  private revision = 0
  private backend: SonderrClient | undefined
  ...
  constructor(
    ...
    this.unsubscribeState = connection.onStateChange((state) => {
      ...
      void this.recover(revision).catch((error: unknown) => {
        console.error("[Sonderr New] NotebookBridge: pending request recovery failed:", error)
      })
    })
  }
  ...
  dispose(): void {
    ...
    for (const context of this.contexts.values()) {
      void context
        ...
        .catch((error: unknown) => console.error("[Sonderr New] NotebookBridge: context disposal failed:", error))
    }
    this.contexts.clear()
    this.origins.clear()
    this.outcomes.clear()
  ...
  private event(event: SSEPayload, directory?: string): void {
    if (event.type === "sonderr.notebook.requested") {
      this.request(event as EventSonderrNotebookRequested, directory)
      return
    }
    if (event.type === "sonderr.notebook.cancelled") {
      this.cancel(event as EventSonderrNotebookCancelled, directory)
    }
  }

  private request(event: EventSonderrNotebookRequested, directory?: string): void {
  ...
  private start(request: NotebookRequest, origin: RequestOrigin): void {
    ...
    void this.run(request, origin, active).catch((error: unknown) => {
      console.error(`[Sonderr New] NotebookBridge: request ${request.id} failed:`, error)
    })
  }

  private cancel(event: EventSonderrNotebookCancelled, directory?: string): void {
  ...
  private async reply(requestID: string, directory: string, result: NotebookResult): Promise<boolean> {
    try {
      ...
      console.error(`[Sonderr New] NotebookBridge: reply ${requestID} failed:`, error)
      return false
    }
  }
  ...
  private async reject(requestID: string, directory: string, error: NotebookFailure): Promise<boolean> {
    try {
      ...
      console.error(`[Sonderr New] NotebookBridge: rejection ${requestID} failed:`, cause)
      return false
    }
  }
    ...
    for (const directory of this.connection.getKnownDirectories()) {
      try {
        ...
        for (const request of response.data ?? []) {
          this.request({ id: request.id, type: "sonderr.notebook.requested", properties: request }, directory)
        }
      } catch (error) {
        console.error(`[Sonderr New] NotebookBridge: could not list requests for ${directory}:`, error)
      }
    }
  }
...
export function createNotebookBridge(connection: SonderrConnectionService): NotebookBridge {
