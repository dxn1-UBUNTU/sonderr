...
export async function removeMarketplaceItemFromAllScopes(
  ...
  try {
    ...
    console.warn("[Sonderr New] Marketplace removal failed:", err)
    return false
  }
}
...
async function fetchSkills(ctx: MarketplaceActionContext, dir: string) {
  try {
    ...
    console.warn("[Sonderr New] Failed to fetch CLI skills for marketplace:", err)
    return undefined
  }
}

async function invalidate(
  ctx: { connection: SonderrConnectionService },
  scope: "project" | "global",
  dir: string,
): Promise<void> {
  const client = await ctx.connection.getClientAsync(dir).catch((err: unknown) => {
    console.warn("[Sonderr New] Marketplace CLI invalidation deferred:", err)
    return null
  })
  if (!client) return
  ...
  if (scope === "global") {
    await client.global.config.update({ config: {} }).catch((err: unknown) => {
      console.warn("[Sonderr New] global.config.update after marketplace change failed:", err)
    })
  }
  await client.instance.dispose({ directory: dir }).catch((err: unknown) => {
    console.warn("[Sonderr New] instance.dispose() after marketplace change failed:", err)
  })
}
...
async function removeLegacyMcp(
  ...
  if (project && scope !== "global") {
    files.push(vscode.Uri.file(path.join(project, ".sonderr", "mcp.json")))
    files.push(vscode.Uri.file(path.join(project, ".sonderr", "mcp.json")))
  }
  ...
  for (const uri of files) {
    ...
    try {
      ...
      console.warn("[Sonderr New] Failed to remove legacy MCP from", uri.fsPath, err)
    }
  }
  return removed
}
