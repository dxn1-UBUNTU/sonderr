...
const sonderrSandboxWorker = "sonderr-sandbox-mutation-worker.js"
const bwrap = "bwrap"
const bwrapLicense = path.join("licenses", "bubblewrap")
const bwrapLicenseFiles = ["NOTICE", "COPYING", "MUSL-COPYRIGHT", "build.ts"]
const sandboxNetworkFiles = ["sonderr-sandbox-network-relay.js", "sonderr-sandbox-seccomp"]
...
export function resolveTreeSitterEnv(root: string): Record<string, string> {
  return { SONDERR_TREE_SITTER_WASM_DIR: treeSitterDirForExtension(root) }
}
...
export function sonderrSandboxWorkerForBinary(file: string): string {
  const p = paths(file)
  return p.join(p.dirname(file), sonderrSandboxWorker)
}

export function hasSonderrSandboxWorker(file: string): boolean {
  return fs.existsSync(sonderrSandboxWorkerForBinary(file))
}
...
export async function copySandboxResources(source: string, target: string): Promise<void> {
  ...
  for (const file of sandboxNetworkFiles) {
    ...
    if (file === "sonderr-sandbox-seccomp") await fs.promises.chmod(output, 0o755)
  }
...
export async function copySonderrSandboxWorker(source: string, target: string): Promise<void> {
  const from = sonderrSandboxWorkerForBinary(source)
  const to = sonderrSandboxWorkerForBinary(target)
  if (!fs.existsSync(from)) throw new Error(`Sonderr sandbox mutation worker not found at ${from}`)
  await fs.promises.copyFile(from, to)
}

function cacheRoot() {
  const root = process.env.XDG_CACHE_HOME ?? path.join(os.homedir(), ".cache")
  return path.join(root, "sonderr-vscode", "bwrap")
}
...
export function resolveLocalBwrapEnv(
  ...
  return { SONDERR_BWRAP_PATH: executable }
}
