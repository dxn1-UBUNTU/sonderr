...
export function resolveIndexingEnv(folders: readonly WorkspaceFolderLike[] | undefined): Record<string, string> {
  ...
  return { SONDERR_DISABLE_CODEBASE_INDEXING: "vscode-no-workspace" }
}
...
export class ServerManager {
  ...
  async getServer(): Promise<ServerInstance> {
    console.log("[Sonderr New] ServerManager: 🔍 getServer called")
    if (this.instance) {
      console.log("[Sonderr New] ServerManager: ♻️ Returning existing instance:", { port: this.instance.port })
      return this.instance
    }

    if (this.startupPromise) {
      console.log("[Sonderr New] ServerManager: ⏳ Startup already in progress, waiting...")
      return this.startupPromise
    }

    console.log("[Sonderr New] ServerManager: 🚀 Starting new server instance...")
    this.startupPromise = this.startServer()
    try {
      this.instance = await this.startupPromise
      console.log("[Sonderr New] ServerManager: ✅ Server started successfully:", { port: this.instance.port })
      return this.instance
    } finally {
  ...
  private async startServer(): Promise<ServerInstance> {
    ...
    console.log("[Sonderr New] ServerManager: 📍 CLI path:", cliPath)
    console.log("[Sonderr New] ServerManager: 🔐 Generated password (length):", password.length)
    ...
    console.log("[Sonderr New] ServerManager: 📄 CLI isFile:", stat.isFile())
    console.log("[Sonderr New] ServerManager: 📄 CLI mode (octal):", (stat.mode & 0o777).toString(8))

    return new Promise((resolve, reject) => {
      console.log("[Sonderr New] ServerManager: 🎬 Spawning CLI process:", cliPath, ["serve", "--port", "0"])
      const cfg = vscode.workspace.getConfiguration("sonderr-code.new")
      ...
      const bwrapEnv = process.env.SONDERR_BWRAP_PATH ? {} : resolveLocalBwrapEnv(this.context.extensionPath, localCli)
      ...
      //     `sonderr-code.new.extraCaCerts` (NODE_EXTRA_CA_CERTS).
      ...
      const serverProcess = spawn(cliPath, ["serve", "--port", "0"], {
        ...
        env: {
          ...
          SONDERR_SERVER_PASSWORD: password,
          ...
          SONDERR_PARENT_PID: String(process.pid),
          SONDERR_CLIENT: "vscode",
          SONDERR_ENABLE_QUESTION_TOOL: "true",
          SONDERR_FEATURE: "vscode-extension",
          ...indexingEnv,
          ...
          SONDERR_VSCODE_VERSION: vscode.version,
          SONDERR_VERSION: this.context.extension.packageJSON.version,
          SONDERR_EDITOR_NAME: `${vscode.env.appName} ${vscode.version}`,
          ...(!claudeCompat && { SONDERR_DISABLE_CLAUDE_CODE: "true" }),
      ...
      console.log("[Sonderr New] ServerManager: 📦 Process spawned with PID:", serverProcess.pid)
      ...
      serverProcess.stdout?.on("data", (data: Buffer) => {
        ...
        console.log("[Sonderr New] ServerManager: 📥 CLI Server stdout:", chunk)
        ...
        if (port !== null && !resolved) {
          resolved = true
          console.log("[Sonderr New] ServerManager: 🎯 Port detected:", port)
      ...
      serverProcess.stderr?.on("data", (data: Buffer) => {
        ...
        console.error("[Sonderr New] ServerManager: ⚠️ CLI Server stderr:", errorOutput)
        stderrLines.push(errorOutput)
      })

      serverProcess.on("error", (err: NodeJS.ErrnoException) => {
        console.error("[Sonderr New] ServerManager: ❌ Process error:", err)
        if (!resolved) {
      ...
      serverProcess.on("exit", (code, signal) => {
        console.warn("[Sonderr New] ServerManager: 🛑 Process exited:", { code, signal })
      ...
      setTimeout(() => {
        if (!resolved) {
          console.error(`[Sonderr New] ServerManager: ⏰ Server startup timeout (${STARTUP_TIMEOUT_SECONDS}s)`)
  ...
  private getCliPath(): string {
    ...
    console.log("[Sonderr New] ServerManager: 📦 Using CLI path:", cliPath)
    return cliPath
  }
  ...
  dispose(): void {
    ...
    console.log("[Sonderr New] ServerManager: 🔴 Disposing — sending SIGTERM to process group, PID:", proc.pid)
    ...
    const timer = setTimeout(() => {
      if (proc.exitCode === null) {
        console.warn("[Sonderr New] ServerManager: ⚠️ Process did not exit after SIGTERM, sending SIGKILL")
