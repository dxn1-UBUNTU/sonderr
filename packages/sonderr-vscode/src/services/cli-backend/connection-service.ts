...
import { createSonderrClient, type SonderrClient } from "@sonderr/sdk/v2/client"
...
async function parallel(items: string[], fn: (item: string) => Promise<void>): Promise<void> {
  ...
  for (const [index, error] of failures.slice(1)) {
    console.warn(`[Sonderr New] ConnectionService: Additional prompt drain failed for ${items[index]}:`, error)
  }
  throw failures[0]![1]
}
...
async function drainNetworkWaits(client: SonderrClient, dir: string) {
...
/**
 * Shared connection service that owns the single ServerManager, SonderrClient (SDK), and SdkSSEAdapter.
 * Multiple SonderrProvider instances subscribe to it for SSE events and state changes.
 */
export class SonderrConnectionService {
  readonly sandboxPreference: SandboxPreference
  private readonly serverManager: ServerManager
  private client: SonderrClient | null = null
   ...
   */
  async getClientAsync(dir?: string): Promise<SonderrClient> {
  ...
  /**
   * Subscribe to notification dismiss events broadcast from any SonderrProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a notification dismiss event to all subscribed SonderrProvider instances.
   */
  ...
  /**
   * Subscribe to language change events broadcast from any SonderrProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a language change event to all subscribed SonderrProvider instances.
   */
  ...
  /**
   * Subscribe to profile change events broadcast from any SonderrProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a profile change event to all subscribed SonderrProvider instances.
   */
  ...
  /**
   * Subscribe to migration-complete events broadcast from any SonderrProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a migration-complete event to all subscribed SonderrProvider instances.
   */
  ...
  /**
   * Subscribe to favorites change events broadcast from any SonderrProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a favorites change event to all subscribed SonderrProvider instances.
   */
  ...
  /**
   * Subscribe to model-selector expand/collapse changes broadcast from any SonderrProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a model-selector expand/collapse change to all subscribed SonderrProvider instances.
   */
   ...
   * SonderrProvider (root + worktree dirs). Used by drainPendingPrompts() to
   ...
   * directory known to any currently-mounted SonderrProvider.
   *
  ...
  private sendViewed(): void {
    ...
    void this.client.session
      ...
      .catch((err) => console.warn("[Sonderr New] ConnectionService: viewed flush failed:", err))
      .finally(() => {
  ...
  private startHealthPoll(baseUrl: string, password: string): void {
    ...
    this.healthPollTimer = setInterval(async () => {
      ...
      if (!healthy && this.state === "connected") {
        console.warn("[Sonderr New] ConnectionService: ❤️‍🩹 Health check failed — forcing SSE reconnect")
  ...
  private async checkHealth(baseUrl: string, password: string): Promise<boolean> {
    try {
      ...
      const res = await fetch(`${baseUrl}/global/health`, {
        headers: { Authorization: `Basic ${Buffer.from(`sonderr:${password}`).toString("base64")}` },
  ...
  private handleServerExit(code: number | null, signal: NodeJS.Signals | null): void {
    ...
    console.warn(`[Sonderr New] ConnectionService: CLI background process exited with ${reason}`)
  ...
  private async doConnect(workspaceDir: string): Promise<void> {
    ...
    const authHeader = `Basic ${Buffer.from(`sonderr:${server.password}`).toString("base64")}`
...
async function drainSuggestions(client: SonderrClient, directory: string): Promise<void> {
