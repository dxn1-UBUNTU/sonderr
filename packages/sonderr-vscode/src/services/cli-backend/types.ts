...
// equivalents in @sonderr/sdk. All API types (Session, Event, Agent,
// McpStatus, Config, etc.) should be imported from "@sonderr/sdk/v2/client".

import type { IndexingStatus as SdkIndexingStatus } from "@sonderr/sdk/v2/client"
...
export interface SonderrOrganization {
  id: string
  name: string
  role: string
}

export interface SonderrProfile {
  email: string
  name?: string
  organizations?: SonderrOrganization[]
...
// Cloud session from the Sonderr cloud API (cli_sessions_v2)
...
// Full cloud session data for preview (from /sonderr/cloud/session/:id)
