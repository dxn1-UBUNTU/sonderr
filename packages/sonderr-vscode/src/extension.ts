...
import { SonderrProvider } from "./SonderrProvider"
import { AgentManagerProvider } from "./agent-manager/AgentManagerProvider"
import { VscodeHost } from "./agent-manager/vscode-host"
import { SonderrClawProvider } from "./sonderrclaw/SonderrClawProvider"
...
import { SonderrConnectionService } from "./services/cli-backend"
...
import { registerCodeActions, registerTerminalActions, SonderrActionProvider } from "./services/code-actions"
...
const RESTORE_KEY = "sonderr.workbench.restore"
...
// without requiring the user to open a Sonderr sidebar or panel first. The CLI backend is NOT spawned here;
// it starts lazily when a webview connects or when ensureBackendForAutocomplete() triggers it.
export async function activate(context: vscode.ExtensionContext) {
  console.log("Sonderr extension is now active")
  shuttingDown = false

  // Drives the "!sonderr-code.new.isCursor" guards on the native view/title and
  // editor/title menu contributions — see isCursorHost() for why.
  void vscode.commands.executeCommand("setContext", "sonderr-code.new.isCursor", isCursorHost())

  const telemetry = TelemetryProxy.getInstance()

  // Create shared connection service (one server for all webviews)
  const connectionService = new SonderrConnectionService(context)
  ...
  const unsubscribeStateChange = connectionService.onStateChange((state) => {
    if (state === "connected") {
      ...
      if (config) {
        ...
        // CLI reads SONDERR_TELEMETRY_LEVEL once at spawn, so without this call
      ...
      try {
        ...
        console.log("[Sonderr New] CLI connected, calling remoteService.refresh()")
        remoteService.refresh().catch((err) => console.warn("[Sonderr New] initial remote refresh failed:", err))
      } catch {
  ...
  for (const folder of vscode.workspace.workspaceFolders ?? []) {
    void markWorkspace(folder.uri.fsPath, (msg) => console.warn(`[Sonderr New] ${msg}`))
  }
  ...
  const tabPanels = new Map<vscode.WebviewPanel, SonderrProvider>()
  ...
  const provider = new SonderrProvider(context.extensionUri, connectionService, context, {
    focusContext: "sonderr-code.new.sidebarFocused",
  })
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider(SonderrProvider.viewType, provider, {
  ...
  // commands by default.
  const skip = [
    "sonderr-code.new.agentManagerOpen",
    "sonderr-code.new.agentManager.showTerminal",
    "sonderr-code.new.agentManager.previousTerminal",
    "sonderr-code.new.agentManager.nextTerminal",
  ]
  if (process.platform === "darwin") skip.push("sonderr-code.new.agentManager.runScript")
  ensureCommandsSkipShell(skip)

  // Create SonderrClaw chat provider for editor panel
  const sonderrClawProvider = new SonderrClawProvider(context.extensionUri, connectionService)
  ...
  const git = createGitExecutable({
    ...
    log: (message) => console.warn(`[Sonderr New] ${message}`),
  })
  ...
  // Register serializer so SonderrClaw panel restores when VS Code restarts
  context.subscriptions.push(
    vscode.window.registerWebviewPanelSerializer(SonderrClawProvider.viewType, {
      deserializeWebviewPanel(panel: vscode.WebviewPanel) {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewPanelSerializer("sonderr-code.new.TabPanel", {
      deserializeWebviewPanel(panel: vscode.WebviewPanel) {
        const tabProvider = new SonderrProvider(context.extensionUri, connectionService, context, {
        ...
        panel.onDidDispose(
          () => {
            console.log("[Sonderr New] Tab panel restored from restart disposed")
  ...
  for (const suffix of settingsViews) {
    context.subscriptions.push(
      vscode.window.registerWebviewPanelSerializer(`sonderr-code.new.${suffix}`, {
        deserializeWebviewPanel(panel: vscode.WebviewPanel) {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewPanelSerializer("sonderr-code.new.SubAgentViewerPanel", {
      deserializeWebviewPanel(panel: vscode.WebviewPanel) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("sonderr-code.new.sidebarTitle.plusButtonClicked", () => {
      track("new_task", "sonderr-code.new.plusButtonClicked")
    }),
    vscode.commands.registerCommand("sonderr-code.new.sidebarTitle.historyButtonClicked", () => {
      track("history", "sonderr-code.new.historyButtonClicked")
    }),
    vscode.commands.registerCommand("sonderr-code.new.sidebarTitle.agentManagerOpen", () => {
      track("agent_manager", "sonderr-code.new.agentManagerOpen")
    }),
    vscode.commands.registerCommand("sonderr-code.new.sidebarTitle.sonderrClawOpen", () => {
      track("sonderrclaw", "sonderr-code.new.sonderrClawOpen")
    }),
    vscode.commands.registerCommand("sonderr-code.new.sidebarTitle.marketplaceButtonClicked", () => {
      track("marketplace", "sonderr-code.new.marketplaceButtonClicked")
    }),
    vscode.commands.registerCommand("sonderr-code.new.sidebarTitle.profileButtonClicked", () => {
      track("profile", "sonderr-code.new.profileButtonClicked")
    }),
    vscode.commands.registerCommand("sonderr-code.new.sidebarTitle.settingsButtonClicked", () => {
      track("settings", "sonderr-code.new.settingsButtonClicked")
    }),
    vscode.commands.registerCommand("sonderr-code.new.plusButtonClicked", () => {
    ...
    vscode.commands.registerCommand("sonderr-code.new.agentManagerOpen", () => {
      agentManagerProvider.openPanel()
    }),
    vscode.commands.registerCommand("sonderr-code.new.marketplaceButtonClicked", (directory?: string | null) => {
      marketplacePanelProvider.openPanel(directory)
    }),
    vscode.commands.registerCommand("sonderr-code.new.sonderrClawOpen", () => {
      sonderrClawProvider.openPanel()
    }),
    vscode.commands.registerCommand("sonderr-code.new.historyButtonClicked", () => {
      const tab = activeTabProvider()
    ...
    vscode.commands.registerCommand("sonderr-code.new.cycleAgentMode", () => {
      const tab = activeTabProvider()
    ...
    vscode.commands.registerCommand("sonderr-code.new.cyclePreviousAgentMode", () => {
      const tab = activeTabProvider()
    ...
    vscode.commands.registerCommand("sonderr-code.new.profileButtonClicked", () => {
      settingsEditorProvider.openPanel("profile")
    }),
    vscode.commands.registerCommand("sonderr-code.new.settingsButtonClicked", (tab?: string, projectId?: string) => {
      settingsEditorProvider.openPanel("settings", tab, projectId)
    }),
    vscode.commands.registerCommand("sonderr-code.new.openIndexingSettings", () => {
      settingsEditorProvider.openPanel("settings", "indexing")
    }),
    vscode.commands.registerCommand("sonderr-code.new.showMemory", async () => {
      if (agentManagerProvider.isActive()) {
        await agentManagerProvider.showMemory()
        return
      }
      const target = activeTabProvider() ?? provider
      if (target === provider) await vscode.commands.executeCommand("sonderr-code.SidebarProvider.focus")
      await target.waitForReady()
      await target.showMemory()
    }),
    vscode.commands.registerCommand("sonderr-code.new.toggleMemory", async () => {
      if (agentManagerProvider.isActive()) {
    ...
    vscode.commands.registerCommand("sonderr-code.new.openMigrationWizard", () => {
      provider.postMessage({ type: "migrationState", needed: true, source: "legacy" })
    }),
    // legacy-migration end
    vscode.commands.registerCommand("sonderr-code.new.generateTerminalCommand", async () => {
      ...
      if (!input) return
      await vscode.commands.executeCommand("sonderr-code.SidebarProvider.focus")
      await provider.waitForReady()
      provider.postMessage({ type: "triggerTask", text: `Generate a terminal command: ${input}` })
    }),
    vscode.commands.registerCommand("sonderr-code.new.toggleRemote", () => {
      remoteService.toggle().catch((err) => console.error("[Sonderr New] toggleRemote command failed:", err))
    }),
    vscode.commands.registerCommand("sonderr-code.new.openInTab", () => {
    ...
    vscode.commands.registerCommand("sonderr-code.new.agentManager.previousSession", () => {
      agentManagerProvider.postMessage({ type: "action", action: "sessionPrevious" })
    }),
    vscode.commands.registerCommand("sonderr-code.new.agentManager.nextSession", () => {
      agentManagerProvider.postMessage({ type: "action", action: "sessionNext" })
    }),
    vscode.commands.registerCommand("sonderr-code.new.agentManager.previousTab", () => {
      agentManagerProvider.postMessage({ type: "action", action: "tabPrevious" })
    }),
    vscode.commands.registerCommand("sonderr-code.new.agentManager.nextTab", () => {
      agentManagerProvider.postMessage({ type: "action", action: "tabNext" })
    }),
    vscode.commands.registerCommand("sonderr-code.new.agentManager.previousTerminal", () => {
      agentManagerProvider.postMessage({ type: "action", action: "terminalPrevious" })
    }),
    vscode.commands.registerCommand("sonderr-code.new.agentManager.nextTerminal", () => {
      agentManagerProvider.postMessage({ type: "action", action: "terminalNext" })
    }),
    vscode.commands.registerCommand("sonderr-code.new.agentManager.search", () => {
      agentManagerProvider.postMessage({ type: "action", action: "search" })
    }),
    vscode.commands.registerCommand("sonderr-code.new.agentManager.showTerminal", () => {
    ...
    vscode.commands.registerCommand("sonderr-code.new.agentManager.runScript", () => {
      agentManagerProvider.postMessage({ type: "action", action: "runScript" })
    }),
    vscode.commands.registerCommand("sonderr-code.new.agentManager.toggleDiff", () => {
      agentManagerProvider.postMessage({ type: "action", action: "toggleDiff" })
    }),
    vscode.commands.registerCommand("sonderr-code.new.agentManager.showShortcuts", () => {
      agentManagerProvider.postMessage({ type: "action", action: "showShortcuts" })
    }),

    vscode.commands.registerCommand("sonderr-code.new.agentManager.newTab", () => {
      agentManagerProvider.postMessage({ type: "action", action: "newTab" })
    }),
    vscode.commands.registerCommand("sonderr-code.new.agentManager.newTerminalTab", () => {
      agentManagerProvider.postMessage({ type: "action", action: "newTerminalTab" })
    }),
    vscode.commands.registerCommand("sonderr-code.new.agentManager.newSideTerminal", () => {
      agentManagerProvider.postMessage({ type: "action", action: "newSideTerminal" })
    }),
    vscode.commands.registerCommand("sonderr-code.new.agentManager.closeTab", () => {
      agentManagerProvider.postMessage({ type: "action", action: "closeTab" })
    }),
    vscode.commands.registerCommand("sonderr-code.new.agentManager.newWorktree", () => {
      agentManagerProvider.postMessage({ type: "action", action: "newWorktree" })
    }),
    vscode.commands.registerCommand("sonderr-code.new.agentManager.quickWorktree", () => {
      agentManagerProvider.postMessage({ type: "action", action: "quickWorktree" })
    }),
    vscode.commands.registerCommand("sonderr-code.new.agentManager.openWorktree", () => {
      agentManagerProvider.postMessage({ type: "action", action: "openWorktree" })
    }),
    vscode.commands.registerCommand("sonderr-code.new.agentManager.openPR", () => {
      agentManagerProvider.postMessage({ type: "action", action: "openPR" })
    }),
    vscode.commands.registerCommand("sonderr-code.new.agentManager.closeWorktree", () => {
      agentManagerProvider.postMessage({ type: "action", action: "closeWorktree" })
    }),
    vscode.commands.registerCommand("sonderr-code.new.agentManager.advancedWorktree", () =>
      agentManagerProvider.openAdvancedWorktree(),
    ),
    ...Array.from({ length: 9 }, (_, i) =>
      vscode.commands.registerCommand(`sonderr-code.new.agentManager.jumpTo${i + 1}`, () => {
        agentManagerProvider.postMessage({ type: "action", action: `jumpTo${i + 1}` })
      }),
    ),
  )

  // Register URI handler for extension deep links (vscode://sonderr.sonderr-code/sonderr/...)
  context.subscriptions.push(
    vscode.window.registerUriHandler({
      async handleUri(uri: vscode.Uri) {
        const sessionMatch = uri.path.match(/^\/sonderr\/s\/([a-zA-Z0-9_-]+)$/)
        const sessionId = sessionMatch?.[1]
        if (sessionId) {
          console.log("[Sonderr New] URI handler: opening cloud session:", sessionId)
          await vscode.commands.executeCommand(`${SonderrProvider.viewType}.focus`)
          provider.openCloudSession(sessionId)
          return
        }

        if (uri.path !== "/sonderr/switch" && uri.path !== "/sonderr/model") return
        const params = new URLSearchParams(uri.query)
        const modelID = params.get("model") || undefined
        const agent = params.get("agent") || undefined
        if (!modelID && !agent) return
        console.log("[Sonderr New] URI handler: applying linked Sonderr selection:", { modelID, agent })
        await vscode.commands.executeCommand(`${SonderrProvider.viewType}.focus`)
        provider.selectSonderrModel(modelID, agent)
      },
    }),
  )
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("sonderr-code.new.reload", () => {
      provider.reload().catch((e) => console.error("[Sonderr New] reload command failed:", e))
    }),
  )
  ...
  context.subscriptions.push(
    vscode.languages.registerCodeActionsProvider(
      { scheme: "file" },
      new SonderrActionProvider(),
      SonderrActionProvider.metadata,
    ),
  )
...
function openSonderrInNewTab(
  ...
  connectionService: SonderrConnectionService,
  agentManagerProvider: AgentManagerProvider,
  tabPanels: Map<vscode.WebviewPanel, SonderrProvider>,
  ...
  panel.iconPath = {
    light: vscode.Uri.joinPath(context.extensionUri, "assets", "icons", "sonderr-light.svg"),
    dark: vscode.Uri.joinPath(context.extensionUri, "assets", "icons", "sonderr-dark.svg"),
  }
  ...
  panel.onDidDispose(
    () => {
      console.log("[Sonderr New] Tab panel disposed")
      tabPanels.delete(panel)
      tabProvider.dispose()
    },
    null,
