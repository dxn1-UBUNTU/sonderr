 ...
 *
 * Reads legacy Sonderr v5.x data from VS Code SecretStorage and the extension's
...
const MIGRATION_STATUS_KEY = "sonderr.legacyMigrationStatus"
...
export async function clearLegacyData(context: vscode.ExtensionContext): Promise<void> {
  ...
  const legacyStateKeys = [
    "sonderr-code.allowedCommands",
    "sonderr-code.deniedCommands",
    "sonderr-code.autoApprovalEnabled",
    "sonderr-code.fuzzyMatchThreshold",
    "sonderr-code.diffEnabled",
    "sonderr-code.language",
    "sonderr-code.customModes",
    "sonderr-code.firstInstallCompleted",
    "sonderr-code.telemetrySetting",
  ...
  // Clear legacy VS Code settings registered under the "sonderr-code" configuration scope.
  ...
  const cfg = vscode.workspace.getConfiguration("sonderr-code")
...
async function migrateProvider(
  ...
  // For providers that support an organization ID (e.g. Sonderr Gateway), migrate using OAuth
...
async function migrateDefaultModel(settings: LegacyProviderSettings, client: SonderrClient): Promise<MigrationResultItem> {
  const provider = settings.apiProvider
  if (!provider) {
...
async function migrateAutocomplete(settings: LegacyAutocompleteSettings): Promise<MigrationResultItem> {
  try {
    const config = vscode.workspace.getConfiguration("sonderr-code.new.autocomplete")
...
function readLegacySettings(context: vscode.ExtensionContext): LegacySettings {
  ...
  return {
    autoApprovalEnabled: context.globalState.get<boolean>("sonderr-code.autoApprovalEnabled"),
    allowedCommands: context.globalState.get<string[]>("sonderr-code.allowedCommands"),
    deniedCommands: context.globalState.get<string[]>("sonderr-code.deniedCommands"),
    ...
    language: context.globalState.get<string>("sonderr-code.language"),
