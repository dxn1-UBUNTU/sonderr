/**
 * Provider action handlers extracted from SonderrProvider to stay under max-lines.
 * These are pure async functions that operate on the SDK client — no vscode dependency.
 */
import type { Config, SonderrClient } from "@sonderr/sdk/v2"
...
import { isCustomProviderPackage, SONDERR_AUTO, SONDERR_PROVIDER_ID, parseModelString } from "./shared/provider-model"
...
export async function fetchProviderData(client: SonderrClient, dir: string) {
  const authRequest =
    typeof client.provider.auth === "function"
      ? client.provider
          ...
          .catch(() => ({}))
      : Promise.resolve({})
  const sonderrRequest = client.sonderr
    .authStatus({ directory: dir }, { throwOnError: true })
    .then((r) => (r.data?.authenticated ? (r.data.type ?? null) : null))
    .catch(() => null)

  const [{ data: response }, authMethods, sonderrAuth] = await Promise.all([
  ...
  delete authStates[SONDERR_PROVIDER_ID]
  if (sonderrAuth) authStates[SONDERR_PROVIDER_ID] = sonderrAuth
  ...
  return {
    ...
    disposeGlobal: async (reason: string) => {
      ...
      await client.global.dispose().catch((error: unknown) => {
        console.warn(`[Sonderr New] SonderrProvider: global.dispose() after ${reason} failed:`, error)
      })
    },
...
async function removeAuth(ctx: ActionContext, id: string, configured: boolean) {
  try {
    ...
    console.warn(`[Sonderr New] auth.remove failed for configured provider ${id} (non-fatal):`, err)
  }
}
