 ...
 * user to press Enter; `sonderr run` auto-retries with backoff. This module implements auto-reply
 ...
 */
export function handleNetworkEvent(type: string, props: Props, client: SonderrClient | null, dir: GetDir) {
  ...
  if (type === "session.network.restored" && props.requestID) {
    ...
    if (!entry) return
    console.log("[Sonderr New] network: auto-replying to restore", props.requestID)
