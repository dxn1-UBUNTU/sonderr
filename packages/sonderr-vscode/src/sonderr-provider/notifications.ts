...
const KEY = "sonderr.dismissedNotificationIds"
...
export async function fetchAndSendNotifications(ctx: NotificationsContext): Promise<void> {
  ...
  try {
    const { data: all } = await retry(() => ctx.client!.sonderr.notifications(undefined, { throwOnError: true }))
    ...
    console.error("[Sonderr New] SonderrProvider: Failed to fetch notifications:", error)
  }
}
