/**
 * Authentication handlers — extracted from SonderrProvider.
 *
...
export interface AuthContext {
  readonly client: SonderrClient | null
...
export async function handleLogin(ctx: AuthContext, attempt: number, getAttempt: () => number): Promise<void> {
  ...
  console.log("[Sonderr New] SonderrProvider: 🔐 Starting login flow...")
  ...
  try {
    ...
    const { data: auth } = await ctx.client.provider.oauth.authorize(
      { providerID: "sonderr", method: 0, directory: dir },
      { throwOnError: true },
    )
    console.log("[Sonderr New] SonderrProvider: 🔐 Got auth URL:", auth.url)
    ...
    await ctx.client.provider.oauth.callback({ providerID: "sonderr", method: 0, directory: dir }, { throwOnError: true })

    // Check if this attempt was cancelled
    if (attempt !== getAttempt()) return

    console.log("[Sonderr New] SonderrProvider: 🔐 Login successful")

    ctx.invalidateProviderUsage()
    await ctx.disposeGlobal()

    // Step 3: Fetch profile and push to webview
    const { data: profile } = await ctx.client.sonderr.profile(undefined, { throwOnError: true })
...
export async function handleLogout(ctx: AuthContext): Promise<void> {
  ...
  try {
    console.log("[Sonderr New] SonderrProvider: 🚪 Logging out...")
    await ctx.client.auth.remove({ providerID: "sonderr" }, { throwOnError: true })
    console.log("[Sonderr New] SonderrProvider: 🚪 Logged out successfully")
    ...
    console.error("[Sonderr New] SonderrProvider: ❌ Logout failed:", error)
    ctx.postMessage({
      type: "error",
...
export async function handleSetOrganization(ctx: AuthContext, organizationId: string | null): Promise<void> {
  ...
  console.log("[Sonderr New] SonderrProvider: Switching organization:", organizationId ?? "personal")
  try {
    await ctx.client.sonderr.organization.set({ organizationId }, { throwOnError: true })
  } catch (error) {
    console.error("[Sonderr New] SonderrProvider: Failed to switch organization:", error)
    // Re-fetch current profile to reset webview state — best-effort
    try {
      const result = await ctx.client.sonderr.profile()
      ctx.postMessage({ type: "profileData", data: result.data ?? null })
    } catch (profileError) {
      console.error("[Sonderr New] SonderrProvider: Failed to refresh profile after org switch error:", profileError)
    }
    return
  }
  ...
  try {
    ...
    console.error("[Sonderr New] SonderrProvider: Failed to refresh agents after org switch:", error)
  }
  try {
    await ctx.fetchAndSendSpeechToTextModels()
  } catch (error) {
    console.error("[Sonderr New] SonderrProvider: Failed to refresh speech-to-text models after org switch:", error)
  }
}
...
export async function handleRefreshProfile(ctx: AuthContext): Promise<void> {
  ...
  console.log("[Sonderr New] SonderrProvider: 🔄 Refreshing profile...")
  const result = await ctx.client.sonderr.profile().catch(() => ({ data: null }))
  ctx.postMessage({ type: "profileData", data: result.data ?? null })
}
