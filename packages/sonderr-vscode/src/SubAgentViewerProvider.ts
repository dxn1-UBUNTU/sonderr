 ...
 *
 * Uses a full SonderrProvider so the viewer has backend connectivity
...
export class SubAgentViewerProvider implements vscode.Disposable {
  ...
  private providers = new Map<string, SonderrProvider>()
  ...
  openPanel(sessionID: string, title?: string, directory?: string): void {
    ...
    const panel = vscode.window.createWebviewPanel("sonderr-code.new.SubAgentViewerPanel", label, vscode.ViewColumn.One, {
      enableScripts: true,
    ...
    const readyDisposable = panel.webview.onDidReceiveMessage((msg) => {
      ...
      try {
        ...
        void client.session
          ...
          .catch((err: unknown) => {
            console.error("[Sonderr New] SubAgentViewerProvider: Failed to load session metadata:", err)
          })
      } catch (err) {
        console.error("[Sonderr New] SubAgentViewerProvider: Failed to load session metadata:", err)
      }
    ...
    panel.onDidDispose(() => {
      console.log("[Sonderr New] Sub-agent viewer panel disposed:", sessionID)
