...
import { getErrorMessage, sessionToWebview } from "../sonderr-provider-utils"
...
export class AgentManagerProvider implements Disposable {
  public static readonly viewType = "sonderr-code.new.AgentManagerPanel"
  ...
  constructor(
    ...
    this.outputChannel = host.createOutput("Sonderr Agent Manager")
    ...
    this.unsubTool = this.connectionService.onEventFiltered(
      (event) => (event as { type?: string }).type === "sonderr.agent_manager.start",
  ...
  private async initializeState(): Promise<void> {
    ...
    // When the .sonderr → .sonderr migration rewrote git worktree refs, nudge
  ...
  private openWorktreeFile(id: string, filePath: string, line?: number, column?: number): void {
    ...
    try {
      ...
      console.error("[Sonderr New] AgentManagerProvider: Cannot resolve file path:", err)
      return
    }
   ...
   * Called from SonderrProvider when the sidebar sends "continueInWorktree".
   */
