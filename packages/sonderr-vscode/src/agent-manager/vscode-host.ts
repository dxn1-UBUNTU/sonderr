...
export class VscodeHost implements Host {
  ...
  openPanel(opts: {
    ...
    const panel = vscode.window.createWebviewPanel(
      "sonderr-code.new.AgentManagerPanel",
      "Agent Manager",
  ...
  private wirePanel(
    ...
    panel.iconPath = {
      light: vscode.Uri.joinPath(this.extensionUri, "assets", "icons", "sonderr-light.svg"),
      dark: vscode.Uri.joinPath(this.extensionUri, "assets", "icons", "sonderr-dark.svg"),
    }
    ...
    const provider = new SonderrProvider(this.extensionUri, this.connectionService, this.context, {
      tabTitle: (title) => {
        panel.title = title
      },
      ...
      focusTargetContext: {
        prompt: "sonderr-code.new.agentManagerPromptFocused",
        mainTerminal: "sonderr-code.new.agentManagerMainTerminalFocused",
        sideTerminal: "sonderr-code.new.agentManagerSideTerminalFocused",
      },
  ...
  private async listProjectSessions(dir: string): Promise<Session[]> {
    try {
      ...
      console.warn(`[Sonderr New] Agent Manager: failed to list project sessions for ${dir}:`, err)
      return []
    }
  }
  ...
  multiProject(): boolean {
    return vscode.workspace.getConfiguration("sonderr-code.new.experimental").get("multiProject", false)
  }
  ...
  onDidChangeMultiProject(cb: (enabled: boolean) => void): Disposable {
    return vscode.workspace.onDidChangeConfiguration((e) => {
      if (e.affectsConfiguration("sonderr-code.new.experimental.multiProject")) cb(this.multiProject())
    })
  }
  ...
  autoBranchNaming(): { enabled: boolean; prefix: string } {
    const cfg = vscode.workspace.getConfiguration("sonderr-code.new.agentManager")
    return {
  ...
  openSettings(tab?: string, projectId?: string): void {
    void vscode.commands.executeCommand("sonderr-code.new.settingsButtonClicked", tab, projectId)
  }
