...
// TODO: Remove the legacy .sonderr -> .sonderr migration helpers below after the
// GA release cleanup tracked in https://github.com/Sonderr-Org/sonderr/issues/6986.
...
/** Sonderr config directory name (project-level and inside worktrees). */
export const SONDERR_DIR = ".sonderr"

/** Legacy config directory name for backward compatibility reads. */
export const LEGACY_DIR = ".sonderr"

/** Agent Manager files that should be migrated from .sonderr/ to .sonderr/. */
...
export interface MigrationResult {
  /** Number of git worktree refs that were rewritten from .sonderr → .sonderr. */
  refsFixed: number
}

/**
 * Migrate Agent Manager data from .sonderr/ to .sonderr/.
 *
 * Moves individual Agent Manager files/directories (worktrees, state,
 * setup scripts) from the legacy .sonderr/ into .sonderr/. Skips items
 * that already exist in .sonderr/ (the new location wins). This is safe
 * because Agent Manager exclusively owns these files.
 *
 * Fixes git worktree internal references (.git/worktrees/{name}/gitdir)
 * whenever .sonderr/worktrees/ exists so partially migrated repos recover too.
 *
...
export async function migrateAgentManagerData(root: string, log: (msg: string) => void): Promise<MigrationResult> {
  ...
  const target = path.join(root, SONDERR_DIR)

  if (await isDirectory(legacy)) {
    // Ensure .sonderr/ exists
    try {
    ...
    for (const item of AGENT_MANAGER_ITEMS) {
      ...
      if (await exists(dst)) {
        log(`Skipping ${item}: already exists in ${SONDERR_DIR}`)
        continue
      }

      try {
        await fs.promises.rename(src, dst)
        log(`Migrated ${item} from ${LEGACY_DIR} to ${SONDERR_DIR}`)
      } catch (err) {
...
/**
 * After moving worktrees from .sonderr/ to .sonderr/, fix git internal refs.
 *
 ...
 * gitdir files that reference the old .sonderr path.
 *
...
async function fixGitWorktreeRefs(root: string, log: (msg: string) => void): Promise<number> {
  ...
  const newSegment = path.join(root, SONDERR_DIR) + path.sep
  let fixed = 0
