 ...
 *
 * Persists to `.sonderr/agent-manager.json`. Decouples worktrees from sessions
...
import { SONDERR_DIR, migrateAgentManagerData, type MigrationResult } from "./constants"
...
export class WorktreeStateManager {
  ...
  constructor(root: string, log: (msg: string) => void) {
    this.root = root
    this.file = path.join(root, SONDERR_DIR, STATE_FILE)
    this.log = log
  }
  ...
  async load(): Promise<StateLoadResult> {
    // Migrate Agent Manager data from .sonderr → .sonderr before first read
  ...
  private apply(content: string): void {
    ...
    for (const [id, wt] of Object.entries(data.worktrees ?? {})) {
      // Rewrite stale .sonderr paths while preserving the separator style already stored.
      const fixed =
        wt.path?.replace(/([/\\])\.sonderr([/\\])/g, (_match, leadingSep, trailingSep) => {
          return `${leadingSep}.sonderr${trailingSep}`
        }) ?? wt.path
