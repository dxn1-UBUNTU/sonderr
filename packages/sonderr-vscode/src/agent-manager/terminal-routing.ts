...
export interface TerminalRoutingDeps {
  ...
  getClientAsync(): Promise<SonderrClient>
  /** Loopback URL + basic-auth password for the running `sonderr serve`. */
...
export class TerminalRouter {
  ...
  private async handleCreate(
    ...
    try {
      ...
      // client accessor when this is the first Sonderr action in the window.
   ...
   * (`packages/cli/src/server/middleware.ts:48`): base64-encoded
  ...
  private buildWsUrl(ptyID: string, cwd: string): string {
    ...
    const token = Buffer.from(`sonderr:${config.password}`).toString("base64")
