 ...
 *
 * Maps Agent Manager terminal IDs to backend PTY IDs (from `sonderr serve`).
...
const env = { SONDERR_UNICODE_LOGO: "0" }
 ...
 * per-instance PTY map — see `packages/cli/src/server/instance/middleware.ts`).
 */
interface Entry {
  terminalId: string
  ptyID: string
...
export class TerminalManager {
  ...
  private async createImpl(params: {
    ...
    const { data, error } = await client.pty.create({
      ...
      // Sonderr's modern wordmark, so use the compatible logo in embedded tabs.
      env,
      size: initial,
    })
    if (error || !data) {
   ...
   *  the server-side PTY would linger until `sonderr serve` exits. */
  async close(terminalId: string): Promise<boolean> {
    ...
    try {
      ...
      if (error) {
        ...
        this.deps.log(`Terminal close failed (${terminalId}): ${msg} — PTY may linger until sonderr serve exits`)
        return false
      }
   ...
   *     server-side PTYs are then only reaped when `sonderr serve` itself
  ...
  async dispose(): Promise<void> {
    ...
    const client = (() => {
      try {
        ...
        const msg = err instanceof Error ? err.message : String(err)
        this.deps.log(
          `Terminal dispose: SDK client unavailable (${msg}); relying on sonderr serve process-group kill to reap PTYs`,
        )
        return undefined
      }
    })()
    ...
    if (failed > 0) {
      this.deps.log(`Terminal dispose: ${failed}/${snapshot.length} PTYs may linger until sonderr serve exits`)
    }
