...
const AM_PREFIX = "sonderr-code.new.agentManager."

/** Global commands whose keybindings are forwarded to the webview. */
const GLOBAL_KEYBINDINGS: Record<string, string> = {
  "sonderr-code.new.agentManagerOpen": "agentManagerOpen",
  "sonderr-code.new.cycleAgentMode": "cycleAgentMode",
  "sonderr-code.new.cyclePreviousAgentMode": "cyclePreviousAgentMode",
}

function addBinding(bindings: Record<string, string>, name: string, value: string, when?: string): void {
  if (name === "newTerminalTab" && when?.includes("!sonderr-code.new.agentManagerSideTerminalFocused")) {
