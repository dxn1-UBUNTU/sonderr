 ...
 *
 * Ported from sonderr/src/core/sonderr/agent-manager/WorktreeManager.ts.
 * Handles creation, discovery, and cleanup of worktrees stored in
 * {projectRoot}/.sonderr/worktrees/
 */
...
import { type GitOps, isSonderrOwnedSshCommand, nonInteractiveEnv } from "./GitOps"
...
const TEMP_PREFIX = ".sonderr-delete-"
...
import { SONDERR_DIR, LEGACY_DIR, migrateAgentManagerData } from "./constants"

const SESSION_ID_FILE = "session-id"
const METADATA_FILE = "metadata.json"
const GIT_METADATA_FILE = "sonderr-agent-manager-metadata.json"
...
export class WorktreeManager {
  ...
  constructor(root: string, log: (msg: string) => void, ops?: GitOps, binary?: string) {
    this.root = root
    this.dir = path.join(root, SONDERR_DIR, "worktrees")
    this.ops = ops
  ...
  /** Run once before first read/write to migrate Agent Manager data from .sonderr → .sonderr. */
  ...
  private async removeWorktreeImpl(worktreePath: string, branch?: string): Promise<void> {
    ...
    const temp = path.join(path.dirname(worktreePath), `.sonderr-delete-${randomUUID()}`)
    try {
  ...
  /** Remove orphaned .sonderr-delete-* temp dirs left by interrupted deletions. */
  ...
  async readMetadata(
    ...
    // Check .sonderr/ first, then legacy .sonderr/
    for (const dirName of [SONDERR_DIR, LEGACY_DIR]) {
  ...
  async ensureGitExclude(): Promise<void> {
    ...
    const items = [
      [".sonderr/worktrees/", "Sonderr agent worktrees"],
      [".sonderr/agent-manager.json", "Sonderr Agent Manager state"],
      [".sonderr/setup-script", "Sonderr worktree setup script"],
      [".sonderr/setup-script.sh", "Sonderr worktree setup script"],
      [".sonderr/setup-script.ps1", "Sonderr worktree setup script"],
      [".sonderr/setup-script.cmd", "Sonderr worktree setup script"],
      [".sonderr/setup-script.bat", "Sonderr worktree setup script"],
      [".sonderr/worktrees/", "Sonderr legacy agent worktrees"],
      [".sonderr/agent-manager.json", "Sonderr Agent Manager legacy state"],
      [".sonderr/setup-script", "Sonderr legacy worktree setup script"],
      [".sonderr/setup-script.sh", "Sonderr legacy worktree setup script"],
      [".sonderr/setup-script.ps1", "Sonderr legacy worktree setup script"],
      [".sonderr/setup-script.cmd", "Sonderr legacy worktree setup script"],
      [".sonderr/setup-script.bat", "Sonderr legacy worktree setup script"],
    ] as const
  ...
  private async refreshBase(branch: string, requested?: string): Promise<void> {
    ...
    // is the fixed value Sonderr injects — never for an inherited one, which
    // could be attacker-controlled.
    const env = nonInteractiveEnv()
    await this.client(this.root, isSonderrOwnedSshCommand(env))
      .env(env)
