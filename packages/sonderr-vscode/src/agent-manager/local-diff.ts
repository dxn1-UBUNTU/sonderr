 ...
 *  used to spike `sonderr serve`; now that the detail path runs in the
 ...
 * `sonderr serve` process, which leaks native memory on every `Bun.spawn` on
...
/** Ported from `packages/cli/src/file/ignore.ts` — identical patterns,
