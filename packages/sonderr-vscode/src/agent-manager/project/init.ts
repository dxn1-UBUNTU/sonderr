...
import { sessionToWebview } from "../../sonderr-provider-utils"
 ...
 * SonderrProvider resolve project-qualified session refs to exact directories
  ...
  const byDir = await Promise.all(
    dirs.map(async (entry) => {
      const items = await list(entry.dir).catch((err) => {
        console.warn(`[Sonderr New] Agent Manager: listing sessions for ${entry.dir} failed:`, err)
        return [] as Session[]
      })
      return { ...entry, items }
    }),
