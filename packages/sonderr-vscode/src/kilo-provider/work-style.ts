...
function getConfig() {
  return vscode.workspace.getConfiguration("sonderr-code.new")
}
...
export function watchWorkStyleConfig(post: (message: unknown) => void, next?: vscode.Disposable) {
  ...
  const watcher = vscode.workspace.onDidChangeConfiguration((event) => {
    if (keys.some((key) => event.affectsConfiguration(`sonderr-code.new.${key}`))) post(getWorkStylePayload())
  })
...
async function hasAnySession(connection: SonderrConnectionService, directory: string): Promise<boolean> {
...
async function initializeWorkStyle(connection: SonderrConnectionService, directory: string): Promise<void> {
...
export async function handleWorkStyleMessage(input: {
  ...
  if (input.message.type === "requestWorkStyle") {
    const initialized = await initializeWorkStyle(input.connection, input.directory)
      .then(() => true)
      .catch((err: unknown) => {
        console.error("[Sonderr New] Failed to initialize work style:", err)
        return false
      })
  ...
  if (!input.message.style) {
    console.error("[Sonderr New] Missing style in setWorkStyle message")
    return true
  }
