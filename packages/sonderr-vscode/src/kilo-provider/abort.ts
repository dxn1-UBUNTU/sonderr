import type { SonderrClient, SessionStatus } from "@sonderr/sdk/v2/client"
...
export class SessionAbort {
  ...
  async stop(
    ...
    if (failures.length > 0) {
      console.error("[Sonderr New] SonderrProvider: Failed to abort session in one or more directories:", failures)
      return false
    }
...
export async function abortSession(input: { client: SonderrClient; sessionID: string; dir: string }) {
