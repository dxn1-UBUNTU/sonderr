...
import { isMemoryOperation, type MemoryOperation } from "@sonderr/sonderr-memory/commands"
import { MemorySchema } from "@sonderr/sonderr-memory/schema"
import type { SonderrClient, Session } from "@sonderr/sdk/v2/client"
import { retry } from "../services/cli-backend/retry"
import { getErrorMessage } from "../sonderr-provider-utils"

type MemorySourceFile = MemorySchema.Source
type MemoryApi = SonderrClient["memory"]
const CACHE_LIMIT = 8
const STORED_LIMIT = 16
const NO_PROJECT = "No active project for memory. Open a file in the target folder to manage its memory."

export type SonderrProviderMemoryMessage = {
...
export type SonderrProviderMemoryInput = {
  client(): SonderrClient | undefined
...
function memory(client: SonderrClient | undefined): MemoryApi | undefined {
...
function request(input: Record<string, unknown>): { value: SonderrProviderMemoryMessage } | { error: string } {
...
export class SonderrProviderMemory {
  private readonly cached = new Map<string, unknown>()
  private tail = Promise.resolve()

  constructor(private readonly input: SonderrProviderMemoryInput) {}
  ...
  async handle(message: Record<string, unknown>): Promise<boolean> {
    if (message.type === "requestMemory") {
      this.fetch(typeof message.sessionID === "string" ? message.sessionID : undefined).catch((err: unknown) =>
        console.error("[Sonderr New] fetchAndSendMemory failed:", err),
      )
      return true
    }
  ...
  private async load(sessionID?: string): Promise<void> {
    try {
      ...
      console.error("[Sonderr New] SonderrProvider: Failed to fetch memory:", err)
      this.input.post({
        type: "memoryLoaded",
        sessionID,
    ...
    try {
      ...
      if (mode === "show" && items.length === 0) {
        void vscode.window.showInformationMessage(
          "This project doesn't have any memory yet. It will start showing after you use Sonderr.",
        )
        return
      }
      ...
      console.error("[Sonderr New] SonderrProvider: Failed to show memory:", err)
      this.input.post({
        type: "memoryLoaded",
        sessionID,
  ...
  run(message: SonderrProviderMemoryMessage): Promise<boolean> {
  ...
  private async execute(message: SonderrProviderMemoryMessage): Promise<boolean> {
    const client = this.input.client()
    if (!client) {
      this.input.post({
        type: "memoryOperationResult",
    ...
    try {
      ...
      const refreshed =
        message.operation === "status"
          ...
          : await retry(() => api.status({ directory }, { throwOnError: true })).catch((err: unknown) => {
              console.warn("[Sonderr New] Memory changed but refresh failed:", err)
              return undefined
            })
      ...
      console.error("[Sonderr New] SonderrProvider: Failed memory operation:", err)
      this.input.post({
        type: "memoryOperationResult",
        operation: message.operation,
        sessionID: message.sessionID,
  ...
  private async action(api: MemoryApi, directory: string, message: SonderrProviderMemoryMessage) {
  ...
  private async remember(api: MemoryApi, directory: string, message: SonderrProviderMemoryMessage) {
  ...
  private async correct(api: MemoryApi, directory: string, message: SonderrProviderMemoryMessage) {
    const text = message.text?.trim()
  ...
  private async forget(api: MemoryApi, directory: string, message: SonderrProviderMemoryMessage) {
  ...
  private async purge(api: MemoryApi, directory: string, message: SonderrProviderMemoryMessage) {
  ...
  private async auto(api: MemoryApi, directory: string, message: SonderrProviderMemoryMessage) {
