...
async function apply(connection: SonderrConnectionService, directory: string, style: WorkStyle) {
  const settings = vscode.workspace.getConfiguration("sonderr-code.new")
...
export async function handleWorkStyleApplyMessage(input: {
  ...
  if (input.message.style !== "human-in-the-loop" && input.message.style !== "autonomous") {
    console.error("[Sonderr New] Invalid style in applyWorkStyle message")
