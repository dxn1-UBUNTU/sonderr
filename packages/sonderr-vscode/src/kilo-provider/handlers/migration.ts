/**
 * Legacy migration handlers — extracted from SonderrProvider.
 *
 * Manages the migration wizard for users upgrading from Sonderr v5.x.
...
export async function checkAndShowMigrationWizard(ctx: MigrationContext): Promise<void> {
  ...
  console.log("[Sonderr New] SonderrProvider: 🔄 Legacy data detected, showing migration wizard")
...
async function startLegacyMigration(
  ...
  try {
    ...
    console.error("[Sonderr New] SonderrProvider: ❌ Migration failed", error)
    ctx.postMessage({
      type: "migrationComplete",
      source: "legacy",
      operationId,
