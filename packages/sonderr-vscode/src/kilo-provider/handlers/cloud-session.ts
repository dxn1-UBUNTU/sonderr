/**
 * Cloud session handlers — extracted from SonderrProvider.
 *
...
import type { SonderrClient, Session, TextPartInput, FilePartInput } from "@sonderr/sdk/v2/client"
import type { CloudSessionData, EditorContext } from "../../services/cli-backend/types"
import { getErrorMessage, sessionToWebview, mapCloudSessionMessageToWebviewMessage } from "../../sonderr-provider-utils"
...
export async function handleRequestCloudSessions(
  ...
  try {
    const result = await ctx.client.sonderr.cloudSessions({
    ...
    console.error("[Sonderr New] SonderrProvider: Failed to fetch cloud sessions:", error)
    ctx.postMessage({
      type: "error",
...
export async function handleRequestCloudSessionData(ctx: CloudSessionContext, sessionId: string): Promise<void> {
  ...
  try {
    const result = await ctx.client.sonderr.cloud.session.get({ id: sessionId }, { signal: AbortSignal.timeout(TIMEOUT) })
    ...
    console.error("[Sonderr New] Failed to load cloud session data:", err)
    ctx.postMessage({
      type: "cloudSessionImportFailed",
      cloudSessionId: sessionId,
...
export async function handleImportAndSend(
  ...
  try {
    const result = await ctx.client.sonderr.cloud.session.import(
      {
    ...
    console.error("[Sonderr New] SonderrProvider: ❌ Cloud session import failed:", error)
    ctx.postMessage({
      type: "cloudSessionImportFailed",
      cloudSessionId,
  ...
  try {
    ...
    console.error("[Sonderr New] Failed to send message after cloud import:", err)
    ctx.postMessage({
