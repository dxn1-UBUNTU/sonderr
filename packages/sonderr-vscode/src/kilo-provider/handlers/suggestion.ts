/**
 * Suggestion handlers — extracted from SonderrProvider.
 *
 * Manages suggestion accept and dismiss flows plus recovery after SSE reconnects.
 * No vscode dependency.
 */

import type { SonderrClient, SuggestionRequest } from "@sonderr/sdk/v2/client"
...
export async function handleSuggestionAccept(
  ...
  try {
    ...
    console.error("[Sonderr New] SonderrProvider: Failed to accept suggestion:", error)
    ctx.postMessage({ type: "suggestionError", requestID })
  }
}
...
export async function handleSuggestionDismiss(
  ...
  try {
    ...
    console.error("[Sonderr New] SonderrProvider: Failed to dismiss suggestion:", error)
    ctx.postMessage({ type: "suggestionError", requestID })
  }
}
...
export async function fetchAndSendPendingSuggestions(ctx: SuggestionContext): Promise<void> {
  ...
  try {
    ...
    console.error("[Sonderr New] SonderrProvider: Failed to fetch pending suggestions:", error)
  }
}
