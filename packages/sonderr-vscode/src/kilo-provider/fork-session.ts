import type { Session, SessionStatus } from "@sonderr/sdk/v2/client"
...
export async function handleForkSession(ctx: ForkContext, sessionId: string, messageId?: string): Promise<void> {
  const status =
    ...
    (await Promise.resolve()
      ...
      .catch((e) => {
        console.error("[Sonderr New] refreshForkStatus failed:", e)
  ...
  await forkSession(
    {
      ...
      registerSession: () => {},
      log: (...args) => console.log("[Sonderr New] SonderrProvider:", ...args),
    },
    sessionId,
    undefined,
    messageId,
