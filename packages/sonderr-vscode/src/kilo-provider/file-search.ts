...
async function fetchBackend(client: SonderrClient, dir: string, query: string): Promise<[string[], string[]]> {
...
function settled(result: PromiseSettledResult<{ data: string[] }>, kind: "file" | "folder"): string[] {
  ...
  console.error(`[Sonderr New] File search (${kind}) failed:`, result.reason)
  return []
}
