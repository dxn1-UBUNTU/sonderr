...
import type { SonderrConnectionService } from "../services/cli-backend/connection-service"
...
const CONFIG = "sonderr-code.new.autoApprove"
const KEY = "enabled"
...
export function registerToggleAutoApprove(
      ...
      try {
        ...
        for (const req of pending) {
          ...
          await client.permission
            ...
            .catch((err) => {
              console.error("[Sonderr New] toggleAutoApprove: failed to drain pending:", err)
            })
        }
      } catch (err) {
        console.error("[Sonderr New] toggleAutoApprove: failed to list pending permissions:", err)
      }
    }
  ...
  const approve = async (event: Asked, directory?: string) => {
    ...
    return client.permission
      ...
      .then(
        () => true,
        (err) => {
          console.error("[Sonderr New] toggleAutoApprove: failed to auto-reply:", err)
          return false
        },
      )
  }
  ...
  context.subscriptions.push(vscode.commands.registerCommand("sonderr-code.new.toggleAutoApprove", toggle))
...
function tryGetClient(connectionService: SonderrConnectionService): SonderrClient | undefined {
  try {
