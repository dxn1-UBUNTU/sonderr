...
import type { AgentManagerSettingsHandler } from "./sonderr-provider/options"

type PanelView = "settings" | "profile" | "indexing"

const PANEL_TITLES: Record<PanelView, string> = {
  settings: "Sonderr Settings",
  profile: "Sonderr Profile",
 ...
 *
 * Uses a full SonderrProvider under the hood so each panel has
...
export class SettingsEditorProvider implements vscode.Disposable {
  ...
  private providers = new Map<PanelView, SonderrProvider>()
  ...
  /** Extract the PanelView from a viewType string like "sonderr-code.new.settingsPanel". */
  static viewFromType(type: string): PanelView | undefined {
    const match = type.match(/^sonderr-code\.new\.(\w+)Panel$/)
  ...
  private wirePanel(panel: vscode.WebviewPanel, view: PanelView, projectDirectory: string | null): void {
    ...
    // Create a dedicated SonderrProvider for this panel so it has full
    ...
    const readyDisposable = panel.webview.onDidReceiveMessage((msg) => {
      if (msg.type === "webviewReady") {
        // Small delay to let SonderrProvider's own webviewReady handler finish first
        setTimeout(() => {
    ...
    panel.onDidDispose(() => {
      console.log(`[Sonderr New] ${title} panel disposed`)
