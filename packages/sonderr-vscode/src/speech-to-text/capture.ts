...
export async function startSpeechCapture(input: Input): Promise<boolean> {
  ...
  try {
    const file = path.join(os.tmpdir(), `sonderr-stt-${process.pid}-${Date.now()}.m4a`)
    if (useMacCapture(process.platform, process.env)) {
      const result = await startMac(file, input).catch((err: unknown) => {
        console.warn("[Sonderr New] Native macOS speech capture failed, falling back to FFmpeg", err)
        return undefined
      })
...
export async function stopSpeechCapture(requestId: string): Promise<Audio> {
  ...
  const size = await stat(state.file)
    .then((info) => info.size)
    .catch((err: unknown) => {
      console.warn("[Sonderr New] Failed to stat speech recording", err)
      return 0
    })
...
export function useMacCapture(platform: NodeJS.Platform, env: NodeJS.ProcessEnv): boolean {
  return platform === "darwin" && !env.SONDERR_FFMPEG_PATH && !env.FFMPEG_PATH
}
...
async function findFFmpeg(): Promise<string> {
  const paths = [
    process.env.SONDERR_FFMPEG_PATH,
  ...
  for (const bin of paths) {
    ...
    try {
      ...
      console.warn(`[Sonderr New] FFmpeg candidate failed: ${bin}`, err)
    }
  }

  throw new Error("Speech input needs the bundled FFmpeg helper, but it was not found. Rebuild or reinstall Sonderr.")
}
...
async function inputArgSets(bin: string): Promise<Args[]> {
  ...
  if (process.platform === "linux") {
    const device = process.env.SONDERR_FFMPEG_AUDIO_DEVICE
    if (device)
      return [
...
async function windowsInputArgSets(bin: string): Promise<Args[]> {
  const configured = process.env.SONDERR_FFMPEG_AUDIO_DEVICE
...
async function removeFile(file: string): Promise<void> {
  await unlink(file).catch((err: unknown) => {
    console.warn("[Sonderr New] Failed to remove speech recording", err)
  })
}
