...
const PATH = "/sonderr/models/transcriptions"
...
export async function fetchSpeechToTextModels(
  ...
  if (!cfg) return fail("Not connected to the Sonderr backend")
...
function toModel(model: CatalogModel): SpeechToTextModelDef {
  ...
  const provider = index === -1 ? model.id.split("/", 1)[0] || "Sonderr Gateway" : model.name.slice(0, index).trim()
  return {
    id: model.id,
