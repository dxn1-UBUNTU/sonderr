...
export class DiffViewerProvider implements vscode.Disposable {
  public static readonly viewType = "sonderr-code.new.DiffViewerPanel"
  ...
  constructor(
    ...
    this.output = vscode.window.createOutputChannel("Sonderr Diff Panel")
  }
  ...
  /**
   * Entry point for the `sonderr-code.new.showChanges` command. Composes the
  ...
  private onWebviewReady(): void {
    ...
    void this.panel.webview.postMessage({
      ...
      languageOverride: vscode.workspace.getConfiguration("sonderr-code.new").get<string>("language"),
