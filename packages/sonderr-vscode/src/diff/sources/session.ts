...
import type { SnapshotFileDiff } from "@sonderr/sdk/v2/client"
import { normalize, text } from "@sonderr/sonderr-ui/session-diff"
...
export function toSessionDiffFile(raw: SnapshotFileDiff): DiffFile {
  ...
  const view = (() => {
    ...
    try {
      ...
      console.warn("[Sonderr New] Failed to parse session diff", { file, err })
      return null
    }
  })()
