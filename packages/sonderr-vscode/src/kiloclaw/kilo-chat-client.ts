/**
 * HTTP client for the sonderr-chat Cloudflare Worker.
 *
 * Minimal inline port of `@sonderr/sonderr-chat/client` (cloud monorepo) tailored
 ...
 * the sonderr-chat worker is the source of truth and validates at its edge.
 */
...
export type SonderrChatClientConfig = {
  baseUrl: string
  getToken: () => Promise<string>
  onUnauthorized?: () => void
}

export class SonderrChatApiError extends Error {
  constructor(
    public readonly status: number,
    public readonly body: unknown,
  ) {
    super(`SonderrChat request failed: ${status}${formatBodyDetail(body)}`)
    this.name = "SonderrChatApiError"
  }
}
...
export class SonderrChatClient {
  private readonly baseUrl: string
  private readonly getToken: () => Promise<string>
  private readonly onUnauthorized: (() => void) | undefined
  private readonly sendQueues: SendQueue = new Map()

  constructor(config: SonderrChatClientConfig) {
  ...
  private async request<T>(path: string, opts: HttpOpts = {}): Promise<T> {
    ...
    if (!res.ok) {
      ...
      throw new SonderrChatApiError(res.status, body)
    }
