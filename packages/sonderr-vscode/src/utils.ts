...
export function getWebviewFontSize(): number {
  const raw = vscode.workspace.getConfiguration("sonderr-code.new").get<number>("fontSize", 13)
  return clamp(raw)
}
...
function fontStyle(): string {
  ...
  const vars = SIZES.map((size) => `--sonderr-font-size-${size}: ${(base * size) / 13}px;`).join("\n      ")
  return `:root {
      ${vars}
      --sonderr-font-scale: ${base / 13};
      --font-size-x-small: var(--sonderr-font-size-10);
      --font-size-small: var(--sonderr-font-size-11);
      --font-size-base: var(--sonderr-font-size-13);
      --font-size-large: var(--sonderr-font-size-16);
    }`
}
...
export function buildWebviewHtml(
  ...
  return `<!DOCTYPE html>
<html lang="en" data-theme="sonderr-vscode" data-sidebar="${opts.sidebar ?? ""}">
<head>
  <meta charset="UTF-8">
...
<body>
  <div id="root"></div>
  <script nonce="${nonce}">window.ICONS_BASE_URI = "${opts.iconsBaseUri}"; window.SONDERR_SHIKI_WORKER_URI = "${opts.workerUri}"; window.SONDERR_MARKDOWN_SHIKI_WORKER_URI = "${markdownWorkerUri}"; window.SONDERR_TOP_BAR = ${opts.topBar !== false}; window.SONDERR_TOP_BAR_SURFACE = "${opts.topBarSurface ?? "sidebar_title"}"; window.SONDERR_AGENT_MANAGER_SETTINGS = ${opts.agentManagerSettings === true};</script>
