...
import { TRANSIENT as MEMORY_TRANSIENT } from "@sonderr/sonderr-memory/schema"
import type {
  SonderrClient,
  ProviderUsage,
  Session,
  ...
  FilePartInput,
  Config,
} from "@sonderr/sdk/v2/client"
import { MaxCostNudge, type MaxCostChoice } from "@sonderr/core/sonderr/cost/max-cost-nudge"
import { type SonderrConnectionService, ServerStartupError } from "./services/cli-backend"
...
import { saveImage } from "./sonderr-provider/save-image"
import { handleEditorAction } from "./sonderr-provider/editor-actions"
import { exportTranscript } from "./sonderr-provider/export-transcript"
import {
  TelemetryProxy,
...
} from "./sonderr-provider-utils"
import { GitOps } from "./agent-manager/GitOps"
import { GitStatsPoller, type LocalStats } from "./agent-manager/GitStatsPoller"
import { createMarketplaceRemover, removeMcp } from "./sonderr-provider/remove-config-item"
...
import { slimInfo, slimPart, slimParts } from "./sonderr-provider/slim-metadata"
import { handleSidebarWorktreeMessage } from "./sonderr-provider/sidebar-worktree"
import { parseMessageFiles, type MessageFile } from "./sonderr-provider/message-files"
import { renameSession } from "./sonderr-provider/rename-session"
import { handleFileSearch } from "./sonderr-provider/file-search"
import { handleSessionSearch } from "./sonderr-provider/session-search"
import { handleFilePicker } from "./sonderr-provider/file-picker"
import { watchFontSizeConfig } from "./sonderr-provider/font-size"
import { getTerminalContents } from "./services/terminal/context"
import { disposeGitChangesTarget } from "./sonderr-provider/git-changes-target"
import { interceptMessage } from "./sonderr-provider/git-changes-request"
import { matchFollowup, recordFollowup, type Followup } from "./sonderr-provider/followup-session"
import { clearCommandsCache, loadCommands } from "./sonderr-provider/commands"
import { fetchMessagePage, MESSAGE_PAGE_LIMIT } from "./sonderr-provider/message-page"
import { editPaths } from "./sonderr-provider/session-edits"
import {
  dismissNotification,
...
} from "./sonderr-provider/notifications"
import { childID } from "./sonderr-provider/task-session"
import { VisibleTaskStreams } from "./sonderr-provider/visible-task-streams"
import { handleNetworkEvent, clearNetworkWaits } from "./sonderr-provider/network"
import { SessionAbort } from "./sonderr-provider/abort"
import {
  buildAutocompleteSettingsMessage,
  validAutocompleteSetting,
  watchAutocompleteConfig,
} from "./services/autocomplete/settings"
import { routeEarlyMessage } from "./sonderr-provider/early-message"
import * as ModelState from "./sonderr-provider/model-state"
import { handleModelUsageMessage } from "./sonderr-provider/model-usage"
import { handleForkSession } from "./sonderr-provider/fork-session"
import { openConfig } from "./sonderr-provider/open-config"
import {
  getWorkStylePayload,
  handleWorkStyleMessage,
  isWorkStyleSetting,
  watchWorkStyleConfig,
} from "./sonderr-provider/work-style"
import * as McpOAuth from "./sonderr-provider/mcp-oauth"
import { retryable, backoff, MAX_RETRIES } from "./util/retry"
import { hasGit } from "./sonderr-provider/git-status"
...
} from "./sonderr-provider/handlers/migration"
...
import {
  ...
  type AuthContext,
} from "./sonderr-provider/handlers/auth"
import {
  handleRequestCloudSessions,
  handleRequestCloudSessionData,
  handleImportAndSend,
  type CloudSessionContext,
} from "./sonderr-provider/handlers/cloud-session"
import {
  handlePermissionResponse,
  fetchAndSendPendingPermissions,
  type PermissionContext,
} from "./sonderr-provider/handlers/permission-handler"
import {
  handleQuestionReply,
  handleQuestionReject,
  fetchAndSendPendingQuestions,
} from "./sonderr-provider/handlers/question"
import { fetchAndSendPendingSuggestions } from "./sonderr-provider/handlers/suggestion"
import { nativeTitle } from "./sonderr-provider/native-tab-title"
...
import { completesWithoutStatus } from "./sonderr-provider/command-completion"
import { SonderrProviderMemory } from "./sonderr-provider/memory"
...
import type { Agent } from "@sonderr/sdk/v2/client"
import { configFeatures, serverFeatures } from "./features"
import { fetchSnapshot } from "./sonderr-provider/config-snapshot"
import { createAutoApproveBridge } from "./sonderr-provider/auto-approve"
import type { SonderrProviderOptions } from "./sonderr-provider/options"
...
import { fetchSonderrEmbeddingModelCatalog } from "@sonderr/sonderr-gateway"
...
import { stopSessionProcesses } from "./sonderr-provider/background-process"
import { sandboxDefault, sandboxSessionMetadata } from "./shared/sandbox-session"
import {
  buildIndexingSettingsMessage,
  validIndexingSetting,
  watchIndexingConfig,
} from "./sonderr-provider/indexing-settings"
import {
  ConfigBindings,
  type ConfigBinding,
  type ConfigProject,
  type ConfigTarget,
} from "./sonderr-provider/config-bindings"
import { canonicalizePath, projectIdFor, samePath } from "./agent-manager/project/paths"
import { validChatSetting, watchChatConfig } from "./sonderr-provider/chat-settings"
import { buildThroughputSettingMessage, watchThroughputConfig } from "./sonderr-provider/throughput-settings"
import {
  buildAutoApprovalReasonSettingMessage,
  watchAutoApprovalReasonConfig,
} from "./sonderr-provider/auto-approval-reason-settings"
...
function sandboxClient(client: SonderrClient | null) {
...
export class SonderrProvider implements vscode.WebviewViewProvider, TelemetryPropertiesProvider {
  public static readonly viewType = "sonderr-code.SidebarProvider"
  ...
  private readonly extensionVersion =
    vscode.extensions.getExtension("sonderr.sonderr-code")?.packageJSON?.version ?? "unknown"
  ...
  /** Cached sonderrEmbeddingModelsLoaded payload so requestSonderrEmbeddingModels is resilient offline. */
  private cachedSonderrEmbeddingModelsMessage: unknown = null
  ...
  private pendingSonderrModel: { modelID?: string; agent?: string } | null = null
  ...
  private readonly memory = new SonderrProviderMemory({
  ...
  constructor(
    ...
    private readonly connectionService: SonderrConnectionService,
    private readonly extensionContext?: vscode.ExtensionContext,
    private readonly opts: SonderrProviderOptions = {},
  ) {
  ...
  private checkpoint(sid: string, run: () => Promise<void>): void {
    ...
    void pending.then(cleanup, (error) => {
      console.error("[Sonderr New] checkpoint mutation failed:", error)
      cleanup()
    })
  }
  ...
  /**
   * Convenience getter that returns the shared SDK SonderrClient or null if not yet connected.
   * Preserves the existing null-check pattern used throughout handler methods.
   */
  private get client(): SonderrClient | null {
    try {
  ...
  private openMarketplacePanel(directory: unknown): void {
    if (typeof directory === "string" && directory) {
      vscode.commands.executeCommand("sonderr-code.new.marketplaceButtonClicked", directory)
      return
    }
    vscode.commands.executeCommand("sonderr-code.new.marketplaceButtonClicked", this.projectDirectory)
  }

  // Strip metadata unused by the webview to keep session switches fast.
  // Logic in sonderr-provider/slim-metadata.ts.
  ...
  private async syncWebviewState(reason: string): Promise<void> {
    ...
    console.log("[Sonderr New] SonderrProvider: 🔄 syncWebviewState()", {
      reason,
    ...
    if (!this.isWebviewReady) {
      console.log("[Sonderr New] SonderrProvider: ⏭️ syncWebviewState skipped (webview not ready)")
      return
    }
    ...
    if (serverInfo) {
      const langConfig = vscode.workspace.getConfiguration("sonderr-code.new")
      this.postMessage({
        type: "ready",
        serverInfo,
    ...
    // Profile returns 401 when user isn't logged into Sonderr Gateway — that's expected.
    ...
    if (this.connectionState === "connected" && this.client) {
      console.log("[Sonderr New] SonderrProvider: 👤 syncWebviewState fetching profile...")
      const profileResult = await retry(() => this.client!.sonderr.profile())
      const profileData = profileResult.data ?? null
      console.log("[Sonderr New] SonderrProvider: 👤 syncWebviewState profile:", profileData ? "received" : "null")
      this.postMessage({
        type: "profileData",
        data: profileData,
      })
  ...
  private setSidebarVisible(visible: boolean): void {
    ...
    vscode.commands.executeCommand("setContext", "sonderr-code.new.sidebarVisible", visible)
    if (!visible && this.opts.focusContext) {
      void vscode.commands.executeCommand("setContext", this.opts.focusContext, false)
    }
  }

  /** Resolve a WebviewPanel for displaying Sonderr in an editor tab. */
  ...
  public async getSessionInfo(sessionId: string): Promise<Session | undefined> {
    ...
    return retry(() => client.session.get({ sessionID: sessionId, directory }, { throwOnError: true }))
      ...
      .catch((error: unknown) => {
        console.warn("[Sonderr New] SonderrProvider: Failed to resolve managed session:", error)
        return undefined
      })
  }
  ...
  public selectSonderrModel(modelID?: string, agent?: string): void {
    if (!modelID && !agent) return
    this.pendingSonderrModel = { ...(modelID && { modelID }), ...(agent && { agent }) }
    this.flushPendingSonderrModel()
  }
    ...
    this.webviewMessageDisposable = webview.onDidReceiveMessage(async (message) => {
      ...
      if (
        await handleSidebarWorktreeMessage(message, {
          post: (msg) => this.postMessage(msg),
          openAgentManager: () => vscode.commands.executeCommand("sonderr-code.new.agentManagerOpen"),
          openAdvancedWorktree: () => vscode.commands.executeCommand("sonderr-code.new.agentManager.advancedWorktree"),
          openChanges: (sessionId?: string, turnId?: string) =>
            vscode.commands.executeCommand("sonderr-code.new.showChanges", {
              sessionId,
              turnId,
              directory: sessionId ? this.sessionGitDirectories.get(sessionId) : undefined,
            }),
          openProfile: () => vscode.commands.executeCommand("sonderr-code.new.profileButtonClicked"),
      ...
      switch (message.type) {
        case "webviewReady":
          console.log("[Sonderr New] SonderrProvider: ✅ webviewReady received")
        ...
        case "loadMessages":
          ...
          break
        case "loadSessions":
          this.handleLoadSessions().catch((e) => console.error("[Sonderr New] handleLoadSessions failed:", e))
          break
        ...
        case "openSettingsPanel":
          vscode.commands.executeCommand("sonderr-code.new.settingsButtonClicked", message.tab, message.projectId)
          break
        case "openSonderrClaw":
          vscode.commands.executeCommand("sonderr-code.new.sonderrClawOpen")
          break
        ...
        case "forkSession":
          handleForkSession(this.forkCtx, message.sessionId, message.messageId).catch((e) =>
            console.error("[Sonderr New] handleForkSession failed:", e),
          )
          break
        case "retryConnection":
          console.log("[Sonderr New] SonderrProvider: 🔄 Retrying connection...")
          this.initializeConnection().catch((e) =>
            console.error("[Sonderr New] SonderrProvider: ❌ Retry connection failed:", e),
          )
          break
        case "reload":
          this.handleReload().catch((e) => console.error("[Sonderr New] SonderrProvider: Reload failed:", e))
          break
        case "openSubAgentViewer":
          vscode.commands.executeCommand(
            "sonderr-code.new.openSubAgentViewer",
            message.sessionID,
            message.title,
        ...
        case "requestProviders":
          this.fetchAndSendProviders().catch((e) => console.error("[Sonderr New] fetchAndSendProviders failed:", e))
          break
        ...
        case "fetchCustomProviderModels":
          this.handleFetchCustomProviderModels(message).catch((e) =>
            console.error("[Sonderr New] fetchCustomProviderModels failed:", e),
          )
          break
        case "compact":
          ...
          break
        case "requestAgents":
          this.fetchAndSendAgents().catch((e) => console.error("[Sonderr New] fetchAndSendAgents failed:", e))
          break
        case "requestSkills":
          this.fetchAndSendSkills().catch((e) => console.error("[Sonderr New] fetchAndSendSkills failed:", e))
          break
        case "requestCommands":
          this.fetchAndSendCommands().catch((e) => console.error("[Sonderr New] fetchAndSendCommands failed:", e))
          break
        case "removeSkill":
          this.removeSkillViaCli(message.location).catch((e: unknown) =>
            console.error("[Sonderr New] removeSkill failed:", e),
          )
          break
        case "removeAgent":
          this.handleRemoveAgent(message.name).catch((e) => console.error("[Sonderr New] handleRemoveAgent failed:", e))
          break
        case "removeMcp":
          this.handleRemoveMcp(message.name).catch((e) => console.error("[Sonderr New] handleRemoveMcp failed:", e))
          break
        case "requestMcpStatus":
          this.fetchAndSendMcpStatus().catch((e) => console.error("[Sonderr New] fetchAndSendMcpStatus failed:", e))
          break
        case "connectMcp": {
          ...
          if (c1) {
            ...
            ).catch((e) => console.error("[Sonderr New] connectMcpServer failed:", e))
          }
          break
        }
        case "disconnectMcp": {
          ...
          if (c2) {
            void McpOAuth.disconnectMcpServer(c2, message.name, this.getWorkspaceDirectory(), () =>
              this.refreshMcpStatus(),
            ).catch((e) => console.error("[Sonderr New] disconnectMcpServer failed:", e))
          }
          break
        }
        case "authenticateMcp": {
          ...
          if (c) {
            void McpOAuth.authenticateMcpServer(c, message.name, this.getWorkspaceDirectory(), () =>
              this.refreshMcpStatus(),
            ).catch((e) => console.error("[Sonderr New] authenticateMcpServer failed:", e))
          }
          break
        }
        ...
        case "toggleSandbox":
          ...
          break
        case "requestConfig":
          this.fetchAndSendConfig().catch((e) => console.error("[Sonderr New] fetchAndSendConfig failed:", e))
          break
        case "requestGlobalConfig":
          this.fetchAndSendGlobalConfig().catch((e) => console.error("[Sonderr New] fetchAndSendGlobalConfig failed:", e))
          break
        case "requestIndexingStatus":
          this.fetchAndSendIndexingStatus().catch((e) =>
            console.error("[Sonderr New] fetchAndSendIndexingStatus failed:", e),
          )
          break
        ...
        case "setIndexingConsent":
          ...
          break
        case "requestSonderrEmbeddingModels":
          this.fetchAndSendSonderrEmbeddingModels().catch((e) =>
            console.error("[Sonderr New] fetchAndSendSonderrEmbeddingModels failed:", e),
          )
          break
        case "requestImageModels":
          this.fetchAndSendImageModels().catch((e) => console.error("[Sonderr New] fetchAndSendImageModels failed:", e))
          break
        case "updateConfig":
        ...
        case "openSettingsTab":
          if (message.tab === "indexing") {
            await vscode.commands.executeCommand("sonderr-code.new.openIndexingSettings")
          }
          break
        case "setLanguage":
          await vscode.workspace
            .getConfiguration("sonderr-code.new")
        ...
        case "requestRemoteStatus":
          this.remoteService
            ...
            .catch((err) => console.error("[Sonderr New] remote message failed:", err))
          break
        case "deleteSession":
        ...
        case "requestNotifications":
          this.fetchAndSendNotifications().catch((e) =>
            console.error("[Sonderr New] fetchAndSendNotifications failed:", e),
          )
          break
        ...
        case "enhancePrompt": {
          ...
          void sdkClient.enhancePrompt
            ...
            .catch((err: unknown) => {
              ...
              console.error("[Sonderr New] SonderrProvider: Failed to enhance prompt:", err)
  ...
  private handleChildSyncMessage(
    ...
    if (message.type === "syncSession") {
      ...
      this.handleSyncSession(message.sessionID, parent).catch((e) =>
        console.error("[Sonderr New] handleSyncSession failed:", e),
      )
      return true
    }
   ...
   * Subscribes to the shared SonderrConnectionService.
   */
  ...
  private async doInitializeConnection(): Promise<void> {
    console.log("[Sonderr New] SonderrProvider: 🔧 Starting initializeConnection...")
    ...
    try {
      ...
      this.unsubscribeEvent = this.connectionService.onEventFiltered(
        (payload, directory) => {
          ...
          if (event.type === "sonderr-sessions.remote-status-changed") return true
          ...
          // SonderrProvider instance. The Settings panel is a separate provider with no tracked
      ...
      this.unsubscribeState = this.connectionService.onStateChange(async (state, error) => {
        ...
        if (state === "connected") {
          ...
          try {
            ...
            const sdkClient = this.client
            if (sdkClient) {
              const profileResult = await sdkClient.sonderr.profile()
            ...
            console.error("[Sonderr New] SonderrProvider: ❌ Failed during connected state handling:", error)
            this.postMessage({
              type: "error",
      ...
      // Subscribe to notification dismiss broadcast from other SonderrProvider instances
      this.unsubscribeNotificationDismiss = this.connectionService.onNotificationDismissed(() => {
        this.fetchAndSendNotifications()
      })

      // Subscribe to language change broadcast from other SonderrProvider instances
      ...
      // Subscribe to profile change broadcast from other SonderrProvider instances
      this.unsubscribeProfileChange = this.connectionService.onProfileChanged((data) => {
        this.postMessage({ type: "profileData", data })
      })

      // Subscribe to favorites change broadcast from other SonderrProvider instances
      this.unsubscribeFavoritesChange = this.connectionService.onFavoritesChanged((favorites) => {
        this.postMessage({ type: "favoritesLoaded", favorites })
      })

      // Subscribe to model-selector expand/collapse broadcast from other SonderrProvider instances
      this.unsubscribeModelSelectorExpanded = this.connectionService.onModelSelectorExpandedChanged((value) => {
        this.postMessage({ type: "modelSelectorExpandedLoaded", value })
      })

      // legacy-migration start
      // Subscribe to migration-complete broadcast from any SonderrProvider instance
      ...
      console.log("[Sonderr New] SonderrProvider: ✅ initializeConnection completed successfully")
    } catch (error) {
      console.error("[Sonderr New] SonderrProvider: ❌ Failed to initialize connection:", error)
      this.connectionState = "error"
      this.postMessage({
        type: "connectionState",
        state: "error",
  ...
  private async handleCreateSession(): Promise<void> {
    ...
    try {
      ...
      console.error("[Sonderr New] SonderrProvider: Failed to create session:", error)
      this.postMessage({
        type: "error",
  ...
  private refreshSessionDetails(sessionID: string, dir: string, signal?: AbortSignal): void {
    ...
    this.client.session
      ...
      .catch((e: unknown) => console.warn("[Sonderr New] SonderrProvider: getSession failed (non-critical):", e))
  ...
  private sync(sessionID: string, dir: string, signal?: AbortSignal, refresh?: number): void {
    ...
    void client.session
      ...
      .catch((error: unknown) => console.error("[Sonderr New] SonderrProvider: Failed to fetch session statuses:", error))
  }
  ...
  private fetchAndSendSessionModelUsage(sessionID: string, requestID: string): Promise<void> {
    ...
    return this.connectionService
      ...
      .catch((error: unknown) => {
        console.warn("[Sonderr New] SonderrProvider: Failed to load session model usage:", error)
  ...
  private async handleLoadMessages(
    ...
    try {
      ...
      if (abort?.signal.aborted) return
      console.error("[Sonderr New] SonderrProvider: Failed to load messages:", error)
  ...
  private async handleSyncSession(sessionID: string, parentSessionID?: string): Promise<void> {
    ...
    try {
      ...
      console.error("[Sonderr New] SonderrProvider: Failed to sync child session:", err)
    }
  }
  ...
  private async flushPendingSessionRefresh(reason: string): Promise<void> {
    ...
    console.log("[Sonderr New] SonderrProvider: 🔄 Flushing deferred sessions refresh", { reason })
    ...
    try {
      ...
      console.error("[Sonderr New] SonderrProvider: Failed to flush session refresh:", error)
    }
  ...
  private async handleLoadSessions(): Promise<void> {
    ...
    try {
      ...
      console.error("[Sonderr New] SonderrProvider: Failed to load sessions:", error)
      this.postMessage({
        type: "error",
  ...
  private async handleTerminalContext(requestId: string): Promise<void> {
    try {
      ...
      console.error("[Sonderr New] Failed to capture terminal context:", error)
      this.postMessage({
  ...
  private async handleDeleteSession(sessionID: string): Promise<void> {
    ...
    try {
      ...
      console.error("[Sonderr New] SonderrProvider: Failed to delete session:", error)
      this.postMessage({
        type: "error",
  ...
  private async handleDeleteMessage(sessionID: string, messageID: string, requestID?: string): Promise<void> {
    ...
    try {
      ...
      console.error("[Sonderr New] SonderrProvider: Failed to delete message:", error)
      this.postMessage({
        type: "error",
  ...
  private async handleRenameSession(sessionID: string, title: string): Promise<void> {
    try {
      ...
      console.error("[Sonderr New] SonderrProvider: Failed to rename session:", error)
  ...
  private async handleExportSessionTranscript(sessionID: string): Promise<void> {
    ...
    try {
      ...
      console.error("[Sonderr New] SonderrProvider: Failed to export session transcript:", error)
      this.postMessage({
        type: "error",
    ...
    const task = (async () => {
      ...
      while (true) {
        ...
        try {
          ...
          const settings = vscode.workspace.getConfiguration("sonderr-code.new.model")
          const message = {
          ...
          console.error("[Sonderr New] SonderrProvider: Failed to fetch providers:", error)
        }
        if (!this.providersQueued) return
        generation = this.providersGeneration
      }
  ...
  private async fetchAndSendAgents(): Promise<void> {
    ...
    try {
      ...
      console.error("[Sonderr New] SonderrProvider: Failed to fetch agents:", error)
    }
  }
  ...
  private async fetchAndSendSkills(): Promise<void> {
    ...
    try {
      ...
      console.error("[Sonderr New] SonderrProvider: Failed to fetch skills:", error)
    }
  }
  ...
  private async fetchAndSendCommands(): Promise<void> {
    ...
    try {
      ...
      console.error("[Sonderr New] SonderrProvider: Failed to fetch commands:", error)
    }
  }
  ...
  private async removeSkillViaCli(location: string): Promise<boolean> {
    ...
    try {
      ...
      console.error("[Sonderr New] Failed to remove skill:", error)
      this.cachedSkillsMessage = null
      this.cachedCommandsMessage = null
      await Promise.all([this.fetchAndSendSkills(), this.fetchAndSendCommands()])
      return false
  ...
  private async handleRemoveAgent(name: string): Promise<void> {
    ...
    if (!result.success) {
      console.error("[Sonderr New] Failed to remove agent:", result.error)
  ...
  private async handleRemoveMcp(name: string): Promise<void> {
    ...
    if (!removed) {
      console.error("[Sonderr New] SonderrProvider: Failed to remove MCP server:", name)
    }
  }
  ...
  private async fetchAndSendMcpStatus(): Promise<void> {
    ...
    try {
      ...
      console.error("[Sonderr New] SonderrProvider: Failed to fetch MCP status:", error)
    }
  }
  ...
  private async fetchAndSendConfig(): Promise<void> {
    ...
    try {
      ...
      console.error("[Sonderr New] SonderrProvider: Failed to fetch config:", error)
    }
  }
  ...
  private async fetchAndSendGlobalConfig(): Promise<void> {
    ...
    try {
      ...
      console.error("[Sonderr New] SonderrProvider: Failed to fetch global config:", error)
    }
  }
  ...
  private async fetchAndSendIndexingStatus(directory?: string, projectId?: string): Promise<void> {
    ...
    try {
      ...
      console.error("[Sonderr New] SonderrProvider: Failed to fetch indexing status:", error)
    }
  }

  private async fetchAndSendSonderrEmbeddingModels(): Promise<void> {
    const catalog = await fetchSonderrEmbeddingModelCatalog()
    const message = { type: "sonderrEmbeddingModelsLoaded", catalog }
    this.cachedSonderrEmbeddingModelsMessage = message
    this.postMessage(message)
  }
  ...
  private async fetchAndSendBackgroundJobs(sessionID: string, requestID: string): Promise<void> {
    ...
    try {
      ...
      console.error("[Sonderr New] SonderrProvider: Failed to fetch background jobs:", error)
      this.postMessage({
  ...
  private async cancelBackgroundJob(jobID: string, sessionID: string, requestID: string): Promise<void> {
    ...
    try {
      ...
      console.error("[Sonderr New] SonderrProvider: Failed to cancel background job:", error)
      this.postMessage({
        type: "backgroundJobsLoaded",
        sessionID,
        requestID,
  ...
  private async promoteBackgroundJob(jobID: string, sessionID: string): Promise<void> {
    ...
    try {
      ...
      console.error("[Sonderr New] SonderrProvider: Failed to promote background job:", error)
    }
  }
  ...
  private async fetchAndSendConfigUpdated(): Promise<void> {
    ...
    try {
      ...
      console.error("[Sonderr New] SonderrProvider: Failed to fetch config after update:", error)
    }
  }
  ...
  private async checkConfigWarnings(from: string): Promise<void> {
    if (this.configWarningsShown) {
      console.log("[Sonderr New] SonderrProvider: config warnings already shown", { from })
      return
    }
    if (!this.client) {
      console.log("[Sonderr New] SonderrProvider: config warnings skipped (no client)", { from })
      return
    }
    try {
      const dir = this.getWorkspaceDirectory()
      console.log("[Sonderr New] SonderrProvider: checking config warnings", { from, dir })
      const result = await this.client.config.warnings({ directory: dir })
      const list = result?.data ?? []
      console.log("[Sonderr New] SonderrProvider: config warnings fetched", { from, count: list.length })
      if (list.length === 0) return
      this.configWarningsShown = true

      const first = list[0]!
      const summary = list.length === 1 ? first.message : `${first.message} (and ${list.length - 1} more)`
      console.warn("[Sonderr New] SonderrProvider: showing config warnings", { from, count: list.length, path: first.path })
      ...
      if (action === "Show Details") {
        ...
        const channel = vscode.window.createOutputChannel("Sonderr Config Warnings")
        channel.clear()
        channel.appendLine(lines.join("\n\n"))
        channel.show()
      }
    } catch (err) {
      console.warn("[Sonderr New] SonderrProvider: checkConfigWarnings failed:", { from, err })
    }
  }
  ...
  // Cloud session methods extracted to sonderr-provider/handlers/cloud-session.ts
  ...
  private sendNotificationSettings(): void {
    const attention = vscode.workspace.getConfiguration("sonderr-code.new.attention")
    this.postMessage({
  ...
  private async runToggleSandbox(
    ...
    try {
      ...
      const remembered = await this.connectionService.sandboxPreference
        ...
        .then(() => true)
        .catch((error) => {
          console.error("[Sonderr New] Failed to persist sandbox default:", error)
          return false
        })
      this.postMessage({
        type: "sandboxStatus",
  ...
  private async handleUpdateConfig(
    ...
    try {
      ...
      ]).catch((error) => console.error("[Sonderr New] SonderrProvider: Post-config refresh failed:", error))
    } catch (error) {
      this.postConfigFailure(error, completed, snapshot, dir)
    } finally {
      this.pending--
  ...
  private postConfigFailure(
    ...
    console.error("[Sonderr New] SonderrProvider: Failed to update config:", error)
  ...
  private async withRetry(
    ...
    try {
      for (let attempt = 1; ; attempt++) {
        ...
        console.log(`[Sonderr New] SonderrProvider: Retry on ${status}, attempt ${attempt}/${MAX_RETRIES}, delay ${delay}ms`)
  ...
  private maxCostSetting(): number {
    return this.setMaxCost(vscode.workspace.getConfiguration("sonderr-code.new").get<number>("maxCost", 0))
  }

  private commitMessageLanguageSetting(): string {
    return vscode.workspace.getConfiguration("sonderr-code.new").get<string>("languageCommitMessage", "sync")
  }

  private multiProjectSetting(): boolean {
    return vscode.workspace.getConfiguration("sonderr-code.new.experimental").get<boolean>("multiProject", false)
  }
  ...
  private async syncIndexingConsent(
    ...
    const auth = Buffer.from(`sonderr:${config.password}`).toString("base64")
    const res = await fetch(`${config.baseUrl}/indexing/consent`, {
      method: "PUT",
      headers: {
        Authorization: `Basic ${auth}`,
        "Content-Type": "application/json",
        "x-sonderr-directory": dir,
      },
  ...
  private async handleSendMessage(
    ...
    try {
      ...
      await runWithMessageConfirmation(this.confirmations, messageID, "SonderrProvider: Message request", () =>
        this.withRetry(
          () =>
      ...
      console.error("[Sonderr New] SonderrProvider: Failed to send message:", error)
      this.postMessage({
        type: "sendMessageFailed",
  ...
  private async handleSendCommand(
    ...
    try {
      ...
      await this.checkpoints.get(sid)
      await runWithMessageConfirmation(this.confirmations, messageID, "SonderrProvider: Command request", () =>
        this.withRetry(
          () =>
      ...
      console.error("[Sonderr New] SonderrProvider: Failed to send command:", error)
      this.postMessage({
        type: "sendMessageFailed",
  ...
  private async handleResumeSession(sessionID: string, messageID: string, requestID: string): Promise<void> {
    try {
      ...
      console.error("[Sonderr New] Failed to resume session:", error)
      this.postMessage({
  ...
  private async handleRevertSession(sessionID: string, messageID: string, partID?: string): Promise<void> {
    ...
    if (error) {
      console.error("[Sonderr New] SonderrProvider: Failed to revert session:", error)
  ...
  private async handleUnrevertSession(sessionID: string): Promise<void> {
    ...
    if (error) {
      console.error("[Sonderr New] SonderrProvider: Failed to unrevert session:", error)
  ...
  private async handleCompact(sessionID?: string, providerID?: string, modelID?: string): Promise<void> {
    ...
    if (!target) {
      console.error("[Sonderr New] SonderrProvider: No sessionID for compact")
      return
    }

    if (!providerID || !modelID) {
      console.error("[Sonderr New] SonderrProvider: No model selected for compact")
      this.postMessage({
        type: "error",
    ...
    try {
      ...
      console.error("[Sonderr New] SonderrProvider: Failed to compact session:", error)
      this.postMessage({
        type: "error",
  ...
  // Permission + question handlers extracted to sonderr-provider/handlers/permission.ts and question.ts
  ...
  // Cloud session handlers extracted to sonderr-provider/handlers/cloud-session.ts
  ...
  // Auth handlers extracted to sonderr-provider/handlers/auth.ts
  ...
  private async disposeGlobal(): Promise<void> {
    ...
    await this.client.global
      .dispose()
      .catch((e: unknown) => console.warn("[Sonderr New] SonderrProvider: global.dispose() after org switch failed:", e))

    // Org switch succeeded — refresh profile and providers independently (best-effort)
    try {
      const profileResult = await this.client!.sonderr.profile()
      ...
      console.error("[Sonderr New] SonderrProvider: Failed to refresh profile after org switch:", error)
    }
    try {
      await this.fetchAndSendProviders()
    } catch (error) {
      console.error("[Sonderr New] SonderrProvider: Failed to refresh providers after org switch:", error)
    }
  }

  /**
   * Handle a generic setting update from the webview.
   * The key uses dot notation relative to `sonderr-code.new` (e.g. "browserAutomation.enabled").
   */
  private async handleUpdateSetting(key: string, value: unknown): Promise<void> {
    ...
    const config = vscode.workspace.getConfiguration(`sonderr-code.new${section ? `.${section}` : ""}`)
  ...
  /**
   * Reset all "sonderr-code.new.*" extension settings to their defaults by reading
   * contributes.configuration from the extension's package.json at runtime.
   * Only resets settings under the "sonderr-code.new." namespace to avoid touching
   * settings from the previous version of the extension which shares the same
   * extension ID and "sonderr-code.*" namespace.
   */
  private async handleResetAllSettings(): Promise<void> {
    const confirmed = await vscode.window.showWarningMessage(
      "Reset all Sonderr extension settings to defaults?",
      { modal: true },
      "Reset",
    )
    if (confirmed !== "Reset") return

    const prefix = "sonderr-code.new."
    const ext = vscode.extensions.getExtension("sonderr.sonderr-code")
    ...
    await this.extensionContext?.globalState.update("sonderr.dismissedNotificationIds", undefined)
    await this.extensionContext?.globalState.update("sonderr.agentMigrationBannerDismissed", undefined)
    await this.extensionContext?.globalState.update("sonderr.marketplace.dismissedSuggestions", undefined)
    ...
    vscode.window.showInformationMessage("Sonderr settings have been reset to defaults.")
  }
  ...
  private sendBrowserSettings(): void {
    const config = vscode.workspace.getConfiguration("sonderr-code.new.browserAutomation")
    this.postMessage({
  ...
  private sendClaudeCompatSetting(): void {
    const enabled = vscode.workspace.getConfiguration("sonderr-code.new").get<boolean>("claudeCodeCompat", false)
    this.postMessage({
  ...
  private async handleReload(): Promise<void> {
    if (!this.client) {
      console.warn("[Sonderr New] handleReload: no client connection")
      return
    }
    ...
    try {
      ...
      console.error("[Sonderr New] handleReload: reload endpoint failed:", err)
  ...
  private handleEvent(event: ProviderEvent, directory?: string): void {
    if (event.type === "sonderr-sessions.remote-status-changed") {
    ...
    // so this runs on every SonderrProvider instance — including the Settings panel which has no
    ...
    if (event.type === "message.part.updated") {
      ...
      if (childId && !this.trackedSessionIds.has(childId)) {
        console.log("[Sonderr New] SonderrProvider: 🔗 Auto-adopting child session from task tool", { childId })
    ...
    if (!this.webview) {
      const type =
        ...
        typeof (message as { type?: unknown }).type === "string"
          ...
          : "<unknown>"
      console.warn("[Sonderr New] SonderrProvider: ⚠️ postMessage dropped (no webview)", { type })
      return
    }

    void this.webview.postMessage(message).then(undefined, (error) => {
      console.error("[Sonderr New] SonderrProvider: ❌ postMessage failed", error)
    })
  }

  private flushPendingSonderrModel(): void {
    if (!this.webview || !this.isWebviewReady || !this.client || !this.pendingSonderrModel) return

    const pending = this.pendingSonderrModel
    this.pendingSonderrModel = null
    this.postMessage({ type: "selectSonderrModel", ...pending })
  }
  ...
  public async appendReviewComments(comments: unknown[], autoSend = false): Promise<void> {
    ...
    if (!this.webview) {
      await vscode.commands.executeCommand(`${SonderrProvider.viewType}.focus`)
    }
  ...
  public async toggleMemory(sessionID?: string): Promise<void> {
    try {
      ...
      console.error("[Sonderr New] SonderrProvider: Failed to toggle memory:", error)
  ...
  private async getGitRemoteUrl(): Promise<string | undefined> {
    try {
      ...
      console.warn("[Sonderr New] SonderrProvider: Failed to get git remote URL:", error)
      return undefined
    }
  }
   ...
   * given directory, filtered through .sonderrignore.
   */
  ...
  private async gatherEditorContext(dir?: string): Promise<EditorContext> {
    ...
    // Visible files (capped to avoid bloating context, filtered through .sonderrignore)
    const visibleFiles = [
      ...new Set(
        [
    ...
    // Active file (also filtered through .sonderrignore)
  ...
  private getWorkspaceDirectory(sessionId?: string): string {
    ...
    if (routed === null)
      console.warn(`[Sonderr New] SonderrProvider: session ${sessionId} is ambiguous across projects, using workspace root`)
  ...
  private getSessionDirectory(sessionId: string, session?: Session): string {
    ...
    if (routed === null)
      console.warn(
        `[Sonderr New] SonderrProvider: session ${sessionId} is ambiguous across projects, using tracked directory`,
      )
    if (routed) return routed
  ...
  private async recoverSessionGitStatus(parts: unknown[], sessionID: string, cursor?: string): Promise<void> {
    ...
    const history = await retry(() =>
      ...
      console.warn("[Sonderr New] SonderrProvider: Failed to recover session Git directory:", error)
      return undefined
    })
    if (!history) {
      this.sessionGitRecoveries.delete(sessionID)
  ...
  // legacy-migration start -------------------------------------------------------
  // Migration handlers extracted to sonderr-provider/handlers/migration.ts
