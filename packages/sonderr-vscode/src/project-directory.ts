 ...
 * ## Why this exists
 *
 * The sidebar SonderrProvider always uses `getWorkspaceDirectory()`, which returns
 ...
 *
 * ## How SonderrProvider uses this
 *
 * Each SonderrProvider instance can receive an explicit `projectDirectory` via
 * `SonderrProviderOptions`. When set:
 *
...
/**
 * Resolve the effective project directory for a SonderrProvider instance.
 *
 * @param override - Explicit directory from SonderrProviderOptions. `undefined`
 ...
 *   `getWorkspaceDirectory(sessionId)` from SonderrProvider).
 */
