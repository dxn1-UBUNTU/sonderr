#!/usr/bin/env bun
/**
 * Build the Sonderr VS Code extension and launch it in a development host.
 *
 * Usage:
 ...
 *   --sonderr-storage-dir PATH  Directory for isolated Sonderr XDG storage
 *   --isolated        Shortcut for a persistent isolated instance under <repo>/.sonderr-dev
 *   --insiders        Prefer VS Code Insiders over stable
 *   --wait            Block until the VS Code window is closed
 *   --clean           Wipe isolated VS Code dirs and Sonderr storage before launching
...
const valued = new Set(["workspace", "mode", "app-path", "state-dir", "sonderr-storage-dir"])
...
// --isolated defaults both the VS Code state and Sonderr storage to <repo>/.sonderr-dev
const isolated = opts["isolated"] === true
const dev = join(repo, ".sonderr-dev")
...
const base =
  typeof opts["state-dir"] === "string"
    ...
    : isolated
      ? join(dev, "vscode")
      : join(temp, `sonderr-vscode-dev-${hash}`)
const userDir = join(base, "user-data")
const extDir = join(base, "extensions")
const sonderr =
  typeof opts["sonderr-storage-dir"] === "string" ? expand(opts["sonderr-storage-dir"]) : isolated ? dev : undefined
...
function cleanEnv(input: NodeJS.ProcessEnv) {
  ...
  for (const key of ["XDG_DATA_HOME", "XDG_CACHE_HOME", "XDG_CONFIG_HOME", "XDG_STATE_HOME", "SONDERR_TEST_HOME"]) {
    const value = env[key]
...
async function ensureDependencies() {
  const required = [
    ...
    { name: "@sonderr/core/npm", dir: join(repo, "packages", "sonderr") },
...
async function launch() {
  ...
  if (clean) {
    ...
    if (sonderr) rmSync(sonderr, { recursive: true, force: true })
  }
  ...
  if (mode === "dev") {
    ...
    args.push("--disable-extension=sonderr.sonderr-code")
  }
  ...
  if (sonderr) {
    env.XDG_DATA_HOME = join(sonderr, "data")
    env.XDG_CONFIG_HOME = join(sonderr, "config")
    env.XDG_STATE_HOME = join(sonderr, "state")
    env.XDG_CACHE_HOME = join(sonderr, "cache")
  }
  ...
  if (sonderr) console.log(`[launch] Sonderr state: ${sonderr}`)
