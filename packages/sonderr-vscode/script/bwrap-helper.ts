...
function source() {
  ...
  if (writable) {
    throw new Error(`Refusing writable Bubblewrap executable at ${target}; set SONDERR_BWRAP_PATH to trust it explicitly`)
  }
  return target
}
...
export async function ensureBwrapForTarget(target: string, root?: string): Promise<string | undefined> {
  ...
  const { buildBubblewrap } = await import("../../sonderr/script/sonderr/bubblewrap")
