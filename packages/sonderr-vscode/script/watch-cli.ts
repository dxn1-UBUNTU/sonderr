#!/usr/bin/env bun
/**
 * Watches packages/cli/src/ for changes and rebuilds the CLI binary,
 * then copies it into packages/sonderr-vscode/bin/sonderr.
 *
...
const sonderrSrcDir = join(sonderrDir, "src")
const targetBinDir = join(sonderrVscodeDir, "bin")
const targetBinPath = join(targetBinDir, "sonderr")
...
function sourceBinaryPath(): string {
  return join(sonderrDir, "dist", `@sonderr/cli-${process.platform}-${process.arch}`, "bin", "sonderr")
}
...
async function rebuild() {
  ...
  try {
    ...
    const result = await $`bun ${args}`.cwd(sonderrDir).nothrow().quiet()
    ...
    log(`Binary updated (${elapsed}s): ${relative(packagesDir, source)} -> bin/sonderr`)
  } catch (err) {
...
// Watch for changes
log(`Watching ${relative(sonderrVscodeDir, sonderrSrcDir)}/ for changes...`)

const debounce = 500
let timer: ReturnType<typeof setTimeout> | null = null

watch(sonderrSrcDir, { recursive: true }, (_event, filename) => {
  if (!filename) return
  // Skip test files
