...
/**
 * Force all solid-js imports (from sonderr-ui and the webview) to resolve to
...
const solidDedupePlugin = {
  name: "solid-dedupe",
  setup(build) {
    // Resolve these bare specifiers to the sonderr-vscode-local copy
...
/**
 * Route the shared `@sonderr/ui/pierre/worker` module (and its relative
 * variants) to the Sonderr implementation in `webview-ui/pierre-worker.ts`.
 *
 * The upstream module loads Pierre's Shiki worker via a Vite-only
 * `?worker&url` import that esbuild can't resolve. The Sonderr replacement loads
...
const markdownWorkerUrlPlugin = {
  ...
  setup(build) {
    build.onResolve({ filter: /markdown-shiki\.worker\.ts\?worker&url$/ }, () => ({
      ...
      namespace: "sonderr-worker-url",
    }))
    build.onLoad({ filter: /.*/, namespace: "sonderr-worker-url" }, () => ({
      contents: "export default window.SONDERR_MARKDOWN_SHIKI_WORKER_URI",
      loader: "js",
    }))
  },
}
...
/**
 * Resolve the synthetic `sonderr-shiki-worker` entry point to Pierre's Shiki worker
...
const shikiWorkerEntryPlugin = {
  ...
  setup(build) {
    build.onResolve({ filter: /^sonderr-shiki-worker$/ }, async () => {
    ...
    build.onLoad({ filter: /sprite\.svg$/ }, (args) => {
      ...
      return {
        contents: `
          ...
          const inject = () => {
            if (!document.getElementById("sonderr-sprite")) {
              const el = document.createElement("div");
              el.id = "sonderr-sprite";
...
function getWebviewsConfig() {
  return {
    entryPoints: {
      ...
      sonderrclaw: "webview-ui/sonderrclaw/index.tsx",
...
function getShikiWorkerConfig() {
  return {
    entryPoints: ["sonderr-shiki-worker"],
    bundle: true,
    format: "iife",
    minify: production,
    sourcemap: !production,
