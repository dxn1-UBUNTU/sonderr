...
import ai.sonderr.client.session.views.tool.EditToolView
import ai.sonderr.client.session.views.tool.ApprovalReasonTarget
 ...
 * driven by [ai.sonderr.client.session.ui.SessionLayout] so that each
 ...
 * messages is owned by [ai.sonderr.client.session.ui.SessionLayout].
 */
class MessageView(
    val msg: Message,
...
) : ai.sonderr.client.session.ui.SessionLayoutPanel(
     ...
     * for lookup/streaming but invisible so [ai.sonderr.client.session.ui.SessionLayout] skips it and
    ...
    private inner class PromptWrap(
        ...
        private fun queue(onDelete: () -> Unit) = Stack.horizontal(UiStyle.Gap.sm()).also { row ->
            row.isOpaque = false
            row.next(JBLabel(SonderrBundle.message("session.queued")).apply {
                foreground = SessionUiStyle.Text.Secondary.foreground()
            })
            row.next(toolbarButton(
                ToolbarButtonAction(
                    AllIcons.Actions.Close,
                    SonderrBundle.message("session.queued.remove"),
                    onDelete,
                ),
            ))
        }
