...
class SonderrWorkspaceService internal constructor(
    private val cs: CoroutineScope,
    private val rpc: SonderrWorkspaceRpcApi?,
) {
    /** Platform constructor — resolves RPC from the service container. */
    constructor(cs: CoroutineScope) : this(cs, null)

    companion object {
        private val LOG = SonderrLog.create(SonderrWorkspaceService::class.java)
        private val INIT = SonderrWorkspaceStateDto(SonderrWorkspaceStatusDto.PENDING)
    }
    ...
    private suspend fun <T> call(block: suspend SonderrWorkspaceRpcApi.() -> T): T {
        val api = rpc
        return if (api != null) block(api) else durable { block(SonderrWorkspaceRpcApi.getInstance()) }
    }

    private fun <T> stream(block: suspend SonderrWorkspaceRpcApi.() -> Flow<T>): Flow<T> = flow {
        val api = rpc
        if (api != null) block(api).collect { emit(it) }
        else durable { block(SonderrWorkspaceRpcApi.getInstance()).collect { emit(it) } }
    }
