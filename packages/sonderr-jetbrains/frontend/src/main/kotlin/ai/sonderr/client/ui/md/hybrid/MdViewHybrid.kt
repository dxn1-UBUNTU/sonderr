package ai.sonderr.client.ui.md.hybrid

import ai.sonderr.client.session.ui.SessionSurface
...
import ai.sonderr.client.ui.diagram.Fault
import ai.sonderr.client.ui.diagram.Out
import ai.sonderr.client.ui.diagram.ui.DiagramBlock
import ai.sonderr.client.ui.diagram.ui.DiagramPanel
import ai.sonderr.client.ui.diagram.ui.Diagrams
import ai.sonderr.client.ui.diagram.ui.diagramPalette
import ai.sonderr.client.ui.diagram.ui.diagramSpec
import ai.sonderr.client.ui.diagram.ui.openDiagramWindow
...
internal open class MdViewHybrid(
    ...
    companion object {
        private val LOG = SonderrLog.create(MdViewHybrid::class.java)
    }
    ...
    private inner class DiagramView(desc: Desc.Code, kind: Kind.Diagram, disposable: Disposable) :
        ...
        private val label = JBLabel(SonderrBundle.message("diagram.rendering")).apply {
        ...
        init {
            ...
            panel.toolTipText = SonderrBundle.message("diagram.viewer.hint")
            panel.addMouseListener(click)
            panel.onFault = { fail(SonderrBundle.message("diagram.paint")) }
        ...
        private fun kick() {
            val item = desc as Desc.Code
            if (!Registry.`is`("sonderr.diagram.inline.enabled", true)) {
                status("")
                showSource()
                return
            }
            if (item.open) {
                showSource()
                status(SonderrBundle.message("diagram.rendering"))
                return
            }
        ...
        private fun fail(out: Out.Err) {
            if (out.fault == Fault.Unsupported) {
                status(SonderrBundle.message("diagram.unsupported"))
                showSource()
                root.revalidate()
                root.repaint()
                return
        ...
        private fun fail(message: String) {
            val text = message.ifBlank { SonderrBundle.message("diagram.rendering") }
            status(SonderrBundle.message("diagram.error", text), true)
            showSource()
            root.revalidate()
            root.repaint()
        }
