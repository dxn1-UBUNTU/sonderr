...
@State(
    name = "SonderrLogSettings",
    storages = [Storage("sonderrLogSettings.xml")],
)
class SonderrLogSettingsService : PersistentStateComponent<SonderrLogSettingsService.State> {
    ...
    fun apply(app: SonderrAppService = service()) {
        applyLocal()
        app.applyLogConfigAsync(dto())
    }

    companion object {
        fun getInstance(): SonderrLogSettingsService = service()
    }
}
