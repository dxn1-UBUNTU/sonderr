...
import ai.sonderr.client.ui.PickerButton
import ai.sonderr.client.ui.picker.PickerPopup
import ai.sonderr.client.ui.picker.popupBackground
...
class ModelPicker : PickerButton() {
    ...
    var emptyText: String = SonderrBundle.message("settings.models.notSet")
    ...
    init {
        isEnabled = false
        text = " "
        toolTipText = SonderrBundle.message("model.picker.tooltip")
    ...
    private fun refresh() {
        ...
        toolTipText = if (item?.let(ModelText::collectsData) == true) ModelText.dataCollectedTooltip() else SonderrBundle.message("model.picker.tooltip")
        isEnabled = true
...
internal const val MODEL_PICKER_EXPANDED_KEY = "sonderr.model.picker.expanded"
...
internal object ModelText {
    private val small = setOf("sonderr-auto/small", "auto-small")
    ...
    fun buttonLabel(item: ModelPicker.Item): String {
        ...
        if (item.provider == "sonderr") return part
        val provider = item.providerName.trim()
        if (provider.isEmpty()) return part
        return "$provider / $part"
    }

    fun small(item: ModelPicker.Item): Boolean = item.provider == "sonderr" && item.id in small

    fun providerSort(id: String): Int = if (id == "sonderr") 0 else 1

    fun dataCollected(): String = SonderrBundle.message("model.picker.dataCollected")

    fun dataCollectedTooltip(): String = XmlStringUtil.wrapInHtmlLines(
        SonderrBundle.message("model.picker.tooltip"),
        SonderrBundle.message("model.picker.dataCollected.current"),
    )

    fun freeLabel(): String = SonderrBundle.message("model.picker.free")
