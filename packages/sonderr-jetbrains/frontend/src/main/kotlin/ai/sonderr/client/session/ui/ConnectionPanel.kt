...
import ai.sonderr.client.actions.SonderrActionPlaces
...
import ai.sonderr.client.session.controller.SessionControllerListener
...
class ConnectionPanel(
    ...
    companion object {
        internal const val CLI_GROUP_ID = "Sonderr.CliGroup"
    ...
    private val retry = ActionLink(SonderrBundle.message("session.connection.retry")) {
        showRecoveryPopup()
    }.apply {
        isVisible = false
    ...
    private fun showConnecting() {
        ...
        label.text = SonderrBundle.message("session.connection.connecting")
        detail = null
        expanded = false
    ...
    private fun showDownloading(percent: Int, version: String?, platform: String?) {
        ...
        label.text = if (version != null && platform != null) {
            SonderrBundle.message("session.connection.downloading.version", version, platform, pct)
        } else {
            SonderrBundle.message("session.connection.downloading", pct)
        }
        detail = null
        expanded = false
        toggle.isVisible = false
    ...
    private fun showRecoveryPopup() {
        JBPopupFactory.getInstance()
            .createActionGroupPopup(
                ...
                true,
                SonderrActionPlaces.connectionRetryPopup(),
            )
    ...
    private fun recoveryGroup(): ActionGroup {
        ...
        group.add(object : DumbAwareAction(SonderrBundle.message("session.connection.retry")) {
            override fun actionPerformed(e: AnActionEvent) {
                controller.retryConnection()
            }
        })
        group.addSeparator()
        ActionManager.getInstance().getAction("Sonderr.Restart")?.let { group.add(it) }
        ActionManager.getInstance().getAction("Sonderr.Reinstall")?.let { group.add(it) }
        return group
    }
