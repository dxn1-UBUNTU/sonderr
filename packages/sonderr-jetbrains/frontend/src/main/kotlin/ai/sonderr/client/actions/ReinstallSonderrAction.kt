...
class ReinstallSonderrAction : AnAction(), DumbAware {
    override fun actionPerformed(e: AnActionEvent) {
        Telemetry.send("CLI Reinstall Clicked", mapOf("surface" to "settings"))
        service<SonderrAppService>().reinstallAsync()
    }

    override fun update(e: AnActionEvent) {
        e.presentation.isEnabled = true
        if (e.place == SonderrActionPlaces.connectionRetryPopup()) {
            e.presentation.text = SonderrBundle.message("action.Sonderr.Reinstall.cli.text")
        }
    }
}
