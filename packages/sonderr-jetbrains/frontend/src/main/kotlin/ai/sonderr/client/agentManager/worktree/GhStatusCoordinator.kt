...
class GhStatusCoordinator(
    ...
    companion object {
        private val LOG = SonderrLog.create(GhStatusCoordinator::class.java)
    ...
    private fun probe(reason: String) {
        ...
        cs.launch {
            runCatching {
                val dir = project.sonderrRoot() ?: return@runCatching null
                LOG.info("gh probe dir=$dir")
                service<SonderrWorktreeService>().ghStatus(dir)
            }
                .onSuccess { next ->
                    if (next == null) {
    ...
    private fun notify(project: Project?, value: GhAvailability) {
        ...
        if (value == GhAvailability.GIT_MISSING) {
            SonderrNotifications.suggestion(
                target,
                SonderrBundle.message("worktree.git.missing.title"),
                SonderrBundle.message("worktree.git.missing.content"),
                SonderrBundle.message("worktree.gh.learnMore"),
        ...
        if (value == GhAvailability.MISSING) {
            SonderrNotifications.suggestion(
                target,
                SonderrBundle.message("worktree.gh.missing.title"),
                SonderrBundle.message("worktree.gh.missing.content"),
                SonderrBundle.message("worktree.gh.learnMore"),
        ...
        SonderrNotifications.suggestion(
            target,
            SonderrBundle.message("worktree.gh.unauth.title"),
            SonderrBundle.message("worktree.gh.unauth.content"),
            SonderrBundle.message("worktree.gh.authorize"),
        ) {
            if (target == null) {
