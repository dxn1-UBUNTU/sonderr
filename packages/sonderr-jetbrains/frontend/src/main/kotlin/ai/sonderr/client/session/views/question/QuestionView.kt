package ai.sonderr.client.session.views.question
...
class QuestionView(
    ...
    private val back = HoverIcon().apply {
        ...
        toolTipText = SonderrBundle.message("session.question.back")
    ...
    private val fwd = HoverIcon().apply {
        ...
        icon = ico
        disabledIcon = IconLoader.getDisabledIcon(ico)
        toolTipText = SonderrBundle.message("session.question.next")
    ...
    private fun syncPage() {
        ...
        body.removeAll()
        if (review(q)) {
            setHeader(SonderrBundle.message("session.question.review.title"))
            addReview(q)
        } else {
            val item = q.items[idx]
            val hint = SonderrBundle.message(
    ...
    private fun syncHeader(q: Question) {
        ...
        summary.text = SonderrBundle.message("session.question.summary", shown, total)
    ...
    private fun syncFooter(q: Question) {
        ...
        actions.add(DialogView.Action(ID_DISMISS, SonderrBundle.message("session.question.dismiss"), primary = false) { doReject() })

        if (review(q)) {
            actions.add(DialogView.Action(ID_BACK, SonderrBundle.message("session.question.back"), primary = false) { goBack() })
            actions.add(DialogView.Action(ID_MAIN, SonderrBundle.message("session.question.submit"), primary = true) { doReply() })
        } else {
            val label = when {
                direct(q) -> SonderrBundle.message("session.question.submit")
                lastItem(q) -> SonderrBundle.message("session.question.review")
                else -> SonderrBundle.message("session.question.next")
            }
    ...
    private fun reviewRow(item: QuestionItem, i: Int): JPanel {
        ...
        val answer = text(
            joined.ifBlank { SonderrBundle.message("session.question.review.notAnswered") },
    ...
    private fun customRow(item: QuestionItem, set: MutableSet<String>): JPanel {
        ...
        val label = text(SonderrBundle.message("session.question.custom.label"), SessionUiStyle.Colors.foreground(), true)
    ...
    private fun buildCustomEditor(): SessionEditorTextField {
        ...
        ed.setPlaceholder(SonderrBundle.message("session.question.custom.placeholder"))
