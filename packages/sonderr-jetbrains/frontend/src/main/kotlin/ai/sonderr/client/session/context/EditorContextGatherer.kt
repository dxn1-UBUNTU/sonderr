...
internal object EditorContextGatherer {
    private val LOG = SonderrLog.create(EditorContextGatherer::class.java)
    ...
    fun gather(project: Project, root: String): Result {
        if (!SonderrPluginSettings.getAutoEditorContext()) {
        ...
        val ignore = project.service<SonderrIgnoreCache>().matcher(rootDir(listOfNotNull(activeFile) + openFiles, base))
    ...
    private fun selection(editor: Editor, root: Path, ignore: SonderrIgnore): PromptPartDto? {
    ...
    private fun local(file: VirtualFile, root: Path): Path? {
        if (file.fileSystem.protocol == SonderrVirtualFileSystem.PROTOCOL) return null
