...
internal object MdCommon {
    ...
    private val single = setOf("readme.md", "package.json", "tsconfig.json", "jsconfig.json", "sonderr.json", "sonderr.jsonc")
    private const val REF_SEGMENT_LIMIT = 16_384

    /** Anchor class [ai.sonderr.client.ui.md.hybrid.MdProjector]'s code-span linkifier tags its links with. */
    const val URL_REF_CLASS = "sonderr-url-ref"
    ...
    fun rules(opts: MdStyle): String {
        ...
        rules.append("a.sonderr-file-ref, code a.sonderr-file-ref { color: ${hex(SessionUiStyle.View.Markdown.string())}; font-family: '${css(opts.codeFont)}', monospace; text-decoration: underline } ")
    ...
    private fun paths(text: String): String {
        ...
        return ref.replace(text) { match ->
            ...
            val href = path.replace(" ", "%20")
                .replace("(", "%28")
                .replace(")", "%29")
            "<a class=\"sonderr-file-ref\" href=\"${attr(href)}\">$path</a>"
        }
    }
