...
import ai.sonderr.client.diff.SonderrDiffEditorKind
import ai.sonderr.client.diff.SonderrInlineDiffStore
import ai.sonderr.client.diff.diffParams
...
import ai.sonderr.client.migration.MigrationUiState
import ai.sonderr.client.migration.ui.MigrationWizardPanel
...
import ai.sonderr.client.session.subagent.SubagentSessionEditorKind
import ai.sonderr.client.session.subagent.SubagentTitleCache
import ai.sonderr.client.session.subagent.ensureSubagentSessionEditorKind
import ai.sonderr.client.session.subagent.subagentSessionParams
import ai.sonderr.client.session.ui.ConnectionPanel
import ai.sonderr.client.session.ui.empty.EmptySessionPanel
import ai.sonderr.client.session.ui.LoadingPanel
import ai.sonderr.client.session.ui.ReasoningPicker
import ai.sonderr.client.session.ui.RevertBanner
import ai.sonderr.client.session.ui.mode.ModePicker
import ai.sonderr.client.session.ui.model.ModelPicker
import ai.sonderr.client.session.ui.prompt.SonderrPromptCompletionProvider
import ai.sonderr.client.session.ui.prompt.MentionAction
import ai.sonderr.client.session.ui.prompt.PromptDataKeys
import ai.sonderr.client.session.ui.prompt.PromptPanel
import ai.sonderr.client.session.ui.prompt.SlashAction
import ai.sonderr.client.session.ui.prompt.mentionParts as promptMentionParts
import ai.sonderr.client.session.settings.ApprovalReasonVisibilityListener
import ai.sonderr.client.session.ui.account.SessionAccountOverlay
import ai.sonderr.client.session.ui.popup.HeaderPopupController
import ai.sonderr.client.session.ui.SessionDropOverlay
import ai.sonderr.client.session.ui.SessionRootPanel
import ai.sonderr.client.session.ui.SessionMessageListPanel
import ai.sonderr.client.session.ui.attachment.AttachmentEditorKind
import ai.sonderr.client.session.ui.attachment.attachmentParams
import ai.sonderr.client.session.ui.attachment.ensureAttachmentEditorKind
import ai.sonderr.client.session.ui.attachment.isEmbeddedAttachment
import ai.sonderr.client.agentManager.worktree.SonderrWorktreeService
import ai.sonderr.client.session.ui.header.BranchDock
import ai.sonderr.client.session.ui.header.SessionHeaderPanel
import ai.sonderr.client.session.ui.selection.SessionContextMenu
import ai.sonderr.client.session.ui.selection.SessionHoverCopyOverlay
...
import ai.sonderr.client.session.controller.EVENT_FLUSH_MS
import ai.sonderr.client.session.controller.PromptSelection
import ai.sonderr.client.session.controller.SessionController
import ai.sonderr.client.session.controller.SessionControllerEvent
import ai.sonderr.client.session.context.EditorContextGatherer
import ai.sonderr.client.session.ui.style.SessionUiStyle
import ai.sonderr.client.session.views.LoginRequiredView
import ai.sonderr.client.session.views.SessionOutcomeView
import ai.sonderr.client.session.views.permission.PermissionView
import ai.sonderr.client.session.views.question.QuestionView
...
import ai.sonderr.rpc.dto.SessionRevertDto
...
class SessionUi(
    project: Project,
    workspace: Workspace,
    sessions: SonderrSessionService,
    ...
    private val migration: MigrationUiController = service<SonderrMigrationService>(),
    private val timers: UiTimerSource = UiTimers,
) : JPanel(BorderLayout()), Disposable, SessionEditorStyleTarget, UiDataProvider, SessionActions {

    companion object {
        private val LOG = SonderrLog.create(SessionUi::class.java)
    ...
    private val flushMs =
        Registry.intValue("sonderr.session.flushMs", EVENT_FLUSH_MS.toInt())
            .takeIf { it > 0 }
            ?.toLong()
            ?: EVENT_FLUSH_MS
    ...
    private val controller = SessionController(
        ...
        comp = this,
        flushMs = flushMs,
        condense = Registry.`is`("sonderr.session.condense", true),
        displayMs = displayMs,
    ...
    private lateinit var completion: SonderrPromptCompletionProvider
    ...
    private fun onShareResult(url: String?, err: Throwable?, on: Boolean) {
        if (err != null) {
            ...
            SonderrNotifications.error(project, SonderrBundle.message(key))
            return
        }
        if (!on) {
            SonderrNotifications.info(project, SonderrBundle.message("session.unshare.done"))
            return
        }
        val link = url ?: return
        CopyPasteManager.getInstance().setContents(StringSelection(link))
        SonderrNotifications.info(
            project,
            SonderrBundle.message("session.share.done"),
            link,
            SonderrBundle.message("session.share.open"),
    ...
    override fun uiDataSnapshot(sink: DataSink) {
        ...
        // the reused Sonderr.StopSession action works from any right-click. Nearest-provider-wins keeps
    ...
    private fun buildUi() {
        ...
        completion = SonderrPromptCompletionProvider(
            workspace = workspace,
            service = workspaces,
    ...
    private fun syncPromptRevert() {
        ...
        val text = msg.parts.values.filterIsInstance<ai.sonderr.client.session.model.Text>().firstOrNull()?.content?.toString() ?: return
        prompt.setText(text)
    ...
    private fun slashActions(): List<SlashAction> {
        val fns: Map<SlashAction.Spec, () -> Unit> = mapOf(
            ...
            SlashAction.SETTINGS to { openSonderrSettings() },
    ...
    private fun bind(spec: SlashAction.Spec, action: () -> Unit) = SlashAction(
        spec.name,
        SonderrBundle.message(spec.descriptionKey),
        spec.hints,
        {
    ...
    private fun openInlineDiff(files: List<DiffFileDto>, title: String, key: String) {
        ...
        cs.launch {
            ...
            val label = branch?.let { SonderrBundle.message("diff.editor.inline.title.named", title, it) } ?: title
            LOG.info("open inline diff session=${controller.id ?: "pending"} dir=${ChatLogSummary.dir(dir)} files=${files.size}")
            withContext(Dispatchers.Main) {
                ensureDiffEditorKind()
                project.service<SonderrInlineDiffStore>().put(key, files)
                project.service<SonderrVfsManager>().open(
                    SonderrDiffEditorKind.ID,
    ...
    private fun openBranchDiff(branch: String?) {
        ...
        ensureDiffEditorKind()
        val dir = workspace.directory
        val title = branch?.let { SonderrBundle.message("diff.editor.branch.title.named", it) }
            ?: SonderrBundle.message("diff.editor.branch.title")
        project.service<SonderrVfsManager>().open(
            SonderrDiffEditorKind.ID,
    ...
    private fun refreshBranch() {
        ...
        branchJob?.cancel()
        branchJob = cs.launch {
            val status = runCatching { service<SonderrWorktreeService>().branchStatus(workspace.directory) }
                .getOrElse {
                    if (it is CancellationException) throw it
    ...
    private fun openAttachment(messageId: String, item: FileAttachment) {
        ...
        if (isEmbeddedAttachment(url)) {
            ...
            LOG.info("kind=attachment-open route=sonderr-vfs session=$id message=$messageId part=${item.id} name=${attachmentName(item)}")
    ...
    private fun openSonderrSettings() {
        ShowSettingsUtil.getInstance().showSettingsDialog(
            project,
            Predicate { cfg: Configurable ->
                cfg is ConfigurableWithId && cfg.getId() == SonderrSettingsConfigurable.ID
            },
            { _: Configurable -> },
        )
    }
...
private fun ai.sonderr.client.session.model.QuestionItem.planFollowup() =
