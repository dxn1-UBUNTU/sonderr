...
internal val DIFF_FILE_KEY: Key<String> = Key.create("sonderr.diff.file")
...
internal class DiffEditorView(
    ...
    private val banner = EditorNotificationPanel(EditorNotificationPanel.Status.Warning).apply {
        text(SonderrBundle.message("diff.editor.outdated"))
        createActionLabel(SonderrBundle.message("diff.editor.refresh")) { refresh() }
        isVisible = false
    }
    ...
    private val openFileAction = object : DumbAwareAction(
        SonderrBundle.message("diff.editor.openFile"),
        SonderrBundle.message("diff.editor.openFile"),
        ...
        override fun actionPerformed(e: AnActionEvent) {
            ...
            scope.launch { service<SonderrWorkspaceService>().openFile(path) }
        }
    }
    ...
    private fun installMenu() {
        val group = DefaultActionGroup(
            ...
            TreeAction(SonderrBundle.message("diff.editor.refresh"), AllIcons.Actions.Refresh, ::refresh),
        )
    ...
    private fun labels(): Pair<String, String> {
        if (params["source"] == "branch") {
            return SonderrBundle.message("diff.editor.side.base") to SonderrBundle.message("diff.editor.side.current")
        }
        return SonderrBundle.message("diff.editor.side.original") to SonderrBundle.message("diff.editor.side.modified")
    }
...
internal fun emptyChangesComponent(): JComponent = JPanel(BorderLayout()).apply {
    add(com.intellij.ui.components.JBLabel(SonderrBundle.message("diff.editor.empty")), BorderLayout.CENTER)
}
...
internal fun treeToolbarGroup(tree: Tree, refresh: () -> Unit) = DefaultActionGroup(
    TreeAction(SonderrBundle.message("diff.editor.refresh"), AllIcons.Actions.Refresh, refresh),
    Separator.getInstance(),
    TreeAction(SonderrBundle.message("diff.editor.tree.expandAll"), AllIcons.Actions.Expandall) { expandAll(tree) },
    TreeAction(SonderrBundle.message("diff.editor.tree.collapseAll"), AllIcons.Actions.Collapseall) { collapseAll(tree) },
)

private fun fileCount(count: Int): String = SonderrBundle.message(
