...
import ai.sonderr.client.diff.DiffLineNumbers
...
import ai.sonderr.client.session.ui.popup.HeaderPopupBody
import ai.sonderr.client.session.ui.popup.HeaderPopupRequest
...
import ai.sonderr.client.session.views.base.HeaderOpenAction
...
class EditToolView(
    ...
    private val open = HeaderOpenAction(SessionViewIcons.openDiff, SonderrBundle.message("session.part.tool.openDiff"), ::openDiffViewer)
     ...
     * known, so [ai.sonderr.client.session.views.MessageView] rebinds it once the opener is wired.
     */
    @RequiresEdt
    ...
    private fun sync(): Boolean {
        ...
        val titleText = if (count > 1) SonderrBundle.message("session.part.tool.patch") else title(item)
    ...
    private fun syncFilesTag(count: Int): Boolean {
        ...
        if (show) changed = setText(filesTag, SonderrBundle.message("session.part.tool.edit.files", count)) || changed
        return changed
    }
...
private fun diffTitle(tool: Tool): String =
    ...
    if (editFiles(tool).size > 1) SonderrBundle.message("session.part.tool.patch") else tail(editPath(tool))
