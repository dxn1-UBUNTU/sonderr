...
import ai.sonderr.client.session.views.tool.ToolApprovalFooter
import ai.sonderr.client.session.views.tool.approvalReasonsVisible
...
class QuestionResultView(
    ...
    fun bodyText(): String = result.questions.mapIndexed { i, q ->
        ...
        listOf(q, joined.ifBlank { SonderrBundle.message("session.question.review.notAnswered") }).joinToString("\n")
    }.joinToString("\n")
    ...
    private fun syncLabels() {
        parts.title.text = SonderrBundle.message("session.question.result.title")
        val count = result.answers.count { it.isNotEmpty() }
        parts.sub.text = SonderrBundle.message("session.question.result.answered", count)
