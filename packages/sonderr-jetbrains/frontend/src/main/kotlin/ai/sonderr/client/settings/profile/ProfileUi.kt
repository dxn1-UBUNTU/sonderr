...
internal const val PASS_URL = "https://kilo.ai/pricing/sonderr-pass"
...
internal class ProfileUi(
    ...
    private val account = LoggedInProfileUi(
        ...
        pass = {
            telemetry("Sonderr Pass Opened", mapOf("surface" to "settings"))
            browse(PASS_URL)
        },
        logout = ::logout,
    ...
    init {
        border = JBUI.Borders.empty(0, UiStyle.Gap.xl(), 0, UiStyle.Gap.xl())
     ...
     * account content) when [SonderrAppStateDto.progress]`.profile` is [ProfileStatusDto.PENDING],
     ...
     */
    @RequiresEdt
    fun update(state: SonderrAppStateDto) {
        checkEdt()
     ...
     */
    @RequiresEdt
    fun update(profile: ProfileDto?, status: SonderrAppStatusDto) {
        checkEdt()
        this.status = status
    ...
    private fun targetCard(): Card {
        ...
        val transientLoad = s == SonderrAppStatusDto.DOWNLOADING || s == SonderrAppStatusDto.CONNECTING || s == SonderrAppStatusDto.LOADING || s == SonderrAppStatusDto.MIGRATION_REQUIRED
        if (transientLoad && shown == Card.LOGGED_IN) return Card.LOGGED_IN
        return when {
            s == SonderrAppStatusDto.DISCONNECTED || transientLoad -> Card.LOGGED_OUT
            s == SonderrAppStatusDto.ERROR -> Card.LOGGED_OUT
     ...
     * Called from [ai.sonderr.client.settings.profile.UserProfileConfigurable.disposeUIResources].
     */
    @RequiresEdt
    fun dispose() {
        attempt++
...
internal fun compactLoginError(e: Exception): String {
    val msg = e.message?.trim() ?: return SonderrBundle.message("profile.login.failed")
    val lower = msg.lowercase()
    if (HTML_MARKERS.any { lower.contains(it) }) {
        val status = HTTP_STATUS_RE.find(msg)?.groupValues?.getOrNull(1)
        return if (status != null) "${SonderrBundle.message("profile.login.failed")} ($status)"
        else SonderrBundle.message("profile.login.failed")
    }
    val norm = msg.replace(Regex("\\s+"), " ")
    val summary = norm.take(180)
    return if (summary.isNotBlank()) summary else SonderrBundle.message("profile.login.failed")
}
