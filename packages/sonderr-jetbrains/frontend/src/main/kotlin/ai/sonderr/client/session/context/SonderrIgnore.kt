 ...
 *  - if `.sonderrignore` exists and is non-empty, use only its patterns (plus the
 *    `.sonderrignore` file itself);
 ...
 * `/`), and the `*`, `**`, `?`, and `[..]` globs.
 */
internal class SonderrIgnore private constructor(private val rules: List<Rule>) {
    ...
    companion object {
        val EMPTY = SonderrIgnore(emptyList())

        const val SONDERR = ".sonderrignore"
         ...
         */
        fun load(root: VirtualFile?): SonderrIgnore {
            if (root == null) return EMPTY
            val sonderr = read(root, SONDERR)
            if (!sonderr.isNullOrBlank()) return SonderrIgnore(compile(sonderr) + compile(SONDERR))
            val rules = mutableListOf<Rule>()
            read(root, GIT)?.takeIf { it.isNotBlank() }?.let { rules += compile(it) }
            rules += SENSITIVE.mapNotNull { rule(it) }
            return SonderrIgnore(rules)
        }

        /** Test seam: build a matcher directly from ignore-file text. */
        fun of(text: String): SonderrIgnore = SonderrIgnore(compile(text))
