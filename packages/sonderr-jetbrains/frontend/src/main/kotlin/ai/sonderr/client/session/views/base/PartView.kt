 ...
 * to [ai.sonderr.client.session.views.MessageView] without an extra component wrapper.
 *
...
abstract class PartView : JPanel(), Disposable, SessionEditorStyleTarget {
     ...
     * Called when [ai.sonderr.client.session.model.SessionModelEvent.ContentUpdated] fires.
     */
    abstract fun update(content: Content)

    /**
     * Append a streaming delta to the existing content.
     * Only meaningful for text-bearing renderers ([ai.sonderr.client.session.views.TextView],
     * [ai.sonderr.client.session.views.ReasoningView]); others ignore deltas by default.
     */
