...
import ai.sonderr.client.session.model.SessionHeaderSnapshot
...
import ai.sonderr.client.session.views.todo.TodoListPanel
...
class SessionHeaderPanel(
    ...
    companion object {
        ...
        internal const val EXPANDED_KEY = "sonderr.session.header.expanded"
    }
    ...
    private val compact = HoverIcon().apply {
        icon = COMPRESS_ICON
        cursor = java.awt.Cursor.getPredefinedCursor(java.awt.Cursor.HAND_CURSOR)
        toolTipText = SonderrBundle.message("session.header.compact.description")
        accessibleContext.accessibleName = SonderrBundle.message("session.header.compact")
        addActionListener { controller.compact() }
    }
    private val expand = JBLabel().apply {
        border = JBUI.Borders.empty(0, UiStyle.Gap.sm())
        cursor = java.awt.Cursor.getPredefinedCursor(java.awt.Cursor.HAND_CURSOR)
        toolTipText = SonderrBundle.message("session.header.expand")
        accessibleContext.accessibleName = SonderrBundle.message("session.header.expand")
        addMouseListener(object : MouseAdapter() {
            override fun mouseClicked(event: MouseEvent) {
                toggle()
            }
    ...
    private val tokenTitle = JBLabel(SonderrBundle.message("session.header.tokens"))
    ...
    private val todoRow = JPanel(FlowLayout(FlowLayout.LEFT, UiStyle.Gap.sm(), 0)).apply {
        ...
        cursor = java.awt.Cursor.getPredefinedCursor(java.awt.Cursor.HAND_CURSOR)
        toolTipText = SonderrBundle.message("session.header.todos.toggle")
        accessibleContext.accessibleName = SonderrBundle.message("session.header.todos.toggle")
        add(todoArrow)
        add(todos)
    }
    ...
    private fun setTokens(value: TokensDto?) {
        ...
        tokenTitle.text = SonderrBundle.message("session.header.tokens")
        tokens.toolTipText = SonderrBundle.message("session.header.tokens.description")
        tokenTitle.isVisible = total > 0
        set(input, if (sent > 0) num(sent) else null)
        set(output, if (received > 0) num(received) else null)
        set(cacheWrite, if (write > 0) SonderrBundle.message("session.header.cache.write", num(write)) else null)
        set(cacheRead, if (read > 0) SonderrBundle.message("session.header.cache.read", num(read)) else null)
    ...
    private fun setCost(value: String?) {
        ...
        cost.text = costValue
        cost.icon = null
        val tip = costValue.takeIf { it.isNotBlank() }?.let { SonderrBundle.message("session.header.cost.tooltip", it) }
    ...
    private fun setExpand(expanded: Boolean) {
        ...
        expand.toolTipText = SonderrBundle.message(key)
        expand.accessibleContext.accessibleName = SonderrBundle.message(key)
    }
