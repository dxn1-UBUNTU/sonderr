...
internal object AdvancedLogActions {
    ...
    private fun save(parent: JComponent, log: LogFileDto?) {
        if (log == null) {
            notify(NotificationType.WARNING, SonderrBundle.message("settings.advanced.logs.backend.missing"))
            return
        }
        val descriptor = FileSaverDescriptor(
            SonderrBundle.message("settings.advanced.logs.backend.save.title"),
            SonderrBundle.message("settings.advanced.logs.backend.save.description"),
            "log",
        )
        ...
        ApplicationManager.getApplication().executeOnPooledThread {
            runCatching { wrapper.file.writeText(log.content, Charsets.UTF_8) }
                .onSuccess { notify(NotificationType.INFORMATION, SonderrBundle.message("settings.advanced.logs.backend.saved", wrapper.file.name)) }
                .onFailure { notify(NotificationType.ERROR, SonderrBundle.message("settings.advanced.logs.backend.failed"), it.message) }
        }
    }
