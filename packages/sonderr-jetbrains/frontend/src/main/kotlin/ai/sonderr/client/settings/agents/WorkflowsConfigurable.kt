...
class WorkflowsConfigurable : AgentBehaviorConfigurableBase<JComponent>() {
    ...
    override fun getDisplayName(): String = SonderrBundle.message("settings.agentBehavior.workflows.displayName")
    ...
    companion object { const val ID = "ai.sonderr.jetbrains.settings.agentBehavior.workflows" }
}
...
internal class WorkflowsSettingsUi(
    ...
    override suspend fun fetch(): List<ActiveListItem> {
        val items = withTimeoutOrNull(WORKFLOW_LOAD_TIMEOUT_MS) {
            service<SonderrAgentBehaviorService>().loadCommandFiles(dir)
        } ?: throw SettingsMessageException(SonderrBundle.message("settings.agentBehavior.workflows.load.timeout"))
    ...
    override fun searchPlaceholder() = SonderrBundle.message("settings.agentBehavior.workflows.search")

    override fun emptyText() = SonderrBundle.message("settings.agentBehavior.workflows.empty")
    ...
    override fun applyDraft() {
        ...
        if (!launch("apply") { id ->
            ...
            if (failed == null) {
                for (location in target.deleted) {
                    if (!behavior.removeCommand(dir, location)) {
                        failed = SonderrBundle.message("settings.agentBehavior.workflows.delete.failed")
                        break
                    }
                }
            }
            ...
            withContext(workflowEdt) {
                if (!active(id)) {
                    if (failed == null) SonderrNotifications.info(SonderrBundle.message("settings.agentBehavior.workflows.saved.notification"))
                ...
                if (failed == null) {
                    ...
                    if (reloaded) clearProgress() else showProgress(SonderrBundle.message("settings.agentBehavior.workflows.reload.blocked"))
            ...
            val failed = SonderrBundle.message("settings.agentBehavior.save.failed")
            state.fail(token, failed)
            showError(failed)
            return
        }
    ...
    private fun item(flow: CommandFileDto) = object : ActiveListItem {
        ...
        override val cells = listOfNotNull(
            ActiveListCell(
                OPEN_CELL,
                SonderrBundle.message("settings.agentBehavior.workflows.openInEditor"),
                primary = true,
            ).takeIf { flow.editable },
            ActiveListCell(
                EDIT_CELL,
                SonderrBundle.message(if (flow.editable) "settings.agentBehavior.edit" else "common.open"),
    ...
    private fun open(flow: CommandFileDto) {
        ...
        if (!launch("open") { id ->
            val opened = service<SonderrWorkspaceService>().openFile(flow.location)
            withContext(workflowEdt) {
                if (!active(id)) return@withContext
                setBusy(false)
                if (opened) return@withContext
                clearProgress()
                SonderrNotifications.error(SonderrBundle.message("settings.agentBehavior.workflows.openInEditor.failed"))
            }
        }) return
        showProgress(SonderrBundle.message("settings.agentBehavior.workflows.openInEditor.pending"))
    }

    private fun remove(flow: CommandFileDto) {
        val result = Messages.showYesNoDialog(
            SonderrBundle.message("settings.agentBehavior.workflows.delete.message", flow.name),
            SonderrBundle.message("settings.agentBehavior.workflows.delete.title"),
            SonderrBundle.message("common.delete"),
    ...
    private companion object {
        ...
        val LOG = SonderrLog.create(WorkflowsSettingsUi::class.java)
...
internal class WorkflowEditDialog(private val flow: CommandFileDto, private val savable: Boolean) : DialogWrapper(true), WorkflowEditDialogHandle {
    ...
    private fun initial() = flow.content?.takeIf { it.isNotBlank() }
        ...
        ?: SonderrBundle.message("settings.agentBehavior.workflows.content.empty")
}
