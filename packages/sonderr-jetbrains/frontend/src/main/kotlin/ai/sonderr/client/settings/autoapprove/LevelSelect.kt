...
internal fun levelLabel(level: String): String = when (level) {
    "allow" -> SonderrBundle.message("settings.autoApprove.level.allow")
    "ask" -> SonderrBundle.message("settings.autoApprove.level.ask")
    "deny" -> SonderrBundle.message("settings.autoApprove.level.deny")
    else -> level
}
...
internal class LevelSelect(
    ...
    init {
        renderer = SimpleListCellRenderer.create("") { item ->
            when (item) {
                is Item.Default -> SonderrBundle.message("settings.autoApprove.default", levelLabel(item.resolved))
