...
internal abstract class BaseSettingsUi<C : BaseContentPanel, D, P, R, W>(
    ...
    protected var appState: SonderrAppStateDto = app.state.value
        private set
    ...
    @RequiresEdt
    private fun updateApp(state: SonderrAppStateDto) {
        appState = state
    ...
    private fun load() {
        ...
        if (appState.status != SonderrAppStatusDto.READY || workspaceLoading || workspaceLoaded) return
    ...
    @RequiresEdt
    protected abstract fun draft(state: SonderrAppStateDto): D
    ...
    @RequiresEdt
    protected open fun onSaveFailedAfterDispose(change: P) = SonderrNotifications.error(failedText())
    ...
    @RequiresEdt
    protected open fun unavailable(state: SonderrAppStateDto) = Unit
