package ai.sonderr.client.ui.picker
...
internal class PickerPopup<T>(
    ...
    private val emptyText: String = SonderrBundle.message("model.picker.no.matches"),
) {
    ...
    private val field = if (search) SearchTextField(false).apply {
        textEditor.emptyText.text = SonderrBundle.message("model.picker.search")
    } else null
    ...
    /**
     * Rebuilds the rows and restores the highlight. Unlike [ai.sonderr.client.ui.list.ActiveListView],
    ...
    private fun syncExpand() {
        ...
        expand.toolTipText = if (expanded) {
            SonderrBundle.message("model.picker.details.minimize")
        } else {
            SonderrBundle.message("model.picker.details.maximize")
        }
