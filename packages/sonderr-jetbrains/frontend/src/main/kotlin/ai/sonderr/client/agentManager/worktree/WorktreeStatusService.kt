...
class WorktreeStatusService internal constructor(
    ...
    companion object {
        private val LOG = SonderrLog.create(WorktreeStatusService::class.java)
    ...
    private fun loadStats() {
        cs.launch {
            val dir = project.sonderrRoot() ?: return@launch
            runCatching { service<SonderrWorktreeService>().stats(dir) }
    ...
    private fun loadPr() {
        cs.launch {
            val dir = project.sonderrRoot() ?: return@launch
            runCatching { service<SonderrWorktreeService>().prStatus(dir) }
                .onSuccess { dto ->
