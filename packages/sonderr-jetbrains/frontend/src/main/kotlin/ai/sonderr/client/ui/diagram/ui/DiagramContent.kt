...
internal fun diagramContent(source: String, parent: Disposable): JComponent {
    ...
    fun render() {
        ...
        label.text = SonderrBundle.message("diagram.rendering")
        ...
        service<Diagrams>().render(source, diagramSpec(style), parent) { out ->
            when (out) {
                ...
                is Out.Err -> {
                    ...
                    label.text = if (hint) SonderrBundle.message("diagram.unsupported") else SonderrBundle.message("diagram.error", out.message)
