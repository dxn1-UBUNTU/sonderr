...
internal fun modelPickerRows(
    ...
    emptyText: String = SonderrBundle.message("settings.models.notSet"),
    ...
    if (q.isBlank()) {
        ...
        if (fav.isNotEmpty()) {
            val section = SonderrBundle.message("model.picker.favorites")
            out += fav.map { ModelPickerRow(it, section, favorite = true) }
        }
    }
    if (recommended.isNotEmpty()) {
        val section = SonderrBundle.message("model.picker.recommended")
