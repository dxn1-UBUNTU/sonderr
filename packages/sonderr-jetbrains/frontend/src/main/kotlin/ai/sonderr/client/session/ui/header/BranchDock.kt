 ...
 *   the changes badge.
 *
 * The two actions are registered platform actions (`Sonderr.Chat.NewWorktree`, `Sonderr.Chat.MoveToWorktree`)
...
internal class BranchDock(
    ...
    private val group = DefaultActionGroup().apply {
        ActionManager.getInstance().getAction("Sonderr.Chat.NewWorktree")?.let { add(it) }
        ActionManager.getInstance().getAction("Sonderr.Chat.MoveToWorktree")?.let { add(it) }
    }
    ...
    private companion object {
        ...
        const val PLACE = "SonderrChatBranchDock"
    }
}
