...
class RevertBanner(
    ...
    private val diff = toolbarButton(
        ToolbarButtonAction(SessionViewIcons.openDiff, SonderrBundle.message("session.part.tool.openDiff"), ::openDiffViewer),
    ).apply {
        isEnabled = false
        isVisible = false
    }
    ...
    private val hint = JBLabel(SonderrBundle.message("revert.banner.hint")).apply {
        font = JBFont.small()
    }

    private val notice = JBLabel(SonderrBundle.message("revert.banner.filesNotRestored")).apply {
        font = JBFont.small()
    }
    ...
    init {
        ...
        setContent(body)
        setActions(listOf(
            DialogView.Action("redo", SonderrBundle.message("revert.banner.redo"), primary = false) { redoAction() },
            DialogView.Action("all", SonderrBundle.message("revert.banner.redo.all"), primary = false) { redoAllAction() },
        ))
    ...
    fun update() {
        ...
        title.text = SonderrBundle.message(if (total == 1) "revert.banner.count.one" else "revert.banner.count.other", total)
     ...
     */
    private fun resolveDiffs(revert: ai.sonderr.rpc.dto.SessionRevertDto): List<DiffFileDto> =
    ...
    private fun openDiffViewer() {
        ...
        openDiff(diffs, SonderrBundle.message("revert.banner.openDiff.title"), "revert:${sessionId ?: "pending"}:${revert.messageID}")
    }
