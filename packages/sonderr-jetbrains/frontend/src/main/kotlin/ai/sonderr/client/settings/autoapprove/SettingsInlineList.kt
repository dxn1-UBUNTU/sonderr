...
import ai.sonderr.client.settings.base.SettingsInlineListPanel
...
internal fun levelChoiceLabel(choice: LevelChoice): String = when (choice) {
    is LevelChoice.Default -> SonderrBundle.message("settings.autoApprove.default", levelLabel(choice.level))
...
internal class SettingsInlineList(
    ...
    internal var editInput: (String) -> String? = { key ->
        Messages.showInputDialog(
            this,
            placeholder,
            SonderrBundle.message("settings.autoApprove.edit"),
            null,
            key,
            null,
        )
    ...
    override fun toolbarActions(): List<AnAction> = buildList {
        ...
        if (add != null && onAdd != null) {
            add(SettingsToolbarAction(
                SonderrBundle.message("settings.autoApprove.add"),
                add,
        ...
        if (onRemove != null) {
            add(SettingsToolbarAction(
                SonderrBundle.message("settings.autoApprove.delete"),
                SonderrBundle.message("settings.autoApprove.delete.description"),
    ...
    private data class PermissionItem(val row: PermissionListRow) : ActiveListItem {
        ...
        override val cells: List<ActiveListCell> = buildList {
            if (row.editable) add(ActiveListCell(
                id = EDIT_CELL,
                label = SonderrBundle.message("settings.autoApprove.edit"),
            ))
            add(ActiveListCell(
                id = LEVEL_CELL,
                label = if (row.inherited) SonderrBundle.message(
