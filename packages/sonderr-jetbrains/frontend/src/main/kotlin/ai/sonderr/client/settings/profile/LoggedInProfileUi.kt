...
import ai.sonderr.client.ui.RoundedContentPanel
...
internal class LoggedInProfileUi(
    ...
    companion object {
        private val LOG = SonderrLog.create(LoggedInProfileUi::class.java)
    }
    ...
    private val logoLabel = JBLabel(IconLoader.getIcon("/icons/sonderr-profile.svg", LoggedInProfileUi::class.java)).apply {
        name = "sonderr.profile.logo.loggedIn"
        accessibleContext.accessibleName = SonderrBundle.message("settings.sonderr.displayName")
    }

    private val titleLabel = JBLabel(SonderrBundle.message("profile.balance.title")).apply {
        foreground = UiStyle.Colors.weak()
    }
    ...
    private val refreshBtn = JButton(SonderrBundle.message("profile.action.refresh"), AllIcons.Actions.Refresh)
        .also {
            it.isOpaque = false
    ...
    private val passTitleLabel = JBLabel(SonderrBundle.message("profile.pass.title")).apply {
    ...
    private val passBonusLabel = JBLabel(SonderrBundle.message("profile.pass.bonusLabel")).apply {
        foreground = UiStyle.Colors.weak()
        font = UiStyle.Fonts.small()
    }
    ...
    private val passRenewLabel = JBLabel(SonderrBundle.message("profile.pass.renewsLabel")).apply {
        foreground = UiStyle.Colors.weak()
        font = UiStyle.Fonts.small()
    }
    ...
    private val passMeter = PassMeter().apply {
        name = "sonderr.profile.passMeter"
    }
    ...
    private val passRenewRow = Stack.horizontal(UiStyle.Gap.md())
        ...
        .fill(UiStyle.Gap.md())
        .next(passRenewValue)
    private val passLink = ActionLink(SonderrBundle.message("profile.pass.subscribe")) {
        pass()
    }.apply {
        name = "sonderr.profile.passSubscribe"
    }
    ...
    private val passPanel = Stack.vertical(UiStyle.Gap.md()).apply {
        name = "sonderr.profile.passPanel"
        next(passInfo)
    ...
    private val balanceCard = RoundedContentPanel(UiStyle.Gap.pad(), UiStyle.Gap.xl()).apply {
        name = "sonderr.profile.balanceCard"
        addToTop(titleLabel)
    ...
    val dashboardBtn = JButton(SonderrBundle.message("profile.action.dashboard"))
        .also { it.addActionListener { dashboard() } }
    val topUpBtn = JButton(SonderrBundle.message("profile.action.topUp"))
        .also { it.addActionListener { topUp() } }
    val logoutBtn = JButton(SonderrBundle.message("profile.action.logout"))
    ...
    fun update(profile: ProfileDto) {
        ...
        val item = if (personal) profile.sonderrPass else null
    ...
    @RequiresEdt
    private fun syncPass(item: ProfileSonderrPassDto?, show: Boolean): Boolean {
        var changed = false
        ...
        if (item != null) {
            val usage = SonderrBundle.message(
                "profile.pass.used",
    ...
    fun setRefreshing(refreshing: Boolean) {
        ...
        val text = if (refreshing) SonderrBundle.message("profile.action.refreshing")
        else SonderrBundle.message("profile.action.refresh")
    ...
    private fun applyOrganizations(profile: ProfileDto) {
        ...
        val keys: List<Pair<String?, String>> = (if (personal) listOf(null to SonderrBundle.message("profile.personalAccount")) else emptyList()) +
