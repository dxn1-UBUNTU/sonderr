...
import ai.sonderr.client.session.ui.model.ModelSearch
...
internal data class ProviderListRow(
    ...
    override val badges: List<ActiveListBadge>
        get() = when (provider.source) {
            "env" -> listOf(ActiveListBadge(SonderrBundle.message("settings.providers.badge.env")))
            else -> emptyList()
        }
...
internal fun providerListActionText(action: ProviderListAction) = when (action) {
    ProviderListAction.CONNECT -> SonderrBundle.message("settings.providers.connect")
    ProviderListAction.OAUTH -> SonderrBundle.message("settings.providers.oauth")
    ProviderListAction.EDIT -> SonderrBundle.message("settings.providers.edit")
    ProviderListAction.DISCONNECT -> SonderrBundle.message("settings.providers.disconnect")
    ProviderListAction.DELETE -> SonderrBundle.message("settings.providers.delete")
    ProviderListAction.ENABLE -> SonderrBundle.message("settings.providers.enable")
}
...
internal fun providerListRows(state: ProviderSettingsDto, query: String, disabledRows: Boolean = false): List<ProviderListRow> {
    ...
    rows += connected.map { ProviderListRow(it, SonderrBundle.message("settings.providers.connected"), providerActions(it, state, disabled), disabled = disabledRows) }
    rows += popular.map { ProviderListRow(it, SonderrBundle.message("settings.providers.popular"), providerActions(it, state, disabled), disabled = disabledRows) }
    rows += all.map { ProviderListRow(it, SonderrBundle.message("settings.providers.all"), providerActions(it, state, disabled), disabled = disabledRows) }
    return rows
}
...
internal fun providerActions(
    ...
    if (provider.id == SONDERR_PROVIDER_ID && configured(provider, state, state.connected.toSet())) return emptyList()
