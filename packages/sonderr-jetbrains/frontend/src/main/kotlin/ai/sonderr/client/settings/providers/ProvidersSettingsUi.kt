...
import ai.sonderr.client.app.SonderrProviderService
import ai.sonderr.client.plugin.SonderrBundle
import ai.sonderr.client.settings.auth.DeviceOAuthInfo
import ai.sonderr.client.settings.auth.DeviceOAuthPanel
import ai.sonderr.client.settings.auth.DeviceOAuthText
import ai.sonderr.client.settings.base.BaseContentPanel
import ai.sonderr.client.settings.base.SettingsPanel
...
import ai.sonderr.client.ui.picker.PickerListRenderer
import ai.sonderr.client.ui.picker.PickerPopup
import ai.sonderr.log.SonderrLog
import ai.sonderr.rpc.dto.CustomModelDto
...
import ai.sonderr.rpc.dto.ProviderAuthMethodDto
import ai.sonderr.rpc.dto.ProviderAuthOptionDto
...
import ai.sonderr.rpc.dto.ProviderSettingsProviderDto
...
internal fun customSaveError(id: String, result: ProviderActionResultDto): String? {
    ...
    if (result.state.providers.none { it.id == id }) return SonderrBundle.message("settings.providers.customNotUsable")
    return null
}
...
internal class ProvidersSettingsUi(
    ...
    companion object {
        val LOG = SonderrLog.create(ProvidersSettingsUi::class.java)
    }

    private val add = SettingsToolbarAction(
        SonderrBundle.message("settings.providers.addCustom"),
        SonderrBundle.message("settings.providers.addCustom.description"),
        AllIcons.General.Add,
        { !busy },
    ) { custom() }
    private val refresh = SettingsToolbarAction(
        SonderrBundle.message("settings.providers.refresh"),
        SonderrBundle.message("settings.providers.refresh.description"),
        AllIcons.Actions.Refresh,
        { !busy },
    ) { reload() }
    private val view = ProvidersContent(::connect, ::oauth, ::disconnect, ::enable, ::edit)
    private val search = SearchTextField(false).apply {
        textEditor.emptyText.text = SonderrBundle.message("settings.providers.search")
    }
    ...
    fun reload() {
        ...
        if (!launch("reload") { id ->
            val next = service<SonderrProviderService>().state(directory)
    ...
    private fun syncLoading() {
        checkEdt()
        showProgress(SonderrBundle.message("settings.providers.loading"))
    }
    ...
    private fun connect(provider: ProviderSettingsProviderDto) {
        ...
        if (!launch("connect provider=${provider.id}") { id ->
            val result = service<SonderrProviderService>().connect(ProviderConnectDto(directory, provider.id, key, metadata))
    ...
    private fun oauth(provider: ProviderSettingsProviderDto) {
        ...
        if (!launch("authorize provider=${provider.id}") { id ->
            val ready = service<SonderrProviderService>().authorize(ProviderOAuthAuthorizeDto(directory, provider.id, method))
                    ...
                    if (ready.method == "auto" && url != null) {
                        showOAuthDevice(
                            ...
                            DeviceOAuthInfo(
                                ...
                                expiresIn = (SonderrProviderService.OAUTH_RPC_TIMEOUT_MS / 1000).toInt(),
            ...
            val result = service<SonderrProviderService>().callback(ProviderOAuthCallbackDto(directory, provider.id, method, code))
            apply(id, result.state, result.error)
        }) return
        showProgress(
            SonderrBundle.message("settings.providers.oauth.starting", provider.name),
            SonderrBundle.message("settings.providers.oauth.cancel"),
        ) { cancelOAuth(request) }
    }
    ...
    private fun disconnect(provider: ProviderSettingsProviderDto) {
        ...
        if (!launch("disconnect provider=${provider.id}") { id ->
            val result = service<SonderrProviderService>().disconnect(ProviderDisconnectDto(directory, provider.id))
            apply(id, result.state, result.error)
        }) return
        syncLoading()
    }
    ...
    private fun enable(provider: ProviderSettingsProviderDto) {
        ...
        if (!launch("enable provider=${provider.id}") { id ->
            val result = service<SonderrProviderService>().enable(ProviderEnableDto(directory, provider.id))
            apply(id, result.state, result.error)
        }) return
        syncLoading()
    }
    ...
    private fun openCustomDialog(existing: CustomProviderEdit?) {
        ...
        val dialog = CustomProviderDialog(
            cs,
            directory,
            { service<SonderrProviderService>().fetchCustomModels(it) },
            { service<SonderrProviderService>().saveCustom(it) },
            existing,
        )
        if (!dialog.showAndGet()) return
    ...
    private fun syncOAuthWaiting(id: Int) {
        checkEdt()
        if (!active(id)) return
        val expiry = System.currentTimeMillis() + SonderrProviderService.OAUTH_RPC_TIMEOUT_MS
        fun text(): String {
            val ms = (expiry - System.currentTimeMillis()).coerceAtLeast(0)
            val remain = ((ms + 999) / 1000).toInt()
            val min = remain / 60
            val sec = remain % 60
            return SonderrBundle.message("settings.providers.oauth.waitingTimed", "$min:${sec.toString().padStart(2, '0')}")
        }
        stopTimer()
        showProgress(text(), SonderrBundle.message("settings.providers.oauth.cancel")) { cancelOAuth(id) }
        timer = Timer(1000) {
            if (!active(id)) {
                stopTimer()
                return@Timer
    ...
    private fun showOAuthDevice(id: Int, provider: ProviderSettingsProviderDto, info: DeviceOAuthInfo) {
        ...
        val panel = DeviceOAuthPanel(
            DeviceOAuthText(
                title = SonderrBundle.message("settings.providers.oauth.starting", provider.name),
                qrDescription = SonderrBundle.message("profile.login.qr.description"),
            ),
            cancel = { cancelOAuth(id) },
            browse = { BrowserUtil.browse(it) },
            prefix = "sonderr.provider.oauth",
        )
        oauth?.dispose()
        oauth = panel
        panel.update(info)
...
internal class ProvidersContent(
    ...
    private val view = ActiveListView(SonderrBundle.message("settings.providers.noMatches"), ActiveListConfig.Preferred) { key, id ->
        activate(key, id)
    }
    private var state = ProviderSettingsDto()
    private var busy = false
...
private class ApiKeyDialog(title: String, method: ProviderAuthMethodDto?) : DialogWrapper(true) {
    ...
    override fun createCenterPanel(): JComponent {
        ...
        panel.next(JBLabel(SonderrBundle.message("settings.providers.apiKey")))
        panel.next(key)
    ...
    override fun doValidate(): ValidationInfo? {
        if (key().isBlank()) return ValidationInfo(SonderrBundle.message("settings.providers.apiKeyRequired"), key)
        return null
    }
...
internal class CustomProviderDialog(
    ...
    private val pick = JButton(SonderrBundle.message("settings.providers.customSelectModels"))
    ...
    init {
        title = if (existing != null) {
            SonderrBundle.message("settings.providers.customEditTitle")
        } else {
            SonderrBundle.message("settings.providers.customTitle")
        }
        setOKButtonText(
            if (existing != null) SonderrBundle.message("settings.providers.customSave")
            else SonderrBundle.message("settings.providers.customAdd"),
        )
        init()
        initValidation()
    ...
    override fun createCenterPanel(): JComponent {
        val panel = Stack.vertical(UiStyle.Gap.sm())
        listOf(
            SonderrBundle.message("settings.providers.customId") to id,
            SonderrBundle.message("settings.providers.customName") to name,
            SonderrBundle.message("settings.providers.customUrl") to url,
            SonderrBundle.message("settings.providers.apiKey") to key,
            SonderrBundle.message("settings.providers.customEnv") to env,
        ).forEach { (label, field) ->
            panel.next(JBLabel(label))
            panel.next(field)
        }
        panel.next(JBLabel(SonderrBundle.message("settings.providers.customModels")))
    ...
    override fun doValidate(): ValidationInfo? {
        if (id.text.isBlank()) return ValidationInfo(SonderrBundle.message("settings.providers.customIdRequired"), id)
        if (url.text.isBlank()) return ValidationInfo(SonderrBundle.message("settings.providers.customUrlRequired"), url)
        actionError?.let { return ValidationInfo(it) }
        if (!fetching && modelIds().isEmpty()) return ValidationInfo(SonderrBundle.message("settings.providers.customModelsRequired"), models)
        return null
    }
    ...
    private fun selectModels() {
        ...
        job = cs.launch {
            ...
            withContext(edt) {
                ...
                if (ids.isEmpty()) {
                    fail(SonderrBundle.message("settings.providers.customModelsEmpty"))
                    return@withContext
                }
                showModelPopup(ids)
            }
    ...
    private fun startFetch() {
        ...
        models.text = SonderrBundle.message("settings.providers.customFetchingModels")
        syncActions()
    }
    ...
    private fun fetchValidationError(): String? {
        if (url.text.isBlank()) return SonderrBundle.message("settings.providers.customUrlRequired")
        if (!url.text.trim().let { it.startsWith("http://") || it.startsWith("https://") }) return SonderrBundle.message("settings.providers.customUrlInvalid")
        return null
    }
    ...
    private fun showModelPopup(ids: List<String>) {
        ...
        val select = ActionLink(SonderrBundle.message("settings.providers.customModelsSelectAll"))
        val clear = ActionLink(SonderrBundle.message("settings.providers.customModelsUnselectAll"))
    ...
    private fun syncActions() {
        ...
        pick.text = if (fetching) {
            SonderrBundle.message("settings.providers.customCancelModels")
        } else {
            SonderrBundle.message("settings.providers.customSelectModels")
        }
    }
