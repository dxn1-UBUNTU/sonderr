...
internal object HistoryTime {
    ...
    fun title(section: HistorySection): String = when (section) {
        HistorySection.TODAY -> SonderrBundle.message("history.group.today")
        HistorySection.YESTERDAY -> SonderrBundle.message("history.group.yesterday")
        HistorySection.WEEK -> SonderrBundle.message("history.group.week")
        HistorySection.MONTH -> SonderrBundle.message("history.group.month")
        HistorySection.OLDER -> SonderrBundle.message("history.group.older")
    }
    ...
    fun relative(item: HistoryItem, now: Long = System.currentTimeMillis()): String {
        ...
        if (diff < MINUTE) return SonderrBundle.message("history.time.moments")
        if (diff < HOUR) return SonderrBundle.message("history.time.minutes", (diff / MINUTE).coerceAtLeast(1))
        if (diff < DAY) return SonderrBundle.message("history.time.hours", (diff / HOUR).coerceAtLeast(1))
        if (diff < 7 * DAY) return SonderrBundle.message("history.time.days", (diff / DAY).coerceAtLeast(1))
        ...
        if (months < 1) return SonderrBundle.message("history.time.days", (diff / DAY).coerceAtLeast(1))
        if (months < 12) return SonderrBundle.message("history.time.months", months)
        return SonderrBundle.message("history.time.years", months / 12)
    }
