...
import ai.sonderr.client.ui.list.ACTIVE_LIST_DELETE_CELL
import ai.sonderr.client.ui.list.ACTIVE_LIST_RENAME_CELL
...
class HistoryPanel(
    ...
    private val repoOnly = JBCheckBox(SonderrBundle.message("history.cloud.repo.only"), true).apply {
        isVisible = false
    ...
    private val localInfo = TabInfo(localPanel)
        .setText(SonderrBundle.message("history.tab.local"))
        .setForeSideComponent(back())
    private val cloudInfo = TabInfo(cloudPanel)
        .setText(SonderrBundle.message("history.tab.cloud"))
        .setForeSideComponent(back())
    ...
    private fun search(model: HistoryModel<out HistoryItem>) = SearchTextField(false).apply {
        textEditor.emptyText.text = SonderrBundle.message("history.search.placeholder")
    ...
    private fun back(): JComponent {
        val label = SonderrBundle.message("history.back")
    ...
    private fun localList() = ActiveList(
        SonderrBundle.message("history.empty"),
    ...
    private fun syncList(list: ActiveList, model: HistoryModel<out HistoryItem>) {
        ...
        list.setEmptyText(when {
            model.loading -> SonderrBundle.message("history.loading")
            model.error != null -> model.error.orEmpty()
            else -> SonderrBundle.message("history.empty")
        })
    }
    ...
    private fun installContextMenu(list: ActiveList) {
        val group = ActionManager.getInstance().getAction("Sonderr.History.ContextMenu")
    ...
    private fun showDeletePopup(items: List<LocalHistoryItem>, cell: String? = null) {
        ...
        val msg = if (active.size == 1) {
            SonderrBundle.message("history.delete.confirm.message", title(active[0]))
        } else {
            SonderrBundle.message("history.delete.confirm.message.multiple", active.size)
        }
    ...
    private class LoadMoreButton : JButton(SonderrBundle.message("history.cloud.load.more")) {
