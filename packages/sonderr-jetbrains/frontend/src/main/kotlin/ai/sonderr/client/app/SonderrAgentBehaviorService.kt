...
class SonderrAgentBehaviorService internal constructor(
    private val cs: CoroutineScope,
    private val rpc: SonderrAgentBehaviorRpcApi?,
) {
    constructor(cs: CoroutineScope) : this(cs, null)

    companion object {
        private val LOG = SonderrLog.create(SonderrAgentBehaviorService::class.java)
    }

    private suspend fun <T> call(block: suspend SonderrAgentBehaviorRpcApi.() -> T): T {
        val api = rpc
        return if (api != null) block(api) else durable { block(SonderrAgentBehaviorRpcApi.getInstance()) }
    }
