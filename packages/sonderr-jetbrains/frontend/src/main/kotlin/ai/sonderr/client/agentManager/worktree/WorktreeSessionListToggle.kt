 ...
 *
 * Not a [ai.sonderr.client.ui.HoverIcon]: that hosts a single icon and pins icon-only buttons to
...
internal class WorktreeSessionListToggle(
    ...
    init {
        ...
        label(SonderrBundle.message("worktree.session.list.expand"))
        add(row)
    ...
    fun update(expanded: Boolean, count: Int, kind: SessionActivityKind?) {
        ...
        label(SonderrBundle.message(if (expanded) "worktree.session.list.collapse" else "worktree.session.list.expand"))
        revalidate()
        repaint()
    }
    ...
    private companion object {
        const val ACTIVATE = "sonderr.worktree.sessionList.activate"
