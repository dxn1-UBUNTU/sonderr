package ai.sonderr.client.migration.ui

import ai.sonderr.client.migration.MigrationItemUiProgress
import ai.sonderr.client.migration.MigrationSelectionBuilder
import ai.sonderr.client.migration.MigrationSettingsUiSelections
import ai.sonderr.client.migration.MigrationUiPhase
import ai.sonderr.client.migration.MigrationUiSelections
import ai.sonderr.client.migration.MigrationUiState
import ai.sonderr.client.migration.groupStatus
...
class MigrationWizardPanel : DialogView() {
    ...
    private val settingsRow = MigrationItemRow(SonderrBundle.message("migration.row.settings"), MigrationItemCategoryDto.settings)
    private val providerRow = MigrationItemRow(SonderrBundle.message("migration.row.providers"), MigrationItemCategoryDto.provider)
    private val mcpRow = MigrationItemRow(SonderrBundle.message("migration.row.mcp"), MigrationItemCategoryDto.mcpServer)
    private val modesRow = MigrationItemRow(SonderrBundle.message("migration.row.modes"), MigrationItemCategoryDto.customMode)
    private val sessionsRow = MigrationItemRow(SonderrBundle.message("migration.row.sessions"), MigrationItemCategoryDto.session)
    private val modelRow = MigrationItemRow(SonderrBundle.message("migration.row.model"), MigrationItemCategoryDto.defaultModel)

    private val keepBox = JBCheckBox(SonderrBundle.message("migration.keep_legacy_settings"), true)

    private val emptyLabel = JBLabel(SonderrBundle.message("migration.empty")).apply {
    ...
    init {
        ...
        setHeader(
            SonderrBundle.message("migration.migrate.title"),
            SonderrBundle.message("migration.migrate.subtitle"),
        )
        setContent(buildContent())
        setActions(
            listOf(
                DialogView.Action(ACTION_SKIP, SonderrBundle.message("migration.button.skip"), primary = false) {
                    onSkip?.invoke()
                },
                DialogView.Action(ACTION_LATER, SonderrBundle.message("migration.button.later"), primary = false) {
                    onLater?.invoke()
                },
                DialogView.Action(ACTION_MIGRATE, SonderrBundle.message("migration.button.migrate"), primary = true) {
                    onStart?.invoke(currentSelections())
                },
                DialogView.Action(ACTION_DONE, SonderrBundle.message("migration.button.done"), primary = true) {
                    onDone?.invoke()
                },
                DialogView.Action(ACTION_CONTINUE, SonderrBundle.message("migration.button.continue"), primary = true) {
    ...
    private fun updateButtons(phase: MigrationUiPhase, running: Boolean) {
        ...
        setActionText(
            ACTION_MIGRATE,
            if (running) SonderrBundle.message("migration.button.migrating") else SonderrBundle.message("migration.button.migrate"),
        )
