package ai.sonderr.client.session.ui.style
...
object SessionUiStyle {
    ...
    object Colors {
        ...
        fun sessionBackground(): Color = JBColor.lazy {
            UIManager.getColor("Sonderr.Session.background") ?: run {
    ...
    object View {
         ...
         * separating stacked content surfaces inside a [ai.sonderr.client.session.ui.SessionContentPanel].
        ...
        /** Markdown colors that mirror Sonderr's VS Code webview tokens. */
        object Markdown {
            fun string(): Color = JBColor.namedColor(
                "Sonderr.Session.Markdown.String",
                JBColor(0xA31515, 0xCE9178),
            )
        }

        object Todo {
            fun checkBg(): Color = JBColor.namedColor("Sonderr.Session.Todo.Checkbox.Background", Color.WHITE)

            fun checkFg(): Color = JBColor.namedColor("Sonderr.Session.Todo.Checkbox.Foreground", Color(0x1F, 0x23, 0x28))
    ...
    object Timeline {
        val READ: Color = JBColor.namedColor("Sonderr.Session.Timeline.Read", Color(0x37, 0x94, 0xff))
        val WRITE: Color = JBColor.namedColor("Sonderr.Session.Timeline.Write", Color(0x00, 0x7f, 0xd4))
        val TOOL: Color = JBColor.namedColor("Sonderr.Session.Timeline.Tool", Color(0x00, 0x7a, 0xcc))
        val SUCCESS: Color = JBColor.namedColor("Label.successForeground", UIUtil.getLabelSuccessForeground())
        val ERROR: Color = JBColor.namedColor("Sonderr.Session.Timeline.Error", UIUtil.getErrorForeground())
        val TEXT: Color = Text.Secondary.foreground()
        val STEP: Color = JBColor.namedColor("Sonderr.Session.Timeline.Step", JBColor.border())
    }
}
