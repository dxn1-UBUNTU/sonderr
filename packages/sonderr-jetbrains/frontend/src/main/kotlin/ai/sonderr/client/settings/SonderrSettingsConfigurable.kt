...
import ai.sonderr.client.settings.agents.AgentBehaviorConfigurable
import ai.sonderr.client.settings.autoapprove.AutoApproveConfigurable
import ai.sonderr.client.settings.context.ContextConfigurable
import ai.sonderr.client.settings.models.ModelsConfigurable
import ai.sonderr.client.settings.providers.ProvidersConfigurable
...
/**
 * Root settings entry under Settings -> Tools -> Sonderr.
 *
 * Displays a brief description and a link to the User Profile child page.
 * Child configurables are registered in XML (`sonderr.jetbrains.frontend.xml`) as
 ...
 */
class SonderrSettingsConfigurable : SearchableConfigurable {

    override fun getId(): String = ID

    override fun getDisplayName(): String = SonderrBundle.message("settings.sonderr.displayName")

    override fun createComponent(): JComponent {
        val panel = Stack.vertical()
        panel.border = JBUI.Borders.empty(UiStyle.Gap.lg(), 0, 0, 0)

        val desc = JBLabel(SonderrBundle.message("settings.sonderr.description"))
        desc.border = JBUI.Borders.emptyBottom(UiStyle.Gap.pad())
        panel.next(desc)

        val link = ActionLink(SonderrBundle.message("settings.profile.displayName")) { e ->
        ...
        val models = ActionLink(SonderrBundle.message("settings.models.displayName")) { e ->
            val src = e.source as? JComponent ?: return@ActionLink
            val settings = Settings.KEY.getData(DataManager.getInstance().getDataContext(src)) ?: return@ActionLink
        ...
        val providers = ActionLink(SonderrBundle.message("settings.providers.displayName")) { e ->
            val src = e.source as? JComponent ?: return@ActionLink
            val settings = Settings.KEY.getData(DataManager.getInstance().getDataContext(src)) ?: return@ActionLink
        ...
        val behavior = ActionLink(SonderrBundle.message("settings.agentBehavior.displayName")) { e ->
            val src = e.source as? JComponent ?: return@ActionLink
            val settings = Settings.KEY.getData(DataManager.getInstance().getDataContext(src)) ?: return@ActionLink
        ...
        val autoApprove = ActionLink(SonderrBundle.message("settings.autoApprove.displayName")) { e ->
            val src = e.source as? JComponent ?: return@ActionLink
            val settings = Settings.KEY.getData(DataManager.getInstance().getDataContext(src)) ?: return@ActionLink
        ...
        val context = ActionLink(SonderrBundle.message("settings.context.displayName")) { e ->
            val src = e.source as? JComponent ?: return@ActionLink
            val settings = Settings.KEY.getData(DataManager.getInstance().getDataContext(src)) ?: return@ActionLink
        ...
        val advanced = ActionLink(SonderrBundle.message("settings.advanced.displayName")) { e ->
            val src = e.source as? JComponent ?: return@ActionLink
            val settings = Settings.KEY.getData(DataManager.getInstance().getDataContext(src)) ?: return@ActionLink
    ...
    companion object {
        const val ID = "ai.sonderr.jetbrains.settings"
    }
}
