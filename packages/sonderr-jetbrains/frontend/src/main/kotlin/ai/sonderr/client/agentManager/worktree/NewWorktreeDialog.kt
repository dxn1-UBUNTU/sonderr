...
import ai.sonderr.client.session.ui.mode.modeItems
...
internal class NewWorktreeDialog(
    ...
    private val name = JBTextField(NAME_COLUMNS).apply {
        emptyText.text = SonderrBundle.message("worktree.dialog.name.placeholder")
    }
    private val completion = SonderrPromptCompletionProvider(
    ...
    private val url = JBTextField().apply {
        emptyText.text = SonderrBundle.message("worktree.import.pr.placeholder")
    }
    ...
    init {
        ...
        title = SonderrBundle.message("worktree.configure.title")
        init()
        setOKButtonText(SonderrBundle.message("worktree.dialog.create"))
    }
    ...
    override fun getDimensionServiceKey(): String = "ai.sonderr.NewWorktreeDialog.v3"
    ...
    private fun tabs(): JComponent {
        val fresh = TabInfo(newContent()).setText(SonderrBundle.message("worktree.dialog.tab.new"))
        val pr = TabInfo(prContent()).setText(SonderrBundle.message("worktree.dialog.tab.pr"))
        val local = TabInfo(branchContent()).setText(SonderrBundle.message("worktree.dialog.tab.branch"))
        val tabs: JBTabs = JBTabsFactory.createTabs(project, disposable).apply {
            ...
            addListener(object : TabsListener {
                ...
                override fun selectionChanged(oldSelection: TabInfo?, newSelection: TabInfo?) {
                    ...
                    setOKButtonText(SonderrBundle.message(if (tab == DialogTab.NEW) "worktree.dialog.create" else "worktree.dialog.import"))
    ...
    private fun prContent(): JComponent = importContent(SettingsStackedRow(
        SonderrBundle.message("worktree.import.pr.section"),
        description = SonderrBundle.message("worktree.import.pr.description"),
        value = url,
    ))

    private fun branchContent(): JComponent = importContent(SettingsStackedRow(
        SonderrBundle.message("worktree.import.branch.section"),
        description = SonderrBundle.message(if (pick.empty) "worktree.import.branch.empty" else "worktree.import.branch.description"),
        value = pick,
    ))
    ...
    private fun fields(): JComponent = object : FormBuilder() {
        ...
        .addLabeledComponent(SonderrBundle.message("worktree.configure.branch"), branch)
        .addLabeledComponent(SonderrBundle.message("worktree.configure.base"), base)
        .panel
    ...
    private fun validBase(value: String?): Boolean {
        ...
        SonderrNotifications.error(
            project,
            SonderrBundle.message("worktree.configure.base.invalid.title"),
            SonderrBundle.message("worktree.configure.base.invalid.content", value.orEmpty()),
        )
        base.focusText()
        return false
    }
    ...
    private fun submitPr() {
        ...
        if (value.isEmpty()) {
            setErrorText(SonderrBundle.message("worktree.import.pr.required"), url)
            url.requestFocusInWindow()
            return
        }
        if (parsePrUrl(value) == null) {
            setErrorText(SonderrBundle.message("worktree.import.pr.invalid"), url)
            url.requestFocusInWindow()
            url.selectAll()
            return
        }
    ...
    private fun submitBranch() {
        ...
        if (target == null || !pick.known(target)) {
            setErrorText(SonderrBundle.message("worktree.import.branch.invalid"), pick)
            pick.focusText()
            return
        }
    ...
    private fun slashActions(): List<SlashAction> {
        ...
        return SlashAction.ALL.map { spec ->
            SlashAction(spec.name, SonderrBundle.message(spec.descriptionKey), spec.hints, actions[spec] ?: {})
        }
    }
