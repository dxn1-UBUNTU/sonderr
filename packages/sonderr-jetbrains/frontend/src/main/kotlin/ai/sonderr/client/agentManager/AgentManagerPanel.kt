...
import ai.sonderr.client.agentManager.worktree.CreateFailure
import ai.sonderr.client.agentManager.worktree.CreateKind
import ai.sonderr.client.agentManager.worktree.NewWorktreeDialog
...
import ai.sonderr.client.agentManager.worktree.GhBanner
import ai.sonderr.client.agentManager.worktree.WorktreeController
import ai.sonderr.client.agentManager.worktree.WorktreeDataKeys
import ai.sonderr.client.agentManager.worktree.WorktreeIcons
import ai.sonderr.client.agentManager.worktree.WorktreeStatusBinding
...
import ai.sonderr.client.agentManager.worktree.WorktreeSessionEditorMatcher
import ai.sonderr.client.agentManager.worktree.WorktreeSessionEditorKind
import ai.sonderr.client.agentManager.worktree.WorktreeTitle
import ai.sonderr.client.agentManager.worktree.openWorktreeSession
import ai.sonderr.client.agentManager.worktree.normalizeWorktreePath
import ai.sonderr.client.agentManager.worktree.worktreeSessionParams
import ai.sonderr.client.ui.prTooltip
import ai.sonderr.client.ui.style
...
import ai.sonderr.client.ui.list.ActiveList
import ai.sonderr.client.ui.list.ActiveListBadge
import ai.sonderr.client.ui.list.ActiveListConfig
import ai.sonderr.client.ui.list.ActiveListDeleteOptions
import ai.sonderr.client.ui.list.ActiveListItem
import ai.sonderr.client.ui.list.ActiveListMenu
import ai.sonderr.client.ui.list.ActiveListMetrics
import ai.sonderr.client.ui.list.ActiveListReorder
import ai.sonderr.client.ui.list.ActiveListSelection
import ai.sonderr.client.ui.list.ActiveListSurface
import ai.sonderr.client.ui.list.ActiveListWeight
...
class AgentManagerPanel(
    ...
    private val group = ActionManager.getInstance().getAction("Sonderr.Worktree.RowMenu") as? ActionGroup ?: DefaultActionGroup()
    private val list = ActiveList(
        SonderrBundle.message("worktree.empty"),
    ...
    init {
        ...
        service<WorktreeNameCache>().addListener(this) { path, name ->
            ...
            project?.service<SonderrVfsManager>()?.updatePresentation(WorktreeSessionEditorKind.ID, mapOf("path" to path))
        }
    ...
    private fun renameWorktree(item: WorktreeDto, name: String) {
        controller.rename(
            ...
            name,
            onSuccess = { updated ->
                project?.service<SonderrVfsManager>()?.updatePresentation(WorktreeSessionEditorKind.ID, worktreeSessionParams(updated))
            },
            onFailure = { err ->
                SonderrNotifications.error(project, SonderrBundle.message("worktree.rename.failed.title", name), err)
            },
        )
    }
    ...
    private fun close(item: WorktreeDto) {
        val target = project ?: return
        target.service<SonderrVfsManager>().close(WorktreeSessionEditorKind.ID, worktreeSessionParams(item))
    }
    ...
    private fun showDeletePopup(item: WorktreeDto, cell: String? = null) {
        val opts = ActiveListDeleteOptions(
            message = SonderrBundle.message("worktree.delete.confirm.message", item.name),
            detail = SonderrBundle.message("worktree.delete.confirm.detail"),
            gate = if (item.locked) SonderrBundle.message("worktree.delete.locked.confirm") else null,
        )
    ...
    private fun notifyCreateFailed(failure: CreateFailure) {
        val title = when (failure.kind) {
            CreateKind.CREATE -> SonderrBundle.message("worktree.create.failed.title")
            CreateKind.BRANCH -> SonderrBundle.message("worktree.import.branch.failed.title", failure.branch)
            CreateKind.PR -> SonderrBundle.message("worktree.import.pr.failed.title")
        }
        SonderrNotifications.error(project, title, failure.error)
    }

    private fun notifyMoveFailed(err: String?) {
        SonderrNotifications.error(project, SonderrBundle.message("worktree.move.failed.title"), err)
    }
    ...
    private fun notifyFailed(item: WorktreeDto, result: RemoveWorktreeResultDto, forced: Boolean) {
        val title = SonderrBundle.message("worktree.delete.failed.title", item.name)
        if (result.locked && !forced) {
            SonderrNotifications.error(
                project,
                title,
                result.error,
                SonderrBundle.message("worktree.delete.force"),
            ) { remove(item, force = true) }
            return
        }
        SonderrNotifications.error(project, title, result.error)
    }
    ...
    private inner class RenameAction : AnAction(
        SonderrBundle.message("worktree.rename.action"),
        null,
    ...
    private inner class WorktreeRow(
        ...
        override val section: String? get() = if (current) null else SonderrBundle.message("worktree.section.local")
    ...
    private fun openBranchDiff(path: String) {
        ...
        target.service<SonderrVfsManager>().open(
            SonderrDiffEditorKind.ID,
            diffParams("branch", path, null, SonderrBundle.message("diff.editor.branch.title")),
        )
    }
}
