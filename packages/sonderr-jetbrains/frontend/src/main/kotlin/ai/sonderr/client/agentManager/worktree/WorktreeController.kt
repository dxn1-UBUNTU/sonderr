...
/**
 * Owns the worktree list model and drives the [SonderrWorktreeService] off the EDT. Model mutations
 * are marshalled back onto the EDT via [edt]. Mirrors the History stack's controller shape.
 */
class WorktreeController(
    private val service: SonderrWorktreeService,
    ...
    fun create(
        ...
        edt {
            ...
            tasks[temp.id] = SonderrBundle.message("worktree.progress.creating")
            model.add(0, temp)
    ...
    fun importPr(url: String) {
        ...
        val temp = WorktreeDto(id, SonderrBundle.message("worktree.import.pr.section"), "", id)
        edt {
            pending[temp.id] = temp
            tasks[temp.id] = SonderrBundle.message("worktree.progress.creating")
            model.add(0, temp)
    ...
    fun remove(
        ...
        tasks[dto.id] = SonderrBundle.message("common.deleting")
        edt { refresh(dto) }
        cs.launch {
            ...
            service<SonderrRunService>().release(directory, dto.path)
     ...
     * then persists the resulting paths via [SonderrWorktreeService.reorder]; on failure the list reloads
    ...
    private fun label(stage: MoveStage): String = when (stage) {
        MoveStage.CAPTURING -> SonderrBundle.message("worktree.progress.capturing")
        MoveStage.CREATING -> SonderrBundle.message("worktree.progress.creating")
        MoveStage.TRANSFERRING -> SonderrBundle.message("worktree.progress.transferring")
        MoveStage.FORKING -> SonderrBundle.message("worktree.progress.starting")
        MoveStage.DONE -> ""
        MoveStage.ERROR -> ""
    }
