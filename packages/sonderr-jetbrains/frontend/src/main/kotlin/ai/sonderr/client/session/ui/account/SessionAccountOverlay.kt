...
import ai.sonderr.client.settings.profile.formatBalance
...
internal class SessionAccountOverlay(
    ...
    private val profileBtn = HoverIcon().apply {
        ...
        toolTipText = SonderrBundle.message("action.Sonderr.ShowProfile.description")
        accessibleContext.accessibleName = SonderrBundle.message("action.Sonderr.ShowProfile.text")
    ...
    @RequiresEdt
    private fun updateLoggedIn(prof: ai.sonderr.rpc.dto.ProfileDto, switching: Boolean, target: String?): Boolean {
        var layout = false

        val orgs = prof.organizations
        val personal = prof.hasPersonalAccount
        val next = (if (personal) listOf(AccountChoice(null, SonderrBundle.message("profile.personalAccount"))) else emptyList()) +
        ...
        val tip = if (switching) {
            SonderrBundle.message("profile.switchingAccount")
        } else {
            SonderrBundle.message("session.account.switcher")
        }
    ...
    @RequiresEdt
    private fun syncBalance(prof: ai.sonderr.rpc.dto.ProfileDto): Boolean {
        var layout = false
        ...
        if (next == null) {
            ...
            val tip = SonderrBundle.message("session.account.balance", next)
