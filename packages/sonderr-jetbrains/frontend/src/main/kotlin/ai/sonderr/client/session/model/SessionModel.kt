package ai.sonderr.client.session.model
 ...
 *
 * **EDT-only access** — no synchronization. [ai.sonderr.client.session.controller.SessionController] guarantees all
...
class SessionModel {
    ...
    var app: SonderrAppStateDto = SonderrAppStateDto(SonderrAppStatusDto.DISCONNECTED)
    var version: String? = null

    var workspace: SonderrWorkspaceStateDto = SonderrWorkspaceStateDto(SonderrWorkspaceStatusDto.PENDING)
    ...
    @RequiresEdt
    fun isReady(): Boolean = app.status == SonderrAppStatusDto.READY && workspace.status == SonderrWorkspaceStatusDto.READY
...
private fun ai.sonderr.rpc.dto.PartTimeDto.durationMs(): Long? {
