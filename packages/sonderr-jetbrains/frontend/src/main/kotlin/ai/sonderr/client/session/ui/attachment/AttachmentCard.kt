...
class AttachmentChip(
    ...
    init {
        ...
        toolTipText = tip
        accessibleContext?.accessibleName = SonderrBundle.message("prompt.attachment.open", item.name)
    ...
    private fun label(): String {
        val start = startLine
        val end = endLine
        if (file && start != null && end != null) return SonderrBundle.message("session.attachment.file.range", item.name, start, end)
        if (file) return item.name
        return SonderrBundle.message("session.attachment.unknown", item.mime.ifBlank { "unknown" })
    }
...
open class AttachmentCard(
    ...
    private val action = remove?.let { callback ->
        CloseButton().apply {
            isVisible = false
            toolTipText = SonderrBundle.message("prompt.attachment.remove", item.name)
...
private fun tooltip(item: AttachmentCardItem): String = XmlStringUtil.wrapInHtml(
    StringUtil.escapeXmlEntities(
        SonderrBundle.message("prompt.attachment.tooltip", item.name, item.mime, location(item)),
    ).replace("\n", "<br>"),
)
...
private fun location(item: AttachmentCardItem): String {
    ...
    if (uri?.scheme == "data") return SonderrBundle.message("prompt.attachment.embedded")
