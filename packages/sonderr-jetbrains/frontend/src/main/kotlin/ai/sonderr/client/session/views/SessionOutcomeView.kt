...
class SessionOutcomeView(
    ...
    fun showError(message: String, kind: String?) {
        setOutlined(true)
        setHeaderIcon(AllIcons.General.Error, kind ?: SonderrBundle.message("session.error.title"))
        setHeader(SonderrBundle.message("session.error.title"), kind)
        error.text = message
    ...
    fun showRetry() {
        ...
        val title = SonderrBundle.message("session.outcome.failed.title")
        setOutlined(true)
    ...
    fun showOutcome(outcome: Outcome, finish: String? = null) {
        when (outcome) {
            Outcome.INTERRUPTED -> {
                setOutlined(false)
                setHeaderIcon(null)
                setHeader("", SonderrBundle.message("session.outcome.interrupted.note"))
                syncRetry(false)
            }
            ...
            Outcome.FAILED -> {
                ...
                setOutlined(true)
                setHeaderIcon(AllIcons.General.Error, title)
                setHeader(title, SonderrBundle.message("session.outcome.failed.description"))
                syncRetry(true)
            }

            Outcome.INCOMPLETE -> {
                val title = SonderrBundle.message("session.outcome.incomplete.title")
                val tip = finish?.let { SonderrBundle.message("session.outcome.incomplete.reason", it) } ?: title
                setOutlined(true)
                setHeaderIcon(AllIcons.General.Warning, tip)
                setHeader(title, SonderrBundle.message("session.outcome.incomplete.description"))
                syncRetry(false)
            }
        }
        setContentPadding()
    ...
    private fun syncRetry(show: Boolean) {
        ...
        setActions(
            listOf(
                Action(
                    id = RETRY_ACTION,
                    text = SonderrBundle.message("session.outcome.retry"),
                    primary = true,
                    handler = run,
                ),
            ),
