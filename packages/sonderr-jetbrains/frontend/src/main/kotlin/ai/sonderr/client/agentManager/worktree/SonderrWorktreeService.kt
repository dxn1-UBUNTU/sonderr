...
/**
 * App-level service wrapping [ai.sonderr.rpc.SonderrWorktreeRpcApi]. Mirrors [ai.sonderr.client.app.SonderrWorkspaceService]:
...
class SonderrWorktreeService internal constructor(
    private val cs: CoroutineScope,
    private val rpc: SonderrWorktreeRpcApi?,
) {
    /** Platform constructor — resolves RPC from the service container. */
    constructor(cs: CoroutineScope) : this(cs, null)

    companion object {
        private val LOG = SonderrLog.create(SonderrWorktreeService::class.java)
    }

    private suspend fun <T> call(block: suspend SonderrWorktreeRpcApi.() -> T): T {
        val api = rpc
        return if (api != null) block(api) else durable { block(SonderrWorktreeRpcApi.getInstance()) }
    }
