...
internal class AdvancedSettingsUi : JPanel(BorderLayout()) {
    ...
    init {
        ...
        val rows = SettingsRows().apply {
            ...
            row(TitledSeparator(SonderrBundle.message("settings.advanced.logging.title")))
            row(SettingsRow(
                SonderrBundle.message("logs.configuration.level.title"),
                SonderrBundle.message("logs.configuration.level.description"),
                level,
            ))
            row(SettingsRow(
                SonderrBundle.message("logs.configuration.preview.title"),
                SonderrBundle.message("logs.configuration.preview.description"),
                mode,
            ))
            row(SettingsRow(
                SonderrBundle.message("logs.configuration.previewSize.title"),
                SonderrBundle.message("logs.configuration.previewSize.description", LogConfig.MIN_PREVIEW, LogConfig.MAX_PREVIEW),
                preview,
            ))
            logRows().forEach(::row)
        }
    ...
    fun error(): String? {
        val value = count() ?: return SonderrBundle.message("logs.configuration.previewSize.invalid")
        if (value !in LogConfig.MIN_PREVIEW..LogConfig.MAX_PREVIEW) {
            return SonderrBundle.message("logs.configuration.previewSize.outOfRange", LogConfig.MIN_PREVIEW, LogConfig.MAX_PREVIEW)
        }
        return null
    }
    ...
    private fun logRows(): List<SettingsRow> {
        if (IdeProductMode.isMonolith) {
            return listOf(SettingsRow(
                SonderrBundle.message("settings.advanced.logs.title"),
                SonderrBundle.message("settings.advanced.logs.description"),
                ActionLink(AdvancedLogActions.revealLabel()) { AdvancedLogActions.reveal() },
            ))
        }
        return listOf(
            SettingsRow(
                SonderrBundle.message("settings.advanced.logs.client.title"),
                SonderrBundle.message("settings.advanced.logs.client.description"),
                ActionLink(AdvancedLogActions.revealLabel()) { AdvancedLogActions.reveal() },
            ),
            SettingsRow(
                SonderrBundle.message("settings.advanced.logs.backend.title"),
                SonderrBundle.message("settings.advanced.logs.backend.description"),
                ActionLink(SonderrBundle.message("settings.advanced.logs.backend.download")) {
