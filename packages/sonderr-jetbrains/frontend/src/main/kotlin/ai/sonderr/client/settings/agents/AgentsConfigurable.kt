...
import ai.sonderr.client.session.ui.model.ModelText
...
class AgentsConfigurable : AgentBehaviorConfigurableBase<JComponent>() {
    ...
    override fun getDisplayName(): String = SonderrBundle.message("settings.agentBehavior.agents.displayName")
    ...
    companion object { const val ID = "ai.sonderr.jetbrains.settings.agentBehavior.agents" }
}
...
internal class AgentsSettingsUi(
    ...
    override suspend fun fetch(): List<ActiveListItem> {
        val agents = service<SonderrAgentBehaviorService>().agents(dir)
        models = items(service<SonderrWorkspaceService>().models(dir).providers)
        val next = agentsDraft(service<SonderrAppService>().state.value.config, agents)
    ...
    override fun toolbarRight(): JComponent = Stack.horizontal(UiStyle.Gap.sm())
        .next(JBLabel(SonderrBundle.message("settings.agentBehavior.agents.default")))
        .next(makePicker())
    ...
    override fun searchPlaceholder() = SonderrBundle.message("settings.agentBehavior.agents.search")
        ...
        if (!launch("apply") { id ->
            ...
            for (name in target.deleted) {
                if (!behavior.removeAgent(dir, name)) {
                    failed = SonderrBundle.message("settings.agentBehavior.agents.delete.failed")
                    break
                }
            ...
            if (failed == null) {
                for ((name, input) in target.created) {
                    if (!behavior.createAgent(dir, input)) {
                        failed = SonderrBundle.message("settings.agentBehavior.agents.create.failed")
                        break
                    }
            ...
            if (failed == null) {
                for ((name, item) in target.imported) {
                    if (app.updateConfig(item) == null) {
                        failed = SonderrBundle.message("settings.agentBehavior.agents.import.failed")
                        break
                    }
            ...
            withContext(edt) {
                if (!active(id)) {
                    if (failed != null) SonderrNotifications.error(failed)
                    return@withContext
                }
                details = agents
                if (failed != null) {
    ...
    private fun rows(): List<ActiveListItem> = displayRows(base, draft).map { row ->
        ...
        object : ActiveListItem {
            ...
            override val badges = listOfNotNull(
                ActiveListBadge(
                    SonderrBundle.message("settings.agentBehavior.badge.notApplied"),
                    UiStyle.Badge.Secondary,
                ).takeIf { row.intent == AgentIntent.Modified || row.intent == AgentIntent.New },
                ActiveListBadge(
                    SonderrBundle.message("settings.agentBehavior.badge.willRemove"),
                    UiStyle.Badge.Alert,
                ...
                ).takeIf { SonderrCliParser.isSubagent(item.mode) },
                ActiveListBadge(
                    SonderrBundle.message("settings.agentBehavior.badge.custom"),
                    UiStyle.Badge.Primary,
                ).takeIf { canDelete(item) },
                ActiveListBadge(SonderrBundle.message("settings.agentBehavior.badge.hidden")).takeIf { item.hidden },
                ActiveListBadge(SonderrBundle.message("settings.agentBehavior.badge.disabled")).takeIf { item.disable },
                ActiveListBadge(
            ...
            override val cells = when (row.intent) {
                ...
                -> listOf(ActiveListCell(UNDO_CELL, SonderrBundle.message("settings.agentBehavior.undo"), primary = true))
                AgentIntent.Unchanged,
                AgentIntent.Modified,
                -> listOfNotNull(
                    ActiveListCell(EDIT_CELL, SonderrBundle.message("settings.agentBehavior.edit")),
                    ActiveListCell(
                    DELETE_CELL,
    ...
    private fun syncNames() {
        names = listOf("") + displayRows(base, draft)
            ...
            .map { it.agent }
            .filter { SonderrCliParser.defaultAgentCandidate(it.mode, it.hidden) && !it.disable }
            .map { it.name }
    }
    ...
    private fun addAction(): DefaultActionGroup = DefaultActionGroup(
        SonderrBundle.message("settings.agentBehavior.agents.add"),
        true,
    ).apply {
    ...
    internal inner class CreateAction : DumbAwareAction(SonderrBundle.message("settings.agentBehavior.agents.create")) {
    ...
    internal inner class ImportAction : DumbAwareAction(SonderrBundle.message("settings.agentBehavior.agents.import")) {
        override fun getActionUpdateThread() = ActionUpdateThread.EDT
        ...
        internal fun perform(file: VirtualFile? = choose(this@AgentsSettingsUi)) {
            ...
            showProgress(SonderrBundle.message("settings.agentBehavior.agents.import.progress"))
        }

        private fun load(file: VirtualFile, names: Collection<String>): AgentImport {
            if (file.length > MAX_AGENT_IMPORT_SIZE) {
                throw SettingsMessageException(SonderrBundle.message(AgentImportError.TOO_LARGE.key))
            }
            val text = String(file.contentsToByteArray(), StandardCharsets.UTF_8)
            return try {
                parseAgentImport(text, names)
            } catch (e: AgentImportException) {
                throw SettingsMessageException(SonderrBundle.message(e.error.key))
            }
        }
    }
    ...
    private companion object {
        ...
        const val SONDERR_PROVIDER = "sonderr"
    }
    ...
    private fun items(providers: ProvidersDto?): List<ModelPicker.Item> {
        ...
        return cfg.providers
            .filter { it.id == SONDERR_PROVIDER || it.id in cfg.connected }
            .flatMap { provider ->
...
private fun chooseImportFile(_parent: JComponent): VirtualFile? {
    val descriptor = FileChooserDescriptor(true, false, false, false, false, false).apply {
        title = SonderrBundle.message("settings.agentBehavior.agents.import.title")
        description = SonderrBundle.message("settings.agentBehavior.agents.import.description")
