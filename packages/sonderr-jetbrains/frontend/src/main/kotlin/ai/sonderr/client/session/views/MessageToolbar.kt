...
import ai.sonderr.client.session.ui.selection.SessionCopyButton
...
internal class MessageToolbar(
    ...
    tooltip: String = SonderrBundle.message("session.copy.hover"),
) : JPanel(FlowLayout(FlowLayout.LEFT, 0, 0)) {
    constructor(text: () -> String?, revert: (() -> Unit)?) : this(
        text,
        actions = revert?.let {
            listOf(ToolbarButtonAction(AllIcons.Actions.Rollback, SonderrBundle.message("revert.message.rollback"), it))
        }.orEmpty(),
        tooltip = SonderrBundle.message("session.copy.prompt"),
    )
