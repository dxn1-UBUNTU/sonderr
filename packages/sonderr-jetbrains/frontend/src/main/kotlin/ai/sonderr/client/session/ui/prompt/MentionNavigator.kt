...
internal class MentionNavigator(
    ...
    private val provider: SonderrPromptCompletionProvider,
    ...
    fun install() {
        ...
        object : DumbAwareAction(SonderrBundle.message("prompt.mention.goto")) {
    ...
    private fun syncTooltip(hit: SonderrPromptCompletionProvider.MentionHit?) {
        val text = hit?.takeIf { !it.resolved }?.let { SonderrBundle.message("prompt.mention.unresolved", it.value) }
        if (editor.contentComponent.toolTipText != text) editor.contentComponent.toolTipText = text
    }

    private fun syncLink(hit: SonderrPromptCompletionProvider.MentionHit?, mouse: MouseEvent) {
