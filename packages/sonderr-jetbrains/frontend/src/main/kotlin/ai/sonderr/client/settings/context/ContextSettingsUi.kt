...
internal class ContextSettingsUi(
...
) : BaseSettingsUi<ContextSettingsContent, ContextDraft, ConfigPatchDto, SonderrAppStateDto, Unit>(
    cs,
    ContextDraft(),
    app,
    workspaces,
    ...
    override fun save(change: ConfigPatchDto, done: (SonderrAppStateDto?) -> Unit) {
        ...
        if (!changed(change)) {
            SonderrPluginSettings.setAutoEditorContext(value)
            done(appState)
            return
        }
        app.updateConfigAsync(change) { result ->
            if (result != null) SonderrPluginSettings.setAutoEditorContext(value)
            done(result)
        }
    }

    override fun base(result: SonderrAppStateDto): ContextDraft = contextDraft(result.config)

    override fun draft(state: SonderrAppStateDto): ContextDraft = contextDraft(state.config)

    override fun saved(base: ContextDraft, draft: ContextDraft): Boolean = savedMatches(base, draft)

    override fun pendingText(): String = SonderrBundle.message("settings.context.save.pending")

    override fun failedText(): String = SonderrBundle.message("settings.context.save.failed")
    ...
    override fun syncContent() {
        ...
        val err = saveError
        if (saving) {
            showProgress(SonderrBundle.message("settings.context.save.pending"))
            return
        }
        if (err != null) {
            showError(err)
        ...
        if (thresholdStatus(draft.threshold) == ThresholdStatus.INVALID) {
            top.showBanner(
                SonderrBundle.message("settings.context.compaction.threshold.invalid"),
                emptyList(),
    ...
    private companion object {
        val LOG = SonderrLog.create(ContextSettingsUi::class.java)
    }
}
...
internal class ContextSettingsContent(
    ...
    private val threshold = ThresholdField(
        SonderrBundle.message("settings.context.compaction.threshold.placeholder"),
    ) { value -> update { copy(threshold = value) } }
    private val patterns = PatternList { value -> update { copy(ignore = value) } }

    init {
        section(
            SonderrBundle.message("settings.context.compaction.title"),
        ).apply {
            row(SettingsRow(
                SonderrBundle.message("settings.context.compaction.auto.title"),
                SonderrBundle.message("settings.context.compaction.auto.description"),
                auto,
            ))
            row(SettingsRow(
                SonderrBundle.message("settings.context.compaction.threshold.title"),
                SonderrBundle.message("settings.context.compaction.threshold.description"),
                Stack.horizontal(UiStyle.Gap.xs())
                    .next(threshold)
                    .next(JBLabel(SonderrBundle.message("settings.context.compaction.threshold.suffix")))
                    .align(HAlign.RIGHT, VAlign.CENTER),
            ))
            row(SettingsRow(
                SonderrBundle.message("settings.context.compaction.prune.title"),
                SonderrBundle.message("settings.context.compaction.prune.description"),
                prune,
            ))
        }
        section(
            SonderrBundle.message("settings.context.editor.title"),
        ).row(SettingsRow(
            SonderrBundle.message("settings.context.editor.auto.title"),
            SonderrBundle.message("settings.context.editor.auto.description"),
            editor,
        ))
        section(
            SonderrBundle.message("settings.context.watcher.title"),
            SonderrBundle.message("settings.context.watcher.description"),
        ).row(patterns)
    }
 ...
 * [ai.sonderr.client.ui.list.ActiveListView], whose value is restoring selection across refreshes
...
internal class PatternList(
    ...
    internal var input: () -> String? = {
        Messages.showInputDialog(
            this,
            SonderrBundle.message("settings.context.watcher.input.prompt"),
            SonderrBundle.message("settings.context.watcher.input.title"),
            null,
        )
    }
    ...
    private val add = HoverIcon().apply {
        ...
        toolTipText = SonderrBundle.message("settings.context.watcher.add")
        addActionListener { add() }
    }
    private val remove = HoverIcon().apply {
        icon = AllIcons.General.Remove
        toolTipText = SonderrBundle.message("settings.context.watcher.remove")
        addActionListener { remove() }
    }
    private val list = JBList(model).apply {
        selectionMode = ListSelectionModel.MULTIPLE_INTERVAL_SELECTION
        isFocusable = true
        emptyText.text = SonderrBundle.message("settings.context.watcher.empty")
