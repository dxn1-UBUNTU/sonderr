package ai.sonderr.client.session.ui.empty
...
class EmptySessionPanel(
    ...
    private val logo = JBLabel(
        IconLoader.getIcon("/icons/sonderr-content.svg", EmptySessionPanel::class.java),
    ).apply {
    ...
    private fun tip(): String? {
        ...
        if (status.worktree) {
            return name?.let { SonderrBundle.message("session.empty.worktree", it) }
                ?: SonderrBundle.message("session.empty.worktree.unknown")
        }
        if (name == null) return null
        return SonderrBundle.message("session.empty.branch", name, worktreePhrase())
    }
    ...
    private fun worktreePhrase(): String {
        val phrase = SonderrBundle.message("session.empty.branch.link")
    ...
    private fun syncTip() {
        ...
        welcomeLabel.text = centeredHtml(
            tip ?: XmlStringUtil.escapeString(SonderrBundle.message("session.empty.welcome")),
        )
    ...
    internal fun feedbackText() = SonderrBundle.message("feedback.button")
    ...
    internal fun explanationText() = SonderrBundle.message("session.empty.welcome")
    ...
    internal fun descriptionText() = tipText() ?: SonderrBundle.message("session.empty.welcome")
    ...
    internal open class ShowHistoryButton(
        text: String = SonderrBundle.message("session.showHistory"),
