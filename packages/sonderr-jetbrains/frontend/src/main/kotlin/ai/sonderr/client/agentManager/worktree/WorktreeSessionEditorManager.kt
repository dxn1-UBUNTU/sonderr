...
open class WorktreeSessionEditorManager(
    ...
    resolve: (String) -> Workspace = { dir -> service<SonderrWorkspaceService>().workspace(dir) },
    status: () -> Map<String, SessionActivityKind> = { project.service<SonderrSessionService>().activitySnapshot() },
    ...
    private val notify: (String, String?) -> Unit = { title, content -> SonderrNotifications.error(project, title, content) },
    ...
    private val adopt: suspend (String, String, String) -> RenameWorktreeResultDto = { dir, path, name ->
        service<SonderrWorktreeService>().adopt(dir, path, name)
    },
    ...
    open fun deleteSessions(ids: List<String>) {
        ...
        active.forEach { id ->
            ...
            del(id) { ok, err ->
                deleting.remove(id)
                onListChanged?.invoke()
                if (!ok) notify(SonderrBundle.message("worktree.session.delete.failed.title", name), err)
            }
        }
    ...
    open fun renameSession(id: String, title: String) {
        ...
        list.rename(id, name) { ok, err ->
            onListChanged?.invoke()
            if (ok) return@rename
            notify(SonderrBundle.message("worktree.session.rename.failed.title", name), err)
        }
        onListChanged?.invoke()
    }
    ...
    private fun title(id: String): String {
        return list.session(id)
            ...
            ?: SonderrBundle.message("worktree.session.untitled")
    }
}

// Mirrors the CLI's Session.isDefaultTitle (packages/cli/src/session/session.ts): a session
