...
import ai.sonderr.client.session.SessionHost
import ai.sonderr.client.session.SessionManager
import ai.sonderr.client.session.SessionRef
import ai.sonderr.client.session.history.HistorySection
import ai.sonderr.client.session.history.HistoryTime
import ai.sonderr.client.session.history.LocalHistoryItem
...
import ai.sonderr.client.ui.list.ActiveListEditOptions
import ai.sonderr.client.ui.list.ActiveListItem
import ai.sonderr.client.ui.list.ActiveListMenu
import ai.sonderr.client.ui.list.ActiveListRowHeight
...
class WorktreeSessionEditorPanel(
    ...
    private val worktree: ai.sonderr.client.app.Workspace,
    ...
    private val group = ActionManager.getInstance().getAction("Sonderr.WorktreeSession.RowMenu") as? ActionGroup ?: DefaultActionGroup()
    private val list = ActiveList(
        SonderrBundle.message("worktree.session.list.empty"),
    ...
    private fun confirmDelete(ids: List<String>, cell: String? = null) {
        ...
        val msg = if (active.size == 1) {
            SonderrBundle.message("worktree.session.delete.confirm.message", title(active[0]))
        } else {
            SonderrBundle.message("worktree.session.delete.confirm.message.multiple", active.size)
        }
        val opts = ActiveListDeleteOptions(
            message = msg,
            detail = SonderrBundle.message("worktree.session.delete.confirm.detail"),
        )
    ...
    private fun openInNewFrame() {
        ...
        service<SonderrWorktreeService>().openInBackground(dir)
    }
    ...
    private fun openBranchDiff() {
        ...
        target.service<SonderrVfsManager>().open(
            ...
            diffParams("branch", worktree.directory, null, SonderrBundle.message("diff.editor.branch.title")),
        )
    }
    ...
    private fun title(key: String): String {
        return item(key)?.title?.takeIf { it.isNotBlank() } ?: SonderrBundle.message("worktree.session.untitled")
    }
    ...
    private inner class NewAction : AnAction(
        SonderrBundle.message("worktree.session.new.action"),
        null,
    ...
    private inner class DeleteAction : AnAction(
        SonderrBundle.message("worktree.session.delete.action"),
        null,
        AllIcons.Actions.GC,
    ) {
        override fun getActionUpdateThread(): ActionUpdateThread = ActionUpdateThread.EDT
    ...
    private inner class RenameAction : AnAction(
        SonderrBundle.message("worktree.session.rename.action"),
        null,
    ...
    private object NewRow : ActiveListItem {
        ...
        override val title: String get() = SonderrBundle.message("worktree.session.new")
    ...
    private companion object {
        private val LOG = SonderrLog.create(WorktreeSessionEditorPanel::class.java)
        private val TERMINAL_DIR = Key.create<String>("sonderr.worktree.terminal.dir")
    ...
    private inner class SessionRow(
        ...
        override val title: String get() {
            ...
            if (name.isBlank()) return SonderrBundle.message("worktree.session.untitled")
            // Show the placeholder as a friendly "New session" until the agent names the session.
            if (isDefaultSessionTitle(name)) return SonderrBundle.message("worktree.session.new")
            return name
        }
