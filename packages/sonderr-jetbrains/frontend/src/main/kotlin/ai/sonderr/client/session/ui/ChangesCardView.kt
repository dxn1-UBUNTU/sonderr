...
import ai.sonderr.client.session.views.tool.EditFileChange
import ai.sonderr.client.session.views.tool.PatchBody
import ai.sonderr.client.session.views.tool.setFont
import ai.sonderr.client.session.views.tool.setForeground
import ai.sonderr.client.session.views.tool.setIcon
import ai.sonderr.client.ui.DiffBadge
...
internal abstract class ChangesCardView(
    ...
    class Header(
        ...
        val count = JBLabel()
        val open = HeaderOpenAction(SessionViewIcons.openDiff, SonderrBundle.message("session.part.tool.openDiff")) {}
            .apply { enabled = false }
        ...
        fun update(total: Int, additions: Int, deletions: Int) {
            count.text = SonderrBundle.message(if (total == 1) "session.changes.count.one" else "session.changes.count.other", total)
