...
import ai.sonderr.client.session.model.AgentItem
import ai.sonderr.client.session.model.ModelLimitItem
import ai.sonderr.client.session.model.ModelItem
import ai.sonderr.client.session.model.SessionModel
import ai.sonderr.client.session.model.SessionModelEvent
import ai.sonderr.client.session.model.SessionState
import ai.sonderr.client.session.model.Permission
import ai.sonderr.client.session.model.PermissionFileDiff
import ai.sonderr.client.session.model.PermissionMeta
import ai.sonderr.client.session.model.PermissionRuleCandidate
import ai.sonderr.client.session.model.PermissionRuleDecision
import ai.sonderr.client.session.model.PermissionRequestState
import ai.sonderr.client.session.model.Question
import ai.sonderr.client.session.model.QuestionItem
import ai.sonderr.client.session.model.QuestionOption
import ai.sonderr.client.session.model.Reasoning
import ai.sonderr.client.session.ui.mode.agentTitle
import ai.sonderr.client.session.model.ToolCallRef
import ai.sonderr.client.session.model.Text
import ai.sonderr.client.session.model.Outcome
import ai.sonderr.client.session.model.TurnOutcome
import ai.sonderr.client.plugin.SonderrPluginSettings
import ai.sonderr.client.session.SessionRef
import ai.sonderr.client.telemetry.Telemetry
import ai.sonderr.client.util.UiTimer
import ai.sonderr.client.util.UiTimerSource
import ai.sonderr.client.util.UiTimers
import ai.sonderr.client.util.edtLater as edt
...
import ai.sonderr.rpc.dto.PermissionRuleDecisionDto
...
import ai.sonderr.log.ChatLogSummary
...
class SessionController(
  parent: Disposable,
  ref: SessionRef? = null,
  private val sessions: SonderrSessionService,
  private val workspace: Workspace,
  private val app: SonderrAppService,
  ...
  private val log: SonderrLog = LOG,
) : Disposable {
    ...
    companion object {
        private val LOG = SonderrLog.create(SessionController::class.java)
    ...
    val autoApprove: Boolean get() = SonderrPluginSettings.getAutoApprove()
        ...
        cs.launch {
            try {
                ...
                edt {
                    ...
                    val msg = e.message ?: SonderrBundle.message("session.error.prompt")
                    updateModel {
    ...
    fun setAutoApprove(value: Boolean) {
        assertEdt()
        SonderrPluginSettings.setAutoApprove(value)
     ...
     * succeeded, and a non-null error otherwise. The CLI maps every refusal (no Sonderr credentials,
    ...
    fun compact() {
        ...
        cs.launch {
            try {
                ...
                edt {
                    updateModel {
                        model.setState(SessionState.Error(e.message ?: SonderrBundle.message("session.error.compact")))
                    }
                }
            }
        }
    ...
    fun revert(message: String, part: String? = null) {
        ...
        val op = beginReverting(
            SonderrBundle.message("session.status.rollingback"),
     ...
     *   nothing but turn scaffolding (`SonderrSessionPrompt.recoverFailedAssistant`), and keeps it when it
    ...
    fun retry() {
        ...
        model.setState(SessionState.Busy(SonderrBundle.message("session.status.considering")))
        cs.launch {
            try {
                ...
                edt {
                    if (disposed) return@edt
                    model.setState(SessionState.Error(e.message ?: SonderrBundle.message("session.error.prompt")))
                }
            }
        }
    }
    ...
    fun unrevert() {
        ...
        if (revertOp != null) return
        val op = beginReverting(
            SonderrBundle.message("session.status.redoing"),
    ...
    fun cancelRevert() {
        ...
        if (state is SessionState.Reverting) {
            model.setState(state.copy(text = SonderrBundle.message("session.status.operation.finishing")))
        }
        revertJob?.cancel()
    }
    ...
    fun retryConnection() {
        ...
        if (model.app.status != SonderrAppStatusDto.READY || model.app.status == SonderrAppStatusDto.ERROR) {
            app.retryAsync()
            return
        }
        ...
        if (model.workspace.status == SonderrWorkspaceStatusDto.ERROR) {
            workspace.reload()
        }
    }
            ...
            try {
                ...
                edt {
                    updatePermission(
                        ...
                        e.message ?: SonderrBundle.message("session.permission.error"),
                    )
                }
            }
        }
    ...
    private fun approve(id: String, restore: () -> Permission) {
        ...
        updateModel { model.setState(SessionState.Busy(SonderrBundle.message("session.status.considering"))) }
        cs.launch {
            try {
                ...
                edt {
                    ...
                    show(restore().copy(
                        ...
                        message = e.message ?: SonderrBundle.message("session.permission.error"),
                    ))
                }
            }
        }
    ...
    init {
        ...
        cs.launch {
            app.state.collect { state ->
                if (state.status == SonderrAppStatusDto.READY) app.fetchVersionAsync()
        ...
        cs.launch {
            workspace.state.collect { state ->
                fire(SessionControllerEvent.WorkspaceChanged) {
                    ...
                    if (state.status != SonderrWorkspaceStatusDto.READY) return@fire
                    ...
                    model.models = state.providers?.let { providers ->
                        providers.providers
                            .filter { it.id == SONDERR_PROVIDER || it.id in providers.connected }
                            .flatMap { provider ->
                ...
                if (state.status == SonderrWorkspaceStatusDto.READY) {
                    fire(SessionControllerEvent.WorkspaceReady)
                    edt {
        ...
        cs.launch {
            try {
                ...
                edt {
                    ...
                    updateModel {
                        model.setState(SessionState.Error(e.message ?: SonderrBundle.message("history.error.local")))
                    }
                    showSession()
                    loaded(false)
                }
        ...
        cs.launch {
            try {
                ...
                edt {
                    if (disposed) return@edt
                    updateModel {
                        model.setState(SessionState.Error(e.message ?: SonderrBundle.message("history.error.cloud")))
                    }
                    showSession()
                    loaded(false)
                }
     ...
     * [ai.sonderr.client.session.ui.RevertBanner] falls back to `model.diff` when the CLI does not
    ...
    private fun liveStatus(dto: SessionStatusDto?): SessionState? {
        ...
        return when (dto.type) {
            "busy" -> SessionState.Busy(SonderrBundle.message("session.status.considering"))
    ...
    private fun handle(event: ChatEventDto) {
        ...
        when (event) {
            ...
            is ChatEventDto.TurnOpen -> {
                ...
                tool = null
                if (revertOp != null) {
                    revertDeferred = SessionState.Busy(SonderrBundle.message("session.status.considering"))
                    return
                }
                model.setState(SessionState.Busy(SonderrBundle.message("session.status.considering")))
            }
    ...
    private fun error(event: ChatEventDto.Error, reveal: Boolean) {
        ...
        if (isPaidModelAuthRequired(event.error)) {
            ...
            model.setState(SessionState.LoginRequired(SonderrBundle.message("session.login.required.description")))
            return
        }
        if (event.error?.aborted == true) return
        val msg = event.error?.message ?: event.error?.type ?: SonderrBundle.message("session.error.unknown")
    ...
    private fun afterResolve(idle: Boolean = false): SessionState {
        return pending.values.firstOrNull()?.let { SessionState.AwaitingPermission(it) }
            ?: if (idle) SessionState.Idle else SessionState.Busy(SonderrBundle.message("session.status.considering"))
    }
        ...
        val state = when (dto.type) {
            ...
            "busy" -> {
                ...
                if (current is SessionState.Idle || current is SessionState.Error || current is SessionState.TurnEnded)
                    SessionState.Busy(SonderrBundle.message("session.status.considering"))
    ...
    private fun onRevertTimeout(op: RevertOp) {
        ...
        revertJob?.cancel()
        failReverting(op, RuntimeException(SonderrBundle.message("session.error.revert.timeout")))
    }
    ...
    private fun failReverting(op: RevertOp, e: Exception) {
        ...
        revertDeferred = null
        model.setState(SessionState.Error(e.message ?: SonderrBundle.message("session.error.unknown")))
    }
    ...
    private fun configModel(agent: String): String? {
        if (model.app.status != SonderrAppStatusDto.READY) return null
        val cfg = model.app.config
    ...
    private fun status(): String = when (partType) {
        "reasoning" -> SonderrBundle.message("session.status.thinking")
        "text" -> SonderrBundle.message("session.status.writing")
        "tool" -> when (tool) {
            "task" -> SonderrBundle.message("session.status.delegating")
            "todowrite", "todoread" -> SonderrBundle.message("session.status.planning")
            "read" -> SonderrBundle.message("session.status.gathering")
            "glob", "grep", "list" -> SonderrBundle.message("session.status.searching.codebase")
            "webfetch", "websearch", "codesearch" -> SonderrBundle.message("session.status.searching.web")
            "edit", "write" -> SonderrBundle.message("session.status.editing")
            "bash" -> SonderrBundle.message("session.status.commands")
            else -> SonderrBundle.message("session.status.considering")
        }
        else -> SonderrBundle.message("session.status.considering")
    }
    ...
    private fun resolveConnectionState(): SessionControllerEvent.ConnectionChanged {
        ...
        if (app.status == SonderrAppStatusDto.ERROR) {
            return SessionControllerEvent.ConnectionChanged.ShowError(
                SonderrBundle.message("session.connection.error.app"),
                app.errors.toErrorText() ?: app.error,
            )
        }

        if (app.status == SonderrAppStatusDto.DOWNLOADING) {
        ...
        if (workspace.status == SonderrWorkspaceStatusDto.ERROR) {
            return SessionControllerEvent.ConnectionChanged.ShowError(
                SonderrBundle.message("session.connection.error.workspace"),
                workspace.errors.toErrorText() ?: workspace.error,
                "workspace",
            )
        }

        if (workspace.status == SonderrWorkspaceStatusDto.UNSUPPORTED) {
            return SessionControllerEvent.ConnectionChanged.ShowError(
                SonderrBundle.message("session.connection.unsupported"),
                unsupported(workspace.error, directory),
                "workspace",
            )
        }

        if (workspace.status == SonderrWorkspaceStatusDto.MISSING) {
            return SessionControllerEvent.ConnectionChanged.ShowError(
                SonderrBundle.message("session.connection.missing"),
                SonderrBundle.message("session.connection.missing.detail", workspace.error ?: directory),
                "workspace",
            )
        }

        if (app.status == SonderrAppStatusDto.READY && workspace.status == SonderrWorkspaceStatusDto.READY && app.warnings.isNotEmpty()) {
        ...
        if (app.status == SonderrAppStatusDto.READY && workspace.status == SonderrWorkspaceStatusDto.READY) {
...
private fun summary(count: Int): String {
    val base = SonderrBundle.message("session.connection.warning.config")
...
private fun unsupported(reason: String?, directory: String): String {
    val detail = when (reason) {
        "devcontainer_virtual_filesystem" -> SonderrBundle.message("session.connection.unsupported.devcontainer")
        "wsl_virtual_filesystem" -> SonderrBundle.message("session.connection.unsupported.wsl")
        "invalid_virtual_path" -> SonderrBundle.message("session.connection.unsupported.invalid")
        else -> SonderrBundle.message("session.connection.unsupported.unknown")
    }
    val path = SonderrBundle.message("session.connection.unsupported.path", directory)
    val options = SonderrBundle.message("session.connection.unsupported.options")
    return "$path\n\n$detail\n\n$options"
}

private const val SONDERR_PROVIDER = "sonderr"
private const val SONDERR_AUTO_MODEL = "sonderr-auto/free"
...
private fun resolveModelSelection(
    ...
    fallback: ModelSelectionDto? = ModelSelectionDto(SONDERR_PROVIDER, SONDERR_AUTO_MODEL),
): ModelSelectionDto? {
...
private fun valid(providers: ProvidersDto?, item: ModelSelectionDto?): ModelSelectionDto? {
    ...
    if (item.providerID != SONDERR_PROVIDER && item.providerID !in providers.connected) return null
