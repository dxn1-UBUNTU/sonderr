...
import ai.sonderr.client.actions.SendPromptAction
import ai.sonderr.client.actions.StopSessionAction
...
import ai.sonderr.client.session.model.PromptAttachmentExtractor
...
class PromptPanel(
    ...
    private val completion: SonderrPromptCompletionProvider? = null,
    ...
    companion object {
        private val LOG = SonderrLog.create(PromptPanel::class.java)
    ...
    private val reset = HoverIcon().apply {
        ...
        toolTipText = SonderrBundle.message("model.picker.reset")
        accessibleContext.accessibleName = SonderrBundle.message("model.picker.reset")
        isVisible = false
    ...
    /**
     * Opens the Sonderr.Session.PromptMenu popup (auto-approve + sharing). Resolves its context from
    ...
    private val menu = HoverIcon().apply {
        ...
        toolTipText = SonderrBundle.message("prompt.action.menu")
        accessibleContext.accessibleName = SonderrBundle.message("prompt.action.menu")
    ...
    private val enhance = HoverIcon().apply {
        icon = WAND_ICON
        toolTipText = SonderrBundle.message("prompt.action.enhance")
        accessibleContext.accessibleName = SonderrBundle.message("prompt.action.enhance")
    ...
    private fun key(kind: SonderrPromptCompletionProvider.HighlightKind): TextAttributesKey = when (kind) {
        SonderrPromptCompletionProvider.HighlightKind.MENTION -> MENTION_KEY
        SonderrPromptCompletionProvider.HighlightKind.COMMAND -> COMMAND_KEY
        SonderrPromptCompletionProvider.HighlightKind.INVALID -> INVALID_KEY
    }
    ...
    private fun showMenu() {
        val group = ActionManager.getInstance().getAction("Sonderr.Session.PromptMenu") as? ActionGroup ?: return
    ...
    private fun enhance() {
        ...
        if (source.isBlank()) {
            editor.text = SonderrBundle.message("prompt.action.enhance.description")
            syncEditorHeight()
            focus()
            return
        }
    ...
    private fun completeEnhancement(id: Long, source: String, result: Result<String>) {
        ...
        result.onSuccess {
            ...
            SonderrNotifications.error(
                project,
                SonderrBundle.message("prompt.action.enhance.failed"),
                SonderrBundle.message("prompt.action.enhance.failed.description"),
            )
        }
    }
    ...
    private fun syncEnhance() {
        ...
        enhance.toolTipText = if (enhancing) {
            SonderrBundle.message("prompt.action.enhance.loading")
        } else {
            SonderrBundle.message("prompt.action.enhance")
        }
    }
        ...
        cs.launch {
            try {
                ...
                withContext(Dispatchers.Main) {
                    ...
                    notify(SonderrBundle.message("prompt.attachment.send.failed", e.message ?: e.javaClass.simpleName))
                }
            }
        }
    }
    ...
    private fun installCompletionShortcut(ed: com.intellij.openapi.editor.Editor) {
        ...
        val action = completionAction ?: object : DumbAwareAction(SonderrBundle.message("prompt.completion.action")) {
    ...
    private fun addAttachment(item: PromptAttachment) {
        if (!attachment && !item.reference && PromptAttachmentExtractor.image(item.mime)) {
            ...
            notify(SonderrBundle.message("prompt.attachment.unsupported.model"))
            return
        }
    ...
    private fun promptDto(text: String, files: List<PromptPartDto>) = ai.sonderr.rpc.dto.PromptDto(
        parts = buildList {
            ...
            try {
                ...
                if (items.isEmpty()) {
                    if (list.isNotEmpty()) {
                        ApplicationManager.getApplication().invokeLater {
                            if (project.isDisposed) return@invokeLater
                            notify(SonderrBundle.message("prompt.attachment.drop.empty"))
                        }
                    }
    ...
    private fun notify(text: String) {
        com.intellij.notification.Notification("Sonderr", text, com.intellij.notification.NotificationType.WARNING).notify(project)
    }
    ...
    private fun syncAutoApprove() {
        ...
        auto.toolTipText = if (autoApprove) {
            SonderrBundle.message("prompt.action.autoApprove.enabled.tooltip")
        } else {
            SonderrBundle.message("prompt.action.autoApprove.disabled.tooltip")
        }
        auto.accessibleContext.accessibleName = if (autoApprove) {
            SonderrBundle.message("prompt.action.autoApprove.disable")
        } else {
            SonderrBundle.message("prompt.action.autoApprove.enable")
        }
        auto.repaint()
    }
    ...
    private fun tooltip(): String {
        ...
        val text = if (stop) {
            SonderrBundle.message("prompt.button.stop")
        } else {
            SonderrBundle.message("prompt.button.send")
        }
        ...
        return XmlStringUtil.wrapInHtml(
            XmlStringUtil.escapeString(tip) + "<br>" +
                XmlStringUtil.escapeString(SonderrBundle.message("prompt.button.send.tooltip.stop", shortcut))
        )
    }
    ...
    private fun placeholder(): String {
        ...
        if (send.isNotEmpty() && line.isNotEmpty()) {
            return SonderrBundle.message("prompt.placeholder.with.shortcuts", send, line)
        }
        if (send.isNotEmpty()) return SonderrBundle.message("prompt.placeholder.with.send", send)
        if (line.isNotEmpty()) return SonderrBundle.message("prompt.placeholder.with.newline", line)
        return SonderrBundle.message("prompt.placeholder")
    }
