...
internal class EmptySessionFeedback(
    ...
    companion object {
        ...
        fun content(open: (String) -> Unit): JComponent {
            val logo = JBLabel(IconLoader.getIcon("/icons/sonderr-content.svg", EmptySessionPanel::class.java)).apply {
            ...
            val actions = Stack.vertical(gap = UiStyle.Gap.sm())
                .next(ActionButton(SonderrBundle.message("feedback.dialog.github"), AllIcons.Vcs.Vendors.Github) {
                    open(GITHUB_ISSUES_URL)
                }.align(HAlign.CENTER, VAlign.CENTER))
                .next(ActionButton(SonderrBundle.message("feedback.dialog.discord"), DISCORD_ICON) {
                    open(DISCORD_URL)
                }.align(HAlign.CENTER, VAlign.CENTER))
                .next(ActionButton(SonderrBundle.message("feedback.dialog.support"), AllIcons.Actions.Help) {
                    open(SUPPORT_URL)
                }.align(HAlign.CENTER, VAlign.CENTER))
        ...
        private fun messageHtml() = XmlStringUtil.wrapInHtml(
            "<div style='text-align:center'>${XmlStringUtil.escapeString(SonderrBundle.message("feedback.dialog.message"))}</div>"
        )

        private fun buttonHtml() = XmlStringUtil.wrapInHtml(
            XmlStringUtil.escapeString(SonderrBundle.message("feedback.button"))
        )

        private const val SONDERR_URL = "https://kilocode.ai"
        private const val GITHUB_ISSUES_URL = "https://github.com/Sonderr-Org/sonderr/issues/new/choose"
