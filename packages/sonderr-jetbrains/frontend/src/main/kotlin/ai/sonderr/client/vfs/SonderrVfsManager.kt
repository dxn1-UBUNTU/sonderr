...
class SonderrVfsManager(private val project: Project) {
    @RequiresEdt
    fun open(kind: String, params: Map<String, String> = emptyMap(), focus: Boolean = true): Boolean {
        ...
        if (ApplicationManager.getApplication().isUnitTestMode) {
            file.putUserData(FileEditorProvider.KEY, SonderrFileEditorProvider())
        }
    ...
    private fun file(kind: String, params: Map<String, String>): SonderrVirtualFile? {
        val path = SonderrPath(kind, params)
        val fs = SonderrVirtualFileSystem.getInstance()
        return fs.refreshAndFindFileByPath(fs.getPath(path)) as? SonderrVirtualFile
    }

    companion object {
        internal val FOCUS: Key<Boolean> = Key.create("ai.sonderr.vfs.focus")
    }
}
