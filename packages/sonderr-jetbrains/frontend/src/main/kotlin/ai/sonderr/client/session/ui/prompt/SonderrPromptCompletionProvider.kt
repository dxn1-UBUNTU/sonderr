...
class SonderrPromptCompletionProvider(
    private val workspace: Workspace,
    private val service: SonderrWorkspaceService,
    ...
    private fun slash(prefix: String, result: CompletionResultSet) {
        ...
        result.withPrefixMatcher(PlainPrefixMatcher.ALWAYS_TRUE)
            .addElement(info(prefix, SonderrBundle.message("prompt.completion.noMatches")))
    }
    ...
    private fun mention(prefix: String, result: CompletionResultSet) {
        ...
        if (search.indexing) {
            val msg = SonderrBundle.message("prompt.mention.indexing")
        ...
        if (known.isEmpty() && search.files.isEmpty()) {
            val msg = SonderrBundle.message("prompt.completion.noMatches")
            out.addElement(info(prefix, msg))
        }
    }
