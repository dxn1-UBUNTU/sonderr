...
/**
 * App-level service wrapping [ai.sonderr.rpc.SonderrRunRpcApi]. Mirrors [SonderrWorktreeService]:
...
class SonderrRunService internal constructor(
    private val cs: CoroutineScope,
    private val rpc: SonderrRunRpcApi?,
) {
    /** Platform constructor — resolves RPC from the service container. */
    constructor(cs: CoroutineScope) : this(cs, null)

    companion object {
        private val LOG = SonderrLog.create(SonderrRunService::class.java)
    }

    private suspend fun <T> call(block: suspend SonderrRunRpcApi.() -> T): T {
        val api = rpc
        return if (api != null) block(api) else durable { block(SonderrRunRpcApi.getInstance()) }
    }
    ...
    fun states(directory: String): Flow<List<RunStateDto>> = flow {
        ...
        else durable { SonderrRunRpcApi.getInstance().states(directory).collect { emit(it) } }
    }

    /**
     * Fire-and-forget helpers for EDT click handlers, which have no thread-bound coroutine
     * scope (mirrors [SonderrWorktreeService.openInBackground]).
     */
