...
internal abstract class SettingsListPanel(
    ...
    protected val view = ActiveListView(SonderrBundle.message("settings.agentBehavior.empty"), cfg) { key, id ->
        onCell(key, id)
    }
    ...
    protected open fun emptyText(): String = SonderrBundle.message("settings.agentBehavior.empty")

    protected open fun loadingText(): String = SonderrBundle.message("settings.agentBehavior.loading")
    ...
    private fun toolbar(): JComponent {
        ...
        if (showRefresh()) {
            ...
            actions += SettingsToolbarAction(
                SonderrBundle.message("settings.agentBehavior.refresh"),
                SonderrBundle.message("settings.agentBehavior.refresh.description"),
    ...
    protected suspend fun waitForReady() {
        val flow = service<SonderrAppService>().state
        val saw = withTimeoutOrNull(RELOAD_START_TIMEOUT_MS) {
            flow.first { it.status != SonderrAppStatusDto.READY }
        } != null
        if (!saw) return
    ...
    private companion object {
        val LOG = SonderrLog.create(SettingsListPanel::class.java)
