...
internal class AgentCreateDialog(private val names: Collection<String>) : DialogWrapper(true), AgentCreateDialogHandle {
    ...
    private val mode = ComboBox(arrayOf(SonderrCliParser.MODE_PRIMARY, SonderrCliParser.MODE_SUBAGENT, SonderrCliParser.MODE_ALL)).apply {
        selectedItem = SonderrCliParser.MODE_PRIMARY
    }
    ...
    init {
        title = SonderrBundle.message("settings.agentBehavior.agents.create.title")
        init()
        initValidation()
    }
    ...
    override fun result(): AgentCreateDto = AgentCreateDto(
        ...
        mode = mode.selectedItem?.toString() ?: SonderrCliParser.MODE_PRIMARY,
    ...
    override fun createCenterPanel(): JComponent {
        ...
        SettingsRows().apply {
            row(SettingsRow(
                SonderrBundle.message("settings.agentBehavior.agents.create.name"),
                description = SonderrBundle.message("settings.agentBehavior.agents.create.name.description"),
                value = id,
            ))
            row(SettingsRow(
                SonderrBundle.message("settings.agentBehavior.agents.create.mode"),
                description = SonderrBundle.message("settings.agentBehavior.agents.create.mode.description"),
                value = mode,
            ))
            row(SettingsRow(
                SonderrBundle.message("settings.agentBehavior.agents.create.scope"),
                description = SonderrBundle.message("settings.agentBehavior.agents.create.scope.description"),
                value = scope,
            ))
            row(SettingsStackedRow(
                SonderrBundle.message("settings.agentBehavior.agents.create.description"),
                description = SonderrBundle.message("settings.agentBehavior.agents.create.description.description"),
                value = box(description),
            ))
            row(SettingsStackedRow(
                SonderrBundle.message("settings.agentBehavior.agents.create.prompt"),
                description = SonderrBundle.message("settings.agentBehavior.agents.create.prompt.description"),
                value = box(prompt),
            ))
            panel.next(this)
        }
    ...
    override fun getDimensionServiceKey(): String = "Sonderr.AgentCreateDialog"

    override fun doValidateAll(): List<ValidationInfo> = validateAgentCreate(result(), names).map { err ->
        ValidationInfo(SonderrBundle.message(err.key), component(err.field))
    }
    ...
    private class PromptField : EditorTextField(
        ...
        init {
            ...
            setPlaceholder(SonderrBundle.message("settings.agentBehavior.agents.create.prompt.placeholder"))
...
private fun projectLabel() = SonderrBundle.message("settings.agentBehavior.agents.create.scope.project")

private fun globalLabel() = SonderrBundle.message("settings.agentBehavior.agents.create.scope.global")
