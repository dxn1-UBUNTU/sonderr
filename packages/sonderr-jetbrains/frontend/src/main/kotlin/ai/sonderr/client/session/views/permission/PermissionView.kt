package ai.sonderr.client.session.views.permission
...
import ai.sonderr.client.session.views.base.AbstractSessionPartView
...
import ai.sonderr.client.ui.iconButton
import ai.sonderr.client.ui.editor.BashCommandHighlighter
...
import ai.sonderr.client.ui.md.MdCodeBlockBorder
import ai.sonderr.client.ui.md.MdCodeBlockFactory
import ai.sonderr.client.ui.md.MdCodeBlockOptions
import ai.sonderr.client.ui.md.MdCommon
import ai.sonderr.client.ui.md.MdView
import ai.sonderr.client.ui.md.MdViewFactory
...
/**
 * Transcript-style permission view — rendered inside [ai.sonderr.client.session.ui.SessionMessageListPanel]
 * at the end of the transcript when the session is in
 * [ai.sonderr.client.session.model.SessionState.AwaitingPermission].
 *
...
class PermissionView(
    ...
    init {
        ...
        setHeaderIcon(AllIcons.General.Warning, SonderrBundle.message("session.permission.title"))
        setContent(body)
        body.next(desc).next(codeSlot).next(diffRow).next(rules).next(state)
        setActions(
            listOf(
                DialogView.Action(ID_DENY, SonderrBundle.message("session.permission.reject"), primary = false) { reject() },
                DialogView.Action(ID_RUN, SonderrBundle.message("session.permission.allow.once"), primary = true) { allow() },
            ),
        )
    }
    ...
    fun show(permission: Permission) {
        ...
        setHeader(
            if (skillShell && !skill.isNullOrBlank())
                ...
                SonderrBundle.message("session.permission.skillShell.title", escapeControl(skill))
            else SonderrBundle.message("session.permission.title"),
        )
    ...
    private fun syncState(permission: Permission) {
        val msg = when (permission.state) {
            PermissionRequestState.ERROR ->
                permission.message ?: SonderrBundle.message("session.permission.error")
            PermissionRequestState.RESPONDING ->
                SonderrBundle.message("session.permission.responding")
            else -> null
        }
    ...
    // (packages/cli/src/sonderr/skills/display.ts); keep the ranges in sync.
    ...
    private fun toolLabel(tool: String): String = when (tool) {
        "read" -> SonderrBundle.message("session.permission.tool.read")
        "edit" -> SonderrBundle.message("session.permission.tool.edit")
        "write" -> SonderrBundle.message("session.permission.tool.write")
        "patch" -> SonderrBundle.message("session.permission.tool.patch")
        "multiedit" -> SonderrBundle.message("session.permission.tool.multiedit")
        "glob" -> SonderrBundle.message("session.permission.tool.glob")
        "grep" -> SonderrBundle.message("session.permission.tool.grep")
        "list" -> SonderrBundle.message("session.permission.tool.list")
        "bash" -> SonderrBundle.message("session.permission.tool.bash")
        "external_directory" -> SonderrBundle.message("session.permission.tool.external_directory")
        "webfetch" -> SonderrBundle.message("session.permission.tool.webfetch")
        "websearch" -> SonderrBundle.message("session.permission.tool.websearch")
        "codesearch" -> SonderrBundle.message("session.permission.tool.codesearch")
        "todoread" -> SonderrBundle.message("session.permission.tool.todoread")
        "todowrite" -> SonderrBundle.message("session.permission.tool.todowrite")
        "task" -> SonderrBundle.message("session.permission.tool.task")
        "skill" -> SonderrBundle.message("session.permission.tool.skill")
        "lsp" -> SonderrBundle.message("session.permission.tool.lsp")
        else -> tool
    }
    ...
    private fun syncPrimaryText() {
        ...
        setActionText(
            ID_RUN,
            SonderrBundle.message(key),
        )
        setActionText(
            ID_DENY,
            SonderrBundle.message("session.permission.reject"),
        )
        syncButtons(responding)
    }
    ...
    // Test helpers
    internal fun runButtonForTest() = buttons(this).first { it.text == SonderrBundle.message("session.permission.allow") || it.text == SonderrBundle.message("session.permission.allow.once") }
    internal fun denyButtonForTest() = buttons(this).first { it.text == SonderrBundle.message("session.permission.reject") }
...
internal class PermissionRulesView private constructor(
...
) : AbstractSessionPartView(parts.panel, box, expanded = SonderrPluginSettings.getPermissionRulesExpanded()) {
    ...
    override fun userToggled() {
        SonderrPluginSettings.setPermissionRulesExpanded(isExpanded())
    }
    ...
    private class Header {
        ...
        val title = JBLabel(SonderrBundle.message("session.permission.rules.title"))
    ...
    private class RuleRow(
        ...
        fun update(value: PermissionRuleDecision) {
            ...
            hint.text = SonderrBundle.message(when (value) {
        ...
        private fun defaultLabel(): String = when (default) {
            PermissionRuleDecision.APPROVED -> SonderrBundle.message("session.permission.allow")
            PermissionRuleDecision.DENIED -> SonderrBundle.message("session.permission.reject")
            PermissionRuleDecision.PENDING -> SonderrBundle.message("session.permission.ask")
        }
    ...
    private class RuleToggleButton(
        ...
        fun update(value: Boolean) {
            ...
            val text = SonderrBundle.message(key)
            toolTipText = text
