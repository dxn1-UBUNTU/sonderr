...
import ai.sonderr.client.settings.profile.ProfileUi
import ai.sonderr.client.settings.profile.formatResetDate
import ai.sonderr.client.settings.profile.formatShortBalance
import ai.sonderr.client.testing.FakeAppRpcApi
import ai.sonderr.client.testing.TEST_WAIT_MS
...
class UserProfileConfigurableTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        rpc = FakeAppRpcApi()
        app = SonderrAppService(coroutines.scope, rpc)
        app._state.value = SonderrAppStateDto(SonderrAppStatusDto.READY)
        edt {
            panel = ProfileUi(
                profile = null,
    ...
    fun `test login updates profile UI`() {
        ...
        edt {
            ...
            buttons(panel).first { it.text == "Login with Sonderr" }.doClick()
        }
        flush()
    ...
    fun `test logout updates profile UI`() {
        ...
        app._state.value = SonderrAppStateDto(SonderrAppStatusDto.READY, profile = profile)
        edt { panel.update(profile, SonderrAppStatusDto.READY) }
        ...
        edt {
            ...
            assertTrue(buttons(panel).any { it.text == "Login with Sonderr" })
        }
    }
    ...
    fun `test organization switch updates balance UI`() {
        ...
        app._state.value = SonderrAppStateDto(SonderrAppStatusDto.READY, profile = personal)
        edt { panel.update(personal, SonderrAppStatusDto.READY) }
    ...
    fun `test logged in profile uses compact stack and copyable email`() {
        ...
        edt {
            ...
            layout(panel)
            val logo = labelsByName(panel, "sonderr.profile.logo.loggedIn").single()
    ...
    fun `test personal profile shows sonderr pass usage`() {
        val profile = ProfileDto(
            email = "alice@test.com",
            name = "Alice",
            balance = ProfileBalanceDto(267.59),
            sonderrPass = ProfileSonderrPassDto(
        ...
        edt {
            val t = text(panel)
            assertTrue(t, t.contains("Sonderr Pass"))
            ...
            assertTrue("pass meter should be visible", panelsByName(panel, "sonderr.profile.passPanel").single().isVisible)
            assertFalse(t, t.contains("Get Sonderr Pass"))
        }
    }

    fun `test sonderr pass short amounts use half up rounding`() {
        assertEquals("$3", formatShortBalance(2.5))
        assertEquals("$2", formatShortBalance(2.49))
    }

    fun `test sonderr pass hides empty bonus and invalid renewal rows`() {
        val profile = ProfileDto(
            email = "alice@test.com",
            name = "Alice",
            balance = ProfileBalanceDto(267.59),
        ...
        val invalid = profile.copy(
            sonderrPass = profile.sonderrPass?.copy(
                currentPeriodBonusCreditsUsd = 0.0,
                nextBillingAt = "not-a-date",
            ),
        )
        edt { panel.update(invalid, SonderrAppStatusDto.READY) }
        ...
        val none = invalid.copy(sonderrPass = invalid.sonderrPass?.copy(nextBillingAt = null))
        edt { panel.update(none, SonderrAppStatusDto.READY) }
    ...
    fun `test sonderr pass meter clamps base zero and avoids noop repaint`() {
        val profile = ProfileDto(
            email = "alice@test.com",
            name = "Alice",
            balance = ProfileBalanceDto(267.59),
        ...
        val meter = edt { componentsByName(panel, "sonderr.profile.passMeter").single() }
        ...
        edt { panel.update(full, SonderrAppStatusDto.READY) }
    ...
    fun `test personal profile shows sonderr pass without balance`() {
        val profile = ProfileDto(
            email = "alice@test.com",
            name = "Alice",
            sonderrPass = ProfileSonderrPassDto(
        ...
        val updated = profile.copy(
            sonderrPass = profile.sonderrPass?.copy(currentPeriodUsageUsd = 88.0),
        )
        ...
        edt {
            ...
            assertTrue("pass panel should be visible", panelsByName(panel, "sonderr.profile.passPanel").single().isVisible)
    ...
    fun `test personal profile without sonderr pass shows subscribe link`() {
        val profile = ProfileDto(
            email = "alice@test.com",
            name = "Alice",
            balance = ProfileBalanceDto(10.0),
        ...
        edt {
            val t = text(panel)
            assertTrue(t, t.contains("Get Sonderr Pass to add credits and earn bonuses"))
            buttons(panel).first { it.text == "Dashboard" }.doClick()
            buttons(panel).first { it.text == "Top up" }.doClick()
            buttons(panel).first { it.text == "Get Sonderr Pass to add credits and earn bonuses" }.doClick()
        }

        assertEquals(listOf("https://app.kilo.ai/profile", "https://app.kilo.ai/credits", "https://kilo.ai/pricing/sonderr-pass"), urls)
    }

    fun `test sonderr pass renewal date uses utc`() {
    ...
    fun `test org profile hides sonderr pass`() {
        val orgs = listOf(ProfileOrganizationDto(id = "org_1", name = "Acme", role = "ADMIN"))
        val profile = ProfileDto(
            email = "alice@test.com",
            name = "Alice",
        ...
        edt {
            val t = text(panel)
            assertFalse(t, t.contains("Sonderr Pass"))
            assertFalse(t, t.contains("Get Sonderr Pass"))
        }
    }
    ...
    fun `test logged out update retains login button`() {
        edt {
            val btn = buttons(panel).first { it.text == "Login with Sonderr" }
            panel.update(null, SonderrAppStatusDto.READY)
            val btn2 = buttons(panel).first { it.text == "Login with Sonderr" }
            assertSame(btn, btn2)
        }
    }

    fun `test logged out profile shows sonderr icon above login content`() {
        edt {
            panel.update(null, SonderrAppStatusDto.READY)
            panel.setSize(800, 600)
            layout(panel)

            val logo = labelsByName(panel, "sonderr.profile.logo.loggedOut").single()
    ...
    fun `test account update retains name label`() {
        ...
        edt {
            panel.update(alice, SonderrAppStatusDto.READY)
            val lbl = labels(panel).first { it.text == "Alice" }
            panel.update(bob, SonderrAppStatusDto.READY)
    ...
    fun `test organization switch keeps account visible during transient null profile`() {
        ...
        val transientState = SonderrAppStateDto(
            status = SonderrAppStatusDto.READY,
            profile = null,
        ...
        edt {
            panel.update(personal, SonderrAppStatusDto.READY)
    ...
    fun `test login shows device auth card before completion`() {
        ...
        flushUntil { text(panel).contains("Sign in to Sonderr") }

        edt {
            val t = text(panel)
            assertTrue(t, t.contains("Sign in to Sonderr"))
        ...
        edt {
            val qr = labelsByName(panel, "sonderr.login.qr").firstOrNull()
            assertNotNull(qr)
    ...
    fun `test cancel login invalidates stale completion`() {
        ...
        edt { buttons(panel).first { it.text == "Login with Sonderr" }.doClick() }
        flushUntil { text(panel).contains("Sign in to Sonderr") }
    ...
    fun `test auth card retains qr label across sync`() {
        ...
        val qrBefore = edt { labelsByName(panel, "sonderr.login.qr").firstOrNull() }
        assertNotNull(qrBefore)

        // Force another sync call while still pending
        edt { panel.update(null, SonderrAppStatusDto.READY) }
        flush()

        val qrAfter = edt { labelsByName(panel, "sonderr.login.qr").firstOrNull() }
    ...
    fun `test url field selects all on click`() {
        ...
        edt {
            val field = fieldsByName(panel, "sonderr.login.url").firstOrNull()
    ...
    fun `test balance card has card background`() {
        ...
        edt {
            val card = panelsByName(panel, "sonderr.profile.balanceCard").firstOrNull()
    ...
    fun `test code panel has card background`() {
        ...
        edt {
            val codePanel = panelsByName(panel, "sonderr.login.codePanel").firstOrNull()
    ...
    fun `test combo model not rebuilt on balance change with same org list`() {
        ...
        edt { panel.update(updated, SonderrAppStatusDto.READY) }
    ...
    fun `test combo model updated in place when org list changes`() {
        ...
        app._state.value = SonderrAppStateDto(SonderrAppStatusDto.READY, profile = profile1)
        edt { panel.update(profile1, SonderrAppStatusDto.READY) }
        ...
        edt { panel.update(profile2, SonderrAppStatusDto.READY) }
    ...
    fun `test profile update does not trigger organization rpc`() {
        ...
        app._state.value = SonderrAppStateDto(SonderrAppStatusDto.READY, profile = profile)
        edt { panel.update(profile, SonderrAppStatusDto.READY) }
        edt { panel.update(profile.copy(currentOrgId = "org_1"), SonderrAppStatusDto.READY) }
        flush()
    ...
    fun `test connecting while logged in keeps logged-in card visible`() {
        ...
        edt { panel.update(null, SonderrAppStatusDto.CONNECTING) }
    ...
    fun `test loading while logged in keeps logged-in card visible`() {
        ...
        edt { panel.update(null, SonderrAppStatusDto.LOADING) }
    ...
    fun `test loading with null profile while logged in does not crash`() {
        ...
        edt { panel.update(SonderrAppStateDto(SonderrAppStatusDto.LOADING)) }
        ...
        edt { panel.update(switched, SonderrAppStatusDto.READY) }
    ...
    fun `test connecting while logged in with org selected keeps combo selection`() {
        ...
        app._state.value = SonderrAppStateDto(SonderrAppStatusDto.READY, profile = profile)
        edt {
            panel.update(profile, SonderrAppStatusDto.READY)
            combos(panel).single().selectedIndex = 1
        }
    ...
    fun `test preferred focus for logged-out is login button`() {
        edt {
            val focus = panel.preferredFocus()
            val loginBtn = buttons(panel).firstOrNull { it.text == "Login with Sonderr" }
