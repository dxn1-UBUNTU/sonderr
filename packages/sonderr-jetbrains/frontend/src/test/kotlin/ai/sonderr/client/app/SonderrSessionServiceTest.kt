...
import ai.sonderr.client.testing.TestLog
...
class SonderrSessionServiceTest : BasePlatformTestCase() {
    private lateinit var scope: CoroutineScope
    private lateinit var rpc: FakeSessionRpcApi
    private lateinit var service: SonderrSessionService

    override fun setUp() {
        super.setUp()
        scope = CoroutineScope(SupervisorJob())
        rpc = FakeSessionRpcApi()
        service = SonderrSessionService(project, scope, rpc)
    }
    ...
    fun `test events logs normal completion`() = runBlocking(Dispatchers.Default) {
        val log = TestLog()
        service = SonderrSessionService(project, scope, rpc, log)
