...
import ai.sonderr.client.session.model.PromptAttachment
import ai.sonderr.client.session.ui.attachment.AttachmentCard
import ai.sonderr.client.session.ui.attachment.AttachmentCardItem
...
import ai.sonderr.client.session.ui.prompt.PROMPT_ATTACHMENT_PASTE_HANDLER_KEY
import ai.sonderr.client.session.ui.prompt.PromptAttachmentPasteHandler
import ai.sonderr.client.session.ui.prompt.PromptAttachmentPasteProvider
...
import ai.sonderr.client.test.CopyProviderSink
...
class PromptPanelTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        workspaces = SonderrWorkspaceService(scope, rpc)
    }
    ...
    fun `test empty enhance explanation resizes wrapped input`() {
        ...
        assertTrue(editor.preferredSize.height > min)
        assertEquals(SonderrBundle.message("prompt.action.enhance.description"), editor.text)
    }
    ...
    fun `test prompt editor component undo redo shortcuts target prompt editor`() {
        ...
        invokeComponentAction("Sonderr Session Undo", editor)
        assertEquals("", editor.document.text)
        invokeComponentAction("Sonderr Session Redo", editor)
        assertEquals("hello", editor.document.text)
    }
    ...
    fun `test prompt local completion shortcut opens mention lookup`() {
        rpc.searchResult = ai.sonderr.rpc.dto.FileSearchResultDto(
            files = listOf(ai.sonderr.rpc.dto.WorkspaceFileDto("src/deploy.ts", "deploy.ts")),
        )
        val panel = PromptPanel(project = project, onSend = { _, _ -> }, onAbort = {}, onEnhance = { _, _ -> }, completion = completion())
        val field = panel.defaultFocusedComponent as EditorTextField
    ...
    fun `test slash lookup refreshes when server commands load`() {
        ...
        rpc.state.value = SonderrWorkspaceStateDto(SonderrWorkspaceStatusDto.READY, commands = listOf(CommandDto("deploy")))
    ...
    fun `test dialog prompt hides runtime submit and approve controls`() {
        ...
        assertFalse(buttons(panel).any { it.accessibleContext.accessibleName == SonderrBundle.message("prompt.action.menu") })
    }

    fun `test session menu button shown by default and does not throw without a registered action`() {
        val panel = PromptPanel(project = project, onSend = { _, _ -> }, onAbort = {}, onEnhance = { _, _ -> })

        val menu = buttons(panel).single { it.accessibleContext.accessibleName == SonderrBundle.message("prompt.action.menu") }
        assertEquals(SonderrBundle.message("prompt.action.menu"), menu.toolTipText)

        // Sonderr.Session.PromptMenu is not registered with ActionManager in the test fixture (the
    ...
    fun `test hidden submit button still exposes send context from editor`() {
        ...
        val sink = TestSink()
        (editor as UiDataProvider).uiDataSnapshot(sink)
        val send = sink.send as ai.sonderr.client.session.ui.prompt.SendPromptContext
    ...
    fun `test attachment tooltip hides embedded binary content`() {
        ...
        assertTrue(tip.contains("Location: ${SonderrBundle.message("prompt.attachment.embedded")}"))
    ...
    fun `test prompt paste provider invokes registered handler`() {
        ...
        val item = FileListTransferable(listOf(File.createTempFile("sonderr-paste", ".txt")))
    ...
    fun `test file list paste adds attachment`() {
        val panel = PromptPanel(project, { _, _ -> }, {}, { _, _ -> })
        val file = File.createTempFile("sonderr-paste", ".txt")
    ...
    fun `test frontend text file attachment stays file reference`() {
        ...
        val item = ai.sonderr.client.session.model.PromptAttachmentExtractor.files(listOf(file)).single()
    ...
    fun `test pasted frontend file sends reference payload`() {
        var sent: ai.sonderr.rpc.dto.PromptPartDto? = null
    ...
    fun `test file paste ignores companion image flavor`() {
        val panel = PromptPanel(project, { _, _ -> }, {}, { _, _ -> })
        val file = File.createTempFile("sonderr-paste", ".png")
    ...
    fun `test disabled media model allows file reference attachment`() {
        val panel = PromptPanel(project, { _, _ -> }, {}, { _, _ -> })
        val file = File.createTempFile("sonderr-paste", ".php")
    ...
    fun `test prompt button switches between send and stop state`() {
        ...
        assertEquals(KeymapUtil.createTooltipText("Send", "Sonderr.SendPrompt"), panel.buttonForTest().toolTipText)
    ...
    fun `test auto approve button toggles and updates tooltip`() {
        ...
        assertEquals(SonderrBundle.message("prompt.action.autoApprove.enable"), button.accessibleContext.accessibleName)
        assertEquals(SonderrBundle.message("prompt.action.autoApprove.disabled.tooltip"), button.toolTipText)
        val icon = button.icon
        ...
        assertNotSame(icon, button.icon)
        assertEquals(SonderrBundle.message("prompt.action.autoApprove.disable"), button.accessibleContext.accessibleName)
        assertEquals(SonderrBundle.message("prompt.action.autoApprove.enabled.tooltip"), button.toolTipText)
    ...
    private fun autoApproveButton(panel: PromptPanel): JButton {
        val enable = SonderrBundle.message("prompt.action.autoApprove.enable")
        val disable = SonderrBundle.message("prompt.action.autoApprove.disable")
    ...
    private fun attachmentRemoveButton(panel: PromptPanel, item: PromptAttachment): JButton {
        val name = SonderrBundle.message("prompt.attachment.remove", item.name)
    ...
    private fun enhanceButton(panel: PromptPanel): JButton {
        val name = SonderrBundle.message("prompt.action.enhance")
        return buttons(panel).first { it.accessibleContext.accessibleName == name }
    }
    ...
    private fun completion() = SonderrPromptCompletionProvider(
    ...
    private fun invokeCompletionAction(editor: Editor) {
        val action = ActionUtil.getActions(editor.contentComponent).first { item ->
            item.templatePresentation.text == "Sonderr Prompt Completion"
        }
