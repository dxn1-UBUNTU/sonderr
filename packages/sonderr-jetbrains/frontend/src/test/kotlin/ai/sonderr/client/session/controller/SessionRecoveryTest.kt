 ...
 * reconnect. JetBrains now does the same in [ai.sonderr.client.session.controller.SessionController.recoverPending].
 */
class SessionRecoveryTest : SessionControllerTestBase() {
    ...
    // ------ Status seeding from SonderrSessionService.statuses ------
    ...
    fun `test retry status is seeded with message attempt and next`() {
        ...
        assertSession(
            """
            [code] [sonderr/gpt-5] [retry] [Rate limited]
            """,
            m, show = true,
        )
    ...
    fun `test offline status is seeded with message and requestId`() {
        ...
        assertSession(
            """
            [code] [sonderr/gpt-5] [offline] [No network]
            """,
            m, show = true,
        )
    ...
    fun `test aborted tail recovers as interrupted even when the server reports idle`() {
        ...
        assertEquals(
            ai.sonderr.client.session.model.Outcome.INTERRUPTED,
    ...
    fun `test incomplete tail recovers even when the server reports idle`() {
        ...
        assertTrue(state is SessionState.TurnEnded)
        assertEquals(
            ai.sonderr.client.session.model.Outcome.INCOMPLETE,
            (state as SessionState.TurnEnded).outcome,
        )
