...
/**
 * Service-level tests for [SonderrAppService] profile/login/logout/org operations.
 *
 * Uses [FakeAppRpcApi] to avoid RPC/backend involvement.
 */
@Suppress("UnstableApiUsage")
class SonderrAppServiceTest : BasePlatformTestCase() {
    ...
    fun `test refreshProfile returns null and leaves existing state on exception`() = runBlocking(Dispatchers.Default) {
        ...
        app._state.value = SonderrAppStateDto(SonderrAppStatusDto.READY, profile = existing)
    ...
    fun `test logout clears profile when rpc returns true`() = runBlocking(Dispatchers.Default) {
        val prof = profile()
        app._state.value = SonderrAppStateDto(SonderrAppStatusDto.READY, profile = prof)
    ...
    fun `test setOrganization updates profile for personal null selection`() = runBlocking(Dispatchers.Default) {
        ...
        app._state.value = SonderrAppStateDto(SonderrAppStatusDto.READY, profile = org)
