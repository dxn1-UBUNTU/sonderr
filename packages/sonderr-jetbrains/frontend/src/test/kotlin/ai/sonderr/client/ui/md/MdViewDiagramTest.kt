...
import ai.sonderr.client.session.ui.selection.SessionTargetResolver
...
import ai.sonderr.client.ui.diagram.Engine
import ai.sonderr.client.ui.diagram.Fault
import ai.sonderr.client.ui.diagram.Mark
import ai.sonderr.client.ui.diagram.Out
import ai.sonderr.client.ui.diagram.Pt
import ai.sonderr.client.ui.diagram.Role
import ai.sonderr.client.ui.diagram.Scene
import ai.sonderr.client.ui.diagram.Size
import ai.sonderr.client.ui.diagram.Spec
import ai.sonderr.client.ui.diagram.Type
import ai.sonderr.client.ui.diagram.ui.DiagramBlock
import ai.sonderr.client.ui.diagram.ui.DiagramHandle
...
class MdViewDiagramTest : BasePlatformTestCase() {
    ...
    fun `test an unsupported diagram type reads as a note rather than an error`() {
        ...
        assertFalse(diagram().isVisible)
        assertTrue(codePane().isVisible)
        assertTrue(labels().contains(SonderrBundle.message("diagram.unsupported")))
    ...
    private fun copyButton() = descendants(block().copyToolbar!!)
        ...
        .single { it.toolTipText == SonderrBundle.message("session.copy.hover") }
    ...
    private class FakeEngine : Engine {
        ...
        override suspend fun draw(source: String, spec: Spec): Out {
            ...
            return out ?: Out.Ok(
                Scene(
                    Type.Flowchart,
                    listOf(Mark.Edge(listOf(Pt(10.0, 10.0), Pt(80.0, 10.0)), Role.Line, head = ai.sonderr.client.ui.diagram.Head.Arrow)),
                    Size(100.0, 30.0),
                ),
            )
        }
