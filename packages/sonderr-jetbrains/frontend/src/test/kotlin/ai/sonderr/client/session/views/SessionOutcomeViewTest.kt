...
import ai.sonderr.client.session.ui.SessionLayout
...
class SessionOutcomeViewTest : BasePlatformTestCase() {
    ...
    fun `test showError renders title and message`() {
        edt {
            ...
            assertNotNull(findText(view, SonderrBundle.message("session.error.title")))
    ...
    fun `test showOutcome renders interrupted note without icon`() {
        edt {
            ...
            assertTrue(view.isVisible)
            assertNotNull(findText(view, SonderrBundle.message("session.outcome.interrupted.note")))
    ...
    fun `test showOutcome updates without stale text`() {
        edt {
            ...
            assertNotNull(findText(view, SonderrBundle.message("session.outcome.failed.title")))
            assertNotNull(findText(view, SonderrBundle.message("session.outcome.failed.description")))
            assertNull(findText(view, SonderrBundle.message("session.outcome.interrupted.note")))
            assertNull(findText(view, SonderrBundle.message("session.outcome.incomplete.title")))
    ...
    fun `test incomplete outcome shows warning without retry`() {
        edt {
            ...
            assertTrue(view.isVisible)
            assertNotNull(findText(view, SonderrBundle.message("session.outcome.incomplete.title")))
            assertNotNull(findText(view, SonderrBundle.message("session.outcome.incomplete.description")))
            assertIcons(view, AllIcons.General.Warning)
            assertTrue(findAll<JBLabel>(view).any {
                it.icon == AllIcons.General.Warning &&
                    it.toolTipText == SonderrBundle.message("session.outcome.incomplete.reason", "unknown")
            })
    ...
    fun `test incomplete outcome falls back to title tooltip`() {
        edt {
            ...
            assertTrue(findAll<JBLabel>(view).any {
                it.icon == AllIcons.General.Warning &&
                    it.toolTipText == SonderrBundle.message("session.outcome.incomplete.title")
            })
        }
    }
    ...
    fun `test error card hides retry when the transcript has nothing to replay`() {
        edt {
            ...
            view.showError("invalid sonderr.json", "UnknownError")
    ...
    fun `test retry appears once the transcript becomes replayable`() {
        edt {
            ...
            val buttons = findAll<JButton>(view).filter { it.text == SonderrBundle.message("session.outcome.retry") }
    ...
    private fun retryButton(root: Container) =
        findAll<JButton>(root).firstOrNull { it.text == SonderrBundle.message("session.outcome.retry") }
    ...
    fun `test showRetry offers the action with no message of its own`() {
        edt {
            ...
            assertNull(
                ...
                findText(view, SonderrBundle.message("session.outcome.failed.description")),
            )
