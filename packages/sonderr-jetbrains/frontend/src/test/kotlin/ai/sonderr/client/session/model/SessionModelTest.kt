...
class SessionModelTest : BasePlatformTestCase() {
    ...
    fun `test initial app and workspace state`() {
        assertEquals(SonderrAppStatusDto.DISCONNECTED, model.app.status)
        assertEquals(SonderrWorkspaceStatusDto.PENDING, model.workspace.status)
    ...
    fun `test isReady requires app and workspace readiness`() {
        model.app = SonderrAppStateDto(SonderrAppStatusDto.READY)
        assertFalse(model.isReady())

        model.workspace = SonderrWorkspaceStateDto(SonderrWorkspaceStatusDto.READY)
