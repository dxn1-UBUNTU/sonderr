...
class WorktreeSessionEditorPanelTest : BasePlatformTestCase() {
    ...
    private val workspace = Workspace(DIR, kotlinx.coroutines.flow.MutableStateFlow(ai.sonderr.rpc.dto.SonderrWorkspaceStateDto(ai.sonderr.rpc.dto.SonderrWorkspaceStatusDto.READY)), {}, {})
    ...
    fun `test open action disabled without a worktree directory`() {
        val blank = Workspace("", kotlinx.coroutines.flow.MutableStateFlow(ai.sonderr.rpc.dto.SonderrWorkspaceStateDto(ai.sonderr.rpc.dto.SonderrWorkspaceStatusDto.READY)), {}, {})
        val opened = mutableListOf<String>()
        val view = edt {
    ...
    fun `test running session row shows activity badge without leading icon`() {
        ...
        assertEquals(SonderrBundle.message("session.part.tool.running"), row.badges.single().text)
    ...
    fun `test plan session row shows activity badge without leading icon`() {
        ...
        assertNull(row.icon)
        assertEquals(SonderrBundle.message("history.badge.plan"), row.badges.single().text)
    }
    ...
    fun `test deleting row shows deleting state`() {
        ...
        assertEquals(SonderrBundle.message("common.deleting"), row.progress)
    }
    ...
    private companion object {
        const val DIR = "/repo/.sonderr/worktrees/feature-x"
    }
}
