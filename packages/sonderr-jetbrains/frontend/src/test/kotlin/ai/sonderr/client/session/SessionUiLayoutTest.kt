...
import ai.sonderr.client.session.ui.RevertProgress
import ai.sonderr.client.session.ui.SessionDropOverlay
import ai.sonderr.client.session.ui.SessionLayoutPanel
...
class SessionUiLayoutTest : SessionUiTestBase() {
    ...
    fun `test dock hides while a turn runs`() {
        ...
        ApplicationManager.getApplication()
            .replaceService(SonderrWorktreeService::class.java, SonderrWorktreeService(scope, worktree), testRootDisposable)
        rpc.history.addAll(history(1))
    ...
    fun `test dock branch changes refresh on finish and revert`() {
        ...
        workspaceRpc.branchDiffs.add(DiffFileDto("src/A.kt", 2, 1))
        val badge = find<ai.sonderr.client.session.ui.header.BranchChangesBadge>(ui)
    ...
    fun `test account overlay shows after recents complete`() {
        appRpc.state.value = SonderrAppStateDto(SonderrAppStatusDto.READY, profile = ProfileDto(email = "user@example.com"))
        rpc.recent.add(session("ses_1"))
        ui = newUi(displayMs = 1_000)
