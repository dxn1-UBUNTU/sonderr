...
class SonderrIgnoreCacheTest : BasePlatformTestCase() {
    fun `test matcher caches until ignore file changes`() {
        val file = myFixture.addFileToProject(".sonderrignore", "ignored/\n").virtualFile
        val root = file.parent
        val cache = project.service<SonderrIgnoreCache>()
    ...
    fun `test null root allows everything`() {
        assertSame(SonderrIgnore.EMPTY, project.service<SonderrIgnoreCache>().matcher(null))
    }
}
