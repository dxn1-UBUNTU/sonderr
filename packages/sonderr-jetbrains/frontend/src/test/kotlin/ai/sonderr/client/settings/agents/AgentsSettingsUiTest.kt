...
import ai.sonderr.client.testing.FakeAgentBehaviorRpcApi
...
class AgentsSettingsUiTest : BasePlatformTestCase() {
    ...
    fun `test changing default agent saves patch`() {
        ...
        assertEquals("ask", appRpc.configPatches.single().values[SonderrCliParser.CONFIG_DEFAULT_AGENT])
    ...
    fun `test apply waits for backend ready before refetching`() {
        ...
        flushUntil { rows(panel).size == 6 }
        appRpc.afterConfig = {
            app._state.value = app._state.value.copy(status = SonderrAppStatusDto.LOADING)
        ...
        assertTrue(edt { text(panel).contains(SonderrBundle.message("settings.agentBehavior.saving")) })

        app._state.value = app._state.value.copy(status = SonderrAppStatusDto.READY)
    ...
    fun `test importing an agent stages patch until apply`() {
        ...
        appRpc.afterConfig = { patch ->
            if (patch.agents.containsKey("reviewer")) {
                agentRpc.agents = agentRpc.agents + AgentDetailDto(
                    "reviewer",
                    description = "Reviews code",
                    mode = SonderrCliParser.MODE_SUBAGENT,
                    native = false,
                    removable = true,
                )
            }
        ...
        edt {
            ...
            assertEquals(SonderrCliParser.MODE_SUBAGENT, patch.mode)
    ...
    fun `test importing invalid JSON shows settings error message`() {
        ...
        val message = SonderrBundle.message("settings.agentBehavior.agents.import.invalidJson")
    ...
    fun `test apply failure reloads and keeps remaining staged changes`() {
        ...
        flushUntil { text(panel).contains(SonderrBundle.message("settings.agentBehavior.agents.create.failed")) }
    ...
    private fun install() {
        ...
        agentRpc = FakeAgentBehaviorRpcApi().apply {
            agents = listOf(
                AgentDetailDto("ask", displayName = "Ask", description = "Ask questions", mode = SonderrCliParser.MODE_PRIMARY, native = true),
                AgentDetailDto("code", displayName = "Code", description = "Code things", mode = SonderrCliParser.MODE_PRIMARY, native = true),
                AgentDetailDto("generated", description = "Generated", mode = SonderrCliParser.MODE_SUBAGENT, native = false, removable = false),
                AgentDetailDto("hidden", description = "Hidden custom", mode = SonderrCliParser.MODE_PRIMARY, native = false, removable = true, hidden = true),
                AgentDetailDto("old", description = "Old custom", mode = SonderrCliParser.MODE_PRIMARY, native = false, removable = true, deprecated = true),
                AgentDetailDto("worker", description = "Worker", mode = SonderrCliParser.MODE_SUBAGENT, native = false, removable = true),
            )
        }
        val workspaceRpc = FakeWorkspaceRpcApi().apply { models = ModelsWorkspaceDto(providers()) }
        app = SonderrAppService(cs, appRpc)
        val ready = SonderrAppStateDto(
        ...
        ApplicationManager.getApplication().replaceService(SonderrAppService::class.java, app, testRootDisposable)
        ApplicationManager.getApplication().replaceService(SonderrAgentBehaviorService::class.java, SonderrAgentBehaviorService(cs, agentRpc), testRootDisposable)
        ApplicationManager.getApplication().replaceService(SonderrWorkspaceService::class.java, SonderrWorkspaceService(cs, workspaceRpc), testRootDisposable)
    }

    private fun providers() = ProvidersDto(
        providers = listOf(ProviderDto("sonderr", "Sonderr", models = mapOf("gpt-5" to ModelDto("gpt-5", "GPT-5")))),
