...
class PromptLifecycleTest : SessionControllerTestBase() {
    ...
    override fun setUp() {
        super.setUp()
        edt { SonderrPluginSettings.unsetAutoApprove() }
    }
    ...
    fun `test prompt records aggregate mention telemetry`() {
        appRpc.state.value = ai.sonderr.rpc.dto.SonderrAppStateDto(ai.sonderr.rpc.dto.SonderrAppStatusDto.READY, config = ai.sonderr.rpc.dto.ConfigDto(model = "sonderr/gpt-5"))
    ...
    fun `test delete queued message delegates to RPC`() {
        ...
        assertEquals(listOf(ai.sonderr.client.testing.FakeSessionRpcApi.MessageDeleteCall("ses_test", "/test", "u2")), rpc.messageDeletes)
    ...
    fun `test PermissionAsked moves state to AwaitingPermission`() {
        ...
        assertSession(
            ...
            [code] [sonderr/gpt-5] [awaiting-permission]
            """,
            m,
        )
    }
    ...
    fun `test PermissionReplied resumes Busy state`() {
        ...
        assertSession(
            """
            [code] [sonderr/gpt-5] [busy] [considering next steps]
            """,
            m,
        )
    }
    ...
    fun `test QuestionAsked moves state to AwaitingQuestion`() {
        ...
        assertSession(
            ...
            [code] [sonderr/gpt-5] [awaiting-question]
            """,
            m,
        )
    }
    ...
    fun `test recovery surfaces a pending skill shell as a card under auto approve`() {
        ...
        projectRpc.state.value = workspaceReady()
        rpc.pendingPermissionList.add(permission("perm_skill").copy(metadata = mapOf("skillShell" to "true")))
        edt { SonderrPluginSettings.setAutoApprove(true) }
    ...
    fun `test auto approve persists in properties`() {
        ...
        assertFalse(SonderrPluginSettings.getAutoApprove())
        edt { m.setAutoApprove(true) }

        assertTrue(SonderrPluginSettings.getAutoApprove())
    ...
    fun `test child non-permission events do not change root state`() {
        ...
        emit(ChatEventDto.SessionStatusChanged("ses_child", ai.sonderr.rpc.dto.SessionStatusDto("busy")), flush = false)
        ...
        val stateEvents = modelEvents.filterIsInstance<ai.sonderr.client.session.model.SessionModelEvent.StateChanged>()
