...
class SonderrDiffEditorContentTest : BasePlatformTestCase() {
    ...
    fun `test tree toolbar installs actions in order`() {
        ...
        try {
            ...
            assertEquals(SonderrBundle.message("diff.editor.refresh"), actions[0].templatePresentation.text)
            assertTrue(actions[1] is Separator)
            assertEquals(SonderrBundle.message("diff.editor.tree.expandAll"), actions[2].templatePresentation.text)
            assertEquals(SonderrBundle.message("diff.editor.tree.collapseAll"), actions[3].templatePresentation.text)
    ...
    fun `test diff request shows placeholder for blank added patch`() {
        ...
        assertEquals(SonderrBundle.message("diff.editor.patch.unavailable"), contents[1])
    }
    ...
    fun `test inline params require directory and token`() {
        assertTrue(SonderrDiffEditorKind.isValid(diffParams("inline", "/repo", null, "Session Changes", token = "turn:ses_1:u1")))
        assertFalse(SonderrDiffEditorKind.isValid(mapOf("source" to "inline", "directory" to "/repo", "title" to "Session Changes")))
        assertFalse(SonderrDiffEditorKind.isValid(mapOf("source" to "inline", "token" to "turn:ses_1:u1", "title" to "Session Changes")))
    }

    fun `test inline editor title uses params title`() {
        assertEquals("Session Changes", SonderrDiffEditorKind.title(diffParams("inline", "/repo", null, "Session Changes", token = "token")))
    }
