...
class SessionFileLinksTest : BasePlatformTestCase() {
    ...
    fun `test managed worktree storage filter only rejects worktree subtree`() {
        ...
        assertFalse(isManagedWorktreeStorage(".sonderr/plans/x.md"))
        assertTrue(isManagedWorktreeStorage(".sonderr/worktrees"))
        assertTrue(isManagedWorktreeStorage(".sonderr/worktrees/foo/backend/src/Main.java"))
    }
