...
import ai.sonderr.client.agentManager.worktree.WorktreeNames
...
class WorktreeControllerTest : BasePlatformTestCase() {
    ...
    fun `test service open routes the directory to the backend rpc`() {
        ...
        coroutines.scope.launch { result = service.open("/repo/.sonderr/worktrees/feature-x") }
        flush()

        assertEquals(true, result)
        assertEquals(listOf("/repo/.sonderr/worktrees/feature-x"), rpc.opens.toList())
    }
    ...
    fun `test remove waits for process release and ignores duplicate removal`() {
        ...
        assertEquals(SonderrBundle.message("common.deleting"), controller.progress(item.id))
        assertEquals(1, controller.model.size)
    ...
    fun `test refused nested remove keeps the row and surfaces the error`() {
        ...
        assertEquals("Delete nested worktrees first:\n/repo/.sonderr/worktrees/feature-x/.sonderr/worktrees/nested", failures.single().error)
    }

    fun `test force remove passes the force flag and drops the row on success`() {
        val item = WorktreeDto("/repo/.sonderr/worktrees/feature-x", "feature-x", "feature/x", "/repo/.sonderr/worktrees/feature-x", locked = true)
        rpc.listed += item
        val controller = controller()
        controller.reload()
        flush()
    ...
    fun `test move adds placeholder tracks progress and swaps to worktree`() {
        ...
        assertEquals(SonderrBundle.message("worktree.progress.capturing"), controller.progress(temp.id))
    ...
    fun `test create stashes the prompt with its picked selection for the created worktree`() {
        ...
        ApplicationManager.getApplication().invokeAndWait {
            controller.create(
                "feature/y",
                null,
                prompt = PendingPrompt("fix the bug", agent = "plan", provider = "sonderr", model = "gpt-5", variant = "high"),
            )
        }
        flush()
        ...
        ApplicationManager.getApplication().invokeAndWait {
            ...
            assertEquals(PendingPrompt("fix the bug", "plan", "sonderr", "gpt-5", "high"), stashed)
    ...
    fun `test applyName ignores unknown paths, identical names, and blanks`() {
        ...
        ApplicationManager.getApplication().invokeAndWait {
            controller.applyName("/repo/.sonderr/worktrees/other", "Ignored")
