...
class SessionRetryTest : SessionControllerTestBase() {
    ...
    private fun failed(error: MessageErrorDto? = MessageErrorDto(type = "APIError", message = "provider overloaded")) {
        ...
        rpc.history.add(
            MessageWithPartsDto(
                msg("msg_user", "ses_test", "user").copy(providerID = "sonderr", modelID = "gpt-5", agent = "code"),
                emptyList(),
            ),
        )
        rpc.history.add(
        ...
        projectRpc.state.value = workspaceReady(providers = providers(), connected = listOf("sonderr", "anthropic"))
    }
    ...
    private fun unanswered() {
        ...
        projectRpc.state.value = workspaceReady(
            ...
            connected = listOf("sonderr", "anthropic", "snowflake"),
        )
    }
    ...
    fun `test retry continues the failed turn without touching the transcript`() {
        ...
        assertEquals("sonderr", prompt.providerID)
    ...
    /** The auto-routing model id contains a slash ("sonderr-auto/free"), so only the provider may split off. */
    fun `test retry uses an auto routing model whose id contains a slash`() {
        ...
        projectRpc.state.value = workspaceReady(
            providers = listOf(
                ProviderDto(
                    id = "sonderr",
                    name = "Sonderr",
                    models = mapOf("sonderr-auto/free" to ModelDto(id = "sonderr-auto/free", name = "Auto Free")),
                ),
                ProviderDto(
                    id = "snowflake",
                    name = "Snowflake",
            ...
            connected = listOf("sonderr", "snowflake"),
        )
        val m = controller("ses_test")
        flush()

        edt { m.selectModel("sonderr", "sonderr-auto/free") }
        flush()
        edt { m.retry() }
        flush()
        ...
        val prompt = rpc.prompts.single().third
        assertEquals("sonderr", prompt.providerID)
        assertEquals("sonderr-auto/free", prompt.modelID)
    }
    ...
    fun `test retry is unavailable when the last turn completed`() {
        ...
        val m = controller("ses_test")
        flush()
        emit(ChatEventDto.Error(null, MessageErrorDto(type = "UnknownError", message = "invalid sonderr.json")))
