...
import ai.sonderr.client.session.views.PlanExitView
import ai.sonderr.client.session.views.SessionOutcomeView
import ai.sonderr.client.session.views.base.DialogView
import ai.sonderr.client.session.views.base.PartHeader
import ai.sonderr.client.session.views.permission.PermissionView
import ai.sonderr.client.session.views.question.QuestionResultView
import ai.sonderr.client.session.views.question.QuestionView
import ai.sonderr.client.session.ui.selection.SessionCopyTarget
import ai.sonderr.client.session.views.MessageErrorView
import ai.sonderr.client.session.views.MessageToolbar
import ai.sonderr.client.session.views.MessageView
import ai.sonderr.client.session.views.PromptAttachmentView
import ai.sonderr.client.session.views.TextView
import ai.sonderr.client.session.views.TurnView
import ai.sonderr.client.session.views.base.PartView
import ai.sonderr.client.session.views.tool.TaskToolView
import ai.sonderr.client.session.views.tool.ToolView
import ai.sonderr.client.session.views.todo.TodoWriteView
import ai.sonderr.client.ui.DiffStatBadge
...
import ai.sonderr.rpc.dto.MessageErrorDto
...
import ai.sonderr.rpc.dto.TodoDto
...
class SessionMessageListPanelTest : BasePlatformTestCase() {
    ...
    fun `test footer offers retry without repeating the message the card shows`() {
        ...
        assertNotNull(button(ov, SonderrBundle.message("session.outcome.retry")))
    }
    ...
    fun `test generic failed close drops its description when the card explains the turn`() {
        ...
        assertNull(text(ov, SonderrBundle.message("session.outcome.failed.description")))
        assertNotNull(button(ov, SonderrBundle.message("session.outcome.retry")))
    }
    ...
    fun `test incomplete outcome shows footer without message failure card`() {
        ...
        assertNotNull(text(ov, SonderrBundle.message("session.outcome.incomplete.title")))
        assertNotNull(text(ov, SonderrBundle.message("session.outcome.incomplete.description")))
        assertNull(button(ov, SonderrBundle.message("session.outcome.retry")))
    }
    ...
    fun `test queued turn shows badge and remove action`() {
        ...
        assertFalse(components(u1).filterIsInstance<JBLabel>().any { it.text == SonderrBundle.message("session.queued") })
        assertTrue(components(u2).filterIsInstance<JBLabel>().any { it.text == SonderrBundle.message("session.queued") })

        val remove = components(u2).filterIsInstance<HoverIcon>().single()
        assertEquals(SonderrBundle.message("session.queued.remove"), remove.toolTipText)
    ...
    fun `test user prompt toolbar omits rollback when revert handler is absent`() {
        ...
        assertFalse(components(toolbar).filterIsInstance<JButton>().any { it.toolTipText == SonderrBundle.message("revert.message.rollback") })
    }
    ...
    fun `test user prompt toolbar shows rollback when revert handler is present`() {
        ...
        val rollback = components(toolbar)
            ...
            .first { it.toolTipText == SonderrBundle.message("revert.message.rollback") }
        rollback.doClick()
    ...
    fun `test turn ended state makes outcome view visible`() {
        ...
        assertTrue(ov.isVisible)
        assertNotNull(text(item, SonderrBundle.message("session.outcome.interrupted.note")))
    ...
    fun `test rollback banner uses session dialog card with standard actions`() {
        ...
        assertEquals(
            listOf(SonderrBundle.message("revert.banner.redo"), SonderrBundle.message("revert.banner.redo.all")),
            buttons.map { it.text },
        )
        assertEquals(listOf(SonderrBundle.message("revert.banner.redo")), buttons.filter { it.isVisible }.map { it.text })
        ...
        val hint = components(banner)
            ...
            .first { it.text == SonderrBundle.message("revert.banner.hint") }
    ...
    fun `test rollback banner opens rolled back diff`() {
        ...
        val button = components(banner).filterIsInstance<HoverIcon>()
            .first { it.toolTipText == SonderrBundle.message("session.part.tool.openDiff") }
        assertTrue(button.isVisible)
        assertTrue(button.isEnabled)
        button.doClick()

        assertEquals(listOf(diff), opened.single())
        assertEquals(SonderrBundle.message("revert.banner.openDiff.title"), titles.single())
    ...
    fun `test rollback banner shows redo all only for multiple reverted messages`() {
        ...
        assertTrue(components(banner).filterIsInstance<JButton>().first { it.text == SonderrBundle.message("revert.banner.redo.all") }.isVisible)

        model.setRevert(SessionRevertDto("u2"))
        banner.update()

        assertFalse(components(banner).filterIsInstance<JButton>().first { it.text == SonderrBundle.message("revert.banner.redo.all") }.isVisible)
    }
    ...
    fun `test rollback banner buttons invoke actions`() {
        ...
        components(banner).filterIsInstance<JButton>().first { it.text == SonderrBundle.message("revert.banner.redo") }.doClick()
        components(banner).filterIsInstance<JButton>().first { it.text == SonderrBundle.message("revert.banner.redo.all") }.doClick()
    ...
    fun `test rollback state disables banner actions and shows inline progress`() {
        ...
        val buttons = components(banner).filterIsInstance<JButton>().filter { it.text.isNotEmpty() }
        assertTrue(buttons.filter { it.text == SonderrBundle.message("revert.banner.redo") }.all { !it.isEnabled })
        assertTrue(buttons.filter { it.text == SonderrBundle.message("revert.banner.redo.all") }.all { !it.isEnabled })
    ...
    fun `test rollback banner explains snapshotless history only revert`() {
        ...
        val notice = components(banner).filterIsInstance<JBLabel>()
            .first { it.text == SonderrBundle.message("revert.banner.filesNotRestored") }
    ...
    fun `test completed plan update replaces tool view and keeps open file action`() {
        ...
        model.updateContent(
            ...
            toolPart(
                ...
                metadata = mapOf("plan" to ".sonderr/plans/x.md"),
            ),
        )

        val view = mv.part("tp1") as PlanExitView
        view.simulateLink(".sonderr/plans/x.md")

        assertEquals(listOf(".sonderr/plans/x.md"), opened)
    }
    ...
    private fun disableValidateRoots() {
        val key = "sonderr.session.validateRoots"
