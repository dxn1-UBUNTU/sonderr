...
class SonderrAgentBehaviorServiceTest : BasePlatformTestCase() {
    private lateinit var scope: CoroutineScope
    private lateinit var rpc: FakeAgentBehaviorRpcApi
    private lateinit var service: SonderrAgentBehaviorService

    override fun setUp() {
        super.setUp()
        scope = CoroutineScope(SupervisorJob())
        rpc = FakeAgentBehaviorRpcApi()
        service = SonderrAgentBehaviorService(scope, rpc)
    }
    ...
    fun `test refreshCommandFiles returns previous rows on rpc failure`() = runBlocking {
        val fallback = listOf(CommandFileDto("plan", location = "/test/.sonderr/workflows/plan.md"))
        rpc.commandFilesError = RuntimeException("boom")
    ...
    fun `test saveCommands forwards all edits`() = runBlocking {
        rpc.commandFiles = listOf(CommandFileDto("plan", location = "/test/.sonderr/workflows/plan.md"))

        val ok = withContext(Dispatchers.Default) {
            service.saveCommands("/test", mapOf("/test/.sonderr/workflows/plan.md" to "# Saved"))
        }

        assertTrue(ok)
        assertEquals(listOf(Triple("/test", "/test/.sonderr/workflows/plan.md", "# Saved")), rpc.commandSaves)
