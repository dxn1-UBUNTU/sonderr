...
import ai.sonderr.client.agentManager.worktree.SonderrRunService
import ai.sonderr.client.agentManager.worktree.SonderrWorktreeService
import ai.sonderr.client.agentManager.worktree.GhStatusCoordinator
import ai.sonderr.client.agentManager.worktree.NewWorktreeHandle
import ai.sonderr.client.agentManager.worktree.NewWorktreePlan
import ai.sonderr.client.agentManager.worktree.PendingPrompt
import ai.sonderr.client.agentManager.worktree.PendingWorktreePrompt
import ai.sonderr.client.agentManager.worktree.WorktreeController
import ai.sonderr.client.agentManager.worktree.WorktreeEditorMatcher
import ai.sonderr.client.agentManager.worktree.WorktreeEditorMatchers
import ai.sonderr.client.agentManager.worktree.WorktreeIcons
...
import ai.sonderr.client.agentManager.worktree.WorktreeStatusService
...
import ai.sonderr.client.testing.FakeWorktreeRpcApi
...
import ai.sonderr.client.testing.TestUiTimers
import ai.sonderr.client.testing.fire
import ai.sonderr.client.testing.installBrowser
import ai.sonderr.client.ui.list.ActiveListBadge
import ai.sonderr.client.ui.list.ActiveListItem
import ai.sonderr.client.ui.list.ActiveListMetrics
import ai.sonderr.client.ui.list.ActiveListView
import ai.sonderr.client.ui.list.ACTIVE_LIST_PR_CELL
import ai.sonderr.client.ui.list.activeListCellBounds
import ai.sonderr.client.ui.list.activeListToolWindowBackground
...
class AgentManagerPanelTest : BasePlatformTestCase() {
    ...
    private lateinit var service: SonderrWorktreeService
    ...
    override fun setUp() {
        ...
        service = SonderrWorktreeService(coroutines.scope, rpc)
        ApplicationManager.getApplication().replaceService(SonderrWorktreeService::class.java, service, testRootDisposable)
        // remove() releases any worktree run processes through this service before deleting.
        ApplicationManager.getApplication()
            .replaceService(SonderrRunService::class.java, SonderrRunService(coroutines.scope, FakeRunRpcApi()), testRootDisposable)
    ...
    fun `test creating a worktree selects it while pending and after the rpc resolves`() {
        rpc.listed += WorktreeDto("/repo/.sonderr/worktrees/feature-x", "feature-x", "feature/x", "/repo/.sonderr/worktrees/feature-x")
    ...
    fun `test creating a worktree opens the created worktree session editor`() {
        ...
        val created = edt { controller.model.getElementAt(0) }
        assertEquals(created.id, edt { (list.selectedValue as ActiveListItem).key })
        val file = edt { FileEditorManager.getInstance(project).openFiles.single() as SonderrVirtualFile }
        ...
        assertEquals(false, file.getUserData(SonderrVfsManager.FOCUS))
    }
    ...
    fun `test clicking a worktree opens the worktree session editor`() {
        val item = WorktreeDto("/repo/.sonderr/worktrees/feature-x", "feature-x", "feature/x", "${project.basePath!!}/.sonderr/worktrees/feature-x")
        rpc.listed += item
        val controller = WorktreeController(service, "/test", coroutines.scope)
        val panel = edt { AgentManagerPanel(testRootDisposable, controller, project) }
        edt { controller.reload() }
    ...
    fun `test refresh preserves selected worktree across model replace`() {
        val first = WorktreeDto("/repo/.sonderr/worktrees/feature-x", "feature-x", "feature/x", "/repo/.sonderr/worktrees/feature-x")
        val second = WorktreeDto("/repo/.sonderr/worktrees/feature-y", "feature-y", "feature/y", "/repo/.sonderr/worktrees/feature-y")
        rpc.listed += first
        rpc.listed += second
        val controller = WorktreeController(service, "/test", coroutines.scope)
        val panel = edt { AgentManagerPanel(testRootDisposable, controller) }
    ...
    fun `test refresh keeps existing selection`() {
        ...
        edt {
            ...
            project.service<SonderrVfsManager>().open(WorktreeSessionEditorKind.ID, worktreeSessionParams(second), focus = true)
    ...
    fun `test selecting non worktree editor tab clears worktree row selection`() {
        ...
        edt {
            ensureWorktreeSessionEditorKind()
            project.service<SonderrVfsManager>().open(WorktreeSessionEditorKind.ID, worktreeSessionParams(item), focus = true)
        }
        pump()
    ...
    fun `test deleting a worktree closes and releases its worktree session editor`() {
        ...
        val path = SonderrPath(WorktreeSessionEditorKind.ID, params)
        edt {
            ensureWorktreeSessionEditorKind()
            project.service<SonderrVfsManager>().open(WorktreeSessionEditorKind.ID, params)
        }
        assertNotNull(SonderrVirtualFileSystem.getInstance().cached(path))
        ...
        assertNull(SonderrVirtualFileSystem.getInstance().cached(path))
    }

    fun `test deleting the shown worktree selects and opens the next row`() {
        val a = WorktreeDto("/repo/.sonderr/worktrees/a", "a", "a", "/repo/.sonderr/worktrees/a")
        val b = WorktreeDto("/repo/.sonderr/worktrees/b", "b", "b", "/repo/.sonderr/worktrees/b")
        val c = WorktreeDto("/repo/.sonderr/worktrees/c", "c", "c", "/repo/.sonderr/worktrees/c")
        rpc.listed += a
        rpc.listed += b
        rpc.listed += c
        val controller = WorktreeController(service, "/test", coroutines.scope)
        ...
        edt {
            ensureWorktreeSessionEditorKind()
            project.service<SonderrVfsManager>().open(WorktreeSessionEditorKind.ID, worktreeSessionParams(b), focus = true)
        }
        pump()
    ...
    fun `test deleting a background worktree keeps the shown selection`() {
        ...
        edt {
            ensureWorktreeSessionEditorKind()
            project.service<SonderrVfsManager>().open(WorktreeSessionEditorKind.ID, worktreeSessionParams(a), focus = true)
        }
        pump()
        assertEquals(a.id, edt { (list.selectedValue as ActiveListItem).key })
    ...
    fun `test locked worktree row shows the lock icon`() {
        val path = "${project.basePath!!}/.sonderr/worktrees/held"
    ...
    fun `test worktree row shows metrics from status service`() {
        val item = WorktreeDto("${project.basePath!!}/.sonderr/worktrees/feature-x", "feature-x", "feature/x", "${project.basePath!!}/.sonderr/worktrees/feature-x")
        rpc.listed += item
    ...
    fun `test open diff opens the branch diff editor`() {
        ...
        val file = edt { FileEditorManager.getInstance(project).openFiles.single() as SonderrVirtualFile }
        assertEquals(SonderrDiffEditorKind.ID, file.path.kind)
    ...
    fun `test pr title replaces row name and tooltip reveals custom name`() {
        val path = "${project.basePath!!}/.sonderr/worktrees/feature-x"
    ...
    fun `test worktree row hides badge while in progress`() {
        ...
        assertEquals(SonderrBundle.message("worktree.progress.creating"), pending.progress)
    ...
    private fun worktree(name: String): WorktreeDto {
        val path = "${project.basePath!!}/.sonderr/worktrees/$name"
