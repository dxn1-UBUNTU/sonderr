...
class ModelPickerTest : BasePlatformTestCase() {
    ...
    fun `test section order starts with favorites and recommended`() {
        val rows = modelPickerRows(listOf(
            ...
            item("auto", "Sonderr Auto", "sonderr", "Sonderr", 0.0),
        ...
        assertEquals("sonderr/auto", rows[1].key)
    ...
    fun `test sonderr provider group is first and other providers keep source order`() {
        val rows = modelPickerRows(listOf(
            item("gpt", "GPT", "openai", "OpenAI"),
            item("auto", "Auto", "sonderr", "Sonderr"),
            item("claude", "Claude", "anthropic", "Anthropic"),
        ), emptyList(), "")

        assertEquals(listOf("Sonderr", "OpenAI", "Anthropic"), rows.indices.mapNotNull { modelPickerSectionTitle(rows, it) })
    }
    ...
    fun `test small rows are included only when requested`() {
        val items = listOf(
            item("auto-small", "Small", "sonderr", "Sonderr"),
            item("auto", "Auto", "sonderr", "Sonderr"),
        )
        ...
        assertEquals(listOf("sonderr/auto"), default.mapNotNull { it.key })
        assertEquals(listOf("sonderr/auto", "sonderr/auto-small"), small.mapNotNull { it.key }.sorted())
    }
    ...
    fun `test non-sonderr selected model uses provider prefix`() {
        val picker = ModelPicker()
    ...
    fun `test non-sonderr selected model strips duplicate provider prefix`() {
        val picker = ModelPicker()
    ...
    fun `test non-sonderr selected model strips vscode provider prefix`() {
        val picker = ModelPicker()
    ...
    fun `test sonderr selected model remains unprefixed`() {
        val picker = ModelPicker()

        picker.setItems(listOf(item("auto", "Sonderr Auto", "sonderr", "Sonderr")))
    ...
    fun `test selected paid model with training flag indicates data collection`() {
        ...
        picker.setItems(listOf(item("paid", "Paid", "sonderr", "Sonderr", training = true)))
    ...
    fun `test selected free model without training flag does not indicate data collection`() {
        ...
        picker.setItems(listOf(item("free", "Free", "sonderr", "Sonderr", free = true)))
    ...
    fun `test display parts sanitize free suffix`() {
        val parts = ModelText.parts(item("auto", "Auto Free (free)", "sonderr", "Sonderr"))
    ...
    fun `test renderer shows empty favorite star only for selected row`() {
        val row = ModelPickerRow(item("auto", "Auto", "sonderr", "Sonderr"), "Sonderr", false)
    ...
    fun `test renderer keeps favorite star visible`() {
        val row = ModelPickerRow(item("auto", "Auto", "sonderr", "Sonderr"), "Sonderr", false)
        val model = CollectionListModel(listOf(row))
        val renderer = ModelPickerRenderer(model, { null }, { setOf("sonderr/auto") })
        val list = JBList(model)
    ...
    fun `test renderer shows free badge for free model`() {
        val row = ModelPickerRow(ModelPicker.Item("auto", "Auto", "sonderr", "Sonderr", free = true), "Sonderr", false)
        val model = CollectionListModel(listOf(row))
        val renderer = ModelPickerRenderer(model, { null }, { emptySet() })
        val list = JBList(model)
    ...
    fun `test renderer shows data collection warning for paid training model`() {
        val row = ModelPickerRow(item("paid", "Paid", "sonderr", "Sonderr", training = true), "Sonderr", false)
        val model = CollectionListModel(listOf(row))
        val renderer = ModelPickerRenderer(model, { null }, { emptySet() })
        val list = JBList(model)
    ...
    fun `test renderer shows BYOK instead of free when both are available`() {
        val row = ModelPickerRow(
            ModelPicker.Item("claude", "Claude", "sonderr", "Sonderr", free = true, byok = true),
            "Sonderr",
            false,
        )
        val model = CollectionListModel(listOf(row))
