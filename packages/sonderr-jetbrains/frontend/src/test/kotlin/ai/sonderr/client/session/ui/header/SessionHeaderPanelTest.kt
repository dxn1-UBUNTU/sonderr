...
import ai.sonderr.client.session.controller.SessionControllerTestBase
...
class SessionHeaderPanelTest : SessionControllerTestBase() {
    ...
    fun `test starts hidden for empty header`() {
        appRpc.state.value = ai.sonderr.rpc.dto.SonderrAppStateDto(ai.sonderr.rpc.dto.SonderrAppStatusDto.READY)
    ...
    fun `test timeline colors honor semantic named color keys`() {
        val old = UIManager.getColor("Sonderr.Session.Timeline.Read")
        val color = Color(12, 34, 56)

        try {
            UIManager.put("Sonderr.Session.Timeline.Read", color)

            assertEquals(color.rgb, SessionUiStyle.Timeline.READ.rgb)
        } finally {
            UIManager.put("Sonderr.Session.Timeline.Read", old)
        }
    }
    ...
    private fun promptedHeader(): ai.sonderr.client.session.controller.SessionController {
        appRpc.state.value = ai.sonderr.rpc.dto.SonderrAppStateDto(
            ai.sonderr.rpc.dto.SonderrAppStatusDto.READY,
            config = ai.sonderr.rpc.dto.ConfigDto(model = "sonderr/gpt-5"),
        )
        projectRpc.state.value = workspaceReady(
            providers = listOf(
                ProviderDto(
                    ...
                    models = mapOf(
                        "gpt-5" to ModelDto(
                            id = "gpt-5",
                            name = "GPT-5",
                            limit = ai.sonderr.rpc.dto.ModelLimitDto(context = 2_000_000, output = 200_000),
                        ),
                    ),
                ),
            ),
