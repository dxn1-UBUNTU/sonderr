...
class AutoApproveSettingsUiTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        val state = SonderrAppStateDto(SonderrAppStatusDto.READY, config = ConfigDto())
    ...
    fun `test page is not editable before app is ready`() {
        rpc.state.value = SonderrAppStateDto(SonderrAppStatusDto.LOADING)
        app._state.value = SonderrAppStateDto(SonderrAppStatusDto.LOADING)
        edt { ui = AutoApproveSettingsUi(uiScope, app, workspaces, picker) }
        flushUntil { text(requireUi()).contains("External Directory") }
