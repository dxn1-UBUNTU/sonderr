...
class SonderrRecoveryActionsTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        ApplicationManager.getApplication().replaceService(
            SonderrAppService::class.java,
            SonderrAppService(scope, appRpc),
            testRootDisposable,
        )
        ApplicationManager.getApplication().replaceService(
            SonderrWorkspaceService::class.java,
            SonderrWorkspaceService(scope, rpc),
            testRootDisposable,
        )
    }
    ...
    fun `test restart action stays enabled for all app states`() {
        val action = RestartSonderrAction()
    ...
    fun `test reinstall action stays enabled for all app states`() {
        val action = ReinstallSonderrAction()
        val event = event(action)
    ...
    fun `test restart action adds core suffix in connection retry popup`() {
        val action = RestartSonderrAction()
        val event = event(action, place = SonderrActionPlaces.connectionRetryPopup())
    ...
    fun `test core group has visible menu text and info action`() {
        val xml = requireNotNull(javaClass.classLoader.getResourceAsStream("sonderr.jetbrains.frontend.xml"))
            .bufferedReader()
            .use { it.readText() }

        assertTrue(xml.contains("<group id=\"Sonderr.CliGroup\" text=\"Core\" popup=\"true\">"))
        assertTrue(xml.contains("<reference ref=\"Sonderr.Restart\"/>"))
        assertTrue(xml.contains("<reference ref=\"Sonderr.Reinstall\"/>"))
        assertTrue(xml.contains("<reference ref=\"Sonderr.CoreInfo\"/>"))
        assertTrue(xml.contains("<group id=\"Sonderr.OpenConfigGroup\" text=\"Config Files\" popup=\"true\">"))
        assertTrue(xml.contains("<reference ref=\"Sonderr.OpenConfigGroup\"/>"))
        assertFalse(xml.contains("<action id=\"Sonderr.ShowProfile\""))
        assertFalse(xml.contains("<reference ref=\"Sonderr.ShowProfile\"/>"))
    }
    ...
    fun `test local config action says open when target exists`() {
        rpc.localConfigPath = "/test/.sonderr/sonderr.jsonc"
        rpc.localConfigDisplayPath = "~/.sonderr/sonderr.jsonc"
        rpc.localConfigExists = true
        service().localConfig["/test"] = ConfigTargetDto("/test/.sonderr/sonderr.jsonc", "~/.sonderr/sonderr.jsonc", true)
        ...
        assertEquals("Open: local ~/.sonderr/sonderr.jsonc", event.presentation.text)
    ...
    fun `test local config action says create when target is missing`() {
        ...
        service().localConfig["/test"] = ConfigTargetDto("/test/.sonderr/sonderr.jsonc", "~/.sonderr/sonderr.jsonc", false)
        val action = OpenLocalConfigAction()
        val event = event(action, workspace = workspace("/test"))
        ...
        assertTrue(event.presentation.isEnabled)
        assertEquals("Create: local ~/.sonderr/sonderr.jsonc", event.presentation.text)
        assertEquals(0, rpc.localConfigPathCalls)
    }

    fun `test local config action refreshes missing target in background`() {
        rpc.localConfigPath = "/test/.sonderr/sonderr.jsonc"
        rpc.localConfigDisplayPath = "/test/.sonderr/sonderr.jsonc"
        rpc.localConfigExists = true
        ...
        gate.complete(Unit)
        service().localConfig["/test"] = ConfigTargetDto("/test/.sonderr/sonderr.jsonc", "/test/.sonderr/sonderr.jsonc", true)

        val next = event(action, workspace = workspace("/test"))
        update(action, next)

        assertEquals("Open: local /test/.sonderr/sonderr.jsonc", next.presentation.text)
    }
    ...
    fun `test global config action says open when target exists`() {
        rpc.globalConfigPath = "/config/sonderr.jsonc"
        rpc.globalConfigDisplayPath = "~/.config/sonderr/sonderr.jsonc"
        rpc.globalConfigExists = true
        cacheGlobal(ConfigTargetDto("/config/sonderr.jsonc", "~/.config/sonderr/sonderr.jsonc", true))
        val action = OpenGlobalConfigAction()
        val event = event(action)

        update(action, event)

        assertEquals("Open: global ~/.config/sonderr/sonderr.jsonc", event.presentation.text)
    ...
    fun `test global config action says create when target is missing`() {
        ...
        cacheGlobal(ConfigTargetDto("/config/sonderr.jsonc", "~/.config/sonderr/sonderr.jsonc", false))
        val action = OpenGlobalConfigAction()
        val event = event(action)

        update(action, event)

        assertEquals("Create: global ~/.config/sonderr/sonderr.jsonc", event.presentation.text)
        assertEquals(0, rpc.globalConfigPathCalls)
    }

    fun `test global config action refreshes missing target in background`() {
        rpc.globalConfigPath = "/config/sonderr.jsonc"
        rpc.globalConfigDisplayPath = "/config/sonderr.jsonc"
        rpc.globalConfigExists = true
        val call = CompletableDeferred<Unit>()
        val gate = CompletableDeferred<Unit>()
        ...
        gate.complete(Unit)
        cacheGlobal(ConfigTargetDto("/config/sonderr.jsonc", "/config/sonderr.jsonc", true))

        val next = event(action)
        update(action, next)

        assertEquals("Open: global /config/sonderr.jsonc", next.presentation.text)
    }
    ...
    fun `test settings popup group updates recursively in background`() {
        ...
        val wrapped = SonderrSettingsAction.popupGroup(group)

        assertEquals(ActionUpdateThread.BGT, wrapped.actionUpdateThread)
    }

    fun `test settings action prewarms config targets`() {
        val action = SonderrSettingsAction()

        runBlocking {
            SonderrSettingsAction.refreshConfigTargets(event(action, workspace = workspace("/test")), service()).forEach { it.join() }
        }
    ...
    private fun service(): SonderrWorkspaceService = ApplicationManager.getApplication().getService(SonderrWorkspaceService::class.java)

    private fun app(): SonderrAppService = ApplicationManager.getApplication().getService(SonderrAppService::class.java)

    private fun cacheGlobal(target: ConfigTargetDto) {
        val field = SonderrWorkspaceService::class.java.getDeclaredField("globalConfig")
    ...
    private fun workspace(dir: String): Workspace {
        return Workspace(
            dir,
            MutableStateFlow(SonderrWorkspaceStateDto(SonderrWorkspaceStatusDto.READY)),
            reload = {},
            refreshConfigFiles = {},
        )
    }
