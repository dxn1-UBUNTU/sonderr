...
class MdViewHybridTest : BasePlatformTestCase() {
    ...
    fun `test prose inline code is styled without code chrome or editor`() {
        ...
        view.set("Use `packages/cli/src/session/prompt.ts` here")
        val pane = htmls().single()
        val color = MdCommon.hex(SessionUiStyle.View.Markdown.string())

        assertTrue(view.html().contains("<code style=\"color: $color\"><a class=\"sonderr-file-ref\" href=\"packages/cli/src/session/prompt.ts\">packages/cli/src/session/prompt.ts</a></code>"))
    ...
    fun `test prose file refs are styled as file links`() {
        view.set("See packages/cli/src/session/prompt.ts before continuing")
        val html = view.html()
        ...
        assertTrue(view.overrideSheet().contains("a.sonderr-file-ref, code a.sonderr-file-ref"))
    ...
    fun `test file ref links include line suffix and exclude punctuation`() {
        view.set("See sonderr/session/prompt.ts:302, native-plan-prompt.txt:37-38.")
        val html = view.html()
    ...
    fun `test fenced code file refs are not linkified`() {
        view.set("```text\npackages/cli/src/session/prompt.ts\n```\n\nSee packages/cli/src/session/prompt.ts")

        assertEquals("packages/cli/src/session/prompt.ts", editors().single().text)
        assertFalse(view.html().substringBefore("</pre>").contains("sonderr-file-ref"))
        assertTrue(htmls().single().text.contains("sonderr-file-ref"))
    }
    ...
    fun `test shell output renders plain text with semantic highlighters`() {
        val output = """
            ...
            e8b9785 Add second change
             packages/sonderr-jetbrains/frontend/src/main/kotlin/App.kt | 14 ++++++++++----
