...
class SonderrPromptCompletionProviderTest : BasePlatformTestCase() {
    private lateinit var scope: CoroutineScope
    private lateinit var rpc: FakeWorkspaceRpcApi
    private lateinit var provider: SonderrPromptCompletionProvider

    override fun setUp() {
        super.setUp()
        scope = CoroutineScope(SupervisorJob() + Dispatchers.Default)
        rpc = FakeWorkspaceRpcApi()
        val workspaces = SonderrWorkspaceService(scope, rpc)
        provider = SonderrPromptCompletionProvider(
    ...
    fun `test accepting mention mid token replaces glued suffix`() {
        ...
        assertEquals(
            SonderrPromptCompletionProvider.MentionHit(0, 22, "backend/deploy-dev.sh", true),
    ...
    fun `test slash completion ranks prefix matches before contains matches`() {
        rpc.state.value = SonderrWorkspaceStateDto(
            SonderrWorkspaceStatusDto.READY,
            commands = listOf(
    ...
    fun `test highlights known slash command at start`() {
        assertEquals(
            listOf(SonderrPromptCompletionProvider.Highlight(0, 4, SonderrPromptCompletionProvider.HighlightKind.COMMAND)),
    ...
    fun `test highlights client slash command aliases at start`() {
        assertEquals(
            listOf(SonderrPromptCompletionProvider.Highlight(0, 6, SonderrPromptCompletionProvider.HighlightKind.COMMAND)),
    ...
    fun `test highlights server slash command at start`() {
        ...
        assertEquals(
            listOf(SonderrPromptCompletionProvider.Highlight(0, 7, SonderrPromptCompletionProvider.HighlightKind.COMMAND)),
    ...
    fun `test highlights special mentions without tracked paths`() {
        assertEquals(
            listOf(
                SonderrPromptCompletionProvider.Highlight(4, 16, SonderrPromptCompletionProvider.HighlightKind.MENTION),
            ),
    ...
    fun `test serverCommand does not route client aliases`() {
        rpc.state.value = SonderrWorkspaceStateDto(SonderrWorkspaceStatusDto.READY, commands = listOf(CommandDto("clear")))
    ...
    fun `test highlights tracked mentions longest first`() {
        ...
        assertEquals(
            listOf(SonderrPromptCompletionProvider.Highlight(4, 14, SonderrPromptCompletionProvider.HighlightKind.MENTION)),
    ...
    fun `test highlights unresolved mention after validation`() {
        ...
        assertEquals(
            listOf(SonderrPromptCompletionProvider.Highlight(4, 16, SonderrPromptCompletionProvider.HighlightKind.INVALID)),
    ...
    fun `test highlights existing hand typed mention after validation`() {
        ...
        assertEquals(
            listOf(SonderrPromptCompletionProvider.Highlight(4, 13, SonderrPromptCompletionProvider.HighlightKind.MENTION)),
    ...
    fun `test mentionAt resolves tracked file mention`() {
        ...
        assertEquals(
            SonderrPromptCompletionProvider.MentionHit(4, 13, "src/a.ts", true),
    ...
    fun `test mentionAt marks unresolved file mention after validation`() {
        ...
        assertEquals(
            SonderrPromptCompletionProvider.MentionHit(4, 15, "missing.ts", false),
    ...
    private fun noMatches() = SonderrBundle.message("prompt.completion.noMatches")
}
