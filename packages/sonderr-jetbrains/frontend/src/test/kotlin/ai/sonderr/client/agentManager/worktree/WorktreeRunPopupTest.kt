...
class WorktreeRunPopupTest : BasePlatformTestCase() {
    fun testEmptyConfigsShowDisabledRowAndFrameAction() {
        ...
        assertEquals(SonderrBundle.message("worktree.run.empty"), rows[0].templateText)
        assertFalse(enabled(rows[0]))
        assertTrue(rows[1] is Separator)
        assertEquals(SonderrBundle.message("worktree.run.open.frame"), rows[2].templateText)
    }
    ...
    fun testRunningSectionRowsAndCallbacks() {
        ...
        assertEquals(SonderrBundle.message("worktree.run.section.running"), (rows[0] as Separator).text)
        assertEquals(SonderrBundle.message("worktree.run.stop", "dev [wt]"), rows[1].templateText)
        assertTrue(enabled(rows[1]))
        assertEquals(SonderrBundle.message("worktree.run.output", "dev [wt]"), rows[2].templateText)
        assertEquals(SonderrBundle.message("worktree.run.section.start"), (rows[3] as Separator).text)
        assertEquals("dev", rows[4].templateText)
        assertEquals("worker", rows[5].templateText)
        assertTrue(rows[6] is Separator)
        assertEquals(SonderrBundle.message("worktree.run.open.frame"), rows[7].templateText)
    ...
    fun testStoppingDisablesStopRowForUnkillableProcess() {
        ...
        val rows = group.getChildren(null)
        assertEquals(SonderrBundle.message("worktree.run.kill", "dev [wt]"), rows[1].templateText)
    ...
    fun testBuildRowsFollowConfigsAndPrecedeOpenFrame() {
        ...
        assertEquals(
            listOf("dev", "---", "Build", "Rebuild", "---", SonderrBundle.message("worktree.run.open.frame")),
            layout(rows),
        )
    ...
    fun testBuildRowsAreAbsentWhenProjectIsNotBuildable() {
        ...
        assertEquals(
            listOf("dev", "---", SonderrBundle.message("worktree.run.open.frame")),
