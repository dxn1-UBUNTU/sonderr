...
class ModelsSettingsUiTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        val state = SonderrAppStateDto(
            ...
            config = ConfigDto(model = "sonderr/old"),
    ...
    fun `test matching app state before save callback keeps pending target`() {
        ...
        rpc.state.value = state("sonderr/new")
    ...
    fun `test matching app state after save callback preserves dirty draft`() {
        ...
        edt {
            assertSelected(panel, "sonderr/old")
            assertTrue(panel.modified())
        }
    }
    ...
    fun `test stale app state during pending save is ignored`() {
        ...
        rpc.state.value = state("sonderr/old")
    ...
    fun `test failed save after dispose shows notification`() {
        ...
        flushUntil {
            notes.any { it.groupId == "Sonderr" && it.type == NotificationType.ERROR }
        }
    ...
    fun `test logged out save keeps login banner stable`() {
        ...
        edt { ui = ModelsSettingsUi(uiScope, app, workspaces, directory = "/test") }
        val panel = requireUi()
        flushUntil { text(panel).contains("Old") && text(panel).contains("Sign in to Sonderr") }
        ...
        edt {
            select(panel, "new")
            panel.applyDraft()
            assertTrue(text(panel).contains("Sign in to Sonderr"))
    ...
    private fun state(model: String): SonderrAppStateDto = SonderrAppStateDto(
        SonderrAppStatusDto.READY,
    ...
    private fun select(panel: ModelsSettingsUi, id: String) {
        ...
        picker.onSelect(ModelPicker.Item(id, id.replaceFirstChar { it.titlecase() }, "sonderr", "Sonderr"))
    }
