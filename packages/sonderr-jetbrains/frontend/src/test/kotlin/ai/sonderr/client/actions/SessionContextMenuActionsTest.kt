...
import ai.sonderr.client.session.SessionActions
import ai.sonderr.client.session.SessionActionsKeys
import ai.sonderr.client.session.SessionManager
import ai.sonderr.client.session.SessionRef
import ai.sonderr.client.session.SessionUiTestBase
 ...
 * `Sonderr.StopSession` action reads a data key whose local provider is a sibling of the transcript, so
...
class SessionContextMenuActionsTest : SessionUiTestBase() {
    ...
    fun `test context menu group lists actions in order with separators`() {
        assertEquals(
            listOf(
                "Sonderr.Session.AutoApprove",
                "---",
                "Sonderr.Session.CompareToBase",
                "Sonderr.Session.OpenPr",
                "Sonderr.Session.CopyPrRef",
                "---",
                "\$Copy",
                "---",
                "Sonderr.Session.CopyId",
                "Sonderr.Session.Share",
                "Sonderr.Session.CopyShareLink",
                "---",
                "Sonderr.StopSession",
            ),
            menuChildren("Sonderr.Session.ContextMenu"),
        )
    }
    ...
    fun `test context menu does not reference worktree actions`() {
        val worktree = menuChildren("Sonderr.Session.ContextMenu").filter { it.startsWith("Sonderr.Chat.") }
    ...
    fun `test prompt menu group lists session actions in order`() {
        assertEquals(
            ...
            menuChildren("Sonderr.Session.PromptMenu"),
        )
    }
    ...
    fun `test both menus reference only declared or platform actions`() {
        ...
        for (group in listOf("Sonderr.Session.ContextMenu", "Sonderr.Session.PromptMenu")) {
    ...
    fun `test every declared session action has bundle text and description`() {
        val ids = (menuChildren("Sonderr.Session.ContextMenu") + menuChildren("Sonderr.Session.PromptMenu"))
            .filter { it.startsWith("Sonderr.Session.") }
            .distinct()
        ...
        val keys = ids.flatMap { listOf("action.$it.text", "action.$it.description") } +
            ...
            listOf("action.Sonderr.Session.Share.stop.text", "action.Sonderr.Session.Share.stop.description")

        val missing = keys.filter { SonderrBundle.optional(it).isNullOrBlank() }
    ...
    private fun descriptor(): Document {
        val stream = javaClass.classLoader.getResourceAsStream("sonderr.jetbrains.frontend.xml")
            ?: error("sonderr.jetbrains.frontend.xml missing from the test classpath")
        return stream.use {
