...
class AgentEditDialogTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        app = SonderrAppService(scope, FakeAppRpcApi())
    }
    ...
    fun `test loads agent into form`() {
        val agent = draft().copy(
            ...
            model = "sonderr/gpt-5",
            variant = "high",
            mode = SonderrCliParser.MODE_ALL,
            hidden = true,
            disable = true,
            temperature = 0.4,
            topP = 0.8,
        ...
        edt {
            ...
            assertEquals(SonderrCliParser.MODE_ALL, field<ComboBox<*>>(root, title("mode")).selectedItem)
            ...
            val result = d.result()
            assertEquals("sonderr/gpt-5", result.model)
    ...
    fun `test reads edits back into draft`() {
        ...
        val result = edt {
            ...
            field<ComboBox<*>>(root, title("mode")).selectedItem = SonderrCliParser.MODE_SUBAGENT
        ...
        assertEquals(SonderrCliParser.MODE_SUBAGENT, result.mode)
    ...
    fun `test native agent omits restricted editing`() {
        ...
        edt {
            ...
            assertEquals(SonderrCliParser.MODE_PRIMARY, result.mode)
    ...
    private fun draft() = AgentEditDraft(name = "code", mode = SonderrCliParser.MODE_PRIMARY, native = false)
    ...
    private fun title(field: String) = SonderrBundle.message("settings.agentBehavior.agents.edit.$field")
