...
import ai.sonderr.client.testing.FakeProviderRpcApi
import ai.sonderr.client.testing.installBrowser
import ai.sonderr.client.ui.UiStyle
import ai.sonderr.client.ui.list.ActiveListActionCell
import ai.sonderr.client.ui.list.ActiveListConfig
import ai.sonderr.client.ui.list.ActiveListItem
import ai.sonderr.client.ui.list.ActiveListRenderer
import ai.sonderr.client.ui.list.activeListCellAt
import ai.sonderr.client.ui.list.activeListCellBounds
import ai.sonderr.client.ui.list.activeListSectionTitle
import ai.sonderr.client.ui.list.activeListVisibleCells
import ai.sonderr.rpc.dto.CustomModelFetchResultDto
import ai.sonderr.rpc.dto.CustomProviderConfigDto
...
import ai.sonderr.rpc.dto.ProviderMetadataDto
...
class ProvidersSettingsUiTest : BasePlatformTestCase() {
    ...
    fun `test custom save error reports dropped provider`() {
        ...
        assertEquals(SonderrBundle.message("settings.providers.customNotUsable"), customSaveError("my-openai", result))
    }
    ...
    fun `test custom dialog cancels model fetch and ignores late result`() {
        ...
        edt {
            assertEquals(SonderrBundle.message("settings.providers.customFetchingModels"), field.text)
            assertEquals(SonderrBundle.message("settings.providers.customCancelModels"), pick.text)
            ...
            pick.doClick()
            assertEquals("", field.text)
            assertEquals(SonderrBundle.message("settings.providers.customSelectModels"), pick.text)
    ...
    fun `test custom dialog edit prefill locks id and leaves key blank`() {
        ...
        edt {
            ...
            assertEquals(SonderrBundle.message("settings.providers.customSave"), dialog.okText())
            dispose(dialog)
        }
    }
    ...
    fun `test popular rows use vscode order including sonderr`() {
        val rows = providerListRows(
            ProviderSettingsDto(
                providers = listOf(
                    provider("openrouter", "OpenRouter", priority = 5),
                    provider("sonderr", "Sonderr", priority = 0),
        ...
        assertEquals(listOf("sonderr", "anthropic", "deepseek", "openai", "google", "openrouter", "vercel"), rows.map { it.key })
    ...
    fun `test connected sonderr gateway has no provider settings actions`() {
        val rows = providerListRows(
            ProviderSettingsDto(
                providers = listOf(provider("sonderr", "Sonderr Gateway")),
                connected = listOf("sonderr"),
            ),
            "",
        )

        assertEquals(listOf("sonderr"), rows.map { it.key })
        assertEquals("Connected providers", activeListSectionTitle(rows, 0))
    ...
    fun `test provider oauth waiting countdown can be cancelled without refresh`() {
        val callback = CompletableDeferred<ai.sonderr.rpc.dto.ProviderActionResultDto>()
        val rpc = installProvider(
            ProviderSettingsDto(
                providers = listOf(provider("github-copilot", "GitHub Copilot")),
                auth = mapOf("github-copilot" to listOf(ProviderAuthMethodDto("oauth", "OAuth"))),
        ...
        callback.complete(ai.sonderr.rpc.dto.ProviderActionResultDto(providerState(provider("stale", "Stale"))))
    ...
    fun `test provider oauth auto response shows device auth panel`() {
        ...
        edt {
            ...
            assertEquals("https://auth.openai.com/device", fieldsByName(panel, "sonderr.provider.oauth.url").single().text)
            assertEquals(listOf("https://auth.openai.com/device"), browser.urls)
            val qr = components(panel).filterIsInstance<JBLabel>().single { it.name == "sonderr.provider.oauth.qr" }
    ...
    fun `test provider action failure returns error state`() = runBlocking {
        ...
        rpc.disconnectError = IllegalStateException("Sonderr backend is not ready")
        val service = SonderrProviderService(cs, rpc)

        val result = withContext(kotlinx.coroutines.Dispatchers.Default) {
            service.disconnect(ProviderDisconnectDto("/test", "openai"))
        }

        assertEquals("Sonderr backend is not ready", result.error)
    ...
    private fun installProvider(state: ProviderSettingsDto): FakeProviderRpcApi {
        ...
        ApplicationManager.getApplication().replaceService(
            SonderrProviderService::class.java,
            SonderrProviderService(cs, rpc),
            testRootDisposable,
        )
        return rpc
    }
