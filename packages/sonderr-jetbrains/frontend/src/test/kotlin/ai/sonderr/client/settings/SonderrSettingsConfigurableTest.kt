...
class SonderrSettingsConfigurableTest : BasePlatformTestCase() {

    fun `test id matches xml registration`() {
        val cfg = SonderrSettingsConfigurable()
        assertEquals("ai.sonderr.jetbrains.settings", cfg.id)
    }

    fun `test child profile id matches xml registration`() {
        // Verify the constants used in XML registrations are stable
        assertEquals("ai.sonderr.jetbrains.settings.profile", UserProfileConfigurable.ID)
    }

    fun `test child models id matches xml registration`() {
        assertEquals("ai.sonderr.jetbrains.settings.models", ModelsConfigurable.ID)
    }

    fun `test child context id matches xml registration`() {
        assertEquals("ai.sonderr.jetbrains.settings.context", ContextConfigurable.ID)
    }

    fun `test child advanced id matches xml registration`() {
        assertEquals("ai.sonderr.jetbrains.settings.advanced", AdvancedConfigurable.ID)
    }

    fun `test child provider and behavior ids match xml registration`() {
        assertEquals("ai.sonderr.jetbrains.settings.providers", ProvidersConfigurable.ID)
        assertEquals("ai.sonderr.jetbrains.settings.agentBehavior", AgentBehaviorConfigurable.ID)
    ...
    fun `test root implements SearchableConfigurable but not Parent`() {
        ...
        val interfaces = SonderrSettingsConfigurable::class.java.interfaces
        assertFalse(
            "SonderrSettingsConfigurable must not implement SearchableConfigurable.Parent",
    ...
    fun `test open invokes select with child found by id`() {
        ...
        assertEquals("ai.sonderr.jetbrains.settings.profile", UserProfileConfigurable.ID)
        assertEquals("ai.sonderr.jetbrains.settings.profile", profile.id)
    }

    fun `test isModified always false`() {
        assertFalse(SonderrSettingsConfigurable().isModified)
    }
