...
class SessionHeaderControllerTest : SessionControllerTestBase() {
    ...
    fun `test session updated refreshes model metadata`() {
        ...
        assertTrue(modelEvents.any { it is ai.sonderr.client.session.model.SessionModelEvent.SessionUpdated })
    }
    ...
    fun `test compact calls RPC with selected model when eligible`() {
        ...
        assertEquals("sonderr", call.third.providerID)
    ...
    fun `test compact is blocked without model`() {
        appRpc.state.value = ai.sonderr.rpc.dto.SonderrAppStateDto(ai.sonderr.rpc.dto.SonderrAppStatusDto.READY, config = ai.sonderr.rpc.dto.ConfigDto(model = null))
