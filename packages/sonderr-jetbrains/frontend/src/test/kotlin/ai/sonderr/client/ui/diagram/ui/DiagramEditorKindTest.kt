...
import ai.sonderr.client.vfs.SonderrFileEditorProvider
import ai.sonderr.client.vfs.SonderrPath
import ai.sonderr.client.vfs.SonderrSourceEditorProvider
...
class DiagramEditorKindTest : BasePlatformTestCase() {
    ...
    fun `test opening the same source reuses one tab and a different source opens another`() {
        ...
        val files = manager.openFiles.filterIsInstance<SonderrVirtualFile>()
        ...
        assertEquals(2, manager.openFiles.filterIsInstance<SonderrVirtualFile>().size)
    }
    ...
    fun `test the platform builds no document for a diagram file`() {
        ...
        // and SonderrVirtualFile has no content to give.
    ...
    fun `test both providers accept a diagram file and only the diagram kind offers a source view`() {
        ...
        val diagram = file(params(flow))
        val attachment = SonderrVirtualFile(SonderrPath(AttachmentEditorKind.ID, mapOf("partId" to "prt1")))

        assertTrue(SonderrFileEditorProvider().accept(project, diagram))
        assertTrue(SonderrSourceEditorProvider().accept(project, diagram))
        assertTrue(SonderrFileEditorProvider().accept(project, attachment))
        assertFalse(SonderrSourceEditorProvider().accept(project, attachment))
    }
    ...
    fun `test both providers hide other editors so the platform keeps them side by side`() {
        ...
        // as soon as one provider requests it, so both sonderr providers must agree.
        assertEquals(FileEditorPolicy.HIDE_OTHER_EDITORS, SonderrFileEditorProvider().policy)
        assertEquals(FileEditorPolicy.HIDE_OTHER_EDITORS, SonderrSourceEditorProvider().policy)
    }
    ...
    fun `test diagram and source views build named editors and release their editor`() {
        ...
        edtWait {
            val main = SonderrFileEditorProvider().createEditor(project, diagram)
            val source = SonderrSourceEditorProvider().createEditor(project, diagram)
            try {
                assertEquals(SonderrBundle.message("diagram.title"), main.name)
                assertEquals(SonderrBundle.message("diagram.source"), source.name)
    ...
    private fun file(params: Map<String, String>) = SonderrVirtualFile(SonderrPath(DiagramEditorKind.ID, params))
