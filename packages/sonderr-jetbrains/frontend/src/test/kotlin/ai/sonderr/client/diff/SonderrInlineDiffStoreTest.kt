...
class SonderrInlineDiffStoreTest : BasePlatformTestCase() {
    private lateinit var coroutines: TestCoroutines
    private lateinit var workspace: FakeWorkspaceRpcApi
    private lateinit var service: SonderrDiffEditorService

    override fun setUp() {
        super.setUp()
        coroutines = TestCoroutines()
        workspace = FakeWorkspaceRpcApi()
        service = SonderrDiffEditorService(project, coroutines.scope)
        project.replaceService(SonderrInlineDiffStore::class.java, SonderrInlineDiffStore(), testRootDisposable)
        ApplicationManager.getApplication()
            .replaceService(SonderrWorkspaceService::class.java, SonderrWorkspaceService(coroutines.scope, workspace), testRootDisposable)
    }
