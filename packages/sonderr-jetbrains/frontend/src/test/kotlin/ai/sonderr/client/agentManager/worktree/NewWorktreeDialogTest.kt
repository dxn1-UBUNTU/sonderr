...
class NewWorktreeDialogTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        workspaces = SonderrWorkspaceService(scope, ws)
    ...
    fun `test loads the default mode, model, and reasoning options`() {
        ...
        edt {
            ...
            assertEquals("sonderr/gpt-5", model().selectionKeyForTest())
    ...
    fun `test selecting a model persists it for the default agent`() {
        ...
        edt { model().onSelect(ModelPicker.Item("gpt-5", "GPT-5", "sonderr", "Sonderr", variants = listOf("low", "high"))) }

        assertEquals(ModelSelectionDto("sonderr", "gpt-5"), app.models.value.model["build"])
    }
    ...
    fun `test selecting reasoning persists the variant for the current model`() {
        ...
        assertEquals("high", app.models.value.variant["sonderr/gpt-5"])
    }
    ...
    fun `test creating forwards the prompt, resolved branch, and default selection`() {
        ...
        assertEquals("sonderr", payload.provider)
