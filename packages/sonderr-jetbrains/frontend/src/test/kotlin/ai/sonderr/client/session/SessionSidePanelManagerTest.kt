...
import ai.sonderr.client.session.history.HistoryPanel
...
class SessionSidePanelManagerTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        app = SonderrAppService(coroutines.scope, FakeAppRpcApi().also {
            it.state.value = SonderrAppStateDto(SonderrAppStatusDto.READY)
        })
        workspaces = SonderrWorkspaceService(coroutines.scope, FakeWorkspaceRpcApi().also {
    ...
    fun `test inactive sessions keep queued style updates`() {
        ...
        val style = ai.sonderr.client.session.ui.style.SessionEditorStyle.create(family = "Courier New", size = 24)
    ...
    fun `test history overlays update before hidden timeout`() {
        ...
        assertEquals(SonderrBundle.message("history.badge.question"), history.badgeText(0))
    }
    ...
    private fun setInactiveDisposeTimeout(ms: Int) {
        val key = "sonderr.session.inactive.disposeTimeoutMs"
    ...
    private fun JPanel.controller(): ai.sonderr.client.session.controller.SessionController {
        val field = SessionUi::class.java.getDeclaredField("controller")
        field.isAccessible = true
        return field.get(this) as ai.sonderr.client.session.controller.SessionController
    }
