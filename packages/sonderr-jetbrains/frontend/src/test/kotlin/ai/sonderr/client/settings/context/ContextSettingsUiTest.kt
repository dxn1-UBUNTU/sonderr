...
class ContextSettingsUiTest : BasePlatformTestCase() {
    ...
    override fun tearDown() {
        try {
            ...
            uiScope.cancel()
            appScope.cancel()
            SonderrPluginSettings.unsetAutoEditorContext()
        } finally {
            super.tearDown()
        }
    }
    ...
    fun `test editor context toggle marks modified and applies without a config patch`() {
        val panel = requireUi()
        assertTrue(SonderrPluginSettings.getAutoEditorContext())
        ...
        assertFalse(SonderrPluginSettings.getAutoEditorContext())
