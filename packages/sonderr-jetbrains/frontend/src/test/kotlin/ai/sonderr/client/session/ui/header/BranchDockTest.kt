...
import ai.sonderr.client.actions.ChatMoveToWorktreeAction
import ai.sonderr.client.actions.ChatNewWorktreeAction
...
class BranchDockTest : BasePlatformTestCase() {
    ...
    fun `test move action visible with messages`() {
        ...
        assertEquals(SonderrBundle.message("session.dock.move"), p.text)
        assertEquals(SonderrBundle.message("session.dock.move.tooltip.empty"), p.description)
    }
    ...
    fun `test move action visible with changes and no messages`() {
        ...
        val p = update(ChatMoveToWorktreeAction(), dock)
        assertTrue(p.isVisible)
        assertEquals(SonderrBundle.message("session.dock.move.tooltip.one"), p.description)
    }
    ...
    fun `test move action visible with changes and no session`() {
        ...
        assertTrue(p.isVisible)
        assertTrue(p.isEnabled)
        assertEquals(SonderrBundle.message("session.dock.move.tooltip.changes.other", 2), p.description)
    }
    ...
    fun `test new worktree action visible when dock active`() {
        ...
        assertTrue(p.isVisible)
        assertEquals(SonderrBundle.message("session.dock.newWorktree"), p.text)
    }
