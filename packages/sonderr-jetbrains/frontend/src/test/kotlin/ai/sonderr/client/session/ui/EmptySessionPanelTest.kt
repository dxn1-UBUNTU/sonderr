...
class EmptySessionPanelTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        val workspaces = SonderrWorkspaceService(scope, FakeWorkspaceRpcApi().also {
    ...
    fun `test explanation uses welcome message`() {
        ...
        assertEquals(
            "Sonderr is an AI coding assistant. Ask it to build features, fix bugs, or explain your codebase.",
    ...
    fun `test show history button uses localized text`() {
        ...
        assertEquals(ai.sonderr.client.plugin.SonderrBundle.message("session.showHistory"), panel.showHistoryText())
    }

    fun `test feedback button uses localized text and icon`() {
        val panel = panel()

        assertEquals(SonderrBundle.message("feedback.button"), panel.feedbackText())
    ...
    fun `test feedback popup content opens expected destinations`() {
        ...
        assertEquals(
            listOf(
                SonderrBundle.message("feedback.dialog.github"),
                SonderrBundle.message("feedback.dialog.discord"),
                SonderrBundle.message("feedback.dialog.support"),
            ),
            buttons.map { it.text },
        )
    ...
    fun `test feedback discord action has icon`() {
        ...
        val discord = UIUtil.uiTraverser(content)
            ...
            .first { it.text == SonderrBundle.message("feedback.dialog.discord") }
    ...
    fun `test renderer shows running badge for busy recent session`() {
        ...
        assertEquals(SonderrBundle.message("session.part.tool.running"), badgeText(cell))
    }
    ...
    fun `test renderer shows overlay badge for active recent session`() {
        ...
        assertEquals(SonderrBundle.message("history.badge.question"), badgeText(cell))
    }
    ...
    fun `test sync activity updates recent badge kind change`() {
        ...
        panel.syncActivity()
        assertEquals(
            SonderrBundle.message("session.part.tool.running"),
        ...
        assertEquals(
            SonderrBundle.message("history.badge.question"),
            badgeText(panel.rendererComponent(session("ses_1")) as BorderLayoutPanel),
        )
    }
