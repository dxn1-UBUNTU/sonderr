...
class ViewSwitchingTest : SessionControllerTestBase() {
    ...
    fun `test session change in another directory does not refresh recents`() {
        ...
        change("ses_other", "/repo/.sonderr/worktrees/other", SessionChangeKindDto.CREATED)
    ...
    private fun session(id: String) = ai.sonderr.rpc.dto.SessionDto(
        id = id,
        projectID = "prj",
        directory = "/test",
        title = "Title $id",
        version = "1",
        time = ai.sonderr.rpc.dto.SessionTimeDto(created = 1.0, updated = 2.0),
    )
    ...
    fun `test empty session overlay show includes logged in profile`() {
        ...
        appRpc.state.value = SonderrAppStateDto(SonderrAppStatusDto.READY, profile = prof)
        val m = controller()
        val events = collect(m)
    ...
    fun `test selecting personal account emits switching overlay`() {
        ...
        appRpc.state.value = SonderrAppStateDto(
            ...
            profile = ProfileDto(
                email = "user@example.com",
                currentOrgId = "org_1",
                organizations = listOf(ProfileOrganizationDto("org_1", "Sonderr", "OWNER")),
            ),
        )
        val m = controller()
        val events = collect(m)
