...
import ai.sonderr.client.testing.FakeMigrationRpcApi
...
class SonderrMigrationServiceTest : BasePlatformTestCase() {

    private lateinit var scope: CoroutineScope
    private lateinit var rpc: FakeMigrationRpcApi
    private lateinit var service: SonderrMigrationService
    private lateinit var app: MutableStateFlow<SonderrAppStateDto>
    ...
    override fun setUp() {
        ...
        app = MutableStateFlow(SonderrAppStateDto(SonderrAppStatusDto.DISCONNECTED))
        autocomplete.clear()
        telemetry.clear()
        service = SonderrMigrationService(scope, rpc, app, { autocomplete.add(it) }) { event, props -> telemetry.add(event to props) }
    }
    ...
    fun `test migration required app state shows needed without polling`() {
        app.value = SonderrAppStateDto(SonderrAppStatusDto.MIGRATION_REQUIRED, migration = sampleDetection())
        settle()
    ...
    fun `test ready app state hides migration`() {
        app.value = SonderrAppStateDto(SonderrAppStatusDto.MIGRATION_REQUIRED, migration = sampleDetection())
        settle()
        app.value = SonderrAppStateDto(SonderrAppStatusDto.READY)
        settle()
    ...
    fun `test duplicate migration required does not reset running migration`() {
        ...
        app.value = SonderrAppStateDto(SonderrAppStatusDto.MIGRATION_REQUIRED, migration = detection)
        settle()
    ...
    fun `test complete event without items finalizes pending session progress`() = runBlocking {
        app.value = SonderrAppStateDto(SonderrAppStatusDto.MIGRATION_REQUIRED, migration = sampleDetection().copy(
