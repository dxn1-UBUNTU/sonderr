...
import ai.sonderr.client.session.ui.header.BranchChangesBadge
import ai.sonderr.client.ui.FilledBadgeIcon
import ai.sonderr.client.ui.HoverIcon
import ai.sonderr.client.ui.stateLabel
...
class WorktreePrHeaderViewTest : BasePlatformTestCase() {
    ...
    fun `test terminal button is icon-only and triggers callback`() {
        ...
        assertEquals(SonderrBundle.message("worktree.session.terminal.tooltip"), edt { terminal.toolTipText })
    ...
    private fun pull(state: GhState) = WorktreePrDto(
        ...
        number = 123,
        state = state,
        url = "https://github.com/sonderr/test/pull/123",
