...
class SessionUiStyleTest : BasePlatformTestCase() {
    ...
    fun `test secondary text blends foreground toward session background and uses regular font`() {
        ...
        val bg = UIManager.getColor("Sonderr.Session.background")
        val fg = UIManager.getColor("Sonderr.Session.foreground")
        try {
            UIManager.put("Sonderr.Session.background", Color.BLACK)
            UIManager.put("Sonderr.Session.foreground", Color.WHITE)
            assertTrue(luminance(SessionUiStyle.Text.Secondary.foreground()) < luminance(SessionUiStyle.Colors.foreground()))

            UIManager.put("Sonderr.Session.background", Color.WHITE)
            UIManager.put("Sonderr.Session.foreground", Color.BLACK)
            assertTrue(luminance(SessionUiStyle.Text.Secondary.foreground()) > luminance(SessionUiStyle.Colors.foreground()))
        } finally {
            UIManager.put("Sonderr.Session.background", bg)
            UIManager.put("Sonderr.Session.foreground", fg)
        }
