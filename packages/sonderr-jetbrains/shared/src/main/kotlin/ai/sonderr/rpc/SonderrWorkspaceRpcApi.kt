 ...
 * or worktree). Each call routes to a [SonderrBackendWorkspace]
 * via the workspace manager.
 */
@Rpc
interface SonderrWorkspaceRpcApi : RemoteApi<Unit> {
    companion object {
        suspend fun getInstance(): SonderrWorkspaceRpcApi {
            return RemoteApiProviderService.resolve(remoteApiDescriptor<SonderrWorkspaceRpcApi>())
        }
    }
    ...
    suspend fun state(directory: String): Flow<SonderrWorkspaceStateDto>
    ...
    /** Fuzzy file/folder search via Sonderr Core. */
