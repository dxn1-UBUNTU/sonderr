...
class SonderrBackendProviderSettingsManagerTest {
    ...
    @Test
    fun `disconnecting sonderr gateway returns error without logout`() = runBlocking {
        mock.providers = """{
            "all":[{"id":"sonderr","name":"Sonderr Gateway","source":"custom","models":{}}],
            "default":{},
            "connected":["sonderr"],
            "failed":[]
        }""".trimIndent()
        ...
        mock.resetCounts()
        val result = manager.disconnect(ProviderDisconnectDto("/test", "sonderr"))

        assertEquals("Sonderr Gateway cannot be disconnected from provider settings.", result.error)
        assertFalse(result.profileCleared)
        assertNull(mock.lastAuthDeletePath)
        assertEquals(0, mock.requestCount("/auth/sonderr"))
        assertEquals(0, mock.requestCount("/global/dispose"))
    }
    ...
    fun `state waits through dispose triggered reload`() = runBlocking {
        ...
        val app = app()
        val manager = SonderrBackendProviderSettingsManager(app)
        ...
        try {
            ...
            withTimeout(5_000) {
                app.appState.first { it is SonderrAppState.Loading }
            }
    ...
    private suspend fun manager(): SonderrBackendProviderSettingsManager {
        return SonderrBackendProviderSettingsManager(app())
    }
