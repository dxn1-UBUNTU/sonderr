...
import ai.sonderr.backend.app.ConnectionState
import ai.sonderr.backend.app.SonderrConnectionService
...
class SonderrConnectionServiceTest {
    ...
    fun `connect transitions to Connected`() = runBlocking {
        ...
        val svc = SonderrConnectionService(
          scope,
          fake,
    ...
    fun `connect provides API client`() = runBlocking {
        val svc = SonderrConnectionService(scope, fake, {}, log)
        svc.connect()
        mock.awaitSseConnection()
    ...
    fun `connect reports downloading then connecting before ready`() = runBlocking {
        ...
        val svc = SonderrConnectionService(scope, server, {}, log)
    ...
    fun `SSE concurrent callbacks preserve callback order`() = runBlocking {
        ...
        val svc = SonderrConnectionService(scope, fake, {}, blocked)
        val field = SonderrConnectionService::class.java.getDeclaredField("listener")
        ...
        val sourceField = SonderrConnectionService::class.java.getDeclaredField("source")
    ...
    fun `init error transitions to Error state`() = runBlocking {
        ...
        val svc = SonderrConnectionService(scope, failing, {}, log)
        svc.connect()
    ...
    // extractType tests moved to SonderrCliDataParserTest
    ...
    fun `dispose prevents reconnect on late SSE callbacks`() = runBlocking {
        val reconnects = AtomicInteger(0)
        val svc = SonderrConnectionService(scope, fake, { reconnects.incrementAndGet() }, log)
        svc.connect()
        mock.awaitSseConnection()
    ...
    private class BlockingLog : SonderrLog {
