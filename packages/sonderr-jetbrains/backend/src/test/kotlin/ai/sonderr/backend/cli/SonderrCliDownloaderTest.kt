...
class SonderrCliDownloaderTest {
    @TempDir
    ...
    fun `downloads extracts and caches pinned cli`() = runBlocking {
        MockWebServer().use { server ->
            ...
            assertEquals(File(File(dir, "1.2.3"), SonderrCliPlatform.current()).absolutePath, cli.parentFile.parentFile.absolutePath)
            assertEquals("#!/bin/sh\n", cli.readText())
            assertTrue(File(cli.parentFile, "sonderr-sandbox-mutation-worker.js").isFile)
            assertEquals("/release/v1.2.3/sonderr-${SonderrCliPlatform.current()}.${SonderrCliPlatform.archive()}", server.takeRequest().path)
            assertEquals(CliDownload(0, "1.2.3", SonderrCliPlatform.current()), seen.first())
            assertTrue(seen.any { it.percent == 100 && it.version == "1.2.3" && it.platform == SonderrCliPlatform.current() })
            assertTrue(
                log.messages.any {
                    it.startsWith("INFO: Sonderr CLI 1.2.3 for ${SonderrCliPlatform.current()} is not cached; downloading new release into ") &&
                        it.contains("/.tmp/")
                }
            )
            assertTrue(log.messages.any { it.contains("Sonderr CLI path diagnostics:") && it.contains("cacheRoot=${dir.absolutePath}") })
            assertTrue(log.messages.any { it.contains("Sonderr CLI cache target:") && it.contains("exe=${cli.absolutePath}") })
            assertTrue(log.messages.any { it.contains("Sonderr CLI cache lock path:") && it.contains(File(dir, ".lock").canonicalPath) })

            val cachedProgress = mutableListOf<CliDownload>()
            val cached = SonderrCliDownloader(
                log = log,
                root = dir,
                baseUrl = server.url("/release").toString(),
                api = server.url("/api").toString(),
            ...
            assertContains(log.messages, "INFO: Sonderr CLI 1.2.3 (${SonderrCliPlatform.current()}) already cached at ${cli.absolutePath}; skipping download and extraction")

            File(cli.parentFile.parentFile, ".complete").writeText("ok\n")
            server.enqueue(MockResponse().setResponseCode(200).setBody(Buffer().write(bytes)))
            val stale = SonderrCliDownloader(
                log = log,
                root = dir,
                baseUrl = server.url("/release").toString(),
                api = server.url("/api").toString(),
            ...
            server.enqueue(MockResponse().setResponseCode(200).setBody(Buffer().write(bytes)))
            val forced = SonderrCliDownloader(
                log = log,
                root = dir,
                baseUrl = server.url("/release").toString(),
                api = server.url("/api").toString(),
    ...
    fun `falls back to github metadata when bundled checksum is malformed`() = runBlocking {
        MockWebServer().use { server ->
            ...
            val cli = SonderrCliDownloader(
                ...
                baseUrl = server.url("/release").toString(),
                api = server.url("/api").toString(),
                digests = mapOf(SonderrCliPlatform.current() to "not-a-digest"),
            ).resolve("1.2.3")

            assertTrue(cli.isFile)
            assertContains(
                log.messages,
                "WARN: Ignoring malformed bundled Sonderr CLI checksum for ${SonderrCliPlatform.current()}: not-a-digest"
            )
            assertEquals("/api/v1.2.3", server.takeRequest().path)
            assertEquals("/release/v1.2.3/sonderr-${SonderrCliPlatform.current()}.${SonderrCliPlatform.archive()}", server.takeRequest().path)
            assertEquals(2, server.requestCount)
    ...
    fun `prunes stale versions and removes the extracted archive`() = runBlocking {
        MockWebServer().use { server ->
            val stale = File(File(dir, "0.0.1"), SonderrCliPlatform.current())
            ...
            val archived = File(cli.parentFile.parentFile, "sonderr-${SonderrCliPlatform.current()}.${SonderrCliPlatform.archive()}")
    ...
    fun `rejects cli archive with mismatched digest`() = runBlocking {
        MockWebServer().use { server ->
            ...
            assertFalse(File(File(File(dir, "1.2.3"), SonderrCliPlatform.current()), ".complete").exists())
        }
    }
        ...
        MockWebServer().use { server ->
            ...
            val ex = assertFailsWith<IllegalStateException> {
                SonderrCliDownloader(
                    ...
                    baseUrl = server.url("/release").toString(),
                    api = server.url("/api").toString(),
                    digests = mapOf(SonderrCliPlatform.current() to "sha256:${sha256("different".toByteArray())}"),
                ).resolve("1.2.3")
            }
    ...
    fun `fails clearly and logs when the release has no matching asset`() = runBlocking {
        MockWebServer().use { server ->
            ...
            val name = "sonderr-${SonderrCliPlatform.current()}.${SonderrCliPlatform.archive()}"
    ...
    fun `fails clearly and logs when the asset has no digest`() = runBlocking {
        MockWebServer().use { server ->
            server.enqueue(
                MockResponse().setResponseCode(200).setBody(
                    """{"assets":[{"name":"sonderr-${SonderrCliPlatform.current()}.${SonderrCliPlatform.archive()}"}]}"""
                )
            )
            val log = TestLog()
            val ex = assertFailsWith<IllegalStateException> {
    ...
    fun `cache lock times out clearly when already held in this process`() = runBlocking {
        ...
        RandomAccessFile(file, "rw").channel.use { channel ->
            channel.lock().use {
                ...
                assertContains(ex.message.orEmpty(), "Timed out waiting for Sonderr CLI cache lock")
                assertContains(ex.message.orEmpty(), file.canonicalPath)
                assertTrue(log.messages.any { it.contains("Waiting for Sonderr CLI cache lock") && it.contains(file.canonicalPath) })
                assertTrue(log.messages.any { it.contains("Timed out waiting for Sonderr CLI cache lock") && it.contains(file.canonicalPath) })
            }
        }
    }
    ...
    private fun archive(script: String = "#!/bin/sh\n"): ByteArray {
        val files = mapOf(
            "bin/${SonderrCliPlatform.exe()}" to script.toByteArray(),
            "bin/sonderr-sandbox-mutation-worker.js" to "worker\n".toByteArray(),
        )
        if (SonderrCliPlatform.archive() == "zip") return zip(files)
        return tar(files)
    }

    private fun metadata(bytes: ByteArray) = metadata("sha256:${sha256(bytes)}")

    private fun digests(bytes: ByteArray) = mapOf(SonderrCliPlatform.current() to "sha256:${sha256(bytes)}")

    private fun metadata(digest: String) = MockResponse().setResponseCode(200).setBody(
        """{"assets":[{"name":"sonderr-${SonderrCliPlatform.current()}.${SonderrCliPlatform.archive()}","digest":"$digest"}]}"""
    )
