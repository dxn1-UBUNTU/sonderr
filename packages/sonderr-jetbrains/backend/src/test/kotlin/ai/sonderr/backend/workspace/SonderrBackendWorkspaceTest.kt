...
import ai.sonderr.backend.app.SonderrBackendSessionManager
import ai.sonderr.backend.app.SseEvent
import ai.sonderr.backend.cli.SonderrBackendHttpClients
import ai.sonderr.backend.workspace.SonderrBackendWorkspace
...
class SonderrBackendWorkspaceTest {

    private val mock = MockCliServer()
    private val log = TestLog()
    private val scope = CoroutineScope(SupervisorJob() + Dispatchers.Default)
    private val apps = mutableListOf<SonderrBackendAppService>()
    private val root: Path = Files.createTempDirectory("sonderr-backend-workspace")
    ...
    private fun setup(): SonderrBackendAppService =
        SonderrBackendAppService.create(scope, FakeCliServer(mock), log).also { apps.add(it) }

    private suspend fun connect(app: SonderrBackendAppService) {
        app.connect()
    ...
    private suspend fun ready(app: SonderrBackendAppService): SonderrBackendWorkspace {
        connect(app)
    ...
    private suspend fun loaded(ws: SonderrBackendWorkspace) {
        withTimeout(15_000) {
            ws.state.first { it is SonderrWorkspaceState.Ready }
        }
    }
    ...
    fun `get creates workspace on demand after Ready`() = runBlocking {
        ...
        val state = ws.state.value as SonderrWorkspaceState.Ready
    ...
    fun `workspaces stopped on app disconnect`() = runBlocking {
        ...
        assertIs<SonderrWorkspaceState.Pending>(ws.state.value)
    ...
    fun `workspace reaches Ready after creation`() = runBlocking {
        ...
        assertIs<SonderrWorkspaceState.Ready>(ws.state.value)
    }
    ...
    fun `providers failure retries then transitions to Error`() = runBlocking {
        ...
        withTimeout(15_000) {
            ws.state.first { it is SonderrWorkspaceState.Error }
        }

        val err = ws.state.value as SonderrWorkspaceState.Error
    ...
    fun `dev container virtual directory transitions to Unsupported without fetching agents`() = runBlocking {
        ...
        val state = withTimeout(15_000) {
            ws.state.first { it is SonderrWorkspaceState.Unsupported }
        } as SonderrWorkspaceState.Unsupported
    ...
    fun `missing directory transitions to Missing without fetching workspace data`() = runBlocking {
        ...
        connect(app)
        mock.resetCounts()
        val dir = Files.createTempDirectory("sonderr-missing-workspace")
        Files.delete(dir)
        val ws = app.workspaces.get(dir.toString())

        val state = withTimeout(15_000) {
            ws.state.first { it is SonderrWorkspaceState.Missing }
        } as SonderrWorkspaceState.Missing
    ...
    // ------ Data mapping ------
    // Detailed provider/command/path parsing correctness is covered in SonderrCliDataParserTest.
    ...
    fun `concurrent get for same directory returns same instance`() = runBlocking {
        val port = mock.start()
        val http = SonderrBackendHttpClients.api(mock.password)
        val api = DefaultApi(basePath = "http://127.0.0.1:$port", client = http)
        val events = MutableSharedFlow<SseEvent>()
        val sessions = SonderrBackendSessionManager(scope, log)
        val manager = SonderrBackendWorkspaceManager(scope, sessions, log)
        ...
        try {
            ...
            manager.stop()
            SonderrBackendHttpClients.shutdown(http)
        }
    }
    ...
    fun `concurrent load calls on workspace produce valid final state`() = runBlocking {
        ...
        val state = withTimeout(15_000) {
            ws.state.first {
                it is SonderrWorkspaceState.Ready &&
                    it.providers.providers.firstOrNull()?.id == "openai"
            }
        } as SonderrWorkspaceState.Ready
    ...
    fun `SSE global disposed triggers full app reload with new data`() = runBlocking {
        ...
        val prev = (app.appState.value as SonderrAppState.Ready).rev
        val before = mock.requestCount("/global/config")
        val reload = async(start = CoroutineStart.UNDISPATCHED) {
            app.appState.first { it is SonderrAppState.Ready && it.rev > prev }
        }
