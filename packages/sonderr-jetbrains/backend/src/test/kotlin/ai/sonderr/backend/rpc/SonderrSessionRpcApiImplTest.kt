...
class SonderrSessionRpcApiImplTest {
    ...
    private fun app(log: TestLog): SonderrBackendAppService {
        return SonderrBackendAppService.create(scope, FakeCliServer(mock), log).also { apps.add(it) }
    }

    private suspend fun ready(app: SonderrBackendAppService) {
        app.connect()
        withTimeout(10_000) {
            app.appState.first { it is SonderrAppState.Ready }
        }
    }
    ...
    fun `events logs normal completion`() = runBlocking(Dispatchers.Default) {
        val log = TestLog()
        val api = SonderrSessionRpcApiImpl(log = log, source = flowOf(ChatEventDto.TurnOpen("ses_test")))
    ...
    fun `events logs cancelled completion`() = runBlocking(Dispatchers.Default) {
        val log = TestLog()
        val api = SonderrSessionRpcApiImpl(log = log, source = flow { kotlinx.coroutines.awaitCancellation() })
    ...
    fun `events logs failed completion`() = runBlocking(Dispatchers.Default) {
        val log = TestLog()
        val api = SonderrSessionRpcApiImpl(log = log, source = flow { throw IllegalStateException("stream failed") })
    ...
    fun `create logs created session id`() = runBlocking(Dispatchers.Default) {
        ...
        val app = app(log)
        ready(app)
        val api = SonderrSessionRpcApiImpl(appOverride = app, log = log)
    ...
    fun `diffSides rebuilds full before by reverse-applying the patch to the working tree`() = runBlocking(Dispatchers.Default) {
        val dir = createTempDirectory("sonderr-diff")
        try {
            ...
            val diff = SonderrSessionRpcApiImpl().diffSides(null, dir.toString(), DiffFileDto(file, 1, 1, patch, "modified"), null)
    ...
    fun `diffSides returns null when the working tree drifted from the patch`() = runBlocking(Dispatchers.Default) {
        ...
        try {
            ...
            assertNull(SonderrSessionRpcApiImpl().diffSides(null, dir.toString(), DiffFileDto(file, 1, 1, patch, "modified"), null))
        } finally {
            delete(dir)
        }
    }
    ...
    fun `diffSides returns null for added files and missing patches`() = runBlocking(Dispatchers.Default) {
        ...
        try {
            ...
            assertNull(SonderrSessionRpcApiImpl().diffSides(null, dir.toString(), DiffFileDto("new.kt", 1, 0, added, "added"), null))
            assertNull(SonderrSessionRpcApiImpl().diffSides(null, dir.toString(), DiffFileDto("new.kt", 1, 0, null, "added"), null))
        } finally {
            delete(dir)
        }
    }
    ...
    fun `diffSides prefers authoritative CLI content over local reconstruction`() = runBlocking(Dispatchers.Default) {
        ...
        try {
            ...
            val api = SonderrSessionRpcApiImpl(app(server))
    ...
    private suspend fun app(server: MockCliServer): SonderrBackendAppService {
        val app = SonderrBackendAppService.create(scope, FakeCliServer(server), TestLog()).also { apps.add(it) }
        app.connect()
        ...
        assertIs<SonderrAppState.Ready>(state, "App startup failed")
        return app
    }
