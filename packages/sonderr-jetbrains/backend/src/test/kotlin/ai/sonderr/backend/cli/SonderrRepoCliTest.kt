...
class SonderrRepoCliTest {
    ...
    fun `extracts cached repo cli and force re-extracts`() = runBlocking {
        ...
        val cli = SonderrRepoCli.extract(false, dir) { ByteArrayInputStream(first) }
        ...
        val cached = SonderrRepoCli.extract(false, dir) { ByteArrayInputStream(next) }
        assertEquals(cli.absolutePath, cached.absolutePath)
        assertEquals("#!/bin/old\n", cached.readText())

        val forced = SonderrRepoCli.extract(true, dir) { ByteArrayInputStream(next) }
    ...
    fun `extracts only current platform from multi platform archive`() = runBlocking {
        ...
        val cli = SonderrRepoCli.extract(false, dir) {
            ByteArrayInputStream(multi(platform, other))
        }

        assertTrue(cli.isFile)
        assertEquals("current", cli.readText())
        assertFalse(File(dir, "$other/bin/sonderr.exe").exists())
        assertFalse(File(dir, "$other/bin/sonderr").exists())
    }
    ...
    fun `prunes stale bundled cli versions after resolve`() = runBlocking {
        ...
        val cli = SonderrRepoCli.extract(false, root, cleanup = true) {
    ...
    fun `rejects archive entries that escape root`() = runBlocking {
        val ex = assertFailsWith<IllegalStateException> {
            SonderrRepoCli.extract(false, dir) { ByteArrayInputStream(archive(entry = "../../../bad")) }
        }
    ...
    fun `pinned defaults true unless explicitly false`() {
        assertEquals(true, SonderrProps.pinned(Properties()))
        assertEquals(true, SonderrProps.pinned(Properties().apply { setProperty("cli.pinned", "true") }))
        assertEquals(false, SonderrProps.pinned(Properties().apply { setProperty("cli.pinned", "false") }))
    }

    private fun archive(script: String = "#!/bin/sh\n", entry: String = "bin/${SonderrCliPlatform.exe()}"): ByteArray {
        val out = ByteArrayOutputStream()
        ZipOutputStream(out).use { zip ->
            zip.putNextEntry(ZipEntry(entry))
            zip.write(script.toByteArray())
            zip.closeEntry()
            zip.putNextEntry(ZipEntry("bin/sonderr-sandbox-mutation-worker.js"))
    ...
    private fun multi(platform: String, other: String): ByteArray {
        val out = ByteArrayOutputStream()
        ZipOutputStream(out).use { zip ->
            zip.putNextEntry(ZipEntry("$platform/bin/${SonderrCliPlatform.exe()}"))
            zip.write("current".toByteArray())
            zip.closeEntry()
            zip.putNextEntry(ZipEntry("$other/bin/sonderr.exe"))
