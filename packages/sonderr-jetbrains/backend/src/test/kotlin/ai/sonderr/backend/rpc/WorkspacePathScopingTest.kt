...
class WorkspacePathScopingTest {
    ...
    private val base: Path = Path.of(System.getProperty("user.home")).resolve("sonderr-scope-test").normalize()
    ...
    @Test
    fun `workspace scope keeps normal project and sonderr plan files`() {
        assertEquals("backend/src/Main.java", relativeWithinWorkspace(base, at("backend", "src", "Main.java")))
        assertEquals(".sonderr/plans/x.md", relativeWithinWorkspace(base, at(".sonderr", "plans", "x.md")))
    }

    @Test
    fun `workspace scope rejects managed worktree storage from main checkout`() {
        assertNull(relativeWithinWorkspace(base, at(".sonderr", "worktrees")))
        assertNull(relativeWithinWorkspace(base, at(".sonderr", "worktrees", "foo", "backend", "src", "Main.java")))
    }

    @Test
    fun `workspace scope allows files inside the active worktree`() {
        val root = at(".sonderr", "worktrees", "foo")
    ...
    fun `workspace scope rejects sibling worktrees from active worktree`() {
        val root = at(".sonderr", "worktrees", "foo")
        val sibling = at(".sonderr", "worktrees", "bar", "backend", "src", "Main.java")
    ...
    fun `workspace scope rejects nested managed worktree storage`() {
        ...
        assertNull(relativeWithinWorkspace(root, root.resolve(".sonderr/worktrees/bar/backend/src/Main.java")))
    }
    ...
    fun `git changes returns capped large diff without blocking`() = runBlocking {
        ...
        try {
            ...
            val diff = assertNotNull(SonderrWorkspaceRpcApiImpl().gitChanges(dir.toString()))
    ...
    fun `git changes returns null outside git repositories`() = runBlocking {
        val dir = Files.createTempDirectory("sonderr-non-git")
        try {
            assertNull(SonderrWorkspaceRpcApiImpl().gitChanges(dir.toString()))
        } finally {
            delete(dir)
        }
    }
    ...
    private fun repo(): Path? {
        ...
        val dir = Files.createTempDirectory("sonderr-git")
        git(dir, "init")
