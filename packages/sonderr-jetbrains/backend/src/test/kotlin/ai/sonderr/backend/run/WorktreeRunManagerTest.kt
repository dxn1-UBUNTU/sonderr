package ai.sonderr.backend.run

import ai.sonderr.rpc.dto.RunProcessState
...
class WorktreeRunManagerTest : BasePlatformTestCase() {
    ...
    fun testConfigsListsOnlySupportedTypes() {
        val params = register(paramsType("sonderr.test.params.list"))
        val plain = register(plainType("sonderr.test.plain.list"))
        val moduled = register(moduleType("sonderr.test.module.list"))
        add(params, "dev")
        add(plain, "app")
        add(moduled, "mod")
        ...
        assertEquals("Sonderr Params sonderr.test.params.list", configs.first { it.name == "dev" }.type)
    }

    fun testRunTransplantsAndCachesClone() = runBlocking {
        val type = register(paramsType("sonderr.test.params.run"))
    ...
    fun testNestedWorkingDirectoryIsRebasedOntoWorktree() = runBlocking {
        val type = register(paramsType("sonderr.test.params.nested"))
        val settings = add(type, "dev")
        val source = settings.configuration as ParamsConfig
        val repo = requireNotNull(project.basePath)
        source.workingDirectory = "$repo/packages/sonderr-jetbrains"
        val wt = "$repo/.sonderr/worktrees/nested-wt"

        assertTrue(manager().run(settings.uniqueID, wt).ok)
        val cfg = launched.single().configuration as ParamsConfig
        assertEquals(Path.of("$wt/packages/sonderr-jetbrains").toString(), cfg.workingDirectory)
        // The user's own configuration must stay untouched.
        assertEquals("$repo/packages/sonderr-jetbrains", source.workingDirectory)
    }

    fun testGradleNestedProjectPathIsRebasedOntoWorktree() = runBlocking {
        val type = register(esType("sonderr.test.es.nested"))
        val settings = add(type, "runIdeSplitMode")
        val source = settings.configuration as ExternalSystemRunConfiguration
        val repo = requireNotNull(project.basePath)
        source.settings.externalProjectPath = "$repo/packages/sonderr-jetbrains"
        source.settings.taskNames = listOf(":runIdeSplitMode")
        val wt = "$repo/.sonderr/worktrees/gradle-wt"

        assertTrue(manager().run(settings.uniqueID, wt).ok)
        val cfg = launched.single().configuration as ExternalSystemRunConfiguration
        assertEquals(Path.of("$wt/packages/sonderr-jetbrains").toString(), cfg.settings.externalProjectPath)
        ...
        assertEquals("$repo/packages/sonderr-jetbrains", source.settings.externalProjectPath)
        assertTrue(source.settings.env.isEmpty())
    }

    fun testRunRejectsUnknownAndUnsupported() = runBlocking {
        val plain = register(plainType("sonderr.test.plain.run"))
    ...
    fun testTopicTracksStateStopAndTerminate() = runBlocking {
        val type = register(paramsType("sonderr.test.params.topic"))
        val settings = add(type, "srv")
        val mgr = manager()
        val wt = "/tmp/sonderr-topic-wt"
        assertTrue(mgr.run(settings.uniqueID, wt).ok)
        val clone = launched.single()
    ...
    fun testStopDestroysProcessAndDropsTerminatedHandler() = runBlocking {
        val type = register(paramsType("sonderr.test.params.destroy"))
        val settings = add(type, "srv")
        val mgr = manager()
        val wt = "/tmp/sonderr-destroy-wt"
        assertTrue(mgr.run(settings.uniqueID, wt).ok)
        val clone = launched.single()
    ...
    fun testEditedSourceStopsTheReplacedCloneAndTracksTheFreshOne() = runBlocking {
        val type = register(paramsType("sonderr.test.params.fresh"))
        val settings = add(type, "dev")
        val source = settings.configuration as ParamsConfig
        val mgr = manager()
        val wt = "/tmp/sonderr-fresh-wt"
        assertTrue(mgr.run(settings.uniqueID, wt).ok)
    ...
    fun testReleaseStopsProcessesAndForgetsClones() = runBlocking {
        val type = register(paramsType("sonderr.test.params.release"))
        val settings = add(type, "srv")
        val mgr = manager()
        val wt = "/tmp/sonderr-release-wt"
        assertTrue(mgr.run(settings.uniqueID, wt).ok)
        val clone = launched.single()
    ...
    fun testReleaseStopsAStartAlreadyInFlight() = runBlocking {
        val type = register(paramsType("sonderr.test.params.inflight"))
        val settings = add(type, "srv")
        val mgr = manager()
        val wt = "/tmp/sonderr-inflight-wt"
    ...
    fun testCloneNameUsesStoredWorktreeLabel() = runBlocking {
        val type = register(paramsType("sonderr.test.params.label"))
        val settings = add(type, "dev")
        val repo = requireNotNull(project.basePath)
        val wt = "$repo/.sonderr/worktrees/feature"
        val store = Path.of(repo).resolve(".sonderr").resolve("worktree-names.json")
    ...
    fun testSecondStopForceKills() = runBlocking {
        val type = register(paramsType("sonderr.test.params.kill"))
        val settings = add(type, "srv")
        val mgr = manager()
        val wt = "/tmp/sonderr-kill-wt"
        assertTrue(mgr.run(settings.uniqueID, wt).ok)
        val clone = launched.single()
    ...
    fun testBuildIsUnavailableWithoutALinkedExternalProject() = runBlocking {
        ...
        val result = mgr.build("/tmp/sonderr-build-wt", clean = false)
    ...
    private fun esType(id: String) = TestType(id) { project, factory, name ->
        ExternalSystemRunConfiguration(ProjectSystemId("SONDERR_TEST"), project, factory, name).also {
    ...
    ) : ConfigurationTypeBase(id, "Sonderr Params $id", null, null as javax.swing.Icon?) {
        init {
    ...
    private class ParamsConfig(project: Project, factory: ConfigurationFactory, name: String) :
        ...
        override fun writeExternal(element: Element) {
            ...
            element.setAttribute("sonderrEnv", env.toSortedMap().toString())
            element.setAttribute("sonderrDir", dir ?: "")
            element.setAttribute("sonderrParams", params ?: "")
        }
    ...
    private class StubTask : BeforeRunTask<StubTask>(KEY) {
        companion object {
            val KEY = Key.create<StubTask>("sonderr.test.before")
        }
    }

    private class FakeRunner : ProgramRunner<RunnerSettings> {
        override fun getRunnerId(): String = "sonderr.test.runner"
