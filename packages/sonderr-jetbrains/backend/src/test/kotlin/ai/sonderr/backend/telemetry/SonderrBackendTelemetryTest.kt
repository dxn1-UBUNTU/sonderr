package ai.sonderr.backend.telemetry
...
class SonderrBackendTelemetryTest {
    @Test
    fun `capture posts to telemetry endpoint with auth`() = runBlocking {
        val server = MockWebServer()
        server.enqueue(MockResponse().setBody("{}"))
        server.start()
        val http = SonderrBackendHttpClients.api("secret")
        try {
            SonderrBackendTelemetry().capture(http, server.port, "Test Event", mapOf("source" to "test"))
    ...
    fun `set enabled posts to telemetry endpoint`() = runBlocking {
        ...
        val http = SonderrBackendHttpClients.api("secret")
        try {
            SonderrBackendTelemetry().setEnabled(http, server.port, true)
    ...
    fun `capture failure does not throw`() = runBlocking {
        SonderrBackendTelemetry().capture(null, 0, "Test Event", emptyMap())
    }
    ...
    fun `dev mode does not post capture`() = runBlocking {
        ...
        val http = SonderrBackendHttpClients.api("secret")
        try {
            SonderrBackendTelemetry().capture(http, server.port, "Test Event", emptyMap())
