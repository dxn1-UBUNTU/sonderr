...
class SonderrAgentBehaviorRpcApiImplTest {
    ...
    fun `skills and remove skill call CLI endpoints`() = runBlocking {
        val dir = Files.createTempDirectory("sonderr-skill-test")
        ...
        assertEquals(1, mock.requestCount("/sonderr/skill/remove"))
    ...
    fun `command files and remove command call CLI endpoints`() = runBlocking {
        val dir = Files.createTempDirectory("sonderr-command-test")
        ...
        assertEquals(1, mock.requestCount("/sonderr/command/remove"))
    ...
    fun `save commands validates known and new project command paths`() = runBlocking {
        val project = Files.createTempDirectory("sonderr-command-project")
        val known = Files.createDirectories(project.resolve(".sonderr/command")).resolve("known.md")
        val added = project.resolve(".sonderr/commands/new.md")
        val other = Files.createTempFile("sonderr-command-other", ".md")
        ...
        assertEquals(1, mock.requestCount("/sonderr/command/files"))
    ...
    fun `save commands validates new global command paths`() = runBlocking {
        val project = Files.createTempDirectory("sonderr-command-project")
        val config = Files.createTempDirectory("sonderr-command-config")
    ...
    fun `url cached skills are read only`() = runBlocking {
        val cache = Path.of(System.getProperty("user.home"), ".cache", "sonderr", "skills", "remote")
    ...
    fun `custom skills under non cache paths remain editable`() = runBlocking {
        val dir = Files.createTempDirectory("sonderr-skill-test")
        val file = Files.createDirectories(dir.resolve("cache/sonderr/skills/custom")).resolve("SKILL.md")
    ...
    private suspend fun rpc(): SonderrAgentBehaviorRpcApiImpl = SonderrAgentBehaviorRpcApiImpl(app())

    private suspend fun app(): SonderrBackendAppService {
        val app = SonderrBackendAppService.create(scope, FakeCliServer(mock), TestLog())
        app.connect()
        withTimeout(10_000) {
