...
class SonderrBackendSessionManagerTest {
     ...
     */
    private suspend fun awaitChange(app: SonderrBackendAppService, push: () -> Unit): SessionChangeDto =
        withTimeout(10_000) {
    ...
    fun `recent narrows the worktree family to the current worktree`() = runBlocking {
        ...
        app.sessions.recent("/repo/.sonderr/worktrees/feature", 5)
    ...
    fun `cloudSessions passes filters and cursor`() = runBlocking {
        ...
        app.sessions.cloudSessions("/repo path", "cur 1", 25, "git@github.com:Sonderr-Org/sonderr.git")

        val path = mock.lastCloudSessionsPath ?: error("missing cloud sessions request")
        assertTrue(path.startsWith("/sonderr/cloud-sessions?"))
        ...
        assertTrue(decoded.contains("gitUrl=git@github.com:Sonderr-Org/sonderr.git"), path)
    }
    ...
    fun `importCloudSession posts cloud id and returns local session`() = runBlocking {
        ...
        assertTrue(path.startsWith("/sonderr/cloud/session/import?"))
        assertTrue(URLDecoder.decode(path, "UTF-8").contains("directory=/repo path"), path)
