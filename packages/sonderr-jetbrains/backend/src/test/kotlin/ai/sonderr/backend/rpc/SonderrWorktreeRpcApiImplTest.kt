...
import ai.sonderr.rpc.parsePrUrl
...
import ai.sonderr.rpc.dto.MoveStage
...
class SonderrWorktreeRpcApiImplTest {
    private val repo: Path = Files.createTempDirectory("sonderr-worktree")
    private val remote: Path = Files.createTempDirectory("sonderr-origin")
    private val api = SonderrWorktreeRpcApiImpl()
    ...
    fun `branch status skips a missing directory without caching the empty result`() = runBlocking {
        initRepo()
        val dir = repo.resolve(".sonderr").resolve("worktrees").resolve("late")
    ...
    @Test
    fun `stats leaves a gone worktree outside the sonderr storage registered`() = runBlocking {
        initRepo()
    ...
    @Test
    fun `staleWorktrees only reports gone worktrees inside the sonderr storage`() {
        val main = WorktreeDto("/repo", "repo", "main", "/repo", main = true)
        val managed = WorktreeDto(
            "/repo/.sonderr/worktrees/gone",
            "gone",
            "feature/gone",
            "/repo/.sonderr/worktrees/gone",
            prunable = true,
    ...
    fun `parseWorktreeList reads porcelain output and flags the main tree`() {
        val raw = """
            ...
            worktree /repo/.sonderr/worktrees/feature-x
        ...
        assertEquals("/repo/.sonderr/worktrees/feature-x", list[1].path)
    ...
    fun `parseWorktreeList captures the lock flag and reason`() {
        val raw = """
            ...
            worktree /repo/.sonderr/worktrees/hyper-video
            HEAD 2222222222222222222222222222222222222222
    ...
    fun `managedWorktrees keeps only agent manager worktrees`() {
        ...
        assertEquals(listOf("/repo", "/repo/.sonderr/worktrees/feature-x"), list.map { it.path })
    }
    ...
    fun `managedWorktrees rejects the storage root itself`() {
        val raw = """
            ...
            worktree /repo/.sonderr/worktrees
            HEAD 2222222222222222222222222222222222222222
    ...
    fun `managedWorktrees rejects nested and prunable worktrees`() {
        val raw = """
            ...
            worktree /repo/.sonderr/worktrees/feature-x/.sonderr/worktrees/nested
            HEAD 3333333333333333333333333333333333333333
            branch refs/heads/nested

            worktree /repo/.sonderr/worktrees/dead
            HEAD 4444444444444444444444444444444444444444
    ...
    fun `overlayWorktreeNames applies labels only to non-main worktrees`() {
        val main = WorktreeDto("/repo", "repo", "main", "/repo", main = true)
        val child = WorktreeDto("/repo/.sonderr/worktrees/feature-x", "feature-x", "feature/x", "/repo/.sonderr/worktrees/feature-x")
    ...
    fun `worktree names store round trips and tolerates missing or corrupt files`() {
        val file = repo.resolve(".sonderr").resolve("jetbrains.json")

        assertTrue(readWorktreeNames(file).isEmpty())
        writeWorktreeNames(file, mapOf("/repo/.sonderr/worktrees/feature-x" to "Feature Label", "/blank" to ""))

        assertEquals(mapOf("/repo/.sonderr/worktrees/feature-x" to "Feature Label"), readWorktreeNames(file))
    ...
    fun `worktree state round trips and migrates legacy names`() {
        val file = repo.resolve(".sonderr").resolve("jetbrains.json")
        val first = "/repo/.sonderr/worktrees/zebra"
        val second = "/repo/.sonderr/worktrees/alpha"
    ...
    fun `orderWorktrees keeps main first and sorts worktrees by persisted order`() {
        val main = WorktreeDto("/repo", "repo", "main", "/repo", main = true)
        val first = WorktreeDto("/repo/.sonderr/worktrees/zebra", "zebra", "zebra", "/repo/.sonderr/worktrees/zebra")
        val second = WorktreeDto("/repo/.sonderr/worktrees/alpha", "alpha", "alpha", "/repo/.sonderr/worktrees/alpha")
        val third = WorktreeDto("/repo/.sonderr/worktrees/beta", "beta", "beta", "/repo/.sonderr/worktrees/beta")
    ...
    fun `create from inside linked worktree uses main worktree storage`() = runBlocking {
        ...
        assertEquals(repo.resolve(".sonderr").resolve("worktrees").resolve("feature-y").toRealPath().toString(), created.path)
        assertFalse(
            Files.exists(Path.of(first.path).resolve(".sonderr").resolve("worktrees").resolve("feature-y")),
    ...
    fun `create rejects a branch slug that escapes storage`() = runBlocking {
        ...
        assertFalse(Files.exists(repo.resolve(".sonderr").resolve("escape")))
    }
    ...
    fun `create records newest worktree first so reload keeps it on top`() = runBlocking {
        ...
        assertEquals(
            ...
            readWorktreeState(repo.resolve(".sonderr").resolve("jetbrains.json")).worktreeOrder,
        )
    }
    ...
    fun `reorder drops unknown paths and appends omitted worktrees`() = runBlocking {
        ...
        val order = readWorktreeState(repo.resolve(".sonderr").resolve("jetbrains.json")).worktreeOrder
        assertEquals(listOf(second.path, first.path), order)
    }

    @Test
    fun `reorder returns false when the repo has no worktrees`() = runBlocking {
        assertFalse(api.reorder(repo.toString(), listOf("/repo/.sonderr/worktrees/x")))
    }
    ...
    fun `remove prunes names and order from worktree state`() = runBlocking {
        ...
        assertTrue(removed.ok, "remove should report success: ${removed.error}")
        val state = readWorktreeState(repo.resolve(".sonderr").resolve("jetbrains.json"))
    ...
    fun `reconcile drops visibility for vanished worktrees but keeps the main tree`() {
        val main = "/repo"
        val live = "/repo/.sonderr/worktrees/live"
        val dead = "/repo/.sonderr/worktrees/dead"
    ...
    fun `create and list never record session list visibility`() = runBlocking {
        ...
        assertTrue(readWorktreeState(repo.resolve(".sonderr").resolve("jetbrains.json")).sessionList.isEmpty())
    ...
    fun `remove prunes session list visibility`() = runBlocking {
        ...
        assertEquals(
            ...
            readWorktreeState(repo.resolve(".sonderr").resolve("jetbrains.json")).sessionList,
        )
    }
    ...
    fun `rename persists a custom worktree name and list overlays it`() = runBlocking {
        ...
        assertEquals(mapOf(created.path to "Feature Label"), readWorktreeNames(repo.resolve(".sonderr").resolve("jetbrains.json")))
    }
    ...
    fun `adopt names a default worktree and list overlays the adopted name`() = runBlocking {
        ...
        assertEquals(mapOf(created.path to "Fix login bug"), readWorktreeNames(repo.resolve(".sonderr").resolve("jetbrains.json")))
    }
    ...
    fun `remove refuses a worktree containing a live nested worktree`() = runBlocking {
        ...
        val old = Path.of(parent.path).resolve(".sonderr").resolve("worktrees").resolve("nested")
    ...
    fun `parsePrUrl reads owner repo and number and rejects non-PR urls`() {
        val ref = assertNotNull(parsePrUrl("https://github.com/Sonderr-Org/sonderr/pull/12714"))
        assertEquals("Sonderr-Org", ref.owner)
        assertEquals("sonderr", ref.repo)
        assertEquals(12714, ref.number)

        assertNull(parsePrUrl("https://github.com/Sonderr-Org/sonderr/issues/1"))
    ...
    fun `prTargets keeps the main tree and drops detached and prunable entries`() {
        val items = listOf(
            ...
            WorktreeDto("/repo/.sonderr/worktrees/a", "a", "feature/a", "/repo/.sonderr/worktrees/a"),
            WorktreeDto("/repo/.sonderr/worktrees/detached", "detached", "(detached)", "/repo/.sonderr/worktrees/detached"),
            WorktreeDto("/repo/.sonderr/worktrees/gone", "gone", "feature/gone", "/repo/.sonderr/worktrees/gone", prunable = true),
        )

        assertEquals(listOf("/repo", "/repo/.sonderr/worktrees/a"), prTargets(items).map { it.path })
    }

    @Test
    fun `baseBranch reads the main tree branch and ignores a detached one`() {
        val main = WorktreeDto("/repo", "repo", "main", "/repo", main = true)
        val linked = WorktreeDto("/repo/.sonderr/worktrees/a", "a", "feature/a", "/repo/.sonderr/worktrees/a")
    ...
    fun `parsePr reads title from gh output`() {
        val pull = assertNotNull(parsePr("/repo/.sonderr/worktrees/feature-x", """
            {"number":12,"state":"OPEN","isDraft":false,"url":"https://example.test/pr/12","title":"  Fix login bug  "}
        """.trimIndent()))

        assertEquals("/repo/.sonderr/worktrees/feature-x", pull.path)
    ...
    fun `worktree transfer round trips changes without touching the source`() = runBlocking {
        ...
        // Baseline the source status after worktree creation (which adds .sonderr bookkeeping) so the
    ...
    fun `moveToWorktree rolls back the created worktree when a later stage throws`() = runBlocking {
        ...
        assertFalse(
            Files.exists(repo.resolve(".sonderr").resolve("worktrees").resolve("feature-x")),
    ...
    private fun initRepo() {
        ...
        git(repo, "config", "user.name", "Sonderr Test")
    ...
    private fun originWith(pull: Int, head: String): Path {
        ...
        git(remote, "config", "user.name", "Sonderr Test")
