...
class SonderrBackendTelemetry(
    private val log: SonderrLog = SonderrLog.create(SonderrBackendTelemetry::class.java),
) {
    companion object {
    ...
    private fun base(): Map<String, JsonPrimitive> =
        SonderrLog.payload(log).mapValues { JsonPrimitive(it.value) }
}
