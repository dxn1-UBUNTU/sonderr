 ...
 * (`<repo>/packages/sonderr-jetbrains`) rather than the repository root; see [rebase].
 *
...
internal object WorktreeRunAdapter {
    ...
    private val LOG = SonderrLog.create(WorktreeRunAdapter::class.java)
     ...
     * at `<repo>/packages/sonderr-jetbrains` builds `<worktree>/packages/sonderr-jetbrains`, and a root
     ...
     * `<repo>/packages/sonderr-jetbrains` becomes `<worktree>/packages/sonderr-jetbrains`, which keeps
     ...
     *   `<repo>/.sonderr/worktrees/<name>` are never nested a second time
