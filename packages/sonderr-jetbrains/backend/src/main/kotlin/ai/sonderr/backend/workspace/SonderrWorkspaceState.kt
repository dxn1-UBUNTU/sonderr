 ...
 *
 * Only populated after [SonderrAppState.Ready][ai.sonderr.backend.app.SonderrAppState.Ready]
 * — the CLI server must be connected and global data loaded before
 * workspace data can be fetched.
 */
sealed class SonderrWorkspaceState {
    data object Pending : SonderrWorkspaceState()
    data class Loading(val progress: SonderrWorkspaceLoadProgress) : SonderrWorkspaceState()
    data class Ready(
        val providers: ProviderData,
        val agents: AgentData,
        val commands: List<CommandInfo>,
        val skills: List<SkillInfo>,
    ) : SonderrWorkspaceState()
    data class Unsupported(val reason: String) : SonderrWorkspaceState()
    data class Missing(val path: String) : SonderrWorkspaceState()
    data class Error(val message: String, val errors: List<LoadError> = emptyList()) : SonderrWorkspaceState()
}

/**
 * Tracks which workspace data fetches have completed during
 * the [SonderrWorkspaceState.Loading] phase.
 */
data class SonderrWorkspaceLoadProgress(
