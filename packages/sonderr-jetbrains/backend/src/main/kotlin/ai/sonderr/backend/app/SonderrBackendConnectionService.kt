 ...
 *
 * Concurrency is handled by the owning [SonderrBackendAppService] — `connect`,
 * `restart`, and `reinstall` are called under its mutex. Internal reconnect
 * attempts delegate back to the owner via [onReconnect].
 *
 * Not a service — owned and instantiated by [SonderrBackendAppService].
 */
class SonderrConnectionService(
  ...
  private val appLoadTimeoutMs: Long,
    ...
    constructor(
      ...
      server: CliServer,
      onReconnect: () -> Unit,
    ) : this(cs, server, onReconnect, SonderrLog.create(SonderrConnectionService::class.java), 30_000L)
     ...
     *
     * Called under [SonderrBackendAppService]'s mutex — no internal guard needed.
     */
    suspend fun connect() {
        open()
    }
     ...
     *
     * Called under [SonderrBackendAppService]'s mutex.
     */
    suspend fun restart() {
    ...
    private suspend fun open() {
        ...
        val ac = SonderrBackendHttpClients.api(password)
        val lc = SonderrBackendHttpClients.appLoad(password, appLoadTimeoutMs)
        val hc = SonderrBackendHttpClients.health(password)
        apiClient = ac
        appLoadClient = lc
        healthClient = hc
    ...
    private val listener = object : EventSourceListener() {
        ...
        override fun onEvent(src: EventSource, id: String?, type: String?, data: String) {
            ...
            synchronized(lock) {
                ...
                lastEvent.set(System.currentTimeMillis())
                val kind = type ?: SonderrCliDataParser.extractEventType(data)
     ...
     * goes through [SonderrBackendAppService]'s mutex for a full restart.
     */
    ...
    private fun close() {
        api = null
        appLoadApi = null
        apiClient?.let { SonderrBackendHttpClients.shutdown(it) }
        apiClient = null
        appLoadClient?.let { SonderrBackendHttpClients.shutdown(it) }
        appLoadClient = null
        healthClient?.let { SonderrBackendHttpClients.shutdown(it) }
        healthClient = null
    }
    ...
    fun dispose() {
        ...
        log.info("SonderrConnectionService disposed")
    }
}
