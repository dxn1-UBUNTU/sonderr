...
import ai.sonderr.backend.cli.CliServer
import ai.sonderr.backend.cli.SonderrBackendCliManager
import ai.sonderr.backend.cli.SonderrCliDataParser
import ai.sonderr.backend.migration.SonderrBackendLegacyMigrationStoreService
...
import ai.sonderr.backend.workspace.SonderrBackendWorkspaceManager
import ai.sonderr.jetbrains.api.client.DefaultApi
import ai.sonderr.jetbrains.api.infrastructure.ClientError
import ai.sonderr.jetbrains.api.infrastructure.ClientException
import ai.sonderr.jetbrains.api.infrastructure.ServerError
import ai.sonderr.jetbrains.api.infrastructure.ServerException
import ai.sonderr.jetbrains.api.model.ConfigWarnings200ResponseInner
...
import ai.sonderr.jetbrains.api.model.ProviderOauthAuthorizeRequest
import ai.sonderr.jetbrains.api.model.ProviderOauthCallbackRequest
...
class SonderrBackendAppService private constructor(
  ...
  private val loadTimeoutMs: Long,
    ...
    constructor(cs: CoroutineScope) : this(
        cs,
        SonderrBackendCliManager(),
      SonderrLog.create(SonderrBackendAppService::class.java),
        APP_LOAD_TIMEOUT_MS,
    )
    ...
    companion object {
        ...
        ) = SonderrBackendAppService(cs, server, log, loadTimeoutMs)
    }

    private val mutex = Mutex()
    private val connection = SonderrConnectionService(cs, server, onReconnect = {
        cs.launch { reconnect() }
    ...
    private val _appState = MutableStateFlow<SonderrAppState>(SonderrAppState.Disconnected)
    val appState: StateFlow<SonderrAppState> = _appState.asStateFlow()
    ...
    val sessions = SonderrBackendSessionManager(cs, log)
    val chat = SonderrBackendChatManager(cs, log)
    val activity = SonderrBackendActivityManager(cs, log)
    val models = SonderrBackendModelStateManager(log)
    val workspaces = SonderrBackendWorkspaceManager(cs, sessions, log)
    @Volatile var profile: SonderrProfile200Response? = null
        private set

    @Volatile var config: ConfigDto? = null
        private set

    @Volatile var notifications: List<SonderrNotifications200ResponseInner> = emptyList()
        private set
    ...
    suspend fun connect() {
        mutex.withLock {
            ...
            if (current is SonderrAppState.Ready || current is SonderrAppState.Downloading || current is SonderrAppState.Connecting || current is SonderrAppState.Loading || current is SonderrAppState.MigrationRequired) return
            ensureWatcher()
            connection.connect()
        }
    }
    ...
    suspend fun retry() {
        mutex.withLock {
            when (val current = _appState.value) {
                SonderrAppState.Disconnected -> {
                    ensureWatcher()
                    connection.connect()
                }
                is SonderrAppState.Downloading,
                SonderrAppState.Connecting,
                is SonderrAppState.Loading -> Unit
                is SonderrAppState.MigrationRequired -> {
                    log.info("retry: rerunning migration detection")
                    load()
                }
                is SonderrAppState.Ready -> {
                    if (current.data.warnings.isEmpty()) return
                    log.info("retry: refreshing config warnings")
                    refreshConfigState()
                    val next = _appState.value
                    val warns = (next as? SonderrAppState.Ready)?.data?.warnings
                    if (next is SonderrAppState.Ready && warns.isNullOrEmpty()) return
                    restartConnection("warnings remained after refresh")
                }
                is SonderrAppState.Error -> {
    ...
    fun requireReady() {
        when (_appState.value) {
            is SonderrAppState.Ready -> return
            is SonderrAppState.MigrationRequired -> throw IllegalStateException("Migration required")
            else -> throw IllegalStateException("Sonderr backend is not ready")
        }
    }
    ...
    suspend fun awaitReady(timeoutMs: Long = READY_TIMEOUT_MS) {
        when (_appState.value) {
            ...
            is SonderrAppState.MigrationRequired -> throw IllegalStateException("Migration required")
            is SonderrAppState.Downloading,
            is SonderrAppState.Loading,
            SonderrAppState.Connecting -> {
                val state = withTimeoutOrNull(timeoutMs) {
                    appState.first { it !is SonderrAppState.Downloading && it !is SonderrAppState.Loading && it !is SonderrAppState.Connecting }
                }
                when (state) {
                    is SonderrAppState.Ready -> return
                    is SonderrAppState.MigrationRequired -> throw IllegalStateException("Migration required")
    ...
    suspend fun updateConfig(patch: ConfigPatchDto): SonderrAppState {
        val http = connection.apiClient ?: throw IllegalStateException("Not connected")
        val current = _appState.value as? SonderrAppState.Ready ?: throw IllegalStateException("Sonderr backend is not ready")
        val connected = connection.state.value as? ConnectionState.Connected
        val body = SonderrCliDataParser.buildConfigPatch(patch)
        ...
        if (cfg == null) {
            ...
            return (_appState.value as? SonderrAppState.Ready) ?: current
        }
        val warns = fetchWarnings()
        val state = _appState.value
        if (state is SonderrAppState.Ready && state.data === current.data && connection.state.value == connected) {
            config = cfg
    ...
    internal suspend fun resumeAfterMigration() {
        mutex.withLock {
            if (_appState.value !is SonderrAppState.MigrationRequired) return
    ...
    private suspend fun reconnect() {
        mutex.withLock {
            val current = _appState.value
            if (current is SonderrAppState.Ready || current is SonderrAppState.Downloading || current is SonderrAppState.Loading || current is SonderrAppState.MigrationRequired) {
    ...
    private fun ensureWatcher() {
        ...
        watcher = cs.launch {
            connection.state.collect { next ->
                ...
                when (next) {
                    ConnectionState.Disconnected -> _appState.value = SonderrAppState.Disconnected
                    is ConnectionState.Downloading -> _appState.value = SonderrAppState.Downloading(next.percent, next.version, next.platform)
                    ConnectionState.Connecting -> _appState.value = SonderrAppState.Connecting
     ...
     *
     * Progress is tracked via [LoadProgress] and emitted as [SonderrAppState.Loading].
     * On success, transitions to [SonderrAppState.Ready].
     * On failure of required data, transitions to [SonderrAppState.Error].
     */
    private fun load() {
        synchronized(loadLock) {
            ...
            loader = cs.launch {
                ...
                _appState.value = SonderrAppState.Loading(progress.get())
                ...
                if (migration != null) {
                    ...
                    warnings = emptyList()
                    _appState.value = SonderrAppState.MigrationRequired(migration)
                ...
                var prof: SonderrProfile200Response? = null
                var notifs: List<SonderrNotifications200ResponseInner> = emptyList()
                    ...
                    withTimeout(loadTimeoutMs) {
                        ...
                        launch {
                            ...
                            progress.updateAndGet { it.copy(profile = status) }
                                .also { _appState.value = SonderrAppState.Loading(it) }
                        }
                        launch {
    ...
    private fun captureLoad(event: String, start: Long, props: Map<String, String>) {
        ...
        cs.launch {
            runCatching {
                service<SonderrBackendTelemetry>().capture(
                    http,
                    port,
                    event,
    ...
    private fun setTelemetry(enabled: Boolean) {
        ...
        cs.launch {
            runCatching {
                service<SonderrBackendTelemetry>().setEnabled(http, port, enabled)
    ...
    private fun captureBackend(event: String, props: Map<String, String>) {
        ...
        cs.launch {
            runCatching {
                service<SonderrBackendTelemetry>().capture(http, port, event, props)
    ...
    private suspend fun detectMigration(): LegacyMigrationDetection? = withContext(Dispatchers.IO) {
        try {
            ...
            else SonderrBackendLegacyMigrationStoreService.status(log)
            ...
            val source = SonderrBackendLegacyMigrationStoreService.resolveSource(log, includeFile = migrationForceRequested)
            val store = source.store
            val detection = SonderrBackendMigrationManager(http, connection.port).detect(store)
     ...
     */
    private suspend fun fetchProfile(): FetchResult<SonderrProfile200Response?> {
        val client = connection.appLoadApi
            ?: return FetchResult.ok(null)
        return try {
            val response = client.sonderrProfile()
    ...
    private suspend fun fetchConfig(): FetchResult<ConfigDto> {
        ...
        return try {
            ...
            FetchResult.ok(SonderrCliDataParser.parseConfig(body))
        } catch (e: CancellationException) {
            throw e
        } catch (e: Exception) {
    ...
    private suspend fun fetchNotifications(): FetchResult<List<SonderrNotifications200ResponseInner>> {
        val client = connection.appLoadApi
            ?: return FetchResult.fail("notifications", detail = "Not connected")
        return try {
            FetchResult.ok(client.sonderrNotifications())
        } catch (e: Exception) {
    ...
    private suspend fun refreshConfigState() {
        val current = _appState.value as? SonderrAppState.Ready ?: return
        val connection = connection.state.value as? ConnectionState.Connected ?: return
        val cfg = fetchConfig().value ?: return
        val warns = fetchWarnings()
        val state = _appState.value
        if (state !is SonderrAppState.Ready || state.data !== current.data) return
    ...
    private fun setAppReady(data: AppData) {
        ...
        _appState.value = SonderrAppState.Ready(data, rev.incrementAndGet())
    }

    private fun setAppError(message: String, errors: List<LoadError>) {
        val state = SonderrAppState.Error(message, errors)
        warnAppError(state)
        _appState.value = state
    }

    private fun warnAppError(state: SonderrAppState.Error) {
    ...
    private suspend fun awaitLoadResult(prev: SonderrAppState): SonderrAppState {
        val next = appState.first { it !== prev }
        if (next !is SonderrAppState.Loading) return next
        return appState.first { it !is SonderrAppState.Loading }
    }
     ...
     *   Re-fetches config and updates [SonderrAppState.Ready] data in-place.
     *
                ...
                connection.events.collect { event ->
                    when (event.type) {
                        ...
                        "global.disposed" -> {
                            ...
                            val current = _appState.value
                            if (current is SonderrAppState.Ready) load()
                        }
    ...
    private fun reset() {
        ...
        notifications = emptyList()
        warnings = emptyList()
        _appState.value = SonderrAppState.Disconnected
    }
     ...
     * Updates the current [SonderrAppState.Ready] profile in-place if the app is ready.
     */
    suspend fun refreshProfile(): SonderrProfile200Response? {
        val result = fetchProfile()
        val fresh = result.value
        val current = _appState.value
        if (current is SonderrAppState.Ready) {
    ...
    /**
     * Start the Sonderr device auth login flow.
    ...
    suspend fun startLogin(directory: String?): DeviceAuthDto {
        ...
        val response = client.providerOauthAuthorize(providerID = "sonderr", directory = directory, providerOauthAuthorizeRequest = body)
    ...
    /**
     * Complete the Sonderr device auth login flow.
     ...
     */
    suspend fun completeLogin(directory: String?): SonderrProfile200Response? {
        val client = connection.api ?: throw IllegalStateException("Not connected")
        client.providerOauthCallback(providerID = "sonderr", directory = directory, providerOauthCallbackRequest = ProviderOauthCallbackRequest(method = 0.0))
    ...
    suspend fun logout(): Boolean {
        val client = connection.api ?: throw IllegalStateException("Not connected")
        val result = client.authRemove(providerID = "sonderr")
        val current = _appState.value
        if (current is SonderrAppState.Ready) {
            profile = null
     ...
     */
    suspend fun setOrganization(organizationId: String?): SonderrProfile200Response? {
        val http = connection.apiClient ?: throw IllegalStateException("Not connected")
        val body = JsonObject(
            mapOf("organizationId" to (organizationId?.let { JsonPrimitive(it) } ?: JsonNull)),
        ).toString()
        val request = Request.Builder()
            .url("http://127.0.0.1:$port/sonderr/organization")
            .header("Accept", "application/json")
 ...
 * [ConnectionState.Connected], or [ConnectionState.Error] while [SonderrAppState.MigrationRequired]
 ...
 */
internal fun preservesMigration(appState: SonderrAppState, next: ConnectionState): Boolean =
    appState is SonderrAppState.MigrationRequired &&
