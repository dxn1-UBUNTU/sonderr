package ai.sonderr.backend.provider
...
import ai.sonderr.backend.rpc.SonderrWorkspaceDtoMapper
...
internal class SonderrBackendProviderSettingsManager(
    private val app: SonderrBackendAppService,
) {
    companion object {
        private val LOG = SonderrLog.create(SonderrBackendProviderSettingsManager::class.java)
        private val JSON = "application/json".toMediaType()
        private val FETCH by lazy { SonderrBackendHttpClients.modelFetch() }
    ...
    suspend fun state(directory: String): ProviderSettingsDto {
        ...
        val providers = load("providers", errors) {
            SonderrCliDataParser.parseProviderSettingsProviders(get("/provider?directory=${enc(directory)}"))
        }
        val auth = load("provider_auth", errors) {
            SonderrCliDataParser.parseProviderAuth(get("/provider/auth?directory=${enc(directory)}"))
        } ?: emptyMap()
    ...
    suspend fun connect(input: ProviderConnectDto): ProviderActionResultDto {
        val body = SonderrCliDataParser.buildProviderAuthJson(input.key, input.metadata)
    ...
    suspend fun authorize(input: ProviderOAuthAuthorizeDto): ProviderOAuthReadyDto {
        val body = SonderrCliDataParser.buildProviderOAuthJson(input.method, input.inputs)
        val raw = post("/provider/${enc(input.providerId)}/oauth/authorize?directory=${enc(input.directory)}", body, OAUTH_CALL_TIMEOUT_SECONDS)
        val parsed = SonderrCliDataParser.parseOAuthReady(raw)
    ...
    suspend fun callback(input: ProviderOAuthCallbackDto): ProviderActionResultDto {
        val body = SonderrCliDataParser.buildProviderOAuthJson(input.method, code = input.code)
    ...
    suspend fun disconnect(input: ProviderDisconnectDto): ProviderActionResultDto {
        ...
        if (input.providerId == "sonderr") {
            return ProviderActionResultDto(current, error = "Sonderr Gateway cannot be disconnected from provider settings.")
        }
        ...
        if (cfg?.npm == "@ai-sdk/openai-compatible") {
            patch(input.directory, cfg.scope, SonderrCliDataParser.buildCustomProviderDeletePatch(input.providerId))
        ...
        if (provider?.source == "config") {
            ...
            patch(input.directory, scope, SonderrCliDataParser.buildDisabledProviderPatch(ids))
            dispose()
            return ProviderActionResultDto(state(input.directory))
        }
        deleteAuth(input.providerId)
    ...
    suspend fun enable(input: ProviderEnableDto): ProviderActionResultDto {
        ...
        scopes.distinct().forEach { scope ->
            patch(input.directory, scope, SonderrCliDataParser.buildDisabledProviderPatch(disabledFor(current, scope).filter { it != input.providerId }))
        }
        dispose()
        return ProviderActionResultDto(state(input.directory))
    }
    ...
    suspend fun saveCustom(input: CustomProviderSaveDto): ProviderActionResultDto {
        ...
        patch(SonderrCliDataParser.buildCustomProviderPatch(input))
        if (input.envVar.isNullOrBlank()) {
            val key = input.apiKey?.takeIf { it.isNotBlank() }
            if (key != null) put("/auth/${enc(input.id)}", SonderrCliDataParser.buildProviderAuthJson(key, emptyMap()))
        } else {
            deleteAuth(input.id)
        }
        dispose()
    ...
    suspend fun fetch(input: CustomModelFetchDto): CustomModelFetchResultDto {
        ...
        return try {
            ...
            CustomModelFetchResultDto(SonderrCliDataParser.parseModelIds(raw))
        } catch (e: Exception) {
    ...
    private suspend fun <T> load(resource: String, errors: MutableList<LoadErrorDto>, block: suspend () -> T): T? {
        ...
        return try {
            ...
            errors.add(SonderrWorkspaceDtoMapper.error(LoadError(resource = resource, detail = e.message)))
            null
        }
    }
    ...
    private fun parsed(raw: String): ParsedConfig {
        val result = SonderrCliDataParser.parseProviderConfig(raw)
    ...
    private suspend fun request(request: Request, timeoutSeconds: Long = CALL_TIMEOUT_SECONDS): String {
        ...
        val http = app.http?.let { SonderrBackendHttpClients.bounded(it, timeoutSeconds) }
            ?: throw IllegalStateException("Sonderr HTTP client is unavailable")
