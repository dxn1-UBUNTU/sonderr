 ...
 */
internal class SonderrProcessJob private constructor(
    ...
    companion object {
        ...
        private val lib: Kernel32Job? by lazy {
            runCatching { Native.load("kernel32", Kernel32Job::class.java) }
                .getOrElse { SonderrLog.create(SonderrProcessJob::class.java).warn("Native.load(kernel32) failed", it); null }
        }
         ...
         */
        fun assign(pid: Long, log: SonderrLog): SonderrProcessJob? {
        ...
        private fun setup(pid: Long, log: SonderrLog): SonderrProcessJob? {
        ...
        private fun create(k: Kernel32Job, pid: Long, log: SonderrLog): SonderrProcessJob? {
            ...
            try {
                ...
                return SonderrProcessJob(k, job, pid, log)
            } finally {
                if (!owned) closeHandle(k, job, "job (rollback)", pid, log)
            }
        }

        private fun closeHandle(k: Kernel32Job, handle: Pointer, label: String, pid: Long, log: SonderrLog) {
