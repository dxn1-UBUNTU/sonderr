 ...
 * [SonderrBackendWorkspaceManager] for each directory. Receives a
 * pre-connected [DefaultApi] — no null checks needed.
 *
 * Session operations delegate to [SonderrBackendSessionManager] with
 * this workspace's [directory], so the frontend only needs one
 * object per directory.
 */
class SonderrBackendWorkspace(
    ...
    private val sessions: SonderrBackendSessionManager,
    ...
    private val _state = MutableStateFlow<SonderrWorkspaceState>(SonderrWorkspaceState.Pending)
    val state: StateFlow<SonderrWorkspaceState> = _state.asStateFlow()
    ...
    fun load() {
        synchronized(loadLock) {
            ...
            loader = cs.launch {
                ...
                if (reason != null) {
                    log.info("Workspace directory is unsupported for host-side Sonderr runtime: $reason [$directory]")
                    _state.value = SonderrWorkspaceState.Unsupported(reason)
                    return@launch
                }
                if (!Files.isDirectory(Path.of(directory))) {
                    log.info("Workspace directory is missing: $directory")
                    _state.value = SonderrWorkspaceState.Missing(directory)
                    return@launch
                }
                val progress = AtomicReference(SonderrWorkspaceLoadProgress())
                _state.value = SonderrWorkspaceState.Loading(progress.get())
                ...
                try {
                        ...
                        launch {
                            ...
                            if (result.value != null) {
                                ...
                                progress.updateAndGet { it.copy(providers = true) }
                                    .also { _state.value = SonderrWorkspaceState.Loading(it) }
                            } else {
                    ...
                    _state.value = SonderrWorkspaceState.Ready(
                        providers = prov!!,
                        agents = ag!!,
                        commands = cmd!!,
                        skills = sk!!,
    ...
    fun stop() {
        ...
        _state.value = SonderrWorkspaceState.Pending
    }
    ...
    private suspend fun fetchProviders(): FetchResult<ProviderData> = withContext(Dispatchers.IO) {
        try {
            FetchResult.ok(SonderrCliDataParser.parseProviders(fetch("/provider?directory=${encode(directory)}")))
        } catch (e: CancellationException) {
            throw e
        } catch (e: Exception) {
    ...
    private suspend fun fetchCommands(): FetchResult<List<CommandInfo>> = withContext(Dispatchers.IO) {
        try {
            FetchResult.ok(SonderrCliDataParser.parseCommands(fetch("/command?directory=${encode(directory)}")))
        } catch (e: CancellationException) {
            throw e
        } catch (e: Exception) {
    ...
    private fun setWorkspaceError(message: String, errors: List<LoadError>) {
        ...
        _state.value = SonderrWorkspaceState.Error(message, errors)
    }
