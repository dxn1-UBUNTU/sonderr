...
/**
 * Backend adapter for writing migrated data to the CLI (sonderr serve).
 *
...
interface LegacyMigrationBackend {
    ...
    /** POST /sonderr/session-import/project — returns the project ID */
    fun importProject(project: JsonObject): String

    /** POST /sonderr/session-import/session */
    fun importSession(session: JsonObject): LegacyImportResult

    /** POST /sonderr/session-import/message */
    fun importMessage(message: JsonObject)

    /** POST /sonderr/session-import/part */
