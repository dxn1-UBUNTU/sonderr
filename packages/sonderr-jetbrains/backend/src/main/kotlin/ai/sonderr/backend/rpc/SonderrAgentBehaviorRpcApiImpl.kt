...
import ai.sonderr.backend.cli.SonderrClaudeCompatSettings
...
import ai.sonderr.jetbrains.api.model.AgentBuilderSaveRequest
...
import ai.sonderr.rpc.dto.PermissionRuleItemDto
...
class SonderrAgentBehaviorRpcApiImpl(private val backend: SonderrBackendAppService? = null) : SonderrAgentBehaviorRpcApi {
    companion object {
        private val LOG = SonderrLog.create(SonderrAgentBehaviorRpcApiImpl::class.java)
    ...
    private val app: SonderrBackendAppService get() = backend ?: service()
    ...
    override suspend fun agents(directory: String): List<AgentDetailDto> {
        ...
        val removable = SonderrCliDataParser.parseAgentRemovable(request(directory, "/agent", null))
    ...
    override suspend fun skills(directory: String): List<SkillDto> {
        val items = SonderrCliDataParser.parseAgentBehaviorSkills(request(directory, "/skill", null))
    ...
    override suspend fun removeSkill(directory: String, location: String): Boolean =
        post(directory, "/sonderr/skill/remove", JsonObject(mapOf("location" to JsonPrimitive(location))))
    ...
    override suspend fun removeAgent(directory: String, name: String): Boolean =
        post(directory, "/sonderr/agent/remove", JsonObject(mapOf("name" to JsonPrimitive(name))))
    ...
    override suspend fun commands(directory: String) = SonderrCliDataParser.parseAgentBehaviorCommands(request(directory, "/command", null))

    override suspend fun commandFiles(directory: String): List<CommandFileDto> =
        SonderrCliDataParser.parseAgentBehaviorCommandFiles(request(directory, "/sonderr/command/files", null))

    override suspend fun removeCommand(directory: String, location: String): Boolean =
        post(directory, "/sonderr/command/remove", JsonObject(mapOf("location" to JsonPrimitive(location))))
    ...
    override suspend fun mcpStatus(directory: String) = SonderrCliDataParser.parseMcpStatus(request(directory, "/mcp", null)).also { items ->
    ...
    override suspend fun mcpConfig(directory: String): Map<String, McpServerConfigDto> {
        ...
        val workspace = if (directory.isBlank()) emptyMap() else try {
            SonderrCliDataParser.parseConfig(request(directory, "/config", null)).mcp
        } catch (e: Exception) {
    ...
    override suspend fun saveMcp(directory: String, name: String, scope: String, config: McpConfigDto?): Boolean {
        ...
        if (scope == "workspace") {
            patchConfig("/config?directory=${encode(directory)}", SonderrCliDataParser.buildConfigPatch(patch))
        } else {
    ...
    override suspend fun claudeCodeCompat(): Boolean = SonderrClaudeCompatSettings.get()

    override suspend fun setClaudeCodeCompat(value: Boolean): Boolean {
        SonderrClaudeCompatSettings.set(value)
        app.restart()
        return value
    }
    ...
    private suspend fun commandRoots(directory: String): Set<Path> = buildSet {
        ...
        val config = paths?.let(SonderrCliDataParser::parsePathConfig)
        if (config != null) addConfigCommandRoots(this, config)
        val home = paths?.let(SonderrCliDataParser::parsePathHome)
    ...
    private fun addProjectCommandRoots(roots: MutableSet<Path>, dir: String) {
        ...
        for (cfg in listOf(".sonderr", ".sonderr")) {
    ...
    private fun addHomeCommandRoots(roots: MutableSet<Path>, home: String) {
        ...
        for (cfg in listOf(".sonderr", ".sonderr")) addConfigCommandRoots(roots, base.resolve(cfg).toString())
    }
    ...
    private fun urlCached(path: Path): Boolean {
        return cacheRoots().any { root -> path.startsWith(root.resolve("sonderr").resolve("skills").normalize()) }
    }
