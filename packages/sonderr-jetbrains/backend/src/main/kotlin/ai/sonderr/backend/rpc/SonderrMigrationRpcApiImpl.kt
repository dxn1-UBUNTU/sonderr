...
import ai.sonderr.backend.migration.materializeLegacyMigrationSource
...
import ai.sonderr.backend.app.SonderrBackendMigrationManager
...
class SonderrMigrationRpcApiImpl : SonderrMigrationRpcApi {

    companion object {
        private val LOG = SonderrLog.create(SonderrMigrationRpcApiImpl::class.java)
    }

    private val app: SonderrBackendAppService get() = service()
    private val storeService: SonderrBackendLegacyMigrationStoreService get() = service()

    private fun manager(): SonderrBackendMigrationManager {
        val http = app.http ?: throw IllegalStateException("Not connected")
        val port = app.port
        return SonderrBackendMigrationManager(http, port)
    }
        ...
        return channelFlow {
            withContext(Dispatchers.IO) {
                ...
                val sink = object : LegacyMigrationSink {
                    override fun item(progress: ai.sonderr.backend.migration.LegacyMigrationItemProgress) {
                    ...
                    override fun session(progress: ai.sonderr.backend.migration.LegacyMigrationSessionProgress) {
