...
import ai.sonderr.backend.dev.SonderrDevMode
...
/**
 * Manages the Sonderr CLI binary lifecycle.
 *
 * Downloads the pinned CLI into IntelliJ's system directory,
 * spawns `sonderr serve --port 0`, and exposes the result as [State].
 *
 * Concurrency is handled by the owning [SonderrBackendAppService] — all public
 * methods except [exited] are called under its mutex. [exited] is called from
 * [SonderrConnectionService]'s IO dispatcher and is thread-safe via the stale-ref
 * guard and volatile [process] field.
 */
class SonderrBackendCliManager(
    private val log: SonderrLog = SonderrLog.create(SonderrBackendCliManager::class.java),
    ...
    @Volatile
    private var job: SonderrProcessJob? = null
    ...
    private suspend fun resolveCli(onProgress: (CliDownload) -> Unit): File {
        ...
        forceExtract = false
        val version = SonderrProps.cliVersion()
        val platform = SonderrCliPlatform.current()
        if (SonderrRepoCli.available()) {
            if (force) log.info("Force re-extracting bundled CLI $version")
            log.info("Sonderr CLI mode: BUNDLED — using CLI $version ($platform) shipped in the plugin; no download needed")
            val cli = SonderrRepoCli.extract(force)
            onProgress(CliDownload(100, version, platform))
            return cli
        }
        if (force) log.info("Force re-downloading CLI $version")
        log.info("Sonderr CLI mode: DOWNLOAD — resolving CLI $version ($platform) from the GitHub release")
        return SonderrCliDownloader(log = log).resolve(version, force, onProgress)
    }
    ...
    internal fun buildEnv(pwd: String, base: Map<String, String> = EnvironmentUtil.getEnvironmentMap()): Map<String, String> =
        buildSonderrCliEnv(pwd, base, log)
    ...
    private suspend fun spawn(cli: File, start: Long): CliServer.State =
        withContext(Dispatchers.IO) {
            ...
            log.info("CLI env: SONDERR_CLIENT=jetbrains SONDERR_PLATFORM=jetbrains SONDERR_APP_NAME=sonderr-code")
            val proc = try {
                builder.start()
            } catch (e: Exception) {
            ...
            // can never be orphaned. Null on other platforms / when unavailable — see SonderrProcessJob.
            val jobHandle = SonderrProcessJob.assign(proc.pid(), log)
            ...
            }, "sonderr-cli-stderr").apply { isDaemon = true; start() }
            this@SonderrBackendCliManager.stderr = err
    ...
    private fun clearJob(): SonderrProcessJob? = synchronized(lock) {
        val current = job
        job = null
        current
    }
 ...
 */
private fun confirmKilled(proc: Process, kids: List<ProcessHandle>, log: SonderrLog, timeoutSeconds: Long) {
    val parentExited = proc.waitFor(timeoutSeconds, TimeUnit.SECONDS)
 ...
 * order deadlocked IDE shutdown; the order is locked by SonderrBackendCliShutdownTest. Never waits, so
...
internal fun closeStreams(proc: Process, log: SonderrLog) {
...
internal fun startupDiagnostics(cli: File, env: Map<String, String>, log: SonderrLog): String {
    val home = System.getProperty("user.home").orEmpty()
    val profile = EnvironmentUtil.getValue("USERPROFILE").orEmpty()
    val data = env["XDG_DATA_HOME"] ?: home.takeIf { it.isNotBlank() }?.let { File(it, ".local/share/sonderr").absolutePath }.orEmpty()
    ...
    if (data.isNotBlank() && remote(data)) {
        lines += "warning: Sonderr CLI data dir appears to be on a non-local drive (${root(data)}); SQLite WAL may hang. Set XDG_DATA_HOME/XDG_STATE_HOME/XDG_CONFIG_HOME/XDG_CACHE_HOME to a local disk."
    }
    val text = lines.joinToString("\n")
    log.info("CLI startup diagnostics:\n$text")
    if (data.isNotBlank() && remote(data)) {
        log.warn("Sonderr CLI data dir appears to be on a non-local drive (${root(data)}); SQLite WAL may hang. Set XDG_DATA_HOME/XDG_STATE_HOME/XDG_CONFIG_HOME/XDG_CACHE_HOME to a local disk.")
    }
    return text
}
...
internal suspend fun awaitReady(
    ...
    onTimeout: () -> Unit,
    diagnostics: () -> String,
    log: SonderrLog = SonderrLog.create(SonderrBackendCliManager::class.java),
    ...
    }, "sonderr-cli-stdout").apply { isDaemon = true; start() }
    onThread(thread)
...
// Must be called from a background thread — devStorageEnv() performs blocking I/O (mkdirs).
internal fun buildSonderrCliEnv(
    pwd: String,
    base: Map<String, String> = EnvironmentUtil.getEnvironmentMap(),
    log: SonderrLog = SonderrLog.create(SonderrBackendCliManager::class.java),
): Map<String, String> = buildMap {
    putAll(base)
    put("SONDERR_SERVER_PASSWORD", pwd)
    ...
    put("SONDERR_PARENT_PID", ProcessHandle.current().pid().toString())
    put("SONDERR_CLIENT", "jetbrains")
    put("SONDERR_ENABLE_QUESTION_TOOL", "true")
    put("SONDERR_PLATFORM", "jetbrains")
    put("SONDERR_APP_NAME", "sonderr-code")
    put("SONDERR_TELEMETRY_LEVEL", if (SonderrDevMode.enabled()) "off" else "all")
    if (!SonderrClaudeCompatSettings.get()) put("SONDERR_DISABLE_CLAUDE_CODE", "true")
    put("SONDERR_FEATURE", "jetbrains-plugin")
    putIfAbsent("SONDERR_CONFIG_CONTENT", DEFAULT_CONFIG)
    ideEnv(log).forEach { entry -> put(entry.key, entry.value) }
    devStorageEnv(log)?.forEach { entry -> put(entry.key, entry.value) }
}

private fun ideEnv(log: SonderrLog): Map<String, String> = buildMap {
    runCatching {
        val info = ApplicationInfo.getInstance()
        val name = info.fullApplicationName
        val build = info.build.asString()
        put("SONDERR_EDITOR_NAME", name)
        put("SONDERR_EDITOR_NAME", "$name $build")
    }.onFailure { log.info("Could not read ApplicationInfo: ${it.message}") }

    runCatching {
        val version = SonderrPlugin.version()
        if (version != null) put("SONDERR_APP_VERSION", version)
    }.onFailure { log.info("Could not read plugin version: ${it.message}") }

    runCatching {
        put("SONDERR_MACHINE_ID", machineId())
    }.onFailure { log.info("Could not read machine ID: ${it.message}") }
}

private fun machineId(): String {
    val file = File(PathManager.getSystemPath(), "sonderr/machine-id")
...
private fun devStorageEnv(log: SonderrLog): Map<String, String>? {
    val enabled = System.getProperty("sonderr.dev.storage.isolated", "false").toBoolean()
    if (!enabled) return null
    val root = System.getProperty("sonderr.dev.worktree.root") ?: run {
        log.warn("sonderr.dev.storage.isolated=true but sonderr.dev.worktree.root is not set; skipping dev storage isolation")
        return null
    }
    val dev = File(root, ".sonderr-dev")
