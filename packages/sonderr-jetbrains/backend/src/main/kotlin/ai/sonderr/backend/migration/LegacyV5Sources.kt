...
class LegacyV5Sources(
    ...
    private val root = File(home, ".sonderr")
    private val storage = File(root, "globalStorage")
    private val scoped = File(storage, "sonderr code.sonderr-code")

    fun secretsJson(): String? = File(root, "secrets.json").text("secrets")
    fun globalStateXml(): String? = File(config, "options/sonderr-extension-storage.xml").text("globalStateXml")
