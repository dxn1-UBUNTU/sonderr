...
import ai.sonderr.jetbrains.api.model.GlobalSession
import ai.sonderr.jetbrains.api.model.SessionStatus
 ...
 *
 * **Not an IntelliJ service** — owned by [SonderrBackendAppService] which
 * calls [start] after the CLI server reaches [SonderrAppState.Ready] and
 ...
 *
 * All raw JSON parsing is delegated to [SonderrCliDataParser].
 */
class SonderrBackendSessionManager(
        ...
        watcher = cs.launch {
            events.collect { event ->
                if (event.type == "session.status") {
                    val pair = SonderrCliDataParser.parseSessionStatus(event.data)
                    if (pair != null) {
                ...
                SonderrCliDataParser.parseSessionChange(event.type, event.data)?.let { track(it) }
            }
        }
     ...
     * [SonderrBackendActivityManager] resolve a directory for a session this frame never listed, so
    ...
    fun create(dir: String): SessionDto {
        ...
        h.newCall(request).execute().use { response ->
            ...
            val dto = SonderrCliDataParser.parseSession(raw!!)
     ...
     * (see packages/cli/src/sonderr/server/httpapi/session-fork.ts and fork-routing.ts).
     */
     ...
     *
     * Raw HTTP for the same reason as [fork]. The CLI requires Sonderr credentials and refuses when
    ...
    fun cloudSessions(dir: String, cursor: String?, limit: Int, gitUrl: String?): CloudSessionListDto {
        ...
        val path = "$url/sonderr/cloud-sessions?$params"
        ...
        h.newCall(request).execute().use { response ->
            ...
            return SonderrCliDataParser.parseCloudSessions(raw)
        }
    }
    ...
    fun importCloudSession(id: String, dir: String): SessionDto {
        ...
        val request = Request.Builder()
            .url("$url/sonderr/cloud/session/import?directory=${encode(dir)}")
    ...
    private fun dto(s: ai.sonderr.jetbrains.api.model.Session) = dto(
        id = s.id,
        project = s.projectID,
        dir = s.directory,
        parent = s.parentID,
    ...
    private fun dto(s: ai.sonderr.jetbrains.api.model.Session1) = dto(
        id = s.id,
        project = s.projectID,
        dir = s.directory,
        parent = s.parentID,
    ...
    private fun revertDto(s: Any?) = when (s) {
        null -> null
        is ai.sonderr.jetbrains.api.model.SessionRevert -> revertDto(s.messageID, s.partID, s.snapshot, s.diff)
        else -> runCatching {
            val cls = s.javaClass
