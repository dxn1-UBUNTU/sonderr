...
/**
 * Manages [SonderrBackendWorkspace] instances by directory path.
 *
 ...
 */
class SonderrBackendWorkspaceManager(
    private val cs: CoroutineScope,
    private val sessions: SonderrBackendSessionManager,
    private val log: SonderrLog,
) {
    private val workspaces = ConcurrentHashMap<String, SonderrBackendWorkspace>()
     ...
     * Called by [SonderrBackendAppService] after [SonderrAppState.Ready].
    ...
    /**
     * Deactivate all workspaces. Called by [SonderrBackendAppService] on disconnect.
     */
    fun stop() {
     ...
     */
    fun get(dir: String): SonderrBackendWorkspace {
        ...
        return workspaces.computeIfAbsent(dir) { d ->
            ...
            SonderrBackendWorkspace(d, cs, client, http, port, ev, sessions, log).also { it.load() }
        }
    }
