...
import ai.sonderr.backend.rpc.readWorktreeState
...
class WorktreeRunManager internal constructor(
    ...
    companion object {
        private val LOG = SonderrLog.create(WorktreeRunManager::class.java)
    }
    ...
    private fun key(root: String, repo: String, worktree: String): Key = Key("sonderr.build:${relative(root, repo)}", worktree)
    ...
    private suspend fun label(repo: String, worktree: String): String = withContext(Dispatchers.IO) {
        val store = Path.of(repo).normalize().resolve(".sonderr").resolve("worktree-names.json")
