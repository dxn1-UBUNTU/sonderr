...
if grep -Eq '^[[:space:]]*sonderr\.cli\.pinned[[:space:]]*=[[:space:]]*false[[:space:]]*$' "$plugin/gradle.properties"; then
  echo "sonderr.cli.pinned=false is a dev-only mode and cannot be released. Set sonderr.cli.pinned=true before building a version." >&2
  exit 1
fi
...
./gradlew buildPlugin -Pproduction=true -Psonderr.version="$version" -Psonderr.channel=eap

if [[ "$sign" == "1" ]]; then
  ./gradlew signPlugin -Pproduction=true -Psonderr.version="$version" -Psonderr.channel=eap
fi
...
if [[ "$skip_verification" == "1" ]]; then
  ...
  if [[ "$sign" == "1" ]]; then
    ./gradlew verifyPluginSignature -Pproduction=true -Psonderr.version="$version" -Psonderr.channel=eap
  fi
  ./gradlew verifyPlugin -Pproduction=true -Psonderr.version="$version" -Psonderr.channel=eap
fi
