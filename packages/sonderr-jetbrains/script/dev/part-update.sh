...
usage() {
  ...
  printf '       %s <sonderr-log> <session-id>\n' "$0" >&2
}
...
resolve() {
  case "$target" in
    client|frontend)
      for file in \
        "$plugin/.intellijPlatform/sandbox/sonderr.jetbrains/sonderr-frontend/sonderr-dev.log" \
    ...
    backend)
      for file in \
        "$plugin/.intellijPlatform/sandbox/sonderr.jetbrains/sonderr-backend/sonderr-dev.log" \
...

{
  sub(/\r$/, "")
  cls = logger($0)
  if ((mode == "client" || mode == "frontend") && cls !~ /SonderrSessionService$/) next
  if (mode == "backend" && cls !~ /SonderrBackendChatManager$/) next
