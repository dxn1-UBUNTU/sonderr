...
- Confirm `ai.sonderr.jetbrains` is the final permanent Marketplace plugin ID before the first upload.
...
- Confirm the plugin is owned by the correct Sonderr vendor or organization.
...
- Confirm `SONDERR_MAINTAINER_APP_ID` and `SONDERR_MAINTAINER_APP_SECRET` are available to create release PRs and immediate release tags.
...
- Review and edit `packages/sonderr-jetbrains/CHANGELOG.md` in the generated release PR.
...
- Confirm the follow-up `publish-jetbrains-bundled` workflow completes and attaches `sonderr-code-x.y.z-rc.n-bundled.zip` to the prerelease.
...
- Watch the `publish-jetbrains` workflow.
- Confirm the follow-up `publish-jetbrains-bundled` workflow completes, attaches `sonderr-code-x.y.z-bundled.zip`, and updates `https://kilo-org.github.io/sonderr/jetbrains/updatePlugins.xml`.
