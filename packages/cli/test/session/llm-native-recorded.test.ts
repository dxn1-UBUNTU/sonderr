...
import { HttpRecorder } from "@sonderr/http-recorder"
import { HttpRecorderInternal } from "@sonderr/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@sonderr/llm"
import { RequestExecutor } from "@sonderr/llm/route"
...
const SONDERR_FIXTURES_DIR = path.join(FIXTURES_DIR, "sonderr") // sonderr_change

const zenURL = (connection: string) => `https://console.sonderr.ai/proxy/connections/${connection}/v1` // sonderr_change
...
const cloneModel = (model: ModelsDev.Provider["models"][string]) => {
  ...
  if (typeof experimental === "boolean") {
    ...
    return cloned as NonNullable<NonNullable<NonNullable<ConfigV1.Info["provider"]>[string]>["models"]>[string] // sonderr_change
  }
  ...
  return rest as NonNullable<NonNullable<NonNullable<ConfigV1.Info["provider"]>[string]>["models"]>[string] // sonderr_change
}
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.SONDERR_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["sonderr", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("SONDERR_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("SONDERR_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["sonderr", "native", "oauth", "tool-loop"],
  ...
  {
    id: "sonderr-proxy",
    name: "Sonderr proxy", // sonderr_change
    providerID: ProviderV2.ID.sonderr,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["sonderr", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.SONDERR_RECORD_CONSOLE_TOKEN && process.env.SONDERR_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.sonderr,
        name: "Sonderr Zen",
        env: ["SONDERR_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.SONDERR_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.SONDERR_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.SONDERR_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["sonderr", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("SONDERR_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("SONDERR_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("SONDERR_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const canRun = (scenario: RecordedScenario) =>
  ...
  shouldRecord
    ? scenario.canRecord()
    : HttpRecorderInternal.hasCassetteSync(scenario.cassette, { directory: SONDERR_FIXTURES_DIR })
  // sonderr_change end

const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set SONDERR_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the sonderr LLM stack remain real.
  const metadata = {
  ...
  const recordedHttp = shouldRecord
    ? HttpRecorderInternal.cassetteLayer(scenario.cassette, {
        directory: SONDERR_FIXTURES_DIR, // sonderr_change
        mode: "record",
        metadata,
        redactor: HttpRecorderInternal.Redactor.make(redact),
      })
    : HttpRecorder.http(scenario.cassette, { directory: SONDERR_FIXTURES_DIR, metadata, redact }) // sonderr_change
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "sonderr.json"),
      JSON.stringify({ $schema: "https://app.kilo.ai/config.json", ...scenario.config(model) }), // sonderr_change
    ),
  )
