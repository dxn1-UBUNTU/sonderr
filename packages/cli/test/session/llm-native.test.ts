...
import { LLMEvent, ToolFailure } from "@sonderr/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@sonderr/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("sonderr") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("sonderr") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
    ...
    ).toEqual({ type: "unsupported", reason: "provider is not openai, sonderr, or anthropic" })
    expect(
      LLMNativeRuntime.status({
        model: baseModel,
  ...
  test("prefers console provider api key over stored sonderr auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("sonderr") },
        provider: {
          ...providerInfo,
          id: ProviderV2.ID.make("sonderr"),
