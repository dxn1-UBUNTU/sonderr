...
import { legacyReviewMessage } from "../../src/sonderr/review/command"
...
import { SessionMessageTable } from "@sonderr/core/session/sql"
...
import { SessionProjector } from "@sonderr/core/session/projector" // sonderr_change
import { SessionPrompt } from "../../src/session/prompt"
import { SessionRevert } from "../../src/session/revert"
import { SessionRunState } from "../../src/session/run-state"
import { SonderrSession } from "../../src/sonderr/session"
// sonderr_change start - Item 14 cancel→reprompt proof helpers
...
// sonderr_change start - one compiled graph per env. Effect v4 does not memoize nested layers, so
...
// Mirrors upstream's harness, with Sonderr's SonderrSessions/MemoryService/fastAgents deltas.
...
// sonderr_change start - guard provider-compatible max-step request shape
it.instance(
...
noLLMServer.instance.skip(
  ...
  () =>
    Effect.gen(function* () {
      ...
      // sonderr_change start - compile the v2 reader against this test's database graph
...
// sonderr_change start - Item 14 CLI prove-it: cancel settles to Idle promptly,
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* pollWithTimeout(
        Effect.sync(() => (SonderrSessionPromptQueue.hasFollowup(chat.id) ? (true as const) : undefined)),
      ...
      expect(SonderrSessionPromptQueue.hasFollowup(chat.id)).toBe(false)
      expect((yield* status.get(chat.id)).type).toBe("idle")
      const free = yield* run.assertNotBusy(chat.id).pipe(Effect.exit)
      expect(Exit.isSuccess(free)).toBe(true)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const intake = yield* SonderrSessionPrompt.intake(
        sessionID,
        Effect.gen(function* () {
...
it.instance(
  ...
  { git: true },
  30_000, // sonderr_change - isolated suite load can delay queued live-loop cancellation
)
...
it.instance("prompt submitted during an active run is included in the next LLM input", () =>
  Effect.gen(function* () {
    ...
    const messages = inputs.at(-1)?.messages
    if (!Array.isArray(messages)) throw new Error("expected LLM messages")
    // sonderr_change start - Sonderr appends environment details to queued user prompts
    expect(messages.at(-1)).toMatchObject({
      role: "user",
...
unixNoLLMServer(
  ...
  () =>
    Effect.gen(function* () {
      ...
      // sonderr_change start - bind v2 execution and location services in the consolidated test graph
      const messages = yield* SessionV2.Service.use((session) => session.messages({ sessionID: chat.id })).pipe(
        Effect.provide(
          LayerNode.compile(SessionV2.node, [
            [SessionExecution.node, SessionExecution.noopLayer],
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* Effect.acquireRelease(
        Effect.sync(() =>
          Bus.subscribe(SonderrSession.Event.TurnOpen, (event) => {
...
it.instance("does not loop empty assistant turns for a simple reply", () =>
  Effect.gen(function* () {
    ...
    yield* llm.text("packages/cli/src/session/processor.ts")
  ...
  () =>
    Effect.gen(function* () {
      ...
      const other = yield* prompt.prompt({
        sessionID: session.id,
        agent: "build",
        model: { providerID: ProviderV2.ID.make("sonderr"), modelID: ModelV2.ID.make("kimi-k2.5-free") },
        noReply: true,
        parts: [{ type: "text", text: "hello" }],
      })
