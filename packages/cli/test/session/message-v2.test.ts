...
describe("session.message-v2.toModelMessage", () => {
  ...
  // sonderr_change start — send_file delivery attachments must not be replayed to the model
  test("strips send_file delivery attachments from model context", async () => {
        ...
        parts: [
          {
            ...
            state: {
              ...
              output: "File example.log (50 bytes, text/plain) delivered to the user's Sonderr app.",
    ...
    expect(toolContent.output).toStrictEqual({
      type: "text",
      value: "File example.log (50 bytes, text/plain) delivered to the user's Sonderr app.",
    })
  })
  // sonderr_change end
...
describe("session.message-v2.fromError", () => {
  ...
  test("classifies ZlibError from fetch as retryable APIError", () => {
    const zlibError = new Error(
      'ZlibError fetching "https://sonderr.cloudflare.dev/anthropic/messages". For more information, pass `verbose: true` in the second argument to fetch()',
    )
