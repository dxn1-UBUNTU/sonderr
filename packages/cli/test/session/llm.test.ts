...
import { LLMClient, RequestExecutor } from "@sonderr/llm/route"
...
import { USER_AGENT } from "../../src/installation" // sonderr_change
...
type ConfigModel = NonNullable<NonNullable<NonNullable<ConfigV1.Info["provider"]>[string]>["models"]>[string] // sonderr_change
...
describe("session.llm.ai-sdk adapter", () => {
  ...
  // sonderr_change start - preserve AI SDK raw usage for Sonderr provider billing
  ...
  test("preserves providerMetadata on step-finish so Anthropic cache writes survive getUsage", async () => {
    ...
    expect(stepFinish.providerMetadata).toEqual({
      ...
      sonderr: { routedModelID: "claude-3-5-sonnet" },
    })
    // sonderr_change end
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(headers.get("User-Agent") ?? "").toMatch(/^Sonderr-Code\//) // sonderr_change
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const capture = yield* Effect.promise(() => request)
        const body = capture.body
        const headers = capture.headers // sonderr_change

        expect(capture.url.pathname.endsWith("/messages")).toBe(true)
        expect(headers.get("User-Agent")?.split(" ")[0]).toBe(USER_AGENT) // sonderr_change
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        // sonderr_change start - auth keys use the same Google API key header as Standard keys
