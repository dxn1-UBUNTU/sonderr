...
import { Snapshot } from "@/snapshot" // sonderr_change
...
import { LLMEvent, Usage } from "@sonderr/llm"
...
function fake(
  ...
  return {
    ...
    metadata: Effect.fn("TestSessionProcessor.metadata")(() => Effect.void), // sonderr_change
...
type CompactionProcessOptions = {
  ...
  flags?: Partial<RuntimeFlags.Info> // sonderr_change
  snapshot?: Layer.Layer<Snapshot.Service> // sonderr_change
}
...
function compactionProcessLayer(options?: CompactionProcessOptions) {
  const replacements: LayerNode.Replacements = [
    ...
    [RuntimeFlags.node, RuntimeFlags.layer({ experimentalEventSystem: true, ...options?.flags })], // sonderr_change
    [SessionSummary.node, summary],
    ...(options?.snapshot ? ([[Snapshot.node, options.snapshot]] as const) : []), // sonderr_change
  ]
  if (!options?.llm) {
...
// sonderr_change start - keep retry-backoff cancellation tests independent of git snapshot cleanup latency
...
describe("session.compaction.process", () => {
  ...
  // sonderr_change start - configured output ceiling controls automatic tail budgeting
  itCompaction.instance(
  ...
  itCompaction.instance(
    ...
    () => {
      ...
      }).pipe(withCompaction({ llm: stub.llmLayer, snapshot: snap })) // sonderr_change
    },
    { timeout: 10_000 }, // sonderr_change - snapshot is isolated above
  )
    ...
    () =>
      Effect.gen(function* () {
        ...
        return yield* Effect.gen(function* () {
          ...
          // sonderr_change start - Effect 4.x timeout throws TimeoutError on the error
        ...
        }).pipe(withCompaction({ plugin: plugin(ready), snapshot: snap })) // sonderr_change - avoid git snapshot startup
      }),
    {}, // sonderr_change - isolate cancellation from git setup
  )
...
describe("SessionNs.getUsage", () => {
  ...
  // sonderr_change start - Test for OpenRouter provider cost
  ...
  test("uses upstreamInferenceCost for Sonderr provider", () => {
    const model = createModel({
      context: 100_000,
      output: 32_000,
      cost: {
    ...
    const provider = { id: "sonderr" } as Provider.Info
    const result = SessionNs.getUsage({
      model,
      provider,
      usage: usage({ inputTokens: 1_000_000, outputTokens: 100_000, totalTokens: 1_100_000 }),
    ...
    // Should use upstreamInferenceCost for Sonderr provider (BYOK)
  ...
  test("uses regular cost when upstreamInferenceCost is missing for Sonderr", () => {
    const model = createModel({
      context: 100_000,
      output: 32_000,
      cost: {
    ...
    // When upstream cost is missing for Sonderr, fall back to regular cost field
    expect(result.cost).toBe(0.01)
  })

  // Tests for Anthropic Messages / OpenAI Responses / Vercel AI Gateway cost extraction
  // live in test/sonderr/provider-cost.test.ts (sonderr_change).
  // sonderr_change end
