...
import { SessionNetwork } from "../../src/session/network" // sonderr_change
import { Bus } from "../../src/bus" // sonderr_change
...
// sonderr_change start - exercise non-default output token ceilings in the processor
...
// sonderr_change start - configured output ceiling must reach finish-step overflow accounting
...
it.live("session.processor effect tests reset reasoning state across retries", () =>
  provideTmpdirServer(
    ({ dir, llm }) =>
      Effect.gen(function* () {
        const { processors, session, provider } = yield* boot()
        // sonderr_change start — auto-reply to network reconnection prompts triggered by reset()
        ...
        offAsk() // sonderr_change — cleanup subscriber
      }),
    { config: (url) => providerCfg(url) },
  ),
)
...
// sonderr_change start — send_file delivery attachments must skip image normalization.
