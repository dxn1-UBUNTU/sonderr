...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    // sonderr_change start - the override is resolved via Env.Service by the tool and passed in
  ...
  test("is only enabled for sonderr or explicit websearch provider flags", () => {
    // sonderr_change
    expect(webSearchEnabled(ProviderV2.ID.sonderr, { exa: false, parallel: false })).toBe(true) // sonderr_change
    expect(webSearchEnabled(ProviderV2.ID.sonderr, { exa: false, parallel: false })).toBe(false) // sonderr_change
