...
const githubBase = <A, E, R>(url: string, self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      ...
      process.env.SONDERR_REPO_CLONE_GITHUB_BASE_URL = url
      return previous
    }),
    () => self,
    (previous) =>
      Effect.sync(() => {
        if (previous) process.env.SONDERR_REPO_CLONE_GITHUB_BASE_URL = previous
        else delete process.env.SONDERR_REPO_CLONE_GITHUB_BASE_URL
      }),
  )
...
describe("tool.grep", () => {
  ...
  it.instance("does not report an unknown total when results are truncated", () =>
    Effect.gen(function* () {
      ...
      expect(result.output).toContain("100 matches limit reached. Use limit=200 for more, or refine pattern.") // sonderr_change
  ...
  // sonderr_change start - exact-file searches must not widen to siblings
  ...
  it.instance("does not ask for external_directory when alias path is allowed", () =>
    Effect.gen(function* () {
      ...
      const tmp = yield* Effect.acquireRelease(
        Effect.promise(() => fs.mkdtemp(path.join(os.tmpdir(), "sonderr-grep-alias-"))),
