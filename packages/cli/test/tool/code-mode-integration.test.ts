import { afterAll, beforeAll, describe, expect, test } from "bun:test" // sonderr_change
...
import { Cause, Effect, Exit, Layer, ManagedRuntime } from "effect" // sonderr_change
...
let dispose = () => Promise.resolve() // sonderr_change
let runEffect = <A, E>(effect: Effect.Effect<A, E>) => Effect.runPromise(effect) // sonderr_change
...
async function buildTool() {
  ...
  for (const def of listed) {
    mcpTools[McpCatalog.toolName(SERVER, def.name)] = { def, client: client as unknown as Client, clientName: SERVER } // sonderr_change
  }
  ...
  const runtime = ManagedRuntime.make(layer) // sonderr_change
  dispose = () => runtime.dispose() // sonderr_change
  runEffect = (effect) => runtime.runPromise(effect) // sonderr_change
  return {
    tool: await runtime.runPromise(CodeModeTool.pipe(Effect.flatMap(Tool.init))), // sonderr_change
    description: describeCatalog(mcpTools, [SERVER]),
  }
}

const run = (code: string) => runEffect(tool.execute({ code }, ctx)) // sonderr_change
// Program failures die at the tool boundary; recover the defect for message assertions.
const runFailed = async (code: string) => {
  const exit = await runEffect(tool.execute({ code }, ctx).pipe(Effect.exit)) // sonderr_change
