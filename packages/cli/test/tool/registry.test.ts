...
import { Effect, Exit, Layer, Result, Schema } from "effect" // sonderr_change
...
import { run as runSandbox, type Profile } from "@sonderr/sandbox" // sonderr_change
...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".sonderr")])), // sonderr_change
})

type RegistryLayerOptions = {
  flags?: Partial<RuntimeFlags.Info>
  plugin?: Layer.Layer<Plugin.Service>
  config?: Parameters<typeof TestConfig.layer>[0] // sonderr_change
...
const registryLayer = (opts: RegistryLayerOptions = {}) => {
  const replacements = [
    [Config.node, opts.config ? TestConfig.layer(opts.config) : configLayer], // sonderr_change
...
const scout = testEffect(registryLayer({ flags: { experimentalScout: true } })) // sonderr_change
      ...
      tools: () =>
        Effect.succeed({
          weather_current: {
            ...
            clientName: "weather", // sonderr_change
          },
        }),
...
// sonderr_change start - verify the execute catalog is suppressed in restricted sessions
...
describe("tool.registry", () => {
  ...
  // sonderr_change start - background task parameters are available by default
  ...
  it.instance("loads tools from .sonderr/tool (singular)" /* sonderr_change */, () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const sonderr = path.join(test.directory, ".sonderr") // sonderr_change
      const tool = path.join(sonderr, "tool")
  ...
  it.instance("ignores non-tool exports in .sonderr/tool files" /* sonderr_change */, () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".sonderr", "tool") // sonderr_change
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  // came in through `plugin.list()` — `oh-my-sonderr` was registering a tool
  ...
  it.instance("loads tools from .sonderr/tools (plural)" /* sonderr_change */, () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const sonderr = path.join(test.directory, ".sonderr") // sonderr_change
      const tools = path.join(sonderr, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".sonderr", "tools") // sonderr_change
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const sonderr = path.join(test.directory, ".sonderr") // sonderr_change
        const customTools = path.join(sonderr, "tools")
        const plugin = path.join(sonderr, "node_modules", "@sonderr", "plugin") // sonderr_change
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(sonderr, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@sonderr/plugin", type: "module", exports: { ".": "./dist/index.js" } }), // sonderr_change
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            [
              'import { tool } from "@sonderr/plugin"', // sonderr_change
              "export default tool({",
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".sonderr", "tools") // sonderr_change
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(sonderr, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@sonderr/plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(sonderr, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(sonderr, "node_modules", "cowsay")
