...
import { disposeAllInstances, provideTmpdirInstance, TestInstance } from "../fixture/fixture" // sonderr_change
...
// sonderr_change - skip on windows: address windows ci failures #9496
...
describe("tool.skill", () => {
  unix("execute returns skill content block with files", () =>
    Effect.gen(function* () {
      ...
      const skill = path.join(dir, ".sonderr", "skill", "tool-skill") // sonderr_change
...
`,
  ...
  it.live("built-in sonderr-config keeps rendered shell examples inert", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          ...
          const result = yield* tool.execute({ name: "sonderr-config" }, ctx)

          expect(result.metadata.dir).toBe("builtin")
          expect(result.output).toContain("Finding a named command")
          expect(result.output).toContain("~/.config/sonderr/")
          expect(result.output).toContain("~/.sonderr/")
