import { afterAll, describe, expect, test } from "bun:test" // sonderr_change
...
import { Cause, Effect, Exit, Layer, ManagedRuntime, Schema } from "effect" // sonderr_change
import { InstanceRef } from "@/effect/instance-ref" // sonderr_change
import { TestConfig } from "../fixture/config" // sonderr_change
...
const runtimes: { dispose(): Promise<void> }[] = [] // sonderr_change

afterAll(async () => {
  await Promise.all(runtimes.map((runtime) => runtime.dispose())) // sonderr_change
})
...
function mcpTool(
  ...
  return {
    ...
    clientName: name.split("_")[0] ?? name, // sonderr_change
  }
}
...
function harness(input: {
  ...
  sandbox?: boolean // sonderr_change
}) {
  return Layer.mergeAll(
    ...
    }), // sonderr_change - production code mode captures config for sandbox policy
    Layer.succeed(InstanceRef, { directory: process.cwd(), worktree: process.cwd(), project: {} as any }), // sonderr_change
    AppNodeBuilder.build(Database.node), // sonderr_change - sandbox state uses the session database
  )
}
...
function build(
  ...
  const runtime = ManagedRuntime.make(harness({ mcpTools, servers: names, permission, trigger, sandbox })) // sonderr_change
  runtimes.push(runtime) // sonderr_change
  return runtime.runPromise(CodeModeTool.pipe(Effect.flatMap(Tool.init))) // sonderr_change
}
...
describe("code mode execute", () => {
  ...
  test("large catalogs inline a budgeted PARTIAL list plus runtime search", async () => {
    ...
    for (let i = 0; i < 150; i++) {
      tools[`alpha_op_${i}`] = {
        ...
        clientName: "alpha", // sonderr_change
      }
    }
  ...
  // sonderr_change start - code-mode must not advertise remote MCP tools in a network-restricted sandbox
  test("does not expose remote MCP tools in a network-restricted sandbox", async () => {
    ...
    const tool = await build({ remote_tool: entry }, undefined, undefined, undefined, true)
    const scoped = { ...ctx, sessionID: SessionID.make("ses_code-mode-remote") }
    const output = await Effect.runPromise(
      tool.execute(
        { code: "try { await tools.remote.tool({}) } catch (e) { return e.message }" },
        scoped,
      ),
    )
  ...
  test("leaves oversized results to Sonderr's native tool-output truncation", async () => {
    const tool = await build({})
