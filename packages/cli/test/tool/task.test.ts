...
import { Cause, Deferred, Effect, Exit, Fiber, Layer } from "effect" // sonderr_change - Cause for resume-hint coverage
...
import { MessageV2 } from "@/session/message-v2" // sonderr_change
import type { SessionPrompt } from "../../src/session/prompt"
import { MessageID, PartID, SessionID } from "../../src/session/schema" // sonderr_change - SessionID used by cost propagation tests
import { SessionRunState } from "@/session/run-state"
import { SessionStatus } from "@/session/status"
import { Provider } from "../../src/provider/provider" // sonderr_change
import { SonderrSession } from "../../src/sonderr/session" // sonderr_change
...
const background = it // sonderr_change - background subagents are enabled by default
const disabled = testEffect(layer({ experimentalBackgroundSubagents: false })) // sonderr_change
...
// sonderr_change start - stub signature + prompt body extended to persist assistant cost for propagation tests
...
describe("tool.task", () => {
  ...
  // sonderr_change start - verify forked task children remain resumable
  ...
  // sonderr_change start - resumed children rebuild parent platform attribution after restart
  it.instance("execute preserves platform attribution when resuming a task", () =>
    Effect.gen(function* () {
      const sessions = yield* Session.Service
      const { chat, assistant } = yield* seed()
      SonderrSession.setPlatformOverride(chat.id, "agent-manager")
      const child = yield* sessions.create({ parentID: chat.id, title: "Existing child" })
      SonderrSession.clearPlatformOverride(child.id)
      const tool = yield* TaskTool
      const def = yield* tool.init()
      ...
      expect(SonderrSession.resolvePlatform(child.id)).toBe("agent-manager")
      expect(SonderrSession.resolveRoot(child.id)).toBe(chat.id)
    }),
  )
  // sonderr_change end
  ...
  // sonderr_change start - regression for #13469: a trailing synthetic empty text part (the memory marker)
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        // sonderr_change start — use arrayContaining: Sonderr appends inherited caller restrictions
        ...
        expect(seen?.tools).toEqual({
          ...
          todowrite: false,
          task: false, // sonderr_change - Sonderr disallows nested subagents
          bash: false,
          read: false,
        })
      }),
    {
      config: {
        ...
        experimental: {
          ...
          openTelemetry: true, // sonderr_change
        },
      },
    },
  )

  // sonderr_change start - terminal child assistant errors fail the task tool boundary
  ...
  // sonderr_change start - background subagent failures also surface the resumable task_id (#11620)
  ...
  // sonderr_change end
  // sonderr_change start - preserve the disabled-background regression test
  ...
  // sonderr_change start - completed background tasks propagate their invocation cost delta
  ...
  // sonderr_change start - the background.extend() path must also propagate its run's cost delta (regression)
...
// sonderr_change start - subagent cost propagation coverage (#6321)
