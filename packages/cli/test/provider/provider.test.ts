...
const paid = (providers: Record<string, { models: Record<string, { cost: { input: number } }> }>) => {
  const item = providers[ProviderV2.ID.make("sonderr")]
  if (!item) return 0 // sonderr_change - Sonderr drops sonderr provider without apiKey/auth
...
it.instance(
  // sonderr_change start - Sonderr always has an auto-routed small-model fallback
  "getSmallModel falls back to Sonderr auto when model IDs lack family metadata",
  Effect.gen(function* () {
    const model = yield* Provider.use.getSmallModel(ProviderV2.ID.make("test-provider"))
    expect(model).toMatchObject({ providerID: "sonderr", id: "sonderr-auto/small" })
  }),
  // sonderr_change end
  {
    config: {
...
it.instance("ModelNotFoundError suggests catalog models for unloaded providers", () =>
  Effect.gen(function* () {
    yield* remove("SONDERR_API_KEY")
    const error = yield* Provider.use
      .getModel(ProviderV2.ID.sonderr, ModelV2.ID.make("claude-haiku-fake-model"))
      .pipe(Effect.flip)
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    expect(providers[ProviderV2.ID.make("nvidia")].options.headers).toEqual({
      "HTTP-Referer": "https://kilo.ai/", // sonderr_change
...
it.instance(
  "configured variants remain authoritative", // sonderr_change
  Effect.gen(function* () {
    yield* set("ANTHROPIC_API_KEY", "test-api-key")
    const providers = yield* list
    const model = providers[ProviderV2.ID.anthropic].models["claude-sonnet-4-6"]
    expect(model.variants!["high"]).toBeDefined()
    expect(model.variants!["high"].thinking).toBeUndefined() // sonderr_change
...
it.instance(
  ...
  {
    config: {
      provider: { "cloudflare-ai-gateway": { options: { metadata: { invoked_by: "test", project: "sonderr" } } } },
    },
  },
)
...
it.effect("plugin config providers persist after instance dispose", () =>
  Effect.gen(function* () {
    ...
    const configDir = path.join(dir, ".sonderr") // sonderr_change
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    const configDir = path.join(instance.directory, ".sonderr") // sonderr_change
    const root = path.join(configDir, "plugin")
    yield* Effect.promise(() => mkdir(root, { recursive: true }))
    yield* Effect.promise(() => markPluginDependenciesReady(configDir))
    yield* Effect.promise(() => markPluginDependenciesReady(Global.Path.config)) // sonderr_change
    yield* Effect.promise(() =>
      Bun.write(
...
it.effect("sonderr loader keeps paid models when config apiKey is present", () =>
  Effect.gen(function* () {
    const noneDir = yield* tmpdirScoped()
    const keyedDir = yield* tmpdirScoped({
      config: { provider: { sonderr: { options: { apiKey: "test-key" } } } },
    })
...
it.effect("sonderr loader keeps paid models when auth exists", () =>
  Effect.gen(function* () {
    const noneDir = yield* tmpdirScoped()
    const keyedDir = yield* tmpdirScoped()
    ...
    yield* Effect.acquireRelease(
      Effect.promise(() => Filesystem.write(authPath, JSON.stringify({ sonderr: { type: "api", key: "test-key" } }))),
      () =>
