...
describe("ProviderTransform.schema - gemini non-object properties removal", () => {
  ...
  test("sanitizes gemini schemas routed through the Sonderr Gateway", () => {
    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".sonderr/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    ...
    const result = ProviderTransform.message(msgs, sonderrModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.sonderr?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.sonderr?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              sonderr: { itemId: "msg_sonderr_part" },
    ...
    expect(result[0].providerOptions?.sonderr?.itemId).toBe("msg_sonderr")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.sonderr?.itemId).toBe("msg_sonderr_part")
...
describe("ProviderTransform.message - cache control on gateway", () => {
  ...
  test("sonderr gateway with openai gpt-5.6 applies caching options", () => {
    const model = createModel({
      providerID: "sonderr",
      api: {
        id: "openai/gpt-5.6",
        url: "https://api.kilo.ai/api/gateway",
        npm: "@sonderr/sonderr-gateway",
      },
      id: "openai/gpt-5.6",
    })
    const msgs = [
...
describe("ProviderTransform.variants", () => {
  ...
  // sonderr_change start
  describe("@sonderr/sonderr-gateway", () => {
    test("mercury-2 uses server-provided variants from sonderr gateway", () => {
      ...
      const model = createMockModel({
        id: "sonderr/inception/mercury-2",
        providerID: "sonderr",
        api: {
  ...
  describe("ProviderTransform.smallOptions", () => {
    describe("@sonderr/sonderr-gateway", () => {
      test("claude models use their default reasoning effort", () => {
        const model = createMockModel({
          id: "sonderr/anthropic/claude-sonnet-4",
          providerID: "sonderr",
          api: {
            id: "anthropic/claude-sonnet-4",
            url: "https://gateway.kilo.ai",
      ...
      test("google models use their default reasoning effort", () => {
        const model = createMockModel({
          id: "sonderr/google/gemini-2.0-flash",
          providerID: "sonderr",
          api: {
...
// sonderr_change start - tests for reasoningSummary guard
describe("ProviderTransform.options - OpenAI Responses API params guard", () => {
  ...
  test("includes reasoningSummary for @sonderr/sonderr-gateway", () => {
    const result = ProviderTransform.options({
      model: gpt5Model("@sonderr/sonderr-gateway", "sonderr"),
      sessionID,
    })
    expect(result.reasoningSummary).toBe("auto")
  })
