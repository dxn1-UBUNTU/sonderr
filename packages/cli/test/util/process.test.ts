...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.SONDERR_TEST ?? "")'), {
      env: {
        SONDERR_TEST: "set",
      },
    })
  ...
  test("can use a complete environment without inherited values", async () => {
    const key = "SONDERR_TEST_INHERITED_ENV"
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "SONDERR_TEST_SHELL"], {
      shell: true,
      env: {
        SONDERR_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("SONDERR_TEST_SHELL=ok")
  })
