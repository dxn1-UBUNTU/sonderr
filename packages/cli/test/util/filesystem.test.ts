...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "sonderr.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "sonderr.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "sonderr.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "sonderr.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["sonderr.json", "sonderr.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "sonderr.jsonc"),
      ])
    })
  })
