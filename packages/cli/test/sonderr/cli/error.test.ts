...
describe("model not found errors", () => {
  test("indicates when no models are available", () => {
    ...
    expect(named).toContain("sonderr.json")
    expect(named).not.toContain("sonderr.json")
...
describe("remote config authentication errors", () => {
  test("uses the Sonderr login command", () => {
    const error = FormatError({
    ...
    expect(error).toContain("sonderr auth login https://example.com/config.json")
    expect(error).not.toContain("sonderr auth login")
  })
})
