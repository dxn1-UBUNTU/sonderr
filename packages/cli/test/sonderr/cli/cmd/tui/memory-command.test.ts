...
import { memoryRow } from "@/sonderr/cli/cmd/tui/component/memory-status"
import { runMemoryCommand } from "@/sonderr/cli/cmd/tui/memory-command"
import { MemoryTuiEvents } from "@/sonderr/cli/cmd/tui/memory-events"
import { MemoryTuiState } from "@/sonderr/cli/cmd/tui/memory-state"
...
describe("memory TUI command parser", () => {
  test("manual mutation toasts match server event wording", async () => {
    ...
    await runMemoryCommand({ ...base, text: "/memory remember tests run from packages/cli" })
    ...
    expect(calls).toEqual([
      { sessionID: "ses_tui_memory", text: "tests run from packages/cli" },
  ...
  test("status opens overview dialog", async () => {
    const shown: string[] = []
    const opened: string[] = []
    const client = { memory: {} } as unknown as SonderrClient
  ...
  test("inspect reveals the memory folder", async () => {
    ...
    const client = {
      memory: {
        status: async () => ({ data: { root: "/tmp/sonderr-memory", state: { enabled: true } } }),
      },
    } as unknown as SonderrClient
    ...
    expect(opened).toEqual(["/tmp/sonderr-memory"])
    expect(shown).toEqual(["Memory folder: /tmp/sonderr-memory"])
  })
  ...
  test("on and off call enable and disable endpoints", async () => {
    ...
    const client = {
      memory: {
        enable: async () => {
          calls.push("enable")
          return { data: { root: "/tmp/sonderr-data/memory/repo-abc123", index: { tokens: 42 } } }
        },
        disable: async () => {
          calls.push("disable")
  ...
  test("memory commands route to session directory when no workspace is active", async () => {
    ...
    await runMemoryCommand({ ...base, text: "/memory auto off", directory: "/repo/packages/cli" })
    await runMemoryCommand({
      ...base,
    ...
    expect(calls).toEqual([
      { directory: "/repo/packages/cli", autoConsolidate: false },
