...
import { SonderrTuiConfig } from "@/sonderr/cli/cmd/tui/context/tui-config"
import { SonderrTerminalTitle } from "@/sonderr/cli/cmd/tui/terminal-title"
...
function resolve(input: TuiConfig.Info): TuiConfig.Resolved {
  ...
  return {
    ...
    attention: {
      ...
      sound_pack: input.attention?.sound_pack ?? "sonderr.default",
...
describe("SonderrTuiConfig.makeStore", () => {
  test("reactive reads update when set() reconciles a new config", () => {
    const store = SonderrTuiConfig.makeStore(resolve(cfg({ keybinds: { app_exit: "ctrl+c" }, theme: "sonderr" })))
    ...
    createRoot((d) => {
      ...
      createEffect(() =>
        titles.push(
          SonderrTerminalTitle.format({ base: "Sonderr CLI", indicator: "working", icon: store.config.title_icon }),
        ),
      )
    })
    ...
    expect(themes).toEqual(["sonderr"])
    expect(icons).toEqual(["none"])
    expect(titles).toEqual(["Sonderr CLI"])
    ...
    expect(themes).toEqual(["sonderr", "nord"])
    expect(icons).toEqual(["none", "emojis"])
    expect(titles).toEqual(["Sonderr CLI", "💭 Sonderr CLI"])

    dispose()
  })

  test("set() restores the default title icon when the setting is removed", () => {
    const store = SonderrTuiConfig.makeStore(resolve(cfg({ title_icon: "unicode" })))
