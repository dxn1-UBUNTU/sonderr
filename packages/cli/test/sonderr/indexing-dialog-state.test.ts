...
import type { Config, IndexingConfig } from "@sonderr/sdk/v2"
import {
  ...
  mergeIndexingConfig,
  type IndexingScope,
} from "../../src/sonderr/components/indexing-dialog-state"

describe("indexing dialog state", () => {
  test.serial("loads Sonderr models directly from the public catalog", async () => {
    ...
    global.fetch = (async (input) => {
      ...
      return new Response(
        JSON.stringify({
          defaultModel: "sonderr/default",
          models: [{ id: "sonderr/default", name: "Default", dimension: 1024, scoreThreshold: 0.35 }],
          aliases: {},
        }),
    ...
    try {
      const catalog = await loadSonderrEmbeddingModels()

      expect(catalog.models).toHaveLength(1)
      expect(catalog.defaultModel).toBe("sonderr/default")
  ...
  test("builds stable loading, empty, and loaded model options", () => {
    expect(sonderrModelOptions()).toEqual([{ value: "", title: "Loading supported models..." }])
    expect(sonderrModelOptions({ defaultModel: "", models: [], aliases: {} })).toEqual([
    ...
    expect(sonderrModelOptions(catalog)).toEqual([
      { value: "provider/default", title: "Default (1024d)" },
      { value: "provider/code", title: "Code (code, 1536d)" },
    ])
    expect(currentSonderrModel(catalog, "code")).toBe("provider/code")
    expect(currentSonderrModel(catalog, "missing")).toBe("provider/default")
  })
  ...
  test("isolates global auth config from project indexing values", () => {
    const project: IndexingConfig = { sonderr: { apiKey: "project-key", baseUrl: "https://project.test" } }
    const inherited: IndexingConfig = { enabled: true }
    const effective: Config = {
      provider: { sonderr: { options: { apiKey: "provider-key" } } },
      indexing: project,
    }
