...
// These are regression guards — if an upstream Sonderr merge overwrites the
// sonderr integration in shared LSP files, these tests catch it.

import { describe, test, expect, spyOn, afterEach } from "bun:test"
import path from "path"
import * as LSPServer from "../../src/lsp/server"
import { TsClient } from "../../src/sonderr/ts-client"
import { TsCheck } from "../../src/sonderr/ts-check"
import { Flag } from "@sonderr/core/flag/flag"
import { Instance, type InstanceContext } from "../../src/sonderr/instance"
...
describe("typescript lightweight mode", () => {
  ...
  describe("source integration guards", () => {
    // These tests verify that sonderr integration code exists in shared
    ...
    test("lsp/server.ts gates Typescript.spawn behind flag", async () => {
      ...
      expect(src).toContain("SONDERR_EXPERIMENTAL_LSP_TOOL")
