...
function inject(msgs: MessageV2.WithParts[], cache: SonderrSessionPrompt.EnvCache = {}) {
  SonderrSessionPrompt.injectEditorContext({ msgs, session, sessionID, cache })
}
...
describe("injectEditorContext", () => {
  ...
  test("is byte-identical across repeated loop iterations", async () => {
    ...
    const cache: SonderrSessionPrompt.EnvCache = {}
