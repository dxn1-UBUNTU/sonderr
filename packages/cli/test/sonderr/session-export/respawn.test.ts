...
import { getKillSwitchReason, resetEligibility } from "@/sonderr/session-export/eligibility"
...
describe("SessionExport worker respawn", () => {
  ...
  beforeEach(async () => {
    ...
    feature = process.env.SONDERR_FEATURE
    resetEligibility()
  })
  ...
  afterEach(async () => {
    await SessionExport.shutdown()
    resetEligibility()
    if (feature === undefined) delete process.env.SONDERR_FEATURE
    else process.env.SONDERR_FEATURE = feature
  })

  test("passes surface to worker init", () => {
    const workers: FakeWorker[] = []
    process.env.SONDERR_FEATURE = "cli"
    SessionExport.init({
      agentVersion: "v0",
      dbPath: ":memory:",
      subscribeAll: () => () => {},
