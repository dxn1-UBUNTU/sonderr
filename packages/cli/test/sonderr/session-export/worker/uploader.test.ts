...
import { Storage } from "@/sonderr/session-export/worker/storage"
import { Uploader, backoffFor } from "@/sonderr/session-export/worker/uploader"
import { Config } from "@/sonderr/session-export/config"
...
describe("Uploader", () => {
  ...
  beforeEach(() => {
    token = process.env.SONDERR_SESSION_EXPORT_AUTH_TOKEN
    delete process.env.SONDERR_SESSION_EXPORT_AUTH_TOKEN
  ...
  afterEach(() => {
    ...
    if (token === undefined) delete process.env.SONDERR_SESSION_EXPORT_AUTH_TOKEN
    else process.env.SONDERR_SESSION_EXPORT_AUTH_TOKEN = token
  })

  test("2xx response marks rows uploaded and deletes them", async () => {
    const telemetry: unknown[] = []
    const calls: Array<{ input: string; init: RequestInit }> = []
    process.env.SONDERR_SESSION_EXPORT_AUTH_TOKEN = "local-token"
    ...
    expect(headers.get("x-sonderr-export-api-version")).toBe("1")
    expect(headers.get("x-sonderr-export-schema-version")).toBe("1")
    expect(headers.get("x-sonderr-export-agent-version")).toBe("v0")
    expect(headers.get("x-sonderr-export-root-session-id")).toBe("s1")
    expect(headers.get("x-sonderr-export-session-id")).toBe("s1")
    expect(headers.get("x-sonderr-export-seq-start")).toBe("0")
    expect(headers.get("x-sonderr-export-seq-end")).toBe("0")
    expect(headers.get("x-sonderr-export-event-count")).toBe("1")
    expect(headers.get("x-sonderr-export-content-encoding")).toBe("identity")
    expect(headers.get("x-sonderr-export-client-sent-at")).toMatch(/^\d{4}-\d{2}-\d{2}T/)
    expect(headers.get("x-sonderr-export-payload-sha256")).toBe(await sha256(body))
  ...
  test("sends anonymous id with all export headers when auth token is absent", async () => {
    ...
    const headers = new Headers(calls[0].init.headers)
    const names = [
      "x-sonderr-export-api-version",
      "x-sonderr-export-schema-version",
      "x-sonderr-export-agent-version",
      "x-sonderr-export-surface",
      "x-sonderr-export-root-session-id",
      "x-sonderr-export-session-id",
      "x-sonderr-export-batch-id",
      "x-sonderr-export-seq-start",
      "x-sonderr-export-seq-end",
      "x-sonderr-export-event-count",
      "x-sonderr-export-payload-sha256",
      "x-sonderr-export-client-sent-at",
      "x-sonderr-export-content-encoding",
    ]

    expect(headers.get("authorization")).toBeNull()
    expect(headers.get("x-sonderr-anon-id")).toBe("install_abc")
  ...
  test("falls back to telemetry id when anon id is not provided", async () => {
    ...
    const headers = new Headers(calls[0].init.headers)
    expect(headers.get("authorization")).toBeNull()
    expect(headers.get("x-sonderr-anon-id")).toBe("install_from_disk")
  })
  ...
  test("includes surface in batch metadata and upload headers", async () => {
    ...
    expect(headers.get("x-sonderr-export-surface")).toBe("vscode-extension")
