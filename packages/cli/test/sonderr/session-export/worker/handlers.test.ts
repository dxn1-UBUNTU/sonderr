...
import { Chunker } from "@/sonderr/session-export/worker/chunks"
import { Scrubber } from "@/sonderr/session-export/worker/scrub"
import { handleEvent } from "@/sonderr/session-export/worker/handlers"
import type {
  CompactionCaptured,
  LlmRequestCompleted,
  LlmRequestStarted,
...
} from "@/sonderr/session-export/events"
...
function started(id: string, input: Partial<LlmRequestStarted["input"]>): LlmRequestStarted {
  return {
    ...
    agent: "claude",
    modeId: "build",
    model: { providerId: "sonderr", modelId: "free-1", isFree: true },
