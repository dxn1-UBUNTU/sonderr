...
function models(input: unknown, key: "all" | "providers") {
  ...
  const sonderr = input[key].find((provider) => record(provider) && provider.id === "sonderr")
  if (!record(sonderr) || !record(sonderr.models)) return []
  return Object.keys(sonderr.models)
}

function request(path: string, dir: string) {
  return Effect.promise(async () => {
    const result = await Server.Default().app.request(path, { headers: { "x-sonderr-directory": dir } })
...
it.live(
  ...
  Effect.gen(function* () {
      ...
      Effect.promise(() =>
        tmpdir({
          config: {
            formatter: false,
            lsp: false,
            enabled_providers: ["sonderr"],
            hide_prompt_training_models: true,
            provider: { sonderr: { options: { baseURL } } },
          },
        }),
      ),
    ...
    yield* Effect.acquireRelease(
      Effect.sync(() => {
        process.env.SONDERR_API_KEY = "test-key"
      }),
      () =>
        Effect.sync(() => {
