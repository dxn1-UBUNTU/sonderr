...
import { invalidateAfterProviderAuthChange } from "../../../src/sonderr/server/provider-auth-lifecycle"
...
it.effect("clears provider models before disposing instances after auth changes", () =>
  Effect.gen(function* () {
    ...
    yield* invalidateAfterProviderAuthChange("sonderr").pipe(Effect.provide(layer(events)))

    expect(yield* Ref.get(events)).toEqual(["clear:sonderr", "dispose"])
  }),
)
