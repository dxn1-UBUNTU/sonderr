...
const [runtime, gateway, server, fixture] = await (async () => {
  const disabled = process.env.SONDERR_DISABLE_SESSION_INGEST
  const base = process.env.SONDERR_SESSION_INGEST_URL
  process.env.SONDERR_DISABLE_SESSION_INGEST = "1"
  process.env.SONDERR_SESSION_INGEST_URL = ingest.url.origin
  try {
    return await Promise.all([
      import("../../../src/effect/app-runtime"),
      import("../../../src/sonderr/server/httpapi/groups/sonderr-gateway"),
      import("../../../src/server/routes/instance/httpapi/server"),
      import("../../fixture/fixture"),
    ])
  } finally {
    if (disabled === undefined) delete process.env.SONDERR_DISABLE_SESSION_INGEST
    else process.env.SONDERR_DISABLE_SESSION_INGEST = disabled
    if (base === undefined) delete process.env.SONDERR_SESSION_INGEST_URL
    else process.env.SONDERR_SESSION_INGEST_URL = base
  }
})()
const { AppRuntime } = runtime
const { SonderrGatewayPaths } = gateway
...
async function request(directory: string, body: unknown) {
  const auth = process.env.SONDERR_AUTH_CONTENT
  const fetcher = globalThis.fetch
  const token = `test-${crypto.randomUUID()}`
  bodies.set(token, body)
  process.env.SONDERR_AUTH_CONTENT = JSON.stringify({ sonderr: { type: "api", key: token } })
  ...
  try {
    return await HttpApiApp.webHandler().handler(
      new Request(`http://localhost${SonderrGatewayPaths.cloudSessionImport}`, {
        method: "POST",
        headers: { "content-type": "application/json", "x-sonderr-directory": directory },
    ...
    if (auth === undefined) delete process.env.SONDERR_AUTH_CONTENT
    else process.env.SONDERR_AUTH_CONTENT = auth
  }
}

async function routed<T>(id: string, run: () => Promise<T>) {
  const workspace = Flag.SONDERR_WORKSPACE_ID
  Flag.SONDERR_WORKSPACE_ID = id
  try {
    return await run()
  } finally {
    Flag.SONDERR_WORKSPACE_ID = workspace
  }
}
