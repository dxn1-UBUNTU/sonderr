...
const state = Layer.effectDiscard(
  Effect.gen(function* () {
    const original = Flag.SONDERR_EXPERIMENTAL_WORKSPACES
    Flag.SONDERR_EXPERIMENTAL_WORKSPACES = true
    yield* Effect.addFinalizer(() =>
      Effect.promise(async () => {
        Flag.SONDERR_EXPERIMENTAL_WORKSPACES = original
        await resetDatabase()
      }),
    )
  }),
...
describe("Sonderr Console worktree listing", () => {
  run(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const directory = path.join(test.directory, ".sonderr", "worktrees", "console-list")
