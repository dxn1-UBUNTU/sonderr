...
process.env.SONDERR_SESSION_RETRY_LIMIT = "2"
...
afterEach(() => {
  delete process.env.SONDERR_SESSION_RETRY_LIMIT
})
...
describe("session processor retry limit", () => {
  ...
  it.effect("only positive integers enable the limit", () =>
    Effect.promise(async () => {
      const { Flag } = await import("@sonderr/core/flag/flag")

      delete process.env.SONDERR_SESSION_RETRY_LIMIT
      expect(Flag.SONDERR_SESSION_RETRY_LIMIT).toBeUndefined()

      process.env.SONDERR_SESSION_RETRY_LIMIT = "0"
      expect(Flag.SONDERR_SESSION_RETRY_LIMIT).toBeUndefined()

      process.env.SONDERR_SESSION_RETRY_LIMIT = "-1"
      expect(Flag.SONDERR_SESSION_RETRY_LIMIT).toBeUndefined()

      process.env.SONDERR_SESSION_RETRY_LIMIT = "abc"
      expect(Flag.SONDERR_SESSION_RETRY_LIMIT).toBeUndefined()

      process.env.SONDERR_SESSION_RETRY_LIMIT = "2"
      expect(Flag.SONDERR_SESSION_RETRY_LIMIT).toBe(2)
    }),
  )
  ...
  it.effect("reads env at access time (dynamic getter)", () =>
    Effect.promise(async () => {
      ...
      delete process.env.SONDERR_SESSION_RETRY_LIMIT
      expect(Flag.SONDERR_SESSION_RETRY_LIMIT).toBeUndefined()
      process.env.SONDERR_SESSION_RETRY_LIMIT = "5"
      expect(Flag.SONDERR_SESSION_RETRY_LIMIT).toBe(5)
      delete process.env.SONDERR_SESSION_RETRY_LIMIT
    }),
  )
})
