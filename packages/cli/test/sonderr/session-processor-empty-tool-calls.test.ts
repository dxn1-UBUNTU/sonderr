        ...
        Effect.gen(function* () {
          ...
          const warning = parts.find(
            (part): part is MessageV2.TextPart =>
              part.type === "text" && part.text === SonderrSessionProcessor.REASONING_LENGTH_WARNING,
          )

          expect(warning?.ignored).toBe(true)

          const modelMsgs = yield* MessageV2.toModelMessagesEffect([{ info: handle.message, parts }], mdl)
          expect(JSON.stringify(modelMsgs)).not.toContain(SonderrSessionProcessor.REASONING_LENGTH_WARNING)
        }),
      { git: true },
    ),
  )
    ...
    provideTmpdirProject(
      (dir) =>
        Effect.gen(function* () {
          ...
          const warning = parts.find(
            (part): part is MessageV2.TextPart =>
              part.type === "text" && part.text === SonderrSessionProcessor.OUTPUT_LENGTH_WARNING,
          )
          ...
          expect(json).not.toContain(SonderrSessionProcessor.OUTPUT_LENGTH_WARNING)
        }),
      { git: true },
    ),
  )
      ...
      (dir) =>
        Effect.gen(function* () {
          ...
          const selection = {
            ...
            modelID: ModelV2.ID.make("sonderr-auto/efficient"),
          }
          ...
          yield* test.reply(
            ...
            LLMEvent.stepFinish({
              ...
              usage: usage(),
              providerMetadata: {
                sonderr: { routedModelID: "openai/gpt-5.5-20260423" },
                sonderr: { vercelID: "fra1::test" },
                gateway: {
