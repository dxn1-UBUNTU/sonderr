...
import { legacyReviewMessage, parseReviewCommand, reviewCommand } from "../../src/sonderr/review/command"
...
describe("review command parsing", () => {
  test("parses every supported review invocation", () => {
    ...
    expect(parseReviewCommand("/review https://github.com/Sonderr-Org/sonderr/pull/11084")).toBe("review")
...
describe("review command", () => {
  ...
  test("documents worktree metadata candidate precedence", () => {
    const text = cmd.template as string
    expect(text).toContain("git rev-parse --git-path sonderr-agent-manager-metadata.json")
    expect(text).toContain("`.sonderr/metadata.json` in the current worktree checkout")
    expect(text).toContain("`.sonderr/metadata.json` in the current worktree checkout")
    const admin = text.indexOf("git rev-parse --git-path sonderr-agent-manager-metadata.json")
    const sonderr = text.indexOf("`.sonderr/metadata.json` in the current worktree checkout")
    const sonderr = text.indexOf("`.sonderr/metadata.json` in the current worktree checkout")
    expect(admin).toBeLessThan(sonderr)
    expect(sonderr).toBeLessThan(sonderr)
    expect(text).toContain("use `lstat`, not `stat`")
    expect(text).toContain("immediate `.sonderr` or `.sonderr` directory")
