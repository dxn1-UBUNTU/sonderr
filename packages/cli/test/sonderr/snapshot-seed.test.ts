...
import { SonderrSnapshotMaterialize } from "../../src/sonderr/snapshot/materialize"
import { SonderrSnapshotSeed } from "../../src/sonderr/snapshot/seed"
...
test(
  ...
  async () => {
    ...
    expect(
      (
        await $`git --git-dir=${common} rev-parse --verify --quiet ${SonderrSnapshotMaterialize.ref(fast.gitdir)}`
          .nothrow()
          .text()
      ).trim(),
    ).toBe("")
...
test(
  ...
  async () => {
    ...
    const sourceRef = SonderrSnapshotMaterialize.ref(result.gitdir)
    ...
    const expired = `refs/sonderr/snapshots/1/${result.value!}`
    ...
    expect((await $`git --git-dir=${result.gitdir} for-each-ref refs/sonderr/snapshots`.text()).trim()).toContain(
      result.value!,
    )

    const locked = `refs/sonderr/snapshots/2/${result.value!}`
...
test("interrupted seed removes borrowed state after source gc", async () => {
  ...
  const original = await fs.readFile(index)
  const ref = SonderrSnapshotMaterialize.ref(gitdir)
  ...
  await Effect.runPromise(
    Effect.gen(function* () {
      ...
      const fiber = yield* SonderrSnapshotSeed.seed({
        dir: source.path,
