...
import { SonderrSessionRevert } from "@/sonderr/session/revert"
    ...
    provideTmpdirInstance(
      (root) =>
        Effect.gen(function* () {
          ...
          yield* SonderrSessionRevert.restore(item.snapshot, item.after, item.patch.files).pipe(provideInstance(dir))
