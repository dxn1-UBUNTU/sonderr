...
import { hasAtomicChatPlugin } from "@/sonderr/atomic-chat-feature"
import { SonderrDefaultPlugins } from "@/sonderr/config/default-plugins"
import { PluginLoader } from "@/plugin/loader"

const atomic = "@sonderr/plugin-atomic-chat"

describe("sonderr default atomic chat plugin", () => {
  test("injects atomic chat without registering an external plugin origin", () => {
    ...
    SonderrDefaultPlugins.apply(cfg, { disabled: false })
  ...
  test("does not add atomic chat plugin when default plugins are disabled", () => {
    ...
    SonderrDefaultPlugins.apply(cfg, { disabled: true })
