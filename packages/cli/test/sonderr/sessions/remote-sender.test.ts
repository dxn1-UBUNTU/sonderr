...
import { RemoteCommand } from "../../../src/sonderr-sessions/remote-command"
import { RemoteModelCatalog } from "../../../src/sonderr-sessions/remote-model-catalog"
import { RemoteSender } from "../../../src/sonderr-sessions/remote-sender"
import type { RemoteWS } from "../../../src/sonderr-sessions/remote-ws"
import type { RemoteProtocol } from "../../../src/sonderr-sessions/remote-protocol"
...
import { SonderrSessionPromptQueue } from "../../../src/sonderr/session/prompt-queue"
...
describe("RemoteSender", () => {
  ...
  test("drop_queued_message drops a waiting message and ACKs success", async () => {
    ...
    expect(SonderrSessionPromptQueue.snapshot(sessionID)).toEqual([queuedID])
  ...
  test("send_message keeps sonderr-prefixed model unchanged before internal conversion", async () => {
    const { conn } = fakeConn()
    const calls: SessionPrompt.PromptInput[] = []
    const sender = RemoteSender.create({
      conn,
    ...
    sender.handle({
      type: "command",
      id: "req_model_sonderr",
      command: "send_message",
      data: {
        sessionID: "ses_x",
        parts: [{ type: "text", text: "hello" }],
        model: "sonderr/gpt-5-mini",
      },
    })
    ...
    expect(calls).toEqual([
      {
        sessionID: SessionID.make("ses_x"),
        parts: [{ type: "text", text: "hello" }],
        model: { providerID: ProviderV2.ID.make("sonderr"), modelID: ModelV2.ID.make("gpt-5-mini") },
        ephemeralTools: { interactive_terminal: false },
      },
    ])
  })
  ...
  test("send_message does not special-case sonderr-prefixed model", async () => {
    const { conn, sent } = fakeConn()
    const calls: SessionPrompt.PromptInput[] = []
    const sender = RemoteSender.create({
      conn,
    ...
    expect(sent[0]).toEqual({ type: "response", id: "req_model_sonderr", result: {} })
    expect(calls).toEqual([
      {
        sessionID: SessionID.make("ses_x"),
        parts: [{ type: "text", text: "hello" }],
        model: { providerID: ProviderV2.ID.make("sonderr"), modelID: ModelV2.ID.make("sonderr/gpt-5-mini") },
        ephemeralTools: { interactive_terminal: false },
      },
    ])
  })
  ...
  test("subscribe does not replay state for sessions with no pending questions or permissions", async () => {
    ...
    spyOn(SonderrSessionPromptQueue, "snapshot").mockReturnValue([])
  ...
  test("subscribe always replays the current queue snapshot, including empty", async () => {
    ...
    expect(SonderrSessionPromptQueue.snapshot).toHaveBeenCalledWith(SessionID.make("ses_target"))
  })
  ...
  test("subscribe replays a non-empty queue snapshot for the subscribed session", async () => {
    ...
    spyOn(Suggestion, "list").mockResolvedValue([])
    spyOn(SonderrSessionPromptQueue, "snapshot").mockReturnValue(["msg_a", "msg_b"] as any)
...
// sonderr_change start - remote slash command discovery and execution
describe("RemoteSender slash commands", () => {
  ...
  test("exit_cli rejects invalid, missing, unresolved, and unavailable sessions before ACK", async () => {
    ...
    // sonderr_change - K1 W1: the new exit_cli handler requires hasSession
  ...
  test("exit_cli ACKs before invoking the worker callback in a microtask", async () => {
    ...
    try {
      const sender = RemoteSender.create({
        ...
        // sonderr_change - K1 W1: owns the target so the new detach path
  ...
  test("concurrent exit_cli requests ACK while idempotent local Exit cleans up once", async () => {
    ...
    const sender = RemoteSender.create({
      ...
      // sonderr_change - K1 W1: owns both targets; zero remaining after
  ...
  test("exit_cli logs only the error class when graceful exit fails after ACK", async () => {
    ...
    const sender = RemoteSender.create({
      ...
      // sonderr_change - K1 W1: owns the target; zero remaining; the
  ...
  test("create_session in-process attaches the new session via the attach seam", async () => {
    ...
    const sender = RemoteSender.create({
      ...
      // responsible for the on-boot attach via the SONDERR_REMOTE_ATTACH_SESSION
      // init branch in sonderr-sessions.ts.
      attachSession: async () => undefined,
    })
  ...
  test("exit_cli keeps the headless host alive when zero sessions remain and no callback is registered (sonderr remote)", async () => {
    const { conn, sent } = fakeConn()
    const order: string[] = []
    const sender = RemoteSender.create({
      conn,
  ...
  test("create_session forwards agent, model (with variant), and orgId metadata", async () => {
    ...
    sender.handle({
      ...
      data: {
        protocolVersion: 1,
        agent: "build",
        model: { providerID: "sonderr", modelID: "sonderr-auto/efficient", variant: "high" },
        orgId: org,
      },
    })
    await response.promise
    ...
    expect(createCalls).toEqual([
      {
        agent: "build",
        model: {
          id: ModelV2.ID.make("sonderr-auto/efficient"),
          providerID: ProviderV2.ID.make("sonderr"),
          variant: "high",
        },
        metadata: { orgId: org },
  ...
  test("create_session accepts each optional field alone", async () => {
    ...
    for (const [id, data] of [
      ...
      ["req_model", { protocolVersion: 1, model: { providerID: "sonderr", modelID: "m1" } }],
    ...
    expect(createCalls).toEqual([
      { agent: "plan" },
      { model: { id: ModelV2.ID.make("m1"), providerID: ProviderV2.ID.make("sonderr") } },
      { metadata: { orgId: org } },
    ])
  })
  ...
  test("create_session with cloneFromSonderrSessionId imports in-process, attaches, and never creates", async () => {
    const { conn, sent } = fakeConn()
    const dirs: string[] = []
    ...
    sender.handle({
      ...
      command: "create_session",
      sessionId: "ses_current",
      data: { protocolVersion: 1, cloneFromSonderrSessionId: "ses_cloud" },
    })
    await response.promise
    response.restore()
  ...
  test("create_session with cloneFromSonderrSessionId and no importFromCloud seam rejects without creating", () => {
    const { conn, sent } = fakeConn()
    const createCalls: unknown[] = []
    const attachCalls: SessionID[] = []
    const sender = RemoteSender.create({
  ...
  test("create_session clone still rejects unknown fields under strict schema", () => {
    ...
    sender.handle({
      ...
      id: "req_clone_strict",
      command: "create_session",
      data: { protocolVersion: 1, cloneFromSonderrSessionId: "ses_cloud", unknown: true },
    })
  ...
  test("system session.renamed applies setTitle and marks adoption", async () => {
    ...
    const { clear, consumeRenameAdoption } = await import("../../../src/sonderr-sessions/rename-adoptions")
    clear(sid)
  ...
  test("system session.renamed clears adoption mark when setTitle fails", async () => {
    ...
    const { clear, consumeRenameAdoption, markRenameAdopted } = await import(
      "../../../src/sonderr-sessions/rename-adoptions"
    )
    clear(sid)
