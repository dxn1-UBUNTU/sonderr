...
import { LocationServiceMap, type LocationServices } from "@sonderr/core/location-services"
...
const config = ConfigProvider.layerAdd(ConfigProvider.fromUnknown({ SONDERR_EXPERIMENTAL_DISABLE_FILEWATCHER: "false" }), {
  asPrimary: true,
})
...
describe("SonderrWatcher.eager", () => {
  test("skips eager location watchers for VS Code", () => {
    expect(SonderrWatcher.eager("vscode")).toBe(false)
  })

  test("skips eager location watchers for JetBrains", () => {
    expect(SonderrWatcher.eager("jetbrains")).toBe(false)
  })

  test("keeps eager location watchers for the standalone CLI", () => {
    expect(SonderrWatcher.eager("cli")).toBe(true)
    expect(SonderrWatcher.eager(undefined)).toBe(true)
  })
})
  ...
  async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        const watcher = SonderrWatcher.layer.pipe(Layer.provide(Layer.succeed(LocationServiceMap.Service, map)))
        const services = yield* Layer.build(watcher)
        const init = (directory: string) =>
          SonderrWatcher.Service.use((service) => service.init()).pipe(
