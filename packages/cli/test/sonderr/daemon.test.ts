...
import { Daemon } from "../../src/sonderr/daemon/daemon"
import { DaemonClient } from "../../src/sonderr/daemon/client"
import { tmpdir } from "../fixture/fixture"

const original = {
  state: process.env.SONDERR_TEST_DAEMON_STATE_DIR,
  log: process.env.SONDERR_TEST_DAEMON_LOG_DIR,
  disabled: process.env.SONDERR_NO_DAEMON,
}

afterEach(async () => {
  if (process.env.SONDERR_TEST_DAEMON_STATE_DIR !== original.state) await Daemon.stop().catch(() => undefined)
  restore()
})

function restore() {
  if (original.state === undefined) delete process.env.SONDERR_TEST_DAEMON_STATE_DIR
  else process.env.SONDERR_TEST_DAEMON_STATE_DIR = original.state
  if (original.log === undefined) delete process.env.SONDERR_TEST_DAEMON_LOG_DIR
  else process.env.SONDERR_TEST_DAEMON_LOG_DIR = original.log
  if (original.disabled === undefined) delete process.env.SONDERR_NO_DAEMON
  else process.env.SONDERR_NO_DAEMON = original.disabled
}

function dirs(root: string) {
  process.env.SONDERR_TEST_DAEMON_STATE_DIR = path.join(root, "state")
  process.env.SONDERR_TEST_DAEMON_LOG_DIR = path.join(root, "log")
  return {
    XDG_DATA_HOME: path.join(root, "xdg-data"),
    XDG_CONFIG_HOME: path.join(root, "xdg-config"),
    XDG_STATE_HOME: path.join(root, "xdg-state"),
    XDG_CACHE_HOME: path.join(root, "xdg-cache"),
    SONDERR_TEST_DAEMON_EPHEMERAL_PORT: "1",
  }
}
...
function cli(args: string[], env: NodeJS.ProcessEnv = {}) {
  return Bun.spawn([process.execPath, "--conditions=browser", path.join(process.cwd(), "src/index.ts"), ...args], {
    ...
    env: {
      ...process.env,
      ...env,
      SONDERR_CONFIG_CONTENT: '{"experimental":{"openTelemetry":false}}',
...
describe("daemon manager", () => {
  ...
  test("strips inherited cwd flags from daemon child command", () => {
    expect(Daemon.clean(["--conditions=browser", "--cwd", "packages/cli", "--inspect"])).toStrictEqual([
      "--conditions=browser",
      "--inspect",
    ])
    expect(Daemon.clean(["--cwd=packages/cli", "--conditions=browser"])).toStrictEqual(["--conditions=browser"])
  })

  test("does not forward bundled bun entrypoints to the daemon child", () => {
    const proc = {
      argv: ["/tmp/sonderr", "/$bunfs/root/src/index.js", "daemon", "start"],
      execArgv: ["--user-agent=sonderr/test", "--use-system-ca", "--"],
      execPath: "/tmp/sonderr",
    }
    expect(Daemon.command(undefined, proc)).toStrictEqual(["/tmp/sonderr"])
    expect(
      Daemon.command(undefined, {
        ...proc,
        argv: ["C:/tmp/sonderr.exe", "B:/~BUN/root/src/index.js", "daemon", "start"],
        execPath: "C:/tmp/sonderr.exe",
      }),
    ).toStrictEqual(["C:/tmp/sonderr.exe"])
    expect(
      Daemon.command(undefined, {
        ...proc,
        argv: ["C:/tmp/sonderr.exe", "b:\\~BUN\\root\\src\\index.js", "daemon", "start"],
  ...
  test("forwards source entrypoints to the daemon child", () => {
    expect(
      Daemon.command(undefined, {
        argv: ["/tmp/bun", "/tmp/sonderr/src/index.ts", "daemon", "start"],
        execArgv: ["--conditions=browser"],
        execPath: "/tmp/bun",
      }),
    ).toStrictEqual(["/tmp/bun", "--conditions=browser", "/tmp/sonderr/src/index.ts"])
  })
  ...
  test("does not reuse legacy fixed-password daemons", () => {
    ...
    const state: Daemon.State = {
      ...
      username: "sonderr",
      password: "sonderr",
      token: Buffer.from("sonderr:sonderr").toString("base64"),
      version: "test",
  ...
  test("starts, reuses, authenticates, and stops a daemon", async () => {
    ...
    expect(started.state?.password).not.toBe("sonderr")
    expect(started.state?.token).not.toBe(Buffer.from("sonderr:sonderr").toString("base64"))
  ...
  test.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      const source = path.join(process.cwd(), "src/sonderr/daemon/daemon.ts")
      const script = `
  ...
  test("supports console stop as a daemon stop alias", async () => {
    ...
    expect(code).toBe(0)
    expect(stdout).toContain("sonderr daemon stopped")
  ...
  test.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      const stdout = capture(proc.stdout, "Press Ctrl+C to stop the Sonderr daemon.")
      const stderr = new Response(proc.stderr).text()
      ...
      try {
        ...
        expect(await deadline(proc.exited, 10_000)).toBe(0)
        expect((await Daemon.status()).running).toBe(false)
        expect(await stdout.text).toContain("sonderr daemon started")
        await stderr
      } finally {
        if (proc.exitCode === null) proc.kill("SIGKILL")
        await proc.exited
  ...
  test("daemon client honors the escape hatch", async () => {
    await using tmp = await tmpdir()
    const started = await Daemon.start(opts(tmp.path))
    process.env.SONDERR_NO_DAEMON = "1"
