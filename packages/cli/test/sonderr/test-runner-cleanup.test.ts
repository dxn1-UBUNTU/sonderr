...
function env(marker: string) {
  const vars: NodeJS.ProcessEnv = { ...process.env, SONDERR_TEST_RUNNER_PID_FILE: marker }
  delete vars.SONDERR_TEST_PROFILE
  delete vars.SONDERR_TEST_SHARD
  delete vars.SONDERR_TEST_CLI_PATH
  return vars
}

function spawn(name: string, marker: string, args: string[] = []) {
  return Bun.spawn(
    ["bun", "run", "script/test-runner.ts", "--concurrency", "1", "--retries", "-1", ...args, `sonderr/${name}`],
    {
      cwd: root,
      env: env(marker),
      stdout: "pipe",
...
describe("test runner cleanup", () => {
  test(
    ...
    async () => {
      ...
      const state = { pid: 0 }
      const src = [
        "const marker = process.env.SONDERR_TEST_RUNNER_PID_FILE",
        'if (!marker) throw new Error("SONDERR_TEST_RUNNER_PID_FILE is required")',
      ...
      try {
        ...
        expect(await Bun.file(path.join(os.tmpdir(), `sonderr-test-data-${state.pid}`)).exists()).toBe(false)
      } finally {
        if (proc.exitCode === null) proc.kill("SIGKILL")
        await proc.exited
        await fs.rm(file, { force: true })
        if (state.pid) await remove(path.join(os.tmpdir(), `sonderr-test-data-${state.pid}`))
      }
    },
    30_000,
  )
