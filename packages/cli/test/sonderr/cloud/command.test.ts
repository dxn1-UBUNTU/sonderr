...
import type { AgentSendRequest, AgentStartRequest, MessageResult } from "@/sonderr/cloud/contracts"
import { CloudCommands } from "@/sonderr/cloud/commands"
import { CloudError } from "@/sonderr/cloud/errors"
import { Git } from "@/git"
...
it.instance(
  "assembles the default start request from Sonderr state and the current repository",
  () =>
    Effect.acquireUseRelease(
      ...
      (server) =>
        Effect.gen(function* () {
          ...
          yield* run(test.directory, "remote", "add", "origin", "git@github.com:Sonderr-Org/sonderr.git")
          ...
          const response = {
            ...
            sonderrSessionId: "ses_command_test",
            messageId: MESSAGE,
          ...
          const result = yield* CloudCommands.start(
            ...
            {
              env: { SONDERR_API_URL: server.url.origin },
              make: (options) => {
          ...
          expect(requests[0]).toEqual({
            ...
            repository: { type: "github", repo: "Sonderr-Org/sonderr" },
            options: {
              createdOnPlatform: "sonderr-cli",
              sonderrOrganizationId: ORG,
            },
          })
    ...
    ),
  {
    ...
    config: {
      default_agent: "plan",
      agent: { plan: { model: "sonderr/anthropic/command-model" } },
    },
  },
)
    ...
    Effect.acquireUseRelease(
      ...
      (server) =>
        Effect.gen(function* () {
          ...
          const response = {
            cloudAgentSessionId: SESSION,
            sonderrSessionId: "ses_stream_test",
            messageId: MESSAGE,
            delivery: "queued" as const,
...
it.instance("sends follow-ups, prints status without assistant content, and applies the result exit code", () =>
  Effect.gen(function* () {
    ...
    const deps = {
      env: { SONDERR_ORG_ID: "not-relevant-to-existing-sessions" },
      make: () => ({
        async start() {
