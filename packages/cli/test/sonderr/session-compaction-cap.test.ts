    ...
    () =>
      provideTmpdirServer(
        Effect.fnUntraced(function* ({ llm }) {
          ...
          const turnClose = yield* Deferred.make<SonderrSession.CloseReason>()
          const unsub = Bus.subscribe(SonderrSession.Event.TurnClose, (evt) => {
          ...
          expect(yield* llm.calls).toBe(SonderrSessionPrompt.MAX_COMPACTION_ATTEMPTS * 2 + 1)
...
describe("SonderrSessionPrompt.guardCompactionAttempt", () => {
  it.effect("returns { exhausted: false } and does not mutate state below the cap", () =>
    Effect.sync(() => {
      ...
      const result = SonderrSessionPrompt.guardCompactionAttempt({
        sessionID: "ses_under",
        attempts: SonderrSessionPrompt.MAX_COMPACTION_ATTEMPTS - 1,
        closeReasons,
        message: msg,
      })
  ...
  it.effect("sets close reason and attaches error once attempts reach the cap", () =>
    Effect.sync(() => {
      ...
      const result = SonderrSessionPrompt.guardCompactionAttempt({
        sessionID: "ses_cap",
        attempts: SonderrSessionPrompt.MAX_COMPACTION_ATTEMPTS,
        closeReasons,
        message: msg,
      })
