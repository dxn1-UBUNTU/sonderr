...
import type { ConfigV1 } from "@sonderr/core/v1/config/config"
import { Effect } from "effect"
import { Agent } from "../../src/agent/agent"
import { Permission } from "../../src/permission"
import { SonderrTask } from "../../src/sonderr/tool/task"
...
function expectPlan(item: Agent.Info | undefined, action: Permission.Action = "allow") {
  ...
  expect(Permission.evaluate("edit", ".sonderr/plans/fix.md", item!.permission).action).toBe(action)
}
...
// own ruleset from the same global config. SonderrTask.inherited has to carry the guarded set
...
test("plan carries guarded denies into delegated sessions under a global catch-all", async () => {
  ...
  await provideTestInstance({
    ...
    fn: async () => {
      ...
      const child = SonderrTask.merge(
        deriveSubagentSessionPermission({ parentSessionPermission: [], subagent: explore! }),
        SonderrTask.permissions(SonderrTask.inherited({ caller: plan!, session: { permission: [] }, mcp: undefined })),
      )
...
test("plan agent still hard-denies non-plan edits after user edit allow", async () => {
  ...
  await provideTestInstance({
    ...
    fn: async () => {
      ...
      expect(Permission.evaluate("edit", ".sonderr/plans/fix.md", plan!.permission).action).toBe("allow")
...
test("plan agent preserves explicit per-agent edit denies", async () => {
  ...
  expectPlan(plan)
  expect(Permission.evaluate("edit", ".sonderr/plans/private.md", plan!.permission).action).toBe("deny")
})
