...
test("project config permission keys are attributed to the local scope", async () => {
  ...
  await Bun.write(path.join(sonderr, "sonderr.json"), JSON.stringify({ permission: { bash: { "echo *": "allow" } } }))
...
test("a scalar project bash permission maps to the '*' pattern under the local scope", async () => {
  ...
  const sonderr = path.join(dir, ".sonderr")
  await fs.mkdir(sonderr, { recursive: true })
  await Bun.write(path.join(sonderr, "sonderr.json"), JSON.stringify({ permission: { bash: "allow" } }))
