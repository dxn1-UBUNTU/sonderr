...
async function withInstance(fn: () => Promise<void>) {
  ...
  const provide = spyOn(SonderrInstance, "provide").mockImplementation((input) =>
...
describe("plan follow-up", () => {
  ...
  test("ask - hides custom answer row on VS Code where the main prompt input handles typed replies", () =>
    withInstance(async () => {
      ...
      try {
        ...
        process.env.SONDERR_CLIENT = prev
      }
    }))
  ...
  test("ask - retargets prompt queue so injected message is visible in scope", () =>
    withInstance(async () => {
      const { SonderrSessionPromptQueue } = await import("../../src/sonderr/session/prompt-queue")
      ...
      SonderrSessionPromptQueue.retarget(seeded.sessionID, original)
      ...
      const scoped = SonderrSessionPromptQueue.scope(seeded.sessionID, all)
