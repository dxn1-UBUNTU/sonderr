...
import { CodeIndexManager } from "@sonderr/sonderr-indexing/engine"
import { normalizeIndexingStatus } from "@sonderr/sonderr-indexing/status"
import type { Config } from "../../src/config/config"
import { GlobalBus } from "../../src/bus/global"
import { WorkspaceV2 } from "@sonderr/core/workspace"
import { Global } from "@sonderr/core/global"
import { message } from "@sonderr/core/sonderr/fff"
import { WorkspaceContext } from "../../src/control-plane/workspace-context"
import { SonderrIndexing, IndexingModelError } from "../../src/sonderr/indexing"
import { indexingWarningKey } from "../../src/sonderr/indexing-warning"
import { IndexingWorker } from "../../src/sonderr/indexing-worker-client"
...
const sonderr: Partial<Config.Info> = {
  plugin: ["@sonderr/sonderr-indexing"],
  indexing: {
    enabled: true,
    vectorStore: "qdrant",
...
const staleSonderr: Partial<Config.Info> = {
  plugin: ["@sonderr/sonderr-indexing"],
  indexing: {
    enabled: true,
    provider: "sonderr",
...
const configDir = process.env["SONDERR_CONFIG_DIR"]
const disabled = process.env["SONDERR_DISABLE_CODEBASE_INDEXING"]
...
async function wait(read: () => Promise<SonderrIndexing.Status>, state: SonderrIndexing.Status["state"]) {
...
afterEach(async () => {
  ...
  if (configDir === undefined) delete process.env["SONDERR_CONFIG_DIR"]
  else process.env["SONDERR_CONFIG_DIR"] = configDir
  if (disabled === undefined) delete process.env["SONDERR_DISABLE_CODEBASE_INDEXING"]
  else process.env["SONDERR_DISABLE_CODEBASE_INDEXING"] = disabled
...
describe("indexing model catalog", () => {
  test("ignores a project-scoped Sonderr origin", async () => {
    await using tmp = await tmpdir({
      git: true,
      init: async (dir) => {
        const global = path.join(dir, "global")
        const project = path.join(dir, "project")
        await fs.mkdir(path.join(project, ".sonderr"), { recursive: true })
        await fs.mkdir(global, { recursive: true })
        await Bun.write(path.join(global, "sonderr.jsonc"), "{}")
        await Bun.write(
          path.join(project, ".sonderr", "sonderr.jsonc"),
          JSON.stringify({ indexing: { sonderr: { baseUrl: "http://127.0.0.1:4567" } } }),
        )
        return { global, project }
      },
    })
    process.env["SONDERR_CONFIG_DIR"] = tmp.extra.global
    const calls: string[] = []
    ...
    const response = await Server.Default().app.request("/indexing/models", {
      headers: { "x-sonderr-directory": tmp.extra.project },
    })
...
describe("indexing startup degradation", () => {
  test("keeps server routes alive when indexing initialization fails", async () => {
    ...
    process.env["SONDERR_CONFIG_DIR"] = tmp.path
  ...
  test("retains and deduplicates indexing warnings for TUI replay", async () => {
    ...
    const on = (data: {
      ...
      if (data.payload?.type !== SonderrIndexing.Warning.type) return
  ...
  test("coalesces burst indexing progress publications", async () => {
    const complete: SonderrIndexing.Status = {
      state: "Complete",
    ...
    await using tmp = await tmpdir({ git: true, config: cfg })
    process.env["SONDERR_CONFIG_DIR"] = tmp.path
    const events: SonderrIndexing.Status[] = []
    const on = (data: {
      directory?: string
      payload?: { type?: string; properties?: { status?: SonderrIndexing.Status } }
    }) => {
      if (data.directory !== tmp.path) return
      if (data.payload?.type !== SonderrIndexing.Event.type) return
    ...
    try {
      await withTestInstance({
        directory: tmp.path,
        fn: async () => expect(await wait(() => SonderrIndexing.current(), "Complete")).toEqual(complete),
      })
  ...
  test("does not publish initialized status after in-flight startup is disposed", async () => {
    ...
    try {
      await provideTestInstance({
        directory: tmp.path,
        init: Effect.promise(() => SonderrIndexing.init()),
        fn: async () => {
          await called(init)
          expect((await SonderrIndexing.current()).state).toBe("In Progress")
        },
      })
  ...
  test("keeps degraded indexing queryable but releases its failed engine", async () => {
    ...
    try {
      await provideTestInstance({
        ...
        init: Effect.promise(() => SonderrIndexing.init()),
        fn: async () => {
          const status = await wait(() => SonderrIndexing.current(), "Error")

          expect(status.state).toBe("Error")
          expect(status.message).toContain("Failed to initialize: test indexing initialization failed")
          expect(await SonderrIndexing.available()).toBe(false)
          expect(SonderrIndexing.ready()).toBe(false)
          expect(await SonderrIndexing.search("boot failure")).toEqual([])
    ...
    try {
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          expect(SonderrIndexing.ready()).toBe(false)
          expect(await SonderrIndexing.available()).toBe(false)
          const search = SonderrIndexing.search("boot failure")
  ...
  test("stays disabled when indexing enablement is unset", async () => {
    ...
    await provideTestInstance({
      directory: tmp.path,
      fn: async () => {
        const status = await wait(() => SonderrIndexing.current(), "Disabled")
        ...
        expect(await SonderrIndexing.available()).toBe(false)
        expect(SonderrIndexing.ready()).toBe(false)
        expect(await SonderrIndexing.search("disabled")).toEqual([])
  ...
  test("warns for home/root workspaces and aliases without allocating an indexing worker", async () => {
    ...
    for (const target of [path.parse(process.cwd()).root, Global.Path.home]) {
      ...
      for (const directory of [target, link]) {
        await provideTestInstance({
          directory,
          fn: async () => {
            expect(await SonderrIndexing.current()).toMatchObject({ state: "Disabled", message })
            expect(await SonderrIndexing.available()).toBe(false)
            expect(SonderrIndexing.ready()).toBe(false)
            expect(await SonderrIndexing.search("filesystem root")).toEqual([])
            const warnings = await app.request("/config/warnings", { headers: { "x-sonderr-directory": directory } })
    ...
    const warnings = await app.request("/config/warnings", { headers: { "x-sonderr-directory": tmp.path } })
    expect(warnings.status).toBe(200)
  ...
  test.each([false, true])("handles removed directories with no-workspace flag %s", async (disabled) => {
    ...
    process.env["SONDERR_CONFIG_DIR"] = tmp.path
    delete process.env["SONDERR_DISABLE_CODEBASE_INDEXING"]
    if (disabled) process.env["SONDERR_DISABLE_CODEBASE_INDEXING"] = "vscode-no-workspace"
    ...
    await provideTestInstance({
      ...
      fn: async () => {
        ...
        await SonderrIndexing.init()
        const status = await SonderrIndexing.current()
        expect(await SonderrIndexing.available()).toBe(false)
        expect(SonderrIndexing.ready()).toBe(false)
        expect(await SonderrIndexing.search("removed workspace")).toEqual([])
        expect(created).toEqual([])
        if (disabled) {
          expect(status).toMatchObject({
            state: "Disabled",
  ...
  test("does not validate the indexing model when indexing is disabled", async () => {
    ...
    const logger = Log.create({ service: "sonderr-indexing" })
    const warn = spyOn(logger, "warn")
    const key = process.env.SONDERR_API_KEY

    const config: Partial<Config.Info> = {
      ...staleSonderr,
      indexing: { ...staleSonderr.indexing, enabled: false, model: "removed/model" },
    }
    await using tmp = await tmpdir({ git: true, config })
    process.env["SONDERR_CONFIG_DIR"] = tmp.path
    process.env.SONDERR_API_KEY = "sonderr-token"
    ...
    try {
      ...
      if (key === undefined) delete process.env.SONDERR_API_KEY
      else process.env.SONDERR_API_KEY = key
      warn.mockRestore()
    }
  })

  test("disabled indexing does not resolve the Sonderr model catalog", async () => {
    ...
    const config: Partial<Config.Info> = {
      ...staleSonderr,
      indexing: {
        ...staleSonderr.indexing,
        enabled: false,
        model: "removed/model",
        sonderr: { baseUrl: "not a url" },
      },
    }
    await using tmp = await tmpdir({ git: true, config })
    process.env["SONDERR_CONFIG_DIR"] = tmp.path
  ...
  test("requires explicit VS Code consent even when repository config enables indexing", async () => {
    ...
    await using tmp = await tmpdir({ git: true, config: cfg })
    process.env["SONDERR_CONFIG_DIR"] = tmp.path
    process.env["SONDERR_PLATFORM"] = "vscode"

    try {
      await provideTestInstance({
        directory: tmp.path,
        init: Effect.promise(() => SonderrIndexing.setConsent(false)),
        fn: async () => {
          const status = await SonderrIndexing.current()
          expect(status.state).toBe("Disabled")
  ...
  test("shares consent across linked worktrees and revokes every project worker", async () => {
    ...
    try {
      await withTestInstance({
        directory: tmp.path,
        fn: async () => {
          await SonderrIndexing.setConsent(true)
          await wait(() => SonderrIndexing.current(), "Standby")
        },
      })
      await withTestInstance({
        directory: worktree,
        fn: async () => expect((await wait(() => SonderrIndexing.current(), "Standby")).state).toBe("Standby"),
      })
      expect(new Set(created)).toEqual(new Set([tmp.path, worktree]))

      await withTestInstance({
        directory: tmp.path,
        fn: () => SonderrIndexing.setConsent(false),
      })

      expect(new Set(disposed)).toEqual(new Set([tmp.path, worktree]))
      await withTestInstance({
        directory: worktree,
        fn: async () => expect((await SonderrIndexing.current()).state).toBe("Disabled"),
      })
    } finally {
      process.env["SONDERR_PLATFORM"] = "cli"
  ...
  test("enriches Sonderr provider config from env auth", async () => {
    global.fetch = (() =>
      Promise.resolve(
        new Response(
          JSON.stringify({
    ...
    const key = process.env.SONDERR_API_KEY
    const org = process.env.SONDERR_ORG_ID

    await using tmp = await tmpdir({ git: true, config: sonderr })
    process.env["SONDERR_CONFIG_DIR"] = tmp.path
    process.env.SONDERR_API_KEY = "sonderr-token"
    process.env.SONDERR_ORG_ID = "org_123"
    ...
    try {
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          expect(init.mock.calls[0]?.[0]).toMatchObject({
            embedderProvider: "sonderr",
            sonderrApiKey: "sonderr-token",
            sonderrOrganizationId: "org_123",
      ...
      if (org === undefined) delete process.env.SONDERR_ORG_ID
      else process.env.SONDERR_ORG_ID = org
      init.mockRestore()
    }
  })

  test("reports an error for an unsupported explicit Sonderr model instead of falling back", async () => {
    global.fetch = (() =>
      Promise.resolve(
        new Response(
          JSON.stringify({
    ...
    await using tmp = await tmpdir({ git: true, config: staleSonderr })
    process.env["SONDERR_CONFIG_DIR"] = tmp.path
    process.env.SONDERR_API_KEY = "sonderr-token"
    ...
    try {
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          expect(await SonderrIndexing.available()).toBe(false)
          expect(SonderrIndexing.ready()).toBe(false)
          expect(await SonderrIndexing.search("unsupported model")).toEqual([])
  ...
  test("passes a valid explicit Sonderr model through without error", async () => {
    global.fetch = (() =>
      Promise.resolve(
        new Response(
          JSON.stringify({
    ...
    const config: Partial<Config.Info> = {
      ...staleSonderr,
      indexing: { ...staleSonderr.indexing, model: "mistralai/mistral-embed-2312" },
    }
    await using tmp = await tmpdir({ git: true, config })
    process.env["SONDERR_CONFIG_DIR"] = tmp.path
    process.env.SONDERR_API_KEY = "sonderr-token"
    ...
    try {
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          const modelErr = warn.mock.calls.find((call) => IndexingModelError.isInstance(call[1]?.err))?.[1]?.err
          expect(modelErr).toBeUndefined()
          expect((await SonderrIndexing.current()).state).not.toBe("Error")
        },
      })
    } finally {
      if (key === undefined) delete process.env.SONDERR_API_KEY
  ...
  test("uses hosted dimensions for supported Sonderr models", async () => {
    global.fetch = (() =>
      Promise.resolve(
        new Response(
          JSON.stringify({
  ...
  test("leaves Sonderr model metadata unset when the hosted catalog is unavailable", async () => {
  ...
  test("does not default to Sonderr when an existing provider config is present", async () => {
    const init = spyOn(CodeIndexManager.prototype, "initialize").mockResolvedValue({ requiresRestart: false })
    const key = process.env.SONDERR_API_KEY
  ...
  test("stays disabled when VS Code starts without a workspace folder", async () => {
    await using tmp = await tmpdir({ git: true, config: cfg })
    process.env["SONDERR_CONFIG_DIR"] = tmp.path
    process.env["SONDERR_DISABLE_CODEBASE_INDEXING"] = "vscode-no-workspace"
    const init = spyOn(CodeIndexManager.prototype, "initialize")
    ...
    try {
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          expect(await SonderrIndexing.available()).toBe(false)
          expect(SonderrIndexing.ready()).toBe(false)
          expect(await SonderrIndexing.search("no workspace")).toEqual([])
          expect(init).not.toHaveBeenCalled()
        },
      })
    } finally {
