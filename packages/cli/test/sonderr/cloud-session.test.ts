...
import { importCloudSession, reportCloudImportError } from "../../src/sonderr/cloud-session"
...
const client = (imp: (params: { sessionId: string }) => Promise<ImportResult>) =>
  ({ sonderr: { cloud: { session: { import: imp } } } }) as Parameters<typeof importCloudSession>[0]
