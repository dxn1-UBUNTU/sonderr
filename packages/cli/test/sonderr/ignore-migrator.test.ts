...
import { IgnoreMigrator } from "../../src/sonderr/ignore-migrator"
  ...
  *.env  `
  ...
  describe("convertToGlob", () => {
    ...
    test("preserves simple filename pattern", () => {
      // Sonderr's * already matches any path depth
  ...
  describe("migrate", () => {
    test("returns empty permission for project without .sonderrignore", async () => {
    ...
    test("loads project .sonderrignore", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
    ...
    test("handles complex .sonderrignore file", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(
            path.join(dir, ".sonderrignore"),
            `# Secrets
secrets/
*.env
!.env.example
...
`,
  ...
  describe("loadIgnoreConfig", () => {
    test("returns empty object for project without .sonderrignore", async () => {
      await using tmp = await tmpdir()
    ...
    test("returns permission config for project with .sonderrignore", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".sonderrignore"), "secrets/")
        },
