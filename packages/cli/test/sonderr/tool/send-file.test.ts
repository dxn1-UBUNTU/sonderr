...
import { SendFileTool } from "@/sonderr/tool/send-file"
...
const status = spyOn(SonderrSessions, "remoteStatus")
...
describe("send_file tool", () => {
  test("is only available while remote is connected", () => {
    ...
    expect(SonderrToolRegistry.available(tool, agentInfo)).toBe(false)

    status.mockReturnValue({ enabled: true, connected: false })
    expect(SonderrToolRegistry.available(tool, agentInfo)).toBe(false)

    status.mockReturnValue({ enabled: true, connected: true })
    expect(SonderrToolRegistry.available(tool, agentInfo)).toBe(true)
  })
  ...
  test("sniffs correct MIME for a file with wrong extension", async () => {
    const dir = await tmpdir()
    try {
      // Actually just test a real PNG — SonderrReadObject opens by path,
  ...
  // sonderr_change start — send_file now authorizes missing files with external_directory + read
  ...
  // sonderr_change start — send_file now authorizes directories before returning a
  ...
  test("included in extra() list", () => {
    const tool = { id: "send_file" } as Tool.Def
    const extra = SonderrToolRegistry.extra(
      {
        recall: tool,
        managerModels: tool,
        memory: tool,
...
// sonderr_change start — fsService mock now includes stat + realPath for the
// missing/directory authorization sequence that runs before SonderrReadObject.file().
