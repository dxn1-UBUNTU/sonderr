...
import { NotifyUserTool } from "@/sonderr/tool/notify-user"
...
function runNotifyTool(params: { readonly message: string }, sessions: SonderrSessions.Interface) {
  const layer = Layer.mergeAll(
    Layer.succeed(SonderrSessions.Service, SonderrSessions.Service.of(sessions)),
...
describe("notify_user tool", () => {
  ...
  test("registers with id and description", async () => {
    const layer = Layer.mergeAll(
      Layer.succeed(SonderrSessions.Service, SonderrSessions.Service.of({
        init: () => Effect.void,
  ...
  test("trims message before sending", async () => {
    ...
    const sessions = SonderrSessions.Service.of({
      init: () => Effect.void,
  ...
  test("returns failure text when not connected", async () => {
    ...
    expect(result.output).toContain("not connected to Sonderr cloud")
  })
