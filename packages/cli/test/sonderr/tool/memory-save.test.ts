...
import { MemorySaveTool } from "@/sonderr/tool/memory-save"
...
const watch = process.env.SONDERR_EXPERIMENTAL_DISABLE_FILEWATCHER
...
beforeEach(() => {
  process.env.SONDERR_EXPERIMENTAL_DISABLE_FILEWATCHER = "true"
  spyOn(RemoteSender, "create").mockReturnValue({ handle() {}, dispose() {} })
})

afterEach(async () => {
  mock.restore()
  if (watch === undefined) delete process.env.SONDERR_EXPERIMENTAL_DISABLE_FILEWATCHER
  if (watch !== undefined) process.env.SONDERR_EXPERIMENTAL_DISABLE_FILEWATCHER = watch
  await resetDatabase()
})
...
describe("sonderr_memory_save", () => {
  ...
  test("defaults mutating memory tool permission to ask", () => {
    const sonderr = SonderrAgent.prepare({})

    expect(Permission.evaluate("sonderr_memory_recall", "typed", sonderr.defaultsPatch).action).toBe("ask")
    expect(Permission.evaluate("sonderr_memory_save", "remember", sonderr.defaultsPatch).action).toBe("ask")
  })

  test("remembers, corrects, and forgets explicit project memory", async () => {
    await using dir = await tmpdir({ git: true })
    await withConfig(path.join(dir.path, "global", ".sonderr"), async () => {
      ...
      const corrected = await execute(
        dir.path,
        {
          action: "correct",
          key: "sonderr_cli_tui",
          text: "Sonderr CLI should be treated as a terminal UI.",
        },
        gate,
      )
      const shown = await SonderrMemory.show({ ctx: memory })

      expect(saved.title).toBe("Sonderr memory saved: 1 op")
      expect(corrected.title).toBe("Sonderr memory correction saved: 1 op")
      expect(shown.sources.project).toContain("- sonderr_cli_tui :: Sonderr CLI is a TUI.")
      expect(shown.sources.corrections).toContain("- sonderr_cli_tui :: Sonderr CLI should be treated as a terminal UI.")

      const forgotten = await execute(dir.path, { action: "forget", query: "sonderr_cli_tui" }, gate)
      const next = await SonderrMemory.show({ ctx: memory })

      expect(forgotten.title).toBe("Sonderr memory updated: 2 removed")
      expect(next.sources.project).not.toContain("sonderr_cli_tui")
      expect(next.sources.corrections).not.toContain("sonderr_cli_tui")
      expect(asks.map((req) => req.permission)).toEqual(["sonderr_memory_save", "sonderr_memory_save", "sonderr_memory_save"])
  ...
  test("does not save when project memory is disabled", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".sonderr"), async () => {
      ...
      expect(result.title).toBe("Sonderr memory: disabled")
  ...
  test("declines user-level memory but saves project conventions", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".sonderr"), async () => {
      ...
      expect(skipped.title).toBe("Sonderr memory skipped: out of scope")
      expect(skipped.output).toContain("user-level memory is not supported yet")
      expect(blocked.title).toBe("Sonderr memory unchanged")
  ...
  test("handles malformed save inputs without corrupting memory files", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".sonderr"), async () => {
      ...
      expect(missing.title).toBe("Sonderr memory remember: no text")
      expect(forget.title).toBe("Sonderr memory forget: no query")
      expect(secret.title).toBe("Sonderr memory: error")
      expect(secret.output).toContain("rejected secret-like content")
      expect(noisy.title).toBe("Sonderr memory saved: 1 op")
