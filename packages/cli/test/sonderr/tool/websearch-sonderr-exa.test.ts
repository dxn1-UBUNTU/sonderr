...
import {
  SONDERR_EXA_URL,
  MAX_SONDERR_EXA_RESULTS,
  type SonderrExaParams,
  callSonderrExa,
} from "../../../src/sonderr/tool/websearch-sonderr-exa"
...
const runCall = async (
  ...
  recorded?: Recorded,
  sonderrToken = "sonderr-test-token",
) =>
  Effect.runPromiseExit(
    Effect.gen(function* () {
      const http = fakeHttp(respond, recorded)
      return yield* callSonderrExa(http, params, sonderrToken)
    }),
  )

describe("callSonderrExa request shape", () => {
  test("posts to SONDERR_EXA_URL with bearer token and highlights-only contents", async () => {
    ...
    expect(recorded.authorization).toBe("Bearer sonderr-test-token")
    const parsed = JSON.parse(recorded.body!)
    expect(parsed.query).toBe("drone")
    expect(parsed.type).toBe("auto")
    expect(parsed.numResults).toBe(MAX_SONDERR_EXA_RESULTS)
  ...
  test("clamps numResults at MAX_SONDERR_EXA_RESULTS", async () => {
    const recorded: Recorded = {}
    await runCall({ query: "x", numResults: 25 }, () => okJson({ results: [] }), recorded)
    expect(JSON.parse(recorded.body!).numResults).toBe(MAX_SONDERR_EXA_RESULTS)
  })
  ...
  test("SONDERR_EXA_URL is built from SONDERR_API_BASE", () => {
    expect(SONDERR_EXA_URL).toMatch(/\/api\/exa\/search$/)
  })
})

describe("callSonderrExa response formatting", () => {
...
describe("callSonderrExa error handling", () => {
  test("dies with auth-required message on 401", async () => {
    ...
    expect(String((exit as Extract<typeof exit, { _tag: "Failure" }>).cause)).toContain("sonderr auth login")
  })
