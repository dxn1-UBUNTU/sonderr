...
import {
  ...
  IMAGE_MODELS,
  DEFAULT_MODEL,
} from "../../../src/sonderr/tool/generate-image"
...
describe("generate-image provider resolver", () => {
  test("uses Sonderr cloud when Sonderr auth is present", () => {
    const result = resolveProvider({ type: "oauth", access: "sonderr-token", accountId: "org-123" }, undefined)
    expect(result).not.toBeNull()
    expect(result!.token).toBe("sonderr-token")
    expect(result!.organizationId).toBe("org-123")
    expect(result!.provider).toBe("sonderr")
    expect(result!.url).toContain("openrouter")
  })

  test("uses Sonderr cloud with API key auth", () => {
    const result = resolveProvider({ type: "api", key: "sonderr-api-key" }, undefined)
    expect(result!.token).toBe("sonderr-api-key")
    expect(result!.provider).toBe("sonderr")
  })

  test("falls back to OpenRouter with BYO key when no Sonderr auth", () => {
  ...
  test("prefers Sonderr auth over OpenRouter key", () => {
    const result = resolveProvider({ type: "oauth", access: "sonderr-token" }, "or-key")
    expect(result!.provider).toBe("sonderr")
    expect(result!.token).toBe("sonderr-token")
  })
})
