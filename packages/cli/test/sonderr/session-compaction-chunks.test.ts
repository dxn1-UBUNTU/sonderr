...
import { LLMEvent, type LLMEvent as Event } from "@sonderr/llm"
...
import { SonderrCompactionChunks } from "../../src/sonderr/session/compaction-chunks"
import { SonderrSessionCompaction } from "../../src/sonderr/session/compaction"
...
const dbfile = path.join(os.tmpdir(), `sonderr-compaction-chunks-${process.pid}-${crypto.randomUUID()}.db`)
...
describe("SonderrCompactionChunks", () => {
  test("splits oversized history into chronological chunks", async () => {
    ...
    const chunks = await Effect.runPromise(SonderrCompactionChunks.split({ messages, model, size: 2_000 }))
  ...
  test("uses runtime output cap for fallback selection and chunk budget", () => {
    ...
    expect(SonderrCompactionChunks.needed({ cfg, model, tokens: 5_000, outputTokenMax })).toBe(false)
    expect(SonderrCompactionChunks.budget({ cfg, model, outputTokenMax })).toBe(5_692)
  })
