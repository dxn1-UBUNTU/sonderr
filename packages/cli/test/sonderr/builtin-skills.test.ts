...
import * as SonderrSkill from "../../src/sonderr/skill-remove"
import { BUILTIN_SKILLS } from "../../src/sonderr/skills/builtin"
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const skill = yield* Skill.Service
      const item = yield* skill.get("sonderr-config")
      expect(item).toBeDefined()
      expect(item!.name).toBe("sonderr-config")
      expect(item!.location).toBe(Skill.BUILTIN_LOCATION)
      expect(item!.content).toContain("sonderr")
    }),
  { git: true },
)

it.instance(
  "sonderr-config is protected from removal",
  () =>
    Effect.gen(function* () {
      const skill = yield* Skill.Service
      const item = yield* skill.get("sonderr-config")
      expect(item).toBeDefined()
      expect(SonderrSkill.builtin(item!.location)).toBe(true)
    }),
  { git: true },
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const dir = path.join(instance.directory, ".sonderr", "skill", "sonderr-config")
      yield* Effect.promise(() =>
        Bun.write(
          path.join(dir, "SKILL.md"),
          `---
name: sonderr-config
description: User override of sonderr-config.
---

# Custom sonderr-config
...
`,
      ...
      const item = yield* skill.get("sonderr-config")
      expect(item).toBeDefined()
      expect(item!.description).toBe("User override of sonderr-config.")
      expect(item!.location).not.toBe(Skill.BUILTIN_LOCATION)
      expect(item!.location).toContain(path.join("skill", "sonderr-config", "SKILL.md"))
    }),
  { git: true },
)
