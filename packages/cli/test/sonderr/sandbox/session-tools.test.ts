...
import * as ToolNetwork from "@/sonderr/sandbox/network"
import { MCP } from "@/mcp"
...
const sessions = Layer.mock(Session.Service)({
  get: () => Effect.succeed(session("/workspace/project/.sonderr/worktrees/a")),
})
...
const mcp = Layer.mock(MCP.Service)({
  ...
  clients: () => Effect.succeed({}), // sonderr_change - upstream's MCP resource tools probe the clients
})
...
function fixture() {
  return Effect.gen(function* () {
    ...
    const a = path.join(main, ".sonderr", "worktrees", "a")
    const b = path.join(main, ".sonderr", "worktrees", "b")
...
mac("confines model-originated file mutations to the active worktree", () =>
  Effect.gen(function* () {
    ...
    const sonderr = yield* call(write, { filePath: state, content: "state" }, "call-state").pipe(Effect.exit)
    ...
    expect(Exit.isSuccess(sonderr)).toBe(true)
