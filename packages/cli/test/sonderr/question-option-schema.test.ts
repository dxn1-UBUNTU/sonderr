/**
 * Contract tests for Sonderr-specific fields on QuestionOption / QuestionInfo.
 *
 * packages/cli/src/question/index.ts is a shared upstream file.
 * Two Sonderr additions to the Option schema have been silently dropped by
 * upstream merges more than once:
 *
 *   1. labelKey / descriptionKey  — lost during the sonderr v1.3.x
...
describe("QuestionOption schema — Sonderr-specific field contract", () => {
  ...
  // Static source checks — guard the sonderr_change markers so a conflict
  // resolution that drops the fields is caught immediately.
  test("source declares mode as an optional field inside a sonderr_change block", () => {
    const src = fs.readFileSync(SOURCE, "utf-8")
    expect(src).toMatch(/sonderr_change start[^\n]*localization and mode selection hints/)
    expect(src).toMatch(/mode:\s*Schema\.optional\(Schema\.String\)/)
    expect(src).toMatch(/sonderr_change end/)
  })

  test("source declares labelKey and descriptionKey inside a sonderr_change block", () => {
    const src = fs.readFileSync(SOURCE, "utf-8")
    expect(src).toMatch(/sonderr_change start[^\n]*localization and mode selection hints/)
