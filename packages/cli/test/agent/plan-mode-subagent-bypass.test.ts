...
import { SonderrTask } from "../../src/sonderr/tool/task" // sonderr_change
...
// sonderr_change start - preserve Plan edit/notebook ceilings across Sonderr task delegation,
...
it.instance("Plan delegation preserves notebook ceilings without projecting bash denies", () =>
  Effect.gen(function* () {
    ...
    const rules = SonderrTask.inherited({
      caller: caller!,
      session: { permission: [] } as unknown as Parameters<typeof SonderrTask.inherited>[0]["session"],
      mcp: {},
    })
  ...
  () =>
    Effect.gen(function* () {
      ...
      const inherited = (caller: Agent.Info) =>
        SonderrTask.inherited({
          caller,
          session: { permission: [] } as unknown as Parameters<typeof SonderrTask.inherited>[0]["session"],
          mcp: {},
        })
      const effective = (caller: Agent.Info) =>
        Permission.merge(explore!.permission, SonderrTask.permissions(inherited(caller)))
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const effective = Permission.merge(worker!.permission, SonderrTask.permissions(rules))
      expect(Permission.evaluate("bash", "git status", effective).action).toBe("allow")
