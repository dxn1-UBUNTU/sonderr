...
import { MCP } from "../../src/mcp" // sonderr_change
...
const agentLayer = (flags: Partial<RuntimeFlags.Info> = {}) =>
  AppNodeBuilder.build(Agent.node, [
    [MCP.node, Layer.mock(MCP.Service)({})], // sonderr_change
...
const scout = testEffect(agentLayer({ experimentalScout: true })) // sonderr_change
...
it.instance("returns default native agents when no config", () =>
  Effect.gen(function* () {
    ...
    expect(names).not.toContain("scout") // sonderr_change
...
it.instance("build agent has correct default properties", () =>
  Effect.gen(function* () {
    ...
    expect(evalPerm(build, "interactive_terminal")).toBe("allow") // sonderr_change
  }),
)

it.instance("plan agent denies edits except .sonderr/plans/*", () =>
  Effect.gen(function* () {
    const plan = yield* load((svc) => svc.get("plan"))
    expect(plan).toBeDefined()
    // Wildcard is denied
    expect(evalPerm(plan, "edit")).toBe("deny")
    expect(evalPerm(plan, "interactive_terminal")).toBe("deny") // sonderr_change
    // But specific path is allowed
    expect(Permission.evaluate("edit", ".sonderr/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance("explore agent denies edit and write", () =>
  Effect.gen(function* () {
    ...
    expect(evalPerm(explore, "interactive_terminal")).toBe("deny") // sonderr_change
  }),
)
...
// sonderr_change start - Scout is opt-in and owns repository research permissions
...
scout.instance(
  ...
  {
    config: {
      // sonderr_change - Scout-backed Sonderr agents use the supported references config
      references: {
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".sonderr", "skill", "perm-skill") // sonderr_change
...
`,
      ...
      process.env.SONDERR_TEST_HOME = test.directory
