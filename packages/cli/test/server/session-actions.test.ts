...
import { ConfigProvider, Effect, Fiber, Layer } from "effect" // sonderr_change
import { BackgroundJob } from "@/background/job" // sonderr_change
import { Session as SessionNs } from "@/session/session"
import { disposeAllInstances, TestInstance } from "../fixture/fixture"
import { pollWithTimeout, testEffect } from "../lib/effect" // sonderr_change
import { httpApiLayer, requestInDirectory } from "./httpapi-layer"

// sonderr_change start - provide the background-job service for promotion coverage
const it = testEffect(
  Layer.mergeAll(
    ...
    Layer.provide(
      ConfigProvider.layer(
        ConfigProvider.fromUnknown({
          SONDERR_EXPERIMENTAL_BACKGROUND_SUBAGENTS: "false",
...
describe("session action routes", () => {
  // sonderr_change start - background subagents are enabled by default
  it.instance(
  ...
  disabled.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const res = yield* requestInDirectory(`/sonderr/background-jobs/${job.id}/promote`, test.directory, {
          method: "POST",
        })
  ...
  // sonderr_change start - verify HTTP promotion of a running task
  it.instance(
