...
describe("HttpApi workspace proxy", () => {
  ...
  it.live("strips sonderr-internal headers and merges extra headers", () =>
    Effect.gen(function* () {
      ...
      expect(forwarded["x-sonderr-directory"]).toBeUndefined()
      expect(forwarded["x-sonderr-workspace"]).toBeUndefined()
