...
import * as Sse from "effect/unstable/encoding/Sse" // sonderr_change - decode the legacy SSE wire format
import { EventPaths } from "../../src/server/routes/instance/httpapi/groups/event"
// sonderr_change start - verify transformed EventV2 values at the legacy SSE boundary
...
// sonderr_change start - inspect the real global SSE envelope
...
// sonderr_change start - instance SSE also carries Sonderr's legacy Bus events and `sync` envelopes
...
/** Skip Sonderr's ambient instance events (indexing.status, sync envelopes, ...) until `type` shows up. */
...
// sonderr_change start - read transformed values from the global SSE wire payload
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        // sonderr_change - the instance stream also carries Sonderr's ambient events (indexing.status, sync
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        // sonderr_change - skip ambient instance events that may interleave before session.created
        expect(yield* readEventOfType(reader, "session.created")).toMatchObject({ type: "session.created" })
      }),
    { git: true, config: { formatter: false, lsp: false } },
  )

  // sonderr_change start - transformed EventV2 data is numeric on legacy SSE while domain data stays decoded
  ...
  v2.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        // sonderr_change - session.next.prompted is a durable event whose projector writes a session_message
        ...
        const sessionID = yield* Effect.promise(async () => {
          const created = await Server.Default().app.request("/session", {
            method: "POST",
            headers: { "x-sonderr-directory": directory, "content-type": "application/json" },
            body: "{}",
          })
        ...
        yield* events.publish(SessionEvent.Prompted, {
          ...
          delivery: "queue",
          prompt: Prompt.make({ text: "hello", files: [], agents: [] }), // sonderr_change - Prompt is a struct
        })
