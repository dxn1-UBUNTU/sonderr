...
import { PtyID } from "@sonderr/core/pty/schema"
...
// sonderr_change start - route PTY tests must not compete with per-project indexing workers.
...
describe("pty HttpApi bridge", () => {
  test("serves available shell list through experimental Effect routes", async () => {
    ...
    const response = await app().request(PtyPaths.shells, { headers: { "x-sonderr-directory": tmp.path } })
  ...
  testPty("serves PTY JSON routes through experimental Effect routes", async () => {
    await using tmp = await tmpdir({ git: true, config: { formatter: false, lsp: false } })
    const headers = { "x-sonderr-directory": tmp.path }
    const list = await app().request(PtyPaths.list, { headers })
    expect(list.status).toBe(200)
    expect(await list.json()).toEqual([])

    // sonderr_change start - test initial spawn dimensions
  ...
  testPty("hides exited sessions on the legacy surface", async () => {
    ...
    // sonderr_change start - exit propagation can exceed 5s on a loaded CI shard; the loop
  ...
  // sonderr_change start - location disposal must preserve the process-wide PTY registry.
  ...
  test("returns 404 for missing PTY websocket before upgrade", async () => {
    ...
    const response = await app().request(PtyPaths.connect.replace(":ptyID", PtyID.ascending()), {
      headers: { "x-sonderr-directory": tmp.path },
    })
  ...
  // sonderr_change start - portable coverage for the exact legacy routes used by regular Agent Manager terminals
