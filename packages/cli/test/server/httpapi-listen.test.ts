...
const auth = { username: "sonderr", password: "listen-secret" }
...
async function startListener() {
  Flag.SONDERR_SERVER_PASSWORD = auth.password
  Flag.SONDERR_SERVER_USERNAME = auth.username
  process.env.SONDERR_SERVER_PASSWORD = auth.password
  process.env.SONDERR_SERVER_USERNAME = auth.username
...
async function requestTicket(
  ...
  const response = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", id), listener.url), {
    ...
    headers: {
      ...
      ...(options?.ticketHeader === false ? {} : { "x-sonderr-ticket": "1" }),
...
async function createCat(listener: Awaited<ReturnType<typeof startListener>>, dir: string) {
  ...
  expect(response.status).toBe(200)
  return (await response.json()) as { id: string; pid: number } // sonderr_change
}
...
describe("HttpApi Server.listen", () => {
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      const response = await fetch(new URL(PtyPaths.shells, listener.url), {
        headers: { authorization: authorization(), "x-sonderr-directory": tmp.path },
      })
      expect(response.status).toBe(200)
      ...
      // sonderr_change start - true server shutdown must terminate retained PTY processes.
  ...
  test("default in-process handler does not emit Effect HTTP response logs", async () => {
    ...
    try {
      // sonderr_change start - use an authenticated local route instead of proxy-dependent status
      Flag.SONDERR_SERVER_PASSWORD = auth.password
      Flag.SONDERR_SERVER_USERNAME = auth.username
      process.env.SONDERR_SERVER_PASSWORD = auth.password
      process.env.SONDERR_SERVER_USERNAME = auth.username
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    ...
    const previous = process.env.SONDERR_DISABLE_DEFAULT_PLUGINS
    process.env.SONDERR_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.SONDERR_DISABLE_DEFAULT_PLUGINS
      else process.env.SONDERR_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
  ...
  testPty("rejects unsafe PTY ticket mint and connect requests", async () => {
    ...
    try {
      ...
      const ambiguous = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", info.id), listener.url), {
        method: "POST",
        headers: { authorization: authorization(), "x-sonderr-ticket": "1" },
      })
