...
import { pollWithTimeout, testEffect } from "../lib/effect" // sonderr_change

const originalWorkspaces = Flag.SONDERR_EXPERIMENTAL_WORKSPACES
...
// sonderr_change start - released V2 clients persisted media-shaped tool content
...
afterEach(async () => {
  Flag.SONDERR_EXPERIMENTAL_WORKSPACES = originalWorkspaces
...
describe("session HttpApi", () => {
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = { "x-sonderr-directory": test.directory }
  ...
  // sonderr_change start - protect mixed-version session database compatibility
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const response = yield* request(`/api/session/${session.id}/message`, {
          headers: { "x-sonderr-directory": test.directory },
        })
        expect(response.status).toBe(200)
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const headers = { "x-sonderr-directory": test.directory, "content-type": "application/json" }
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        Flag.SONDERR_EXPERIMENTAL_WORKSPACES = true
        ...
        const created = yield* requestJson<Session.Info>(`${SessionPaths.create}?workspace=${workspace.id}`, {
          method: "POST",
          headers: { "x-sonderr-directory": test.directory, "content-type": "application/json" },
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const currentDir = path.join(test.directory, "packages", "sonderr", "src")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = { "x-sonderr-directory": hint, "content-type": "application/json" }
        const created = yield* requestJson<Session.Info>(SessionPaths.create, {
          method: "POST",
          headers,
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = { "x-sonderr-directory": globalWorktreeSentinel, "content-type": "application/json" }
  ...
  // sonderr_change start - deleting a prompt that already started is a successful no-op
  it.live(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        const headers = { "x-sonderr-directory": dir, "content-type": "application/json" }
    ...
    30_000, // sonderr_change - windows CI needs headroom beyond 10s
  )
  // sonderr_change end
