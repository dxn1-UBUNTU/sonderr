...
// sonderr_change start - PTY route tests do not need an indexing worker per temp project;
...
function request(route: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-sonderr-directory", directory)
...
const directoryHeader = (dir: string) => HttpClientRequest.setHeader("x-sonderr-directory", dir)
...
describe("v2 pty HttpApi", () => {
  ...
  // sonderr_change start - portable live PTY coverage on Linux, macOS, and Windows CI
  ...
  ;(process.platform === "win32" ? effectIt.live.skip : effectIt.live)(
    ...
    () =>
      Effect.gen(function* () {
        const dir = yield* tmpdirScoped({ git: true, config: { formatter: false, lsp: false } })
        // sonderr_change start - verify child env precedence and credential stripping through the canonical PTY route
        const previous = {
          password: process.env.SONDERR_SERVER_PASSWORD,
          username: process.env.SONDERR_SERVER_USERNAME,
        }
        yield* Effect.acquireRelease(
          Effect.sync(() => {
            process.env.SONDERR_SERVER_PASSWORD = "host-password"
            process.env.SONDERR_SERVER_USERNAME = "host-username"
          }),
          () =>
            Effect.sync(() => {
              if (previous.password === undefined) delete process.env.SONDERR_SERVER_PASSWORD
              else process.env.SONDERR_SERVER_PASSWORD = previous.password
              if (previous.username === undefined) delete process.env.SONDERR_SERVER_USERNAME
              else process.env.SONDERR_SERVER_USERNAME = previous.username
            }),
        )
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            [
              ...
              '    output.env.SONDERR_TERMINAL = "plugin"',
              '    output.env.SONDERR_PTY_ID = "plugin"',
              '    output.env.SONDERR_SERVER_PASSWORD = "plugin-password"',
              '    output.env.SONDERR_SERVER_USERNAME = "plugin-username"',
        ...
        const created = yield* HttpClientRequest.post("/api/pty").pipe(
          ...
          HttpClientRequest.bodyJson({
            ...
            args: [
              "-c",
              'printf "%s|%s|%s|%s|%s|%s|%s|%s|%s\\n" "$CALLER" "$SHARED" "$PLUGIN" "$TERM" "$SONDERR_TERMINAL" "$SONDERR_PTY_ID" "${SONDERR_SERVER_PASSWORD-unset}" "${SONDERR_SERVER_USERNAME-unset}" "$HOOK_CWD"; sleep 5',
            ],
            cwd,
            env: {
              CALLER: "caller",
              SHARED: "caller",
              TERM: "caller",
              SONDERR_TERMINAL: "caller",
              SONDERR_PTY_ID: "caller",
              SONDERR_SERVER_PASSWORD: "caller-password",
              SONDERR_SERVER_USERNAME: "caller-username",
            },
          }),
          Effect.flatMap(HttpClient.execute),
        )
    ...
    30_000, // sonderr_change - external plugin loading and websocket setup can exceed Bun's 5s default
  )
})
