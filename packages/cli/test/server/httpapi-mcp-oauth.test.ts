...
const TestHttpApi = HttpApi.make("sonderr-instance").addHttpApi(McpApi)
...
const testMcpHandlers = HttpApiBuilder.group(TestHttpApi, "mcp", (handlers) =>
  Effect.succeed(
    handlers
      ...
      .handle("disconnect", () => Effect.die("unexpected MCP disconnect")) // sonderr_change
      .handle("readResource", () => Effect.die("unexpected MCP readResource")) // sonderr_change
      .handle("callTool", () => Effect.die("unexpected MCP callTool")), // sonderr_change
  ),
)
