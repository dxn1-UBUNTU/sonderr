...
describe("reference HttpApi", () => {
  ...
  // sonderr_change start - reference reads must reconcile config changes after instance disposal.
  test("refreshes references after project config updates", async () => {
    ...
    const headers = { "content-type": "application/json", "x-sonderr-directory": tmp.path }
  ...
  // sonderr_change start - direct clients must observe effective Sonderr config before Agent initialization.
  test("lists SONDERR_CONFIG_CONTENT references with metadata on the first request", async () => {
    const previous = process.env.SONDERR_CONFIG_CONTENT
    ...
    try {
      ...
      if (previous === undefined) delete process.env.SONDERR_CONFIG_CONTENT
      else process.env.SONDERR_CONFIG_CONTENT = previous
    }
  })
  // sonderr_change end
})
