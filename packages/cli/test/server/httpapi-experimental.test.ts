...
import { realpath } from "node:fs/promises" // sonderr_change
...
import { AccountV2 } from "@sonderr/core/account"
import { AccountTable } from "@sonderr/core/account/sql"
...
function withCreatedWorktree(
  ...
  return Effect.acquireUseRelease(
    Effect.gen(function* () {
      ...
      expect(info).toMatchObject({ name, branch: "sonderr/api-test" })
      ...
      Effect.gen(function* () {
        ...
        const [consoleState, consoleOrgs, toolList, toolIDs, worktrees, resources] = yield* Effect.all(
          [
            ...
            request(`${ExperimentalPaths.tool}?provider=sonderr&model=gpt-5`, directory),
      ...
      Effect.gen(function* () {
        ...
        yield* withCreatedWorktree(tmp.directory, (info) =>
          Effect.gen(function* () {
            ...
            const directory = yield* Effect.promise(() => realpath(info.directory)) // sonderr_change
            expect(yield* json(listed)).toContainEqual({ directory, managed: true }) // sonderr_change
