...
import type { Method, RequestSpec } from "./types" // sonderr_change
...
export async function disposeApps() {
  // sonderr_change start - an in-flight SSE fiber can leave the in-process router scope unable
...
function toAuthProbeRequest(scenario: ActiveScenario, credentials: "missing" | "valid", signal: AbortSignal) {
  ...
  const headers = {
    ...
    ...spec.headers,
    ...(credentials === "valid" ? { authorization: basic("sonderr", "secret") } : {}), // sonderr_change
  }
  return new Request(new URL(spec.path, "http://localhost"), {
    method: scenario.method,
    headers,
