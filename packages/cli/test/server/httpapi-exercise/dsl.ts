...
class ScenarioBuilder<S = undefined> {
  ...
  constructor(method: Method, path: string, name: string, auth: AuthPolicy) {
    this.state = {
      ...
      path,
      name,
      // sonderr_change start - default to an in-memory project dir; opt into git init only for routes
      ...
      authProbe: undefined,
      validAuthProbe: true, // sonderr_change
      capture: "full",
      mutates: false,
      reset: true,
      auth,
  ...
  // sonderr_change start - blocking routes only prove they reject missing credentials so no valid request leaks into app disposal
  skipValidAuthProbe() {
  ...
  private done(
    ...
    return {
      ...
      validAuthProbe: state.validAuthProbe, // sonderr_change
