 ...
 *
 * The script intentionally isolates `SONDERR_DB` before importing modules that touch
 ...
 */
import { Effect, Layer } from "effect" // sonderr_change
...
import { coverageResult, parseOptions, routeKey, routeKeys, selectedScenarios, shardScenarios } from "./routing" // sonderr_change
import { runScenario } from "./runner"
import { disposeApps } from "./backend"
import { publicApi, runtime } from "./runtime" // sonderr_change
import { type Scenario, type Result } from "./types"
import { sonderrScenarios } from "../../sonderr/server/httpapi-exercise-scenarios" // sonderr_change
import { sessionAfterDefaultAgent } from "../../sonderr/server/httpapi-exercise-ready" // sonderr_change
...
const scenarios: Scenario[] = [
  ...
  http.protected
    ...
    .seeded(() =>
      Effect.promise(() =>
        Bun.write(
          path.join(exerciseConfigDirectory, "sonderr.jsonc"),
      ...
      (body) =>
        Effect.gen(function* () {
          ...
          const text = yield* Effect.promise(() =>
            Bun.file(path.join(exerciseConfigDirectory, "sonderr.jsonc")).text(),
          )
  ...
  // sonderr_change start - opt into git init; the worktree assertion requires a git-backed project
  http.protected.get("/path", "path.get").inProject({ git: true }).json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-sonderr-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-sonderr-directory")
  }),
  // sonderr_change end
  ...
  http.protected
    ...
    .mutating()
    // sonderr_change start - isolated git project: a non-git directory resolves to the
  ...
  http.protected.get("/sonderr/notebook", "sonderr.notebook.list").json(200, array),
  http.protected
    .post("/sonderr/notebook/{requestID}/reply", "sonderr.notebook.reply")
    .at((ctx) => ({
      path: route("/sonderr/notebook/{requestID}/reply", { requestID: "nbr_httpapi_reply" }),
      headers: ctx.headers(),
      body: {
        result: {
          operation: "read",
    ...
    .json(404, object, "status"),
  http.protected
    .post("/sonderr/notebook/{requestID}/reject", "sonderr.notebook.reject")
    .at((ctx) => ({
      path: route("/sonderr/notebook/{requestID}/reject", { requestID: "nbr_httpapi_reject" }),
      headers: ctx.headers(),
      body: { error: { code: "not_found", message: "Notebook not found" } },
    }))
    .json(404, object, "status"),
  http.protected.get("/sonderr/agent-manager", "sonderr.agentManager.list").json(200, array),
  http.protected
    .post("/sonderr/agent-manager/{requestID}/reply", "sonderr.agentManager.reply")
    .at((ctx) => ({
      path: route("/sonderr/agent-manager/{requestID}/reply", { requestID: "amr_httpapi_reply" }),
      headers: ctx.headers(),
      body: { result: { operation: "overview", overview: { sections: [], ungrouped: [] } } },
    }))
    .json(404, object, "status"),
  http.protected
    .post("/sonderr/agent-manager/{requestID}/reject", "sonderr.agentManager.reject")
    .at((ctx) => ({
      path: route("/sonderr/agent-manager/{requestID}/reject", { requestID: "amr_httpapi_reject" }),
      headers: ctx.headers(),
  ...
  http.protected
    ...
    .at((ctx) => ({
      path: `/experimental/tool?${new URLSearchParams({ provider: "sonderr", model: "test" })}`,
      headers: ctx.headers(),
    }))
    .json(200, array, "status"),
  http.protected.get("/experimental/tool/ids", "tool.ids").json(200, array),
  http.protected.get("/experimental/worktree", "worktree.list").inProject({ git: true }).json(200, array), // sonderr_change
  http.protected
  ...
  http.protected.get("/experimental/capabilities", "experimental.capabilities.get").json(200, (body) => {
    // sonderr_change start - background subagents are always available
    check(
  ...
  http.protected
    ...
    .at((ctx) => ({
      ...
      headers: { ...ctx.headers(), "x-sonderr-ticket": "1" },
    }))
    .json(404, object, "status"),
  http.protected
  ...
  http.protected
    ...
    .seeded((ctx) => sessionAfterDefaultAgent(ctx, "Permission create owner")) // sonderr_change
    .at((ctx) => ({
      path: route("/api/session/{sessionID}/permission", { sessionID: ctx.state.id }),
      headers: ctx.headers(),
      body: { action: "read", resources: [".env"] },
    }))
    // sonderr_change start - opt into git init; the permission resolver consults VCS state for unknown resources
    .inProject({ git: true })
    // sonderr_change end
    .json(200, (body) => {
      object(body)
      ...
      check(body.data.effect === "ask", `permission create should create a pending request, got ${JSON.stringify(body.data.effect)}`) // sonderr_change
    }),
  http.protected
  ...
  http.protected
    ...
    .at((ctx) => ({
      ...
      headers: { ...ctx.headers(), "content-type": "application/json" },
      body: { model: { providerID: "sonderr", id: "big-pickle" } },
    }))
    .status(204, undefined, "none"),
  http.protected
  ...
  http.protected
    ...
    .status(500, undefined, "status"), // sonderr_change
  http.protected
  ...
  http.protected
    ...
    .skipValidAuthProbe() // sonderr_change - valid requests intentionally block waiting for queued TUI input
    .mutating()
  ...
  http.protected
    ...
    .status(400),
  ...sonderrScenarios, // sonderr_change
]
...
const main = Effect.gen(function* () {
  ...
  // sonderr_change start - coverage only needs the OpenAPI group; bypass runtime + LLM + DB allocation
  ...
  // sonderr_change end
  // sonderr_change start - dispose final non-mutating instances so shared test scopes can close; skip when coverage returned early so runtime() is never imported in static mode
  ...
  const results: Result[] = yield* Effect.forEach(
    ...
    (scenario) =>
      Effect.gen(function* () {
        ...
        // sonderr_change start - retry a failed scenario before failing the pass, mirroring
        ...
        if (scenario.kind !== "active" || !scenario.reset) return result
...
// sonderr_change start - route-only coverage must not acquire a listening fake LLM server; effect/auth runs alone
