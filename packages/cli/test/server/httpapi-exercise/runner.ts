...
function runActive(options: Options, scenario: ActiveScenario) {
  ...
  return withContext(options, scenario, "shared", (ctx) =>
    Effect.gen(function* () {
      ...
      // sonderr_change start - append the actual response to assertion failures so CI logs
...
function runAuth(scenario: ActiveScenario) {
  return Effect.gen(function* () {
    ...
    if (scenario.auth === "protected") {
      ...
      if (!scenario.validAuthProbe) return // sonderr_change - blocking routes skip the valid probe; a leaked valid request hangs final app disposal
  ...
  return Effect.acquireRelease(
    ...
    Effect.flatMap((context) =>
      Effect.gen(function* () {
        ...
        // sonderr_change start - headers closure extracted so scenarios can build their own requests
        const headers = (extra?: Record<string, string>) => ({
          ...(context.dir?.path ? { "x-sonderr-directory": context.dir.path } : {}),
          ...extra,
        })
        // sonderr_change end
        const base: ScenarioContext = {
          directory: context.dir?.path,
          headers, // sonderr_change
          file: (name, content) =>
            Effect.promise(() => {
...
const resetState = Effect.promise(async () => {
  ...
  // sonderr_change - each exerciser process already owns an isolated DB; unlinking it between scenarios races async Sonderr callbacks
  await Bun.sleep(25)
})
