...
import { awaitWithTimeout, pollWithTimeout, testEffect } from "../lib/effect" // sonderr_change
...
type Sdk = ReturnType<typeof createSonderrClient>
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.SONDERR_SERVER_PASSWORD = input?.password
      Flag.SONDERR_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".sonderr", "skills", "project-rest-skill", "SKILL.md"), // sonderr_change
      `---
...
describe("HttpApi SDK", () => {
  ...
  httpapiInstance(
    ...
    ({ sdk, directory }) =>
      Effect.gen(function* () {
        ...
        const raw = yield* call(() => sdk.v2.fs.read({ path: "hello.txt" })) // sonderr_change
        const session = yield* call(() => sdk.session.create({ title: "sdk" }))
        const v2session = yield* call(() => sdk.v2.session.create({ agent: "build" })) // sonderr_change
        ...
        expect({ status: v2session.response!.status, error: v2session.error }).toEqual({ status: 200, error: undefined }) // sonderr_change
        expect(v2session.data).toMatchObject({ data: { location: { directory } } }) // sonderr_change
  ...
  httpapi(
    ...
    withProject("raw", { setup: writeStandardFiles }, ({ directory }) =>
      Effect.gen(function* () {
        ...
        expect(request!.headers.has("x-sonderr-directory")).toBe(false)
        expect(request!.headers.has("x-sonderr-workspace")).toBe(false)

        // sonderr_change start - encoded legacy directory headers still route on payload requests
        const legacy = yield* client("raw", undefined, {
          headers: { "x-sonderr-directory": encodeURIComponent(directory) },
        })
  ...
  httpapiInstance(
    ...
    ({ directory }) =>
      Effect.gen(function* () {
        ...
        // sonderr_change start - match Hono AuthMiddleware username default ("sonderr")
        const badSdk = yield* client("raw", directory, {
          password: "secret",
          headers: { authorization: authorization("sonderr", "wrong") },
        })
        const bad = yield* capture(() => badSdk.file.read({ path: "hello.txt" }))
        const goodSdk = yield* client("raw", directory, {
          password: "secret",
          headers: { authorization: authorization("sonderr", "secret") },
        })
  ...
  // sonderr_change start - verify invalid user images fail at the real SDK boundary
  ...
  // sonderr_change start - verify provider errors remain in successful assistant messages
