...
import type { Model, Provider } from "@sonderr/sdk/v2"
...
test("discovers Modal workspace models", async () => {
        ...
        data: [
          {
            ...
            interleaved: {
              field: "reasoning_text", // sonderr_change - accept Modal's supported alternate field
            },
          },
          {
  ...
  expect(model.capabilities.interleaved).toEqual({ field: "reasoning_text" }) // sonderr_change
...
// sonderr_change start - discovery failures must retain the static provider catalog
