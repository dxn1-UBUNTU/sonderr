...
import type { HostPluginApi } from "@sonderr/tui/plugin/slots"
...
type Opts = {
  ...
  state?: {
    ...
    globalConfig?: HostPluginApi["state"]["globalConfig"] // sonderr_change
...
export function createTuiPluginApi(opts: Opts = {}): HostPluginApi {
  ...
  let selected = opts.theme?.selected ?? "sonderr"
  const set =
    opts.theme?.set ??
    ((name: string) => {
  ...
  return {
    ...
    attention: {
      ...
      soundboard: {
        ...
        current: () => opts.attention?.soundboard?.current?.() ?? "sonderr.default", // sonderr_change
    ...
    state: {
      ...
      session: {
        ...
        processes: opts.state?.session?.processes ?? (() => []), // sonderr_change
