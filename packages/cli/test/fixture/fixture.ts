...
import { ProjectV2 } from "@sonderr/core/project" // sonderr_change
import { ProjectTable } from "@sonderr/core/project/sql" // sonderr_change
...
import { context as instanceContext, type InstanceContext } from "../../src/project/instance-context" // sonderr_change
...
export async function provideTestInstance<R>(input: {
  ...
  try {
    ...
    return await instanceContext.provide(ctx, () => input.fn(ctx)) // sonderr_change
  } finally {
...
export async function withTestInstance<R>(input: { directory: string; fn: (ctx: InstanceContext) => R }) {
  const ctx = await runTestInstanceStore((store) => store.load({ directory: input.directory }))
  return instanceContext.provide(ctx, () => input.fn(ctx)) // sonderr_change
}
...
// sonderr_change start - dispose a directory's instance (and its watchers) before the directory is deleted
...
function clean(dir: string) {
  return cleanup(dir) // sonderr_change
}
...
export async function tmpdir<T>(options?: TmpDirOptions<T>) {
  const dirpath = sanitizePath(path.join(os.tmpdir(), "sonderr-test-" + Math.random().toString(36).slice(2)))
  await fs.mkdir(dirpath, { recursive: true })
  if (options?.git) {
    await $`git init`.cwd(dirpath).quiet()
    await $`git config core.fsmonitor false`.cwd(dirpath).quiet()
    await $`git config commit.gpgsign false`.cwd(dirpath).quiet()
    await $`git config user.email "test@sonderr.test"`.cwd(dirpath).quiet()
    await $`git config user.name "Test"`.cwd(dirpath).quiet()
    await $`git commit --allow-empty -m "root commit ${dirpath}"`.cwd(dirpath).quiet()
  }
  if (options?.config) {
    await Bun.write(
      path.join(dirpath, "sonderr.json"),
      JSON.stringify({
  ...
  const result = {
    [Symbol.asyncDispose]: async () => {
      try {
        ...
        await disposeInstancesFor(realpath) // sonderr_change - see disposeInstancesFor
  ...
  return Effect.gen(function* () {
    ...
    yield* Effect.addFinalizer(() =>
      Effect.promise(async () => {
        await disposeInstancesFor(dir) // sonderr_change - see disposeInstancesFor
    ...
    if (options?.git) {
      ...
      yield* git("config", "user.email", "test@sonderr.test")
    ...
    if (options?.config) {
      ...
      yield* Effect.promise(() =>
        fs.writeFile(
          ...
          JSON.stringify({ $schema: "https://app.kilo.ai/config.json", ...resolved }), // sonderr_change
        ),
      )
    }
...
// sonderr_change start - custom test runtimes need the instance project in their core database
