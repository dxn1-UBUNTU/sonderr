...
import * as launch from "../../src/lsp/launch" // sonderr_change - spy on spawn
import { CrossSpawnSpawner } from "@sonderr/core/cross-spawn-spawner"
import { provideTestInstance, provideTmpdirInstance, TestInstance, tmpdir } from "../fixture/fixture" // sonderr_change
import { awaitWithTimeout, testEffect } from "../lib/effect"
import { type InstanceContext } from "../../src/project/instance-context"
import { Flag } from "@sonderr/core/flag/flag" // sonderr_change
import { TsCheck } from "../../src/sonderr/ts-check" // sonderr_change

// sonderr_change - Typescript.spawn ignores ctx, so a cast is fine here.
...
describe("lsp.spawn", () => {
  ...
  // sonderr_change start - provide the runtime flag so spawn() is reached past the TsClient short-circuit
  ...
  // sonderr_change start - Typescript spawn is gated behind SONDERR_EXPERIMENTAL_LSP_TOOL.
  test("spawns tsgo LSP when SONDERR_EXPERIMENTAL_LSP_TOOL is enabled", async () => {
    const saved = Flag.SONDERR_EXPERIMENTAL_LSP_TOOL
    Flag.SONDERR_EXPERIMENTAL_LSP_TOOL = true
    ...
    try {
      ...
      Flag.SONDERR_EXPERIMENTAL_LSP_TOOL = saved
      spawnSpy.mockRestore()
      tsgoSpy.mockRestore()
    }
  })

  test("Typescript.spawn returns undefined when SONDERR_EXPERIMENTAL_LSP_TOOL is off", async () => {
    const saved = Flag.SONDERR_EXPERIMENTAL_LSP_TOOL
    Flag.SONDERR_EXPERIMENTAL_LSP_TOOL = false
    try {
