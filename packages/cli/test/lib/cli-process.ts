// Subprocess test harness for the sonderr CLI. Spawns the real binary against
...
//
// Configuration flows through sonderr's built-in test affordances:
//   - SONDERR_CONFIG_CONTENT      : provider config inline, no files to find
//   - SONDERR_TEST_HOME           : pins os.homedir() → tmpdir
//   - SONDERR_DISABLE_PROJECT_CONFIG : skip walking up for sonderr.json
//   - SONDERR_PURE                : skip external plugin discovery + install
//   - SONDERR_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `sonderr.run` is fully wired. The shape supports adding more
// builders (`sonderr.serve(opts)`, `sonderr.acp(opts)`, `sonderr.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of SonderrCli.
...
import { TestCli } from "../../script/sonderr/test-cli" // sonderr_change

const sonderrRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(sonderrRoot, "src/index.ts")
// sonderr_change start - reuse the runner's once-built CLI graph instead of transpiling it in every child
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    SONDERR_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    SONDERR_CONFIG_CONTENT: configJson,
    SONDERR_DISABLE_PROJECT_CONFIG: "1",
    SONDERR_PURE: "1",
    SONDERR_DISABLE_AUTOUPDATE: "1",
    SONDERR_DISABLE_AUTOCOMPACT: "1",
    SONDERR_DISABLE_MODELS_FETCH: "1",
    SONDERR_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `sonderr run`. New flags should
...
// `sonderr serve` is a long-lived process — it never exits on its own.
...
// `sonderr acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type SonderrCli = {
  ...
  // Spawn `sonderr serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `sonderr acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("sonderr.spawn")(function* (args: string[], opts?: SpawnOpts) {
      const start = Date.now()
      const timeoutMs = opts?.timeoutMs ?? 45_000 // sonderr_change - current full CLI startup leaves less than 30s for multi-step runs
      ...
      const command = ChildProcess.make("bun", [...cliArgs, ...args], {
        ...
        extendEnv: true,
        stdin: "ignore",
        detached: false, // sonderr_change - keep test children in the runner's process lifecycle
      })
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          SONDERR_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("sonderr.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
      const options = runOpts(opts)
      const proc = yield* Effect.acquireRelease(
        Effect.sync(() =>
          Bun.spawn(["bun", ...cliArgs, ...runArgs(message, opts)], {
            // sonderr_change - cliArgs carries the solid preload
            cwd: home,
    ...
    const serve = Effect.fn("sonderr.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      const proc = yield* Effect.acquireRelease(
        Effect.sync(() =>
          Bun.spawn(["bun", ...cliArgs, ...argv], {
            ...
            stdout: "pipe",
            stderr: "pipe",
            windowsHide: true, // sonderr_change
          }),
        ),
        (p) =>
          Effect.promise(() => {
      ...
      //   "sonderr server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `sonderr serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("sonderr.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const sonderr: SonderrCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, sonderr })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `sonderr.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "sonderr") {
...
// every call site. Use this for any test that needs the sonderr CLI fixture.
//
...
// resources (e.g. `sonderr.serve`) without an extra `Effect.scoped` wrapper —
...
export const cliIt = {
  ...
  concurrent: <A, E>(
    ...
    // sonderr_change start - full CLI processes contend heavily during startup after the Effect graph migration
    test.serial(
      name,
