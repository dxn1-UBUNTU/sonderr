...
const env = Layer.mergeAll(
  ...
  AppNodeBuilder.build(InstanceStore.node, [[InstanceStore.bootstrapNode, noopBootstrap]]), // sonderr_change
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      expect(result).toEqual({ manual: false, rule: { permission: "bash", pattern: "*", action: "allow" } }) // sonderr_change - ask returns the auto-approval decision instead of void
    }),
  { git: true },
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      expect(result).toEqual({ manual: false, rule: { permission: "bash", pattern: "ls", action: "allow" } }) // sonderr_change - the persisted "always" rule auto-approves; ask reports that decision
    }),
  { git: true },
)
