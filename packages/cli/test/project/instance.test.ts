...
import { capture } from "../../src/sonderr/instance" // sonderr_change
...
describe("InstanceStore", () => {
  it.live("loads instance context", () =>
    Effect.gen(function* () {
      ...
      // sonderr_change start - capture prefers legacy ALS, then falls back to the Effect fiber reference
  ...
  it.live("runs bootstrap with InstanceRef provided", () =>
    Effect.gen(function* () {
      ...
      expect(capture()).toBeUndefined() // sonderr_change - bootstrap legacy ALS does not leak into the caller
    }),
  )
  ...
  // sonderr_change start - reload disposers retain legacy instance context
  ...
  // sonderr_change - InstanceStore.boot provides InstanceRef to bootstrap.run so
  // SonderrBootstrap (and anything it forkDetaches, e.g. SonderrIndexing.init) can read
  // the current directory. This regression test pins the Sonderr contract.
  it.live("provides InstanceRef during bootstrap for Sonderr bootstrap compatibility", () =>
    Effect.gen(function* () {
      const dir = yield* tmpdirScoped({ git: true })
      const store = yield* InstanceStore.Service
