...
const itWithoutClaudeCodeSkills = testEffect(Layer.mergeAll(skills(false, true), node, testInstanceStoreLayer)) // sonderr_change
...
const withHome = <A, E, R>(home: string, self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const prev = process.env.SONDERR_TEST_HOME
    ...
    (prev) =>
      Effect.sync(() => {
        process.env.SONDERR_TEST_HOME = prev
      }),
  )

const discovered = <T extends { location: string }>(list: readonly T[]) =>
  list.filter((skill) => ![Skill.BUILTIN_LOCATION, "<built-in>"].includes(skill.location)) // sonderr_change
...
describe("skill", () => {
  ...
  it.live("discovers skills from .sonderr/skill/ directory", () =>
    // sonderr_change end
      ...
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".sonderr", "skill", "test-skill", "SKILL.md"),
              `---
name: test-skill
...
`,
          ...
          const list = discovered(yield* skill.all()) // sonderr_change
      ...
      (dir) =>
        withHome(
          ...
          Effect.gen(function* () {
            yield* Effect.promise(() =>
              Bun.write(
                path.join(dir, ".sonderr", "skill", "dir-skill", "SKILL.md"), // sonderr_change: .sonderr is primary
                `---
name: dir-skill
...
`,
            ...
            expect(dirs).toContain(path.join(dir, ".sonderr", "skill", "dir-skill")) // sonderr_change: .sonderr is primary
  ...
  // sonderr_change start
  it.live("discovers multiple skills from .sonderr/skill/ directory", () =>
    // sonderr_change end
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              Bun.write(
                path.join(dir, ".sonderr", "skill", "skill-one", "SKILL.md"),
                `---
name: skill-one
...
`,
              ...
              Bun.write(
                path.join(dir, ".sonderr", "skill", "skill-two", "SKILL.md"),
                `---
name: skill-two
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".sonderr", "skill", "no-frontmatter", "SKILL.md"), // sonderr_change: .sonderr is primary
              `# No Frontmatter
...
`,
          ...
          const skill = yield* Skill.Service
          expect(discovered(yield* skill.all())).toEqual([]) // sonderr_change
        }),
      { git: true },
    ),
  )
  ...
  it.live("discovers skills without descriptions", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          // sonderr_change start - load .sonderr skills without falling back to .sonderr
          yield* Effect.promise(() =>
            Promise.all([
              Bun.write(
                path.join(dir, ".sonderr", "skill", "manual-skill", "SKILL.md"),
                `---
name: manual-skill
---
...
`,
              ...
              Bun.write(
                path.join(dir, ".sonderr", "skill", "ignored-skill", "SKILL.md"),
                `---
name: ignored-skill
...
`,
              ...
              // sonderr_change start
              Bun.write(
                path.join(dir, ".sonderr", "skill", "sonderr-skill", "SKILL.md"),
                `---
name: sonderr-skill
description: A skill in the .sonderr/skill directory.
---
...
`,
          ...
          const skill = yield* Skill.Service
          const list = discovered(yield* skill.all()) // sonderr_change
          expect(list.map((s) => s.name)).toEqual(["sonderr-skill"])
        }),
      { git: true },
    ),
  )
...
`,
              ...
              // sonderr_change start
              Bun.write(
                path.join(dir, ".sonderr", "skill", "agent-skill", "SKILL.md"),
                `---
name: sonderr-skill
description: A skill in the .sonderr/skill directory.
---
...
`,
              ...
              Bun.write(
                path.join(dir, ".sonderr", "skills", "agent-skill", "SKILL.md"),
                `---
name: sonderr-skill
description: A skill in the .sonderr/skills directory.
---
