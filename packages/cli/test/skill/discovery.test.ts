...
beforeAll(async () => {
  ...
  server = Bun.serve({
    ...
    async fetch(req) {
      ...
      // sonderr_change start - serve a crafted index whose skill name escapes the cache via `../`
...
describe("Discovery.pull", () => {
  ...
  // sonderr_change start - path-traversal in the remote index must not plant a trusted skill
