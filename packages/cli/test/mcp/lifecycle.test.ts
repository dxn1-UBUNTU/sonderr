...
import * as SandboxNetwork from "../../src/sonderr/sandbox/network" // sonderr_change
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* mcp.add("local-server", {
        type: "local",
        command: [process.execPath, stdioFixture],
      })
      yield* mcp.add("remote-server", remote(server.url))

      const tools = yield* mcp.tools()
      const local = tools["local-server_current_directory"]
      const remoteTool = tools["remote-server_test_tool"]
      if (!local || !remoteTool) return yield* Effect.die(new Error("expected MCP tools are missing"))
      ...
      if (Exit.isFailure(remoteExit)) expect(Cause.pretty(remoteExit.cause)).toContain("remote MCP delegated authority")
    }),
  { config: { mcp: {} } },
)
// sonderr_change end
