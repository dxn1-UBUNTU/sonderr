...
import { LLMEvent, Usage, type LLMEvent as Event } from "@sonderr/llm"
...
function empty(vercelID?: string) {
  ...
  const providerMetadata = vercelID ? { sonderr: { vercelID } } : undefined
  return [
    ...
    provideTmpdirProject(
      (dir) =>
        Effect.gen(function* () {
          ...
          expect(error.data.message).toBe(SonderrSessionProcessor.INCOMPLETE_RESPONSE_MESSAGE)
    ...
    provideTmpdirProject(
      (dir) =>
        Effect.gen(function* () {
          ...
          yield* ctx.test.reply(
            LLMEvent.stepStart({ index: 0 }),
            LLMEvent.toolCall({ id: "call", name: "web_search", input: { query: "Sonderr" }, providerExecuted: true }),
            LLMEvent.toolResult({
              id: "call",
              name: "web_search",
