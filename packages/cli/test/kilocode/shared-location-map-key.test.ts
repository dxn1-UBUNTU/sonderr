...
const sonderr = new URL("../../src/", import.meta.url)
...
describe("shared location service map keys", () => {
  test("all location route consumers include workspaceID in their cache key", () => {
    const consumers = [
      [sonderr, "server/routes/instance/httpapi/handlers/file.ts"],
      [sonderr, "server/routes/instance/httpapi/handlers/pty.ts"],
