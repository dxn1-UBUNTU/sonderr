...
import { SonderrCompactionPayloadRecovery } from "../../src/sonderr/session/compaction-payload-recovery"
...
describe("SonderrCompactionPayloadRecovery", () => {
  test("detects Sonderr gateway payload-size errors", () => {
    const error = new MessageV2.ContextOverflowError({
      message: "Request Entity Too Large",
      responseBody: "Request Entity Too Large\n\nFUNCTION_PAYLOAD_TOO_LARGE",
    }).toObject()

    expect(SonderrCompactionPayloadRecovery.matches(error)).toBe(true)
  })
  ...
  test("strips media and marks completed tool outputs compacted", async () => {
    ...
    await Effect.runPromise(
      SonderrCompactionPayloadRecovery.strip({
