...
// Regression coverage for https://github.com/Sonderr-Org/sonderr/issues/8656
//
...
// `config` hook (see ../fixture/stall-plugin.ts), so the SDK, Sonderr's fetch
...
function project(timeout: number | false) {
  return tmpdir<{ state: string }>({
    init: async (dir) => {
      ...
      await Bun.write(path.join(dir, "sonderr.json"), JSON.stringify(settings(state, timeout), null, 2))
      return { state }
    },
  })
}
...
function session(dir: string) {
  ...
  const headers = { "Content-Type": "application/json", "x-sonderr-directory": dir }
