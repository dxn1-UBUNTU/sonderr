...
const entry = "file:///tmp/sonderr-cache/node_modules/@lancedb/lancedb/dist/index.js"
const add = mock(async () => ({ directory: "/tmp/sonderr-cache", entrypoint: entry }))
const real = await import("@sonderr/core/npm")

mock.module("@sonderr/core/npm", () => ({
  ...real,
  Npm: {
    ...real.Npm,
    add,
...
const env = "SONDERR_LANCEDB_PATH"
const prev = process.env[env]

describe("LanceDBRuntime", () => {
  beforeEach(async () => {
    const { LanceDBRuntime } = await import("../../src/sonderr/lancedb")
    LanceDBRuntime.clear()
    add.mockClear()
    add.mockImplementation(async () => ({ directory: "/tmp/sonderr-cache", entrypoint: entry }))
  })
  ...
  test("retries after a failed install", async () => {
    ...
    add
      ...
      .mockImplementationOnce(async () => ({ directory: "/tmp/sonderr-cache", entrypoint: entry }))
