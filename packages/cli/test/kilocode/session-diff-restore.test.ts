...
import { appendSessionDiffs, mergeSessionDiffs } from "../../src/sonderr/session-portability/cumulative-diff"
import { extractSessionDiffs, restoreSessionDiffs } from "../../src/sonderr/session-portability/session-diff-restore"

const dirs: string[] = []

function tmp() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "sonderr-session-diff-"))
  dirs.push(dir)
  return dir
}
