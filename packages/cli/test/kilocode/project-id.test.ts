...
import { getSonderrProjectId } from "../../src/sonderr/project-id"
...
describe("project-id", () => {
  describe("normalization", () => {
    test("extracts repo name from HTTPS git URL", async () => {
      await using tmp = await tmpdir({
        ...
        init: async (dir) => {
          ...
          await Bun.$`git remote add origin https://github.com/Sonderr-Org/handbook.git`.cwd(dir).quiet()
        },
      })

      const id = await provideTestInstance({
        directory: tmp.path,
        fn: () => getSonderrProjectId(),
      })
    ...
    test("extracts repo name from SSH git URL", async () => {
      await using tmp = await tmpdir({
        ...
        init: async (dir) => {
          ...
          await Bun.$`git remote add origin git@github.com:Sonderr-Org/handbook.git`.cwd(dir).quiet()
        },
      })
    ...
    test("extracts repo name from HTTPS URL without .git extension", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await Bun.$`git remote add origin https://github.com/Sonderr-Org/handbook`.cwd(dir).quiet()
        },
      })
    ...
    test("extracts repo name from ssh:// URL", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await Bun.$`git remote add origin ssh://git@github.com/Sonderr-Org/handbook.git`.cwd(dir).quiet()
        },
      })
    ...
    test("truncates long repo names to 100 characters", async () => {
      ...
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await Bun.$`git remote add origin https://github.com/Sonderr-Org/${longName}.git`.cwd(dir).quiet()
        },
      })
  ...
  describe("config file priority", () => {
    test("uses project.id from .sonderr/config.json", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          ...
          await Bun.write(
            path.join(dir, ".sonderr", "config.json"),
            JSON.stringify({
              project: {
    ...
    test("falls back to .sonderr/config.json when .sonderr/config.json is absent", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await fs.mkdir(path.join(dir, ".sonderr"), { recursive: true })
    ...
    test("prefers .sonderr/config.json over .sonderr/config.json", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await fs.mkdir(path.join(dir, ".sonderr"), { recursive: true })
          await Bun.write(path.join(dir, ".sonderr", "config.json"), JSON.stringify({ project: { id: "new-project" } }))
          await fs.mkdir(path.join(dir, ".sonderr"), { recursive: true })
          await Bun.write(
            path.join(dir, ".sonderr", "config.json"),
      ...
      await using tmp = await tmpdir({
        ...
        init: async (dir) => {
          ...
          await Bun.write(
            ...
            JSON.stringify({
              project: {
                id: "https://github.com/Sonderr-Org/another-repo.git",
              },
            }),
          )
        },
  ...
  describe("fallback behavior", () => {
    ...
    test("handles malformed JSON in config file gracefully", async () => {
      await using tmp = await tmpdir({
        ...
        init: async (dir) => {
          await fs.mkdir(path.join(dir, ".sonderr"), { recursive: true })
          await Bun.write(path.join(dir, ".sonderr", "config.json"), "{ invalid json")
  ...
  describe("caching", () => {
    test("keeps project IDs isolated across active project contexts", async () => {
      await using first = await tmpdir({
        init: async (dir) => {
          await fs.mkdir(path.join(dir, ".sonderr"), { recursive: true })
          await Bun.write(path.join(dir, ".sonderr", "config.json"), JSON.stringify({ project: { id: "first" } }))
        },
      })
      await using second = await tmpdir({
        init: async (dir) => {
          await fs.mkdir(path.join(dir, ".sonderr"), { recursive: true })
          await Bun.write(path.join(dir, ".sonderr", "config.json"), JSON.stringify({ project: { id: "second" } }))
        },
      })

      const ids = await Promise.all([
        withTestInstance({ directory: first.path, fn: () => getSonderrProjectId() }),
        withTestInstance({ directory: second.path, fn: () => getSonderrProjectId() }),
      ])
    ...
    test("invalidates the cached project ID when the instance is disposed", async () => {
      ...
      const ids = await provideTestInstance({
        directory: tmp.path,
        fn: async (ctx) => {
          const first = await getSonderrProjectId()
          await Bun.write(path.join(tmp.path, ".sonderr", "config.json"), JSON.stringify({ project: { id: "second" } }))
          const cached = await getSonderrProjectId()
          await disposeInstance(ctx.directory)
          const refreshed = await getSonderrProjectId()
  ...
  describe("edge cases", () => {
    test("handles git URLs with port numbers", async () => {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await Bun.$`git remote add origin https://github.com:443/Sonderr-Org/handbook.git`.cwd(dir).quiet()
        },
      })
