...
import { NotebookEditTool, NotebookExecuteTool, NotebookReadTool } from "@/sonderr/tool/notebook-host"
...
test("uses dedicated VS Code notebook permission defaults only when enabled", () => {
  ...
  try {
    ...
    const disabled = SonderrAgent.prepare({}).defaultsPatch
    expect(disabled.some((rule) => rule.permission.startsWith("notebook_"))).toBe(false)

    const rules = SonderrAgent.prepare({ experimental: { native_notebook_tools: true } }).defaultsPatch
