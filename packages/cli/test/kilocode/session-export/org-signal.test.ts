...
import { getActiveOrg, resetOrgSource, setOrgSource } from "@/sonderr/session-export/eligibility"
...
describe("getActiveOrg", () => {
  ...
  afterEach(() => {
    if (env === undefined) delete process.env.SONDERR_ORG_ID
    else process.env.SONDERR_ORG_ID = env
    resetOrgSource()
  })
  ...
  test("returns env value when SONDERR_ORG_ID is set", async () => {
    setOrgSource(async () => ({ type: "org", id: "org_auth" }))
    process.env.SONDERR_ORG_ID = "org_envvar"
