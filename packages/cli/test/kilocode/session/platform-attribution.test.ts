  ...
  test("child sessions inherit the root platform override", async () => {
    await provideTestInstance({
      ...
      fn: async () => {
        ...
        const attr = SonderrSession.attribution(child.id)

        expect(SonderrSession.getPlatformOverride(root.id)).toBe("agent-manager")
        expect(SonderrSession.getPlatformOverride(child.id)).toBe("agent-manager")
  ...
  test("child sessions expose parent and root lineage", async () => {
    await provideTestInstance({
      ...
      fn: async () => {
        ...
        expect(SonderrSession.resolveParent(root.id)).toBeUndefined()
        expect(SonderrSession.resolveParent(child.id)).toBe(root.id)
        expect(SonderrSession.resolveParent(leaf.id)).toBe(child.id)
        expect(SonderrSession.resolveRoot(leaf.id)).toBe(root.id)
  ...
  test("turn close events include persisted parent lineage", async () => {
    await provideTestInstance({
      ...
      fn: async () => {
        ...
        expect(SonderrSession.resolveParent(child.id)).toBeUndefined()
