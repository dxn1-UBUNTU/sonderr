...
async function main() {
  ...
  process.env.SONDERR_TEST_HOME = path.join(msg.root, "home")

  const { Path } = await import("@sonderr/core/global")
  const { refreshCodexAuth } = await import("../../../src/sonderr/provider/codex-refresh")
