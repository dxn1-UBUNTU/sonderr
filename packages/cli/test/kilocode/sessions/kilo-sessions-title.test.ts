...
import {
  ...
  markAutoTitle,
  markRenameAdopted,
} from "../../../src/sonderr-sessions/rename-adoptions"
...
const SonderrSessions = (await import("../../../src/sonderr-sessions/sonderr-sessions")).SonderrSessions

// Session must be provideMerged so yield* Session.Service and the
// SonderrSessions Updated handler share one store (otherwise get() misses creates).
...
function layer(overrides: Partial<Config.Interface> = {}) {
  return AppNodeBuilder.build(
    LayerNode.group([SonderrSessions.node, Session.node, SessionProjector.node]),
...
function reset(...tokens: string[]) {
  ...
  clearInFlightCache("sonderr-sessions:org")
...
const ENV_KEYS = [
  "SONDERR_API_KEY",
  "SONDERR_SESSION_INGEST_URL",
  "SONDERR_ORG_ID",
  "SONDERR_AGENT_NOTIFICATION_TIMEOUT_MS",
] as const
...
beforeEach(() => {
  delete process.env.SONDERR_ORG_ID
  clearRenameMarks()
})
...
function metaItems(requests: Req[]) {
  ...
  for (const r of requests) {
    ...
    for (const item of data) {
      if (item?.type === "sonderr_meta") items.push(item.data)
    }
  }
  return items
}
...
const drainIngest = Effect.gen(function* () {
  ...
  yield* Effect.promise(() => SonderrSessions.drainIngestForShutdown())
})

it.instance("meta org precedence: session metadata > SONDERR_ORG_ID > auth accountId", () => {
  const requests: Req[] = []
  installFetch(mockFetch(requests))
  patchEnv({
    SONDERR_API_KEY: "test-token",
    SONDERR_SESSION_INGEST_URL: "https://ingest.kilosessions.ai",
    SONDERR_ORG_ID: ORG_ENV,
  })
  reset("test-token")
  ...
  return Effect.gen(function* () {
    ...
    yield* Effect.gen(function* () {
      ...
      yield* Effect.promise(() => SonderrSessions.bootstrap(withMeta.id))
      requests.length = 0
      ...
      yield* Effect.promise(() => SonderrSessions.bootstrap(plain.id))
      requests.length = 0
      ...
      yield* Effect.promise(() => SonderrSessions.bootstrap(authOnly.id))
      requests.length = 0
      emitUpdated(instance.directory, authOnly.id, authOnly.title)
      yield* drainIngest
      expect(metaItems(requests).some((m) => m.orgId === ORG_AUTH)).toBe(true)
    }).pipe(Effect.ensuring(auth.remove("sonderr").pipe(Effect.orDie)))
...
it.instance("meta falls through invalid metadata orgId to env", () => {
  ...
  return Effect.gen(function* () {
    ...
    yield* Effect.promise(() => SonderrSessions.bootstrap(bad.id))
    requests.length = 0
...
it.instance("meta falls back to env when session row has no resolvable org", () => {
  // Precedence only: real Updated → sonderr_meta with a live session that has no
...
it.instance("meta falls back to env when Session.Service.get fails", () => {
  ...
  // global runtime; unknown id → get rejects → .catch(() => null) → SONDERR_ORG_ID.
  ...
  patchEnv({ SONDERR_ORG_ID: ORG_ENV })
  clearInFlightCache("sonderr-sessions:org")

  return Effect.gen(function* () {
    const result = yield* Effect.promise(() => SonderrSessions._metaForTests("ses_missing_for_meta_get_fail"))
...
it.instance("title broadcast: auto-title posts generated true; custom posts generated false; adoption skips", () => {
  ...
  patchEnv({
    SONDERR_API_KEY: "test-token",
    SONDERR_SESSION_INGEST_URL: "https://ingest.kilosessions.ai",
    SONDERR_AGENT_NOTIFICATION_TIMEOUT_MS: "5000",
  })
  reset("test-token")
  ...
  return Effect.gen(function* () {
    ...
    const id = created.id
    yield* Effect.promise(() => SonderrSessions.bootstrap(id))
...
it.instance("reportSessionTitle goes through readiness and tolerates POST failure", () => {
  ...
  return Effect.gen(function* () {
    ...
    const sessions = yield* Session.Service
    const created = yield* sessions.create({})
    yield* Effect.promise(() => SonderrSessions.bootstrap(created.id))

    const failed = yield* sonderr.reportSessionTitle(created.id, "X", { generated: false })
    expect(failed).toEqual({ ok: false, reason: "http_500" })
    expect(requests.some((r) => r.path.endsWith("/title"))).toBe(true)

    titleStatus = 200
    const ok = yield* sonderr.reportSessionTitle(created.id, "Y", { generated: true })
    expect(ok).toEqual({ ok: true })
  }).pipe(Effect.provide(layer()))
})

it.instance("reportSessionTitle reports not_connected when unauthenticated", () => {
  patchEnv({ SONDERR_API_KEY: undefined })
  delete process.env.SONDERR_API_KEY
  reset()

  return Effect.gen(function* () {
    const sonderr = yield* SonderrSessions.Service
    const result = yield* sonderr.reportSessionTitle("ses_x", "T", { generated: false })
...
it.instance(
  ...
  () => {
    ...
    const customLayer = SonderrSessions.layer.pipe(
