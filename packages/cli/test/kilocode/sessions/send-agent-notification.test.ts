...
beforeEach(() => {
  originalNotificationTimeout = process.env.SONDERR_AGENT_NOTIFICATION_TIMEOUT_MS
  process.env.SONDERR_AGENT_NOTIFICATION_TIMEOUT_MS = "50"
})

afterEach(() => {
  if (originalNotificationTimeout === undefined) {
    delete process.env.SONDERR_AGENT_NOTIFICATION_TIMEOUT_MS
  } else {
    process.env.SONDERR_AGENT_NOTIFICATION_TIMEOUT_MS = originalNotificationTimeout
  }
})
...
it.instance("dedicated immediate POST hits ingest path with an agent_notification item", () => {
  const originalKey = process.env.SONDERR_API_KEY
  const originalIngest = process.env.SONDERR_SESSION_INGEST_URL
  ...
  process.env.SONDERR_API_KEY = "test-token"
  process.env.SONDERR_SESSION_INGEST_URL = "https://ingest.kilosessions.ai"
  reset("test-token")

  return Effect.gen(function* () {
    const sessions = yield* SonderrSessions.Service
    yield* Effect.promise(() => SonderrSessions.bootstrap("session-1"))
    ...
    Effect.ensuring(
      Effect.gen(function* () {
        ...
        if (originalKey === undefined) delete process.env.SONDERR_API_KEY
        else process.env.SONDERR_API_KEY = originalKey
        if (originalIngest === undefined) delete process.env.SONDERR_SESSION_INGEST_URL
        else process.env.SONDERR_SESSION_INGEST_URL = originalIngest
        reset("test-token")
        request.mockRestore()
      }),
    ),
...
it.instance("2xx ingest response returns ok:true", () => {
  ...
  return Effect.gen(function* () {
    const sessions = yield* SonderrSessions.Service
    yield* Effect.promise(() => SonderrSessions.bootstrap("session-2"))
...
it.instance("non-2xx ingest response returns ok:false with no retry", () => {
  ...
  return Effect.gen(function* () {
    const sessions = yield* SonderrSessions.Service
    yield* Effect.promise(() => SonderrSessions.bootstrap("session-3"))
...
it.instance("coalescing: sendAgentNotification awaits in-flight bootstrap for a single /api/session POST", () => {
  ...
  return Effect.gen(function* () {
    const sessions = yield* SonderrSessions.Service
    const createPromise = SonderrSessions.create("session-6")
...
it.instance("coalescing: bootstrapInflight stores the real promise and concurrent create() calls share one POST", () => {
  ...
  return Effect.gen(function* () {
    ...
    expect(SonderrSessions._getBootstrapInflight("session-7")).toBeUndefined()

    // Fire two concurrent create() calls for the same session.
    const create1 = SonderrSessions.create("session-7")
    const create2 = SonderrSessions.create("session-7")
    ...
    const inflight = SonderrSessions._getBootstrapInflight("session-7")
