...
import type { Part, StepFinishPart } from "@sonderr/sdk/v2"
import { RoutedModelMeta } from "../../src/sonderr/cli/cmd/tui/routes/session/routed-model-meta"
import { SonderrRoutedModel } from "../../src/sonderr/session/routed-model"
...
describe("session routed model", () => {
  ...
  test("preserves finish-step response model in provider metadata", async () => {
    const events = await adapt([
      unchecked({
        ...
        usage: {},
        providerMetadata: { openrouter: { routed: true }, sonderr: { existing: true } },
      }),
    ])
    ...
    expect(event.providerMetadata).toEqual({
      ...
      sonderr: { existing: true, routedModelID: "openai/gpt-5.5-20260423" },
    })
  })
  ...
  test("shortens date-suffixed routed model ids for display", () => {
    expect(SonderrRoutedModel.display("moonshotai/kimi-k2.7-code-20260612")).toBe("moonshotai/kimi-k2.7-code")
    expect(SonderrRoutedModel.display("openai/gpt-5.5-2026-04-23")).toBe("openai/gpt-5.5")
    expect(SonderrRoutedModel.display("openai/gpt-5.5")).toBe("openai/gpt-5.5")
  })

  test("formats routed model names for compact display", () => {
    expect(SonderrRoutedModel.displayName("Qwen: Qwen3.7 Plus (20% off)")).toBe("Qwen 3.7 Plus")
    expect(SonderrRoutedModel.displayName("anthropic.claude-opus-4-5-20251101-v1:0")).toBe(
      "anthropic.claude-opus-4-5-20251101-v1:0",
    )
    expect(SonderrRoutedModel.displayName("moonshotai/kimi-k2.7-code")).toBe("kimi-k2.7-code")
    expect(SonderrRoutedModel.displayName("o3")).toBe("o3")
  })

  test("shows compact labels only for Sonderr auto selections", () => {
    ...
    const routed = RoutedModelMeta.info(undefined, parts, false, {
      providerID: "sonderr",
      modelID: "sonderr-auto/efficient",
    })
    ...
    const same = RoutedModelMeta.info(
      undefined,
      [reason, finish({ providerID: "sonderr", modelID: "sonderr-auto/efficient" })],
      false,
      {
        providerID: "sonderr",
        modelID: "sonderr-auto/efficient",
  ...
  test("reads routed model only for selected Sonderr auto models", () => {
    const meta = { sonderr: { routedModelID: "openai/gpt-5.5-20260423" } }

    expect(
      SonderrRoutedModel.readAuto(meta, {
