...
import { AgentManagerTool, Params } from "../../src/sonderr/tool/agent-manager"
import { AgentManagerEvent, type AgentManagerStart } from "../../src/sonderr/agent-manager/event"
import { AgentManager } from "../../src/sonderr/agent-manager/service"
...
const providers = {
  test: {
    ...
    models: {
      ...
      // "Shared" is also offered by the sonderr provider, to exercise provider resolution.
      "test/shared": { id: "test/shared", providerID: "test", name: "Shared", variants: { low: {}, high: {} } },
    },
  } as unknown as Provider.Info,
  sonderr: {
    id: "sonderr",
    name: "Sonderr Gateway",
    models: {
      "sonderr/shared": { id: "sonderr/shared", providerID: "sonderr", name: "Shared", variants: { low: {} } },
      "sonderr/only": { id: "sonderr/only", providerID: "sonderr", name: "Gateway Only", variants: { low: {} } },
    },
  } as unknown as Provider.Info,
  zeta: {
    id: "zeta",
...
// Default provider is `test`, so resolution should prefer test, then sonderr, then others.
...
describe("agent_manager tool", () => {
  ...
  test("validates provider selectors at the task level", () => {
    expect(Schema.is(Params)({ mode: "local", tasks: [{ prompt: "Fix", model: "Shared", provider: "sonderr" }] })).toBe(
      true,
    )
    expect(Schema.is(Params)({ mode: "local", tasks: [{ prompt: "Fix", provider: "sonderr" }] })).toBe(false)
  ...
  test("lists the compact overview with a separate read-only permission pattern", async () => {
    ...
    expect(JSON.parse(result.output)).toEqual({
      instructions:
        "This overview is the source of truth. Use sections[].id as sectionID and sessions[].id/session.id as sessionID for action=move. Do not edit .sonderr/agent-manager.json.",
      sections: [],
      ungrouped: [
        {
          id: "wt-1",
  ...
  test("inherits the latest invoking model and variant when omitted", async () => {
    const task = await publish(runtime, { prompt: "Fix" }, [
      message("msg_current", "sonderr", "sonderr/shared", "low", 2),
      message("msg_old", "test", "reasoning/model", "high", 1),
    ])

    expect(String(task?.model?.providerID)).toBe("sonderr")
    expect(String(task?.model?.modelID)).toBe("sonderr/shared")
  ...
  test("explicit model and variant override the invoking selection", async () => {
    const task = await publish(runtime, { prompt: "Fix", model: "test/reasoning/model", variant: "high" }, [
      message("msg_current", "sonderr", "sonderr/shared", "low"),
    ])
  ...
  test("does not inherit a variant when only the model is overridden", async () => {
    ...
    expect(String(task?.model?.providerID)).toBe("sonderr")
    expect(String(task?.model?.modelID)).toBe("sonderr/only")
    expect(task?.variant).toBeUndefined()
  })
  ...
  test("uses an explicitly selected provider for a shared model name", async () => {
    const task = await publish(runtime, { prompt: "Fix", model: " Shared ", provider: " sonderr " })
    expect(String(task?.model?.providerID)).toBe("sonderr")
    expect(String(task?.model?.modelID)).toBe("sonderr/shared")
  })

  test("uses the provider of a different default model when that is the user's choice", async () => {
    const rt = makeRuntime("sonderr")
  ...
  test("prefers the invoking provider for an explicit model override", async () => {
    const task = await publish(runtime, { prompt: "Fix", model: "Shared", variant: "low" }, [
      message("msg_current", "sonderr", "sonderr/only", "low"),
    ])
    expect(String(task?.model?.providerID)).toBe("sonderr")
    expect(String(task?.model?.modelID)).toBe("sonderr/shared")
  })
  ...
  test("reports a model unavailable from an explicit provider", async () => {
    ...
    const result = await runtime.runPromise(
      provideTmpdirInstance(() =>
        tool.execute(
          { mode: "local", tasks: [{ prompt: "Fix", model: "Reasoning Model", provider: "sonderr" }] },
          { ...ctx, ask: (input: unknown) => Effect.sync(() => calls.push(input)) },
        ),
      ).pipe(Effect.scoped),
    )
    ...
    expect(calls).toEqual([])
    expect(result.output).toContain('model is not available from provider "sonderr": Reasoning Model')
    expect(result.metadata.count).toBe(0)
  })
  ...
  test("falls back to the sonderr gateway when the preferred provider lacks the model", async () => {
    const task = await publish(runtime, { prompt: "Fix", model: "Gateway Only" })
    // Default provider `test` does not offer it; sonderr is preferred over zeta.
    expect(String(task?.model?.providerID)).toBe("sonderr")
  })

  test("narrows to a provider that supports the requested variant", async () => {
    const rt = makeRuntime("sonderr")
    // sonderr is preferred, but only `test`'s Shared has the `high` variant.
