/**
 * Contract test for prompt.ts Sonderr-specific invariants.
 *
 * prompt.ts is a shared upstream file. The Sonderr-specific "new prompt unblocks
 * pending suggestions/questions then enqueues without cancelling the in-flight
 * stream" behaviour lives inside a sonderr_change block. An upstream merge
...
describe("prompt.ts Sonderr-specific invariants", () => {
  test("imports Suggestion from sonderr/suggestion", () => {
    const content = fs.readFileSync(PROMPT_FILE, "utf-8")
    expect(content).toMatch(/import\s*\{[^}]*Suggestion[^}]*\}\s*from\s*["']@\/sonderr\/suggestion["']/)
  })
  ...
  test("enqueue reserves the follow-up before dismissing blockers, without cancelling the in-flight fiber", () => {
    ...
    const block = content.match(
      /sonderr_change start[^\n]*register the queued follow-up[\s\S]*?Suggestion\.dismissAll[\s\S]*?question\.dismissAll[\s\S]*?SonderrSessionPromptQueue\.enqueue\([\s\S]*?dismiss/,
    )
  ...
  test("runLoop breaks out between LLM steps when a newer prompt was enqueued", () => {
    ...
    expect(content).toContain("SonderrSessionPromptQueue.hasFollowup(sessionID)")
  })
})
