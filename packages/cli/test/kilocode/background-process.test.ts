...
`,
      ...
      const client = process.env["SONDERR_CLIENT"]
      ...
      try {
        ...
        yield* Effect.promise(() => BackgroundProcess.stopSession(sessionID))
        if (client === undefined) delete process.env["SONDERR_CLIENT"]
        else process.env["SONDERR_CLIENT"] = client
      }
    }),
  )
...
`,
      ...
      const client = process.env["SONDERR_CLIENT"]
      const scans = process.env["SONDERR_BACKGROUND_PROCESS_PORTS"]
      process.env["SONDERR_CLIENT"] = "cli"
      process.env["SONDERR_BACKGROUND_PROCESS_PORTS"] = "true"
      ...
      try {
        ...
        if (scans === undefined) delete process.env["SONDERR_BACKGROUND_PROCESS_PORTS"]
        else process.env["SONDERR_BACKGROUND_PROCESS_PORTS"] = scans
      }
    }),
  )
...
`,
      ...
      const scans = process.env["SONDERR_BACKGROUND_PROCESS_PORTS"]
      process.env["SONDERR_CLIENT"] = "cli"
      delete process.env["SONDERR_BACKGROUND_PROCESS_PORTS"]
