...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    const flags = {
      source: Flag.SONDERR_MODELS_URL,
    ...
    yield* Effect.acquireUseRelease(
      Effect.promise(async () => {
        Flag.SONDERR_MODELS_URL = source
      ...
      () =>
        Effect.promise(async () => {
          Flag.SONDERR_MODELS_URL = flags.source
          Flag.SONDERR_MODELS_PATH = flags.path
          Flag.SONDERR_DISABLE_MODELS_FETCH = flags.disabled
  ...
  { config: { disabled_providers: ["sonderr", "apertis"] } },
)
