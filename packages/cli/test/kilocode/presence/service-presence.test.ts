...
// Each SonderrViewers.layer construction reads these env vars (post-refactor), so
// setting them here controls presence wiring per test.
process.env.SONDERR_EVENT_SERVICE_URL = "wss://test-presence"
...
const realSessions = await import("@/sonderr-sessions/sonderr-sessions")
const realSetAttached = realSessions.SonderrSessions.setAttachedSessions
mock.module("@/sonderr-sessions/sonderr-sessions", () => ({
  ...realSessions,
  SonderrSessions: {
    ...realSessions.SonderrSessions,
...
mock.module("@/sonderr/event-service/client", () => ({
...
const { SonderrViewers } = await import("@/sonderr/presence/service")
...
const layer = SonderrViewers.layer.pipe(Layer.provide(authLayer))
...
function run(body: (viewers: {
  ...
  return Effect.gen(function* () {
    const v = yield* SonderrViewers.Service
    yield* body(v)
...
describe("SonderrViewers.Service presence contexts", () => {
  ...
  test("kill switch blocks the presence socket but attached union still reaches SonderrSessions", async () => {
    attachedCalls.length = 0
    current = new FakeClient()
    const prev = process.env.SONDERR_DISABLE_PRESENCE
    process.env.SONDERR_DISABLE_PRESENCE = "1"
    try {
      await run((v) =>
        v.update({ viewer: { id: uid, active: true }, attached: ["ses_a"], visible: ["ses_a"] }),
      )
      ...
      if (prev === undefined) delete process.env.SONDERR_DISABLE_PRESENCE
      else process.env.SONDERR_DISABLE_PRESENCE = prev
    }
  })
})
...
describe("SonderrViewers.Service viewer lifecycle", () => {
  ...
  test("account switch disconnects the old client before connecting the new one", async () => {
    ...
    await run(
      ...
      SonderrViewers.layer.pipe(Layer.provide(mutableAuthLayer)),
    )
