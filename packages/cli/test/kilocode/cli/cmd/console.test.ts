...
import { getNetworkIPs, serverUrls } from "../../../../src/sonderr/cli/server-urls"
...
describe("console daemon startup", () => {
  test("detects every explicit network option form", () => {
    ...
    expect(explicitNetworkOptions(["sonderr", "console", "--", "--port=4321"])).toStrictEqual([])
  })
  ...
  test("rejects legacy fixed-password daemon state", () => {
    expect(Daemon.matches({ ...state(), password: "sonderr" }, opts(), [])).toBe(false)
  })
