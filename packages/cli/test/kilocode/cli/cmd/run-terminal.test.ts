...
import { SonderrRunTerminal } from "@/sonderr/cli/cmd/run-terminal"
...
test("routes direct interactive terminal requests through the session workspace", async () => {
  ...
  const terminal = SonderrRunTerminal.create(sdk, () => "ses_terminal")
...
test("retries workspace lookup after a failed request", async () => {
  ...
  const sdk = createSonderrClient({ baseUrl: "http://test", fetch })
  const terminal = SonderrRunTerminal.create(sdk, () => "ses_terminal")
