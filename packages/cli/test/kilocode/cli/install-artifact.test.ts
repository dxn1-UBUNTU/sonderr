...
const wrapper = path.join(root, "bin", "sonderr")
...
describe("npm install artifact behavior", () => {
  test("keeps the CLI wrapper contract", async () => {
    ...
    expect(text).toContain("const envPath = process.env.SONDERR_BIN_PATH")
    expect(text).toContain('const base = "@sonderr/cli-" + platform + "-" + arch')
  ...
  test("copies cached binary runtime resources during postinstall", async () => {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "sonderr-postinstall-artifact-"))
    try {
      const pkg = path.join(tmp, "node_modules", "@sonderr", "cli")
      const native = path.join(tmp, "node_modules", "@sonderr", `cli-${process.platform}-${process.arch}`)
      ...
      await Bun.write(
        ...
        JSON.stringify({
          optionalDependencies: {
            [`@sonderr/cli-${process.platform}-${process.arch}`]: "1.0.0",
          },
        }),
      )
      await Bun.write(
        path.join(native, "package.json"),
        JSON.stringify({ name: `@sonderr/cli-${process.platform}-${process.arch}` }),
      )
      const binary = "#!/bin/sh\n# binary\nexit 0\n"
      await Bun.write(path.join(bin, "sonderr"), binary)
      await Bun.write(path.join(bin, "sonderr-sandbox-mutation-worker.js"), "worker")
      ...
      expect(await Bun.file(path.join(pkg, "bin", ".sonderr")).text()).toBe(binary)
      expect(await Bun.file(path.join(pkg, "bin", "sonderr-sandbox-mutation-worker.js")).text()).toBe("worker")
  ...
  test("links npm bin commands to the wrapper during local install", async () => {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "sonderr-install-artifact-"))
    try {
      ...
      await fs.copyFile(wrapper, path.join(bin, "sonderr"))
      await Bun.write(
        path.join(pkg, "package.json"),
        JSON.stringify(
          {
            name: "sonderr-install-artifact-repro",
            version: "1.0.0",
            bin: {
              sonderr: "./bin/sonderr",
              sonderr: "./bin/sonderr",
      ...
      const commands = ["sonderr", "sonderr"]
      ...
      const hidden = path.join(prefix, "node_modules", ".bin", ".sonderr")
