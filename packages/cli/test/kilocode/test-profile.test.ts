...
import { TestProfile } from "../../script/sonderr/test-profile"
...
describe("test profiles", () => {
  test("darwin profile contains valid test files", () => {
    ...
    expect(result.files).toContain("sonderr/cli/install-artifact.test.ts")
    expect(result.files).toContain("sonderr/cli/tui/thread.test.ts")
    expect(result.files).toContain("sonderr/sandbox/macos-confinement.test.ts")
    expect(result.files).toContain("sonderr/core-watcher.test.ts")
    expect(result.files).toContain("sonderr/background-process.test.ts")
    expect(result.files).toContain("filesystem/filesystem.test.ts")
    expect(result.files).toContain("sonderr/interactive-terminal.test.ts")
    const sandbox = all.filter((file) => file.startsWith("sonderr/sandbox/"))
    expect(result.files.filter((file) => file.startsWith("sonderr/sandbox/"))).toEqual(sandbox)
    ...
    expect(result.files).not.toContain("sonderr/sessions/remote-ws.test.ts")
    ...
    expect(result.files).not.toContain("sonderr/daemon.test.ts")
    ...
    expect(result.files).not.toContain("sonderr/server/config-overlay.test.ts")
