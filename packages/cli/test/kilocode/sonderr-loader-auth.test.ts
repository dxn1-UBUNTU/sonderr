...
// Tests that unauthenticated Sonderr models are assembled with paid models and autoloaded anonymously.

import { expect } from "bun:test"
import { FSUtil } from "@sonderr/core/fs-util"
import { ModelsDev } from "../../src/provider/models"
import * as CoreModels from "@sonderr/core/models-dev"
import { Effect, Layer } from "effect"
import { FetchHttpClient } from "effect/unstable/http"
import { sonderrCustomLoaders, patchSonderrProviderPrivacy } from "../../src/sonderr/provider/provider"
...
it.live("assembles paid Sonderr models without auth", () =>
  Effect.gen(function* () {
    const providers = yield* ModelsDev.Service.use((models) => models.get()).pipe(
      Effect.provide(layer()),
      provideInstance(process.cwd()),
    )
    const sonderr = Provider.fromModelsDevProvider(providers.sonderr)

    expect(sonderr.models["paid-model"]).toMatchObject({
      id: "paid-model",
      providerID: "sonderr",
      cost: { input: 1, output: 2 },
      isFree: false,
...
it.live("does not infer free status from zero catalog prices", () =>
  Effect.gen(function* () {
    ...
    expect(sonderr.models["free-model"].isFree).toBeUndefined()
  }),
)
...
it.effect("enables a paid catalog when config apiKey is present", () =>
  Effect.gen(function* () {
    const result = yield* load({ config: { provider: { sonderr: { options: { apiKey: "test-key" } } } } })
    expect(result.autoload).toBe(true)
...
it.effect("keeps data collection denied after configured options are applied", () =>
  Effect.sync(() => {
    ...
    patchSonderrProviderPrivacy(provider, { hide_prompt_training_models: true })
