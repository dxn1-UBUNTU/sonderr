...
import { TestCli } from "../../script/sonderr/test-cli"
  ...
  test(
    ...
    async () => {
      ...
      try {
        ...
        const outside = await fs.mkdtemp(path.join(os.tmpdir(), "sonderr-test-cli-cwd-"))
        ...
        const output = await (async () => {
          ...
          let text = ""
          while (!text.includes("sonderr server listening on")) {
        ...
        expect(output, `stdout:\n${output}\nstderr:\n${err}`).toContain("sonderr server listening on")
        expect(code, `stdout:\n${output}\nstderr:\n${err}`).not.toBe(1)
      } finally {
