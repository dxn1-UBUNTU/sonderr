...
import { BackgroundProcess } from "../../src/sonderr/background-process"
...
import * as SandboxPolicy from "../../src/sonderr/sandbox/policy"
...
describe("Sonderr task nesting", () => {
  ...
  test("preserves inherited restrictions while refreshing prompt tool toggles", () => {
    const permission = SonderrSessionPrompt.mergeToolPermissions({
      existing: [
      ...
      () =>
        Effect.gen(function* () {
          ...
          const effective = Permission.merge(
            ...
            SonderrSessionPrompt.guardPermissions({ agent: validator, session: child }),
          )
