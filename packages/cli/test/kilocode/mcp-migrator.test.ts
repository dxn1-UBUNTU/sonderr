...
import { McpMigrator } from "../../src/sonderr/mcp-migrator"
...
describe("McpMigrator", () => {
  describe("convertServer", () => {
    test("converts local server with command and args", () => {
      const server: McpMigrator.SonderrMcpServer = {
        command: "npx",
  ...
  describe("migrate", () => {
    ...
    test("migrates servers from project .sonderr/mcp.json", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const settingsDir = path.join(dir, ".sonderr")
          await Bun.write(
    ...
    test("reads from legacy .sonderr/mcp.json when .sonderr/mcp.json is absent", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const settingsDir = path.join(dir, ".sonderr")
          await Bun.write(
    ...
    // Regression: malformed .sonderr/mcp.json must not prevent .sonderr/mcp.json from loading
    test("loads .sonderr/mcp.json even when .sonderr/mcp.json is malformed", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".sonderr", "mcp.json"), "{ corrupt json !!!")
          await Bun.write(
            path.join(dir, ".sonderr", "mcp.json"),
            JSON.stringify({
              mcpServers: {
    ...
    test(".sonderr/mcp.json overrides .sonderr/mcp.json for same server name", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(
            path.join(dir, ".sonderr", "mcp.json"),
    ...
    test("does not read project-level .sonderr/mcp_settings.json", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const settingsDir = path.join(dir, ".sonderr")
          await Bun.write(
