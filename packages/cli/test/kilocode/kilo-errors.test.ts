...
import { SONDERR_ERROR_CODES, isSonderrError, parseSonderrErrorCode } from "../../src/sonderr/sonderr-errors"
...
describe("parseSonderrErrorCode", () => {
  ...
  it("extracts PROMOTION_MODEL_LIMIT_REACHED from { code } (top-level)", () => {
    ...
    expect(parseSonderrErrorCode(error)).toBe("PROMOTION_MODEL_LIMIT_REACHED")
  })
  ...
  it("returns undefined for non-Sonderr error codes", () => {
    const error = makeAPIError({
      statusCode: 429,
      responseBody: JSON.stringify({ error: { code: "SOME_OTHER_ERROR" } }),
    })
    expect(parseSonderrErrorCode(error)).toBeUndefined()
  })
...
describe("isSonderrError", () => {
  it("returns true for PAID_MODEL_AUTH_REQUIRED", () => {
    const error = makeAPIError({
      statusCode: 401,
      responseBody: JSON.stringify({ error: { code: "PAID_MODEL_AUTH_REQUIRED" } }),
    })
    expect(isSonderrError(error)).toBe(true)
  })
  ...
  it("returns false for regular 429 errors without Sonderr code", () => {
    const error = makeAPIError({
      statusCode: 429,
      isRetryable: true,
      message: "Too Many Requests",
    })
    expect(isSonderrError(error)).toBe(false)
  })
...
describe("SessionRetry.retryable with Sonderr errors", () => {
