...
import { ModesMigrator } from "../../src/sonderr/modes-migrator"
...
describe("ModesMigrator", () => {
  ...
  describe("convertPermissions", () => {
    ...
    test("converts fileRegex groups to restricted permissions", () => {
      const groups: ModesMigrator.SonderrMode["groups"] = [
        "read",
    ...
    test("handles tuple without fileRegex", () => {
      const groups: ModesMigrator.SonderrMode["groups"] = [["edit", {}]]
      const permissions = ModesMigrator.convertPermissions(groups)
  ...
  describe("convertMode", () => {
    test("converts full mode to agent config", () => {
      const mode: ModesMigrator.SonderrMode = {
        slug: "translate",
        name: "Translate",
  ...
  - slug: translate
    ...
    groups:
      - read`,
          ...
          // Create project .sonderrmodes (should win)
          await Bun.write(
            path.join(dir, ".sonderrmodes"),
            `customModes:
  - slug: translate
