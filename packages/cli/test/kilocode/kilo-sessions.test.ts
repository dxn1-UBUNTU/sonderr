...
import { provide, Instance } from "../../src/sonderr/instance"
import { writePrLinkOverride } from "../../src/sonderr-sessions/pr-link"
import * as PrLink from "../../src/sonderr-sessions/pr-link"
import { RemoteWS } from "../../src/sonderr-sessions/remote-ws"
import { RemoteSender } from "../../src/sonderr-sessions/remote-sender"
...
import { RemoteProtocol } from "../../src/sonderr-sessions/remote-protocol"
...
function reset(...tokens: string[]) {
  ...
  for (const token of tokens) clearInFlightCache(`sonderr-sessions:token-valid:${token}`)
}
...
it.instance("bootstraps session ingest from SONDERR_API_KEY without stored auth", () => {
  const original = process.env.SONDERR_API_KEY
  const calls: string[] = []
  ...
  process.env.SONDERR_API_KEY = "env-token"
  reset("env-token")

  return Effect.promise(() => SonderrSessions.bootstrap("session-env")).pipe(
    Effect.andThen(() => Effect.sync(() => expect(calls).toEqual(["Bearer env-token", "Bearer env-token"]))),
    Effect.ensuring(
      Effect.sync(() => {
        if (original === undefined) delete process.env.SONDERR_API_KEY
        else process.env.SONDERR_API_KEY = original
        reset("env-token")
        request.mockRestore()
      }),
    ),
...
it.instance("prefers stored auth over SONDERR_API_KEY for session ingest", () => {
  const original = process.env.SONDERR_API_KEY
  const calls: string[] = []
  const fetch: typeof globalThis.fetch = Object.assign(
    async (input: RequestInfo | URL, init?: RequestInit) => {
  ...
  return Effect.gen(function* () {
    ...
    yield* auth.set("sonderr", { type: "api", key: "stored-token" })
    yield* Effect.promise(() => SonderrSessions.bootstrap("session-auth"))
    expect(calls).toEqual(["Bearer stored-token", "Bearer stored-token"])
  }).pipe(
    Effect.ensuring(
      Effect.gen(function* () {
        const auth = yield* Auth.Service
        yield* auth.remove("sonderr").pipe(Effect.orDie)
        if (original === undefined) delete process.env.SONDERR_API_KEY
        else process.env.SONDERR_API_KEY = original
        reset("env-token", "stored-token")
        request.mockRestore()
...
it.instance("does not duplicate created-session subscribers when init is repeated", () => {
  ...
  return Effect.gen(function* () {
    ...
    const sessions = yield* SonderrSessions.Service
    yield* auth.set("sonderr", { type: "api", key: "test-token" })
    yield* sessions.init()
    yield* sessions.init()
...
// sonderr_change start - K1 W1 / DEF-1: instance advertisement + per-session platform.
//
// `enableRemote` is idempotent/coalescing and is called from `/remote`, the
// explicit `sonderr remote` command, and bootstrap auto-enable (`SONDERR_REMOTE=1` /
...
describe("SonderrSessions.setInstanceAdvertisement (K1 W1 / DEF-1)", () => {
  ...
  beforeEach(() => {
    heartbeatCalls = 0
    outOfBand = undefined
    process.env["SONDERR_DISABLE_SESSION_INGEST"] = "0"
    delete process.env["SONDERR_SESSION_INGEST_URL"]
    process.env["SONDERR_API_KEY"] = "tok"
    reset("tok")
    SonderrSessions.resetInstanceAdvertisementForTests()
    ...
    clearInFlightCache("sonderr-sessions:token")
    clearInFlightCache("sonderr-sessions:token-valid:tok")

    // sonderr_change - only mock the specific endpoint authValid() calls
    // (${SONDERR_API_BASE}/api/user). A blanket mock that returned 200 for
  ...
  afterEach(async () => {
    ...
    await provide({
      directory: tmp.path,
      fn: async () => {
        SonderrSessions.disableRemote()
      },
    })
    pub.mockRestore()
    mock.restore()
    delete process.env["SONDERR_DISABLE_SESSION_INGEST"]
    delete process.env["SONDERR_SESSION_INGEST_URL"]
    delete process.env["SONDERR_PLATFORM"]
    delete process.env["SONDERR_API_KEY"]
    reset("tok")
  })

  // Reads the `getSessions` closure that sonderr-sessions.ts passed to
  ...
  test("enableRemote alone advertises the instance (covers /remote and auto-enable)", async () => {
    ...
    await provide({
      ...
      fn: async () => {
        ...
        await SonderrSessions.enableRemote()
  ...
  test("explicit set after enable replaces the payload (sonderr remote race)", async () => {
    await using tmp = await tmpdir({ git: true })
    await provide({
      directory: tmp.path,
      fn: async () => {
        ...
        SonderrSessions.setInstanceAdvertisement({
          name: "mbp-igor",
          projectName: "cloud",
          version: "1.2.3",
        })
  ...
  test("setter triggers an out-of-band heartbeat when a connection is already established", async () => {
    ...
    await provide({
      ...
      fn: async () => {
        ...
        SonderrSessions.setInstanceAdvertisement({ name: "h", projectName: "p" })
        await outOfBand
  ...
  test("setter replaces payload and fires one out-of-band heartbeat per call", async () => {
    ...
    await provide({
      ...
      fn: async () => {
        await SonderrSessions.enableRemote()
        SonderrSessions.setInstanceAdvertisement({ name: "first", projectName: "p" })
        await outOfBand
        const before = heartbeatCalls
        SonderrSessions.setInstanceAdvertisement({ name: "second", projectName: "p" })
        await outOfBand
  ...
  test("explicit set before enableRemote is preserved (no re-set on enable)", async () => {
    ...
    await provide({
      ...
      fn: async () => {
        ...
        SonderrSessions.setInstanceAdvertisement({ name: "pre-set", projectName: "proj", version: "9.9.9" })
        await SonderrSessions.enableRemote()
        const payload = await capturedGetSessions()()
  ...
  test("per-session platform resolution matches meta() order — env var fallback", async () => {
    ...
    //   SonderrSession.resolvePlatform(id) || process.env["SONDERR_PLATFORM"] || "cli"
    ...
    await provide({
      ...
      fn: async () => {
        ...
        const expectedPlatform = process.env["SONDERR_PLATFORM"] || "cli"
...
// sonderr_change start - K1 W1: real integration between SessionStatus,
...
// real SonderrSessions seams and proves that a non-idle status is cleared
// deterministically, which is exactly what lets the fence resolve and the
// exit_cli handler ACK.
describe("SonderrSessions.detachRemoteSession heartbeat fence (K1 W1)", () => {
  let heartbeatCalls = 0
  let outOfBand: Promise<void> | undefined
...
describe("SonderrSessions heartbeat attention status (DEF-3)", () => {
  beforeEach(() => {
    process.env["SONDERR_DISABLE_SESSION_INGEST"] = "0"
    delete process.env["SONDERR_SESSION_INGEST_URL"]
    process.env["SONDERR_API_KEY"] = "tok"
  ...
  test("reports permission when a permission request is pending", async () => {
    ...
    await provide({
      ...
      fn: async () => {
        ...
        const { AppRuntime } = await import("@/effect/app-runtime")
        const { Permission } = await import("@/permission")
        const { PermissionV1 } = await import("@sonderr/core/v1/permission")
  ...
  test("idle/busy/retry unchanged when no attention is pending", async () => {
    ...
    await provide({
      ...
      fn: async () => {
        ...
        await SonderrSessions.attachRemoteSession(idleId)
        await SonderrSessions.attachRemoteSession(busyId)
        await SonderrSessions.attachRemoteSession(retryId)
...
// sonderr_change - PR link advertise (plan 8.2): the heartbeat resolves the
...
describe("SonderrSessions PR link advertise (plan 8.2)", () => {
  ...
  test("stored override advertises prLink and ingests the set triple", async () => {
    ...
    await provide({
      ...
      fn: async () => {
        const id = await setupSession()
        await SonderrSessions.bootstrap(id)
