...
afterAll(async () => {
  ...
  for (const file of ["sonderr.jsonc", "sonderr.json", "config.json", "sonderr.json", "sonderr.jsonc"]) {
    ...
    withDir({ git: true }, () =>
      Effect.gen(function* () {
        const fiber = yield* ask({
          ...
          permission: "bash",
          patterns: ["sonderr-permission-8353 test"],
          metadata: { rules: ["sonderr-permission-8353 *", "sonderr-permission-8353 test"] },
          always: ["sonderr-permission-8353 *", "sonderr-permission-8353 test"],
          ruleset: [],
        }).pipe(Effect.forkScoped)
        ...
        yield* saveAlwaysRules({
          ...
          approvedAlways: ["sonderr-permission-8353 test"],
        })
        ...
        expect(cfg.permission?.bash).toMatchObject({ "sonderr-permission-8353 test": "allow" })
        expect(cfg.permission?.bash).not.toMatchObject({ "sonderr-permission-8353 *": "allow" })

        const broad = yield* ask({
          id: PermissionV1.ID.make("permission_saved_always_broad"),
          sessionID: SessionID.make("session_saved_always"),
          permission: "bash",
          patterns: ["sonderr-permission-8353 install"],
          metadata: {},
          always: [],
          ruleset: [],
        }).pipe(Effect.forkScoped)
