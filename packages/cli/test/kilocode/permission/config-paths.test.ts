...
import { ConfigProtection } from "../../../src/sonderr/permission/config-paths"
...
describe("ConfigProtection.isRequest", () => {
  ...
  const legacy = SonderrPaths.globalDirs().map((d) => path.resolve(d))
  ...
  test("returns false for file-tool external_directory targeting global config", () => {
    const result = ConfigProtection.isRequest({
      permission: "external_directory",
      patterns: [config + "/*"],
      metadata: { filepath: path.join(config, "sonderr.json"), parentDir: config },
    })
  ...
  test("returns true for edit targeting relative config path via patterns", () => {
    const result = ConfigProtection.isRequest({
      permission: "edit",
      patterns: [".sonderr/command/foo.md"],
    })
    expect(result).toBe(true)
  })
  ...
  test("returns false for edit targeting excluded subdir (plans)", () => {
    const result = ConfigProtection.isRequest({
      permission: "edit",
      patterns: [".sonderr/plans/plan.md"],
    })
    expect(result).toBe(false)
  })
  ...
  test("returns true for edit targeting root config files", () => {
    for (const file of ["sonderr.json", "sonderr.jsonc", "AGENTS.md"]) {
      const result = ConfigProtection.isRequest({
        permission: "edit",
        patterns: [file],
      })
  ...
  test("protects package lock files in project config directories", () => {
    for (const file of [".sonderr/package-lock.json", ".sonderr/package-lock.json"]) {
  ...
  test("protects a combined source and config lockfile edit", () => {
    expect(
      ConfigProtection.isRequest({
        permission: "edit",
        patterns: ["src/app/layout.tsx", ".sonderr/package-lock.json", ".sonderr/package-lock.json"],
        metadata: {
          filepath: "src/app/layout.tsx, .sonderr/package-lock.json, .sonderr/package-lock.json",
        },
      }),
    ).toBe(true)
  })
...
describe("ConfigProtection.isGlobalSkillRequest", () => {
  const roots = [Global.Path.config, ...SonderrPaths.globalDirs()]
  ...
  test("rejects glob characters in the canonical rule", async () => {
    ...
    const skill = path.join(root, "sonderr", "skills", "unsafe-root")
