...
import { SonderrConfig } from "../../../src/sonderr/config/config"
...
const cfg: Partial<Config.Info> = {
  plugin: ["@sonderr/sonderr-indexing"],
  indexing: {
    provider: "ollama",
    vectorStore: "qdrant",
    ollama: {
...
afterEach(async () => {
  delete process.env.SONDERR_MD_TEST
  await clear()
...
describe("markdown substitutions", () => {
  test("applies file and env substitutions to parsed markdown body", async () => {
    process.env.SONDERR_MD_TEST = "env content"
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(path.join(dir, "body.md"), "file content")
        await Filesystem.write(
          path.join(dir, "SKILL.md"),
          ["---", "name: test", "description: Test", "---", "{file:body.md}", "{env:SONDERR_MD_TEST}"].join("\n"),
        )
      },
    })
    ...
    try {
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          const config = await Bun.file(path.join(globalTmp.path, "sonderr.jsonc")).json()
...
describe("project MCP trust boundaries", () => {
  ...
  test("drops env-backed project MCP headers without dropping static siblings", async () => {
    ...
    await using tmp = await tmpdir({ git: true })
    const prev = Global.Path.config
    const secret = process.env.SONDERR_PROJECT_MCP_SECRET
    ;(Global.Path as { config: string }).config = globalTmp.path
    process.env.SONDERR_PROJECT_MCP_SECRET = "process-secret"
    await clear()
    await disposeAllInstances()
    ...
    try {
      await writeConfig(tmp.path, {
        mcp: {
          unsafe: {
            type: "remote",
            url: "https://project.example.com/unsafe",
            headers: { Authorization: "Bearer {env:SONDERR_PROJECT_MCP_SECRET}" },
          },
          sibling: {
            type: "remote",
      ...
      if (secret === undefined) delete process.env.SONDERR_PROJECT_MCP_SECRET
      else process.env.SONDERR_PROJECT_MCP_SECRET = secret
      ;(Global.Path as { config: string }).config = prev
      await clear()
      await disposeAllInstances()
    }
  ...
  test("does not carry global credentials through remote-local-remote project layers", async () => {
    ...
    try {
      ...
      await writeConfig(path.join(tmp.path, ".sonderr"), {
...
describe("sonderr web search config", () => {
...
describe("sonderr indexing config", () => {
  ...
  test("updates a project JSON config containing retired experimental flags", async () => {
    await using tmp = await tmpdir({ git: true })
    const file = path.join(tmp.path, ".sonderr", "sonderr.json")
    await Filesystem.write(
      file,
      JSON.stringify({
        username: "keep",
  ...
  test("updates a global JSONC config containing retired experimental flags", async () => {
    await using globalTmp = await tmpdir()
    await using tmp = await tmpdir()
    const file = path.join(globalTmp.path, "sonderr.jsonc")
    const prev = Global.Path.config
    ;(Global.Path as { config: string }).config = globalTmp.path
    await clear()
    await disposeAllInstances()
    ...
    try {
      ...
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          const config = await load()
          const input = SonderrIndexing.input(config.indexing, global.indexing)
  ...
  test("project indexing enabled overrides global enablement", async () => {
    const input = SonderrIndexing.input({ enabled: false }, { enabled: true })
    expect(input.enabled).toBe(false)
    expect(SonderrIndexing.input(undefined, { enabled: true }).enabled).toBe(true)
    expect(SonderrIndexing.input({ enabled: true }, { enabled: false }).enabled).toBe(true)
  })

  test("creates missing project config as .sonderr/sonderr.jsonc", async () => {
    await using tmp = await tmpdir({ git: true })
    ...
    expect(await Bun.file(path.join(tmp.path, ".sonderr", "sonderr.jsonc")).exists()).toBe(true)
    expect(await Bun.file(path.join(tmp.path, ".sonderr", "sonderr.json")).exists()).toBe(false)
  })

  test("accepts delete sentinels for indexing model overrides", () => {
    const patch = decode({ indexing: { model: null, dimension: null } })
    const merged = SonderrConfig.mergeConfig(
      {
        indexing: {
          provider: "openai",
    ...
    const input = SonderrIndexing.input(patch.indexing)
...
describe("sonderr sandbox config", () => {
...
describe("custom provider model config", () => {
  test("persists and removes reasoning across a global config reload", async () => {
    await using globalTmp = await tmpdir()
    const file = path.join(globalTmp.path, "sonderr.json")
    const prev = Global.Path.config
    ;(Global.Path as { config: string }).config = globalTmp.path
    await clear()
    await disposeAllInstances()
...
describe("unset propagation across layered config files", () => {
  ...
  test("removes subagent_model from every global config file when unset", async () => {
    await using globalTmp = await tmpdir()
    const json = path.join(globalTmp.path, "sonderr.json")
    const jsonc = path.join(globalTmp.path, "sonderr.jsonc")
    ...
    try {
      await writeConfig(globalTmp.path, { subagent_model: "sonderr/openai/gpt-5" }, "sonderr.json")
      await Filesystem.write(jsonc, jsoncText)

      await saveGlobal(decode({ subagent_model: null }))

      // The key must be gone from the lower-precedence sonderr.json as well, or
  ...
  test("removes nested sentinels from jsonc siblings while preserving comments", async () => {
    await using globalTmp = await tmpdir()
    const sonderr = path.join(globalTmp.path, "sonderr.jsonc")
    const prev = Global.Path.config
    ;(Global.Path as { config: string }).config = globalTmp.path
    await clear()
    await disposeAllInstances()
    ...
    try {
      await Filesystem.write(path.join(globalTmp.path, "sonderr.jsonc"), '{ "username": "marius" }\n')
      await Filesystem.write(
        sonderr,
        [
          "{",
          ...
          '  "agent": {',
          '    "explore": {',
          '      "model": "sonderr/anthropic/claude-sonnet-4-6",',
          '      "description": "Keep me"',
          "    }",
          "  }",
          "}",
      ...
      const written = await Bun.file(sonderr).text()
  ...
  test("does not rewrite sibling files on set or when the key is absent", async () => {
    await using globalTmp = await tmpdir()
    const json = path.join(globalTmp.path, "sonderr.json")
    const jsonText = JSON.stringify({ subagent_model: "sonderr/old-model", username: "marius" }, null, 2)
    const prev = Global.Path.config
    ;(Global.Path as { config: string }).config = globalTmp.path
    await clear()
    await disposeAllInstances()
    ...
    try {
      ...
      await saveGlobal(decode({ subagent_model: "sonderr/new-model" }))
  ...
  test("removes subagent_model from every project config file when unset", async () => {
    ...
    await Filesystem.write(
      ...
      JSON.stringify({ subagent_model: "sonderr/openai/gpt-5" }),
    )
    await Filesystem.write(path.join(tmp.path, ".sonderr", "sonderr.jsonc"), '{\n  "username": "keep"\n}\n')
    ...
    const json = JSON.parse(await Bun.file(path.join(tmp.path, ".sonderr", "sonderr.json")).text())
    expect(json).not.toHaveProperty("subagent_model")
    // The primary target only gained nothing; the delete was a no-op there.
    const jsonc = JSON.parse(await Bun.file(path.join(tmp.path, ".sonderr", "sonderr.jsonc")).text())
...
describe("project plugin dependencies", () => {
  ...
  test("does not install dependencies for an ordinary project config directory", async () => {
    await sandbox(async (dir) => {
      await writeConfig(path.join(dir, ".sonderr"), { username: "sonderr" })
  ...
  test("installs dependencies for an auto-discovered file plugin and waits for completion", async () => {
    await sandbox(async (dir) => {
      const config = path.join(dir, ".sonderr")
      ...
      expect(calls).toEqual([{ dir: config, name: "@sonderr/plugin" }])
    })
  })
...
describe("agent config", () => {
  ...
  test("removes an agent variant override without removing its model", () => {
    ...
    expect(patch.agent?.explore?.variant).toBeNull()
    expect(merged.agent?.explore).toEqual({ model: "sonderr/anthropic/claude-sonnet-4-6" })
  })
...
describe("project config directory precedence", () => {
  test("prefers .sonderr over legacy .sonderr and ignores .sonderr", async () => {
    await using tmp = await tmpdir()
    const entries = [
      {
        root: ".sonderr",
        ...
        config: {
          username: "sonderr",
          model: "test/sonderr",
          small_model: "test/sonderr",
        },
        names: ["shared", "legacy", "sonderr-only"],
      },
      {
        root: ".sonderr",
        source: "sonderr",
    ...
    await provideTestInstance({
      ...
      fn: async () => {
        ...
        expect(config.username).toBe("sonderr")
        expect(config.model).toBe("test/sonderr")
        expect(config.small_model).toBeUndefined()

        expect(config.command?.shared).toMatchObject({
          description: "sonderr command",
          template: "sonderr command template",
        })
        expect(config.command?.legacy).toMatchObject({
          description: "sonderr command",
          template: "sonderr command template",
        })
        expect(config.command?.["sonderr-only"]).toBeUndefined()

        expect(config.agent?.shared).toMatchObject({
          description: "sonderr agent",
          prompt: "sonderr agent prompt",
        })
        expect(config.agent?.legacy).toMatchObject({
          description: "sonderr agent",
          prompt: "sonderr agent prompt",
        })
        expect(config.agent?.["sonderr-only"]).toBeUndefined()

        const plugins = JSON.stringify(config.plugin)
        expect(plugins).toContain("sonderr.ts")
        expect(plugins).toContain("sonderr.ts")
        expect(plugins).not.toContain("sonderr.ts")
      },
    })
  })
})
...
describe("linked worktree config", () => {
  test("uses primary config directories as local fallbacks", async () => {
    ...
    await Bun.write(path.join(primary.path, "sonderr.json"), JSON.stringify({ model: "test/primary" }))
    await $`git add sonderr.json`.cwd(primary.path).quiet()
    ...
    try {
      await Bun.write(path.join(worktree, "sonderr.json"), JSON.stringify({ model: "test/worktree" }))
      await Bun.write(
        path.join(primary.path, ".sonderr", "sonderr.jsonc"),
        JSON.stringify({ username: "primary-dir", indexing: { enabled: true } }),
      )
      await Bun.write(path.join(worktree, ".sonderr", "sonderr.jsonc"), JSON.stringify({ username: "worktree-dir" }))
  ...
  test("uses nested primary config directories as local fallbacks", async () => {
    ...
    try {
      ...
      await Bun.write(
        path.join(primary.path, "packages", ".sonderr", "sonderr.jsonc"),
        JSON.stringify({ snapshot: true, autoupdate: false, share: "auto", default_agent: "sonderr-only" }),
      )
      await Bun.write(
        path.join(primary.path, "packages", ".sonderr", "sonderr.jsonc"),
        JSON.stringify({ snapshot: true, autoupdate: "notify", share: "disabled" }),
      )
      await Bun.write(path.join(primary.path, "packages", ".sonderr", "sonderr.jsonc"), JSON.stringify({ snapshot: false }))
      await Bun.write(path.join(directory, ".sonderr", "sonderr.jsonc"), JSON.stringify({ share: "manual" }))
  ...
  test("keeps SONDERR_CONFIG_DIR above the primary fallback", async () => {
    await using primary = await tmpdir({ git: true })
    ...
    await Bun.write(path.join(primary.path, ".sonderr", "sonderr.jsonc"), JSON.stringify({ username: "primary-dir" }))
    await Bun.write(path.join(explicit.path, "sonderr.jsonc"), JSON.stringify({ username: "explicit-dir" }))
    const previous = process.env["SONDERR_CONFIG_DIR"]
    process.env["SONDERR_CONFIG_DIR"] = explicit.path

    try {
      const config = await provideTestInstance({ directory: worktree, fn: load })
      expect(config.username).toBe("explicit-dir")
    } finally {
      if (previous === undefined) delete process.env["SONDERR_CONFIG_DIR"]
      else process.env["SONDERR_CONFIG_DIR"] = previous
      await $`git worktree remove --force ${worktree}`.cwd(primary.path).quiet().nothrow()
    }
  })
})

describe("sonderr config migration notice", () => {
  ...
  test("detects a project .sonderr directory", async () => {
    await using globalTmp = await tmpdir()
    await using tmp = await tmpdir()
    await Filesystem.write(path.join(tmp.path, ".sonderr", "sonderr.json"), JSON.stringify({ model: "test/legacy" }))

    // Isolate the global config dir so a real ~/.config/sonderr on the host cannot interfere.
    await withGlobalConfig(path.join(globalTmp.path, "sonderr"), () => {
      const found = SonderrConfig.detectSonderrConfig({ directory: tmp.path, scanProject: true })
      expect(found).toEqual([path.join(tmp.path, ".sonderr")])
    })
  })

  test("detects a global sonderr config directory", async () => {
    await using globalTmp = await tmpdir()
    await using tmp = await tmpdir()
    const sonderrDir = path.join(globalTmp.path, "sonderr")
    await Filesystem.write(path.join(sonderrDir, "sonderr.json"), JSON.stringify({ model: "test/legacy" }))

    await withGlobalConfig(path.join(globalTmp.path, "sonderr"), () => {
      const found = SonderrConfig.detectSonderrConfig({ directory: tmp.path, scanProject: true })
      expect(found).toEqual([sonderrDir])
    })
  })
  ...
  test("skips the project scan when disabled", async () => {
    ...
    await withGlobalConfig(path.join(globalTmp.path, "sonderr"), () => {
      const found = SonderrConfig.detectSonderrConfig({ directory: tmp.path, scanProject: false })
      expect(found).toEqual([])
    })
  })

  test("builds a dismissible notification when sonderr config exists", async () => {
    await using globalTmp = await tmpdir()
    await using tmp = await tmpdir()
    await Filesystem.write(path.join(tmp.path, ".sonderr", "sonderr.json"), JSON.stringify({ model: "test/legacy" }))

    await withGlobalConfig(path.join(globalTmp.path, "sonderr"), () => {
      const notice = SonderrConfig.sonderrConfigNotification({ directory: tmp.path, scanProject: true })
      expect(notice?.id).toBe(SonderrConfig.SONDERR_NOTIFICATION_ID)
      expect(notice?.message).toContain(path.join(tmp.path, ".sonderr"))
      expect(notice?.action?.actionURL).toBe(SonderrConfig.CONFIG_DOCS_URL)
...
describe("bash permission migration", () => {
  ...
  "permission": "${action}"
      ...
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Filesystem.write(path.join(dir, "sonderr.jsonc"), input)
        },
      })
      ...
      try {
        await SonderrConfig.migrateBashPermission()

        const file = path.join(tmp.path, "sonderr.jsonc")
    ...
    test(`preserves string-form ${action} permission in json`, async () => {
      ...
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Filesystem.write(path.join(dir, "sonderr.json"), input)
        },
      })
