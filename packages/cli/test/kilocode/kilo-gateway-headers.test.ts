...
import { buildSonderrHeaders, getFeatureHeader, getEditorNameHeader } from "@sonderr/sonderr-gateway"
import { HEADER_FEATURE, ENV_FEATURE, ENV_EDITOR_NAME, ENV_VERSION, DEFAULT_EDITOR_NAME } from "@sonderr/sonderr-gateway"
...
describe("getEditorNameHeader", () => {
  ...
  it("returns default editor name without version when SONDERR_VERSION is not set", () => {
    delete process.env[ENV_VERSION]
    expect(getEditorNameHeader()).toBe(DEFAULT_EDITOR_NAME)
  })

  it("appends version when SONDERR_VERSION is set", () => {
    process.env[ENV_VERSION] = "1.2.3"
    expect(getEditorNameHeader()).toBe(`${DEFAULT_EDITOR_NAME} 1.2.3`)
  })
})

describe("buildSonderrHeaders", () => {
  const original = process.env[ENV_FEATURE]
  ...
  it("includes feature header when env var is set", () => {
    ...
    const headers = buildSonderrHeaders()
  ...
  it("always includes editor name header", () => {
    delete process.env[ENV_FEATURE]
    const headers = buildSonderrHeaders()
    expect(headers["X-SONDERR-EDITORNAME"]).toBe(getEditorNameHeader())
  })
