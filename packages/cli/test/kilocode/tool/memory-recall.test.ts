...
import { MemoryRecallTool } from "@/sonderr/tool/memory-recall"
...
describe("sonderr_memory_recall description", () => {
...
describe("sonderr_memory_recall", () => {
  ...
  test("shows typed memory hits separately from session digests", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".sonderr"), async () => {
      const memory = { directory: dir.path, worktree: dir.path }
      const enabled = await SonderrMemory.enable({ ctx: memory })
        ...
        ops: [
          {
            ...
            section: "Commands",
            key: "vscode_tests",
            text: "Run VS Code unit tests from packages/sonderr-vscode with bun run test:unit.",
          },
          {
            action: "add",
            file: "project.md",
      ...
      expect(typed.title).toContain("Sonderr memory typed")
      ...
      expect(digest.title).toContain("Sonderr memory digest")
  ...
  test("digest sessionID recall returns full summaries while index stays brief", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".sonderr"), async () => {
      const memory = { directory: dir.path, worktree: dir.path }
      const enabled = await SonderrMemory.enable({ ctx: memory })
      const tail = "SONDERR_FULL_DETAIL_AFTER_480"
  ...
  test("catalog mode lists all stored keys with optional filter", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".sonderr"), async () => {
        ...
        ops: [
          {
            ...
            file: "project.md",
            section: "Facts",
            key: "sonderr_was_originally_a_fork",
            text: "sonderr was originally a fork of roo and has a sonderr-legacy repo",
          },
          {
            action: "add",
            file: "environment.md",
      ...
      expect(all.output).toContain("sonderr_was_originally_a_fork")
      expect(all.output).toContain("vscode_tests")
      expect(filtered.output).toContain("sonderr_was_originally_a_fork")
  ...
  test("catalog mode lists saved session digests", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".sonderr"), async () => {
      ...
      await SonderrMemory.recordSession({
        ...
        summary: "Discussed merging upstream sonderr changes.",
        time: Date.UTC(2026, 0, 1, 0, 0),
      })
  ...
  test("digest recall honors the requested saved session id", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".sonderr"), async () => {
      ...
      await SonderrMemory.recordSession({
        ...
        topic: "Sonderr plugin architecture",
      ...
      await SonderrMemory.recordSession({
        ...
        summary: "Discussed minimizing shared upstream file edits under packages/cli.",
  ...
  test("typed and search modes require a topic query", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".sonderr"), async () => {
      ...
      await SonderrMemory.apply({
        ctx: memory,
        ops: [{ action: "add", key: "cli_tests", text: "Run CLI tests from packages/cli." }],
      })
  ...
  test("search mode renders typed and digest memory without catalog mode", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".sonderr"), async () => {
        ...
        ops: [
          {
            ...
            section: "Commands",
            key: "cli_tests",
            text: "Run CLI tests from packages/cli with bun test.",
          },
        ],
      })
      await SonderrMemory.recordSession({
      ...
      expect(result.title).toContain("Sonderr memory search")
  ...
  test("handles large unmatched recall queries without returning stored memory", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".sonderr"), async () => {
      ...
      await SonderrMemory.apply({
        root: enabled.root,
        ops: [{ action: "add", key: "cli_tests", text: "Run CLI tests from packages/cli with bun test." }],
      })

      const result = await execute(dir.path, { mode: "search", query: "zzzz ".repeat(2000), limit: 50 })

      expect(result.title).toBe("Sonderr memory search: no results")
