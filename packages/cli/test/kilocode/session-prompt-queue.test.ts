...
import { SonderrSessionCompaction } from "@/sonderr/session/compaction"
import { SonderrSessionPromptQueue } from "@/sonderr/session/prompt-queue"
import { SonderrSession } from "@/sonderr/session"
import { Suggestion } from "../../src/sonderr/suggestion"
...
const previous = Flag.SONDERR_DB
const dbfile = path.join(os.tmpdir(), `sonderr-prompt-queue-${process.pid}-${crypto.randomUUID()}.db`)
...
describe("session prompt queue", () => {
  test("scopes queued turns without moving prior assistant history", async () => {
    ...
    const ids = await Effect.runPromise(
      SonderrSessionPromptQueue.enqueue(
        sessionID,
        two,
        Effect.sync(() => SonderrSessionPromptQueue.scope(sessionID, messages).map((item) => item.info.id)),
        Effect.succeed([]),
      ),
    )
  ...
  test("retarget keeps older queued prompts hidden", async () => {
    ...
    const result = await Effect.runPromise(
      SonderrSessionPromptQueue.enqueue(
        ...
        base,
        Effect.sync(() => {
          SonderrSessionPromptQueue.retarget(sessionID, injected)
          return {
            active: SonderrSessionPromptQueue.active(sessionID),
            ids: SonderrSessionPromptQueue.scope(sessionID, messages).map((item) => item.info.id),
          }
        }),
          ...
          SonderrSessionPromptQueue.enqueue(
            ...
            Effect.promise(async () => {
              await Effect.runPromise(
                SonderrSessionCompaction.create({
                  session: store,
              ...
              return { compact, ids: SonderrSessionPromptQueue.scope(session.id, messages).map((item) => item.info.id) }
            }),
  ...
  test("hasFollowup reports true only for prompts enqueued after the active slot started", async () => {
    ...
    const first = Effect.runPromise(
      SonderrSessionPromptQueue.enqueue(
        ...
        Effect.gen(function* () {
          observed.push({ where: "first:start", value: SonderrSessionPromptQueue.hasFollowup(sessionID) })
          firstStarted.resolve()
          yield* Effect.promise(() => firstReleased.promise)
          observed.push({ where: "first:end", value: SonderrSessionPromptQueue.hasFollowup(sessionID) })
          return "first"
        }),
    ...
    const second = Effect.runPromise(
      SonderrSessionPromptQueue.enqueue(
        ...
        Effect.gen(function* () {
          observed.push({ where: "second:start", value: SonderrSessionPromptQueue.hasFollowup(sessionID) })
    ...
    expect(SonderrSessionPromptQueue.hasFollowup(sessionID)).toBe(true)
    ...
    const third = Effect.runPromise(
      SonderrSessionPromptQueue.enqueue(
        ...
        Effect.sync(() => {
          observed.push({ where: "third:start", value: SonderrSessionPromptQueue.hasFollowup(sessionID) })
          return "third"
        }),
    ...
    expect(SonderrSessionPromptQueue.hasFollowup(sessionID)).toBe(false)
  ...
  test("processes queued prompts without aborting the in-flight stream", async () => {
    ...
    try {
      await using tmp = await tmpdir({
        git: true,
        init: async (dir) => {
          await Bun.write(path.join(dir, "sonderr.json"), JSON.stringify(providerCfg(server.url.origin)))
        },
      })
  ...
  test("closes a queued-handoff turn as superseded, not interrupted", async () => {
    ...
    try {
      ...
      await provideTestInstance({
        ...
        fn: async () =>
          scoped(tmp.path, async (prompt) => {
            const closed: SonderrSession.CloseReason[] = []
            const unsubscribe = Bus.subscribe(SonderrSession.Event.TurnClose, (event) => {
            ...
            await Effect.runPromise(
              pollWithTimeout(
                Effect.sync(() => (SonderrSessionPromptQueue.hasFollowup(session.id) ? (true as const) : undefined)),
  ...
  test("cancel on a session with no active tail is a no-op and does not leak state", async () => {
    ...
    await Effect.runPromise(SonderrSessionPromptQueue.cancel(sessionID))

    expect(SonderrSessionPromptQueue._hasInternalState(sessionID)).toBe(false)
        ...
        fn: async () =>
          scoped(tmp.path, async (prompt) => {
            ...
            await Effect.runPromise(
              pollWithTimeout(
                Effect.sync(() =>
                  SonderrSessionPromptQueue.snapshot(session.id).length >= 2 ? (true as const) : undefined,
                ),
            ...
            const ids = await Effect.runPromise(
              SonderrSessionPromptQueue.enqueue(
                session.id,
                MessageID.make("msg_probe"),
                Effect.succeed(SonderrSessionPromptQueue.scope(session.id, []).map((item) => item.info.id)),
                Effect.succeed([]),
              ),
            )
            expect(ids).toEqual([])
            expect(SonderrSessionPromptQueue.hasFollowup(session.id)).toBe(false)
          }),
      })
    } finally {
      server.stop(true)
      ...
      await provideTestInstance({
        ...
        fn: async () =>
          scoped(tmp.path, async (prompt) => {
            ...
            await Effect.runPromise(
              pollWithTimeout(
                Effect.sync(() => (SonderrSessionPromptQueue.hasFollowup(session.id) ? true : undefined)),
                "Timed out waiting for queued prompt",
              ),
            )
            expect(await Effect.runPromise(SonderrSessionPromptQueue.drop(session.id, queuedID))).toBe(true)
            expect(await Effect.runPromise(SonderrSessionPromptQueue.drop(session.id, activeID))).toBe(false)
  ...
  test("drop returns false for the actively running prompt", async () => {
    ...
    await ready.promise
    expect(await Effect.runPromise(SonderrSessionPromptQueue.drop(sessionID, id))).toBe(false)
    done.resolve()
    await first
  })
  ...
  test("drop returns false for an unknown message", async () => {
    ...
    expect(await Effect.runPromise(SonderrSessionPromptQueue.drop(sessionID, MessageID.make("msg_missing")))).toBe(false)
  })
  ...
  test("drop cancels a queued prompt and is idempotent", async () => {
    ...
    expect(await Effect.runPromise(SonderrSessionPromptQueue.drop(sessionID, secondID))).toBe(true)
    expect(await Effect.runPromise(SonderrSessionPromptQueue.drop(sessionID, secondID))).toBe(false)
    done.resolve()
  ...
  describe("session.queue.changed", () => {
    test("snapshot() returns an empty list for an unknown session", () => {
      expect(SonderrSessionPromptQueue.snapshot(SessionID.make("session_unknown"))).toEqual([])
    })
    ...
    test("enqueueing on an idle session does not transiently publish a non-empty snapshot", async () => {
      ...
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          const off = Bus.subscribe(SonderrSession.Event.QueueChanged, (event) => {
          ...
          try {
            ...
            expect(SonderrSessionPromptQueue.snapshot(sessionID)).toEqual([])
      ...
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          try {
            ...
            expect(SonderrSessionPromptQueue.snapshot(sessionID)).toEqual([m2, m3])
      ...
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          try {
            ...
            expect(await Effect.runPromise(SonderrSessionPromptQueue.drop(sessionID, m2))).toBe(true)
            expect(SonderrSessionPromptQueue.snapshot(sessionID)).toEqual([m3])
            expect(SonderrSessionPromptQueue.hasFollowup(sessionID)).toBe(true)
            await Effect.runPromise(
              pollWithTimeout(
      ...
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          try {
            ...
            expect(SonderrSessionPromptQueue.snapshot(sessionID)).toEqual([m2])
            await Bun.sleep(10)
