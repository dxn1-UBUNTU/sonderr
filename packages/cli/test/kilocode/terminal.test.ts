...
import { kitty, sequences } from "../../src/sonderr/cli/cmd/tui/util/terminal"

const keys = ["TERM_PROGRAM", "MSYSTEM", "SONDERR_DISABLE_KITTY_KEYBOARD", "SONDERR_ENABLE_KITTY_KEYBOARD"] as const
...
test("allows explicitly enabling Kitty keyboard", () => {
  env({ SONDERR_ENABLE_KITTY_KEYBOARD: "1", MSYSTEM: "MINGW64" })
...
test("allows explicitly disabling Kitty keyboard", () => {
  env({ SONDERR_DISABLE_KITTY_KEYBOARD: "1", SONDERR_ENABLE_KITTY_KEYBOARD: "1" })
...
test("resets common terminal input modes", () => {
  env({ SONDERR_DISABLE_KITTY_KEYBOARD: "1" })
