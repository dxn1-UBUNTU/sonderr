// Unit tests for SonderrSessionPrompt history-trim / media-strip helpers.
...
import { SonderrSessionMessageOrder } from "../../src/sonderr/session/message-order"
...
describe("SonderrSessionPrompt.hasCompletedSummary", () => {
  test("returns false for empty array", () => {
    expect(SonderrSessionPrompt.hasCompletedSummary([])).toBe(false)
  })
  ...
  test("returns false when only summary has an error", () => {
    ...
    expect(SonderrSessionPrompt.hasCompletedSummary(msgs)).toBe(false)
  })
  ...
  test("returns true when summary has finish and no error", () => {
    ...
    expect(SonderrSessionPrompt.hasCompletedSummary(msgs)).toBe(true)
  })
})
...
describe("MessageV2.latest", () => {
  test("selects chronological state after retained pre-compaction tail", () => {
    ...
    const state = SonderrSessionMessageOrder.latest(msgs)
  ...
  test("keeps queued subtasks moved after a chronologically later assistant", () => {
    ...
    SonderrSessionMessageOrder.annotate([active, queued, done])

    const state = SonderrSessionMessageOrder.latest([active, done, queued])
...
describe("SonderrSessionPrompt.trimBeforeLastSummary", () => {
  ...
  test("trims to summary's text-user parent (reference-session bug)", () => {
    ...
    const result = SonderrSessionPrompt.trimBeforeLastSummary(filtered)
  ...
  test("keeps the newest summary when retained history contains an older summary", () => {
    ...
    expect(SonderrSessionPrompt.trimBeforeLastSummary(filtered)).toBe(filtered)
  })
  ...
  test("is idempotent", () => {
    ...
    const first = SonderrSessionPrompt.trimBeforeLastSummary(msgs)
    const second = SonderrSessionPrompt.trimBeforeLastSummary(first)
...
describe("SonderrSessionPrompt.stripHistoricalMedia", () => {
  test("replaces image file part in historical user message with placeholder text", () => {
    ...
    const result = SonderrSessionPrompt.stripHistoricalMedia(msgs)
  ...
  test("does not mutate the input", () => {
    ...
    const result = SonderrSessionPrompt.stripHistoricalMedia(input)
...
describe("SonderrSessionPrompt.maybeStripHistoricalMedia", () => {
  test("returns input unchanged when no completed summary exists", () => {
    const msgs = [
      user("msg_hist", [filePart("msg_hist", "image/png", "hist.png")]),
      user("msg_last", [textPart("msg_last", "follow-up")]),
    ]
    const result = SonderrSessionPrompt.maybeStripHistoricalMedia(msgs)
    expect(result).toBe(msgs)
  })
