...
//
// Unit tests for SonderrSnapshotTrack.wrap — the slow-repo guard that sits
...
import { SonderrSnapshotTrack } from "../../src/sonderr/snapshot/track"
import { SonderrPartLifecycle } from "../../src/sonderr/session/part-lifecycle"
...
const makeHooks = (
  answer: SonderrSnapshotTrack.Answer | Promise<SonderrSnapshotTrack.Answer>,
): { hooks: SonderrSnapshotTrack.Hooks; calls: Calls } => {
  const calls: Calls = { ask: 0, persist: 0, progress: [] }
  const hooks: SonderrSnapshotTrack.Hooks = {
    async ask() {
      calls.ask += 1
      return answer
    },
...
describe("SonderrSnapshotTrack.protect", () => {
  it.effect("returns at the availability deadline without waiting for cancellation", () =>
    Effect.gen(function* () {
      const state = SonderrSnapshotTrack.makeState()
      ...
      const fiber = yield* SonderrSnapshotTrack.protect({
        inner,
        state,
        fallback,
        operation: "patch",
  ...
  it.effect("bypasses later operations after a deadline opens the directory circuit", () =>
    Effect.gen(function* () {
      ...
      const started = yield* Deferred.make<void>()
      let calls = 0
      const first = yield* SonderrSnapshotTrack.protect({
        inner: Effect.sync(() => {
          calls += 1
      ...
      const second = yield* SonderrSnapshotTrack.protect({
        inner: Effect.sync(() => {
          calls += 1
          return "unexpected"
        }),
  ...
  test("keeps circuit state isolated by directory", () => {
    const states = SonderrSnapshotTrack.makeStates()
...
describe("SonderrSnapshotTrack.wrap", () => {
  ...
  test("concurrent timeout does not override an active continue choice", async () => {
    const state = SonderrSnapshotTrack.makeState()
    const answer = Promise.withResolvers<SonderrSnapshotTrack.Answer>()
    const asked = Promise.withResolvers<void>()
    const { hooks, calls } = makeHooks(answer.promise)
    const firstHooks: SonderrSnapshotTrack.Hooks = {
      ...hooks,
      async ask(input) {
        asked.resolve()
...
describe("SonderrSnapshotTrack progress indicator", () => {
  test("classifies persisted progress as transient", () => {
    const part = SonderrSnapshotTrack.progressPart({
      sessionID: SESSION,
      messageID: MESSAGE,
    ...
    expect(SonderrPartLifecycle.transient(part)).toBe(true)
  })
  ...
  test("slow-but-succeeding path (under timeout) starts then ends the indicator", async () => {
    ...
    expect(withoutFrame(firstText.text)).toBe(
      SonderrSnapshotTrack.formatProgress(SonderrSnapshotTrack.PROGRESS_INITIALIZING, "").trim(),
    )
  ...
  test("timed-out path keeps the initializing label (no text escalation)", async () => {
    ...
    const base = SonderrSnapshotTrack.formatProgress(SonderrSnapshotTrack.PROGRESS_INITIALIZING, "").trim()
    for (const evt of events) {
...
describe("SonderrSnapshotTrack persistDisable", () => {
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const state = SonderrSnapshotTrack.makeState()
        const hooks: SonderrSnapshotTrack.Hooks = {
          ...SonderrSnapshotTrack.defaultHooks,
          async ask() {
            return "disable"
          },
          async startProgress() {},
        ...
        yield* SonderrSnapshotTrack.wrap({
          inner: hangInner(),
          state,
          sessionID: SESSION,
          messageID: MESSAGE,
        ...
        const file = path.join(test.directory, ".sonderr", "sonderr.jsonc")
  ...
  test("persistDisable without instance context does not throw", async () => {
    await SonderrSnapshotTrack.defaultHooks.persistDisable()
  })
})

describe("SonderrSnapshotTrack constants", () => {
  test("TIMEOUT_MS defaults to 10s and respects env override", () => {
    ...
    // with SONDERR_SNAPSHOT_TRACK_TIMEOUT_MS, which this test suite does not set.
    expect(SonderrSnapshotTrack.TIMEOUT_MS).toBe(10_000)
  })

  test("exposes stable answer labels", () => {
    expect(SonderrSnapshotTrack.ANSWER_CONTINUE).toBe("Continue with snapshots")
    expect(SonderrSnapshotTrack.ANSWER_DISABLE).toBe("Disable for this project")
  })
})
