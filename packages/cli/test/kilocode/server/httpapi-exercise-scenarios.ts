...
const skill = async (dir: string) => {
  await Bun.write(
    path.join(dir, ".sonderr/skill/httpapi-remove/SKILL.md"),
    "---\nname: httpapi-remove\ndescription: HTTP API removal fixture.\n---\n# HTTP API remove\n",
  )
  await Bun.write(path.join(dir, ".sonderr/skill/httpapi-remove/KEEP.txt"), "synthetic sentinel\n")
}

const agent = async (dir: string) => {
  await Bun.write(
    path.join(dir, ".sonderr/agent/httpapi-remove.md"),
    "---\ndescription: HTTP API remove\n---\nRemove me.\n",
  )
}

const duplicates = async (dir: string) => {
  for (const name of ["sonderr.jsonc", "sonderr.jsonc"]) {
    await Bun.write(
      path.join(dir, ".sonderr", name),
      JSON.stringify({
...
const command = async (dir: string) => {
  await Bun.write(
    path.join(dir, ".sonderr/command/httpapi-remove.md"),
...
function enable(ctx: ScenarioContext) {
  const dir = directory(ctx)
  return Effect.promise(() => SonderrMemory.enable({ ctx: { directory: dir, worktree: dir } }))
}
...
export const sonderrScenarios: Scenario[] = [
  ...
  http.protected.get("/sonderr/profile", "sonderr.profile").probe({ path: "/path" }).status(401),
  http.protected.get("/sonderr/auth-status", "sonderr.authStatus").json(200, (body) => {
    object(body)
    check(body.authenticated === false, "Sonderr auth status should report signed out")
    check(body.type === undefined, "Sonderr auth status should not expose a credential type while signed out")
  }),
  http.protected.get("/sonderr/modes", "sonderr.modes").json(200, (body) => {
    object(body)
    array(body.modes)
  }),
  http.protected
    .post("/sonderr/fim", "sonderr.fim")
    .at((ctx) => ({ path: "/sonderr/fim", headers: ctx.headers(), body: { prefix: "const value = ", suffix: "\n" } }))
    .status(401),
  http.protected
    .post("/sonderr/edit", "sonderr.edit")
    .at((ctx) => ({ path: "/sonderr/edit", headers: ctx.headers(), body: edit }))
    .status(401),
  http.protected
    .post("/sonderr/audio/transcriptions", "sonderr.audio.transcriptions")
    .at((ctx) => ({
      path: "/sonderr/audio/transcriptions",
      headers: ctx.headers(),
      body: { model: "whisper-1", input_audio: { data: "", format: "wav" } },
    }))
    .status(401),
  http.protected.get("/sonderr/notifications", "sonderr.notifications").json(200, array),
  http.protected.get("/sonderr/models/images", "sonderr.models.images").probe({ path: "/path" }).status(401),
  http.protected.get("/sonderr/models/transcriptions", "sonderr.models.transcriptions").probe({ path: "/path" }).status(401),
  http.protected
    .post("/sonderr/organization", "sonderr.organization.set")
    .at((ctx) => ({ path: "/sonderr/organization", headers: ctx.headers(), body: { organizationId: null } }))
    .status(401),
  http.protected.get("/sonderr/claw/status", "sonderr.claw.status").probe({ path: "/path" }).status(401),
  http.protected.get("/sonderr/claw/chat-credentials", "sonderr.claw.chatCredentials").probe({ path: "/path" }).status(401),
  http.protected.get("/sonderr/cloud-sessions", "sonderr.cloudSessions").probe({ path: "/path" }).status(401),
  http.protected
    .get("/sonderr/cloud/session/{id}", "sonderr.cloud.session.get")
    .probe({ path: "/path" })
    .at((ctx) => ({ path: route("/sonderr/cloud/session/{id}", { id: "httpapi-missing" }), headers: ctx.headers() }))
    .status(401),
  http.protected
    .post("/sonderr/cloud/session/import", "sonderr.cloud.session.import")
    .at((ctx) => ({ path: "/sonderr/cloud/session/import", headers: ctx.headers(), body: { sessionId: "missing" } }))
    .status(401),
  ...
  http.protected
    ...
    .status(400),
  http.protected
    .post("/sonderr/session/{sessionID}/resume", "sonderr.resumeSession")
    .seeded((ctx) => ctx.session({ title: "Empty resume" }))
    .at((ctx) => ({
      path: route("/sonderr/session/{sessionID}/resume", { sessionID: ctx.state.id }),
      headers: ctx.headers(),
      body: { messageID: "msg_httpapi_missing" },
    }))
    .status(400),
  http.protected
    .get("/session/{sessionID}/model-usage", "sonderr.sessionModelUsage")
  ...
  http.protected
    .get("/sonderr/background-jobs", "sonderr.backgroundJobs")
    .at((ctx) => ({
      path: "/sonderr/background-jobs?sessionID=ses_httpapi_missing",
      headers: ctx.headers(),
    }))
    .json(200, (body) => {
      array(body)
  ...
  http.protected
    .post("/sonderr/background-jobs/{jobID}/cancel", "sonderr.backgroundJob.cancel")
    .at((ctx) => ({
      path: route("/sonderr/background-jobs/{jobID}/cancel", { jobID: "job_httpapi_missing" }),
      headers: ctx.headers(),
    }))
    .status(404),
  http.protected
    .post("/sonderr/background-jobs/{jobID}/promote", "sonderr.backgroundJob.promote")
    .at((ctx) => ({
      path: route("/sonderr/background-jobs/{jobID}/promote", { jobID: "job_httpapi_missing" }),
      headers: ctx.headers(),
    }))
    .status(404),
  http.protected
    .post("/sonderr/heap/snapshot", "sonderr.heap.snapshot")
    .mutating()
    .jsonEffect(200, (body) =>
      Effect.gen(function* () {
  ...
  http.protected
    .post("/sonderr/snapshot/remove", "sonderr.removeSnapshot")
    .mutating()
    .inProject({ git: true })
    .seeded((ctx) =>
      Effect.gen(function* () {
        const worktree = path.join(directory(ctx), ".sonderr", "worktrees", "api-snapshot-remove")
    ...
    .at((ctx) => ({
      path: `/sonderr/snapshot/remove?directory=${encodeURIComponent(directory(ctx))}`,
      headers: ctx.headers(),
      body: { worktree: ctx.state },
    }))
    .status(401),
  http.protected
    .get("/sonderr/command/files", "sonderr.commandFiles")
    ...
    .json(200, (body, ctx) => {
      ...
      check(
        item.location === path.join(directory(ctx), ".sonderr/command/httpapi-remove.md"),
  ...
  http.protected
    .post("/sonderr/command/remove", "sonderr.removeCommand")
    .inProject({ git: true, init: command })
    .mutating()
    .preserveDatabase()
    .at((ctx) => ({
      path: "/sonderr/command/remove",
      headers: ctx.headers(),
      body: { location: path.join(directory(ctx), ".sonderr/command/httpapi-remove.md") },
    }))
    .jsonEffect(200, (body, ctx) =>
      Effect.gen(function* () {
        check(body === true, "command removal should return true")
        const location = path.join(directory(ctx), ".sonderr/command/httpapi-remove.md")
        check(
  ...
  http.protected
    .post("/sonderr/skill/remove", "sonderr.removeSkill")
    .inProject({ git: true, init: skill })
    .mutating()
    .preserveDatabase()
    .at((ctx) => ({
      path: "/sonderr/skill/remove",
      headers: ctx.headers(),
      body: { location: path.join(directory(ctx), ".sonderr/skill/httpapi-remove/SKILL.md") },
    }))
    .jsonEffect(200, (body, ctx) =>
      Effect.gen(function* () {
        check(body === true, "skill removal should return true")
        const location = path.join(directory(ctx), ".sonderr/skill/httpapi-remove/SKILL.md")
        const sentinel = path.join(directory(ctx), ".sonderr/skill/httpapi-remove/KEEP.txt")
  ...
  http.protected
    .post("/sonderr/agent/remove", "sonderr.removeAgent")
    .inProject({ git: true, init: agent })
    .mutating()
    .at((ctx) => ({ path: "/sonderr/agent/remove", headers: ctx.headers(), body: { name: "httpapi-remove" } }))
    .jsonEffect(200, (body, ctx) =>
      Effect.gen(function* () {
        check(body === true, "agent removal should return true")
        const location = path.join(directory(ctx), ".sonderr/agent/httpapi-remove.md")
        check(!(yield* Effect.promise(() => Bun.file(location).exists())), "removed agent should not remain on disk")
      }),
    ),
  http.protected
    .get("/sonderr/provider-usage", "sonderr.providerUsage.get")
    .inProject({ git: true })
    .json(200, (body) => {
      object(body)
      array(body.items)
  ...
  http.protected
    .post("/sonderr/provider-usage/refresh", "sonderr.providerUsage.refresh")
    .inProject({ git: true })
    .json(200, (body) => {
      object(body)
      array(body.items)
  ...
  http.protected
    .post("/sonderr/agent/remove", "sonderr.removeAgent.duplicates")
    .inProject({ git: true, init: duplicates })
    .mutating()
    .at((ctx) => ({ path: "/sonderr/agent/remove", headers: ctx.headers(), body: { name: "httpapi-duplicate" } }))
    .jsonEffect(200, (body, ctx) =>
      Effect.gen(function* () {
        check(body === true, "duplicate agent removal should return true")
        for (const name of ["sonderr.jsonc", "sonderr.jsonc"]) {
          const cfg = yield* Effect.promise(() => Bun.file(path.join(directory(ctx), ".sonderr", name)).json())
  ...
  http.protected
    .post("/sonderr/agent/remove", "sonderr.removeAgent")
    .at((ctx) => ({ path: "/sonderr/agent/remove", headers: ctx.headers(), body: { name: "httpapi-missing" } }))
    .json(400, (body) => {
      object(body)
      check(body.message === "agent not found", "agent removal should preserve the backend error message")
    }),
  http.protected
    .post("/sonderr/session-import/project", "sonderr.sessionImport.project")
    .mutating()
    .at((ctx) => ({
      path: "/sonderr/session-import/project",
      headers: ctx.headers(),
      body: {
  ...
  http.protected
    .post("/sonderr/session-import/session", "sonderr.sessionImport.session")
    .mutating()
    .seeded((ctx) => ctx.project())
    .at((ctx) => ({
      path: "/sonderr/session-import/session",
      headers: ctx.headers(),
      body: {
  ...
  http.protected
    .post("/sonderr/session-import/message", "sonderr.sessionImport.message")
    .mutating()
    .seeded((ctx) => ctx.session({ title: "Import message" }))
    .at((ctx) => ({
      path: "/sonderr/session-import/message",
      headers: ctx.headers(),
      body: {
  ...
  http.protected
    .post("/sonderr/session-import/part", "sonderr.sessionImport.part")
    .mutating()
    .seeded((ctx) =>
      Effect.gen(function* () {
    ...
    .at((ctx) => ({
      path: "/sonderr/session-import/part",
      headers: ctx.headers(),
      body: {
