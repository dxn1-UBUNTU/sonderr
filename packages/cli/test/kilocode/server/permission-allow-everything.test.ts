...
import { AllowEverythingPermission } from "../../../src/sonderr/permission/allow-everything"
...
const auth = () => `Basic ${Buffer.from("sonderr:secret").toString("base64")}`
    ...
    await provideTestInstance({
      ...
      fn: async () => {
        ...
        const enable = await Server.Default().app.request("/permission/allow-everything", {
          method: "POST",
          headers: { "Content-Type": "application/json", "x-sonderr-directory": tmp.path, authorization: auth() },
          body: JSON.stringify({ enable: true }),
        })
