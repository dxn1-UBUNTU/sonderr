...
import { SonderrGatewayApi, SonderrGatewayPaths } from "../../../src/sonderr/server/httpapi/groups/sonderr-gateway"
import { sonderrGatewayHandlers } from "../../../src/sonderr/server/httpapi/handlers/sonderr-gateway"
...
const TestHttpApi = HttpApi.make("sonderr-instance").addHttpApi(SonderrGatewayApi)
...
const layer = HttpRouter.serve(
  HttpApiBuilder.layer(TestHttpApi).pipe(
    Layer.provide(sonderrGatewayHandlers),
...
describe("Sonderr gateway HttpApi statuses", () => {
  it.live("reports locally stored API authentication without a Gateway request", () =>
    Effect.gen(function* () {
      ...
      const response = yield* HttpClient.get(SonderrGatewayPaths.authStatus)
  ...
  it.live("preserves cloud session list rate limits", () =>
    Effect.gen(function* () {
      ...
      const response = yield* HttpClient.get(SonderrGatewayPaths.cloudSessions)
  ...
  it.live("preserves missing cloud session previews", () =>
    Effect.gen(function* () {
      ...
      const response = yield* HttpClient.get(SonderrGatewayPaths.cloudSession.replace(":id", "missing"))
  ...
  it.live("preserves cloud session preview server failures", () =>
    Effect.gen(function* () {
      ...
      const response = yield* HttpClient.get(SonderrGatewayPaths.cloudSession.replace(":id", "failed"))
  ...
  it.live("preserves cloud session import authentication failures", () =>
    Effect.gen(function* () {
      ...
      const response = yield* post(SonderrGatewayPaths.cloudSessionImport, { sessionId: "unauthorized" })
  ...
  it.live("maps cloud session import transport failures to internal errors", () =>
    Effect.gen(function* () {
      ...
      const response = yield* post(SonderrGatewayPaths.cloudSessionImport, { sessionId: "failed" })
  ...
  it.live("preserves SonderrClaw worker failures", () =>
    Effect.gen(function* () {
      yield* stub(() => new Response("worker failed", { status: 500 }))

      const response = yield* HttpClient.get(SonderrGatewayPaths.clawStatus)

      expect(response.status).toBe(500)
      expect(yield* response.json).toEqual({ error: "SonderrClaw request failed: 500 worker failed" })
    }),
  )

  it.live("normalizes numeric SonderrClaw timestamps", () =>
    Effect.gen(function* () {
  ...
  it.live("maps SonderrClaw transport failures to bad gateway", () =>
    Effect.gen(function* () {
      yield* stub(() => Promise.reject(new TypeError("network error")))
      ...
      expect(yield* response.json).toEqual({ error: "Failed to reach SonderrClaw" })
    }),
  )
})
