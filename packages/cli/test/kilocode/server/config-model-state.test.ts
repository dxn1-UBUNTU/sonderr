...
describe("config model state routes", () => {
  test("reads TUI model favorites", async () => {
    ...
    await Bun.write(
      ...
      JSON.stringify({
        recent: [{ providerID: "sonderr", modelID: "gpt-5.5" }],
        favorite: [
          { providerID: "sonderr", modelID: "gpt-5.5" },
          { providerID: "sonderr", modelID: "qwen/qwen3-8b" },
        ],
        model: {},
        variant: {},
      }),
  ...
  test("updates favorites while preserving recents", async () => {
    ...
    await Bun.write(
      path.join(tmp.path, "model.json"),
      JSON.stringify({
        recent: [{ providerID: "sonderr", modelID: "recent" }],
        favorite: [],
        model: {},
        variant: {},
      }),
    ...
    const body = await json<{ recent: unknown[]; favorite: unknown[] }>(
      await req("/config/model-state", {
        ...
        body: JSON.stringify({ favorite: [{ providerID: "sonderr", modelID: "gpt-5.5" }] }),
      }),
    )

    expect(body.recent).toEqual([{ providerID: "sonderr", modelID: "recent" }])
    expect(body.favorite).toEqual([{ providerID: "sonderr", modelID: "gpt-5.5" }])
  })
})
