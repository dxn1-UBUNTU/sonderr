...
describe("TUI config routes", () => {
  test("gets effective project TUI config", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const cfg = path.join(dir, ".sonderr")
  ...
  test("loads legacy .sonderr TUI config and ignores .sonderr", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await fs.mkdir(path.join(dir, ".sonderr"), { recursive: true })
        await fs.mkdir(path.join(dir, ".sonderr"), { recursive: true })
        await Bun.write(path.join(dir, ".sonderr", "tui.json"), JSON.stringify({ theme: "dracula" }))
        await Bun.write(path.join(dir, ".sonderr", "tui.json"), JSON.stringify({ theme: "nord" }))
      },
    })
  ...
  test("patches project TUI config", async () => {
    ...
    const saved = await Bun.file(path.join(tmp.path, ".sonderr", "tui.json")).json()
