...
import { forkTargetDirectory } from "@/sonderr/server/routes/fork-routing"
...
describe("forkTargetDirectory", () => {
  test("honors the explicit directory query on a fork request", () => {
    expect(forkTargetDirectory("POST", url("/session/ses_abc/fork?directory=/repo/.sonderr/worktrees/x"), {})).toBe(
      "/repo/.sonderr/worktrees/x",
    )
  })

  test("falls back to the x-sonderr-directory header when no query is present", () => {
    expect(
      forkTargetDirectory("POST", url("/session/ses_abc/fork"), { "x-sonderr-directory": "/repo/.sonderr/worktrees/y" }),
    ).toBe("/repo/.sonderr/worktrees/y")
  })
