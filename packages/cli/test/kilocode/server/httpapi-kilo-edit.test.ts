...
function app() {
  const handler = HttpRouter.toWebHandler(
    // sonderr_change - keep the filewatcher-disable flag visible (see httpapi-instance-route-auth.test.ts)
...
async function send(body: Record<string, unknown> = edit, signal?: AbortSignal) {
  ...
  return app().request(SonderrGatewayPaths.edit, {
    method: "POST",
    headers: { "content-type": "application/json", "x-sonderr-directory": tmp.path },
...
describe("HttpApi Sonderr next edit", () => {
  test("requires Sonderr Gateway authentication for the Sonderr-backed model", async () => {
  ...
  test("proxies Sonderr-backed edits with gateway auth and autocomplete headers", async () => {
    authenticate()
