...
import { AgentBuilderPaths } from "../../../src/sonderr/server/httpapi/groups/agent-builder"
import { BackgroundProcessPaths } from "../../../src/sonderr/server/httpapi/groups/background-process"
import { BranchNamePaths } from "../../../src/sonderr/server/httpapi/groups/branch-name"
import { ConfigConsolePaths } from "../../../src/sonderr/server/httpapi/groups/config-console"
import { IndexingPaths, SonderrEmbeddingModel } from "../../../src/sonderr/server/httpapi/groups/indexing"
import { SonderrGatewayPaths } from "../../../src/sonderr/server/httpapi/groups/sonderr-gateway"
import { SonderrPaths } from "../../../src/sonderr/server/httpapi/groups/sonderr"
import { MemoryPaths } from "../../../src/sonderr/server/httpapi/groups/memory"
import { NetworkPaths } from "../../../src/sonderr/server/httpapi/groups/network"
import { TelemetryPaths } from "../../../src/sonderr/server/httpapi/groups/telemetry"
...
describe("Sonderr PublicApi OpenAPI contract", () => {
  test("uses Sonderr branding", () => {
    const spec = OpenApi.fromApi(PublicApi)
    expect(spec.info.title).toBe("sonderr")
    expect(spec.info.description).toBe("sonderr api")
  })

  test("includes legacy Sonderr events in the generated SDK contract", () => {
  ...
  test("constrains embedding model metadata", () => {
    const accepts = (dimension: number, scoreThreshold: number) =>
      Result.isSuccess(
        EffectSchema.decodeUnknownResult(SonderrEmbeddingModel)({
          id: "provider/model",
          name: "Model",
          dimension,
          scoreThreshold,
  ...
  test("keeps directory routing queries on Sonderr Console routes", () => {
    const spec = OpenApi.fromApi(PublicApi)
    const routes = [
  ...
  test("keeps workspace routing queries on all Sonderr-owned routed endpoints", () => {
    const spec = OpenApi.fromApi(PublicApi)
    const routes = [
      ...
      { method: "get", path: SonderrPaths.providerUsage },
      { method: "post", path: SonderrPaths.providerUsageRefresh },
      { method: "get", path: SonderrPaths.sessionModelUsage },
  ...
  test("keeps personal organization resets nullable", () => {
    const spec = OpenApi.fromApi(PublicApi)
    const body = spec.paths[SonderrGatewayPaths.organization]?.post?.requestBody as Body | undefined
  ...
  test("keeps Sonderr gateway responses nullable", () => {
    const spec = OpenApi.fromApi(PublicApi)
    ...
    const profile = response(SonderrGatewayPaths.profile)?.properties
    expect(profile?.balance).toEqual({ anyOf: [expect.objectContaining({ type: "object" }), { type: "null" }] })
    expect(profile?.sonderrPass).toEqual({ anyOf: [expect.objectContaining({ type: "object" }), { type: "null" }] })
    ...
    const pass = profile?.sonderrPass?.anyOf?.find((item) => item.type === "object")?.properties
    ...
    const auth = response(SonderrGatewayPaths.authStatus)?.properties
    expect(auth).toEqual({
      authenticated: { type: "boolean" },
      type: { type: "string", enum: ["api", "oauth"] },
    })

    const sessions = response(SonderrGatewayPaths.cloudSessions)?.properties
    ...
    const claw = response(SonderrGatewayPaths.clawStatus)?.properties
    ...
    expect(response(SonderrGatewayPaths.clawChatCredentials)).toEqual({
  ...
  test("keeps transcription prompts in the public contract", () => {
    const spec = OpenApi.fromApi(PublicApi)
    const body = spec.paths[SonderrGatewayPaths.audioTranscriptions]?.post?.requestBody as Body | undefined
    const schema = body?.content?.["application/json"]?.schema
  ...
  test("keeps provider usage flat and credential-free", () => {
    ...
    expect(spec.paths[SonderrPaths.providerUsage]?.get?.responses?.["200"]).toBeDefined()
    expect(spec.paths[SonderrPaths.providerUsageRefresh]?.post?.responses?.["200"]).toBeDefined()
  ...
  test("documents the transcription model catalog route", () => {
    const spec = OpenApi.fromApi(PublicApi)
    const route = spec.paths[SonderrGatewayPaths.transcriptionModels]?.get
