...
import { assertWrite, run as runSandbox } from "@sonderr/sandbox"
import { Effect, Exit } from "effect"
import { profile } from "@/sonderr/sandbox/policy"
import { SandboxPreference } from "@/sonderr/sandbox/preference"
...
function expected(...directories: string[]) {
  return new Set([...directories, ...sonderr])
}
...
describe("sandbox policy", () => {
  ...
  test("keeps Sonderr state writable without exposing sandbox policy state", async () => {
    await using tmp = await fixture()
    const dirs = tmp.extra
