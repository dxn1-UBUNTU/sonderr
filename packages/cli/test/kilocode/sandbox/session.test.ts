...
import * as SandboxActivation from "@/sonderr/sandbox/activation"
...
const it = testEffect(
  Layer.mergeAll(
    Session.layer.pipe(
      ...
      Layer.provide(AppNodeBuilder.build(SessionV2.node, [[SessionExecution.node, SessionExecution.noopLayer]])), // sonderr_change
    ),
...
describe("sandbox session cleanup", () => {
  ...
  it.live("creates honor the sonderr.sandbox metadata over the config default", () =>
    Effect.gen(function* () {
      const sessions = yield* Session.Service
      ...
      const session = yield* provideInstance(dir)(
        sessions.create({ title: "sandbox-explicit", metadata: { "sonderr.sandbox": { enabled: true, version: 0 } } }),
      )
