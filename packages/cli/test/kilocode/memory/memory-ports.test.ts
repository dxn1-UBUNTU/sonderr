...
import { MemoryModel, MemorySession } from "../../../src/sonderr/memory/ports"
...
describe("memory ports", () => {
  test("session port extracts the latest turn, recall markers, and all assistant steps", async () => {
    ...
    const diffs = [
      {
        file: "packages/cli/src/sonderr/memory/ports.ts",
        additions: 4,
        deletions: 1,
    ...
    const messages = [
      ...
      assistant({
        ...
        parentID: uid,
        time: 6,
        parts: [text(sessionID, final, "Run bun test from packages/cli for CLI memory tests.")],
      }),
    ]
    ...
    expect(view).toMatchObject({
      ...
      assistant: "Run bun test from packages/cli for CLI memory tests.",
      lastAssistantID: final,
      sessionModel: ref,
      recalledMemory: true,
      diffs,
    })
    expect(view?.recent).toContain("Tool sonderr_memory_recall completed")
