...
import { PermissionProvenance } from "../../src/sonderr/permission/provenance"
...
function planAgentPermission() {
  const planEdit = [
    ...
    { permission: "edit", pattern: ".sonderr/plans/*.md", action: "allow" as const },
...
describe("SonderrSessionPrompt.dedupeRuleset", () => {
  test("keeps the last occurrence of each distinct rule", () => {
    ...
    const deduped = SonderrSessionPrompt.dedupeRuleset(rules)
  ...
  test("keeps rules that differ only by provenance tag", () => {
    ...
    const deduped = SonderrSessionPrompt.dedupeRuleset([...tagged, ...session])
...
describe("SonderrSessionPrompt.buildAskRuleset", () => {
  test("plan mode ruleset has no stacked duplicate rule blocks", () => {
    ...
    expect(editRules as PermissionProvenance.SourcedRule[]).toEqual([
      ...
      { permission: "edit", pattern: ".sonderr/plans/*.md", action: "allow", source: "agent" },
  ...
  test("hard ruleset is deduped too", () => {
    const agent = { name: "plan", permission: planAgentPermission() }
    const { hardRuleset } = SonderrSessionPrompt.buildAskRuleset({
      agent,
      session: { permission: [] },
    })
    ...
    expect(editRules).toEqual([
      { permission: "edit", pattern: "*", action: "deny" },
      { permission: "edit", pattern: ".sonderr/plans/*.md", action: "allow" },
      { permission: "edit", pattern: "plans/*.md", action: "allow" },
    ])
  })
  ...
  test("code mode is left untouched", () => {
    ...
    const { ruleset, hardRuleset } = SonderrSessionPrompt.buildAskRuleset({
      agent,
      session: { permission: sessionPermission },
    })
