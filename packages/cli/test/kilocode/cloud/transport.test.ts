...
import {
  ...
  MessageIdSchema,
} from "../../../src/sonderr/cloud/contracts"
import { parseServiceOrigin } from "../../../src/sonderr/cloud/origin"
import { MAX_CLOUD_AGENT_RESPONSE_BYTES } from "../../../src/sonderr/cloud/response-json"
import { createCloudAgentClient } from "../../../src/sonderr/cloud/trpc"
  ...
  test("places bearer auth only in headers and correlates generated message identities", async () => {
    ...
    try {
      ...
      const start = await agent.start({
        ...
        options: { createdOnPlatform: "sonderr-cli" },
      })
