...
import { CloudRepository } from "../../../src/sonderr/cloud/repository"
...
describe("CloudRepository", () => {
  it.live("resolves and validates an explicit repository branch outside Git", () =>
    Effect.gen(function* () {
      ...
      const error = yield* CloudRepository.resolve({ cwd, repo: "sonderr-org/sonderr", branch: "feature.lock" }).pipe(
        Effect.flip,
      )
      ...
      const type = yield* CloudRepository.resolve({
        cwd,
        repo: "https://github.com/sonderr-org/sonderr.git",
        type: "gitlab",
      }).pipe(Effect.flip)
  ...
  it.live("rejects GitHub repositories that become dot segments after trimming .git", () =>
    Effect.gen(function* () {
      ...
      for (const repo of ["https://github.com/sonderr-org/...git", "git@github.com:sonderr-org/...git"]) {
  ...
  it.live("normalizes an inferred GitHub SCP remote without adding the current branch", () =>
    Effect.gen(function* () {
      ...
      yield* run(cwd, "remote", "add", "origin", "git@github.com:sonderr-org/ssh-repo.git")
      yield* run(cwd, "checkout", "-b", "feature/not-in-output")

      const result = yield* CloudRepository.resolve({ cwd })

      expect(result).toEqual({ type: "github", repo: "sonderr-org/ssh-repo" })
    }),
  )
  ...
  it.live("rejects inferred local remotes that resemble GitHub shorthand", () =>
    Effect.gen(function* () {
      const cwd = yield* tmpdirScoped({ git: true })
      yield* run(cwd, "remote", "add", "origin", "sonderr-org/local-repo")
  ...
  it.live("prefers the tracking remote, then origin, then a sole remote fetch URL", () =>
    Effect.gen(function* () {
      ...
      yield* run(cwd, "remote", "add", "origin", "https://github.com/sonderr-org/origin.git")
      yield* run(cwd, "remote", "add", "tracked", "https://github.com/sonderr-org/tracked.git")
      yield* run(cwd, "remote", "set-url", "--add", "--push", "tracked", "https://github.com/sonderr-org/push.git")
      yield* run(cwd, "config", `branch.${branch}.remote`, "tracked")

      expect(yield* CloudRepository.resolve({ cwd })).toEqual({ type: "github", repo: "sonderr-org/tracked" })

      yield* run(cwd, "config", `branch.${branch}.remote`, "missing")
      expect(yield* CloudRepository.resolve({ cwd })).toEqual({ type: "github", repo: "sonderr-org/origin" })
  ...
  it.live("returns typed errors when no remote can be selected", () =>
    Effect.gen(function* () {
      ...
      yield* run(cwd, "remote", "add", "alpha", "https://github.com/sonderr-org/alpha.git")
      yield* run(cwd, "remote", "add", "beta", "https://github.com/sonderr-org/beta.git")
