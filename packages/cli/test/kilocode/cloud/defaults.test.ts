...
import { CloudAuth } from "@/sonderr/cloud/auth"
import { CloudCatalog } from "@/sonderr/cloud/catalog"
import { CloudDefaults } from "@/sonderr/cloud/defaults"
import { MAX_CLOUD_AGENT_RESPONSE_BYTES } from "@/sonderr/cloud/response-json"
      ...
      Bun.serve({
        ...
        fetch(request) {
          ...
          requests.push({
            ...
            feature: request.headers.get("x-sonderr-feature"),
            organization: request.headers.get("x-sonderr-organizationid"),
            path: url.pathname,
          })
...
it.instance(
    ...
    withCatalog(["anthropic/explicit", "anthropic/default"], "anthropic/default", (url, requests) =>
      Effect.gen(function* () {
        ...
        const auth = Layer.mock(Auth.Service)({
          get: (id) => (id === "sonderr" ? Ref.get(current) : Effect.succeed(undefined)),
          set: (id, info) => (id === "sonderr" ? Ref.set(current, info) : Effect.void),
        })

        const resolved = yield* Effect.gen(function* () {
          const result = yield* CloudDefaults.resolve({
            mode: "debug",
            model: "sonderr/anthropic/explicit",
            orgID: explicitID,
            env: {
              SONDERR_API_KEY: "ignored-env-token",
              SONDERR_ORG_ID: envID,
            },
          })
          const service = yield* Auth.Service
          expect(yield* service.get("sonderr")).toEqual(stored)
          return result
        }).pipe(
          Effect.provide(
            Layer.mergeAll(
              ...
              stateLayer(
                state({
                  model: { debug: { providerID: "sonderr", modelID: "anthropic/saved" } },
                }),
              ),
              CloudCatalog.layer({ env: { SONDERR_API_URL: url.origin } }),
            ),
          ),
        )
    ...
    ),
  {
    config: {
      default_agent: "plan",
      model: "sonderr/anthropic/repository",
      agent: { plan: { model: "sonderr/anthropic/mode" } },
    },
  },
)
        ...
        Effect.gen(function* () {
          ...
          Effect.provide(
            Layer.mergeAll(
              ...
              stateLayer(
                state({
                  model: { code: { providerID: "sonderr", modelID: "anthropic/stale" } },
                  recent: [{ providerID: "sonderr", modelID: "anthropic/recent" }],
                }),
              ),
              CloudCatalog.layer({ env: { SONDERR_API_URL: url.origin } }),
            ),
      ...
      CloudDefaults.resolve().pipe(
        ...
        Effect.provide(
          Layer.mergeAll(
            ...
            stateLayer(
              state({
                model: { code: { providerID: "sonderr", modelID: "anthropic/saved" } },
              }),
            ),
            CloudCatalog.layer({ env: { SONDERR_API_URL: url.origin } }),
          ),
...
it.instance(
    ...
    ),
  {
    config: {
      ...
      agent: {
        code: { model: "sonderr/anthropic/code" },
        custom: { mode: "primary", model: "sonderr/anthropic/custom" },
      },
    },
  },
)
...
it.instance("rejects invalid persisted organization state and insecure catalog origins", () =>
  Effect.gen(function* () {
    ...
    const catalog = yield* CloudDefaults.resolve({ env: { SONDERR_API_URL: "http://example.com" } }).pipe(
      Effect.provide(
        Layer.mergeAll(
          authLayer(new Auth.Api({ type: "api", key: "stored-api-token" })),
          stateLayer(state()),
          CloudCatalog.layer({
            env: { SONDERR_API_URL: "http://example.com" },
    ...
    withCatalog(
      ...
      (url, requests) =>
        Effect.gen(function* () {
          ...
          const resolved = yield* CloudDefaults.resolve({
            env: { SONDERR_API_KEY: "ignored-env-token" },
          }).pipe(
            Effect.provide(
              Layer.mergeAll(
                ...
                stateLayer(
                  state({
                    model: { plan: { providerID: "sonderr", modelID: "anthropic/saved" } },
                    recent: [{ providerID: "sonderr", modelID: "anthropic/recent" }],
                  }),
                ),
                CloudCatalog.layer({ env: { SONDERR_API_URL: url.origin } }),
