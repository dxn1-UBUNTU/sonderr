...
test("extracts ZIP paths with PowerShell metacharacters literally", async () => {
  ...
  const cmd = "Compress-Archive -LiteralPath $env:SONDERR_TEST_SOURCE -DestinationPath $env:SONDERR_TEST_ARCHIVE -Force"

  await fs.writeFile(source, "safe")
  await Process.run(["powershell", "-NoProfile", "-NonInteractive", "-Command", cmd], {
    env: {
      SONDERR_TEST_SOURCE: source,
      SONDERR_TEST_ARCHIVE: archive,
    },
  })
