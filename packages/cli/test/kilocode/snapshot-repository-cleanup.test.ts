...
import { SonderrSnapshotCleanup } from "../../src/sonderr/snapshot/cleanup"
...
const item = (base: string, project = "project", name = "worktree", directory = path.join(base, "project")) => ({
  ...
  project,
  directory,
  worktree: path.join(directory, ".sonderr", "worktrees", name),
})
...
const repo = (input: ReturnType<typeof item>) =>
  Effect.gen(function* () {
    ...
    yield* git(["config", "user.email", "test@sonderr.test"], { cwd: input.directory, env: { GIT_DIR: dir } })
...
const remove = (input: ReturnType<typeof item>) =>
  Effect.gen(function* () {
    ...
    return yield* SonderrSnapshotCleanup.remove({ ...input, fs, flock })
  })
...
it.live("rejects paths outside the managed worktrees directory", () =>
  Effect.gen(function* () {
    ...
    const outside = path.join(input.directory, ".sonderr", "worktrees-evil", "outside")
...
for (const stored of [false, true]) {
  it.live(`cleans ${stored ? "existing" : "absent"} checkpoints without accessing workspace ancestors`, () =>
    Effect.gen(function* () {
      ...
      expect(
        yield* SonderrSnapshotCleanup.remove({
          ...input,
          flock,
          fs: {
            ...fs,
...
for (const name of [".sonderr", "worktrees"]) {
  it.live(`rejects a symlinked managed ${name} directory`, () =>
    Effect.gen(function* () {
      ...
      if (name === ".sonderr") {
        ...
        yield* write(path.join(input.directory, ".sonderr", "placeholder"), "")
        yield* link(outside, path.join(input.directory, ".sonderr", name))
      }
