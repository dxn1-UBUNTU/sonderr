...
import { EventSequenceTable } from "@sonderr/core/event/sql"
...
import { WorkspaceTable } from "@sonderr/core/control-plane/workspace.sql"
...
const originalEnv = {
  SONDERR_AUTH_CONTENT: process.env.SONDERR_AUTH_CONTENT,
  SONDERR_EXPERIMENTAL_WORKSPACES: process.env.SONDERR_EXPERIMENTAL_WORKSPACES,
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@sonderr.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.SONDERR_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=sonderr-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.SONDERR_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.SONDERR_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.SONDERR_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=sonderr-test")
