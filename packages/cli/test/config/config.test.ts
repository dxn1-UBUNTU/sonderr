...
import { ConfigAgentV1 } from "@sonderr/core/v1/config/agent" // sonderr_change
...
import { Git } from "../../src/git" // sonderr_change
import {
...
import { ProjectV2 } from "@sonderr/core/project"
...
import { isIndexingPlugin } from "@sonderr/sonderr-indexing/detect" // sonderr_change
import { isAtomicChatPlugin } from "@/sonderr/atomic-chat-feature" // sonderr_change
...
function remoteConfigClient(input: {
  ...
  return HttpClient.make((request) => {
    if (request.url.includes(".well-known/sonderr")) {
...
const schemaConfig = (config: object) => ({ $schema: "https://app.kilo.ai/config.json", ...config }) // sonderr_change
...
const managedConfigDir = process.env.SONDERR_TEST_MANAGED_CONFIG_DIR!
const originalTestToken = process.env.TEST_TOKEN
const originalConsoleToken = process.env.SONDERR_CONSOLE_TOKEN
...
afterEach(async () => {
  ...
  if (originalConsoleToken === undefined) delete process.env.SONDERR_CONSOLE_TOKEN
  else process.env.SONDERR_CONSOLE_TOKEN = originalConsoleToken
  await clear(true)
})

const writeManagedSettingsEffect = (settings: object, filename?: string) =>
  FSUtil.use.writeWithDirs(path.join(managedConfigDir, filename ?? "sonderr.json"), JSON.stringify(settings)) // sonderr_change

// sonderr_change start
async function writeConfig(dir: string, config: object, name = "sonderr.json") {
  // sonderr_change end
...
const writeConfigEffect = (
  dir: string,
  config: object,
  name = "sonderr.json", // sonderr_change
  ...
  Effect.gen(function* () {
    ...
    yield* Effect.all(
      [
        ...
        input.local ? writeConfigEffect(path.join(directory, ".sonderr"), schemaConfig(input.local)) : undefined, // sonderr_change
...
async function check(map: (dir: string) => string) {
  ...
  try {
    await writeConfig(globalTmp.path, {
      $schema: "https://sonderr.ai/config.json",
      snapshot: false,
    })
...
it.effect("creates global jsonc config with schema when no global configs exist", () =>
  withGlobalConfig({}, ({ dir }) =>
    Effect.gen(function* () {
      ...
      const content = yield* FSUtil.use.readFileString(path.join(dir, "sonderr.jsonc")) // sonderr_change
      expect(content).toContain('"$schema": "https://app.kilo.ai/config.json"') // sonderr_change
    }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
  ),
)

it.effect("does not create global config when SONDERR_CONFIG_DIR is set", () =>
  Effect.gen(function* () {
    ...
    yield* withGlobalConfig({}, ({ dir }) =>
      withProcessEnv(
        ...
        Effect.gen(function* () {
          ...
          expect(yield* FSUtil.use.existsSafe(path.join(dir, "sonderr.jsonc"))).toBe(false)
        }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
      ),
    )
  }),
...
// sonderr_change start
it.instance("preserves Sonderr provider free model metadata", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* writeConfigEffect(test.directory, {
      $schema: "https://app.kilo.ai/config.json",
      model: "sonderr/free-e2e",
      provider: {
        sonderr: {
          models: {
            "free-e2e": {
    ...
    const config = yield* Config.use.get()
    const model = config.provider?.sonderr?.models?.["free-e2e"]
...
it.instance("updates config and preserves empty shell sentinel", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    // sonderr_change - upstream hardcodes project config to config.json; Sonderr writes to sonderr.json
    yield* writeConfigEffect(test.directory, { $schema: "https://sonderr.ai/config.json", shell: "bash" })

    yield* Config.Service.use((svc) => svc.update(ConfigParse.schema(ConfigV1.Info, { shell: "" }, "test:config")))

    const writtenConfig = yield* FSUtil.use.readJson(path.join(test.directory, "sonderr.json")) // sonderr_change
...
it.effect("updates global config and omits empty shell key in json", () =>
  withGlobalConfig({ config: { shell: "bash" } }, ({ dir }) =>
    Effect.gen(function* () {
      ...
      const writtenConfig = yield* FSUtil.use.readJson(path.join(dir, "sonderr.json")) // sonderr_change
...
it.effect("updates global config and omits empty shell key in jsonc", () =>
  withGlobalConfig({ config: { shell: "bash", model: "test/model" }, name: "sonderr.jsonc" }, ({ dir }) =>
    Effect.gen(function* () {
      yield* Config.use.updateGlobal({ shell: "" })
...
it.instance("ignores legacy tui keys in sonderr config", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* writeConfigEffect(test.directory, {
      $schema: "https://sonderr.ai/config.json",
...
it.instance("loads JSONC config file", () =>
  Effect.gen(function* () {
    ...
    yield* FSUtil.use.writeWithDirs(
      // sonderr_change start
      path.join(test.directory, "sonderr.jsonc"),
      `{
        // This is a comment
        "$schema": "https://app.kilo.ai/config.json",
        "model": "test/model",
...
it.instance("jsonc overrides json in the same directory", () =>
  Effect.gen(function* () {
    ...
    yield* writeConfigEffect(
      test.directory,
      {
        $schema: "https://app.kilo.ai/config.json", // sonderr_change
        model: "base",
        username: "base",
      },
      "sonderr.jsonc", // sonderr_change
    )
    yield* writeConfigEffect(test.directory, {
      $schema: "https://app.kilo.ai/config.json", // sonderr_change
      model: "override",
...
// sonderr_change start
it.instance("prefers .sonderr directory config over legacy .sonderr", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* writeConfigEffect(path.join(test.directory, ".sonderr"), {
      $schema: "https://app.kilo.ai/config.json",
      model: "legacy/model",
    })
    yield* writeConfigEffect(path.join(test.directory, ".sonderr"), {
...
// sonderr_change start - project config is untrusted: {env:} rejected; {file:} confined to the project root
...
it.instance("injects $schema into config without existing schema", () =>
  Effect.gen(function* () {
    ...
    yield* FSUtil.use.writeWithDirs(path.join(test.directory, "sonderr.json"), JSON.stringify({ username: "test-user" }))
    const config = yield* Config.use.get()
    expect(config.username).toBe("test-user")
    expect(config.$schema).toBe("https://app.kilo.ai/config.json")

    // Read the file to verify $schema was injected
    const content = yield* FSUtil.use.readFileString(path.join(test.directory, "sonderr.json"))
...
it.instance("injects $schema into comment-first JSONC config", () =>
  Effect.gen(function* () {
    ...
    const content = yield* FSUtil.use.readFileString(path.join(test.directory, "sonderr.jsonc"))
    expect(content).toContain('"$schema": "https://app.kilo.ai/config.json"')
...
it.instance("does not write config when $schema already present", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    const filepath = path.join(test.directory, "sonderr.json")
...
const accountTokenIt = configIt({
  account: Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { sonderr: { options: { apiKey: "{env:SONDERR_CONSOLE_TOKEN}" } } },
        }),
      ),
...
accountTokenIt.instance("resolves env templates in account config with account token", () =>
  Effect.gen(function* () {
    const config = yield* Config.use.get()
    expect(config.provider?.["sonderr"]?.options?.apiKey).toBe("st_test_token")
  }),
)
...
it.instance("reports warning for invalid JSON", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(path.join(test.directory, "sonderr.json"), "{ invalid json }")
    yield* Config.use.get()
    const issues = yield* Config.Service.use((svc) => svc.warnings())
    expect(issues.length).toBeGreaterThan(0)
  }),
...
// sonderr_change start
it.instance("loads config from .sonderr directory", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".sonderr", "agent", "test.md"), // sonderr_change
      `---
model: test/model
---
Test agent prompt`,
...
it.instance("agent markdown permission config preserves user key order", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".sonderr", "agent", "ordered.md"), // sonderr_change
      `---
permission:
  bash: allow
  "*": deny
...
// sonderr_change start
it.instance("loads agents from .sonderr/agents (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".sonderr", "agents", "helper.md"), // sonderr_change
      `---
model: test/model
mode: subagent
---
Helper agent prompt`,
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".sonderr", "agents", "nested", "child.md"), // sonderr_change
      `---
model: test/model
mode: subagent
---
...
// sonderr_change start
it.instance("loads commands from .sonderr/command (singular)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".sonderr", "command", "hello.md"), // sonderr_change
      `---
description: Test command
---
Hello from singular command`,
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".sonderr", "command", "nested", "child.md"), // sonderr_change
      `---
...
// sonderr_change start
it.instance("loads commands from .sonderr/commands (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".sonderr", "commands", "hello.md"), // sonderr_change
      `---
description: Test command
---
Hello from plural commands`,
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".sonderr", "commands", "nested", "child.md"), // sonderr_change
      `---
description: Nested command
---
Nested command template`,
...
// sonderr_change start
it.instance("prefers .sonderr commands over legacy .sonderr commands", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".sonderr", "command", "hello.md"),
      `---
...
it.instance("updates config and writes to file", () =>
  Effect.gen(function* () {
    ...
    const writtenConfig = yield* FSUtil.use.readJson(
      path.join(test.directory, ".sonderr", "sonderr.jsonc"), // sonderr_change
    )
...
it.effect("does not try to install dependencies in read-only SONDERR_CONFIG_DIR", () =>
  Effect.gen(function* () {
    if (process.platform === "win32") return
    ...
    yield* withProcessEnv("SONDERR_CONFIG_DIR", readonly, Config.use.get().pipe(provideInstanceEffect(dir)))
  }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
)

it.effect("ignores an inaccessible SONDERR_CONFIG_DIR", () =>
  Effect.gen(function* () {
    if (process.platform === "win32") return
    ...
    yield* withProcessEnv("SONDERR_CONFIG_DIR", configDir, Config.use.get().pipe(provideInstanceEffect(dir)))
  }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
)

it.effect("creates a missing SONDERR_CONFIG_DIR", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
...
it.effect("installs dependencies in writable SONDERR_CONFIG_DIR", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
    const configDir = path.join(dir, "configdir")
    yield* FSUtil.use.ensureDir(configDir)
...
it.effect("global config remains global when project config is disabled", () =>
  withConfigTree(
    ...
    withProcessEnv(
      "SONDERR_DISABLE_PROJECT_CONFIG",
      "true",
      Effect.gen(function* () {
        const config = yield* Config.use.get()
...
it.instance("does not error when only custom agent is a subagent", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".sonderr", "agent", "helper.md"), // sonderr_change
      `---
model: test/model
mode: subagent
---
...
it.effect("keeps plugin origins aligned with merged plugin list", () =>
  withConfigTree(
    ...
    Effect.gen(function* () {
      ...
      // sonderr_change start - bundled plugins intentionally have no external plugin origins
...
// sonderr_change - Note: preload.ts sets SONDERR_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
it.instance("managed jsonc settings override managed json settings", () =>
  Effect.gen(function* () {
    ...
    yield* writeManagedSettingsEffect({ model: "managed/jsonc" }, "sonderr.jsonc")
...
it.instance("permission config preserves user key order", () =>
  ...
  Effect.gen(function* () {
    const test = yield* TestInstance
    // sonderr_change start — isolate from global config to prevent cross-test contamination
...
// sonderr_change start - preserve legacy agent requirement declarations without forwarding them
...
// sonderr_change start - regression for `env` alias on local MCP entries
...
it.instance("project config can override MCP server enabled status", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    // sonderr_change - base config in .json, override in .jsonc (jsonc loads second and wins)
...
it.instance("MCP config deep merges preserving base config properties", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    // sonderr_change - base config in .json, override in .jsonc (jsonc loads second and wins)
    // sonderr_change - Base config with full MCP definition
    yield* writeConfigEffect(test.directory, {
      $schema: "https://app.kilo.ai/config.json", // sonderr_change
      mcp: {
        myserver: {
    ...
    // sonderr_change - Override just enables it, should preserve other properties
    yield* writeConfigEffect(
      test.directory,
      {
        $schema: "https://app.kilo.ai/config.json", // sonderr_change
...
// sonderr_change start
it.instance("local .sonderr config can override MCP from project config", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* writeConfigEffect(test.directory, {
      $schema: "https://app.kilo.ai/config.json", // sonderr_change
    ...
    yield* writeConfigEffect(
      path.join(test.directory, ".sonderr"), // sonderr_change
      {
        $schema: "https://app.kilo.ai/config.json", // sonderr_change
        mcp: {
          docs: {
...
remoteProjectOverride.it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const config = yield* Config.use.get()
      expect(remoteProjectOverride.seen.wellKnown).toBe("https://example.com/.well-known/sonderr")
...
trailingSlashWellKnown.it.instance("wellknown URL with trailing slash is normalized", () =>
  Effect.gen(function* () {
    yield* Config.use.get()
    expect(trailingSlashWellKnown.seen.wellKnown).toBe("https://example.com/.well-known/sonderr")
  }),
)
      ...
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            ...
            expect(fetchedUrl).toBe(`${server.url.origin}/.well-known/sonderr`)
            expect(config.mcp?.jira?.enabled).toBe(true)
          }),
        ),
      { git: true },
...
const templatedHeaderWellKnown = wellKnown({
  remoteConfig: {
    url: "https://config.example.com/sonderr.json",
...
templatedHeaderWellKnown.it.instance("wellknown remote_config supports templated env vars in headers", () =>
  Effect.gen(function* () {
    const config = yield* Config.use.get()
    expect(templatedHeaderWellKnown.seen.wellKnown).toBe("https://example.com/.well-known/sonderr")
    expect(templatedHeaderWellKnown.seen.remote).toBe("https://config.example.com/sonderr.json")
...
const remotePrecedenceWellKnown = wellKnown({
  ...
  remoteConfig: { url: "https://config.example.com/{env:TEST_TOKEN}/sonderr.json" },
  remote: {
...
remotePrecedenceWellKnown.it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const config = yield* Config.use.get()
      expect(remotePrecedenceWellKnown.seen.remote).toBe("https://config.example.com/test-token/sonderr.json")
      expect(config.mcp?.confluence?.enabled).toBe(true)
    }),
)
...
const nullConfigWellKnown = wellKnown({
  wellKnown: {
    config: null,
    remote_config: { url: "https://config.example.com/sonderr.json" },
  },
  remote: {
    mcp: { confluence: { type: "remote", url: "https://confluence.example.com/mcp", enabled: true } },
  },
...
nullConfigWellKnown.it.instance("wellknown config null is treated as absent", () =>
  Effect.gen(function* () {
    const config = yield* Config.use.get()
    expect(nullConfigWellKnown.seen.remote).toBe("https://config.example.com/sonderr.json")
    expect(config.mcp?.confluence?.enabled).toBe(true)
  }),
)

const invalidRemoteWellKnown = wellKnown({
  remoteConfig: { url: "https://config.example.com/sonderr.json" },
...
invalidRemoteWellKnown.it.instance("wellknown remote_config rejects non-object config responses", () =>
  Effect.gen(function* () {
    ...
    expect(invalidRemoteWellKnown.seen.remote).toBe("https://config.example.com/sonderr.json")
...
loginPageWellKnown.it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const exit = yield* Config.use.get().pipe(Effect.exit)
      expect(loginPageWellKnown.seen.remote).toBe("https://config.example.com/sonderr.json")
      expect(Exit.isFailure(exit)).toBe(true)
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    ...
    const file = path.join(tmp.path, "sonderr.json") // sonderr_change
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-sonderr@2.4.3", file)).toBe("oh-my-sonderr@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-sonderr@2.4.3", "file:///project/.sonderr/plugin/oh-my-sonderr.js"] // sonderr_change
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.sonderr/plugin/demo.ts", "file:///project/.sonderr/plugin/demo.ts"] // sonderr_change

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.sonderr/plugin/demo.ts"]) // sonderr_change
  })
  ...
  it.effect("loads auto-discovered local plugins as file urls", () =>
    withConfigTree(
      ...
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* FSUtil.use.writeWithDirs(
          path.join(test.directory, ".sonderr", "plugin", "my-plugin.js"), // sonderr_change
          "export default {}",
        )
...
describe("SONDERR_DISABLE_PROJECT_CONFIG", () => {
  // sonderr_change start
  ...
  it.instance("skips project .sonderr directory when flag is set", () =>
    withProcessEnv(
      "SONDERR_DISABLE_PROJECT_CONFIG",
      "true",
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* FSUtil.use.writeWithDirs(
          path.join(test.directory, ".sonderr", "command", "test-cmd.md"),
  ...
  it.instance(
    ...
    () =>
      withProcessEnvs(
        { SONDERR_CONFIG_DIR: undefined, SONDERR_DISABLE_PROJECT_CONFIG: "true" },
        Effect.gen(function* () {
          const test = yield* TestInstance
  ...
  it.instance(
    "SONDERR_CONFIG_DIR still works when flag is set",
    () =>
      Effect.gen(function* () {
        ...
        yield* withProcessEnvs(
          { SONDERR_DISABLE_PROJECT_CONFIG: "true", SONDERR_CONFIG_DIR: configDir },
          Effect.gen(function* () {
            const config = yield* Config.use.get()
...
// Regression for #28206: malformed SONDERR_PERMISSION JSON used to crash
...
describe("SONDERR_PERMISSION env var", () => {
  it.instance("does not crash when SONDERR_PERMISSION contains invalid JSON", () =>
    withProcessEnv(
      "SONDERR_PERMISSION",
      "{invalid",
      Effect.gen(function* () {
...
describe("SONDERR_CONFIG_CONTENT token substitution", () => {
  it.instance("substitutes {env:} tokens in SONDERR_CONFIG_CONTENT", () =>
    withProcessEnv(
      "TEST_CONFIG_VAR",
      "test_api_key_12345",
      withProcessEnv(
  ...
  it.instance("substitutes {file:} tokens in SONDERR_CONFIG_CONTENT", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
  ...
  const config = ConfigParse.schema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          PayloadDisplayName: "Sonderr Managed",
          PayloadIdentifier: "ai.sonderr.managed.test",
          PayloadType: "ai.sonderr.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.schema(
    ConfigV1.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://sonderr.ai/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://sonderr.ai/config.json")
})
