...
// sonderr_change start - describe Sonderr's branded CLI
// terminal sizes. The default sonderr TUI command is excluded —
// `sonderr --help` includes an ASCII banner that pulls in the install
// sonderr_change end
...
function normalize(text: string): string {
  // sonderr_change start - snapshot Sonderr help independently of lifecycle logs
  const help = text.slice(text.indexOf("sonderr "))
  const output = help
    .replace(/(?=INFO  \d{4}-\d{2}-\d{2}).*$/s, "")
    .replace(/ {4}(?=\[aliases: ls\])/g, "")
    .replace(/ {4}(?=\[string\] \[default: "sonderr\.local"\])/g, "")
  // sonderr_change end
  // sonderr_change start - normalize the branded help output
...
// sonderr_change start - describe Sonderr's command list
// Top-level commands. Order matches what `sonderr --help` prints today;
...
// top-level help on `--help` and exits 1; not a real sonderr command.
// sonderr_change end
const TOP_LEVEL = [
  "acp",
  "mcp",
...
// sonderr_change start - name snapshots after the shipped CLI
describe("Sonderr CLI help-text snapshots", () => {
  // sonderr_change end
  ...
  cliIt.live(
    ...
    ({ sonderr }) =>
      Effect.gen(function* () {
        const topLevel = yield* sonderr.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* sonderr.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`sonderr ${argv.join(" ")}: exit ${result.exitCode}`) // sonderr_change
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`sonderr ${argv.join(" ")} --help`) // sonderr_change
        }
