...
`,
  ...
  const pure = process.env.SONDERR_PURE
  const meta = process.env.SONDERR_PLUGIN_META_FILE
  process.env.SONDERR_PURE = "1"
  process.env.SONDERR_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.SONDERR_PURE
    } else {
      process.env.SONDERR_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.SONDERR_PLUGIN_META_FILE
    } else {
      process.env.SONDERR_PLUGIN_META_FILE = meta
    }
  }
})
