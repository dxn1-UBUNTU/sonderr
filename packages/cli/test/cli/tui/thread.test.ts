...
describe("tui thread", () => {
  ...
  // sonderr_change start - preserve the sanitized Sonderr worker environment
  ...
  test("ignores stale PWD when resolving a relative mini project", async () => { // sonderr_change
    await using pwd = await tmpdir({ git: true })
    await using cwd = await tmpdir({ git: true })

    expect(resolveThreadDirectory(".", pwd.path, cwd.path)).toBe(cwd.path) // sonderr_change
  ...
  cliIt.live("rejects mini-only options without --mini", ({ sonderr }) =>
    Effect.gen(function* () {
      const result = yield* sonderr.spawn(["--replay-limit", "10"])
  ...
  cliIt.live("routes attached sessions to mini mode", ({ sonderr }) =>
    Effect.gen(function* () {
      const result = yield* sonderr.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ sonderr }) =>
    Effect.gen(function* () {
      const result = yield* sonderr.spawn(["--mini", "--port", "4096"])
