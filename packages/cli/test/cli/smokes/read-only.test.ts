...
describe("sonderr read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // SONDERR_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ sonderr }) =>
      Effect.gen(function* () {
        const r = yield* sonderr.spawn(["mcp", "list"])
        sonderr.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ sonderr }) =>
      Effect.gen(function* () {
        const r = yield* sonderr.spawn(["providers", "list"])
        sonderr.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ sonderr }) =>
      Effect.gen(function* () {
        const r = yield* sonderr.spawn(["models"])
        sonderr.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ sonderr }) =>
      Effect.gen(function* () {
        const r = yield* sonderr.spawn(["agent", "list"])
        sonderr.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh SONDERR_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ sonderr }) =>
      Effect.gen(function* () {
        const r = yield* sonderr.spawn(["session", "list"])
        sonderr.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ sonderr }) =>
      Effect.gen(function* () {
        const r = yield* sonderr.spawn(["stats"])
        sonderr.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under SONDERR_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ sonderr }) =>
      Effect.gen(function* () {
        const r = yield* sonderr.spawn(["db", "path"])
        sonderr.expectExit(r, 0, "db path")
