...
// Mock @/sonderr-sessions/pr-link before importing the command so the status
// handler reads the override/detection from these stubs instead of spawning
// `gh` or touching real Storage.
const realPrLink = await import("@/sonderr-sessions/pr-link")
...
void mock.module("@/sonderr-sessions/pr-link", () => ({
  ...realPrLink,
