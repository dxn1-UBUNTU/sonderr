...
import { createTuiAttention } from "@sonderr/tui/attention"
...
describe("createTuiAttention", () => {
  ...
  test("supports focused-only requests", async () => {
    ...
    expect(renderer.notifications).toEqual([{ title: "Sonderr", message: "focused" }]) // sonderr_change
  })
  ...
  test("notification can deliver while focused when requested", async () => {
    ...
    expect(audio.playCalls).toBe(1)
    expect(renderer.notifications).toEqual([{ title: "Sonderr", message: "hello" }]) // sonderr_change
  })
  ...
  test("notifies while blurred", async () => {
    ...
    expect(await attention.notify({ title: "Sonderr", message: "hello", sound: false })).toEqual({
      // sonderr_change
      ok: true,
      notification: true,
      sound: false,
  ...
  test("can play sound always while notification is blurred-only", async () => {
    ...
    expect(renderer.notifications).toEqual([{ title: "Sonderr", message: "hello again" }]) // sonderr_change
  })
  ...
  test("plays named sounds from the active sound pack", async () => {
    ...
    dispose()
    expect(attention.soundboard.current()).toBe("sonderr.default") // sonderr_change
  })
