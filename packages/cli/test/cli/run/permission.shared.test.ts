...
import {
  ...
  permissionEscape,
  permissionInfo,
  permissionOptions, // sonderr_change
  permissionReject,
  permissionRun,
...
describe("run permission shared", () => {
  test("replies immediately for allow once", () => {
    ...
    expect(out.reply).toEqual({
      requestID: "perm-1",
      reply: "once",
      interactive: true, // sonderr_change
    })
  })
  ...
  // sonderr_change start - skill-shell options
  ...
  test("formats always-allow copy for wildcard and explicit patterns", () => {
    expect(permissionAlwaysLines(req({ permission: "bash", always: ["*"] }))).toEqual([
      "This will allow bash until Sonderr is restarted.",
    ])

    expect(permissionAlwaysLines(req({ always: ["src/**/*.ts", "src/**/*.tsx"] }))).toEqual([
      "This will allow the following patterns until Sonderr is restarted.",
      "- src/**/*.ts",
      "- src/**/*.tsx",
    ])
  })
