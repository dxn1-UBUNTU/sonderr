...
import { SonderrClient, type GlobalEvent } from "@sonderr/sdk/v2"
...
type GlobalEventStream = Awaited<ReturnType<SonderrClient["global"]["event"]>>["stream"]
type SessionMessage = NonNullable<Awaited<ReturnType<SonderrClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<SonderrClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<SonderrClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://sonderr.test"),
...
function sdk(
  input: {
    ...
    globalEvent?: SonderrClient["global"]["event"]
    promptAsync?: SonderrClient["session"]["promptAsync"]
    status?: SonderrClient["session"]["status"]
    messages?: SonderrClient["session"]["messages"]
    children?: SonderrClient["session"]["children"]
    permissions?: SonderrClient["permission"]["list"]
    questions?: SonderrClient["question"]["list"]
  } = {},
) {
  const client = new SonderrClient()

  const globalEvent: SonderrClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: SonderrClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: SonderrClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: SonderrClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: SonderrClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: SonderrClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: SonderrClient["question"]["list"] = input.questions ?? (() => ok([]))
