...
import { extend, testRender, useRenderer } from "@opentui/solid" // sonderr_change - register test components in this renderer instance
import { createSignal } from "solid-js"
import { createDefaultOpenTuiKeymap } from "@opentui/keymap/opentui"
import type { QuestionRequest } from "@sonderr/sdk/v2"
import { SonderrKeymapProvider, registerSonderrKeymap } from "@sonderr/tui/keymap"
import {
...
// sonderr_change start - keep renderer tests independent of the animated spinner's native teardown
...
async function renderFooter(
  ...
  function Harness() {
    ...
    offKeymap = registerSonderrKeymap(keymap, renderer, config)
...
test("direct subagent panel closes when moving up from the first item", async () => {
  ...
  try {
    ...
    // sonderr_change start - ctrl+p is the same navigation command as ArrowUp
...
// sonderr_change start - cover direct-mode Sonderr skill picker transitions
...
test("direct footer shows editable prompts and additional queued work while running", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    offKeymap = registerSonderrKeymap(keymap, renderer, tuiConfig)
    ...
    return (
      <SonderrKeymapProvider keymap={keymap}>
        <RunFooterView
          ...
          onLayout={() => {}}
          onStatus={() => {}}
          onTerminalWrite={async () => {}} // sonderr_change
          onTerminalResize={async () => {}} // sonderr_change
          onTerminalClose={async () => {}} // sonderr_change
          onQueuedRemove={async () => true}
        />
      </SonderrKeymapProvider>
    )
...
test("direct footer separates a lone context hint from model and command hint", async () => {
  const app = await renderFooter({
    providers: [provider()],
    currentModel: { providerID: "sonderr", modelID: "gpt-5" },
...
test.skip("direct custom answer submits through keymap return binding", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    off = registerSonderrKeymap(keymap, renderer, tuiConfig)
...
test("direct model panel renders current model selector", async () => {
  ...
  const [current] = createSignal<RunInput["model"]>({ providerID: "sonderr", modelID: "gpt-5" })
  ...
  try {
    ...
    expect(frame).toContain("Search")
    expect(frame).toContain("sonderr")
    expect(frame).toContain("GPT-5")
