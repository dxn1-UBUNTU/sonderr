// Subprocess integration tests for `sonderr run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `sonderr.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `SONDERR_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { createSonderrClient } from "@sonderr/sdk/v2" // sonderr_change
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("sonderr run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, sonderr }) =>
      Effect.gen(function* () {
        ...
        const result = yield* sonderr.run("say hi")
  ...
  cliIt.concurrent(
    ...
    ({ llm, sonderr }) =>
      Effect.gen(function* () {
        ...
        const result = yield* sonderr.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, sonderr }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* sonderr.run("think", { extraArgs: ["--thinking"] })
        sonderr.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* sonderr.run("think again")
        sonderr.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, sonderr }) =>
      Effect.gen(function* () {
        ...
        // sonderr_change - settle bash up front so the run exercises the stream finish rather than
        // Sonderr's auto-reject exit contract, which a plain headless run would trip first.
        const result = yield* sonderr.run("trigger midstream error", {
          timeoutMs: 30_000,
  ...
  // sonderr_change start - Sonderr headless runs must signal an unsuccessful session to automation
  cliIt.concurrent(
    ...
    ({ llm, sonderr }) =>
      Effect.gen(function* () {
        ...
        const result = yield* sonderr.run("trigger midstream error", { timeoutMs: 30_000 })
        sonderr.expectExit(result, 1)
  ...
  cliIt.concurrent(
    ...
    ({ llm, sonderr }) =>
      Effect.gen(function* () {
        ...
        sonderr.expectExit(result, 1)
        expect(result.stderr).toContain("upstream provider exploded mid-stream")
        expect(sonderr.parseJsonEvents(result.stdout).some((event) => event.type === "error")).toBe(true)
      }),
    60_000,
  )
  ...
  cliIt.concurrent(
    ...
    ({ llm, sonderr }) =>
      Effect.gen(function* () {
        ...
        const result = yield* sonderr.run("run sed on readme", { timeoutMs: 45_000 })
        sonderr.expectExit(result, 1)
  ...
  cliIt.concurrent(
    ...
    ({ llm, sonderr }) =>
      Effect.gen(function* () {
        yield* llm.tool("bash", { command: "sed -n 1,5p README.md" })
        const result = yield* sonderr.run("run sed on readme", {
          format: "json",
          timeoutMs: 75_000,
        })
        sonderr.expectExit(result, 1)
  ...
  cliIt.concurrent(
    ...
    ({ llm, sonderr }) =>
      Effect.gen(function* () {
        ...
        const result = yield* sonderr.run("say hi", { format: "json" })
        sonderr.expectExit(result, 0)

        const events = sonderr.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ sonderr }) =>
      Effect.gen(function* () {
        const result = yield* sonderr.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
        ...
        expect(result.exitCode).not.toBe(0)
        const events = sonderr.parseJsonEvents(result.stdout)
        // sonderr_change - upstream expects a single record. Sonderr emits two, and has since before
  ...
  cliIt.concurrent(
    ...
    ({ llm, sonderr }) =>
      Effect.gen(function* () {
        ...
        const result = yield* sonderr.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, sonderr }) =>
      Effect.gen(function* () {
        ...
        // sonderr_change - settle bash up front; see the note on the reason assertion below
        const result = yield* sonderr.run("fail after output", { format: "json", permission: { bash: "deny" } })
        ...
        expect(events.map((event) => event.type)).toEqual([
          "step_start",
          "tool_use",
          "text", // sonderr_change - a pre-denied tool settles before the SDK closes its preceding text part
          "step_finish",
          "step_start",
          "step_finish",
        ])
        // sonderr_change start - the pre-denied tool completes before text-end
        expect(events.find((event) => event.type === "text")?.part).toEqual(
          expect.objectContaining({ type: "text", text: "partial json" }),
        )
        // sonderr_change end
        // sonderr_change - upstream asserts reason "unknown" here. Reaching that requires the bash call
        // to proceed without permission friction, which a Sonderr headless run never does: left alone the ask
  ...
  cliIt.concurrent(
    "auto-rejects requested permissions by default and allows them with the dangerous flag", // sonderr_change
    ({ home, llm, sonderr }) =>
      Effect.gen(function* () {
        yield* llm.tool("bash", { command: "rm -f denied-file", description: "Remove a test file" })
        yield* llm.text("continued after rejection")
        const denied = yield* sonderr.run("request permission", { permission: { bash: "ask" } })
        sonderr.expectExit(denied, 1) // sonderr_change
        expect(denied.stderr).toContain("permission requested: bash")
        expect(denied.stderr).toContain("run ended with an auto-rejected permission; pass --auto for autonomous use") // sonderr_change
        expect(denied.stdout).toBe("")

        yield* llm.reset
        yield* llm.tool("bash", { command: "rm -f allowed-file", description: "Remove a test file" })
        yield* llm.text("continued after approval")
        const allowed = yield* sonderr.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        sonderr.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* sonderr.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        sonderr.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, sonderr }) =>
      Effect.gen(function* () {
        ...
        const server = yield* sonderr.serve()

        const result = yield* sonderr.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, sonderr }) =>
      Effect.gen(function* () {
        const result = yield* sonderr.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, sonderr }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* sonderr.startRun("wait forever")
        yield* llm.wait(1)
        const interrupted = Date.now() // sonderr_change - assert signal handling, independent of contended CLI startup
        run.interrupt()
        const result = yield* run.result

        expect(result.exitCode).not.toBe(0)
        expect(Date.now() - interrupted).toBeLessThan(10_000) // sonderr_change
      }),
    60_000, // sonderr_change
  )

  // sonderr_change start - non-interactive runs exclude human-driven tools like suggest
  cliIt.concurrent(
    "sonderr run --auto excludes suggest tool from LLM request",
    ({ llm, sonderr }) =>
      Effect.gen(function* () {
        yield* llm.text("done")
        const result = yield* sonderr.run("do work", { extraArgs: ["--auto"] })
        sonderr.expectExit(result, 0)
  ...
  cliIt.live(
    "sonderr run auto-dismisses suggestion and exits cleanly if suggest tool is invoked in attached session",
    ({ llm, sonderr }) =>
      Effect.gen(function* () {
        const server = yield* sonderr.serve()
        ...
        const result = yield* sonderr.run("do work", {
