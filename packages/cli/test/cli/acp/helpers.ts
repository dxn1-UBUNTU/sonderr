...
export function createAcpClient(input: Pick<CliFixture, "sonderr">, env?: Record<string, string>) {
  return Effect.gen(function* () {
    return createJsonRpcAcpClient(yield* input.sonderr.acp(env ? { env } : undefined))
  })
}
  ...
  return Effect.gen(function* () {
    return expectOk(
      yield* acp.request<InitializeResponse>("initialize", {
        ...
        clientInfo: { name: "sonderr-local-acp", version: "0.1.0" },
      }),
    )
  })
}
