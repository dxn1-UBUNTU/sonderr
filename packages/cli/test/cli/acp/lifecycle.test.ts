...
describe("sonderr acp lifecycle subprocess", () => {
  cliIt.live(
    "stdin EOF exits cleanly",
    ({ sonderr }) =>
      Effect.gen(function* () {
        const acp = yield* sonderr.acp()
        acp.close()

        const code = yield* Effect.promise(() => acp.exited).pipe(Effect.timeout(Duration.seconds(15))) // sonderr_change
        expect(code).toBe(0)
      }),
    60_000,
  )
    ...
    ({ home, llm, sonderr }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient(
          { sonderr },
          { SONDERR_CONFIG_CONTENT: JSON.stringify(verifierConfig(llm.url)) },
        )
