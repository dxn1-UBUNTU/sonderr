...
type PermissionReplyParams = Parameters<SonderrClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "sonderr-acp-permission-"))
...
describe("acp permissions", () => {
  it("sends requestPermission and replies with the selected outcome", async () => {
    ...
    // sonderr_change start - human selections are marked interactive
  ...
  // sonderr_change start - skill shell batches surface their command list and cannot be persisted
