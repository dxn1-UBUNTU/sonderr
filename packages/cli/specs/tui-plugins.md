...
- Author package entrypoint is `@sonderr/plugin/tui`.
...
```json
{
  ...
  "plugin": ["@acme/sonderr-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@sonderr/plugin/tui`.
- `@sonderr/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
- The value is a semver range checked against the running Sonderr version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.sonderr` is absent, no check is performed (backward compatible).
...
- `sonderr plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `sonderr plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.sonderr` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.sonderr`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.sonderr`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.sonderr` or legacy `.sonderr` directory, or `SONDERR_CONFIG_DIR`, Sonderr installs `@sonderr/plugin` into that directory and writes:
...
That is what makes local config-scoped plugins able to import `@sonderr/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Sonderr's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Sonderr registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Sonderr pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@sonderr/plugin/tui`.
...
- `message` is required; `title` defaults to `"Sonderr"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `sonderr.default`.
...
- Local plugins persist installed themes under `.sonderr/themes`, or under `themes` beside a config source in `.sonderr` or legacy `.sonderr`.
...
- `--pure` / `SONDERR_PURE` skips external TUI plugins only.
...
The upstream `.sonderr` smoke fixtures remain in the repository for Sonderr compatibility, but Sonderr does not auto-load them. Sonderr-local examples must live under `.sonderr` or legacy `.sonderr`.
