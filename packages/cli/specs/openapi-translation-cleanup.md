...
Trim `packages/cli/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/cli/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/cli`.
- `[x]` `bun typecheck` from `packages/cli`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/cli/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/cli`.
- `[x]` `bun dev generate > /tmp/sonderr-openapi.json` from `packages/cli`.
...
- Edit `packages/cli/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/sonderr-openapi.json` from `packages/cli`.
...
- Check source schemas in files like `packages/cli/src/session/schema.ts`, `packages/cli/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/cli/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/cli/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
