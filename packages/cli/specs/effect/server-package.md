...
Practical reference for a future `packages/server` split after the sonderr
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/cli`.
...
Until enough shared service code lives outside `packages/cli`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/cli`

It should not import `packages/cli` services while `packages/cli`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on sonderr-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/cli` implementation details.
