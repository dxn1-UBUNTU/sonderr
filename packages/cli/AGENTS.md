# Sonderr CLI package guidelines
...
The MCP `StdioClientTransport` (third-party SDK) is handled separately via a process shim in `src/mcp/index.ts` that sets `process.type = "browser"` when running inside the VS Code extension (`SONDERR_PLATFORM=vscode`), which causes the SDK's internal `isElectron()` check to return `true` and enable `windowsHide`.

## Storage

Filesystem-based JSON, not a database. Data lives in `~/.local/share/sonderr/storage/`. Keys are path arrays: `Storage.write(["session", projectID, sessionID], data)`.

## TUI

Built with **SolidJS + OpenTUI** (`@opentui/solid`) -- a terminal UI framework. JSX renders to the terminal using elements like `<box>`, `<text>`, `<scrollbox>`. The TUI communicates with the server via `@sonderr/sdk`.
...
`sonderr/` is a fork of upstream sonderr. When a change must touch a shared upstream file, extract the Sonderr-specific logic into a mirror file under `src/sonderr/<same/path>.ts` (tests under `test/sonderr/<same/path>.test.ts`) and call into it from the upstream file behind a single `sonderr_change` marker. Example: a Sonderr override for `src/cli/cmd/tui/component/dialog-provider.tsx` lives at `src/sonderr/cli/cmd/tui/component/dialog-provider.tsx`. Avoid inlining Sonderr-specific logic directly into shared upstream files. Files and directories whose path contains `sonderr` never need `sonderr_change` markers.
