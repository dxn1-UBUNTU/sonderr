...
import { path } from "@sonderr/core/effect/app-node-platform"
...
import type { ProjectV2 } from "@sonderr/core/project"
...
import { WorktreeCleanup } from "@/sonderr/worktree-cleanup" // sonderr_change
import { clearPtys } from "@/sonderr/worktree/pty-cleanup" // sonderr_change
import { WorktreeEvent } from "@sonderr/schema/worktree-event"
...
export class Service extends Context.Service<Service, Interface>()("@sonderr/Worktree") {}
  ...
  Effect.gen(function* () {
    ...
    const candidate = Effect.fn("Worktree.candidate")(function* (input: {
      ...
      for (const attempt of Array.from({ length: MAX_NAME_ATTEMPTS }, (_, i) => i)) {
        ...
        const branch = input.detached ? undefined : `sonderr/${name}`
    ...
    const boot = Effect.fnUntraced(function* (info: Info, startCommand?: string) {
      ...
      // sonderr_change start - signal full readiness once setup also completes
      GlobalBus.emit("event", {
        directory: info.directory,
        project: ctx.project.id,
        workspace: workspaceID,
    ...
    // sonderr_change start - use Sonderr cleanup helper for slow Windows handle release
    ...
    const remove = Effect.fn("Worktree.remove")(function* (input: RemoveInput) {
      const ctx = yield* InstanceState.context
      const workspaceID = yield* InstanceState.workspaceID // sonderr_change
      if (ctx.project.vcs !== "git") {
        return yield* new NotGitError({ message: "Worktrees are only supported for git projects" })
      }
      ...
      if (!entry?.path) {
        yield* clearPtys(directory, workspaceID) // sonderr_change
      ...
      yield* clearPtys(entry.path, workspaceID) // sonderr_change
