...
import { which } from "@sonderr/core/util/which"
import { Module } from "@sonderr/core/util/module"
import { spawn } from "./launch"
import { Npm } from "@sonderr/core/npm"
import { TsCheck } from "../sonderr/ts-check" // sonderr_change
import type { RuntimeFlags } from "@/effect/runtime-flags"

const log = Log.create({ service: "lsp.server" }) // sonderr_change
...
// sonderr_change start - tsgo native LSP or lightweight diagnostic client
// When SONDERR_EXPERIMENTAL_LSP_TOOL is enabled, spawn tsgo --lsp --stdio as a
...
export const Typescript: Info = {
  ...
  async spawn(root) {
    if (!Flag.SONDERR_EXPERIMENTAL_LSP_TOOL) return undefined
...
export const Clangd: Info = {
  ...
  async spawn(root, _ctx, flags) {
    ...
    // sonderr_change start - reject release metadata before it becomes an executable path
    ...
    // sonderr_change start - do not use remote metadata as a local path
...
export const JDTLS: Info = {
  ...
  async spawn(root, _ctx, flags) {
    ...
    const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "sonderr-jdtls-data"))
    return {
      process: spawn(
        java,
        [
          "-Djava.import.generatesMetadataFilesAtProjectRoot=false", // sonderr_change
          "-jar",
          launcherJar,
          "-configuration",
          configFile,
...
export const LuaLS: Info = {
  ...
  async spawn(root, _ctx, flags) {
    ...
    if (!bin) {
      ...
      // sonderr_change start - use a fixed local archive name
