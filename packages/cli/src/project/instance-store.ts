...
import { makeGlobalNode, Node } from "@sonderr/core/effect/app-node"
...
import { context as instanceContext, type InstanceContext } from "./instance-context" // sonderr_change
...
export class Service extends Context.Service<Service, Interface>()("@sonderr/InstanceStore") {}
  ...
  Effect.gen(function* () {
    ...
    const boot = (input: LoadInput & { directory: string }) =>
      Effect.gen(function* () {
        ...
        // sonderr_change start - run bootstrap inside the Instance ALS so SonderrBootstrap
    ...
    // sonderr_change start - complete the deferred and drop failed entries on any exit,
    ...
    const disposeEntry = Effect.fnUntraced(function* (directory: string, entry: Entry, ctx: InstanceContext) {
      if (cache.get(directory) !== entry) return false
      // sonderr_change start - remove disposed entries even when event publication fails
      ...
      return Effect.uninterruptibleMask((restore) =>
        Effect.gen(function* () {
          ...
          yield* Effect.gen(function* () {
            ...
            if (previous) {
              // sonderr_change start - dispose reloads under the previous instance context
    ...
    const disposeAllOnce = Effect.fnUntraced(function* () {
      ...
      // sonderr_change start - dispose independent worktrees concurrently without interrupting siblings
