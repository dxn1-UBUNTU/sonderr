...
import * as ModelsDev from "./models" // sonderr_change - assemble dynamic Sonderr models around upstream core catalog
...
import { optional, optionalOmitUndefined } from "@sonderr/core/schema" // sonderr_change
...
import {
  SONDERR_BUNDLED_PROVIDERS,
  sonderrCustomLoaders,
  SONDERR_MODEL_SCHEMA_EXTENSIONS,
  patchModelsDevModel as patchSonderrModel,
  patchConfigModel as patchSonderrConfigModel,
  customProviderVariants,
  patchCustomLoaderResult,
  patchSonderrProviderPrivacy,
  sonderrSmallModelPriority,
  buildTimeoutSignal,
  requestTimeout,
  wrapFirstByte,
    ...
    async pull(ctrl) {
      const part = await new Promise<Awaited<ReturnType<typeof reader.read>>>((resolve, reject) => {
        const id = setTimeout(() => {
          ...
          ctl.abort(err)
          void reader.cancel(err).catch(() => undefined) // sonderr_change - handle Bun 1.4 cancellation rejection
          reject(err)
        }, ms)
...
const BUNDLED_PROVIDERS: Record<string, () => Promise<(opts: any) => BundledSDK>> = {
  ...
  "@ai-sdk/github-copilot": () =>
    import("@sonderr/core/github-copilot/copilot-provider").then((m) => m.createOpenaiCompatible),
  "venice-ai-sdk-provider": () => import("venice-ai-sdk-provider").then((m) => m.createVenice),
  ...SONDERR_BUNDLED_PROVIDERS, // sonderr_change
}
...
function custom(dep: CustomDep): Record<string, CustomLoader> {
  return {
    ...
    sonderr: Effect.fnUntraced(function* (input: Info) {
      ...
      const ok =
        ...
        Boolean((yield* dep.config()).provider?.["sonderr"]?.options?.apiKey)
    ...
    azure: Effect.fnUntraced(function* (provider: Info) {
      ...
      // sonderr_change start - prefer explicit Azure endpoint over resource name to avoid conflicting SDK options
      const endpoint = iife(() => {
        return [
      ...
      if (!resource && !endpoint) {
        ...
        return {
          ...
          async getModel() {
            throw new Error(
              "Azure resource name or endpoint is missing. Set AZURE_RESOURCE_NAME, AZURE_OPENAI_RESOURCE_NAME, AZURE_OPENAI_ENDPOINT, or reconnect the azure provider.", // sonderr_change
            )
          },
        }
      }
      ...
      return {
        ...
        options: {
          ...(endpoint ? { baseURL: endpoint } : { resourceName: resource }), // sonderr_change
        },
    ...
    "amazon-bedrock": Effect.fnUntraced(function* () {
      ...
      const stored = bedrockAuth(auth) // sonderr_change
      const env = yield* dep.env()

      // Region precedence: 1) config file, 2) stored credentials, 3) env var, 4) default // sonderr_change
      const configRegion = providerConfig?.options?.region
      const envRegion = env["AWS_REGION"]
      const defaultRegion = configRegion ?? stored?.region ?? envRegion ?? "us-east-1" // sonderr_change
      ...
      // sonderr_change start - pass stored bearer tokens directly without leaking them into process.env
      ...
      const providerOptions: Record<string, any> = {
        ...
        ...(stored ? { credentialProvider: async () => stored.credentials } : {}), // sonderr_change
      }

      // Only use credential chain if no bearer token exists
      // Bearer token takes precedence over credential chain (profiles, access keys, IAM roles, web identity tokens)
      // sonderr_change start - stored static credentials use the credential provider above
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>, model?: Model) {
          ...
          // 1. options.region from sonderr.json provider config
      ...
      Effect.succeed({
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://kilo.ai/",
            "X-Title": "Sonderr", // sonderr_change
            "X-BILLING-INVOKE-ORIGIN": "Sonderr", // sonderr_change
          },
        },
      }),
    vercel: () =>
    ...
    "google-vertex": Effect.fnUntraced(function* (provider: Info) {
      const env = yield* dep.env()
      const stored = vertexAuth(yield* dep.auth("google-vertex")) // sonderr_change
      ...
      const location = String(
        provider.options?.location ??
          stored?.location ?? // sonderr_change
      ...
      return {
        ...
        options: {
          project,
          location,
          ...(stored ? vertexCredentials(stored.credentials) : {}), // sonderr_change
          fetch: async (input: RequestInfo | URL, init?: RequestInit) => {
            const { GoogleAuth } = await import("google-auth-library")
            // sonderr_change start - authenticate OpenAI-compatible Vertex endpoints with stored credentials
    ...
    gitlab: Effect.fnUntraced(function* (input: Info) {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `sonderr/${InstallationVersion} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`, // sonderr_change
    ...
    "cloudflare-workers-ai": Effect.fnUntraced(function* (input: Info) {
      ...
      return {
        ...
        options: {
          apiKey,
          headers: {
            "User-Agent": `sonderr/${InstallationVersion} cloudflare-workers-ai (${os.platform()} ${os.release()}; ${os.arch()})`,
          },
        },
        async getModel(sdk: any, modelID: string) {
          return sdk.languageModel(modelID)
    ...
    "cloudflare-ai-gateway": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `sonderr auth cloudflare-ai-gateway`.",
        )
      }
      ...
      const opts = {
        ...
        headers: {
          "User-Agent": `sonderr/${InstallationVersion} cloudflare-ai-gateway (${os.platform()} ${os.release()}; ${os.arch()})`,
        },
      }
    ...
    cerebras: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "Sonderr", // sonderr_change
          },
        },
      }),
    sonderr: () =>
    ...
    "snowflake-cortex": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!account || !token) {
        ...
        return {
          ...
          async getModel() {
            throw new Error(
              `Snowflake Cortex: missing credentials (${missing}). Provide a bearer token (OAuth, JWT, or PAT) via env var, Sonderr auth, or provider options.`, // sonderr_change
            )
          },
        }
      }
...
export const Model = Schema.Struct({
  ...
  ...SONDERR_MODEL_SCHEMA_EXTENSIONS, // sonderr_change
...
export const Info = Schema.Struct({
  id: ProviderV2.ID,
  name: Schema.String,
  description: optionalOmitUndefined(Schema.String), // sonderr_change
  source: Schema.Literals(["env", "config", "custom", "api"]),
  env: Schema.Array(Schema.String),
  key: optional(Schema.String),
  metadata: optionalOmitUndefined(ProviderMetadata), // sonderr_change
  options: Schema.Record(Schema.String, Schema.Any),
...
export const ListResult = Schema.Struct({
  ...
  failed: Schema.Array(Schema.String), // sonderr_change
})
...
export class ModelNotFoundError extends Schema.TaggedErrorClass<ModelNotFoundError>()("ProviderModelNotFoundError", {
  ...
  modelsEmpty: Schema.optional(Schema.Boolean), // sonderr_change
...
export class Service extends Context.Service<Service, Interface>()("@sonderr/Provider") {}
...
function fromModelsDevModel(provider: ModelsDev.Provider, model: ModelsDev.Model): Model {
  ...
  Object.assign(base, patchSonderrModel(provider.id, model)) // sonderr_change
  const variants = ProviderTransform.reasoningVariants(model, base) ?? ProviderTransform.variants(base) // sonderr_change

  return {
    ...base,
    variants: mapValues(variants, (v) => v), // sonderr_change
  }
}
...
export function fromModelsDevProvider(provider: ModelsDev.Provider): Info {
  ...
  return {
    ...
    source: "custom",
    name: provider.name,
    description: provider.description, // sonderr_change
    env: [...(provider.env ?? [])],
    options: {},
    models,
  }
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        for (const [providerID, provider] of configProviders) {
          if (!provider) continue // sonderr_change - null entries are transient delete sentinels
          ...
          for (const [modelID, model] of Object.entries(provider.models ?? {})) {
            if (!model) continue // sonderr_change - null entries are transient delete sentinels
            ...
            const parsedModel: Model = {
              ...
              // variants: {}, // sonderr_change, moved into patchSonderrConfigModel
              ...patchSonderrConfigModel(model, existingModel), // sonderr_change
            }
            // sonderr_change start
            ...
            parsedModel.variants = mapValues(
              pickBy(merged, (v): v is NonNullable<typeof v> => !!v && !v.disabled), // sonderr_change - drop null delete sentinels
              (v) => omit(v, ["disabled"]),
            )
        ...
        // sonderr_change start - load auths before env so OAuth plugins can override inherited credentials
        ...
        for (const [id, provider] of Object.entries(database)) {
          ...
          if (disabled.has(providerID)) continue
          // sonderr_change start - prefer explicit OAuth auth over inherited env credentials
          if (
        ...
        for (const [id, provider] of Object.entries(auths)) {
          ...
          if (provider.type === "api") {
            mergeProvider(providerID, {
              source: "api",
              key: providerKey(providerID, provider), // sonderr_change - keep structured credentials provider-specific
            })
          }
        }
        ...
        // sonderr_change start - resolve env once for patchCustomLoaderResult (azure env fallback)
        const sonderrEnv = yield* env.all()
        // sonderr_change end
        for (const [id, fn] of Object.entries({ ...custom(dep), ...sonderrCustomLoaders(dep) })) {
          // sonderr_change
          const providerID = ProviderV2.ID.make(id)
          if (disabled.has(providerID)) continue
          ...
          if (result) patchCustomLoaderResult(id, result, sonderrEnv) // sonderr_change
        ...
        for (const [id, provider] of configProviders) {
          if (!provider) continue // sonderr_change - null entries are transient delete sentinels
          const providerID = ProviderV2.ID.make(id)
          // sonderr_change start - keep OAuth plugin source when config and Codex auth coexist
          const oauth =
        ...
        patchSonderrProviderPrivacy(providers[ProviderV2.ID.make("sonderr")], cfg) // sonderr_change

        const gitlab = ProviderV2.ID.make("gitlab")
        if (discoveryLoaders[gitlab] && providers[gitlab] && isProviderAllowed(gitlab)) {
          // sonderr_change start - keep discovery failures visible instead of swallowing them
    ...
    yield* ModelsRefresh.watch(state) // sonderr_change
    ...
    async function resolveSDK(model: Model, s: State, envs: Record<string, string | undefined>) {
      try {
        ...
        vertexOptions(model.providerID, model.api.npm, options) // sonderr_change - hydrate stored credentials
        ...
        options["fetch"] = async (input: any, init?: BunFetchRequestInit) => {
          ...
          const timeout = buildTimeoutSignal(options) // sonderr_change - use cancellable timeout for connection phase
          // sonderr_change start - extend the same deadline to the first response byte
          ...
          if (timeout.signal) signals.push(timeout.signal) // sonderr_change
          if (firstByteCtl) signals.push(firstByteCtl.signal) // sonderr_change

          const combined = signals.length === 0 ? null : signals.length === 1 ? signals[0] : AbortSignal.any(signals)
          if (combined) opts.signal = combined

          // sonderr_change start - clear connection-phase timeout once headers arrive
          try {
            ...
            timeout.clear()
            // sonderr_change start - hand the remaining deadline to the first-byte guard
    ...
    const getModel = Effect.fn("Provider.getModel")(function* (providerID: ProviderV2.ID, modelID: ModelV2.ID) {
      ...
      if (!provider) {
        ...
        const suggestions = catalogProvider
          ...
          : fuzzysort
              ...
              .map((m) => m.target)
        const empty = false // sonderr_change
        return yield* new ModelNotFoundError({ providerID, modelID, suggestions, modelsEmpty: empty }) // sonderr_change
      }
      ...
      if (!info) {
        ...
        const empty = Object.keys(provider.models).length === 0 // sonderr_change
        return yield* new ModelNotFoundError({ providerID, modelID, suggestions, modelsEmpty: empty }) // sonderr_change
      }
      return info
    })
    ...
    const getSmallModel = Effect.fn("Provider.getSmallModel")(function* (providerID: ProviderV2.ID) {
      ...
      // sonderr_change start - Sonderr's auto model is an ID, while upstream priorities are model families.
      const sonderrPriority = sonderrSmallModelPriority(providerID)
      if (sonderrPriority) {
        for (const id of sonderrPriority) {
          const model = provider.models[id]
      ...
      const priority = providerID.startsWith("sonderr")
        ? ["gpt-nano"]
      ...
      // sonderr_change start - fall back to sonderr's auto small model
      const sonderrFallback = s.providers[ProviderV2.ID.make("sonderr")] ?? s.catalog[ProviderV2.ID.make("sonderr")]
      if (sonderrFallback?.models["sonderr-auto/small"]) return sonderrFallback.models["sonderr-auto/small"]
      // sonderr_change end
