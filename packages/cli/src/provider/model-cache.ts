...
import { fetchSonderrModels, type SonderrModelsResult } from "@sonderr/sonderr-gateway"
import { Context, Deferred, Duration, Effect, Exit, Layer, Schema, Scope } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest, HttpClientResponse } from "effect/unstable/http"
import { Config } from "../config/config"
import { Auth } from "../auth"
import type { Provider } from "@sonderr/core/models-dev"
...
import { httpClient } from "@sonderr/core/effect/app-node-platform" // sonderr_change

type Models = Provider["models"]
type SonderrOptions = NonNullable<Parameters<typeof fetchSonderrModels>[0]>
type Options = { -readonly [K in keyof SonderrOptions]?: SonderrOptions[K] } & { apiKey?: string }
type Failure = NonNullable<SonderrModelsResult["error"]>
...
export interface SonderrModels {
  readonly fetch: (options: SonderrOptions) => Effect.Effect<SonderrModelsResult, unknown>
}

export class SonderrModelsService extends Context.Service<SonderrModelsService, SonderrModels>()(
  "@sonderr/ModelCache/SonderrModels",
) {}

export const sonderrModelsLayer = Layer.succeed(
  SonderrModelsService,
  SonderrModelsService.of({ fetch: (options) => Effect.tryPromise(() => fetchSonderrModels(options)) }),
)
type Cell = {
...
export class Service extends Context.Service<Service, Interface>()("@sonderr/ModelCache") {}
...
export const layer: Layer.Layer<
  Service,
  never,
  Auth.Service | Config.Service | SonderrModelsService | HttpClient.HttpClient
> = Layer.effect(
  Service,
  Effect.gen(function* () {
    const auth = yield* Auth.Service
    const cfg = yield* Config.Service
    const sonderr = yield* SonderrModelsService
    ...
    const authOptions = Effect.fn("ModelCache.authOptions")(function* (providerID: string) {
      if (providerID !== "sonderr" && providerID !== "apertis") return {}
      const config = yield* cfg.get()
      const options: Options = {}

      if (providerID === "sonderr") {
        const item = config.provider?.[providerID]
        if (item?.options?.apiKey) options.sonderrToken = item.options.apiKey
        if (item?.options?.sonderrOrganizationId) options.sonderrOrganizationId = item.options.sonderrOrganizationId

        const info = yield* auth.get(providerID)
        if (info?.type === "api") options.sonderrToken = info.key
        if (info?.type === "oauth") {
          options.sonderrToken = info.access
          if (info.accountId) options.sonderrOrganizationId = info.accountId
        }

        if (process.env.SONDERR_API_KEY) options.sonderrToken = process.env.SONDERR_API_KEY
        if (process.env.SONDERR_ORG_ID) options.sonderrOrganizationId = process.env.SONDERR_ORG_ID
        log.debug("auth options resolved", {
          providerID,
          hasToken: !!options.sonderrToken,
          hasOrganizationId: !!options.sonderrOrganizationId,
        })
      }
    ...
    const fetchModels = (providerID: string, options: Options): Effect.Effect<Result, unknown> => {
      if (providerID === "sonderr") return sonderr.fetch(options)
    ...
    const key = (providerID: string, options?: Options) => {
      if (providerID === "sonderr") {
        return JSON.stringify([providerID, options?.baseURL, options?.sonderrOrganizationId, options?.sonderrToken])
      }
...
const sonderrModels = LayerNode.make({ name: "sonderr-models", layer: sonderrModelsLayer, deps: [] })
export const node = LayerNode.make({
  service: Service,
  layer,
  deps: [Auth.node, Config.node, sonderrModels, httpClient],
})
