...
import { httpClient, path } from "@sonderr/core/effect/app-node-platform"
...
import { isSafeSegment, isSafeRelativePath } from "@/sonderr/skill/discovery-validate" // sonderr_change
...
export class Service extends Context.Service<Service, Interface>()("@sonderr/SkillDiscovery") {}
...
const layer: Layer.Layer<Service, never, FSUtil.Service | Path.Path | HttpClient.HttpClient> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const pull = Effect.fn("Discovery.pull")(function* (url: string) {
      ...
      // sonderr_change start - resolve the index origin so file downloads can be pinned to it
      ...
      // sonderr_change start - the remote index controls skill.name and file, so validate every segment,
      ...
      // sonderr_change start - download each validated, origin-pinned, cache-confined plan
      const dirs = yield* Effect.forEach(
        planned,
        (skill) =>
          Effect.gen(function* () {
            const { root, version } = skill
            const versionFile = path.join(root, ".sonderr-version")
            ...
            if (version === undefined || current === version) {
              ...
              yield* Effect.gen(function* () {
                ...
                yield* fs.writeFileString(path.join(staging, ".sonderr-version"), version)
