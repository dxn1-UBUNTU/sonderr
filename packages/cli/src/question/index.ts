...
import { SonderrQuestion } from "@/sonderr/question" // sonderr_change
import { EventV2Bridge } from "@/event-v2-bridge"
import { QuestionV1 } from "@sonderr/schema/question-v1"
...
export interface Interface {
  readonly ask: (input: {
    ...
    blocking?: boolean // sonderr_change
    tool?: Tool
  ...
  readonly dismissAll: (sessionID: SessionID) => Effect.Effect<void> // sonderr_change
}

export class Service extends Context.Service<Service, Interface>()("@sonderr/Question") {}
  ...
  Effect.gen(function* () {
    ...
    const ask = Effect.fn("Question.ask")(function* (input: {
      ...
      const info: Request = {
        ...
        blocking: input.blocking, // sonderr_change
        tool: input.tool,
      }

      // sonderr_change start
      yield* SonderrQuestion.guardFollowup(input.sessionID, () => new RejectedError())
      // sonderr_change end
      ...
      return yield* Effect.ensuring(
        ...
        // sonderr_change start - every asked question gets a terminal event when its waiter is interrupted
        SonderrQuestion.finalize({
          pending,
          id,
    ...
    // sonderr_change start - body lives in @/sonderr/question/SonderrQuestion.makeDismissAll
    const dismissAll = SonderrQuestion.makeDismissAll({
      state,
    ...
    return Service.of({ ask, reply, reject, list, dismissAll }) // sonderr_change
  }),
)

// sonderr_change - preserve legacy layer composition for Sonderr callers
