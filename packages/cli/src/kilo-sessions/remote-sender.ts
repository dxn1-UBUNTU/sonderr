import { RemoteCommand } from "@/sonderr-sessions/remote-command"
import { RemoteExit } from "@/sonderr-sessions/remote-exit"
import { RemoteModelCatalog } from "@/sonderr-sessions/remote-model-catalog"
import { RemoteProtocol } from "@/sonderr-sessions/remote-protocol"
import { consumeRenameAdoption, markRenameAdopted } from "@/sonderr-sessions/rename-adoptions"
import type { RemoteWS } from "@/sonderr-sessions/remote-ws"
import { GlobalBus } from "@/bus/global"
import { RemoteAttachments } from "@/sonderr/remote-attachments"
import { Session } from "@/session/session"
import type { MessageV2 } from "@/session/message-v2"
import { SessionPrompt } from "@/session/prompt"
import { Question } from "@/question"
import { Suggestion } from "@/sonderr/suggestion" // sonderr_change
import { SonderrSessionPromptQueue } from "@/sonderr/session/prompt-queue"
...
import z from "zod"
import { zodObject } from "@sonderr/core/effect-zod"
import { Effect, Option, Schema } from "effect"
import { readdir } from "node:fs/promises"
import { isAbsolute, join, relative, sep } from "path"
import { Filesystem } from "@/util/filesystem"

type Provide = typeof import("@/sonderr/instance").provide
...
// sonderr_change start - create_session: strict v1 request with optional inheritance fields
...
const CreateSessionRequest = z
  .object({
    ...
    // sonderr_change - cloneFromSonderrSessionId: optional cloud-session import.
    ...
    cloneFromSonderrSessionId: z.string().min(1).optional(),
  })
  .strict()
...
// sonderr_change start - redact anything but the error class so messages/credentials
...
// sonderr_change - k1: resolve a client-supplied directory path under the
...
// sonderr_change - create_session cloud-import error mapping. The import seam
...
// sonderr_change start — lazy init to avoid circular dependency
...
function normalizeModel(model: string | RemoteModelCatalog.ModelRef | undefined) {
  ...
  return {
    ...
    modelID: ModelV2.ID.make(model.startsWith("sonderr/") ? model.slice("sonderr/".length) : model),
  }
}
...
export namespace RemoteSender {
  export type Options = {
    ...
    session?: {
      ...
      // sonderr_change start - injectable create hook for create_session.
      ...
      // sonderr_change - injectable remove hook used to roll back an orphan
      ...
      // sonderr_change - injectable setTitle for system session.renamed handling
      readonly setTitle?: (input: { sessionID: SessionID; title: string }) => Promise<void>
      // sonderr_change end
    }
    // sonderr_change - K1 W1: in-process attach/detach/ownership/cancel
    // seams. All four are optional and default to a lazy import of
    // `SonderrSessions` (production wires them in `enableRemote`, so the
    ...
    // sonderr_change - injectable cloud-session import seam for create_session
  ...
  export function create(options: Options): Sender {
    ...
    // sonderr_change start - orphan rollback for create_session: when
    ...
    // sonderr_change end
    // sonderr_change - K1 W1: session create + in-process attach seams used by
    // create_session. Production wires `attachSession` to
    // `SonderrSessions.attachRemoteSession` from inside `enableRemote` (see
    // sonderr-sessions.ts). Test fixtures inject stubs via the Options object.
    ...
    const attachSession =
      ...
      (async (id: SessionID) => {
        const { SonderrSessions } = await import("@/sonderr-sessions/sonderr-sessions")
        await SonderrSessions.attachRemoteSession(id)
      })
    const detachSession =
      options.detachSession ??
      (async (id: SessionID) => {
        const { SonderrSessions } = await import("@/sonderr-sessions/sonderr-sessions")
        await SonderrSessions.detachRemoteSession(id)
      })
    ...
    // sonderr_change end
    // sonderr_change start - injectable slash command discovery + execution
    ...
    async function replay(sessionId: string) {
      ...
      const queued = SonderrSessionPromptQueue.snapshot(SessionID.make(sessionId))
      options.conn.send({
        type: "event",
        sessionId,
        ...(root ? { parentSessionId: root } : {}),
    ...
    function dispatch(msg: RemoteProtocol.Command) {
      // sonderr_change start - slash command discovery and execution
      ...
      // sonderr_change - k1: connection-scoped directory listing for the
      ...
      if (msg.command === "exit_cli") {
        // sonderr_change - K1 W1: `exit_cli` now means "detach THIS remote
        // session and (if this is the last interactive session) close the
        // CLI." It is NOT "terminate the CLI." A headless `sonderr remote` host
        ...
        //      flush first. If zero remain + no callback (headless `sonderr
        ...
        void (async () => {
          try {
            ...
            //    (`sonderr remote`) never register a RemoteExit callback, so
      ...
      if (msg.command === "create_session") {
        // sonderr_change - K1 W1: in-process create_session. Optional wire
        // fields (agent/model/orgId) ride protocolVersion 1; orgId lands in
        // session metadata so the first sonderr_meta carries the claim. Handler
        ...
        // sonderr_change - clone: an optional cloneFromSonderrSessionId imports a
        ...
        const cloneId = parsed.data.cloneFromSonderrSessionId
        ...
        // sonderr_change - k1: a present `directory` starts the session in a
      ...
      // sonderr_change end
      // sonderr_change start - sessionless list_models for the pre-session instance picker
      ...
      // sonderr_change start - drop a queued (not yet running) remote message
      if (msg.command === "drop_queued_message") {
        ...
        dispatchQuick(msg, directoryFor(session.value), async () => {
          if (!Effect.runSync(SonderrSessionPromptQueue.drop(session.value, messageID))) {
            ...
            try {
              ...
              await run({
                ...
                fn: async () => {
                  ...
                  // setTitle and the sonderr-sessions consumer is deferred, so a
