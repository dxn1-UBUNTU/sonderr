...
import { SonderrConfigSources } from "./sources"

export namespace SonderrConfigOverlay {
  const log = Log.create({ service: "sonderr.config.overlay" })
  ...
  export const Result = z.object({
    ...
    sources: z.array(SonderrConfigSources.Source),
  ...
  export type Input = {
    ...
    global: Config.Info
    sources: SonderrConfigSources.Source[]
  }

  const files = ["sonderr.jsonc", "sonderr.json", "sonderr.jsonc", "sonderr.json"] as const
  const dirs = [".sonderr", ".sonderr"] as const
  ...
  const fieldPaths = [
    ...
    ["indexing", "sonderr", "apiKey"],
    ["indexing", "sonderr", "baseUrl"],
    ["indexing", "sonderr", "organizationId"],
  ...
  export async function project(input: { directory: string; worktree?: string }): Promise<Config.Info> {
    ...
    // sonderr_change - project config is untrusted; confine {file:} reads to the project root
    ...
    return configs.reduce((result, cfg) => SonderrConfig.mergeConfig(result, cfg), {} as Config.Info)
  }
  ...
  export async function projectTarget(input: { directory: string; worktree?: string }) {
    ...
    return candidates.find((file) => existsSync(file)) ?? path.join(input.directory, ".sonderr", "sonderr.jsonc")
  }
  ...
  export async function resolve(input: Input): Promise<Result> {
    // sonderr_change start - project agents untrusted, {file:} confined to the project root; global agents trusted
    const root = input.worktree && input.worktree !== "/" ? input.worktree : input.directory
  ...
  function globalDirs() {
    return [Global.Path.config, path.join(Global.Path.home, ".sonderr"), path.join(Global.Path.home, ".sonderr")]
  }

  // sonderr_change start - root confines untrusted agent {file:} reads
  async function withAgents(input: Config.Info, dirs: string[], trusted: boolean, root?: string): Promise<Config.Info> {
    ...
    const next = SonderrConfig.mergeConfig(SonderrConfig.mergeConfig(input, { agent }), { agent: mode })
  ...
  async function load(file: string, fileScope?: ConfigVariable.FileScope): Promise<Config.Info> {
    // sonderr_change start - a single unsafe/invalid project config file must not break the settings overlay;
  ...
  async function loadUnsafe(file: string, fileScope?: ConfigVariable.FileScope): Promise<Config.Info> {
    ...
    // sonderr_change start - remove variable-bearing MCP headers before resolving other project file references
