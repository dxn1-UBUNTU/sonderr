...
import { enabled as sandboxed } from "@sonderr/sandbox"
...
export const Params = Schema.Struct({
  ...
  persistent: Schema.optional(Schema.Boolean).annotate({
    description: "Keep the process running and manageable after the session or Sonderr exits",
  }),
}).check(
  Schema.makeFilter(
    (params: {
  ...
  Effect.succeed({
    ...
    execute: (params, ctx) =>
      Effect.gen(function* () {
        ...
        const parent = params.inherit ? SonderrSession.resolveParent(ctx.sessionID) : undefined
