...
const log = Log.create({ service: "sonderr-task-model" })
...
export namespace SonderrTask {
   ...
   *
   * Sonderr removed parent-agent inheritance entirely in anomalyco/sonderr#31696.
   * Sonderr intentionally differs: parent edit/notebook/MCP denials remain hard ceilings
  ...
  const saved = Effect.fn("SonderrTask.savedModel")(function* (name: string) {
    if (Flag.SONDERR_CLIENT !== "cli") return undefined
  ...
  export const resolveModel = Effect.fn("SonderrTask.resolveModel")(function* (input: {
    name: string
