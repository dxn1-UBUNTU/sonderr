...
import { AgentManagerEvent, type AgentManagerTask } from "@/sonderr/agent-manager/event"
import { AgentManager, HostError } from "@/sonderr/agent-manager/service"
import type { Result } from "@/sonderr/agent-manager/protocol"
...
const WireParams = Schema.Struct({
  ...
  action: Schema.optional(
    Schema.NullOr(Schema.Literals(["list", "prompt", "stop", "move", "answer"])).annotate({
      description:
        "Use list first to discover IDs and assignments. Use move only after list, once per worktree. Never edit .sonderr/agent-manager.json for these operations. Send null when starting sessions with mode and tasks, otherwise the action is used instead of the start request.",
    }),
  ),
...
// Prefer the provider the user already uses for the invoking turn, then the Sonderr Gateway,
...
function rank(providerID: string, preferred: string | undefined): number {
  ...
  if (providerID === "sonderr") return 1
  return 2
}
    ...
    return {
      ...
      execute: (params, ctx) =>
        Effect.gen(function* () {
          ...
          const msg = SonderrSessionMessageOrder.latest(ctx.messages).user
