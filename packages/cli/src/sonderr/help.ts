...
async function getHelpText(name: string, cmd: Cmd): Promise<string> {
  const inst = yargs([])
    .scriptName(name ? `sonderr ${name}` : "sonderr")
    .wrap(null)
  if (cmd.builder) {
...
async function getSubcommands(
  ...
  const inst = yargs([]).scriptName(`sonderr ${name}`).wrap(null)
  builder(inst)
  ...
  try {
    ...
    for (const [sub, handler] of Object.entries(handlers as Record<string, any>)) {
      ...
      const full = `${name} ${sub}`
      const subInst = yargs([]).scriptName(`sonderr ${full}`).wrap(null)
...
function formatMarkdown(
  ...
  for (const section of sections) {
    parts.push(`## ${section.name ? `sonderr ${section.name}` : "sonderr"}`)
    parts.push("")
    if (section.hidden) {
...
function formatText(
  ...
  for (const section of sections) {
    parts.push(rule)
    const display = section.name ? `sonderr ${section.name}` : "sonderr"
    ...
    for (const sub of section.subs) {
      const sublabel = sub.hidden ? `--- sonderr ${sub.name} [internal] ---` : `--- sonderr ${sub.name} ---`
      parts.push(sublabel)
      parts.push("")
      parts.push(sub.help)
      parts.push("")
...
export async function generateCommandTable(options?: { commands?: Cmd[] }) {
  ...
  for (const cmd of cmds) {
    ...
    const display = raw.startsWith("$0") ? "sonderr" + raw.slice(2) : "sonderr " + raw
