...
import { ConfigPermissionV1 as ConfigPermission } from "@sonderr/core/v1/config/permission"
import { SonderrPaths } from "./paths"
import type { OrganizationMode } from "@sonderr/sonderr-gateway"

export namespace ModesMigrator {
  // Sonderr mode structure
  export interface SonderrMode {
    slug: string
    name: string
  ...
  export interface SonderrModesFile {
    customModes: SonderrMode[]
  }
  ...
  export function convertPermissions(groups: SonderrMode["groups"]): ConfigPermission.Info {
    ...
    // This is critical because Sonderr defaults to "ask" for missing permissions
  ...
  export function convertMode(mode: SonderrMode): ConfigAgentV1.Info {
  ...
  export async function readModesFile(filepath: string): Promise<SonderrMode[]> {
    try {
      const content = await fs.readFile(filepath, "utf-8")
      // Wrap YAML content in frontmatter delimiters so gray-matter can parse it
      const wrapped = `---\n${content}\n---`
      const parsed = matter(wrapped).data as SonderrModesFile
  ...
  export async function migrate(options: {
    ...
    const allModes: SonderrMode[] = []

    if (!options.skipGlobalPaths) {
      // 1. VSCode extension global storage (primary location for global modes)
      const vscodeGlobalPath = path.join(SonderrPaths.vscodeGlobalStorage(), "settings", "custom_modes.yaml")
      allModes.push(...(await readModesFile(vscodeGlobalPath)))

      // 2. CLI global settings (fallback/alternative location)
      const cliGlobalPath = path.join(os.homedir(), ".sonderr", "cli", "global", "settings", "custom_modes.yaml")
      allModes.push(...(await readModesFile(cliGlobalPath)))

      // 3. Home directory .sonderrmodes
      const homeModesPath = path.join(os.homedir(), ".sonderrmodes")
    ...
    // 5. Project .sonderrmodes
    const projectModesPath = path.join(options.projectDir, ".sonderrmodes")
    allModes.push(...(await readModesFile(projectModesPath)))

    // Deduplicate by slug (later entries win)
    const modesBySlug = new Map<string, SonderrMode>()
    ...
    for (const [slug, mode] of modesBySlug) {
      // Skip default modes - let Sonderr's native agents handle these
      if (isDefaultMode(slug)) {
        result.skipped.push({
          slug,
          reason: "Default mode - using Sonderr native agent instead",
        })
        continue
      }
