...
import { ConfigErrorV1, FrontmatterError } from "@sonderr/core/v1/config/error"
...
export namespace ConfigValidation {
  ...
  function isConfig(filepath: string): boolean {
    ...
    // Global config dirs (e.g. ~/.config/sonderr/)
    if (ConfigProtection.isAbsolute(filepath)) return true
    // Project-local config (e.g. /project/.sonderr/command/foo.md)
    try {
