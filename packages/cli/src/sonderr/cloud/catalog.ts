import {
  ...
  getDefaultHeaders,
  getSonderrUrlFromToken,
  resolveSonderrOpenRouterBaseUrl,
...
export namespace CloudCatalog {
  ...
  export class Service extends Context.Service<Service, Interface>()("@sonderr/CloudCatalog") {}
  ...
  export const layer = (options: Options = {}) => {
    ...
    const request = Effect.fn("CloudCatalog.request")(function* <A>(url: string, input: Input, schema: z.ZodType<A>) {
        ...
        try: () =>
          new Request(url, {
            headers: {
              ...
              ...buildSonderrHeaders(
                undefined,
                input.organizationID ? { sonderrOrganizationId: input.organizationID } : undefined,
              ),
              "X-SONDERR-FEATURE": "sonderr-cli",
          ...
          new CatalogError({
            kind: "schema",
            message: "Sonderr catalog URL is invalid",
          }),
      })
      const response = yield* Effect.tryPromise({
        try: () => fetcher(req),
          ...
          new CatalogError({
            kind: "network",
            message: "Unable to reach the Sonderr model catalog",
          }),
      })
      if (!response.ok) {
        ...
        return yield* Effect.fail(
          new CatalogError({
            ...
            message:
              kind === "auth"
                ? "Sonderr credentials or organization were rejected by the model catalog"
                : "The Sonderr model catalog is unavailable",
          }),
        )
      }
      ...
      const body = yield* Effect.tryPromise({
        try: () => readBoundedJson(response),
          ...
          new CatalogError({
            kind: "schema",
            message: "The Sonderr model catalog returned an invalid response",
          }),
      })
    ...
    const base = Effect.fn("CloudCatalog.base")(function* (input: Input) {
      const raw = env.SONDERR_API_URL?.trim()
      const fallback = raw || DEFAULT_SONDERR_API_URL
      const value = getSonderrUrlFromToken(fallback, Redacted.value(input.token))
      return yield* Effect.try({
        try: () => {
          const url = new URL(resolveSonderrOpenRouterBaseUrl({ baseURL: value }))
          ...
          new CatalogError({
            kind: "schema",
            message: "Sonderr catalog URL must be secure",
          }),
      })
    })
