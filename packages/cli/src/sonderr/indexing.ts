...
import { type IndexingTelemetryEvent, type VectorStoreSearchResult } from "@sonderr/sonderr-indexing/engine"
import { toIndexingConfigInput, type IndexingConfig } from "@sonderr/sonderr-indexing/config"
import { hasIndexingPlugin } from "@sonderr/sonderr-indexing/detect"
import { IndexingStatus, disabledIndexingStatus } from "@sonderr/sonderr-indexing/status"
import { Telemetry } from "@sonderr/sonderr-telemetry"
import { fetchSonderrEmbeddingModelCatalog } from "@sonderr/sonderr-gateway"
import { allowed, message } from "@sonderr/core/sonderr/fff"
...
import { indexingWithSonderrDefault, resolveSonderrIndexingAuth, type SonderrIndexingAuth } from "./indexing-auth"
import { primaryWorktree } from "./primary-worktree"

const log = Log.create({ service: "sonderr-indexing" })
...
async function sonderrAuth(cfg: Config.Info): Promise<SonderrIndexingAuth> {
  const info = await auth.runPromise((svc) => svc.get("sonderr"))
  return resolveSonderrIndexingAuth({ config: cfg, auth: info })
}

function enrichSonderr(input: ReturnType<typeof toIndexingConfigInput>, auth: SonderrIndexingAuth) {
  if (input.embedderProvider !== "sonderr") return input

  return {
    ...input,
    sonderrApiKey: input.sonderrApiKey ?? auth.apiKey,
    sonderrBaseUrl: input.sonderrBaseUrl ?? auth.baseUrl,
    sonderrOrganizationId: input.sonderrOrganizationId ?? auth.organizationId,
  }
}

async function model(input: ReturnType<typeof toIndexingConfigInput>, auth: SonderrIndexingAuth) {
  if (input.embedderProvider !== "sonderr" || !input.enabled) return input

  const catalog = await fetchSonderrEmbeddingModelCatalog({ baseURL: auth.baseUrl, token: auth.apiKey })
  ...
  if (!found) {
    if (input.modelId || input.modelDimension) {
      log.warn("ignoring unsupported Sonderr embedding model configuration", { model: input.modelId })
    }
...
export namespace SonderrIndexing {
  ...
  const boot = async (hit: Cache): Promise<Entry> => {
    ...
    if (process.env["SONDERR_DISABLE_CODEBASE_INDEXING"] === "vscode-no-workspace") {
    ...
    if (process.env["SONDERR_PLATFORM"] === "vscode" && !consent.get(project)) {
  ...
  export async function setConsent(enabled: boolean) {
    if (process.env["SONDERR_PLATFORM"] !== "vscode") return
    const dir = Instance.directory
    const project = (await AppRuntime.runPromise(primaryWorktree(dir))) ?? dir
  ...
  export async function models() {
    try {
      ...
      const fallback = await fetchSonderrEmbeddingModelCatalog()
      return fallback.models.length > 0 ? fallback : catalog
    } catch (err) {
      log.warn("falling back to public Sonderr embedding model catalog", { err })
      return fetchSonderrEmbeddingModelCatalog()
    }
  }
