...
export namespace RulesMigrator {
  // Only support .sonderrrules (no migration for .roorules or .clinerules)
  const LEGACY_RULE_FILE = ".sonderrrules"
  const home = () => process.env.SONDERR_TEST_HOME || process.env.HOME || process.env.USERPROFILE || os.homedir()

  // Directory-based rules (read from both .sonderr and .sonderr)
  const SONDERR_RULES_DIRS = [".sonderr/rules", ".sonderr/rules"]
  const globalRulesDirs = () => [path.join(home(), ".sonderr", "rules"), path.join(home(), ".sonderr", "rules")]

  // Known modes for mode-specific rule discovery
  const KNOWN_MODES = ["code", "architect", "ask", "debug", "orchestrator", "hive"]
  ...
  export async function discoverRules(projectDir: string): Promise<RuleFile[]> {
    ...
    // 1. Global rules directories (~/.sonderr/rules/*.md and ~/.sonderr/rules/*.md)
    ...
    // 2. Project .sonderr/rules/ and .sonderr/rules/ directories
    const seen = new Set<string>()
    for (const rulesRel of SONDERR_RULES_DIRS) {
    ...
    // 3. Legacy .sonderrrules file (only sonderr, not roo/cline)
    ...
    for (const mode of KNOWN_MODES) {
      // Mode-specific directories (.sonderr/rules-{mode}/*.md and .sonderr/rules-{mode}/*.md)
      const modeSeen = new Set<string>()
      for (const prefix of [".sonderr", ".sonderr"]) {
      ...
      // Legacy mode-specific file (.sonderrrules-{mode})
      const legacyModeFile = path.join(projectDir, `.sonderrrules-${mode}`)
  ...
  export async function migrate(options: {
    ...
    for (const rule of rules) {
      ...
      if (rule.source === "legacy") {
        warnings.push(
          `Legacy rule file '${path.basename(rule.path)}' found. Consider migrating to .sonderr/rules/ directory.`,
        )
      }
    }
