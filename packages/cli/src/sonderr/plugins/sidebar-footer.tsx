...
const id = "internal:sonderr-sidebar-footer"
...
export function passLine(pass: SonderrPassState) {
...
// A billed turn: a completed Sonderr assistant message with a non-zero cost.
export function billable(info: Message) {
  if (info.role !== "assistant") return false
  return info.providerID === "sonderr" && info.time.completed !== undefined && info.cost > 0
}
...
function View(props: { api: TuiPluginApi }) {
  ...
  const has = createMemo(() =>
    props.api.state.provider.some(
      (item) =>
        (item.id !== "sonderr" && item.id !== "sonderr") ||
    ...
    void props.api.client.sonderr
      ...
      .then((res) => {
        ...
        const next: State = {
          ...
          pass: res.data.sonderrPass ?? null,
        }
        setState(next)
              ...
              {(pass) => (
                <box gap={0}>
                  <box flexDirection="row" justifyContent="space-between" gap={1}>
                    <text fg={theme().textMuted}>{" └ Sonderr Pass"}</text>
