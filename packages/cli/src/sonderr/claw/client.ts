...
/**
 * SonderrClaw Sonderr Chat client wrapper for the TUI.
 *
 * Hosts the HTTP and WebSocket clients, exposes a multi-conversation API
 * (list/create/select/rename/delete), and translates Sonderr Chat events for
...
import { SonderrChatClient } from "./sonderr-chat-client"
...
export async function connect(input: ConnectInput): Promise<ClawChatClient> {
  ...
  const chat = new SonderrChatClient({
    baseUrl: input.envelope.sonderrChatUrl,
    getToken: async () => input.envelope.token,
  })
  ...
  const sandboxCtx = `/sonderrclaw/${input.sandboxId}`
  ...
  function setActive(id: string | null): void {
    ...
    clearTyping()
    if (id) {
      activeCtx = `/sonderrclaw/${input.sandboxId}/${id}`
