...
import type { Daemon } from "@/sonderr/daemon/daemon"
...
// deferral pattern as upstream sonderr#30453).
...
function print(input: Daemon.Status, json?: boolean) {
  ...
  if (!input.running) {
    console.log(input.stale ? `sonderr daemon stale: ${input.reason}` : `sonderr daemon not running`)
    console.log(`state: ${input.file}`)
    if (input.state?.log) console.log(`log: ${input.state.log}`)
    return
  }
  console.log(`sonderr daemon running`)
  if (input.state?.urls) {
...
async function hold(enabled: boolean, json: boolean, run: (signal?: AbortSignal) => Promise<Daemon.State>) {
  ...
  const { Daemon } = await import("@/sonderr/daemon/daemon")
  await Daemon.foreground(async (signal) => {
    const state = await run(signal)
    if (!signal.aborted && !json) console.log("Press Ctrl+C to stop the Sonderr daemon.")
    return state
  })
}

async function network(args: { [key: string]: unknown }) {
  const { warnedNetworkOptions } = await import("@/sonderr/cli/port-warning")
...
function start(command: string) {
  return cmd({
    command,
    describe: "start the local sonderr daemon",
    ...
    handler: async (args) => {
      await hold(Boolean(args.foreground), Boolean(args.json), async (signal) => {
        ...
        if (!state) throw new Error("Sonderr daemon did not provide process state")
        if (signal?.aborted) return state
        if (args.json) print(result, true)
        if (!args.json) {
          console.log(
            result.reused
              ? "sonderr daemon already running"
              : daemon.restarted
                ? "sonderr daemon restarted"
                : "sonderr daemon started",
          )
          print(result)
        }
        return state
...
const StatusCommand = cmd({
  command: "status",
  describe: "show local sonderr daemon status",
...
export const StopCommand = cmd({
  command: "stop",
  describe: "stop the local sonderr daemon",
  builder: (yargs) => withJson(yargs),
  handler: async (args) => {
    const { Daemon } = await import("@/sonderr/daemon/daemon")
    ...
    console.log(result.stopped ? "sonderr daemon stopped" : "sonderr daemon not running")
  },
})

const RestartCommand = cmd({
  command: "restart",
  describe: "restart the local sonderr daemon",
  builder: (yargs) => withForeground(withJson(withNetworkOptions(yargs))),
  handler: async (args) => {
    await hold(Boolean(args.foreground), Boolean(args.json), async (signal) => {
      const opts = await network(args)
      ...
      if (args.json) print(result, true)
      if (!args.json) {
        console.log("sonderr daemon restarted")
        print(result)
      }
      return state
    })
