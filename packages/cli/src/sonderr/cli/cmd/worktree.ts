...
// `sonderr worktree list`/`remove`: CLI-side counterpart to `sonderr --worktree <name>`
...
import { slugify } from "@/sonderr/cli/cmd/tui-worktree"
...
// Lazy: this module is imported eagerly by SonderrCli.register (see setup.ts), so
...
export const WorktreeCreateCommand = cmd({
  ...
  async handler(args) {
    ...
    // instance context (it's shared with `sonderr --worktree`'s pre-TUI-launch
    // path in tui-worktree.ts), so it can't run inside effectCmd's own.
    const { resolveWorktree } = await import("@/sonderr/cli/cmd/tui-worktree")
    ...
    // do e.g. `cd "$(sonderr worktree create foo)"`.
...
export const WorktreeListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.worktree.list")(function* () {
    ...
    // Data rows go to stdout (like `sonderr session list`'s table/JSON), not
    // UI.println's stderr, so `sonderr worktree list | ...` actually captures them.
