...
// Supports `sonderr --worktree <name>` (create/reuse a git worktree before the TUI
// starts, placed at `.sonderr/worktrees/<name>` to match Agent Manager's own
...
const log = Log.create({ service: "sonderr.tui-worktree" })

// Matches packages/sonderr-vscode/src/agent-manager/WorktreeManager.ts's placement
// and its ensureGitExclude(), keeping `.sonderr/worktrees/` out of `git status`.
const SONDERR_WORKTREE_DIR = ".sonderr/worktrees"
...
export async function ensureGitExclude(root: string) {
  ...
  if (current.includes(`${SONDERR_WORKTREE_DIR}/`)) return
  const prefix = current ? `${current}\n\n` : ""
  await Filesystem.write(excludePath, `${prefix}# Sonderr agent worktrees\n${SONDERR_WORKTREE_DIR}/\n`).catch((err) =>
...
// Exported for `sonderr worktree create` (worktree.ts), which calls this and
...
export async function resolveWorktree(name: string, root: string, timeoutMs = 10 * 60_000) {
  ...
  const { primaryWorktree } = await import("@/sonderr/primary-worktree")
  ...
  return withInstance(root, async (run) => {
    ...
    // it unconditionally would nest `.sonderr/worktrees/<name>` inside another
    ...
    const directory = path.join(ctx.worktree, SONDERR_WORKTREE_DIR, slug)
...
async function resolveSessionWorktree(sessionID: string, fallback: string) {
  try {
    ...
    const { SessionTable } = await import("@sonderr/core/session/sql")
