...
/**
 * Sonderr-specific TUI app customizations.
 *
...
import { isSonderrError, showSonderrErrorToast } from "@/sonderr/sonderr-errors"
import { registerSonderrCommands } from "@/sonderr/sonderr-commands"
import { initializeTUIDependencies } from "@sonderr/sonderr-gateway/tui"
import { DialogProcessList } from "@/sonderr/cli/cmd/tui/component/dialog-process-list"
import { useIndexingWarnings } from "@/sonderr/cli/cmd/tui/indexing-warning"
import { SonderrTerminalTitle } from "./terminal-title"
import type { SonderrTitleIcon } from "./title-icon"
import { Session as SessionApi } from "@/session/session"

// Re-export so upstream can render the route without importing directly
export { SonderrClawView } from "@/sonderr/claw/view"
export { SonderrTerminalTitle } from "./terminal-title"

// Hot reload TUI-local settings (keybinds/theme/ui) when changed from the Sonderr Console.
// Called from the App body (below SDKProvider and the TuiConfig provider).
export { useTuiConfigHotReload } from "@/sonderr/cli/cmd/tui/context/tui-config-hot-reload"
export { SonderrTuiConfig } from "@/sonderr/cli/cmd/tui/context/tui-config"
...
export const APP_TITLE = "Sonderr CLI"
...
export function useSessionEffects(deps: {
  ...
  const pty = process.env.SONDERR_PTY_ID
...
export function getTerminalTitle(input: {
  ...
  icon?: SonderrTitleIcon.Value
}): SonderrTerminalTitle.Result | undefined {
  if (input.route.data.type === "home") {
    return {
      title: SonderrTerminalTitle.format({ base: input.base, indicator: "none", icon: input.icon }),
      active: false,
      indicator: "none",
    }
  }
  ...
  if (input.route.data.type === "session") {
    const state = SonderrTerminalTitle.session({
      base: input.base,
    ...
    return {
      ...state,
      title: SonderrTerminalTitle.format({ base: input.base, title, indicator: state.indicator, icon: input.icon }),
    }
  }

  if (input.route.data.type === "plugin") {
    return {
      title: SonderrTerminalTitle.format({
        base: input.base,
  ...
  if (input.route.data.type === "sonderrclaw") {
    return {
      title: SonderrTerminalTitle.format({ base: input.base, title: "SonderrClaw", indicator: "none", icon: input.icon }),
      active: false,
      indicator: "none",
    }
  }
...
/**
 * Intercepts Sonderr-specific errors and shows a warning toast.
 * Returns `true` if the error was handled, `false` otherwise.
 */
export function handleSessionError(error: unknown, toast: ReturnType<typeof useToast>): boolean {
  if (error && typeof error === "object" && isSonderrError(error as any)) {
    showSonderrErrorToast(error as any, toast)
    return true
  }
  return false
}
 ...
 *
 * - Injects TUI dependencies into sonderr-gateway
 * - Registers Sonderr Gateway commands (profile, teams, sonderrclaw, etc.)
...
export function init() {
  ...
  // Inject TUI dependencies for sonderr-gateway
  ...
  // Register Sonderr Gateway commands (profile, teams, sonderrclaw, remote, etc.)
  registerSonderrCommands(useSDK)
  ...
  useBindings(() => ({
    commands: [
      ...
      },
      {
        ...
        // sonderr_change - the saved rule is server side, so it also stops VS Code, JetBrains and
