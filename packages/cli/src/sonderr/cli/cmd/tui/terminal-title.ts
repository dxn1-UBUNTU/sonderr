import { SonderrTitleIcon } from "./title-icon"
...
export namespace SonderrTerminalTitle {
  ...
  } satisfies Record<SonderrTitleIcon.Value, Record<Indicator, string>>

  export function format(input: { base: string; title?: string; indicator: Indicator; icon?: SonderrTitleIcon.Value }) {
    const text = input.title ? `${input.base} | ${truncate(input.title)}` : input.base
    const prefix = icons[input.icon ?? SonderrTitleIcon.Default][input.indicator]
