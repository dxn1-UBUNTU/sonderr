 ...
 * (`sdk.tui.config.get`) and reconcile it into the store via `SonderrTuiConfig.useSet`. The
...
import { SonderrTuiConfig } from "./tui-config"
...
export function useTuiConfigHotReload() {
  const set = SonderrTuiConfig.useSet()
  const sdk = useSDK()
