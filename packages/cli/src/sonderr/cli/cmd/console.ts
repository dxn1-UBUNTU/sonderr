...
import { serverUrls } from "@/sonderr/cli/server-urls"
import { hasDisplay } from "@/sonderr/cli/cmd/tui/util/display"
import { StopCommand } from "@/sonderr/cli/cmd/daemon"
...
const OpenCommand = cmd({
  command: "$0",
  describe: "open the local Sonderr Console (deprecated)",
  builder: (yargs) =>
  ...
  handler: async (args) => {
    console.warn("Sonderr Console is deprecated and will be removed in a future release.")
    ...
    const run = async (signal?: AbortSignal) => {
      ...
      if (!state) throw new Error("Sonderr daemon did not provide connection state")
      if (signal?.aborted) return state
      if (daemon.restarted) console.warn("Restarted the Sonderr daemon to apply the requested network options")
      ...
      if (hasDisplay()) {
        ...
        console.warn("No display detected; open the Sonderr Console URL manually")
      }
      console.log("Sonderr Console:")
    ...
    await Daemon.foreground(async (signal) => {
      ...
      if (!signal.aborted) console.log("Press Ctrl+C to stop the Sonderr daemon.")
      return state
    })
  },
})

export const SonderrConsoleCommand = cmd({
  command: "console",
