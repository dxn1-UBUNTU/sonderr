...
const log = Log.create({ service: "sonderr.cloud-session" })
 ...
 * Wraps the SDK's `.sonderr.cloud.session.import()` which returns `unknown` due to
...
export async function importCloudSession(
  ...
  const result = await client.sonderr.cloud.session.import({ sessionId })
