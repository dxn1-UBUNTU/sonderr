...
import type { IndexingConfig, Config } from "@sonderr/sdk/v2"
import * as Log from "@sonderr/core/util/log"
import { hasSonderrIndexingAuth, resolveSonderrIndexingAuth, shouldDefaultIndexingToSonderr } from "../indexing-auth"
import {
...
function hasSonderrAuth(sync: ReturnType<typeof useSync>, scope: IndexingScope, indexing: IndexingConfig): boolean {
  const provider = sync.data.provider_next.all.find((item) => item.id === "sonderr")
  const config = indexingScopeConfig(scope, sync.data.config, sync.data.globalConfig, indexing)
  return hasSonderrIndexingAuth({ config, provider })
}
...
function defaultIndexing(
  ...
  const provider = sync.data.provider_next.all.find((item) => item.id === "sonderr")
  const config = indexingScopeConfig(scope, sync.data.config, sync.data.globalConfig, indexing)
  const auth = resolveSonderrIndexingAuth({ config, provider })
  if (!shouldDefaultIndexingToSonderr({ ...global, ...indexing }, auth)) return indexing
  return { ...indexing, provider: "sonderr", model: null, dimension: null }
}
...
function ProviderSelect(props: SubDialogProps) {
  ...
  const options: DialogSelectOption<EmbeddingProvider>[] = (
    ...
    .filter(([value]) => value !== "sonderr" || hasSonderrAuth(sync, props.scope, indexing) || indexing.provider === "sonderr")
    .map(([value, title]) => ({
      value,
      title,
...
function SonderrModelSelect(props: SubDialogProps) {
  const dialog = useDialog()
  const sync = useSync()
  const sdk = props.useSDK()
  const toast = useToast()
  ...
  const [catalog] = createResource(() => loadSonderrEmbeddingModels(setError))
  ...
  createEffect(() => {
    ...
    seen.state = state
    log.info("Sonderr embedding model resource changed", {
      state: catalog.state,
      models: cfg?.models.length ?? 0,
      current: currentSonderrModel(cfg, indexing.model),
      defaultModel: cfg?.defaultModel || undefined,
      scope: props.scope,
    })
  })
  const options = createMemo(() => sonderrModelOptions(catalog()))
  const current = createMemo(() => currentSonderrModel(catalog(), indexing.model))

  return (
    <DialogSelect
      title="Sonderr Embedding Model"
      options={options()}
      current={current()}
      renderFilter={(catalog()?.models.length ?? 0) > 0}
      onSelect={async (option) => {
        if (!option.value || !catalog()?.models.some((model) => model.id === option.value)) return
        log.info("selected Sonderr embedding model", { model: option.value, scope: props.scope })
...
function ScopeSelect(props: DialogIndexingProps & { scope: IndexingScope }) {
  ...
  const options: DialogSelectOption<IndexingScope>[] = [
    ...
    { value: "project", title: "Project", description: "Stored in this repo's .sonderr config" },
  ]
  ...
  const options = createMemo<DialogSelectOption<string>[]>(() => {
    ...
    const result: DialogSelectOption<string>[] = [
      ...
      },
      {
        ...
        category: "Embedding",
        description: mark(
          indexing.provider === "sonderr" ? (indexing.model ?? "Sonderr catalog") : (indexing.model ?? "default"),
          [["model"]],
        ),
      },
      {
      ...
      onSelect={async (option) => {
        ...
        switch (option.value) {
          ...
          case "providerSettings":
            ...
            break
          case "model": {
            if (indexing.provider === "sonderr") {
              dialog.replace(() => (
                <SonderrModelSelect useSDK={props.useSDK} scope={scope()} indexing={indexing} raw={raw} />
              ))
              break
            }
          ...
          case "dimension": {
            if (indexing.provider === "sonderr") break
