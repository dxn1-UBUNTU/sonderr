...
/**
 * Custom OAuth handler for Sonderr Gateway
 *
...
import { DialogSonderrOrganization } from "./dialog-sonderr-organization.js"
...
interface SonderrAutoMethodProps {
  index: number
  providerID: string
  title: string
...
export function SonderrAutoMethod(props: SonderrAutoMethodProps) {
  ...
  onMount(async () => {
    try {
      ...
      const profileResponse = await sdk.client.sonderr.profile()
      ...
      console.warn("Failed to fetch Sonderr profile, using personal account:", error)
      setStatus("error")
