import { fetchSonderrEmbeddingModelCatalog, resolveSonderrGatewayBaseUrl } from "@sonderr/sonderr-gateway"
import type { Config, IndexingConfig, SonderrEmbeddingModelCatalog } from "@sonderr/sdk/v2"
...
export async function loadSonderrEmbeddingModels(onError?: (message: string) => void) {
  const endpoint = new URL("embedding-models", resolveSonderrGatewayBaseUrl()).toString()
  log.info("loading Sonderr embedding model catalog", { endpoint })
  const catalog = await fetchSonderrEmbeddingModelCatalog({
    onError: (issue) => {
      log.warn("failed to load Sonderr embedding model catalog", {
        code: issue.code,
        status: issue.status,
  ...
  log.info("loaded Sonderr embedding model catalog", {
...
export function sonderrModelOptions(catalog?: SonderrEmbeddingModelCatalog) {
...
export function currentSonderrModel(catalog: SonderrEmbeddingModelCatalog | undefined, model?: string | null) {
