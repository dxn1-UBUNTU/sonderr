...
import { Bus } from "@/bus"
import { Instance, type InstanceContext } from "@/sonderr/instance"
...
import { ENV_FEATURE } from "@sonderr/sonderr-gateway"
import { existsSync } from "fs"
import path from "path"
import { iife } from "@/util/iife"
import { SonderrSessionEvent, type SonderrSessionCloseReason } from "./event"

export namespace SonderrSession {
  const log = Log.create({ service: "session.sonderr" })

  // ---------------------------------------------------------------------------
  // Events
  // ---------------------------------------------------------------------------

  export const Event = SonderrSessionEvent
  export type CloseReason = SonderrSessionCloseReason
  ...
  // Emitted by SonderrSessionPromptQueue on every transition that changes the set
  ...
  // context (e.g. SonderrSessionPromptQueue transitions, which fire from inside
  ...
  export function attribution(id: string): { rootID: string; feature?: string } {
    ...
    const platform = resolvePlatform(rootID) ?? process.env["SONDERR_PLATFORM"]
  ...
  // ---------------------------------------------------------------------------
  // Provider-reported cost (Sonderr / OpenRouter / Vercel AI Gateway)
  // ---------------------------------------------------------------------------
   ...
   *                                      (`costDetails.upstreamInferenceCost` for Sonderr)
   ...
   *
   * Sonderr does not charge end users a per-request fee, so for the Sonderr provider the
  ...
  export function providerCost(input: {
    ...
    const isSonderr = (input.provider?.id ?? input.providerID) === "sonderr"
    ...
    if (orUsage) {
      ...
      // Sonderr doesn't charge a fee on top of the upstream inference cost, so for Sonderr
      ...
      const cost = isSonderr && upstream !== undefined ? upstream : regular
      if (cost !== undefined) return cost
    }

    // 2. Anthropic Messages or OpenAI Responses via OpenRouter. The Sonderr Gateway wrapper
    //    restores the verbatim usage payload under the AI SDK's raw usage escape hatch.
    //    Sonderr doesn't charge end users a per-request fee, so only upstream cost is relevant.
    ...
    //    gateway fee that Sonderr would pass through, but Sonderr doesn't charge end users a
  ...
  export function shareSession(id: SessionID) {
    return EffectBridge.fromPromise(async () => {
      ...
      return SonderrSessions.share(id)
  ...
  export function unshareSession(id: SessionID) {
    return EffectBridge.fromPromise(async () => {
      const { SonderrSessions } = await import("@/sonderr-sessions/sonderr-sessions")
      await SonderrSessions.unshare(id)
    }).pipe(Effect.catchCause((cause) => Effect.fail(Cause.squash(cause))))
  }

  export async function removeSession(id: string): Promise<void> {
    const { SonderrSessions } = await import("@/sonderr-sessions/sonderr-sessions")
    await SonderrSessions.remove(id).catch(() => {})
  }
    ...
    return Effect.gen(function* () {
      ...
      const worktree = (dir: string) => {
        for (const root of sorted) {
          ...
          if ((parts[0] === ".sonderr" || parts[0] === ".sonderr") && parts[1] === "worktrees" && parts[2]) {
...
export { sonderrSessionFork } from "./fork-command"
