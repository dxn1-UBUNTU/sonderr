...
import type { ConfigPermissionV1 as ConfigPermission } from "@sonderr/core/v1/config/permission"

export namespace IgnoreMigrator {
  const log = Log.create({ service: "sonderr.ignore-migrator" })

  const SONDERRIGNORE_FILE = ".sonderrignore"
  const GLOBAL_SONDERRIGNORE = path.join(os.homedir(), ".sonderr", SONDERRIGNORE_FILE)
  ...
  /**
   * Parse .sonderrignore content into patterns.
  ...
  /**
   * Convert gitignore pattern to Sonderr wildcard pattern.
   *
   * Sonderr's Wildcard module uses simple patterns:
   ...
   *
   * Note: Sonderr's wildcard `*` already matches any path depth because
  ...
  export function convertToGlob(pattern: string): string {
    ...
    // Remove **/ prefix if present - Sonderr's * already matches paths
    if (glob.startsWith("**/")) {
      glob = "*" + glob.slice(3)
    }
  ...
  /**
   * Load patterns from a .sonderrignore file
   */
  ...
  /**
   * Migrate .sonderrignore to Sonderr permission config
   */
  export async function migrate(options: { projectDir: string; skipGlobalPaths?: boolean }): Promise<MigrationResult> {
    ...
    // 1. Load global .sonderrignore (lower priority)
    if (!options.skipGlobalPaths) {
      const globalPatterns = await loadIgnoreFile(GLOBAL_SONDERRIGNORE, "global")
      allPatterns.push(...globalPatterns)

      if (globalPatterns.length > 0) {
        log.debug("loaded global .sonderrignore", { count: globalPatterns.length })
      }
    }

    // 2. Load project .sonderrignore (higher priority - added last)
    const projectIgnorePath = path.join(options.projectDir, SONDERRIGNORE_FILE)
    const projectPatterns = await loadIgnoreFile(projectIgnorePath, "project")
    allPatterns.push(...projectPatterns)

    if (projectPatterns.length > 0) {
      log.debug("loaded project .sonderrignore", { count: projectPatterns.length })
    }
  ...
  /**
   * Load .sonderrignore and return permission config.
  ...
  export async function loadIgnoreConfig(
    ...
    try {
      ...
      if (result.patternCount > 0) {
        log.info("loaded .sonderrignore patterns", {
      ...
      log.warn("failed to load .sonderrignore", { error: err })
      return {}
    }
  }
}
