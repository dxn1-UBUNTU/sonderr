...
const log = Log.create({ service: "sonderr-watcher" })

export namespace SonderrWatcher {
  export interface Interface {
    readonly init: () => Effect.Effect<void>
  }

  export class Service extends Context.Service<Service, Interface>()("@sonderr/Watcher") {}
  ...
  export function eager(client = Flag.SONDERR_CLIENT) {
  ...
  export const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      return Service.of({
        init: Effect.fn("SonderrWatcher.init")(function* () {
  ...
  export const defaultLayer = Layer.unwrap(
    Effect.gen(function* () {
      if (!eager() || (yield* Flag.SONDERR_EXPERIMENTAL_DISABLE_FILEWATCHER.pipe(Effect.orElseSucceed(() => false))))
