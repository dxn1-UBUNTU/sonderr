...
import { model as modelEnv } from "@/sonderr/process/env"
import { SessionID } from "@/session/schema"
import { Shell } from "@sonderr/core/shell"
import { ProjectV2 } from "@sonderr/core/project"
import { Process } from "@/util/process"
import { NonNegativeInt, PositiveInt, optionalOmitUndefined, withStatics } from "@sonderr/core/schema"
import { zod, ZodOverride } from "@sonderr/core/effect-zod"
...
export namespace BackgroundProcess {
  ...
  class StateService extends Context.Service<StateService, { readonly get: () => Effect.Effect<State> }>()(
    "@sonderr/BackgroundProcess.State",
  ) {}
  ...
  async function claim(shared: Shared) {
    ...
    const pending = Flock.acquire(shared.key, {
      ...
      .catch((err) => {
        log.debug("persistent process scope is managed by another Sonderr process", { err, scope: shared.key })
        return false
      })
      .finally(() => {
  ...
  function infer() {
    return Flag.SONDERR_CLIENT === "cli" && process.env.SONDERR_BACKGROUND_PROCESS_PORTS === "true"
  }
  ...
  function env(id?: ID, token?: string) {
    const result: NodeJS.ProcessEnv = modelEnv({
      TERM: "dumb",
      ...(id ? { SONDERR_BACKGROUND_PROCESS_ID: id } : {}),
      ...(token ? { SONDERR_BACKGROUND_PROCESS_TOKEN: token } : {}),
    })
    delete result.SONDERR_BACKGROUND_PROCESS_PORTS
    return result
  }
  ...
  async function linux(active: Active): Promise<Probe> {
    ...
    if (leader && pgrp(leader) === pid) {
      ...
      if (data?.toString("utf8").split("\0").includes(`SONDERR_BACKGROUND_PROCESS_TOKEN=${token}`)) return "owned"
    }
  ...
  async function launch(state: State, input: StartInput, id = ID.ascending()) {
    ...
    if (logpath) {
      ...
      if (!(await claim(state.shared)))
        throw new Error("Persistent processes for this project are managed by another Sonderr process")
  ...
  SonderrShutdown.register(() => runtime.dispose())
