...
import { SonderrConfigOverlay } from "@/sonderr/config/overlay"
import { SonderrConfigWriter } from "@/sonderr/config/writer"
import { SonderrConfigSources } from "@/sonderr/config/sources"
import { SonderrModelState } from "@/sonderr/config/model-state"
import { ConfigRules } from "@/sonderr/server/routes/config-rules"
import { SonderrKeybinds } from "@/sonderr/tui/keybinds"
import { SonderrTuiConfig } from "@/sonderr/tui/config"
...
export const configConsoleHandlers = HttpApiBuilder.group(InstanceHttpApi, "config-console", (handlers) =>
  Effect.gen(function* () {
    ...
    const overlayUpdate = Effect.fn("ConfigConsoleHttpApi.overlayUpdate")(function* (ctx: {
      ...
      const patch = SonderrConfigOverlay.patch(body)
    ...
    const modelState = Effect.fn("ConfigConsoleHttpApi.modelState")(function* () {
      return yield* Effect.promise(() => SonderrModelState.get())
    })
    ...
    const modelStateUpdate = Effect.fn("ConfigConsoleHttpApi.modelStateUpdate")(function* (ctx: {
      ...
      return yield* Effect.promise(() =>
        SonderrModelState.update({ favorite: ctx.payload.favorite?.map((item) => ({ ...item })) }),
      )
    })

    const tuiConfigGet = Effect.fn("ConfigConsoleHttpApi.tuiConfigGet")(function* () {
      const instance = yield* InstanceState.context
      return yield* Effect.promise(() => SonderrTuiConfig.get({ directory: instance.directory }))
    })

    const tuiKeybindList = Effect.fn("ConfigConsoleHttpApi.tuiKeybindList")(function* () {
      return { keybinds: SonderrKeybinds.list() }
    })
