...
export function matchLegacySonderrOpenApi(input: Record<string, unknown>) {
  rebrand(input)
  ...
  const body = spec.paths?.["/sonderr/organization"]?.post?.requestBody?.content?.["application/json"]?.schema
  ...
  const profile = json("/sonderr/profile")?.schema?.properties
  const pass = profile?.sonderrPass?.properties
  ...
  if (profile?.sonderrPass) profile.sonderrPass = nullable(profile.sonderrPass)
  if (profile?.currentOrgId) profile.currentOrgId = nullable(profile.currentOrgId)

  const sessions = json("/sonderr/cloud-sessions")?.schema?.properties
  ...
  const claw = json("/sonderr/claw/status")?.schema?.properties
  ...
  const credentials = json("/sonderr/claw/chat-credentials")
  ...
  const fim = spec.paths?.["/sonderr/fim"]?.post?.responses
  if (!fim) return
  fim["200"] = {
...
function rebrand(value: unknown): void {
  ...
  for (const [key, item] of Object.entries(value)) {
    ...
    ;(value as Record<string, unknown>)[key] = item
      .replaceAll("Sonderr", "Sonderr")
      .replaceAll("sonderr.local", "sonderr.local")
      .replaceAll("sonderr serve", "sonderr serve")
      .replaceAll("https://sonderr.ai/", "https://kilo.ai/")
  }
}
