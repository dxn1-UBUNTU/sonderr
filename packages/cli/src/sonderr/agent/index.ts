...
import { SonderrConfigSources } from "../config/sources"
...
// Exported so SonderrTask.inherited carries the same set into delegated sessions; a tool
...
function planEditRules(worktree: string) {
  return {
    "*": "deny" as const,
    [path.join(".sonderr", "plans", "*.md")]: "allow" as const,
...
export interface SonderrData {
  mcpRules: Record<string, "allow" | "ask" | "deny">
  defaultsPatch: Permission.Ruleset
}

// Prepare sonderr-specific data derived from config. Call once per state initialization.
export function prepare(cfg: Config.Info): SonderrData {
  const mcpRules = getMcpRules(cfg)
  const defaultsPatch = Permission.fromConfig({
    bash,
    recall: "ask",
    ...(Flag.SONDERR_CLIENT === "vscode" && cfg.experimental?.native_notebook_tools === true
      ? { notebook_read: "ask" as const, notebook_edit: "ask" as const, notebook_execute: "ask" as const }
      : {}),
    sonderr_memory_recall: "ask",
    sonderr_memory_save: "ask",
  })
...
// Lift Sonderr-internal metadata onto typed agent fields and remove it from `options`.
...
// Patch the base agents map in-place with all sonderr-specific changes:
// - Rename build → code
// - Patch plan with readOnlyBash, mcpRules, .sonderr paths
...
export function patchAgents(
  ...
  // Patch plan mode
  if (agents.plan) {
    const guard = planGuard(worktree, sonderr.mcpRules)
    agents.plan = {
      ...agents.plan,
  ...
  // Add ask agent
  const guard = askGuard(sonderr.mcpRules)
  agents.ask = {
    name: "ask",
 ...
 * config-backed agent entries, and/or removing it from legacy .sonderrmodes YAML files.
...
export async function remove(input: {
  ...
  let found = false
  const result = await SonderrConfigSources.list({ directory: input.directory, worktree: input.worktree })
  ...
  // 2. Remove from legacy .sonderrmodes YAML files (read by ModesMigrator)
  const { ModesMigrator } = await import("@/sonderr/modes-migrator")
  const { SonderrPaths } = await import("@/sonderr/paths")
  ...
  const legacy = [
    {
      ...
      file: path.join(SonderrPaths.vscodeGlobalStorage(), "settings", "custom_modes.yaml"),
    },
    {
      scope: "global" as const,
      file: path.join(home, ".sonderr", "cli", "global", "settings", "custom_modes.yaml"),
    },
    { scope: "global" as const, file: path.join(home, ".sonderrmodes") },
    { scope: "project" as const, file: path.join(input.directory, ".sonderrmodes") },
  ]
