...
async function headers(args: HeaderArgs): Promise<Headers> {
  ...
  const out = new Headers({
    ...
    "x-sonderr-export-api-version": "1",
    "x-sonderr-export-schema-version": "1",
    "x-sonderr-export-agent-version": args.agentVersion,
    "x-sonderr-export-surface": args.surface,
    "x-sonderr-export-root-session-id": first.rootSessionId,
    "x-sonderr-export-session-id": first.sessionId,
    "x-sonderr-export-batch-id": args.batchId,
    "x-sonderr-export-seq-start": String(Math.min(...seqs)),
    "x-sonderr-export-seq-end": String(Math.max(...seqs)),
    "x-sonderr-export-event-count": String(args.rows.length),
    "x-sonderr-export-payload-sha256": await sha256Hex(args.body),
    "x-sonderr-export-client-sent-at": new Date().toISOString(),
    "x-sonderr-export-content-encoding": "identity",
  })
  const token = process.env.SONDERR_SESSION_EXPORT_AUTH_TOKEN
  if (token) out.set("authorization", `Bearer ${token}`)
  const anon = args.anonId ?? (await anonId(args.anonIdPath))
  if (!token && anon) out.set("x-sonderr-anon-id", anon)
  return out
}
