...
declare global {
  const SONDERR_SESSION_EXPORT_WORKER_PATH: string
}
...
function target(): WorkerTarget {
  if (typeof SONDERR_SESSION_EXPORT_WORKER_PATH !== "undefined") return SONDERR_SESSION_EXPORT_WORKER_PATH
...
function currentSurface(): string {
  return process.env.SONDERR_FEATURE?.trim() || "unknown"
}
