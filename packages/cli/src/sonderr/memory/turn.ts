...
import { MemoryTurn as TurnCore } from "@sonderr/sonderr-memory/effect/turn"
...
import { SonderrSessionPrompt } from "@/sonderr/session/prompt"
...
/** Host turn-open/turn-close hooks: adapt sonderr's session/provider services into the package
...
export namespace MemoryLifecycle {
  export const subscribe = Effect.fn("MemoryLifecycle.subscribe")(function* (input: {
    ...
    yield* input.bus.subscribeCallback(SonderrSession.Event.TurnOpen, (evt) =>
      bridge.fork(
    ...
    yield* input.bus.subscribeCallback(SonderrSession.Event.TurnClose, (evt) =>
      bridge.fork(
        Effect.gen(function* () {
          const ctx = yield* InstanceState.context
          const enabled = yield* SonderrSessionPrompt.memoryToolEnabled({ ctx })
          if (!enabled) return
