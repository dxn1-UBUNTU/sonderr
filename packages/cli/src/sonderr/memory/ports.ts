...
import { MemoryConfig } from "@sonderr/sonderr-memory/effect/config"
import { MemoryError } from "@sonderr/sonderr-memory/effect/errors"
import type { MemoryPorts } from "@sonderr/sonderr-memory/effect/ports"
import { MemoryRedact } from "@sonderr/sonderr-memory/redact"
import { MemoryShared } from "@sonderr/sonderr-memory/shared"
...
function recalledMemory(turn: Turn) {
  return [turn.user, ...turn.assistants].flatMap((item) => item.parts).some((part) => {
    if (part.type === "tool") {
      return (
        part.tool === "sonderr_memory_recall" &&
    ...
    const marker = (part.metadata as { sonderrMemory?: { type?: string; count?: number } } | undefined)?.sonderrMemory
...
/** Host SessionPort: extracts a TurnView from sonderr's message store + snapshot diffs so the
...
/** Host ModelPort: resolves the consolidation model through sonderr's provider and runs it via the
