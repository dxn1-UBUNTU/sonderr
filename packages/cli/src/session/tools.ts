...
import { MemoryMarker } from "@/sonderr/memory/marker" // sonderr_change
...
export const resolve = Effect.fn("SessionTools.resolve")(function* (input: {
  ...
  processor: Pick<SessionProcessor.Handle, "message" | "metadata" | "completeToolCall"> // sonderr_change
  bypassAgentCheck: boolean
  messages: SessionV1.WithParts[]
  promptOps: TaskPromptOps
  memoryCache: MemoryMarker.Cache // sonderr_change
}) {
  ...
  // sonderr_change start - permission provenance
  const config = yield* Config.Service
  const cfg = yield* config.get()
  const permissionOrigins = cfg.permission_origins
  // sonderr_change end
  const flags = yield* RuntimeFlags.Service
  const restricted = yield* SandboxPolicy.networkRestricted(input.session.id) // sonderr_change
  const sandboxed = (yield* SandboxPolicy.status(input.session.id)).enabled // sonderr_change
  const context = (args: Record<string, unknown>, options: ToolExecutionOptions): Tool.Context => {
    const extra = {
      model: input.model,
      bypassAgentCheck: input.bypassAgentCheck,
      promptOps: input.promptOps,
      sandboxed, // sonderr_change
  ...
  for (const item of yield* registry.tools({
    ...
    family: input.model.family, // sonderr_change
    agent: input.agent,
    permission: input.session.permission,
    networkRestricted: restricted, // sonderr_change - let the registry suppress code-mode in restricted sessions
  })) {
      ...
      execute(args, options) {
        return run.promise(
          Effect.gen(function* () {
            ...
            // sonderr_change - mark successful targeted memory recalls for the assistant badge
            if (item.id === "sonderr_memory_recall") MemoryMarker.recall({ result: output, cache: input.memoryCache }) // sonderr_change
            yield* plugin.trigger(
              "tool.execute.after",
  ...
  const mcpTools = restricted ? {} : yield* mcp.tools() // sonderr_change
    ...
    item.execute = (args, opts) =>
      run.promise(
        Effect.gen(function* () {
          ...
          // sonderr_change start - propagate MCP App UI metadata so hosts can preload the UI resource
          ...
          const result: Awaited<ReturnType<NonNullable<typeof execute>>> = yield* SandboxPolicy.executeMcp(
            ctx.sessionID,
            entry, // sonderr_change - retain the native entry's local/remote network authority marker
            Effect.gen(function* () {
          ...
          const metadata = {
            ...
            truncated: truncated.truncated,
            ...(truncated.truncated && { outputPath: truncated.outputPath }),
            ...mcpAppMeta, // sonderr_change - MCP App UI metadata
          }
