...
import { SonderrCompactionPayloadRecovery } from "@/sonderr/session/compaction-payload-recovery"
import { SonderrCompactionChunks } from "@/sonderr/session/compaction-chunks"
...
import { buildPrompt } from "@sonderr/core/session/compaction"
import { SessionCompactionEvent } from "@sonderr/schema/session-compaction-event"
...
// sonderr_change start - allow safe pruning at cache-invalidating boundaries
...
export interface Interface {
  ...
  readonly prune: (input: { sessionID: SessionID; reason?: PruneReason }) => Effect.Effect<void> // sonderr_change
...
export class Service extends Context.Service<Service, Interface>()("@sonderr/SessionCompaction") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // sonderr_change start - preserve normal opt-in pruning, but allow payload/compaction cleanup by default
    ...
    const processCompaction = Effect.fn("SessionCompaction.process")(function* (input: {
      ...
      let replay:
        ...
        | undefined
      // sonderr_change start - false is preflight replay; undefined disables replay
      ...
      const tokens = Token.estimate(JSON.stringify(modelMessages)) // sonderr_change
      ...
      // sonderr_change start
      const result = SonderrCompactionChunks.needed({ cfg, model, tokens, outputTokenMax: flags.outputTokenMax })
        ? "compact"
        : yield* SonderrCompactionPayloadRecovery.process({
            processor,
            user: userMessage,
            agent,
            sessionID: input.sessionID,
          ...
          }).pipe(Effect.provideService(Database.Service, database)) // sonderr_change

      const fallback = SonderrCompactionChunks.eligible({
        result,
        error: processor.message.error ?? processor.compactError?.(),
      })
        ? yield* SonderrCompactionChunks.process({
            processors,
            session,
            user: userMessage,
            agent,
      ...
      if (fallback === "continue" && input.auto) {
        // sonderr_change end
        if (replay) {
          // sonderr_change start - compact oversized replay turns instead of looping into replay overflow
          replay = yield* SonderrCompactionChunks.replay({
            processors,
            session,
            user: userMessage,
            agent,
          ...
          const replayMsg = yield* session.updateMessage({
            ...
            editorContext: original.editorContext, // sonderr_change
          })
          SonderrSessionPromptQueue.retarget(input.sessionID, replayMsg.id) // sonderr_change - expose replay to scope()
          for (const part of replay.parts) {
            if (part.type === "compaction") continue
            // sonderr_change start - preserve media for preflight replay but strip it after provider overflow
            const replayPart =
        ...
        if (!replay) {
          ...
          if (
            ...
            SonderrSessionPromptQueue.retarget(input.sessionID, continueMsg.id) // sonderr_change - expose auto-continue to scope()
            const text =
              (input.overflow
      ...
      // sonderr_change start - compaction already invalidates cache, so collapse stale tool outputs too
        ...
        if (flags.experimentalEventSystem) {
          if (summary)
            yield* events.publish(SessionEvent.Compaction.Ended, {
              ...
              text: summary ?? "",
              recent,
              include: recent, // sonderr_change - released Core V2 readers recognize this field
            })
        }
        // sonderr_change start - export self-contained compaction capture
        const parent = SonderrSession.resolveParent(input.sessionID)
    ...
    const create = Effect.fn("SessionCompaction.create")(function* (input: {
      ...
      // sonderr_change start - keep auto-compaction markers visible during queued turns
      SonderrSessionPromptQueue.retarget(input.sessionID, msg.id)
      // sonderr_change end
      if (flags.experimentalEventSystem) {
    ...
    return Service.of({
      isOverflow,
      prune,
      process: (input) => processCompaction(input).pipe(Effect.orDie), // sonderr_change
      create,
    })
  }),
)
