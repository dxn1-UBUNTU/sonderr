...
import { isSonderrError } from "@/sonderr/sonderr-errors" // sonderr_change
...
export type RetryReason = string & {} // sonderr_change - Sonderr does not support Sonderr Go upsell reasons
...
// sonderr_change - Sonderr does not emit Sonderr Go actions
export function retryable(error: Err, _provider?: string): Retryable | undefined {
  // context overflow errors should not be retried
  if (SessionV1.ContextOverflowError.isInstance(error)) return undefined
  if (SessionV1.APIError.isInstance(error)) {
    const status = error.data.statusCode
    // sonderr_change start - Current Sonderr errors require user action (login/signup), don't retry
    if (isSonderrError(error)) return undefined
    // sonderr_change end
    ...
    // sonderr_change start - Sonderr does not support Sonderr Go upsells. FreeUsageLimitError is not retryable: retrying
...
export function policy(opts: {
  ...
  return Schedule.fromStepWithMetadata(
    Effect.succeed((meta: Schedule.InputMetadata<unknown>) => {
      // sonderr_change start — enforce retry limit
      ...
      return Effect.gen(function* () {
        // sonderr_change start — handle network disconnect via offline handler
