...
import fs from "node:fs" // sonderr_change
...
import { SonderrSession } from "@/sonderr/session" // sonderr_change
import { SessionTranscript } from "@/sonderr/session/transcript" // sonderr_change
import { SonderrCostPropagation } from "@/sonderr/session/cost-propagation" // sonderr_change
import { SonderrSessionProcessor } from "@/sonderr/session/processor" // sonderr_change
import * as SonderrWorkflowVariant from "@/sonderr/session/workflow-variant" // sonderr_change
import { SonderrSessionOverflow } from "@/sonderr/session/overflow" // sonderr_change
import { SonderrReference } from "@/sonderr/reference/contains" // sonderr_change
import { SonderrReadObject } from "@/sonderr/tool/read-object" // sonderr_change
import { isInterrupted } from "@/sonderr/effect/cause" // sonderr_change
import * as SandboxPolicy from "@/sonderr/sandbox/policy" // sonderr_change
import { CommandTimeout } from "@/sonderr/command-timeout" // sonderr_change
import { Suggestion } from "@/sonderr/suggestion" // sonderr_change
import { Question } from "@/question" // sonderr_change
import { BUILTIN_COMMANDS } from "@/sonderr/session/builtin-commands" // sonderr_change
import { legacyReviewMessage } from "@/sonderr/review/command" // sonderr_change
...
import { MAX_STEPS_PROMPT } from "@sonderr/core/session/runner/max-steps"
...
import * as DateTime from "effect/DateTime" // sonderr_change
import { SessionEvent } from "@sonderr/core/session/event" // sonderr_change
import { SessionMessage } from "@sonderr/core/session/message" // sonderr_change
import { InstanceState } from "@/effect/instance-state"
import { InstanceRef } from "@/effect/instance-ref"
import { Instance } from "@/sonderr/instance"
import { EffectBridge } from "@/effect/bridge"
import { TaskTool, type TaskPromptOps } from "@/tool/task"
import { assertExternalDirectoryEffect } from "@/tool/external-directory" // sonderr_change
...
import * as SonderrConfiguredReference from "@/sonderr/reference" // sonderr_change
...
import { SonderrSessionContinuation } from "@/sonderr/session/continuation" // sonderr_change
...
export const shouldAskPlanFollowup = SonderrSessionPrompt.shouldAskPlanFollowup // sonderr_change - retain Sonderr plan handoff policy

// sonderr_change start - persistent tool-output pruning when payload is already large
...
export class Service extends Context.Service<Service, Interface>()("@sonderr/SessionPrompt") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const question = yield* Question.Service // sonderr_change - dismiss superseded pending questions through the shared service
    ...
    const cache = Option.getOrUndefined(yield* Effect.serviceOption(RepositoryCache.Service)) // sonderr_change
    const { db } = database
    ...
    const cancel = Effect.fn("SessionPrompt.cancel")(function* (sessionID: SessionID) {
      ...
      yield* SonderrSessionPrompt.cancelTree({ sessionID, sessions, cancel: state.cancel }) // sonderr_change - stop queued work and subagents
    })

    // sonderr_change start - preserve configured reference mentions on the Core reference architecture
    const resolveReferenceParts = Effect.fnUntraced(function* (template: string, skip = new Set<string>()) {
      const ctx = yield* InstanceState.context
      const cfg = yield* config.get()
      const refs = SonderrConfiguredReference.resolveAll({
      ...
      for (const match of ConfigMarkdown.files(template)) {
        ...
        if (reference.kind === "git" && cache) yield* SonderrConfiguredReference.ensure(cache, reference) // sonderr_change
    ...
    const resolvePromptParts = Effect.fn("SessionPrompt.resolvePromptParts")(function* (template: string) {
      const ctx = yield* InstanceState.context
      const roots = yield* resolveReferenceParts(template) // sonderr_change
      ...
      yield* Effect.forEach(
        ...
        Effect.fnUntraced(function* (match) {
          const name = match[1]
          if (!name) return
          // sonderr_change start - configured references were already added above
          const alias = name.split("/")[0]
    ...
    const title = Effect.fn("SessionPrompt.ensureTitle")(function* (input: {
      ...
      const msgs = onlySubtasks
        ...
        : yield* MessageV2.toModelMessagesEffect(context, mdl).pipe(
            Effect.provideService(Database.Service, database), // sonderr_change - provide the migrated message store
          )
      const text = yield* llm
        .stream({
          ...
          tools: {},
          model: mdl,
          sessionID: SonderrSessionPrompt.titleID(input.session.id), // sonderr_change - isolate title requests from the agent task
          retries: 2,
      ...
      // sonderr_change start - auto-title mark/re-check owned by SonderrSessionPrompt
      const fresh = yield* sessions.get(input.session.id).pipe(Effect.orElseSucceed(() => null))
      if (
        !SonderrSessionPrompt.prepareAutoTitle({
      ...
      yield* sessions.setTitle({ sessionID: input.session.id, title: t }).pipe(
        Effect.catchCause((cause) => {
          SonderrSessionPrompt.clearAutoTitleMark(input.session.id, t)
    ...
    const handleSubtask = Effect.fn("SessionPrompt.handleSubtask")(function* (input: {
      ...
      const taskVariant = task.variant ?? lastUser.model.variant // sonderr_change
      const assistantMessage: SessionV1.Assistant = yield* sessions.updateMessage({
        ...
        mode: task.agent,
        agent: task.agent,
        variant: taskVariant, // sonderr_change
      ...
      // sonderr_change start - distinguish explicit workflow model selection from the effective subtask model
      ...
      // sonderr_change start - shared reader for the child session id written by task.ts ctx.metadata (#6321)
      const childID = () => {
      ...
      const result = yield* taskTool
        .execute(taskArgs, {
          ...
          // sonderr_change start - resolve permissions at ask time so active tools see config edits
          ask: (req: any) =>
            SonderrSessionPrompt.askPermission({
              permission,
              agents,
              sessions,
              agent: taskAgent,
        ...
        .pipe(
          ...
          Effect.onInterrupt(() =>
            Effect.gen(function* () {
              ...
              // sonderr_change start - propagate partial subagent cost on cancel (#6321)
              const cid = childID()
              if (cid) {
                assistantMessage.cost = yield* SonderrCostPropagation.childCost(sessions, SessionID.make(cid))
              }
              // sonderr_change end
      ...
      assistantMessage.finish = "tool-calls"
      assistantMessage.time.completed = Date.now()
      // sonderr_change start - include subagent total cost on the wrapper message (#6321)
      ...
      const summaryUserMsg: SessionV1.User = {
        ...
        editorContext: lastUser.editorContext, // sonderr_change — preserve editor context
      }
      ...
      return yield* Effect.uninterruptibleMask((restore) =>
        Effect.gen(function* () {
          ...
          const { msg, part, cwd } = yield* Effect.gen(function* () {
            ...
            const callID = ulid() // sonderr_change - correlate v2 shell events with the persisted tool part
            ...
            // sonderr_change start - dual-write the v2 shell record so the durable timeline correlates with the tool part
          ...
          let output = ""
          let aborted = false
          let timeout: string | undefined // sonderr_change
    ...
    const getModel = Effect.fn("SessionPrompt.getModel")(function* (
      ...
      if (isInterrupted(exit.cause)) return yield* Effect.interrupt // sonderr_change
      const err = Cause.squash(exit.cause)
      if (Provider.ModelNotFoundError.isInstance(err)) {
        const hint = err.suggestions?.length ? ` Did you mean: ${err.suggestions.join(", ")}?` : ""
        const empty = err.modelsEmpty ? " No models are currently available." : "" // sonderr_change
        yield* events.publish(Session.Event.Error, {
          sessionID,
          error: new NamedError.Unknown({
            message: `Model not found: ${err.providerID}/${err.modelID}.${hint}${empty}`, // sonderr_change
          }).toObject(),
        })
      }
    ...
    const createUserMessage = Effect.fn("SessionPrompt.createUserMessage")(function* (input: PromptInput) {
      const agentName = input.agent ?? (yield* sessions.get(input.sessionID).pipe(Effect.orDie)).agent // sonderr_change
      ...
      // sonderr_change start - retain the source session variant across Agent Manager's model-less fork handoff
      ...
      const info: SessionV1.User = {
        ...
        sessionID: input.sessionID,
        time: { created: Date.now() },
        tools: { ...input.tools, ...input.ephemeralTools }, // sonderr_change - apply non-persistent remote tool restrictions
        agent: ag.name,
        model: {
          providerID: model.providerID,
          modelID: model.modelID,
        ...
        system: input.system,
        format: input.format,
        editorContext: input.editorContext, // sonderr_change
      }
      ...
      const ctx = yield* InstanceState.context // sonderr_change - resolve V1 reference roots for attachment authorization
      const references = SonderrConfiguredReference.resolveAll({
      ...
      const referenceContextFromFilePart = Effect.fnUntraced(function* (
        ...
        return { root: reference.path } // sonderr_change - carry the Core reference root for authorization
      })
        ...
        if (part.type === "file") {
          ...
          switch (url.protocol) {
            case "data:":
              ...
              // sonderr_change start - normalize user image data before persistence
              ...
              // sonderr_change end
              break
            // sonderr_change start - resolve @-mentioned past chats into transcript context
            case "session:":
            ...
            case "file:": {
              ...
              // sonderr_change start - authorize prompt attachments like model-issued read calls
              const controller = new AbortController()
              const ask: Tool.Context["ask"] = (request) =>
                Effect.gen(function* () {
                  const session = yield* sessions.get(input.sessionID)
                  yield* SonderrSessionPrompt.askPermission({
                    permission,
                    agents,
                    sessions,
                    agent: ag,
              ...
              const execRead = (args: Parameters<typeof read.execute>[0], extra?: Tool.Context["extra"]) => {
                return read
                  .execute(args, ctx(extra)) // sonderr_change - enforce read and external_directory permissions
              ...
              // sonderr_change start - authorize metadata, then reopen and verify before consuming bytes
              const access = yield* Effect.gen(function* () {
                const file = yield* SonderrReadObject.file(filepath)
                const instance = yield* InstanceState.context
                const context = ctx()
                const explicit = reference ? yield* SonderrReference.path(fsys, reference.root, file.target) : false
                const referenced =
                  explicit || (yield* SonderrReference.contains({ fs: fsys, references, target: file.target }))
                ...
                return yield* SonderrReadObject.use(file, (bound) =>
                  Effect.gen(function* () {
              ...
              return [
                ...
                access.value, // sonderr_change - retain the attachment read through the authorized object
              ]
            }
          }
        }
      ...
      // sonderr_change start - expand references from direct prompt text, not only command templates
      ...
      // sonderr_change - sonderr normalizes images inside resolvePart, so there is no separate normalization pass here (unlike upstream)
    ...
    const prompt: Interface["prompt"] = Effect.fn("SessionPrompt.prompt")(
      function* (input: PromptInput) {
        const session = yield* sessions.get(input.sessionID).pipe(Effect.orDie)
        yield* revert.cleanup(session)
        // sonderr_change start - recover interrupted Sonderr turns before accepting a follow-up
        yield* SonderrSessionPrompt.recoverDanglingAssistant({ sessionID: input.sessionID, status, sessions })
        yield* SonderrSessionPrompt.recoverProviderFinishError({ sessionID: input.sessionID, status, sessions })
        yield* SonderrSessionPrompt.recoverFailedAssistant({ sessionID: input.sessionID, status, sessions })
        // sonderr_change end
        const message = yield* SonderrSessionPrompt.intake(input.sessionID, createUserMessage(input)) // sonderr_change
        ...
        if (permissions.length > 0) {
          // sonderr_change start - preserve inherited task restrictions while refreshing prompt tool toggles
          const merged = SonderrSessionPrompt.mergeToolPermissions({
        ...
        // sonderr_change start — register the queued follow-up before dismissing blockers.
        ...
        return yield* SonderrSessionPromptQueue.enqueue(
          input.sessionID,
          message.info.id,
          bridge.run(
    ...
    const lastAssistant = Effect.fnUntraced(function* (sessionID: SessionID) {
      // sonderr_change start - retry when cancel races before shellImpl writes messages
    ...
    const runLoop: (input: LoopInput) => Effect.Effect<MessageV2.WithParts, NotFoundError> = Effect.fn(
      ...
      // sonderr_change end
      // sonderr_change — cache environment details per turn (prompt caching)
      const envCache: SonderrSessionPrompt.EnvCache = {}
      const memoryCache = SonderrSessionPrompt.memoryCache() // sonderr_change
      closeReasons.delete(sessionID) // sonderr_change
      let compactionAttempts = 0 // sonderr_change - cap compaction attempts per turn to avoid infinite loops
      const ctx = yield* InstanceState.context
      ...
      while (true) {
        ...
        // sonderr_change start - provide the upstream Effect database to Sonderr's retained prompt loop
        let msgs = yield* MessageV2.filterCompactedEffect(sessionID).pipe(
          Effect.provideService(Database.Service, database),
        )
        // sonderr_change end
        msgs = SonderrSessionPromptQueue.scope(sessionID, msgs) // sonderr_change - hide later queued prompts
        msgs = SonderrSessionPrompt.trimBeforeLastSummary(msgs) // sonderr_change - trim on any completed summary (e.g. manual /compact against a text user)

        // sonderr_change start - select loop state by chronology after retained-tail projection
        const latest = SonderrSessionMessageOrder.latest(msgs)
        const { user: lastUser, assistant: lastAssistant, finished: lastFinished, tasks } = latest
        // sonderr_change end

        if (input.resume && step === 0 && SonderrSessionContinuation.target(msgs) !== input.resume) break // sonderr_change
        if (!lastUser) throw new Error("No user message found in stream. This should never happen.")

        const lastAssistantMsg = msgs.findLast(
          (msg) => msg.info.role === "assistant" && msg.info.id === lastAssistant?.id,
        )
        // sonderr_change start - compare chronology, not generated IDs
        const userBeforeAssistant =
          latest.userMessage &&
          latest.assistantMessage &&
          SonderrSessionMessageOrder.compare(latest.userMessage, latest.assistantMessage) < 0
        // sonderr_change end
        // sonderr_change start - carry local review command marker into LLM telemetry
        const telemetry =
          SonderrSessionProcessor.extractReviewTelemetry(
            msgs.findLast((m) => m.info.role === "user" && m.info.id === lastUser.id)?.parts ?? [],
          ) ?? SonderrSessionProcessor.extractSuggestionReviewTelemetry(lastAssistantMsg?.parts ?? [])
        // sonderr_change end
        ...
        if (
          ...
          userBeforeAssistant &&
          SonderrSessionPrompt.shouldAskPlanFollowup({ messages: msgs, abort: AbortSignal.any([]) })
        ) {
          const action = yield* Effect.promise((signal) =>
            SonderrSessionPrompt.askPlanFollowup({ sessionID, messages: msgs, abort: signal, question }),
          )
        ...
        if (
          ...
          lastAssistant.id !== input.resume && // sonderr_change
          !hasToolCalls &&
          lastAssistant.parentID === lastUser.id && // sonderr_change - unrelated later assistants do not answer this turn
          userBeforeAssistant // sonderr_change - compare chronology, not generated IDs
        ) {
        ...
        if (task?.type === "compaction") {
          const result = yield* compaction.process({
            messages: msgs,
            parentID: task.messageID, // sonderr_change
            sessionID,
            auto: task.auto,
            overflow: task.overflow,
          })
          // sonderr_change start - compaction.process only returns "stop" after
        ...
        if (
          ...
          // sonderr_change start
          const guard = SonderrSessionPrompt.guardCompactionAttempt({
            sessionID,
          ...
          compactionAttempts++
          // sonderr_change end
          yield* compaction.create({ sessionID, agent: lastUser.agent, model: lastUser.model, auto: true, overflow: false }) // sonderr_change
          continue
        }
        ...
        const handle = yield* processor
          .create({
            ...
            sessionID,
            model,
            telemetry, // sonderr_change
            snapshotInitialization: input.snapshotInitialization, // sonderr_change
          })
        ...
        const outcome: "break" | "continue" = yield* Effect.gen(function* () {
          ...
          const tools = yield* SessionTools.resolve({
            ...
            messages: msgs,
            promptOps,
            memoryCache, // sonderr_change
          }).pipe(
            Effect.provideService(Plugin.Service, plugin),
            Effect.provideService(Permission.Service, permission),
            Effect.provideService(Agent.Service, agents), // sonderr_change
            Effect.provideService(Session.Service, sessions), // sonderr_change
            Effect.provideService(ToolRegistry.Service, registry),
            Effect.provideService(MCP.Service, mcp),
            Effect.provideService(Truncate.Service, truncate),
            // sonderr_change start - provide services used by session tool resolution
            Effect.provideService(Config.Service, config),
          ...
          // sonderr_change start — ephemeral context injection + post-summary
          ...
          SonderrSessionPrompt.injectEditorContext({ msgs, session, sessionID, cache: envCache })
          msgs = SonderrSessionPrompt.maybeStripHistoricalMedia(msgs)
          // sonderr_change end
          ...
          if (size > REQUEST_PRUNE_BYTES) {
            ...
            msgs = SonderrSessionPromptQueue.scope(sessionID, msgs)
            msgs = SonderrSessionPrompt.trimBeforeLastSummary(msgs)
            yield* plugin.trigger("experimental.chat.messages.transform", {}, { messages: msgs })
            SonderrSessionPrompt.injectEditorContext({ msgs, session, sessionID, cache: envCache })
            msgs = SonderrSessionPrompt.maybeStripHistoricalMedia(msgs)
          ...
          const result = yield* handle.process({
            // sonderr_change start - keep Ask/Plan tool filtering hardened against session allows
            user: lastUser,
            agent,
            permission: SonderrSessionPrompt.guardPermissions({ agent, session }),
            // sonderr_change end
            sessionID,
            ...
            messages: [
              ...modelMsgs,
              ...SonderrSessionContinuation.context(!!input.resume && step === 1), // sonderr_change
              ...(isLastStep ? [{ role: "user" as const, content: MAX_STEPS_PROMPT }] : []), // sonderr_change - avoid provider-incompatible assistant prefill
            ],
            tools,
            model,
            toolChoice: format.type === "json_schema" ? "required" : undefined,
            // sonderr_change start - feed the provider-reported context size from the last finished
            // turn into the output-token cap, so image/vision input is measured by the provider
            // rather than by encoded payload bytes (see SonderrLLM.capOutputTokens). Summary messages
            ...
            reportedContextTokens:
              lastFinished && lastFinished.summary !== true
                ? SonderrSessionOverflow.count(lastFinished.tokens)
                : undefined,
            // sonderr_change end
          })

          // sonderr_change start - persist a lightweight marker when this assistant step had memory context
          const marker = SonderrSessionPrompt.memoryPart({ sessionID, message: handle.message, cache: memoryCache })
          ...
          if (finished && !handle.message.error) {
            if (handle.message.finish === "content-filter") {
              ...
              closeReasons.set(sessionID, "error") // sonderr_change - retain Sonderr close-reason propagation
              return "break" as const
            }
            ...
            if (handle.message.finish === "error") {
              SonderrSessionProcessor.providerFinishError(handle.message)
              yield* sessions.updateMessage(handle.message)
              closeReasons.set(sessionID, "error")
              return "break" as const
            }
          ...
          // sonderr_change start — break out so a newer queued prompt can take over
          ...
          if (SonderrSessionPromptQueue.hasFollowup(sessionID)) {
            closeReasons.set(sessionID, "superseded")
            return "break" as const
          }
          // sonderr_change end
          // sonderr_change start - guard against providers that end the stream
    ...
    const loop: (input: LoopInput) => Effect.Effect<MessageV2.WithParts, NotFoundError> = Effect.fn(
      ...
      yield* SonderrSession.publishTurnOpen({ sessionID: input.sessionID })
      return yield* Effect.onExit(
        ...
        Effect.fnUntraced(function* (exit) {
          yield* SonderrSession.publishTurnClose({
            sessionID: input.sessionID,
            parentID: session.parentID,
            reason: SonderrSessionPrompt.resolveCloseReason({
              sessionID: input.sessionID,
              closeReasons,
              exit,
            }),
    ...
    // sonderr_change start - resume command handler
    ...
    const handleResume = Effect.fn("SessionPrompt.handleResume")(function* (input: {
      ...
      // sonderr_change start - test-only resume roots via Context.Service
      ...
      if (msgs.length > 0) {
        const error = new NamedError.Unknown({
          message: "Start a new Sonderr session, then run the resume command again.",
        })
    ...
    const command = Effect.fn("SessionPrompt.command")(function* (input: CommandInput) {
      ...
      if (!cmd) {
        ...
        available.push(...BUILTIN_COMMANDS) // sonderr_change - surface built-in session commands in error hint
        available.sort() // sonderr_change - alphabetical for stable, easy-to-scan output
      ...
      // sonderr_change start - resume commands import external transcripts
      const fmt = isResumeCommand(input.command)
      if (fmt) {
        return yield* handleResume({ cmdInput: input, format: fmt })
      }
      // sonderr_change end
      // sonderr_change start - deprecated review aliases should display a static notice without an LLM turn
      ...
      if (legacy) {
        ...
        const user = yield* SonderrSessionPrompt.intake(
          input.sessionID,
          createUserMessage({
            sessionID: input.sessionID,
            messageID: input.messageID,
      ...
      // sonderr_change start - skill templates run !`cmd`` only when trusted and the kill-switch is off,
      ...
      const task = yield* getModel(taskModel.providerID, taskModel.modelID, input.sessionID) // sonderr_change
      ...
      // sonderr_change start
      const variant = SonderrWorkflowVariant.resolve({
        command: cmd,
        agent,
        model: taskModel,
        selected: task,
      ...
      SonderrSessionProcessor.markReviewTelemetry(templateParts, input.command) // sonderr_change - mark review commands for completion telemetry
      ...
      const result = yield* prompt({
        ...
        agent: userAgent,
        parts,
        variant: isSubtask ? input.variant : variant, // sonderr_change
        snapshotInitialization: input.snapshotInitialization, // sonderr_change
      })
      yield* events.publish(Command.Event.Executed, {
        name: input.command,
...
export const PromptInput = Schema.Struct({
  ...
  // sonderr_change start - managed product slow-snapshot policy
  ...
  // sonderr_change end
  // sonderr_change start - reuse shared editor context schema
...
// sonderr_change start - retain precise prompt input types for Sonderr callers
...
export class LoopInput extends Schema.Class<LoopInput>("SessionPrompt.LoopInput")({
  sessionID: SessionID,
  resume: Schema.optional(MessageID), // sonderr_change
  snapshotInitialization: Schema.optional(Schema.Literal("wait")), // sonderr_change
}) {
...
const repositoryCacheNode = RepositoryCache.node // sonderr_change
