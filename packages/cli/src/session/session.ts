...
import { PartTable, SessionTable } from "@sonderr/core/session/sql"
...
import { SessionMessage } from "@sonderr/core/session/message" // sonderr_change - shared Revert.State brand

import type { Provider } from "@/provider/provider"
import { Permission } from "@/permission"
import { Global } from "@sonderr/core/global"
// sonderr_change start - Sonderr session behavior extensions
import { BackgroundProcess } from "@/sonderr/background-process"
import * as SandboxInheritance from "@/sonderr/sandbox/inheritance"
import { InteractiveTerminal } from "@/sonderr/interactive-terminal"
import { SonderrSession } from "@/sonderr/session"
import { sonderrSessionFork } from "@/sonderr/session/fork-command"
import { SonderrSessionEvent } from "@/sonderr/session/event"
import { SessionExport } from "@/sonderr/session-export"
import * as SandboxPolicy from "@/sonderr/sandbox/policy"
import { carryForkDiff } from "@/sonderr/session-portability/cumulative-diff" // sonderr_change
// sonderr_change end
import { Effect, Layer, Option, Context, Schema, Types } from "effect"
import { NonNegativeInt, optionalOmitUndefined } from "@sonderr/core/schema"
import { AbsolutePath } from "@sonderr/core/schema" // sonderr_change
...
export function fromRow(row: SessionRow): Info {
  ...
  // sonderr_change start - the shared column stores the upstream Revert.State brand; project it to the v1 shape
...
export function toRow(info: Info) {
  return {
    ...
    // sonderr_change - re-brand the v1 messageID to the shared Revert.State brand for the column
    revert: info.revert
...
const Summary = Schema.Struct({
  ...
  files: Schema.Finite,
  diffs: optionalOmitUndefined(Schema.Array(Snapshot.SummaryFileDiff)), // sonderr_change - lightweight diff without patch
})
...
const Revert = Schema.Struct({
  ...
  workspace: optionalOmitUndefined(Schema.Literals(["restored", "snapshots-disabled", "unavailable"])), // sonderr_change
})
...
export const GlobalInfo = Schema.Struct({
  ...
  worktreeName: Schema.optional(Schema.String), // sonderr_change - basename of the specific worktree directory
...
export const CreateInput = Schema.optional(
  Schema.Struct({
    ...
    platform: Schema.optional(Schema.String), // sonderr_change - per-session platform override for telemetry attribution
    // sonderr_change start - server-issued sandbox inheritance grant
...
export type GlobalListInput = {
  // sonderr_change start - worktree-family filters for the Agent Manager
  projectID?: string
  directory?: string
  directories?: string[]
...
export const Event = {
  ...
  TurnOpen: SonderrSessionEvent.TurnOpen,
  TurnClose: SonderrSessionEvent.TurnClose,
  // sonderr_change end
}

export function plan(input: { slug: string; time: { created: number } }, instance: InstanceContext) {
  const base = instance.project.vcs
    ? path.join(instance.worktree, ".sonderr", "plans") // sonderr_change
...
export const getUsage = (input: {
  ...
  provider?: Provider.Info // sonderr_change
}) => {
  ...
  // sonderr_change start - Use provider-reported cost when available for OpenRouter/Sonderr
  const reported = SonderrSession.providerCost({
...
export interface Interface {
  ...
  // sonderr_change start - session create metadata and sandbox inheritance extensions
  ...
  readonly create: (input?: {
    ...
    permission?: PermissionV1.Ruleset
    platform?: string // sonderr_change - per-session platform override for telemetry attribution
    workspaceID?: WorkspaceV2.ID
...
export class Service extends Context.Service<Service, Interface>()("@sonderr/Session") {}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // sonderr_change start - inherited sandbox policy source
    const createNext = Effect.fn("Session.createNext")(function* (input: {
      ...
      permission?: PermissionV1.Ruleset
      platform?: string // sonderr_change - per-session platform override for telemetry attribution
      sourceID?: SessionID // sonderr_change - inherited sandbox policy source
      sourceDirectory?: string
      sandboxFallback?: SandboxPolicy.Snapshot // sonderr_change - confinement to seed when source state lives in another directory
    }) {
      ...
      // sonderr_change start - legacy sessions must satisfy the upstream project foreign key
      yield* db
        .insert(ProjectTable)
        .values({
          id: ctx.project.id,
      ...
      // sonderr_change start - initialize inherited state before session.created subscribers run
      SonderrSession.register({ id: result.id, parentID: result.parentID, platform: input.platform })
    ...
    // sonderr_change start - preserve Sonderr's cross-project worktree-family filtering
    const listGlobal = Effect.fn("Session.listGlobal")((input?: GlobalListInput) =>
      SonderrSession.listGlobal<GlobalInfo>({ ...input, fromRow }).pipe(Effect.provideService(Database.Service, database)),
    )
    // sonderr_change end

    // sonderr_change start - scope children by persisted parent project_id
    ...
    const remove: Interface["remove"] = Effect.fnUntraced(function* (sessionID: SessionID) {
      ...
      try {
        ...
        yield* SandboxPolicy.dispose(
          sessionID,
          Effect.gen(function* () {
            yield* Effect.promise(() => SonderrSession.removeSession(sessionID)).pipe(Effect.ignore)
            SonderrSession.clearPlatformOverride(sessionID)
            if (hasInstance) {
            ...
            // sonderr_change - migrated from legacy sync.run/sync.remove to EventV2 (events.publish/remove)
            yield* events.publish(SessionV1.Event.Deleted, { sessionID, info: session })
            // sonderr_change - capture final session-export workspace delta on close/delete
            const workspaceKey = hasInstance ? yield* InstanceState.directory : undefined // sonderr_change
            yield* Effect.promise(() => SessionExport.onSessionClose(sessionID, workspaceKey)) // sonderr_change
    ...
    const updateMessage = <T extends SessionV1.Info>(msg: T): Effect.Effect<T> =>
      Effect.gen(function* () {
        // sonderr_change start - ignore FK errors when session was deleted while processor was still running
        yield* SonderrSession.runSyncSafe(
    ...
    const create = Effect.fn("Session.create")(function* (input?: {
      ...
      // sonderr_change end
      // sonderr_change start - propagate trusted sandbox inheritance grant
      const session = yield* createNext({
        ...
        platform: input?.platform, // sonderr_change
        sourceID: grant?.sessionID, // sonderr_change
        sourceDirectory: grant?.directory, // sonderr_change
    ...
    const fork = Effect.fn("Session.fork")(function* (input: { sessionID: SessionID; messageID?: MessageID }) {
      ...
      // sonderr_change start - forks into another directory cannot read the source confinement from the new dir, so carry it over explicitly
      const sandboxFallback = yield* SandboxPolicy.peek(original.directory, input.sessionID)
      // sonderr_change end
      // sonderr_change start - historical forks must use the model from retained context, not a later source-session selection
      ...
      const session = yield* createNext({
        ...
        model, // sonderr_change - preserve the model + variant active at the fork point
        sourceID: input.sessionID, // sonderr_change - forks preserve initialized confinement
        sandboxFallback, // sonderr_change - seed confinement from the source session's original directory
        platform: SonderrSession.resolvePlatform(original.id), // sonderr_change - inherit platform telemetry attribution
      })
      ...
      for (const msg of msgs) {
        ...
        const cloned = yield* updateMessage({
          ...
          id: newID,
          ...(msg.info.role === "assistant" && { cost: 0 }), // sonderr_change - count only spend incurred after the fork
          ...(parentID && { parentID }),
        })

        for (const part of msg.parts) {
          // sonderr_change - detach task calls + drop transient parts before copying the forked transcript
          const prepared = SonderrSession.prepareForkedPart(part)
          ...
          const p: SessionV1.Part = {
            ...
            messageID: cloned.id,
            sessionID: session.id,
            ...(prepared.type === "step-finish" && { cost: 0 }), // sonderr_change - exclude pre-fork spend from model stats
          }
      ...
      // sonderr_change - preserve imported/cumulative diffs when forking (self-contained Storage runtime keeps this shared file off the legacy Storage layer)
      yield* carryForkDiff(input.sessionID, session.id)
      // sonderr_change start - fork terminal task children under the new parent and remap their references
      yield* SonderrSession.remapChildren({
        sessionID: session.id,
...
function listByProject(
  ...
  // sonderr_change start - SonderrSession.filters keeps sessions visible across project_id changes
  ...
  const conditions =
    input.path !== undefined
      ...
      : SonderrSession.filters({ projectID: input.projectID, directory: input.directory })
  // sonderr_change end
  ...
  if (input.path !== undefined) {
    ...
    // sonderr_change start - directory filtering handled by SonderrSession.filters above
    // if (input.directory) {
...
// sonderr_change start - delegate to SonderrSession.listGlobal (adds projectID worktree family + directories[])
export function listGlobal(input?: {
  ...
  return SonderrSession.listGlobal<GlobalInfo>({ ...input, fromRow })
}
// sonderr_change end

// sonderr_change - delegate the exported Promise facade to the Sonderr session runtime
export const fork = sonderrSessionFork
