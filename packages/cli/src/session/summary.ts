...
import { appendSessionDiffs, readSessionDiffBase } from "@/sonderr/session-portability/cumulative-diff" // sonderr_change
import { Storage } from "@/storage/storage" // sonderr_change
...
export interface Interface {
  ...
  readonly diff: (input: DiffInput) => Effect.Effect<Snapshot.FileDiff[]> // sonderr_change - full-content detail input
  readonly computeDiff: (input: { messages: SessionV1.WithParts[] }) => Effect.Effect<Snapshot.FileDiff[]>
}

export class Service extends Context.Service<Service, Interface>()("@sonderr/SessionSummary") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const storage = yield* Storage.Service // sonderr_change
    ...
    const summarize = Effect.fn("SessionSummary.summarize")(function* (input: {
      ...
      if ((yield* config.get()).snapshot === false) return // sonderr_change - respect snapshot config toggle

      // sonderr_change start - preserve imported cumulative diffs when summarizing cloud-forked sessions
      ...
      yield* storage.write(["session_diff", input.sessionID], diffs).pipe(Effect.ignore) // sonderr_change
      yield* events.publish(Session.Event.Diff, { sessionID: input.sessionID, diff: diffs })

      if (!target || target.info.role !== "user") return
      const msgDiffs = base.length > 0 ? local : yield* computeDiff({ messages }) // sonderr_change
      target.info.summary = { ...target.info.summary, diffs: msgDiffs }
      yield* sessions.updateMessage(target.info)
    })

    const diff = Effect.fn("SessionSummary.diff")(function* (input: DiffInput) { // sonderr_change - full-content detail input
      // sonderr_change start - authoritative full-content detail for one file (editor diff tabs)
      ...
      // sonderr_change end
      // sonderr_change start - retain cumulative diffs for legacy TUI and VS Code consumers
      if (!input.messageID) {
...
export const DiffInput = Schema.Struct({
  ...
  full: Schema.optional(Schema.Boolean), // sonderr_change - request full-content detail
  file: Schema.optional(Schema.String), // sonderr_change - scope full detail to one file
})
...
export const node = LayerNode.make({
  service: Service,
  layer: layer,
  deps: [Session.node, Snapshot.node, EventV2Bridge.node, Config.node, Storage.node], // sonderr_change
})
