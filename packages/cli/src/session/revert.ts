...
import { Config } from "@/config/config" // sonderr_change
...
import { SonderrSessionRevert } from "@/sonderr/session/revert" // sonderr_change
...
export class Service extends Context.Service<Service, Interface>()("@sonderr/SessionRevert") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const revert = Effect.fn("SessionRevert.revert")(function* (input: RevertInput) {
      ...
      // sonderr_change start - keep the entire workspace transition atomic
      const prior = session.revert ? SonderrSessionRevert.files(all, session.revert) : []
      ...
      yield* SonderrSessionRevert.apply(
        snap,
        baseline,
        files,
        Effect.gen(function* () {
          if (session.revert?.snapshot) yield* SonderrSessionRevert.restore(snap, session.revert.snapshot, prior)
    ...
    const unrevert = Effect.fn("SessionRevert.unrevert")(function* (input: { sessionID: SessionID }) {
      ...
      // sonderr_change start - preserve the reverted workspace if redo cannot complete
      const all = yield* sessions.messages({ sessionID: input.sessionID }).pipe(Effect.orDie)
      const files = SonderrSessionRevert.files(all, session.revert)
      ...
      yield* SonderrSessionRevert.apply(
        ...
        files,
        Effect.gen(function* () {
          if (session.revert?.snapshot) yield* SonderrSessionRevert.restore(snap, session.revert.snapshot, files)
    ...
    const cleanup = Effect.fn("SessionRevert.cleanup")(function* (session: Session.Info) {
      ...
      if (session.revert.partID && target) {
        ...
        if (idx >= 0) {
          ...
          // sonderr_change start - clear a reverted provider error from the retained assistant message
