...
import {
  ...
  User,
  WithParts,
} from "@sonderr/core/v1/session"

export { EditorContext } from "@/sonderr/editor-context" // sonderr_change
...
import { MessageTable, PartTable, SessionTable } from "@sonderr/core/session/sql"
...
import { SessionNetwork } from "./network" // sonderr_change
import { CodexAuthExpiredError } from "@/sonderr/provider/codex-refresh" // sonderr_change
import { SonderrSessionMessageOrder } from "@/sonderr/session/message-order" // sonderr_change
import * as TextStream from "@/sonderr/text-stream" // sonderr_change
...
// sonderr_change - upstream moved these message/part types to SessionV1; re-export them so the
...
function truncateToolOutput(text: string, maxChars?: number) {
  ...
  // sonderr_change start - avoid persisting malformed Unicode in compacted tool output
...
// sonderr_change start - strip bloated metadata fields from stored parts to prevent multi-MB payloads
...
export function stripPartMetadata(part: Part): Part {
  // sonderr_change - exported for testing
...
// sonderr_change - apply stripping inside helpers so all read paths are covered
...
export const toModelMessagesEffect = Effect.fnUntraced(function* (
  ...
  for (const msg of input) {
    ...
    if (msg.info.role === "assistant") {
      ...
      for (const part of msg.parts) {
        // sonderr_change - !part.ignored keeps local UI warnings out of future prompts
        ...
        if (part.type === "tool") {
          ...
          if (part.state.status === "completed") {
            ...
            // sonderr_change start — do not replay send_file delivery attachments to the model;
...
export function parts(messageID: MessageID) {
  return Effect.gen(function* () {
    ...
    const rows = yield* db
      ...
      .all()
      .pipe(Effect.orDie)
    return rows.map(part) // sonderr_change - part() applies stripPartMetadata to cover all read paths
  })
}
...
export function filterCompacted(msgs: Iterable<WithParts>) {
  ...
  result.reverse()
  SonderrSessionMessageOrder.annotate(result) // sonderr_change - preserve chronology before retained-tail projection
...
export function fromError(
  ...
  switch (true) {
    ...
    case e instanceof CodexAuthExpiredError: // sonderr_change start
      return new AuthError(
        {
          providerID: "openai",
          message: e.message,
        ...
        { cause: e },
      ).toObject() // sonderr_change end
    case SessionNetwork.disconnected(e): // sonderr_change start
      return new APIError(
        {
          message: SessionNetwork.message(e), // sonderr_change end
          isRetryable: true,
          metadata: {
