import { FinishReason, LLMEvent, ProviderMetadata, ToolResultValue } from "@sonderr/llm"
import { Effect, Schema } from "effect"
import { type streamText } from "ai"
import { errorMessage } from "@/util/error"
import { SonderrRoutedModel } from "@/sonderr/session/routed-model" // sonderr_change
import { SonderrResponseMetadata } from "@/sonderr/session/response-metadata" // sonderr_change
...
// Move this extraction into @sonderr/llm when Copilot is handled by the native runtime.
...
function usage(value: unknown) {
  ...
  const item = value as {
    ...
    raw?: Record<string, unknown> // sonderr_change - preserve provider billing details
  }
  const entries = Object.entries({
    ...
    providerMetadata: item.raw ? { aiSdk: item.raw } : undefined, // sonderr_change - retain Sonderr billing details
      ...
      return Effect.sync(() => {
        ...
        return [
          LLMEvent.stepFinish({
            ...
            providerMetadata: SonderrResponseMetadata.write(
              SonderrRoutedModel.write(metadata, event.response?.modelId),
