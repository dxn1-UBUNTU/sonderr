...
import type { LLMClientShape } from "@sonderr/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("sonderr"))
    return { type: "unsupported", reason: "provider is not openai, sonderr, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @sonderr/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains sonderr-owned. The native runtime only adapts
      // the @sonderr/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
