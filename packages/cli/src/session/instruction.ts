...
import { SonderrInstruction } from "@/sonderr/session/instruction" // sonderr_change
import type { SonderrMarkdown } from "@/sonderr/config/markdown" // sonderr_change
...
export class Service extends Context.Service<Service, Interface>()("@sonderr/Instruction") {}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const globalFiles = [
      // sonderr_change start - prefer SONDERR_CONFIG_DIR profile when set
      ...(Flag.SONDERR_CONFIG_DIR ? [path.join(Flag.SONDERR_CONFIG_DIR, "AGENTS.md")] : []),
      // sonderr_change end
    ...
    const relative = Effect.fnUntraced(function* (instruction: string) {
      ...
      // sonderr_change - prefer SONDERR_CONFIG_DIR profile when set, else fall back to global.config
      const root = Flag.SONDERR_CONFIG_DIR ?? global.config
      return yield* fs.globUp(instruction, root, root).pipe(Effect.catch(() => Effect.succeed([] as string[]))) // sonderr_change
    })

    // sonderr_change start - project instructions cannot read env or files outside the project root
    const options = Effect.fnUntraced(function* (filepath: string, origin?: SonderrMarkdown.Source) {
      const ctx = yield* InstanceState.context
    ...
    const read = Effect.fnUntraced(function* (filepath: string, origin?: SonderrMarkdown.Source) {
      const opts = yield* options(filepath, origin)
      return yield* Effect.promise(() => SonderrInstruction.read(filepath, opts).catch(() => ""))
    })
    // sonderr_change end
    ...
    // sonderr_change start - retain declaration provenance through instruction path expansion
    const systemSources = Effect.fn("Instruction.systemSources")(function* () {
      const config = yield* cfg.get()
      const ctx = yield* InstanceState.context
      const root = ctx.worktree === "/" ? ctx.directory : ctx.worktree
      const paths = new Map<string, SonderrMarkdown.Source>()
      const add = (item: string, origin: SonderrMarkdown.Source) => {
      ...
      if (config.instructions) {
        for (const raw of config.instructions) {
          ...
          const trusted = declared.trusted && (path.isAbsolute(instruction) || Flag.SONDERR_DISABLE_PROJECT_CONFIG)
    ...
    const system = Effect.fn("Instruction.system")(function* () {
      const config = yield* cfg.get()
      const sources = yield* systemSources() // sonderr_change
      const paths = Array.from(sources.keys()) // sonderr_change
