...
export namespace SessionNetwork {
  ...
  export function disconnected(err: unknown) {
    ...
    // sonderr_change - recognize AbortSignal.timeout() errors
    for (const item of chain(err)) {
  ...
  export function message(err: unknown) {
    // sonderr_change - check for timeout first
    for (const item of chain(err)) {
  ...
  export const reply = fn(
    ...
    async (input) => {
      ...
      // sonderr_change start - reconnect failed remote MCP servers after network recovery
