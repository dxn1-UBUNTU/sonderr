...
import { ConfigAgentV1 } from "@sonderr/core/v1/config/agent"
import { configEntryNameFromPath } from "./entry-name"
import * as ConfigMarkdown from "./markdown"
import { ConfigParse } from "./parse"
import { ConfigVariable } from "./variable" // sonderr_change
// sonderr_change start
import { ConfigErrorV1 as ConfigError, FrontmatterError } from "@sonderr/core/v1/config/error"
...
// sonderr_change start - trusted gates {env:}; fileScope confines untrusted agent prompt {file:} reads
export async function load(
  ...
  for (const item of await Glob.scan("{agent,agents}/**/*.md", {
    ...
    // sonderr_change start - substitute agent prompt variables relative to the agent file. Project agents are
    ...
    // sonderr_change end
    // sonderr_change start - use Effect schema (propertyOrder: original) + non-fatal handleInvalid
    try {
      result[config.name] = ConfigParse.schema(ConfigAgentV1.Info, config, item)
    } catch (err) {
      if (ConfigError.InvalidError.isInstance(err)) {
        await SonderrConfig.handleInvalid("agent", item, err.data.issues ?? [], err, warnings)
        continue
      }
      throw err
    }
