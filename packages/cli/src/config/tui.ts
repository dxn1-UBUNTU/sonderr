...
import { isRecord } from "@sonderr/tui/util/record"
...
import { TuiKeybind } from "@sonderr/tui/config/keybind"
...
import { SonderrDefaultPlugins } from "@/sonderr/config/default-plugins" // sonderr_change
import { FormatError, FormatUnknownError } from "@/cli/error"
import { TuiConfig } from "@sonderr/tui/config"
...
export interface Interface {
  ...
  readonly info: () => Effect.Effect<Info> // sonderr_change - editable config for Sonderr console
  readonly pluginOrigins: () => Effect.Effect<ConfigPlugin.Origin[]>
  readonly waitForDependencies: () => Effect.Effect<void>
}

export class Service extends Context.Service<Service, Interface>()("@sonderr/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // sonderr_change start - trusted gates {env:}; fileScope confines untrusted {file:} reads
  const load = (
    text: string,
    ...
    // sonderr_change end
    Effect.gen(function* () {
      // sonderr_change start - only trusted tui config resolves {env:}; untrusted {file:} confined to fileScope
      ...
      // (mirroring the old sonderr.json shape) still get their settings applied.
  ...
  // sonderr_change start - trusted + fileScope threaded to load
  const loadFile = (filepath: string, trusted: boolean, fileScope?: ConfigVariable.FileScope): Effect.Effect<Info> =>
    ...
    Effect.gen(function* () {
      ...
      return yield* load(text, filepath, trusted, fileScope) // sonderr_change
    })

  // sonderr_change start - trusted + fileScope threaded to loadFile
  const mergeFile = (acc: Acc, file: string, trusted: boolean, fileScope?: ConfigVariable.FileScope) =>
    // sonderr_change end
    Effect.gen(function* () {
      const data = yield* loadFile(file, trusted, fileScope) // sonderr_change
  ...
  // sonderr_change start - discover canonical and legacy Sonderr config directories
  // Every config dir we may read from: global config, .sonderr and legacy .sonderr
  // folders between cwd and home, and SONDERR_CONFIG_DIR.
  // sonderr_change end
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.SONDERR_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  for (const file of ConfigPaths.fileInDirectory(Global.Path.config, "tui")) {
    yield* mergeFile(acc, file, true) // sonderr_change - global config is trusted
  }

  // 2. Explicit SONDERR_TUI_CONFIG override, if set.
  if (Flag.SONDERR_TUI_CONFIG) {
    const configFile = Flag.SONDERR_TUI_CONFIG
    yield* mergeFile(acc, configFile, true) // sonderr_change - explicit env-provided path is trusted
    yield* Effect.logDebug("loaded custom tui config", { path: configFile })
  }

  // 3. Project tui files, applied root-first so the closest file wins.
  for (const file of projectFiles) {
    yield* mergeFile(acc, file, false, { root: ctx.directory, source: file }) // sonderr_change - untrusted, {file:} confined to project
  }

  // sonderr_change start - load tui.json from supported Sonderr config directories
  // 4. `.sonderr` and legacy `.sonderr` directories (and SONDERR_CONFIG_DIR)
  ...
  const dirs = unique(directories).filter(
    (dir) => dir.endsWith(".sonderr") || dir.endsWith(".sonderr") || dir === Flag.SONDERR_CONFIG_DIR,
  )
  // sonderr_change end

  for (const dir of dirs) {
    // sonderr_change start - trust global (home/SONDERR_CONFIG_DIR) dirs like config.ts; in-repo .sonderr/.sonderr stay untrusted
  ...
  if (acc.result.attention?.sound_pack === undefined) result.attention.sound_pack = "sonderr.default" // sonderr_change - preserve Sonderr's default sound pack

  // sonderr_change start - inject Sonderr default plugins to keep TUI aligned with server config
  const defaults = SonderrDefaultPlugins.apply(
    { plugin: result.plugin ? [...result.plugin] : undefined, plugin_origins: acc.plugin_origins },
    { disabled: Flag.SONDERR_DISABLE_DEFAULT_PLUGINS },
  )
  ...
  return {
    config,
    info: { ...acc.result, plugin: defaults.plugin }, // sonderr_change - include applied Sonderr defaults
    pluginOrigins: defaults.plugin_origins ?? [], // sonderr_change - exclude builtins from dependency installation
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const info = Effect.fn("TuiConfig.info")(() => Effect.succeed(data.info)) // sonderr_change
    const pluginOrigins = Effect.fn("TuiConfig.pluginOrigins")(() => Effect.succeed(data.pluginOrigins))

    const waitForDependencies = Effect.fn("TuiConfig.waitForDependencies")(() =>
      Effect.forEach(deps, Fiber.join, { concurrency: "unbounded" }).pipe(Effect.ignore(), Effect.asVoid),
    )
    return Service.of({ get, info, pluginOrigins, waitForDependencies }) // sonderr_change
