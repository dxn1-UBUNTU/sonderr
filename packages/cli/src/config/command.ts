...
import { SchemaIssue } from "effect" // sonderr_change - preserve Effect issue details in Sonderr warnings
import { Glob } from "@sonderr/core/util/glob"
import { ConfigCommandV1 } from "@sonderr/core/v1/config/command"
...
export async function load(
  ...
  for (const item of await Glob.scan("{command,commands}/**/*.md", {
    ...
    await SonderrConfig.handleInvalid("command", item, issues, cause, warnings)
    // sonderr_change end
  }
  return result
}
