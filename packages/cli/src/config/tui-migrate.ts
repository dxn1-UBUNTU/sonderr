...
import { access, constants } from "fs/promises" // sonderr_change
...
const TUI_SCHEMA_URL = "https://app.kilo.ai/tui.json" // sonderr_change
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from sonderr.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const sonderr = await sonderrFiles(input)
  for (const file of sonderr) {
...
async function sonderrFiles(input: { directories: string[]; cwd: string }) {
  // sonderr_change start: use sonderr directory everywhere
  const project = Flag.SONDERR_DISABLE_PROJECT_CONFIG
    ? []
    : await Filesystem.findUp(["sonderr.json", "sonderr.jsonc"], input.cwd, undefined, { rootFirst: true })
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "sonderr")]
  // sonderr_change end
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "sonderr"))
  }
  if (Flag.SONDERR_CONFIG) files.push(Flag.SONDERR_CONFIG)
