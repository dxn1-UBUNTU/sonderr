// sonderr_change - Sonderr compatibility layer. Upstream deleted this Bus (Effect PubSub) service in v1.16.2
// in favour of EventV2; Sonderr keeps it ONLY for existing Sonderr-owned callers (sonderr/* features) that rely on
// its eager-callback subscription + fork-atomicity semantics. Do NOT add new shared/upstream-shaped consumers.
// Full migration of Sonderr callers onto core EventV2 is tracked as a dedicated follow-up.
import { Effect, Exit, Fiber, Layer, PubSub, Scope, Context, Stream, Schema } from "effect" // sonderr_change
...
import { context as instanceContext, type InstanceContext } from "@/project/instance-context" // sonderr_change
import { InstanceRef } from "@/effect/instance-ref"
import { LocalContext } from "@/util/local-context" // sonderr_change
import { LayerNode } from "@sonderr/core/effect/layer-node" // sonderr_change
...
export class Service extends Context.Service<Service, Interface>()("@sonderr/Bus") {}
...
export const node = LayerNode.make({ service: Service, layer, deps: [] }) // sonderr_change

const { runPromise } = makeRuntime(Service, layer) // sonderr_change
...
// sonderr_change start - legacy callback facade inherits the active instance context
function active() {
