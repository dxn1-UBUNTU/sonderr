...
import { GitHubSecurity } from "@/sonderr/security/github" // sonderr_change
...
const AGENT_USERNAME = "sonderrconnect[bot]" // sonderr_change
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/sonderr.yml" // sonderr_change
  ...
  yield* Effect.promise(async () => {
    {
      ...
      function printNextSteps() {
        ...
        prompts.outro(
          [
            ...
            step2,
            "",
            "    3. Go to a GitHub issue and comment `/sonderr summarize` to see the agent in action", // sonderr_change
            "",
            "   Learn more about the GitHub agent - https://kilo.ai/docs/code-with-ai/platforms/github", // sonderr_change
          ].join("\n"),
        )
      }
      ...
      async function installGitHubApp() {
        ...
        // Open browser
        const url = "https://github.com/apps/sonderrconnect" // sonderr_change
        const command =
        ...
        async function getInstallation() {
          // sonderr_change start - updated to new endpoint
      ...
      async function addWorkflowFiles() {
        // sonderr_change start - updated workflow template with Sonderr branding and gateway secrets
        const providerEnvStr =
          provider === "amazon-bedrock"
            ? ""
            : providers[provider].env.map((e) => `\n          ${e}: \${{ secrets.${e} }}`).join("")

        const sonderrGatewayEnv =
          provider === "sonderr"
            ? `\n          SONDERR_API_KEY: \${{ secrets.SONDERR_API_KEY }}\n          SONDERR_ORG_ID: \${{ secrets.SONDERR_ORG_ID }}`
            : ""

        const envStr = providerEnvStr || sonderrGatewayEnv ? `\n        env:${providerEnvStr}${sonderrGatewayEnv}` : ""
...
jobs:
  sonderr:
    if: |
      ...
      contains(github.event.comment.body, ' /sonderr') ||
      startsWith(github.event.comment.body, '/sonderr')
    ...
    steps:
      ...
      - name: Run Sonderr
        uses: Sonderr-Org/sonderr/github@latest${envStr}
        with:
...
export const githubRun = Effect.fn("Cli.github.run")(function* (args: { event?: string; token?: string }) {
  ...
  yield* Effect.promise(async () => {
    ...
    const shareBaseUrl = isMock ? "https://dev.kilo.ai" : "https://kilo.ai" // sonderr_change
    ...
    try {
      ...
      // Setup sonderr session // sonderr_change
      ...
      console.log("sonderr session", session.id) // sonderr_change
    ...
    function normalizeOidcBaseUrl(): string {
      ...
      if (!value) return "https://api.kilo.ai" // sonderr_change
    ...
    async function getUserPrompt() {
      ...
      const mentions = (process.env["MENTIONS"] || "/sonderr,/kc") // sonderr_change
        .split(",")
      ...
      for (const m of matches) {
        const tag = m[0]
        // sonderr_change start - only fetch canonical GitHub attachment routes
    ...
    async function chat(message: string, files: PromptFiles = []) {
      console.log("Sending message to sonderr...") // sonderr_change
    ...
    async function getOidcToken() {
      try {
        return await core.getIDToken("sonderr-github-action") // sonderr_change
      } catch (error) {
    ...
    async function exchangeForAppToken(token: string) {
      // sonderr_change start - updated endpoint URLs per new API structure
    ...
    function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
      ...
      if (type === "schedule" || type === "dispatch") {
        ...
        return `sonderr/${type}-${hex}-${timestamp}` // sonderr_change
      }
      return `sonderr/${type}${issueId}-${timestamp}` // sonderr_change
    }
    ...
    function footer(opts?: { image?: boolean }) {
      // sonderr_change start - simplified footer with text branding (no image backend yet)
      const share = shareId ? `[sonderr session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
      return `\n\n---\n*Powered by [Sonderr](https://kilo.ai)*&nbsp;&nbsp;|&nbsp;&nbsp;${share}[github run](${runUrl})`
      // sonderr_change end
    }
  ...
  repository(owner: $owner, name: $repo) {
    ...
    function buildPromptDataForIssue(issue: GitHubIssue) {
      ...
      return [
        ...
        "- Git push and PR creation are handled AUTOMATICALLY by the sonderr infrastructure after your response", // sonderr_change
