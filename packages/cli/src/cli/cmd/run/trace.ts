...
//
// Enable with SONDERR_DIRECT_TRACE=1. Writes one JSON line per event to
// ~/.local/share/sonderr/log/direct/<timestamp>-<pid>.jsonl. Also writes // sonderr_change
...
export function trace(): Trace | undefined {
  ...
  if (!process.env.SONDERR_DIRECT_TRACE) {
    state = false
    return undefined
  }
