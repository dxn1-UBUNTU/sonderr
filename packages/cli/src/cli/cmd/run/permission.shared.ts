...
export function permissionInfo(request: PermissionRequest): PermissionInfo {
  ...
  if (request.permission === "sandbox_escalation") {
    ...
    return {
      icon: "!",
      title: "Allow Git operation outside the sandbox", // sonderr_change
      lines: command
...
export function permissionAlwaysLines(request: PermissionRequest): string[] {
  if (request.always.length === 1 && request.always[0] === "*") {
    return [`This will allow ${request.permission} until Sonderr is restarted.`] // sonderr_change
  }

  return [
    "This will allow the following patterns until Sonderr is restarted.", // sonderr_change
...
export function permissionReply(requestID: string, reply: PermissionReply["reply"], message?: string): PermissionReply {
  return {
    requestID,
    reply,
    interactive: true, // sonderr_change - footer replies are human-driven; the server refuses non-interactive skill-shell approvals
...
export function permissionShift(state: PermissionBodyState, dir: -1 | 1, temporary?: boolean): PermissionBodyState {
  // sonderr_change
  const list = permissionOptions(state.stage, temporary) // sonderr_change
  if (list.length === 0) {
    return state
  }
