...
import { normalizePromptContent } from "@sonderr/tui/editor"
import fuzzysort from "fuzzysort"
import path from "path"
import { createEffect, createMemo, createResource, createSignal, onCleanup, onMount, type Accessor } from "solid-js"
import { slashDisplay, slashMatches } from "@/sonderr/cli/cmd/command-display" // sonderr_change
...
import { SONDERR_BASE_MODE, useBindings } from "@sonderr/tui/keymap"
...
export function createPromptState(input: PromptInput): PromptState {
  ...
  const slashOptions = createMemo<SlashOption[]>(() => {
    const builtins = [
      ...
      { kind: "slash", name: "exit", display: "/exit", description: "close direct mode" } satisfies SlashOption, // sonderr_change
    ]
    ...
    return [
      ...(showSkillMenu
        ...
        : []),
      ...(input.commands() ?? [])
        .filter((item) => !hidden.has(item.name)) // sonderr_change - suggest skills as slash commands
        .map(
          (item) =>
            ({
              kind: "slash",
              name: item.name,
              display: slashDisplay(item), // sonderr_change
              description: item.description,
            }) satisfies SlashOption,
        ),
      ...builtins,
  ...
  const select = (item?: PromptOption) => {
    ...
    if (next.kind === "slash") {
      ...
      const text = `${next.display}${separator}` // sonderr_change
