// Top-level orchestrator for `sonderr --mini`.
//
...
import { SonderrRunTerminal } from "@/sonderr/cli/cmd/run-terminal" // sonderr_change
import { createRunDemo } from "./demo"
import { resolveModelInfo, resolveRunTuiConfig, resolveSessionInfo } from "./runtime.boot"
import { createRuntimeLifecycle } from "./runtime.lifecycle"
import { trace } from "./trace"
import { cycleVariant, formatModelLabel, resolveSavedVariant, resolveVariant, saveVariant } from "./variant.shared"
// sonderr_change - preserve compatible variants when switching models
import { resolvePreservedVariant } from "@/sonderr/cli/cmd/run/variant" // sonderr_change
...
async function runInteractiveRuntime(input: RunRuntimeInput, deps: RunRuntimeDeps = {}): Promise<void> {
  ...
  const terminal = SonderrRunTerminal.create(ctx.sdk, () => state.sessionID) // sonderr_change
  ...
  const shell = await (deps.createRuntimeLifecycle ?? createRuntimeLifecycle)({
    ...
    // sonderr_change start - human-driven terminal in direct interactive mode
    ...
    onModelSelect: async (model) => {
      ...
      // sonderr_change start - preserve the active effort across model switches
      ...
      const switching = resolveSavedVariant(model).then((saved) => {
        ...
        // sonderr_change - prefer the active effort over a model-specific saved preference
        state.activeVariant =
  ...
  void modelTask.then((info) => {
    ...
    // sonderr_change start - preserve the active effort when the model catalog arrives asynchronously
    const next =
