...
import { openEditor } from "@sonderr/tui/editor"
import { registerSonderrKeymap } from "@sonderr/tui/keymap"
...
export type LifecycleInput = {
  ...
  onTerminalWrite: (input: { terminalID: string; data: string }) => Promise<void> // sonderr_change
  onTerminalResize: (input: { terminalID: string; cols: number; rows: number }) => Promise<void> // sonderr_change
  onTerminalClose: (terminalID: string) => Promise<void> // sonderr_change
...
export async function createRuntimeLifecycle(input: LifecycleInput): Promise<Lifecycle> {
  ...
  try {
    const renderer = await createCliRenderer({
      ...
      targetFps: 30,
      maxFps: 60,
      useMouse: true, // sonderr_change - interactive terminal close and scroll controls
      autoFocus: false,
    ...
    unregisterKeymap = registerSonderrKeymap(keymap, renderer, input.tuiConfig)
