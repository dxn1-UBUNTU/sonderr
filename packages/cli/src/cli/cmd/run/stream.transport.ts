...
import type { Event, GlobalEvent, SonderrClient } from "@sonderr/sdk/v2" // sonderr_change - revert to upstream native Event type
...
class Service extends Context.Service<Service, TransportService>()("@sonderr/RunStreamTransport") {}
...
function sid(event: Event): string | undefined {
  ...
  if (event.type === "interactive_terminal.updated") return event.properties.info.sessionID // sonderr_change
  ...
  if (
    ...
    event.type === "interactive_terminal.data" || // sonderr_change
    event.type === "interactive_terminal.deleted" || // sonderr_change
...
function globalPayloadEvent(value: unknown): Event | undefined {
  ...
  // sonderr_change start - revert to upstream: ignore sync compatibility copies
...
function pickView(data: SessionData, subagent: SubagentData, order: Map<string, number>): FooterView {
  return pickBlockerView({
    terminal: data.terminal, // sonderr_change
