...
import type { Event, ToolPart } from "@sonderr/sdk/v2" // sonderr_change - revert to upstream native Event type
...
function emitTask(state: State): void {
  ...
  const part = {
    ...
    state: {
      status: "running",
      input: {
        filePath: "packages/cli/src/cli/cmd/run/stream.ts",
        offset: 1,
        limit: 200,
      },
      time: {
  ...
  if (kind === "read") {
    ...
    askPermission(state, {
      ...
      always: [target],
        ...
        title: "read",
        output: ["1: {", '2:   "name": "sonderr",', '3:   "private": true', "4: }"].join("\n"),
        metadata: {},
      },
    })
    return
