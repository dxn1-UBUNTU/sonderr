...
// sonderr_change start - keep equivalent up keybindings aligned at panel boundaries
...
function handleKey(input: {
  ...
  // sonderr_change start - treat ctrl+p as the up binding
  if (up(input.event)) {
    input.event.preventDefault()
    input.menu.move(-1)
    return
...
export function RunSubagentSelectBody(props: {
  ...
  useKeyboard((event) => {
    ...
    // sonderr_change start - close on either up binding at the first row
...
export function RunModelSelectBody(props: {
  ...
  const entries = createMemo<ModelEntry[]>(() =>
    (props.providers() ?? [])
        ...
        Object.entries(provider.models)
          ...
          .map(([modelID, model]) => {
            ...
            const footer = current
              ? "current"
              : model.cost?.input === 0 && provider.id === "sonderr"
                ? "Free"
                : title !== modelID
                  ? modelID
                  : undefined
      ...
      .sort((a, b) => {
        const provider = Number(a.providerID !== "sonderr") - Number(b.providerID !== "sonderr")
        if (provider !== 0) {
          return provider
        }
