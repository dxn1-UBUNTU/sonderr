...
export function RejectField(props: {
  ...
  return (
    <textarea
      ...
      maxHeight={3}
      wrapMode="word"
      placeholder={"Tell Sonderr what to do differently" /* sonderr_change */}
...
export function RunPermissionBody(props: {
  ...
  const temporary = createMemo(() => temporaryPermission(props.request)) // sonderr_change
  const opts = createMemo(() => permissionOptions(state().stage, temporary())) // sonderr_change
  ...
  const shift = (dir: -1 | 1) => {
    setState((prev) => permissionShift(prev, dir, temporary())) // sonderr_change
  }
      ...
      >
        ...
        <Switch>
          ...
          <Match when={state().stage === "reject"}>
            <box paddingLeft={1}>
              <text fg={props.theme.muted}>{"Tell Sonderr what to do differently" /* sonderr_change */}</text>
            </box>
          </Match>
        </Switch>
      </box>
