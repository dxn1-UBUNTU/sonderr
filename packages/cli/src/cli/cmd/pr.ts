...
import { existsSync } from "node:fs" // sonderr_change
import { detectPrLink, parsePrUrl, readPrLinkOverride, writePrLinkOverride } from "@/sonderr-sessions/pr-link" // sonderr_change

const subcommand = "pr" // sonderr_change

// sonderr_change start - resolve the currently running CLI instead of hardcoding sonderr
export function cliCommand(
  ...
  if (script === subcommand) return [input.execPath] // sonderr_change
...
export const PrCommand = cmd({
  command: subcommand,
  describe: "manage pull requests", // sonderr_change
  builder: (yargs: Argv) =>
    yargs
...
export const PrCheckoutCommand = effectCmd({
  ...
  describe: "fetch and checkout a GitHub PR branch, then run sonderr", // sonderr_change
  builder: (yargs) =>
  ...
  handler: Effect.fn("Cli.pr.checkout")(function* (args) {
    ...
    const cli = cliCommand() // sonderr_change
    ...
    if (prInfoResult.code === 0 && prInfoResult.text.trim()) {
      ...
      if (prInfo?.body) {
        const sessionMatch = prInfo.body.match(/https:\/\/app\.sonderr\.ai\/s\/([a-zA-Z0-9_-]+)/) // sonderr_change
        if (sessionMatch) {
    ...
    UI.println()
    UI.println("Starting sonderr...") // sonderr_change
    UI.println()

    const run = sessionId ? [...cli, "-s", sessionId] : cli // sonderr_change
    ...
    if (code !== 0) return yield* Effect.die(new Error(`sonderr exited with code ${code}`)) // sonderr_change
  }),
})

// sonderr_change start - link/unlink/status write and read the manual PR override in Storage
