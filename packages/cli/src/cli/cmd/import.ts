import type { Session as SDKSession, Message, Part } from "@sonderr/sdk/v2"
import { SessionV1 } from "@sonderr/core/v1/session"
import { Session } from "@/session/session"
import { CliError, effectCmd } from "../effect-cmd"
import { Database } from "@sonderr/core/database/database"
import { SessionTable, MessageTable, PartTable } from "@sonderr/core/session/sql"
...
const log = Log.create({ service: "import" }) // sonderr_change
...
/** Extract share token from a Sonderr share URL like https://app.kilo.ai/s/<jwt> */
export function parseShareUrl(url: string): string | null {
  const match = url.match(/^https?:\/\/app\.sonderr\.ai\/s\/([A-Za-z0-9_.-]+)$/)
...
async function ingestBootstrap(sessionId: string) {
  ...
  return SonderrSessions.bootstrap(sessionId)
}
...
const runImport = Effect.fn("Cli.import.body")(function* (file: string, ctx: InstanceContext) {
  ...
  if (isUrl) {
    // sonderr_change start - Migrate to upstream ShareNext architecture #10281
