...
export const RemoteCommand = cmd({
  ...
  handler: async () => {
    ...
    await bootstrap(process.cwd(), async () => {
      // sonderr_change - K1 W1: advertise this instance on the relay
      // heartbeat so the cloud side can show it as a spawn-capable instance.
      // The process-wide `SONDERR_REMOTE_ATTACH_SESSION` guard was removed in K1
      // (in-process sessions only; no spawned children), so this is always
      // advertised for the explicit `sonderr remote` command path.
      ...
      SonderrSessions.setInstanceAdvertisement(buildInstanceAdvertisement(Instance.directory))
