...
// sonderr_change - @/sonderr/auth/remove is dynamically imported in the logout handler to keep startup fast
...
export const ProvidersCommand = cmd({
  // sonderr_change start - keep "auth" as primary command name
  command: "auth",
  aliases: ["providers"],
  // sonderr_change end
  ...
  handler: Effect.fn("Cli.providers.login")(function* (args) {
    ...
    if (args.url) {
      ...
      const wellknown = (yield* cliTry(`Failed to load auth provider metadata from ${url}: `, () =>
        fetch(`${url}/.well-known/sonderr`).then((x) => x.json()),
      )) as {
    ...
    if (args.provider) {
      ...
      // sonderr_change start - accept codex as an alias for OpenAI ChatGPT auth
    ...
    if (provider === "other") {
      ...
      yield* Prompt.log.warn(
        `This only stores a credential for ${provider} - you will need configure it in sonderr.json, check the docs for examples.`, // sonderr_change
      )
    }
    ...
    if (provider === "amazon-bedrock") {
      yield* Prompt.log.info(
        "Amazon Bedrock authentication priority:\n" +
          ...
          "Configure via sonderr.json options (profile, region, endpoint) or\n" + // sonderr_change
    ...
    if (["cloudflare", "cloudflare-ai-gateway"].includes(provider)) {
      yield* Prompt.log.info(
        "Cloudflare AI Gateway can be configured with CLOUDFLARE_GATEWAY_ID, CLOUDFLARE_ACCOUNT_ID, and CLOUDFLARE_API_TOKEN environment variables. Read more: https://kilo.ai/docs/ai-providers/cloudflare", // sonderr_change
      )
    }
...
export const ProvidersLogoutCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.providers.logout")(function* (args) {
    ...
    const { remove: removeAuth } = yield* Effect.promise(() => import("@/sonderr/auth/remove"))
