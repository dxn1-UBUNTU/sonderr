...
// sonderr_change - SonderrMcpConfig is dynamically imported in addMcpToConfig to keep startup fast
...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    if (servers.length === 0) {
      ...
      prompts.outro("Add servers with: sonderr mcp add") // sonderr_change
      return
    }
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in sonderr.json:") // sonderr_change
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // sonderr_change start - prefer supported Sonderr config directories over root files
  const roots = [
    path.join(baseDir, "sonderr.jsonc"),
    path.join(baseDir, "sonderr.json"),
    path.join(baseDir, "sonderr.jsonc"),
    path.join(baseDir, "sonderr.json"),
  ]
  const candidates = global
    ? roots
    : [
        path.join(baseDir, ".sonderr", "sonderr.jsonc"),
        path.join(baseDir, ".sonderr", "sonderr.json"),
        path.join(baseDir, ".sonderr", "sonderr.jsonc"),
        path.join(baseDir, ".sonderr", "sonderr.json"),
        path.join(baseDir, ".sonderr", "sonderr.jsonc"),
        path.join(baseDir, ".sonderr", "sonderr.json"),
  ...
  // Default to sonderr.json if none exist
  return path.join(baseDir, "sonderr.json")
  // sonderr_change end
}
...
async function addMcpToConfig(name: string, mcpConfig: ConfigMCPV1.Info, configPath: string) {
  ...
  // sonderr_change start - lazy import keeps the CLI startup graph light
  const { SonderrMcpConfig } = await import("@/sonderr/cli/cmd/mcp")
  const result = SonderrMcpConfig.format(configPath, applyEdits(text, edits))
  // sonderr_change end
...
export const McpAddCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.add")(function* (args) {
    ...
    const ctx = maybeCtx
    const global = Flag.SONDERR_CONFIG_DIR ?? Global.Path.config // sonderr_change - honor the active Sonderr config profile
    yield* Effect.promise(async () => {
      ...
      if (args.name) {
        ...
        const configPath = await resolveConfigPath(global, true) // sonderr_change
      ...
      const [projectConfigPath, globalConfigPath] = await Promise.all([
        ...
        resolveConfigPath(global, true), // sonderr_change
      ])
      ...
      if (type === "local") {
        const command = await prompts.text({
          ...
          placeholder: "e.g., sonderr x @modelcontextprotocol/server-filesystem", // sonderr_change
          validate: (x) => (x && x.length > 0 ? undefined : "Required"),
        })
        ...
        const response = await fetch(serverConfig.url, {
          ...
          body: JSON.stringify({
            ...
            params: {
              ...
              capabilities: {},
              clientInfo: { name: "sonderr-debug", version: InstallationVersion }, // sonderr_change
            },
            id: 1,
          }),
        })
        ...
        if (response.status === 401) {
          ...
          try {
            const client = new Client({
              name: "sonderr-debug", // sonderr_change
