...
import { errorMessage } from "@sonderr/tui/util/error"
...
import type { StartInput } from "@/sonderr/cli/cmd/tui/thread" // sonderr_change - runtime imports deferred into handlers
import { win32InstallCtrlCGuard } from "@sonderr/tui/terminal-win32"
import { validate as validateSession } from "@/sonderr/cli/cmd/tui" // sonderr_change
// sonderr_change start - correlate the TUI worker with its parent process
import {
  SONDERR_PROCESS_ROLE,
  SONDERR_RUN_ID,
  ensureRunID,
  sanitizedProcessEnv,
} from "@sonderr/core/util/sonderr-process"
// sonderr_change end
import type { RemoteExitBridgeClient } from "@/sonderr/cli/cmd/tui/remote-exit-bridge" // sonderr_change - runtime import deferred
import type { Exit } from "@sonderr/tui/context/exit" // sonderr_change

declare global {
  const SONDERR_WORKER_PATH: string
}

type RpcClient = ReturnType<typeof Rpc.client<typeof rpc>>

// sonderr_change start - bridge remote exit only for the embedded worker transport
...
// sonderr_change start - share the extracted TUI runner between daemon and worker paths
...
async function target() {
  if (typeof SONDERR_WORKER_PATH !== "undefined") return SONDERR_WORKER_PATH
...
export function resolveThreadDirectory(project?: string, envPWD = process.env.PWD, cwd = process.cwd()) {
  // sonderr_change start - ignore stale PWD from wrappers such as `bun --cwd`, except sonderr-dev's caller cwd
  const dev = process.env.SONDERR_DEV_CWD
  ...
  return dev ? root : real // sonderr_change
}

export const TuiThreadCommand = cmd({
  command: "$0 [project]",
  describe: "start sonderr tui", // sonderr_change
  builder: (yargs) =>
    withNetworkOptions(yargs)
      .positional("project", {
        type: "string",
        describe: "path to start sonderr in", // sonderr_change
      })
      .option("model", {
        type: "string",
        alias: ["m"],
      ...
      // sonderr_change start - create/reuse a git worktree before starting
      .option("worktree", {
        type: "string",
        describe: "create (or reuse) a git worktree with this name and start sonderr there",
      })
      // sonderr_change end
      .option("prompt", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    // sonderr_change start - lazy Sonderr implementations so other CLI commands
    ...
    const { SonderrTuiThreadDaemon } = await import("@/sonderr/cli/cmd/tui/thread")
    ...
    try {
      ...
      // sonderr_change start - `--worktree <name>` creates/reuses a worktree; resuming
      ...
      // sonderr_change start
      const preloads = preload(typeof SONDERR_WORKER_PATH !== "undefined", () =>
      ...
      // sonderr_change start - default TUI sessions attach to the daemon unless explicitly disabled
      if (await SonderrTuiThreadDaemon.attach({ args, cwd, input: () => input(args.prompt), start })) return
      // sonderr_change end
      const auth = SonderrTuiThreadDaemon.workerAuth() // sonderr_change - protect TUI-owned HTTP routes from unauthenticated local callers
      // sonderr_change start - propagate stable run metadata and an explicit worker role
      const env = sanitizedProcessEnv({
        [SONDERR_PROCESS_ROLE]: "worker",
        [SONDERR_RUN_ID]: ensureRunID(),
        ...auth.env,
        SONDERR_BACKGROUND_PROCESS_PORTS: "true",
      })
      // sonderr_change end
      const worker = new Worker(file, {
        preload: preloads, // sonderr_change
        env, // sonderr_change
      })
      worker.onerror = (e) => {
      ...
      // sonderr_change start - graceful shutdown on external signals
      ...
      // sonderr_change - external kill -INT takes the same graceful path as SIGHUP/SIGTERM.
      ...
      const { TuiConfig } = await import("@/config/tui") // sonderr_change
      ...
      const transport = external
        ? {
            ...
            fetch: undefined,
            headers: auth.headers, // sonderr_change
            events: undefined,
          }
        : {
      ...
      // sonderr_change - upstream validates here, but --cloud-fork's session id is only local after
      ...
      try {
        // sonderr_change start - import cloud session before TUI renders
        if (args.cloudFork && args.session) {
          UI.println("Importing session from cloud...")
          const { createSonderrClient } = await import("@sonderr/sdk/v2")
          const sdk = createSonderrClient({
            baseUrl: transport.url,
            fetch: transport.fetch,
            headers: transport.headers, // sonderr_change
            directory: cwd,
          })
          try {
        ...
        try {
          await validateSession({
            url: transport.url, // sonderr_change
            sessionID: localSessionID(args), // sonderr_change
            directory: cwd,
            fetch: transport.fetch,
            headers: transport.headers, // sonderr_change
          })
        ...
        await start(
          {
            // sonderr_change - shared lazy loader also supports daemon attach
            url: transport.url,
            async onSnapshot() {
