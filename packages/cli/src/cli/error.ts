...
export function FormatError(input: unknown): string | undefined {
  ...
  if (NamedError.hasName(input, "MCPFailed")) {
    ...
    return `MCP server "${data}" failed.` // sonderr_change
  }
  ...
  // ProviderModelNotFoundError: { providerID: string, modelID: string, suggestions?: string[], modelsEmpty?: boolean } // sonderr_change
  ...
  if (providerModelNotFound) {
    ...
    return [
      ...
      ...(providerModelNotFound.modelsEmpty === true ? ["No models are currently available."] : []), // sonderr_change
      `Try: \`sonderr models\` to list available models`, // sonderr_change
  ...
  if (remoteAuth) {
    ...
    return [
      ...
      ...(url ? [`Run \`sonderr auth login ${url}\` to re-authenticate.`] : []), // sonderr_change
    ].join("\n")
  }
