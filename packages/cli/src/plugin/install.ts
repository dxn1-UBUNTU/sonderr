...
export type PatchDeps = {
  ...
  files: (dir: string, name: "sonderr" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".sonderr") // sonderr_change
}

function patchName(kind: Kind): "sonderr" | "tui" {
  if (kind === "server") return "sonderr"
  return "tui"
}
