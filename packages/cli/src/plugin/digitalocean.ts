...
import type { Model } from "@sonderr/sdk/v2"
...
<html>
  <head>
    <meta charset="utf-8" />
    <title>Sonderr - DigitalOcean Authorization</title>
    <style>
    ...
    <script>
      (async function() {
        ...
        try {
          ...
          msgEl.textContent = "You can close this window and return to Sonderr."
...
async function startOAuthServer(): Promise<void> {
  ...
  oauthServer = createServer((req, res) => {
    ...
    if (req.method === "GET" && url.pathname === OAUTH_REDIRECT_PATH) {
      ...
      res.end(HTML_CALLBACK) // sonderr_change - shared callback page is currently Sonderr-branded
      return
    }
      ...
      methods: [
        {
          ...
          async authorize() {
            ...
            return {
              url,
              instructions:
                "Sign in to DigitalOcean in your browser. Sonderr will use your DigitalOcean API token directly for inference and load your Inference Routers. Re-run /connect to refresh routers later.",
