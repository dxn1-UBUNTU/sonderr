...
import { escapeHtml } from "@/util/html" // sonderr_change - retained OAuth error page uses escaped diagnostics
...
function authHeaders() {
  return {
    ...
    "User-Agent": `sonderr/${InstallationVersion}`, // sonderr_change
  }
}
...
export function buildAuthorizeUrl(
  ...
  // clients. `referrer=sonderr` lets xAI attribute sonderr-originated
  // sonderr_change end
  ...
  const params = new URLSearchParams({
    ...
    nonce,
    plan: "generic",
    referrer: "sonderr", // sonderr_change
  })
...
<html>
  <head>
    <title>Sonderr - xAI Authorization Successful</title>
    <style>
      body {
        font-family:
          system-ui,
...
<html>
  <head>
    <title>Sonderr - xAI Authorization Failed</title>
    <style>
      body {
        font-family:
          system-ui,
...
async function startOAuthServer(): Promise<{ port: number; redirectUri: string }> {
  ...
  const server = createServer((req, res) => {
    ...
    if (url.pathname === OAUTH_REDIRECT_PATH) {
      ...
      if (error) {
        ...
        res.end(HTML_ERROR(errorMsg)) // sonderr_change - shared callback page is currently Sonderr-branded
        return
      }
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    server.listen(OAUTH_PORT, OAUTH_HOST, () => {
      ...
      // sonderr_change start
      // behavior and crash the entire Sonderr process. Matches the silent-
      // sonderr_change end
      ...
      async loader(getAuth) {
        ...
        return {
          ...
          async fetch(requestInput: RequestInfo | URL, init?: RequestInit) {
            ...
            headers.set("User-Agent", `sonderr/${InstallationVersion}`) // sonderr_change
