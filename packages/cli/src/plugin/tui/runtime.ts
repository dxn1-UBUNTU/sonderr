...
import { errorData, errorMessage } from "@sonderr/tui/util/error"
...
import { hasTheme, upsertTheme } from "@sonderr/tui/context/theme"
...
import type { HostPluginApi, HostSlots } from "@sonderr/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@sonderr/core/v1/config/plugin"
import { createCommandShim } from "@sonderr/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@sonderr/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    // sonderr_change start - install local themes into supported Sonderr config directories
    const base = path.basename(source_dir)
    const local_dir =
      base === ".sonderr" || base === ".sonderr"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".sonderr", "themes")
    // sonderr_change end
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".sonderr", "tui.json"), // sonderr_change
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.SONDERR_PURE ? [] : pluginOrigins
    if (Flag.SONDERR_PURE && pluginOrigins.length) {
    }
