import type { Hooks, PluginInput } from "@sonderr/plugin"
...
import { refreshCodexAuth } from "@/sonderr/provider/codex-refresh" // sonderr_change
import { OpenAIWebSocketPool } from "./ws-pool"

const log = Log.create({ service: "plugin.codex" }) // sonderr_change
...
const ALLOWED_MODELS = new Set([
  ...
  "gpt-5.4",
  "gpt-5.4-mini",
  // sonderr_change start - additional codex models supported by Sonderr
  "gpt-5.1-codex",
  "gpt-5.1-codex-max",
  "gpt-5.1-codex-mini",
  "gpt-5.2-codex",
...
function buildAuthorizeUrl(redirectUri: string, pkce: PkceCodes, state: string): string {
  const params = new URLSearchParams({
    ...
    state,
    originator: "sonderr", // sonderr_change
  })
...
async function refreshAccessToken(refreshToken: string, issuer = ISSUER, signal?: AbortSignal): Promise<TokenResponse> {
  const response = await fetch(`${issuer}/oauth/token`, {
    method: "POST",
    signal,
    headers: { "Content-Type": "application/x-www-form-urlencoded", "User-Agent": `sonderr/${InstallationVersion}` },
    // sonderr_change end
    body: new URLSearchParams({
...
// sonderr_change start - retain Sonderr-branded OAuth callback until the shared page supports Sonderr branding
const HTML_SUCCESS = `<!doctype html>
<html>
  <head>
    <!-- sonderr_change start -->
    <title>Sonderr - Codex Authorization Successful</title>
    <!-- sonderr_change end -->
    <style>
      body {
        font-family:
  ...
  <body>
    <div class="container">
      ...
      <!-- sonderr_change start -->
      <p>You can close this window and return to Sonderr.</p>
      <!-- sonderr_change end -->
    </div>
    <script>
...
<html>
  <head>
    <!-- sonderr_change start -->
    <title>Sonderr - Codex Authorization Failed</title>
    <!-- sonderr_change end -->
    <style>
      body {
        font-family:
...
async function startOAuthServer(): Promise<{ port: number; redirectUri: string }> {
  ...
  oauthServer = createServer((req, res) => {
    ...
    if (url.pathname === "/auth/callback") {
      ...
      res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" })
      res.end(HTML_SUCCESS) // sonderr_change - shared callback page is currently Sonderr-branded
      return
    }
...
export async function CodexAuthPlugin(input: PluginInput, options: CodexAuthPluginOptions = {}): Promise<Hooks> {
  ...
  return {
      ...
      async models(provider, ctx) {
        ...
        return Object.fromEntries(
          Object.entries(provider.models)
            .filter(([, model]) => {
              if (model.options?.reasoningMode === "pro") return false // sonderr_change - tolerate catalog entries without options
              if (ALLOWED_MODELS.has(model.api.id)) return true
              if (DISALLOWED_MODELS.has(model.api.id)) return false // sonderr_change
              ...
              modelID,
              {
                ...
                limit: model.id.includes("gpt-5.5")
                  ...
                  : model.id.includes("gpt-5.6")
                    ? {
                        context: 1_050_000, // sonderr_change - use the current Codex limits for GPT-5.6 OAuth models
                        input: 922_000, // sonderr_change
                        output: 128_000,
                      }
                    : model.limit,
              },
    ...
    "chat.headers": async (input, output) => {
      ...
      output.headers.originator = "sonderr"
      output.headers["User-Agent"] = `sonderr/${InstallationVersion} (${os.platform()} ${os.release()}; ${os.arch()})`
