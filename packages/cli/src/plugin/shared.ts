...
export const DEPRECATED_PLUGIN_PACKAGES = ["sonderr-openai-codex-auth", "sonderr-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, sonderrVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(sonderrVersion) || semver.major(sonderrVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.sonderr
  if (typeof range !== "string") return
  if (!semver.satisfies(sonderrVersion, range)) {
    throw new Error(`Plugin requires sonderr ${range} but running ${sonderrVersion}`)
  }
}
