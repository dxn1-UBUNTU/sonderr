...
import { EventSequenceTable, EventTable } from "@sonderr/core/event/sql" // sonderr_change - upstream moved the event tables to core
import { EventID } from "./schema"
import { Context, Effect, Layer, Schema as EffectSchema } from "effect"
import type { DeepMutable } from "@sonderr/core/schema"
...
> = {
  ...
  wire?: boolean // sonderr_change - EventV2 rows cross persistence and bus boundaries as encoded data
}
...
// sonderr_change start - serialized rows carry the schema's encoded representation
...
export class Service extends Context.Service<Service, Interface>()("@sonderr/SyncEvent") {}
...
export const layer = Layer.effect(Service)(
  Effect.gen(function* () {
    ...
    const replay: Interface["replay"] = Effect.fn("SyncEvent.replay")(function* (event, options) {
      ...
      // sonderr_change start - decode only EventV2 rows
...
export const defaultLayer = layer.pipe(
  Layer.provide([ProjectBus.defaultLayer, AppNodeBuilder.build(RuntimeFlags.node)]), // sonderr_change
)
...
export function init(input: { projectors: Array<[Definition, ProjectorFunc]>; convertEvent?: ConvertEvent }) {
  ...
  for (const entry of EventManifest.Latest.values()) {
    if (!entry.durable) continue // sonderr_change - mirror current durable events into legacy sync
    register({
      type: entry.type,
      version: entry.durable.version, // sonderr_change
      aggregate: entry.durable.aggregate, // sonderr_change
      properties: entry.data,
      schema: entry.data,
      wire: true, // sonderr_change
    })
  }
...
function process<Def extends Definition>(
  ...
  Database.transaction((tx) => {
    ...
    const data = def.wire ? EventWire.encode(def.schema, event.data) : event.data // sonderr_change
    ...
    if (options.experimentalWorkspaces) {
      ...
      tx.insert(EventTable)
        .values({
          id: EventV2.ID.make(event.id), // sonderr_change - core event table uses the branded EventV2 ID
          seq: event.seq,
          aggregate_id: event.aggregateID,
          type: versionedType(def.type, def.version),
          data: data as Record<string, unknown>, // sonderr_change
        })
        .run()
    }
    ...
    Database.effect(() => {
      ...
      const publish = (value: unknown) =>
        ...
        options.bridge.fork(
          Effect.gen(function* () {
            // sonderr_change start - encode EventV2 properties before crossing the legacy boundary
            if (def.wire) {
...
export function effectPayloads() {
  return [
    ...
    ...EventManifest.Latest.values()
      .filter(
        (definition) =>
          definition.durable !== undefined && // sonderr_change
          !registry.has(versionedType(definition.type, definition.durable.version)), // sonderr_change
      )
      .map((definition) =>
        EffectSchema.Struct({
          type: EffectSchema.Literal("sync"),
          name: EffectSchema.Literal(versionedType(definition.type, definition.durable!.version)), // sonderr_change
          id: EffectSchema.String,
          seq: EffectSchema.Finite,
          aggregateID: EffectSchema.Literal(definition.durable!.aggregate), // sonderr_change
