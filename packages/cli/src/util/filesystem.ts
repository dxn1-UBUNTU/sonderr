import { chmod, mkdir, readFile, rename, stat as statFile, writeFile } from "fs/promises" // sonderr_change
import { createWriteStream, existsSync, statSync } from "fs"
import { realpathSync } from "fs"
// sonderr_change start - harden containment checks
...
export function stat(p: string): ReturnType<typeof statSync> | undefined {
  // sonderr_change start - also treat ENOTDIR/EACCES as absent, every caller expects undefined
  try {
...
// sonderr_change start - Windows transient locked-file errors on atomic rename
...
export async function write(p: string, content: string | Buffer | Uint8Array, mode?: number): Promise<void> {
  // sonderr_change start - atomic write via temp-file + rename to avoid partial reads on concurrent saves
