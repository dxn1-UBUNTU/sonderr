...
import * as SonderrOAuthCallback from "../sonderr/mcp-oauth-callback" // sonderr_change

const log = Log.create({ service: "mcp.oauth-callback" }) // sonderr_change
...
<head>
  ...
  <title>Sonderr - Authorization Successful</title>
  <!-- sonderr_change end -->
  <style>
...
<head>
  <!-- sonderr_change start -->
  <title>Sonderr - Authorization Failed</title>
  <!-- sonderr_change end -->
  <style>
    body { font-family: system-ui, -apple-system, sans-serif; display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; background: #1a1a2e; color: #eee; }
    .container { text-align: center; padding: 2rem; }
...
export async function ensureRunning(redirectUri?: string): Promise<void> {
  // sonderr_change start - delegate Sonderr-specific callback binding from here because OAuth state lives in this module
  await SonderrOAuthCallback.ensureRunning({
    redirectUri,
