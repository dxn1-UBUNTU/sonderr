...
if (process.platform === "win32" && !("type" in process)) {
  Object.defineProperty(process, "type", { value: "sonderr-bun", configurable: true })
}
// sonderr_change end
...
import { ConfigMCPV1 } from "@sonderr/core/v1/config/mcp"
...
import * as SandboxNetwork from "@/sonderr/sandbox/network" // sonderr_change
import { McpCatalog } from "./catalog"
import { McpEvent } from "@sonderr/schema/mcp-event"
import { McpBrowser } from "./browser"

const DEFAULT_TIMEOUT = 30_000
const CLIENT_OPTIONS = {
  capabilities: {
    // Upstream issue anomalyco/sonderr#11948 // sonderr_change
    // sampling: {},
    // Upstream issue anomalyco/sonderr#23066 // sonderr_change
    // elicitation: {},
    // Upstream issue anomalyco/sonderr#2308 // sonderr_change
    roots: {},
    // Upstream issue anomalyco/sonderr#28567 // sonderr_change
    // tasks: {},
  },
} satisfies ClientOptions

// sonderr_change start - inject --rm for Docker containers to prevent stopped container accumulation
...
function createClient(directory: string) {
  const client = new Client({ name: "sonderr", version: InstallationVersion }, CLIENT_OPTIONS) // sonderr_change
...
export interface McpTool {
  ...
  // sonderr_change start - identifies the owning server for MCP Apps routing
...
export class Service extends Context.Service<Service, Interface>()("@sonderr/MCP") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
            ...
            if (isAuthError) {
              if (lastError.message.includes("registration") || lastError.message.includes("client_id")) {
                ...
                return events
                  .publish(TuiEvent.ToastShow, {
                    title: "MCP Authentication Required",
                    message: `Server "${key}" requires authentication. Run: sonderr mcp auth ${key}`, // sonderr_change
                    variant: "warning",
                    duration: 8000,
                  })
                  .pipe(Effect.ignore, Effect.as(undefined))
    ...
    const connectLocal = Effect.fn("MCP.connectLocal")(function* (
      ...
      const finalArgs = ensureDockerRm(cmd, args) // sonderr_change
      const baseDir = yield* InstanceState.directory
      const cwd = mcp.cwd ? path.resolve(baseDir, mcp.cwd) : baseDir
      const bridge = yield* EffectBridge.make() // sonderr_change - drain child stderr without writing over the TUI
      const transport = new StdioClientTransport({
        stderr: "pipe",
        command: cmd,
        args: finalArgs, // sonderr_change
        cwd,
        // sonderr_change start - local MCPs must not inherit backend credentials
        env: modelEnv({
          ...(cmd === "sonderr" ? { BUN_BE_BUN: "1" } : {}),
          ...mcp.environment,
        }),
        // sonderr_change end
      })
      // sonderr_change start - a piped stderr stream must be consumed or verbose MCP servers can block
    ...
    const tools = Effect.fn("MCP.tools")(function* () {
      ...
      for (const [clientName, client] of Object.entries(s.clients)) {
        ...
        for (const def of listed) {
          const tool = { def, client, clientName, timeout } // sonderr_change - clientName for MCP Apps routing
          // sonderr_change start - preserve remote MCP authority on the native entry for every execution path
    ...
    function collectFromConnected<T extends { name: string }>(
      ...
      return Effect.gen(function* () {
        ...
        return yield* Effect.forEach(
          ...
          ([clientName, client]) =>
            McpCatalog.collect(
              // sonderr_change - distinguish collection from direct network fetch
              clientName,
              client,
    ...
    // sonderr_change start - `opts?: { callback?: boolean }` parameter is Sonderr-specific
    const startAuth = Effect.fn("MCP.startAuth")(function* (mcpName: string, opts?: { callback?: boolean }) {
      ...
      // sonderr_change start - authenticate() defers binding the callback port until a redirect is needed
    ...
    const authenticate = Effect.fn("MCP.authenticate")(function* (
      ...
      const result = yield* startAuth(mcpName, { callback: false }) // sonderr_change
      ...
      // sonderr_change start - bind only after redirect exists, and clean up if binding fails
      const mcpConfig = yield* getMcpConfig(mcpName)
