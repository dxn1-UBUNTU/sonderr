...
import { CodeMode, Tool as SandboxTool, toolError } from "@sonderr/codemode"
import { MCP } from "@/mcp"
...
const invokeChildTool = Effect.fn("CodeMode.invokeChildTool")(function* (input: {
  ...
  callID: string
  ctx: Tool.Context
  bridge: EffectBridge.Shape // sonderr_change
}) {
...
export const CodeModeTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    const bridge = yield* EffectBridge.make() // sonderr_change
    ...
    const init: Tool.DefWithoutID<typeof Parameters, Metadata> = {
      ...
      execute: Effect.fn("CodeMode.execute")(function* (params, ctx) {
        ...
        const restricted = yield* bridge.run(SandboxPolicy.networkRestricted(ctx.sessionID)) // sonderr_change
        const mcpTools = restricted ? {} : Permission.visibleTools(yield* mcp.tools(), ruleset) // sonderr_change
