...
import { Session } from "../session/session" // sonderr_change
import { SessionID } from "../session/schema" // sonderr_change
import { RecallSearch } from "../sonderr/session/recall-search" // sonderr_change
...
export const RecallTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    const sessions = yield* Session.Service // sonderr_change
    return {
...
async function search(
  ...
  const dirs = await bridge.promise(WorktreeFamily.list().pipe(Effect.provideService(Git.Service, git))) // sonderr_change
  const boundary = SonderrSessionPromptQueue.active(ctx.sessionID) ?? RecallSearch.active(ctx.messages, ctx.messageID)
