...
// sonderr_change start — support absolute glob patterns (e.g. ~/.config/sonderr/command/*.md)
    ...
    return {
      ...
      execute: (params: { pattern: string; path?: string }, ctx: Tool.Context) =>
        Effect.gen(function* () {
          ...
          const absolute = split(params.pattern) // sonderr_change
          yield* ctx.ask({
            permission: "glob",
          ...
          const limit = 100
          // sonderr_change start - retain bounded-search metadata from Core ripgrep.
          const result = yield* ripgrep.glob({
            cwd: search,
            pattern: absolute?.pattern ?? params.pattern, // sonderr_change - absolute patterns are split into cwd + relative glob
            limit,
          ...
          if (files.length > 0) {
            ...
            if (result.partial) output.push("", "(Some discovered files could not be read.)") // sonderr_change
          }
