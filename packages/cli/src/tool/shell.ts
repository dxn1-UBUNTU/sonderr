import { Effect, Fiber, Stream } from "effect" // sonderr_change - Fiber
import os from "os"
...
import { model as modelEnv } from "@/sonderr/process/env" // sonderr_change
import { Shell } from "@sonderr/core/shell"
import { ShellID } from "./shell/id"

import * as Truncate from "./truncate"
import { Plugin } from "@/plugin"
import { normalizeUrls } from "@/sonderr/util/url" // sonderr_change
import { CommandTimeout } from "@/sonderr/command-timeout" // sonderr_change
import { heredocs } from "@/sonderr/tool/shell-heredoc" // sonderr_change
import { unparsed } from "@/sonderr/tool/shell-unparsed" // sonderr_change
import { ChildProcess } from "effect/unstable/process"
import { ChildProcessSpawner } from "effect/unstable/process/ChildProcessSpawner"
import { ShellPrompt, type Parameters } from "./shell/prompt"
import { BashArity } from "@/permission/arity"
import { mutates as mutatesGit } from "@/sonderr/sandbox/git" // sonderr_change
...
const ask = Effect.fn("ShellTool.ask")(function* (
  ...
  scan: Scan,
  command: string,
  metadata: ReturnType<typeof heredocs>, // sonderr_change
  description?: string, // sonderr_change
) {
  // sonderr_change
  if (scan.dirs.size > 0) {
    ...
    yield* ctx.ask({
      ...
      patterns: globs,
      always: globs,
      // sonderr_change start - retain read classification alongside upstream permission context
      metadata: {
        command,
  ...
  yield* ctx.ask({
    ...
    metadata: { command: normalizeUrls(command), ...(description ? { description } : {}), ...metadata }, // sonderr_change
  })
})

// sonderr_change start - share bash permission scanning with Sonderr interactive terminal
type PermissionInput = {
  command: string
  cwd: string
  shell: string
  description?: string
  escalate?: boolean // sonderr_change
}
...
export const ShellPermission = Effect.gen(function* () {
  ...
  const collect = Effect.fn("ShellTool.collect")(function* (
    ...
    // sonderr_change start - fail closed on commands the grammar failed to parse (#12326)
  ...
  const check = Effect.fn("ShellTool.permission")(function* (ctx: Tool.Context, input: PermissionInput) {
    ...
    yield* Effect.scoped(
      Effect.gen(function* () {
        ...
        const metadata = heredocs(tree.rootNode, ShellID.toKind(Shell.name(input.shell))) // sonderr_change
        if (!containsPath(input.cwd, instance)) {
          scan.dirs.add(input.cwd)
          scan.access = "unknown"
        }
        yield* ask(ctx, scan, input.command, metadata, input.description) // sonderr_change
        const gitMutation = commands(tree.rootNode).some((node) => mutatesGit(node.text))
        if (input.escalate && gitMutation) {
          yield* ctx.ask({
            permission: "sandbox_escalation", // sonderr_change
  ...
  // sonderr_change start - expose the tree-sitter scan (sub-command patterns + external-dir globs) for skill-shell batching
  ...
  return { ask: check, resolve, decompose } // sonderr_change - decompose for skill-shell
})
// sonderr_change end

function cmd(shell: string, command: string, cwd: string, env: NodeJS.ProcessEnv) {
  if (process.platform === "win32" && Shell.ps(shell)) {
    // sonderr_change start - PowerShell args
...
export const ShellTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    const permission = yield* ShellPermission // sonderr_change
    ...
    const shellEnv = Effect.fn("ShellTool.shellEnv")(function* (ctx: Tool.Context, cwd: string) {
      ...
      return modelEnv(extra.env) // sonderr_change - model shells must not inherit backend credentials
    })
    ...
    const run = Effect.fn("ShellTool.run")(function* (
      ...
      const code: number | null = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          const reader = yield* Effect.forkScoped(
            // sonderr_change - keep the fiber so trailing output can be drained
          ...
          const timeout = Effect.sleep(`${CommandTimeout.duration(input.timeout)} millis`) // sonderr_change
          ...
          // sonderr_change start - closing the scope interrupts the reader fiber, which can drop
      ...
      return {
        title: input.description, // sonderr_change - UI shows the model's description, command goes in metadata
        metadata: {
          output: last || preview(output),
          exit: code,
          description: input.description, // sonderr_change
          truncated: cut,
        ...
        return {
          ...
          execute: (params: Parameters, ctx: Tool.Context) =>
            Effect.gen(function* () {
              ...
              const timeout = CommandTimeout.clamp(params.timeout ?? defaultTimeoutMs).timeout // sonderr_change
              ...
              yield* permission.ask(ctx, {
                ...
                escalate: sandboxed, // sonderr_change
              }) // sonderr_change
                ...
                run(
                  {
                    ...
                    timeout,
                    description: params.description ?? params.command, // sonderr_change
                  },
                  ctx,
                ),
              )
