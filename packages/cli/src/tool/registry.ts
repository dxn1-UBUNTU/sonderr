...
import { type ToolContext as PluginToolContext, type ToolDefinition } from "@sonderr/plugin"
...
import { SonderrToolRegistry } from "../sonderr/tool/registry" // sonderr_change
import { Notebook } from "@/sonderr/notebook/service" // sonderr_change
import { AgentManager } from "@/sonderr/agent-manager/service" // sonderr_change
import { RepoOverviewTool } from "@/sonderr/tool/repo-overview" // sonderr_change
import { RepoCloneTool } from "./repo_clone" // sonderr_change
import { Flag } from "@sonderr/core/flag/flag" // sonderr_change
import { Auth } from "@/auth" // sonderr_change
import { Env } from "@/env" // sonderr_change - websearch resolves its config via Env.Service
...
import { Effect, Layer, Context, Option } from "effect" // sonderr_change
import { ChildProcessSpawner } from "effect/unstable/process/ChildProcessSpawner"
import { HttpClient } from "effect/unstable/http" // sonderr_change
...
import { SessionStatus } from "@/session/status" // sonderr_change
import { SonderrSessions } from "@/sonderr-sessions/sonderr-sessions" // sonderr_change - provide SonderrSessions.Service so the notify_user tool's init resolves
import { Git } from "@/git" // sonderr_change
import { BackgroundJob } from "@/background/job"
import { RuntimeFlags } from "@/effect/runtime-flags"
import * as ToolNetwork from "@/sonderr/sandbox/network" // sonderr_change
import { MemoryService } from "@sonderr/sonderr-memory/effect/service" // sonderr_change
...
import { RipgrepBinary } from "@sonderr/core/ripgrep/binary" // sonderr_change
import { AppProcess } from "@sonderr/core/process" // sonderr_change
import { MCP } from "@/mcp"
import { PermissionV1 } from "@sonderr/core/v1/permission"
import { McpCatalog } from "@/mcp/catalog"
import { InstanceRef } from "@/effect/instance-ref" // sonderr_change

export function webSearchEnabled(
  providerID: ProviderV2.ID,
  flags = { exa: Flag.SONDERR_ENABLE_EXA, parallel: Flag.SONDERR_ENABLE_PARALLEL },
) {
  return providerID === ProviderV2.ID.sonderr || flags.exa || flags.parallel // sonderr_change
}
...
export interface Interface {
  ...
  readonly tools: (model: {
    ...
    networkRestricted?: boolean // sonderr_change - hide network-backed code-mode catalogs in restricted sessions
  }) => Effect.Effect<Tool.Def[]>
  // sonderr_change end
}

export class Service extends Context.Service<Service, Interface>()("@sonderr/ToolRegistry") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const sonderrToolInfos = yield* SonderrToolRegistry.infos(manager, notebook).pipe(Effect.provide(MemoryService.layer))
    // sonderr_change end
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("ToolRegistry.state")(function* (ctx) {
        const codeModeTool = codeMode
          ...
          : undefined // sonderr_change - initialize code mode with the active instance context
        ...
        const indexing = SonderrToolRegistry.indexing(cfg, global)
        // sonderr_change end
        ...
        const tool = yield* Effect.all({
          ...
          clone: Tool.init(clone), // sonderr_change
          overview: Tool.init(overview), // sonderr_change
          ...
          ...(codeModeTool ? { execute: Tool.init(codeModeTool) } : {}), // sonderr_change
        })

        // sonderr_change start
        const sonderr = yield* SonderrToolRegistry.build(sonderrToolInfos, {
          agent: agents,
          truncate,
        ...
        return {
          ...
          builtin: SonderrToolRegistry.describe(
            [
              ...
              tool.todo,
              tool.search,
              ...(flags.experimentalScout ? [tool.clone, tool.overview] : []), // sonderr_change
              tool.skill,
              tool.patch,
              tool.plan,
              ...(["cli", "vscode"].includes(flags.client) ? [tool.suggest] : []),
              ...SonderrToolRegistry.extra(sonderr, cfg),
    ...
    const all: Interface["all"] = Effect.fn("ToolRegistry.all")(function* () {
      ...
      return [...s.builtin.map(ToolNetwork.builtin), ...s.custom] as Tool.Def[] // sonderr_change
    })
    ...
    const describeCodeMode = Effect.fn("ToolRegistry.describeCodeMode")(function* (input: {
      agent: Agent.Info
      permission?: PermissionV1.Ruleset
      networkRestricted?: boolean // sonderr_change
    }) {
      if (!codeMode) return
      if (input.networkRestricted) return // sonderr_change
    ...
    const tools: Interface["tools"] = Effect.fn("ToolRegistry.tools")(function* (input) {
      const cfg = yield* config.get() // sonderr_change
      const filtered = (yield* all()).filter((tool) => {
        if (!SonderrToolRegistry.available(tool, input.agent)) return false // sonderr_change
        if (tool.id === WebSearchTool.id) {
          if (cfg.web_search === true) return true // sonderr_change
          return webSearchEnabled(input.providerID, { exa: flags.enableExa, parallel: flags.enableParallel })
        }

        const usePatch = SonderrToolRegistry.usePatch(input) // sonderr_change
        if (tool.id === ApplyPatchTool.id) return usePatch
        if (tool.id === EditTool.id) return !usePatch // sonderr_change

        return true
      })
      const sonderrFiltered = yield* SonderrToolRegistry.applyVisibility(filtered) // sonderr_change

      const codeModeDescription = filtered.some((tool) => tool.id === "execute")
        ? yield* describeCodeMode(input)
        : undefined
      const visible = sonderrFiltered.filter((tool) => tool.id !== "execute" || codeModeDescription) // sonderr_change
...
// sonderr_change start - preserve Sonderr registry dependencies and sandbox-aware HTTP in the upstream node graph
...
export const node = LayerNode.suspend(() =>
  LayerNode.make({
    ...
    deps: [
      ...
      Bus.node,
      Auth.node,
      Env.node, // sonderr_change - websearch resolves its config via Env.Service
      SessionStatus.node,
      AgentManager.node,
      Notebook.node,
