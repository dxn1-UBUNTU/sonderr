import { Effect, Schema, Scope } from "effect" // sonderr_change - stable object reads do not use Option
import { NonNegativeInt } from "@sonderr/core/schema"
import * as path from "path"
import { Readable } from "stream" // sonderr_change
...
import { Config } from "@/config/config" // sonderr_change - optional configured reference authorization
import { assertExternalDirectoryEffect } from "./external-directory"
import { Instruction } from "../session/instruction"
import { isPdfAttachment, sniffAttachmentMime } from "@/util/media"
// sonderr_change start
import * as Encoding from "../sonderr/encoding"
import { SonderrReference } from "@/sonderr/reference/contains"
import * as SonderrConfiguredReference from "@/sonderr/reference"
import { SonderrReadObject } from "@/sonderr/tool/read-object"
import * as Extract from "../sonderr/tool/read-extract"
import * as TextStream from "../sonderr/text-stream"
// sonderr_change end

const DEFAULT_READ_LIMIT = 2000
const MAX_LINE_LENGTH = 2000
// sonderr_change start - report the safe Unicode slice length
...
>(
  ...
  Effect.gen(function* () {
    ...
    // sonderr_change start - authorize missing paths without enumerating sibling names
    ...
    // sonderr_change start - extracted formats and text consume the authorized open object
    const lines = Effect.fn("ReadTool.lines")(
      (file: SonderrReadObject.File, opts: { limit: number; offset: number }, abort: AbortSignal) =>
        Effect.tryPromise({
          try: async (signal) => {
    ...
    const isBinaryFile = (filepath: string, bytes: Uint8Array) => {
      ...
      // sonderr_change start - UTF-16/32 BOM: NUL bytes are legitimate, skip the NUL/control-char heuristic
    ...
    const run = Effect.fn("ReadTool.execute")(function* (
      ...
      // sonderr_change start - resolve V1 configured references without introducing a Core location-layer dependency
      const config = yield* Effect.serviceOption(Config.Service)
      const references =
        config._tag === "Some"
          ? SonderrConfiguredReference.resolveAll({
          ...
          : []
      // sonderr_change end
      // sonderr_change start - fail before read authorization when the target is missing
      ...
      // sonderr_change start - directory mentions expose only a bound listing, never child file bodies
      if (info.type === "Directory") {
        ...
        const explicit =
          ...
          (yield* SonderrReference.path(fs, ctx.extra["referenceRoot"], target))
        const referenced =
          explicit ||
          (yield* SonderrReference.contains({ fs, references, target }))
        ...
        // sonderr_change start - reject any canonical path change after permission approval
        ...
        const limit = Math.max(1, params.limit ?? DEFAULT_READ_LIMIT) // sonderr_change - prevent zero-limit loops
      ...
      // sonderr_change start - authorize metadata, then bind every content read to the same reopened object
      const file = yield* SonderrReadObject.file(requested)
      const explicit =
        typeof ctx.extra?.["referenceRoot"] === "string" &&
        (yield* SonderrReference.path(fs, ctx.extra["referenceRoot"], file.target))
      const referenced =
        explicit ||
        (yield* SonderrReference.contains({ fs, references, target: file.target }))
...
// sonderr_change start - extracted formats use native readers; ordinary text is supplied by FSUtil above
async function collect(stream: Readable, opts: { limit: number; offset: number }) {
  ...
  try {
    for await (const text of rl) {
      ...
      // sonderr_change start - keep truncated output valid Unicode
