...
import { filterDiagnostics } from "./diagnostics" // sonderr_change
import { ConfigValidation } from "../sonderr/config-validation" // sonderr_change
import * as EncodedIO from "../sonderr/tool/encoded-io" // sonderr_change
import { Format } from "../format"
import * as Bom from "@/util/bom"
import { assertMutablePath } from "../sonderr/agent-manager/protection" // sonderr_change
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("ApplyPatchTool.execute")(function* (
      ...
      const fileChanges: Array<{
        ...
        deletions: number
        bom: boolean
        encoding: string // sonderr_change - preserved per-file encoding
      }> = []
      ...
      for (const hunk of hunks) {
        ...
        assertMutablePath(filePath) // sonderr_change
        ...
        switch (hunk.type) {
          case "add": {
            ...
            fileChanges.push({
              ...
              deletions,
              bom: next.bom,
              encoding: "utf-8", // sonderr_change - new files default to utf-8
            })
          ...
          case "update": {
            ...
            // sonderr_change start - encoding-aware read so non-UTF-8 files decode without
            ...
            let bom = source.bom
            let encoding = read.encoding // sonderr_change - overwritten by deriveNewContentsFromChunks below
          ...
          case "delete": {
            // sonderr_change start - encoding-aware read so non-UTF-8 files decode without corruption
            ...
            fileChanges.push({
              ...
              deletions,
              bom: source.bom,
              encoding: deleteRead.encoding, // sonderr_change
            })
      ...
      for (const change of fileChanges) {
        ...
        switch (change.type) {
          case "add":
            ...
            yield* EncodedIO.write(afs, change.filePath, Bom.join(change.newContent, change.bom), change.encoding) // sonderr_change - encoding-aware write (mkdirs) replaces afs.writeWithDirs
            updates.push({ file: change.filePath, event: "add" })
            break

          case "update":
            yield* EncodedIO.write(afs, change.filePath, Bom.join(change.newContent, change.bom), change.encoding) // sonderr_change - encoding-aware write replaces afs.writeWithDirs
          ...
          case "move":
            if (change.movePath) {
              // Create parent directories (recursive: true is safe on existing/root dirs)
              yield* EncodedIO.write(afs, change.movePath, Bom.join(change.newContent, change.bom), change.encoding) // sonderr_change - encoding-aware write (mkdirs) replaces afs.writeWithDirs
      ...
      // sonderr_change start - append Sonderr config validation warnings
      ...
      return {
        ...
        metadata: {
          diff: totalDiff,
          files,
          diagnostics: filterDiagnostics(diagnostics, changedPaths), // sonderr_change
        },
        output,
      }
    })
