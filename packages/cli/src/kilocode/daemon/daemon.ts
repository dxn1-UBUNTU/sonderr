...
export namespace Daemon {
  const username = "sonderr"
  const lock = "sonderr-daemon"
  ...
  function root() {
    return process.env.SONDERR_TEST_DAEMON_STATE_DIR ?? Global.Path.state
  }

  function logs() {
    return process.env.SONDERR_TEST_DAEMON_LOG_DIR ?? Global.Path.log
  }
  ...
  export function matches(state: State, input: Options, explicit: readonly NetworkOption[]) {
    if (state.password === "sonderr") return false
  ...
  async function port(input: Options) {
    ...
    if (input.env?.SONDERR_TEST_DAEMON_EPHEMERAL_PORT) return 0
    ...
    try {
      const child = spawn(cmd[0], [...cmd.slice(1), ...args(input)], {
        ...
        env: {
          ...process.env,
          ...input.env,
          SONDERR_SERVER_USERNAME: username,
          SONDERR_SERVER_PASSWORD: password,
          SONDERR_FEATURE: "daemon",
        },
  ...
  async function line(out: string) {
    ...
    const match = text.match(/sonderr server listening on http:\/\/([^:\s]+):(\d+)/)
