...
import { ConfigReference } from "@sonderr/core/config/reference"
...
import type { LocationServiceMap } from "@sonderr/core/location-services"
...
// Keep Core V2 tools on the same effective Sonderr config used by stable tools. Core's standalone
// scanner cannot see account/managed config or SONDERR_CONFIG_CONTENT, so replace its provisional
// references after both config systems finish booting.
export const sync = Effect.fn("SonderrReference.sync")(function* (input: {
...
export const list = Effect.fn("SonderrReference.list")(function* (
