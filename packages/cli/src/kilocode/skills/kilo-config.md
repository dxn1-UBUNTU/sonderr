# Sonderr CLI Configuration Reference

All config lives in `sonderr.json` (or `sonderr.jsonc`). Precedence low-to-high: remote well-known, global (`~/.config/sonderr/sonderr.json`), env `SONDERR_CONFIG`, project `./sonderr.json`, `.sonderr/sonderr.json`, `SONDERR_CONFIG_CONTENT`, managed (see Config File Locations). Deep-merged; later wins.

This also covers where Sonderr looks for config files, commands, agents, and skills across project, global, and legacy paths such as `.sonderr/`, `.sonderr/`, and `~/.config/sonderr/`, plus Agent Manager setup/run scripts in the VS Code extension.

## Commands (`.sonderr/command/*.md`)

Markdown files with YAML frontmatter. The filename (minus `.md`) becomes the command name invoked via `/name`. Commands can live in `.sonderr/`, legacy `.sonderr/`, and global config roots, with both `command/` and `commands/` directory names supported. See Config File Locations for the full search order.
...
2. `~/.sonderr/`
3. `~/.sonderr/`
4. The `SONDERR_CONFIG_DIR` directory (if the env var is set)
5. project `.sonderr/` and `.sonderr/` directories from the current working directory up to the worktree root
...
## Agents (`.sonderr/agent/*.md`)

Also loaded from legacy `.sonderr/` directories and plural `agents/` variants.
...
Markdown files in `.sonderr/workflows/` or `.sonderr/workflows/` (project-level) and `~/.sonderr/workflows/` or `~/.sonderr/workflows/` (global). These are automatically converted to commands at startup. The filename (minus `.md`) becomes the command name. Project workflows override global ones with the same name.
...
Agent Manager setup/run scripts are project files in the main repository's `.sonderr/` directory. They are not `sonderr.json` settings and should not be configured inside generated `.sonderr/worktrees/<name>/` checkouts.

Agent Manager worktrees usually live under `.sonderr/worktrees/`. Think of each worktree as a separate checkout on its own branch: it enables parallel edits, but dependencies, build output, caches, databases, and generated files can consume significant disk space across many worktrees.
...
|---|---|
| macOS / Linux | `.sonderr/setup-script`, `.sonderr/setup-script.sh` |
| Windows | `.sonderr/setup-script.ps1`, `.sonderr/setup-script.cmd`, `.sonderr/setup-script.bat` |
...
- For port conflicts (`EADDRINUSE`, browser/tests hitting the wrong worktree), inspect the app's dev-server config as well as `.sonderr/run-script`. If fixing it requires application changes, ask whether the user wants the app made configurable or only wants a run-script workaround.
...
Tool permissions: `read`, `edit`, `glob`, `grep`, `list`, `bash`, `task`, `webfetch`, `websearch`, `semantic_search`, `sonderr_memory_save`, `sonderr_memory_recall`, `lsp`, `skill`, `external_directory`, `todowrite`, `todoread`, `question`, `doom_loop`.
...
For example, this configuration will hide all models from the built-in Sonderr Gateway as well as any from the OpenAI provider which may be enabled automatically through environment variables.

```jsonc
{
  "$schema": "https://app.kilo.ai/config.json",
  "disabled_providers": ["sonderr", "openai"],
}
```

The provider ID is the lowercase name used in the `provider/model` format (e.g., `sonderr`, `openai`, `anthropic`, `google`, `groq`).
...
Skills are markdown files at `skills/<name>/SKILL.md` (or `skill/<name>/SKILL.md`) with `name` and `description` in frontmatter. Discovered inside `.sonderr/` and legacy `.sonderr/` directories.
...
|---|---|---|---|
| Switch theme | `<leader>t` | `/themes` | Pick from 35+ built-in themes (sonderr, catppuccin, dracula, github, gruvbox, nord, tokyonight, etc.) |
| Toggle appearance (dark/light) | — | — | Ctrl+P → "Toggle appearance" |

Custom themes: place JSON files in `~/.config/sonderr/themes/` or `.sonderr/themes/`.
...
### Config files (sonderr.json)

| Scope | Path |
|---|---|
| Project | `./sonderr.json`, `./sonderr.jsonc`, `./sonderr.json` (legacy), `./sonderr.jsonc` (legacy) |
| Global | `~/.config/sonderr/sonderr.json`, `~/.config/sonderr/sonderr.jsonc`, `~/.config/sonderr/sonderr.json` (legacy), `~/.config/sonderr/sonderr.jsonc` (legacy), `~/.config/sonderr/config.json` (legacy) |
| Managed | Linux: `/etc/sonderr/`, macOS: `/Library/Application Support/sonderr/`, Windows: `%ProgramData%\sonderr\` — loads `sonderr.json`, `sonderr.jsonc`, `sonderr.json`, `sonderr.jsonc` (enterprise, highest priority) |

Each config directory (`.sonderr/` and legacy `.sonderr/`) can also contain `sonderr.json`, `sonderr.jsonc`, `sonderr.json`, or `sonderr.jsonc`.

### Config directories

Two directory names are scanned: `.sonderr` (canonical) and `.sonderr` (legacy fallback). Both are checked at each level, and `.sonderr` wins when both define the same entry. `.sonderr` directories are not loaded.

- **Project**: walks up from CWD to the git worktree root, checking both directories at each level
- **Home**: `~/.sonderr/` and `~/.sonderr/`
- **XDG global**: `~/.config/sonderr/` (always loaded, lowest file-based precedence)
...
Example: `~/.config/sonderr/command/*.md` (global), `~/.sonderr/command/*.md` (legacy home), and `.sonderr/commands/*.md` (project) all load commands.
...
|---|---|
| `SONDERR_CONFIG` | Path to an additional config file (loaded after global) |
| `SONDERR_CONFIG_DIR` | Path to an additional config directory (appended to search list) |
| `SONDERR_CONFIG_CONTENT` | Inline JSON config string (high precedence, after project dirs) |
| `SONDERR_DISABLE_PROJECT_CONFIG` | Skip all project-level config (files and directories) |
