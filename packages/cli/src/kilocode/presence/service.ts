...
import { EventServiceClient } from "@/sonderr/event-service/client"
import { SONDERR_EVENT_SERVICE_URL } from "@sonderr/sonderr-gateway"
...
const log = Log.create({ service: "sonderr-viewers" })

function inferPlatform(): Platform | undefined {
  const p = process.env.SONDERR_PLATFORM
...
function extract(auth: Auth.Info | undefined): { token: string | undefined; identity: string | undefined } {
  const envKey = process.env.SONDERR_API_KEY?.trim()
...
export namespace SonderrViewers {
  export interface Interface {
    readonly update: (snapshot: ViewerSnapshot) => Effect.Effect<void>
    readonly invalidateAuth: () => Effect.Effect<void>
  }

  export class Service extends Context.Service<Service, Interface>()("@sonderr/SonderrViewers") {}

  export const layer = Layer.effect(
    Service,
    Effect.gen(function* () {
      const auth = yield* Auth.Service
      const SonderrSessions = (yield* Effect.promise(() => import("@/sonderr-sessions/sonderr-sessions"))).SonderrSessions

      const platform = inferPlatform()
      const killSwitch = process.env.SONDERR_DISABLE_PRESENCE === "1"
      // Same endpoint the server envelope hands SonderrClaw; SONDERR_EVENT_SERVICE_URL
      // is a presence-specific override on top of the gateway's EVENT_SERVICE_URL.
      const url = process.env.SONDERR_EVENT_SERVICE_URL || SONDERR_EVENT_SERVICE_URL
      ...
      function pushAttached() {
        ...
        if (!sameArr(union, s.prevAttached)) {
          ...
          SonderrSessions.setAttachedSessions(union)
        }
      }
      ...
      const readAuth = auth.get("sonderr").pipe(Effect.orElseSucceed((): Auth.Info | undefined => undefined))

      const update = Effect.fn("SonderrViewers.update")(function* (snapshot: ViewerSnapshot) {
      ...
      const invalidateAuth = Effect.fn("SonderrViewers.invalidateAuth")(function* () {
        disconnectClient()
      ...
      yield* Effect.addFinalizer(() =>
        Effect.sync(() => {
          ...
          disconnectClient()
          s.viewers.clear()
          SonderrSessions.setAttachedSessions([])
        }),
      )
