...
import { appendTerminalOutput } from "@/sonderr/interactive-terminal/output"
import { model as modelEnv } from "@/sonderr/process/env"
import { Identifier } from "@/id/id"
import { capture, Instance, type InstanceContext } from "@/sonderr/instance"
...
import type { Disp, Proc } from "@sonderr/core/pty/driver"
...
export namespace InteractiveTerminal {
  ...
  class StateService extends Context.Service<StateService, State>()("@sonderr/InteractiveTerminal.State") {}

  class RuntimeService extends Context.Service<
    RuntimeService,
    { readonly get: (ctx: InstanceContext) => Effect.Effect<State, never, Scope.Scope> }
  >()("@sonderr/InteractiveTerminal.Runtime") {}
  ...
  function gate(shell: string, command: string) {
    ...
    return `stty -echo; IFS= read -r __sonderr_gate; stty echo; ${command}`
  }
  ...
  function environment(input: NodeJS.ProcessEnv) {
    ...
    env.SONDERR_TERMINAL = "1"
    env.SONDERR_INTERACTIVE_TERMINAL = "1"
  ...
  async function launch(state: State, input: RunInput) {
    ...
    const { spawn } = await import("@sonderr/core/pty/driver")
