...
import { MessageTable, PartTable } from "@sonderr/core/session/sql"
...
import { baseKey } from "@/sonderr/session-portability/cumulative-diff"
import { extractSessionDiffs, restoreSessionDiffs } from "@/sonderr/session-portability/session-diff-restore"
 ...
 * graph (never closing over sonderrGatewayHandlers group variables) and fails
 * typed errors that the callers translate into their own wire shapes.
 */
export namespace CloudSessionImportInProcess {
  // Missing sonderr credentials (auth absent or no token).
  ...
  const importSessionCore = Effect.fn("CloudSessionImportInProcess.importSessionCore")(function* (sessionId: string) {
    ...
    const info = yield* auth.get("sonderr").pipe(Effect.mapError(() => new Unauthorized()))
