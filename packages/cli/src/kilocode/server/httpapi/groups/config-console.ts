...
import { SonderrTitleIcon } from "@/sonderr/cli/cmd/tui/title-icon"
...
const TuiConfigShape = {
  ...
  title_icon: Schema.optional(SonderrTitleIcon.Value),
...
export const TuiKeybindListResponse = Schema.Struct({ keybinds: Schema.Array(SonderrKeybinds.Info) }).annotate({
...
export const ConfigConsoleApi = HttpApi.make("config-console")
  .add(
    HttpApiGroup.make("config-console")
      .add(
        ...
        HttpApiEndpoint.get("rules", ConfigConsolePaths.rules, {
          ...
          OpenApi.annotations({
            ...
            description: "List project instruction files used by Sonderr and return their current contents.",
          }),
        ),
        ...
        HttpApiEndpoint.patch("modelStateUpdate", ConfigConsolePaths.modelState, {
          ...
          OpenApi.annotations({
            ...
            description: "Patch TUI-compatible model selections shared with Sonderr Console.",
          }),
        ),
      ...
      .annotateMerge(OpenApi.annotations({ title: "config-console", description: "Sonderr Console config routes." }))
