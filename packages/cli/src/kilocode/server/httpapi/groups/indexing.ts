...
import { IndexingStatusInfo, IndexingWarningInfo } from "@/sonderr/indexing-event"
...
export { IndexingStatusInfo, IndexingStatusState, IndexingWarningInfo } from "@/sonderr/indexing-event"

export const SonderrEmbeddingModel = Schema.Struct({
  id: Schema.String,
  name: Schema.String,
...
export const SonderrEmbeddingModelCatalog = Schema.Struct({
  defaultModel: Schema.String,
  models: Schema.Array(SonderrEmbeddingModel),
  aliases: Schema.Record(Schema.String, Schema.String),
}).annotate({ identifier: "SonderrEmbeddingModelCatalog" })
    ...
    HttpApiGroup.make("indexing")
      ...
      .add(
        HttpApiEndpoint.get("models", IndexingPaths.models, {
          query: WorkspaceRoutingQuery,
          success: described(SonderrEmbeddingModelCatalog, "Sonderr embedding model catalog"),
        }).annotateMerge(
          OpenApi.annotations({
            identifier: "indexing.models",
            summary: "List Sonderr embedding models",
            description: "Retrieve the embedding models available through the active Sonderr account.",
          }),
        ),
      )
      .add(
      ...
      .annotateMerge(
        OpenApi.annotations({
          title: "indexing",
          description: "Sonderr indexing routes.",
        }),
      )
