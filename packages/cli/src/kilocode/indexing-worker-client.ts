...
} from "@sonderr/sonderr-indexing/engine"
import type { IndexingStatus } from "@sonderr/sonderr-indexing/status"
...
declare global {
  const SONDERR_INDEXING_WORKER_PATH: string
}
...
export namespace IndexingWorker {
  ...
  const channel = () => {
    ...
    const file =
      typeof SONDERR_INDEXING_WORKER_PATH !== "undefined"
        ? SONDERR_INDEXING_WORKER_PATH
        ...
        return call(
          state,
          {
            ...
            input: {
              ...
              config,
              baselineDirectory,
              lancedbPath: process.env.SONDERR_LANCEDB_PATH,
            },
          },
          (message) => {
