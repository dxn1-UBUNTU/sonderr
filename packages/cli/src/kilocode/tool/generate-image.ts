...
import { SONDERR_OPENROUTER_BASE } from "@sonderr/sonderr-gateway"
import DESCRIPTION from "./generate-image.txt"

const log = Log.create({ service: "tool.generate_image" })

const SONDERR_OPENROUTER_URL = `${SONDERR_OPENROUTER_BASE}/chat/completions`
...
function buildRequest(resolved: ResolvedProvider, prompt: string, model: string, inputImage?: string): ResolvedRequest {
  ...
  if (resolved.organizationId) headers["X-SONDERR-ORGANIZATIONID"] = resolved.organizationId
    ...
    return {
      ...
      execute: (params: Schema.Schema.Type<typeof Parameters>, ctx: Tool.Context) =>
        Effect.gen(function* () {
          ...
          const auth = yield* authSvc.get("sonderr")
          ...
          if (!resolved) {
            return {
              ...
              output:
                "No image generation provider available. Log in to Sonderr or set OPENROUTER_API_KEY, then try again.",
