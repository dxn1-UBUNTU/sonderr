...
import { AgentManager, HostError } from "@/sonderr/agent-manager/service"
...
const log = Log.create({ service: "sonderr-tool-registry" })
type Deps = { agent: Agent.Interface; truncate: Truncate.Interface; indexing?: boolean }
type Loaders = {
  indexing?: () => Promise<{ SonderrIndexing: { ready: () => boolean } }>
  semantic?: () => Promise<Pick<typeof import("@/sonderr/tool/semantic-search"), "SemanticSearchTool">>
}

export namespace SonderrToolRegistry {
  const hint =
  ...
  export function usePatch(input: { modelID: string; family?: string }) {
    if (process.env["SONDERR_E2E_LLM_URL"]) return true
  ...
  /** Resolve Sonderr-specific tool Infos outside any InstanceState, so their Truncate/Agent deps are
  ...
  export function infos(host?: AgentManager.Interface, notebook?: Notebook.Interface) {
    return Effect.gen(function* () {
      ...
      // The notify_user tool depends on SonderrSessions.Service, which the tool-registry layer provides
      // via SonderrSessions.defaultLayer (see src/tool/registry.ts). Grabs the service from the surrounding
  ...
  /** Finalize Sonderr-specific tools into Tool.Defs. Call this inside the InstanceState state Effect —
  ...
  function semanticTool(deps: Deps, loaders: Loaders) {
    return Effect.gen(function* () {
      const ready = yield* deps.indexing === undefined
        ? (() => {
            const indexing = loaders.indexing ?? (() => import("@/sonderr/indexing"))
            return Effect.tryPromise(() => indexing().then((mod) => mod.SonderrIndexing.ready())).pipe(
              Effect.catch((err) =>
                Effect.sync(() => {
      ...
      const semantic = loaders.semantic ?? (() => import("@/sonderr/tool/semantic-search"))
  ...
  export function available(tool: Tool.Def, agent: Agent.Info) {
    if (tool.id === "notify_user") return SonderrSessions.remoteStatus().enabled
    if (tool.id === "send_file") return SonderrSessions.remoteStatus().connected
    if (tool.id !== "interactive_terminal") return true
    return agent.mode === "primary"
  }

  /** Sonderr-specific tools to append to the builtin list */
  export function extra(
    ...
    return [
      ...
      ...(Flag.SONDERR_CLIENT === "vscode" ? [tools.managerModels, tools.manager] : []),
      ...(Flag.SONDERR_CLIENT === "vscode" &&
  ...
  /** Per-turn cache of `SonderrMemory.toolEnabled` keyed by root string, with a short TTL so the
   * step-loop coalesces probes inside a single turn. Cache is invalidated immediately on enable /
   * disable / purge / rebuild via the MemoryEvents bus (subscribed in sonderr/bootstrap.ts). */
  export function memoryToolsEnabled(input: { ctx: MemoryPaths.Ctx }) {
    return Effect.gen(function* () {
      ...
      const enabled = yield* Effect.tryPromise({
        try: () => SonderrMemory.toolEnabled({ ctx: input.ctx }),
        catch: (err) => err,
      }).pipe(
        Effect.catch((err) =>
          Effect.sync(() => {
  ...
  /** Hide Sonderr memory tools from the model when project memory is disabled. */
  export const applyVisibility = Effect.fn("SonderrToolRegistry.applyVisibility")(function* (tools: Tool.Def[]) {
    const ctx = yield* InstanceState.context
    const memoryEnabled = yield* memoryToolsEnabled({ ctx })
    return tools.filter((tool) => {
      if (tool.id.startsWith("sonderr_memory_")) return memoryEnabled
      return true
    })
  })
