...
import { SonderrKeybinds } from "./keybinds"
...
export namespace SonderrTuiConfig {
  ...
  async function projectTarget(input: { directory: string; worktree?: string }) {
    ...
    return path.join(input.directory, ".sonderr", "tui.json")
  }
  ...
  function writable(config: Patch | TuiConfig.Info, defaults = true): Editable {
    ...
    const keybinds: Record<string, string> = defaults
      ? Object.fromEntries(SonderrKeybinds.list().map((item) => [item.id, item.default]))
      : {}
