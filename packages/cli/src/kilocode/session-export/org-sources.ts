...
import { resolveSonderrIndexingAuth } from "@/sonderr/indexing-auth"
...
export async function getAuthOrgId(): Promise<OrgState> {
  try {
    const [cfg, info] = await Promise.all([
      ...
      auth.runPromise((svc) => svc.get("sonderr")),
    ])
    const id = resolveSonderrIndexingAuth({ config: cfg, auth: info }).organizationId
