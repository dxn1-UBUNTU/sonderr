...
export namespace CloudDefaults {
  ...
  export class ModelState extends Context.Service<ModelState, ModelStateInterface>()("@sonderr/CloudModelState") {}

  export const modelStateLayer = Layer.succeed(
    ModelState,
    ModelState.of({ get: () => Effect.promise(() => SonderrModelState.get()) }),
  )
  ...
  function normalize(value: string) {
    return value.startsWith("sonderr/") ? value.slice("sonderr/".length) : value
  }
  ...
  export const resolve = Effect.fn("CloudDefaults.resolve")(function* (input: Input = {}) {
    ...
    return yield* Effect.fail(
      new ResolutionError({
        kind: "model",
        message: "The Sonderr model catalog has no available default model",
      }),
    )
  })
}
