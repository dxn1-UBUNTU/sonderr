import type { CodeIndexManager } from "@sonderr/sonderr-indexing/engine"
...
async function init(request: Extract<Request, { method: "init" }>) {
  ...
  if (request.input.lancedbPath) process.env.SONDERR_LANCEDB_PATH = request.input.lancedbPath
  const [engine, status] = await Promise.all([
    import("@sonderr/sonderr-indexing/engine"),
    import("@sonderr/sonderr-indexing/status"),
  ])
