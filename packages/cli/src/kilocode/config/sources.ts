...
import { SonderrConfig } from "./config"

export namespace SonderrConfigSources {
  ...
  export async function list(input: Input): Promise<Result> {
    ...
    const dirs = Flag.SONDERR_DISABLE_PROJECT_CONFIG ? [] : await configDirSources(input)
    const sources = [
  ...
  async function envFileSources(): Promise<Pending[]> {
    if (!Flag.SONDERR_CONFIG) return []
    return [
      await fileSource({
        kind: "env-file",
        scope: "env",
        label: "SONDERR_CONFIG",
        file: Flag.SONDERR_CONFIG,
        reason: "Explicit config file from SONDERR_CONFIG.",
      }),
    ]
  }
  ...
  async function configDirSources(input: Input): Promise<Pending[]> {
    ...
    const env = Flag.SONDERR_CONFIG_DIR ? [Flag.SONDERR_CONFIG_DIR] : []
    const dirs = unique([Global.Path.config, ...project, ...home, ...env]).filter((dir) =>
      SonderrConfig.isConfigDir(dir, Flag.SONDERR_CONFIG_DIR),
    )
    ...
    for (const dir of dirs) {
      ...
      for (const name of SonderrConfig.ALL_CONFIG_FILES) {
  ...
  function dirScope(dir: string, input: { project: string[]; home: string[] }): Scope {
    if (dir === Flag.SONDERR_CONFIG_DIR) return "env"
  ...
  function envContentSources(): Pending[] {
    ...
    if (process.env.SONDERR_CONFIG_CONTENT) {
      sources.push({
        kind: "env-content",
        scope: "env",
        label: "SONDERR_CONFIG_CONTENT",
        source: "SONDERR_CONFIG_CONTENT",
        exists: true,
        editable: false,
    ...
    if (Flag.SONDERR_CONFIG_DIR) {
      sources.push({
        kind: "runtime-env",
        scope: "env",
        label: "SONDERR_CONFIG_DIR",
        source: "SONDERR_CONFIG_DIR",
        path: Flag.SONDERR_CONFIG_DIR,
        exists: true,
        editable: false,
    ...
    if (Flag.SONDERR_DISABLE_PROJECT_CONFIG) {
      sources.push({
        kind: "runtime-env",
        scope: "env",
        label: "SONDERR_DISABLE_PROJECT_CONFIG",
        source: "SONDERR_DISABLE_PROJECT_CONFIG",
        exists: true,
        editable: false,
    ...
    return [
      {
        kind: "cloud-org",
        scope: "cloud",
        label: "Sonderr Cloud organization config",
        source: `${account.url}/api/config`,
        exists: true,
        editable: false,
        reason: "Active organization config is managed by Sonderr Cloud; values are not exposed here.",
      },
    ]
  }
  ...
  async function managedSources(): Promise<Pending[]> {
    ...
    const files = await Promise.all(
      SonderrConfig.ALL_CONFIG_FILES.map((name) =>
        fileSource({
          kind: "managed-file",
          scope: "managed",
          label: `Managed ${name}`,
  ...
  async function managedPreferenceSources(): Promise<Pending[]> {
    ...
    const files = [
      path.join("/Library/Managed Preferences", user, "ai.sonderr.managed.plist"),
      path.join("/Library/Managed Preferences", "ai.sonderr.managed.plist"),
    ]
    return Promise.all(
  ...
  function runtimeSources(): Pending[] {
    return [
      runtimeSource("SONDERR_PERMISSION", Flag.SONDERR_PERMISSION, "Runtime permission overlay."),
      runtimeSource("SONDERR_DISABLE_AUTOCOMPACT", process.env.SONDERR_DISABLE_AUTOCOMPACT, "Disables automatic compaction."),
      runtimeSource("SONDERR_DISABLE_PRUNE", process.env.SONDERR_DISABLE_PRUNE, "Disables tool-output pruning."),
