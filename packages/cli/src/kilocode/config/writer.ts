...
import { SonderrConfigOverlay } from "./overlay"

export namespace SonderrConfigWriter {
  export type Conflict = {
    ok: false
    code: "target-changed" | "revision-conflict" | "target-not-writable"
    message: string
    target: SonderrConfigOverlay.Target
  }
  ...
  export async function write(input: {
    directory: string
    worktree?: string
    scope: SonderrConfigOverlay.Scope
    ...
    const target = await SonderrConfigOverlay.target(input)
    ...
    const patch = SonderrConfigOverlay.patch({ scope: input.scope, set: input.set, unset: input.unset })
    if (Object.keys(patch).length === 0) return { ok: true, target, changed: false, sandboxChanged: false }
    await mkdir(path.dirname(target.path), { recursive: true })
    await input.beforeWrite?.()
    const checked = await SonderrConfigOverlay.target(input)
    ...
    if (
      expected &&
      SonderrConfigOverlay.revision(checked.path, checked.exists, checked.exists ? before : "") !== expected.revision
    ) {
      return {
        ok: false,
    ...
    return {
      ok: true,
      target: await SonderrConfigOverlay.target(input),
