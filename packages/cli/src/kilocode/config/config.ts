...
import { ConfigErrorV1 as ConfigError } from "@sonderr/core/v1/config/error"
import type { Config } from "../../config/config"
import type { ConfigAgentV1 } from "@sonderr/core/v1/config/agent"
import { ModesMigrator } from "../modes-migrator"
import { fetchOrganizationModes } from "@sonderr/sonderr-gateway"
...
export namespace SonderrConfig {
  const log = Log.create({ service: "sonderr.config" })
  ...
  /** All config file names in precedence order (sonderr + sonderr). */
  export const ALL_CONFIG_FILES = ["sonderr.jsonc", "sonderr.json", "sonderr.jsonc", "sonderr.json"] as const

  /** Config directory suffixes in update-target preference order. */
  export const SONDERR_DIR_SUFFIXES = [".sonderr", ".sonderr"] as const
   ...
   * preference order.
   */
  export const projectConfigFiles = Effect.fn("SonderrConfig.projectConfigFiles")(function* (input: {
    fs: FSUtil.Interface
    directory: string
    worktree?: string
  }) {
    const dirs = yield* input.fs
      .up({ targets: [...SONDERR_DIR_SUFFIXES], start: input.directory, stop: input.worktree })
      .pipe(Effect.orDie)
  ...
  export const updateProjectConfig = Effect.fn("SonderrConfig.updateProjectConfig")(function* (input: {
    fs: FSUtil.Interface
    directory: string
    worktree?: string
    config: Config.Info
    ...
    const file = files.find((item) => existsSync(item)) ?? path.join(input.directory, ".sonderr", "sonderr.jsonc")
   ...
   */
  export const propagateUnset = Effect.fn("SonderrConfig.propagateUnset")(function* (input: {
    fs: FSUtil.Interface
  ...
  /**
   * Load all Sonderr legacy configs (modes, workflows, rules, MCP, ignore).
  ...
  export async function loadLegacyConfigs(input: {
    ...
    try {
      ...
      if (Object.keys(migration.agents).length > 0) {
        ...
        log.debug("loaded sonderr custom modes", {
          count: Object.keys(migration.agents).length,
          modes: Object.keys(migration.agents),
        })
      }
      for (const skipped of migration.skipped) {
        log.debug("skipped sonderr mode", { slug: skipped.slug, reason: skipped.reason })
      }
    } catch (err) {
      log.warn("failed to load sonderr modes", { error: err })
    }

    // Load Sonderr workflows as commands
    try {
      ...
      if (Object.keys(migration.commands).length > 0) {
        ...
        log.debug("loaded sonderr workflows as commands", {
          count: Object.keys(migration.commands).length,
          commands: Object.keys(migration.commands),
        })
      }
    } catch (err) {
      log.warn("failed to load sonderr workflows", { error: err })
    }
    ...
    try {
      ...
      if (migration.instructions.length > 0) {
        ...
        log.debug("loaded sonderr rules", {
          count: migration.instructions.length,
          files: migration.instructions,
        })
      }
      for (const warning of migration.warnings) {
        log.debug("sonderr rules warning", { warning })
      }
    } catch (err) {
      log.warn("failed to load sonderr rules", { error: err })
    }

    // Load Sonderr MCP servers (skip global VSCode extension paths unless running in an editor or Console daemon)
    const skipGlobal = process.env["SONDERR_PLATFORM"] !== "vscode" && process.env["SONDERR_FEATURE"] !== "daemon"
    const mcp = await McpMigrator.loadMcpConfig(input.projectDir, skipGlobal)
    if (Object.keys(mcp).length > 0) {
      result = input.merge(result, { mcp })
    }

    // Load .sonderrignore patterns
    try {
      const permission = await IgnoreMigrator.loadIgnoreConfig(input.projectDir)
      if (Object.keys(permission).length > 0) {
        result = input.merge(result, { permission })
        log.debug("loaded sonderr ignore patterns", {
          hasRead: !!(permission as Record<string, unknown>).read,
          hasEdit: !!(permission as Record<string, unknown>).edit,
        })
      }
    } catch (err) {
      log.warn("failed to load sonderr ignore patterns", { error: err })
    }
  ...
  /**
   * Load organization custom modes from the Sonderr Cloud API.
  ...
  export async function loadOrganizationModes(
    ...
    const warnings: Config.Warning[] = []
    try {
      const sonderr = auth["sonderr"]
      if (sonderr?.type === "oauth" && sonderr.access && sonderr.accountId) {
        const modes = await fetchOrganizationModes(sonderr.access, sonderr.accountId)
        if (modes.length > 0) {
  ...
  export const GLOBAL_CONFIG_FILES = ["config.json", "sonderr.json", "sonderr.jsonc", "sonderr.json", "sonderr.jsonc"]
  ...
  export function isConfigDir(dir: string, flagDir?: string): boolean {
    return dir.endsWith(".sonderr") || dir.endsWith(".sonderr") || dir === flagDir
  }

  // ── Sonderr config migration notice ─────────────────────────────────

  /** Client-neutral docs page describing where Sonderr reads configuration from. */
  export const CONFIG_DOCS_URL = "https://kilo.ai/docs/getting-started/settings"

  /** Stable id for the synthetic "move your sonderr config" notification (used for client-side dismissal). */
  export const SONDERR_NOTIFICATION_ID = "sonderr.local.sonderr-config-detected"

  /**
   * Detect leftover sonderr config directories. Sonderr used to fall back to
   * sonderr configuration but no longer reads `.sonderr` directories.
   * Returns the existing `.sonderr` locations (global + project), highest first.
   */
  export function detectSonderrConfig(input: { directory: string; worktree?: string; scanProject: boolean }): string[] {
    const found: string[] = []

    // Global sonderr config dir (sibling of the sonderr global config dir, e.g. ~/.config/sonderr).
    const globalDir = path.join(path.dirname(Global.Path.config), "sonderr")
    if (existsSync(globalDir)) found.push(globalDir)

    // Project `.sonderr` directories, walked from the working directory up to the worktree root.
    if (input.scanProject) {
      let current = input.directory
      while (true) {
        const candidate = path.join(current, ".sonderr")
  ...
  /**
   * Build the synthetic notification shown when a leftover `.sonderr` config
   ...
   */
  export function sonderrConfigNotification(input: { directory: string; worktree?: string; scanProject: boolean }) {
    const found = detectSonderrConfig(input)
    if (found.length === 0) return undefined
    const suffix = found.length > 1 ? ` (and ${found.length - 1} more)` : ""
    return {
      id: SONDERR_NOTIFICATION_ID,
      title: "Move your sonderr configuration",
      message:
        `Sonderr no longer falls back to sonderr configuration. ` +
        `Found sonderr config at ${found[0]}${suffix}. ` +
        `Move it into a .sonderr directory (project) or ${Global.Path.config} (global).`,
