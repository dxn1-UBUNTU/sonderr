...
import { PlanFollowup } from "@/sonderr/plan-followup"
import { PlanFile } from "@/sonderr/plan-file"
import { SonderrSession } from "@/sonderr/session"
import { SonderrSessionMessageOrder } from "@/sonderr/session/message-order"
...
import { environmentDetails } from "@/sonderr/editor-context"
import { Identifier } from "@/id/id"
import { Filesystem } from "@/util/filesystem"
import NATIVE_PLAN_PROMPT from "@/sonderr/session/native-plan-prompt.txt"
...
import { consumeAutoTitle, markAutoTitle } from "@/sonderr-sessions/rename-adoptions"

export namespace SonderrSessionPrompt {
  ...
  export const abortIntakes = Effect.fn("SonderrSessionPrompt.abortIntakes")(function* (sessionID: SessionID) {
    const entries = [...(intakes.get(sessionID) ?? [])]
  ...
  function supportsPlanFollowup() {
    return ["cli", "vscode", "jetbrains"].includes(Flag.SONDERR_CLIENT)
  }
  ...
  export const cancelTree = Effect.fn("SonderrSessionPrompt.cancelTree")(function* (input: {
    sessionID: SessionID
    ...
    yield* Effect.forEach(
      ...
      (sessionID) =>
        Effect.gen(function* () {
          yield* SonderrSessionPromptQueue.cancel(sessionID)
  ...
  export const recoverDanglingAssistant = Effect.fn("SonderrSessionPrompt.recoverDanglingAssistant")(function* (input: {
    sessionID: SessionID
  ...
  export const recoverProviderFinishError = Effect.fn("SonderrSessionPrompt.recoverProviderFinishError")(
    function* (input: {
      sessionID: SessionID
      status: Pick<SessionStatus.Interface, "get">
      sessions: Pick<Session.Interface, "messages" | "removeMessage">
   ...
   */
  export const recoverFailedAssistant = Effect.fn("SonderrSessionPrompt.recoverFailedAssistant")(function* (input: {
    sessionID: SessionID
    status: Pick<SessionStatus.Interface, "get">
    sessions: Pick<Session.Interface, "messages" | "removeMessage">
  }) {
  ...
  export const askPermission = Effect.fn("SonderrSessionPrompt.askPermission")(function* (input: {
  ...
  export function memoryToolEnabled(input: { ctx: MemoryPaths.Ctx }) {
    return SonderrToolRegistry.memoryToolsEnabled({ ctx: input.ctx })
  }
  ...
  export const memoryInject = Effect.fn("SonderrSessionPrompt.memoryInject")(function* (input: {
    ctx: MemoryPaths.Ctx
    sessionID: SessionID
    record: boolean
    ...
    const verbose =
      input.cache.verbose ??
      (enabled
        ? yield* Effect.tryPromise(() => SonderrMemory.status({ ctx: input.ctx })).pipe(
    ...
    const built =
      cached?.enabled === enabled
        ? cached
        : yield* SonderrSystemPrompt.memoryBlocks({
            ctx: input.ctx,
            sessionID: input.sessionID,
            record: input.record,
            enabled,
   ...
   * OneDrive ReparsePoint directories in some Node versions (sonderr#9755).
   */
  ...
  export function resolveCloseReason(input: {
    sessionID: string
    closeReasons: Map<string, SonderrSession.CloseReason>
    exit: Exit.Exit<any, any>
  }): SonderrSession.CloseReason {
  ...
  export function trimBeforeLastSummary(msgs: MessageV2.WithParts[]): MessageV2.WithParts[] {
    const summary = msgs.reduce<{ msg: MessageV2.WithParts; index: number } | undefined>((latest, msg, index) => {
      ...
      if (!latest || SonderrSessionMessageOrder.compare(msg, latest.msg, index, latest.index) > 0) return { msg, index }
      return latest
    }, undefined)
