...
import type { Logger } from "@sonderr/core/util/log"
import type { Provider } from "@/provider/provider"
import { SonderrSessionOverflow } from "./overflow"

const SAFETY = 2048
const MIN_OUTPUT = 1024

export namespace SonderrLLM {
   ...
   *
   * Like sonderr, the provider is the source of truth: when the last finished
  ...
  export function capOutputTokens(input: {
    ...
    usage?: ReturnType<typeof SonderrSessionOverflow.measure>
    reported?: number
  }): number | undefined {
    ...
    const estimated =
      ...
      SonderrSessionOverflow.measure({ messages: input.messages, tools: input.tools }).normalized
