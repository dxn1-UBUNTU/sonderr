...
import { SonderrSessionMessageOrder } from "./message-order"

export namespace SonderrSessionContinuation {
  export function target(messages: MessageV2.WithParts[]) {
    const latest = SonderrSessionMessageOrder.latest(messages)
    ...
    if (SonderrSessionMessageOrder.compare(user, assistant, messages.indexOf(user), messages.indexOf(assistant)) >= 0)
      return undefined
