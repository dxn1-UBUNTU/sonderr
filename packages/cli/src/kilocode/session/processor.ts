...
import { Telemetry, type ReviewCommand } from "@sonderr/sonderr-telemetry"
import { SessionNetwork } from "@/session/network"
import type { SessionID } from "@/session/schema"
import type { SessionStatus } from "@/session/status"
import { MessageV2 } from "@/session/message-v2"
import { isRecord } from "@/util/record"
import { parseReviewCommand, reviewCommandName } from "@/sonderr/review/command"
import * as Log from "@sonderr/core/util/log"
import { Cause, Effect, Exit } from "effect"
import { Flag } from "@sonderr/core/flag/flag"
import { EffectBridge } from "@/effect/bridge"
import type { LLMEvent, ProviderMetadata, Usage } from "@sonderr/llm"
import type { ProviderV2 } from "@sonderr/core/provider"
import { SessionRetry } from "@/session/retry"
import { computeMetrics as computeMetricsHelper, type TokenRates } from "@/sonderr/session/metrics"
...
export namespace SonderrSessionProcessor {
  const log = Log.create({ service: "session.processor.sonderr" })
  ...
  /**
   * Returns the Sonderr-specific retry policy options (limit + offline handler).
  ...
  export function retryOpts(input: {
    ...
    const limit = Flag.SONDERR_SESSION_RETRY_LIMIT
    return {
