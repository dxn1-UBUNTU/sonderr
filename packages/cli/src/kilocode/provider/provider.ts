...
//
// Sonderr-specific provider logic extracted from packages/cli/src/provider/provider.ts
// to minimize merge conflicts with upstream sonderr.
//
// This module exports patch functions and data that the upstream provider.ts
// calls at well-defined injection points (each marked with sonderr_change).

import { createSonderr, type SonderrProvider, AI_SDK_PROVIDERS, PROMPTS } from "@sonderr/sonderr-gateway"
import { DEFAULT_HEADERS } from "@/sonderr/const"
import { ProviderV2 } from "@sonderr/core/provider"
import { ModelV2 } from "@sonderr/core/model"
import { optionalOmitUndefined } from "@sonderr/core/schema"
...
export const SONDERR_BUNDLED_PROVIDERS: Record<string, () => Promise<(options: any) => BundledSDK>> = {
  "@sonderr/sonderr-gateway": async () => createSonderr as unknown as (options: any) => BundledSDK,
}
...
export const SONDERR_MODEL_SCHEMA_EXTENSIONS = {
...
// ---------------------------------------------------------------------------
// fromModelsDevModel patch — returns sonderr-specific fields
// ---------------------------------------------------------------------------

export function patchModelsDevModel(providerID: string, source: any) {
  return {
    variants: providerID === "sonderr" ? (source.variants ?? {}) : {},
...
// ---------------------------------------------------------------------------
// Config model patch — merges sonderr-specific fields from config + existing
// ---------------------------------------------------------------------------
...
export function patchSonderrProviderPrivacy(provider: { options?: Record<string, any> } | undefined, config: any) {
  if (!provider || config.hide_prompt_training_models !== true) return
  provider.options = { ...provider.options, dataCollection: "deny" }
}

export function sonderrCustomLoaders(dep: CustomDep): Record<string, CustomLoader> {
  return {
    ...
    sonderr: Effect.fnUntraced(function* (input: any) {
      ...
      const hasKey = yield* Effect.gen(function* () {
        ...
        if (config.provider?.["sonderr"]?.options?.apiKey) return true
        return false
      })

      const options: Record<string, string> = {}
      if (env.SONDERR_ORG_ID) {
        options.sonderrOrganizationId = env.SONDERR_ORG_ID
      }
      ...
      return {
        ...
        options,
        async getModel(sdk: SonderrProvider, modelID: string) {
    ...
    // Override sonderr to prevent auto-connecting without credentials
    sonderr: () =>
      Effect.succeed({
        autoload: false,
  ...
  switch (providerID) {
    ...
    case "cerebras":
      result.options.headers = {
        ...result.options.headers,
        "X-Cerebras-3rd-Party-Integration": "sonderr",
      }
      break
    case "azure": {
    ...
    // in provider.ts with single-line sonderr_change markers
  }
}
...
export function sonderrSmallModelPriority(providerID: string): string[] | undefined {
  if (providerID.startsWith("sonderr")) return ["sonderr-auto/small"]
  return undefined
}
