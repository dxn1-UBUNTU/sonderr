...
export const PlanFollowupRuntime = {
  ...
  handover(input: LLM.StreamInput, signal: AbortSignal) {
    return llm().runPromise((svc) => SonderrLLM.text(svc.stream(input)).pipe(Effect.orDie), { signal })
  },
...
export namespace PlanFollowup {
  ...
  async function resolveCodeModel(input: Pick<MessageV2.User, "model">) {
    const state = Flag.SONDERR_CLIENT === "cli" ? await SonderrModelState.get().catch(() => undefined) : undefined
      ...
      questions: [
        {
          ...
          custom: Flag.SONDERR_CLIENT === "cli" || Flag.SONDERR_CLIENT === "jetbrains",
          options: [
            {
