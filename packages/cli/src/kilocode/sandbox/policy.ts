...
import { backendSupport, run as runSandbox, unrestricted, type Profile } from "@sonderr/sandbox"
import { Bus } from "@/bus"
...
export function profile(
  ...
  return {
    ...
    environment: {
      deny: ["SONDERR_CONFIG", "SONDERR_CONFIG_CONTENT", "SONDERR_CONFIG_DIR", "SONDERR_SERVER_PASSWORD", "SONDERR_SERVER_USERNAME"],
      set: {
...
const snapshot = Effect.fn("SandboxPolicy.snapshot")(function* (sessionID: SessionID) {
  ...
  return yield* locked(
    ...
    Effect.gen(function* () {
      ...
      // A session's create-time sonderr.sandbox toggle takes precedence over the config default, so a
