/**
 * SonderrClaw chat panel
 *
 * Renders a scrollable message list, a slash-command autocomplete popup,
 * and a textarea for chatting with the SonderrClaw bot via Sonderr Chat.
...
export function ClawChat(props: {
  ...
  // `bot:sonderrclaw:{sandboxId}` — the bot is the only non-self member in 1:1
  // chats, so we resolve bot ids to the user-configured `botName` (which
  // falls back to "SonderrClaw" upstream) and render any human collaborators
