...
/**
 * SonderrClaw SolidJS reactive helpers — Sonderr Chat protocol.
 *
 * Exposes:
 *   - `createClawStatus`: polled instance status
 *   - `createClawChat`: Sonderr Chat connection + active-conversation state
 *
...
/**
 * Poll the SonderrClaw instance status every `interval` ms.
 */
export function createClawStatus(sdk: SDK, interval = 10_000) {
  ...
  onMount(() => {
    const poll = async () => {
      const res = await sdk.client.sonderr.claw.status().catch(() => null)
...
export function createClawChat(sdk: SDK) {
  ...
  onMount(async () => {
    ...
    const statusRes = await sdk.client.sonderr.claw.status().catch(() => null)
    ...
    const credsRes = await sdk.client.sonderr.claw.chatCredentials().catch((e: unknown) => {
    ...
    if (!envelope.sonderrChatUrl) missing.push("sonderrChatUrl")
    ...
    try {
      log.info("connecting sonderr-chat")
      chat = await connect({
        envelope,
