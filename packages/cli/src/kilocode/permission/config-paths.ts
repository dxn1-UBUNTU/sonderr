...
import { SonderrPaths } from "@/sonderr/paths"

export namespace ConfigProtection {
  /**
   * Config directory prefixes (relative paths, forward-slash normalized).
   * Matches .sonderr/ and legacy .sonderr/ at any depth within the project.
   */
  const CONFIG_DIRS = [".sonderr/", ".sonderr/"]
   ...
   */
  const CONFIG_ROOT_FILES = new Set(["sonderr.json", "sonderr.jsonc", "sonderr.json", "sonderr.jsonc", "AGENTS.md"])
  ...
  export function isRelative(pattern: string): boolean {
    ...
    for (const dir of CONFIG_DIRS) {
      const bare = dir.slice(0, -1) // e.g. ".sonderr"
      // Match at root (e.g. ".sonderr/foo") or nested (e.g. "packages/sub/.sonderr/foo")
  ...
  function configs(): string[] {
    return Array.from(
      new Set([Global.Path.config, process.env.XDG_CONFIG_HOME ? path.join(process.env.XDG_CONFIG_HOME, "sonderr") : ""]),
    ).filter(Boolean)
  }
  ...
  function skillRoot(pattern: string): string | undefined {
    ...
    const roots = [...configs(), ...SonderrPaths.globalDirs()]
  ...
  function fallback(p: string): boolean {
    ...
    return keys(p).some(
      (key) =>
        key.endsWith("/config/sonderr") ||
        key.includes("/config/sonderr/") ||
        key.endsWith("/.config/sonderr") ||
        key.includes("/.config/sonderr/"),
    )
  }
