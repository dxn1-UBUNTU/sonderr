...
/**
 * Read project ID from .sonderr/config.json, falling back to .sonderr/config.json
...
/**
 * Resolve project ID with priority: .sonderr/config.json -> .sonderr/config.json -> git origin URL
 * @returns Normalized project ID or undefined
 */
async function resolveProjectId(dir: string): Promise<string | undefined> {
  // Priority 1: .sonderr/config.json (falls back to .sonderr/config.json)
...
export namespace SonderrProjectID {
  export interface Interface {
    readonly get: () => Effect.Effect<string | undefined>
  }

  export class Service extends Context.Service<Service, Interface>()("@sonderr/SonderrProjectID") {}
...
const { runPromise } = makeRuntime(SonderrProjectID.Service, SonderrProjectID.defaultLayer)

/**
 * Get the project ID for the current Instance context (cached per-project)
 * @returns Normalized project ID or undefined
 */
export async function getSonderrProjectId(): Promise<string | undefined> {
