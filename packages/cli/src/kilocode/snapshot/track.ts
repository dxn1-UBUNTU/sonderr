...
//          persist `"snapshot": false` to `.sonderr/sonderr.json`, and skip. All
...
//     Instead we write the file directly via `SonderrConfig.updateProjectConfig`
...
//
// All of this is Sonderr-specific — the upstream snapshot module remains a thin
...
import { SonderrPartLifecycle } from "@/sonderr/session/part-lifecycle"
import { SonderrConfig } from "@/sonderr/config/config"
import { capture } from "@/sonderr/instance"
...
export namespace SonderrSnapshotTrack {
  ...
  export const TIMEOUT_MS = duration("SONDERR_SNAPSHOT_TRACK_TIMEOUT_MS", 10_000)
  export const TURN_TIMEOUT_MS = duration("SONDERR_SNAPSHOT_TURN_TIMEOUT_MS", 120_000)
  ...
  export const progressPart = (input: {
    ...
    text: input.text,
    synthetic: true,
    metadata: { [SonderrPartLifecycle.key]: "transient" },
  })
