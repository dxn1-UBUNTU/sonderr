import { createSonderrClient, type SonderrClient } from "@sonderr/sdk/v2"
import { UI } from "@/cli/ui"
import { DaemonClient } from "@/sonderr/daemon/client"
import { isBuiltinCommand, type BuiltinCommand } from "@/sonderr/session/builtin-commands"
import { Provider } from "@/provider/provider"
import { Filesystem } from "@/util/filesystem"

export namespace SonderrRun {
  export async function resolveBuiltin(sdk: SonderrClient, command?: string, directory?: string) {
...
export namespace SonderrRunDaemon {
  export type Input = {
    directory?: string
    execute: (client: SonderrClient) => Promise<void>
  }
  ...
  export async function attach(input: Input) {
    ...
    const client = createSonderrClient({ baseUrl: daemon.url, directory: dir, headers: daemon.headers })
