...
import type { SonderrBalance, SonderrProfile } from "@sonderr/sonderr-gateway"
...
// deferral pattern as upstream sonderr#30453). The runtime is created lazily on
...
interface Args {
  ...
  getProfile?: (token: string) => Promise<SonderrProfile>
  getBalance?: (token: string, organizationId?: string) => Promise<SonderrBalance | null>
...
export async function handle(args: Args) {
  ...
  const auth = await get("sonderr")
  const error = args.error ?? UI.error
  const exit = args.exit ?? ((code: number) => (process.exitCode = code))

  if (!auth || auth.type !== "oauth") {
    error("Not authenticated with Sonderr Gateway")
    exit(1)
    return
  }

  const { fetchBalance, fetchProfile } = await import("@sonderr/sonderr-gateway")
