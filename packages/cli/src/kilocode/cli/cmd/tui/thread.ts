...
import { validate as validateSession } from "@/sonderr/cli/cmd/tui"
import { DaemonClient } from "@/sonderr/daemon/client"

type TuiInput = import("@sonderr/tui").TuiInput
...
async function session(input: Input, daemon: DaemonClient.Connection) {
  ...
  const [{ createSonderrClient }, { importCloudSession, reportCloudImportError }] = await Promise.all([
    import("@sonderr/sdk/v2"),
...
export namespace SonderrTuiThreadDaemon {
  ...
  export function workerAuth() {
    const password = Flag.SONDERR_SERVER_PASSWORD ?? randomUUID()
    const username = Flag.SONDERR_SERVER_USERNAME ?? "sonderr"
    return {
      env: { SONDERR_SERVER_USERNAME: username, SONDERR_SERVER_PASSWORD: password },
