 ...
 * changed from the Sonderr Console. Fetched config stays serializable and is resolved into a fresh
...
export namespace SonderrTuiConfig {
  ...
  export function makeStore(initial: TuiConfig.Resolved) {
    ...
    const set: SetTuiConfig = (next) => {
      ...
      const config: TuiConfig.Resolved = {
        ...next,
        title_icon: next.title_icon ?? SonderrTitleIcon.Default,
        attention: {
          ...
          sound_pack: next.attention?.sound_pack ?? "sonderr.default",
