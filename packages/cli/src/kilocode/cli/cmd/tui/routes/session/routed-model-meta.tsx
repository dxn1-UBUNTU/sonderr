...
export namespace RoutedModelMeta {
  ...
  export function label(list: Providers, model: StepFinishPart["model"]) {
    ...
    const id = SonderrRoutedModel.display(model.modelID)
    ...
    return SonderrRoutedModel.displayName(text)
  }

  function routed(model: StepFinishPart["model"], message: Message) {
    if (!model) return undefined
    if (message.providerID !== "sonderr") return undefined
    if (!message.modelID.startsWith("sonderr-auto/")) return undefined
