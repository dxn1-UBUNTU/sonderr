...
const MARKER_START = "# >>> sonderrdev launcher >>>"
const MARKER_END = "# <<< sonderrdev launcher <<<"
...
export const DevSetupCommand = cmd({
  ...
  handler: async (args) => {
    ...
    UI.empty()
    UI.println(`  ${B}Sonderr CLI dev launcher setup${N}`)
    UI.empty()
    ...
    UI.println(
      `  ${action === "install" ? "This will add" : "This will replace"} one line so ${H}sonderrdev${N} runs this checkout from any directory:`,
    )
    UI.empty()
    UI.println(`      ${snippet}`)
    UI.empty()
...
// Deprecated: kept so `sonderr dev-alias` keeps working for scripts/docs.
...
function aliasLine(shell: Shell, repo: string): string {
  const sh = path.join(repo, "bin", "sonderrdev")
  const bat = path.join(repo, "bin", "sonderrdev.cmd")
  const q = (s: string) => `'${s.replaceAll("'", `'\\''`)}'`
  if (shell === "fish") return `alias sonderrdev ${q(sh)}`
  if (shell === "powershell") return `function sonderrdev { & '${bat.replaceAll("'", "''")}' @args }`
  return `alias sonderrdev=${q(sh)}`
}
...
async function applyBlock(rc: string, snippet: string): Promise<{ created: boolean; backup?: string }> {
  ...
  const backup = `${rc}.sonderrdev.bak.${stamp()}`
...
// packages/cli/package.json. Exported for test coverage.
export async function findRepoFrom(start: string): Promise<string | undefined> {
  let dir = start
  while (true) {
    const candidate = path.join(dir, "packages", "sonderr", "package.json")
...
export async function detectRepo(): Promise<string> {
  const hint = process.env.SONDERR_DEV_REPO
  if (hint) return hint
  ...
  // For a local build it's <repo>/packages/cli/dist/<target>/bin/sonderr, so
  // findRepoFrom eventually hits the repo's packages/cli/package.json.
  ...
  throw new Error(
    "cannot locate sonderr source checkout; set SONDERR_DEV_REPO=/path/to/sonderr or run ./bin/sonderrdev dev-setup from the repo",
  )
}
