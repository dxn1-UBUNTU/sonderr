...
import { Struct } from "effect" // sonderr_change
...
import { Info } from "@sonderr/schema/file-diff"
// sonderr_change start
import { Flag } from "@sonderr/core/flag/flag"
import { DiffFull } from "../sonderr/snapshot/diff-full"
import { SonderrSnapshotTrack } from "../sonderr/snapshot/track"
import { SonderrSnapshotSeed } from "../sonderr/snapshot/seed"
import { SonderrSnapshotMaterialize } from "../sonderr/snapshot/materialize"
import type { MessageID, SessionID } from "../session/schema"
import { withStatics } from "@sonderr/core/schema"
import { zod } from "@sonderr/core/effect-zod"
// sonderr_change end

export const Patch = Schema.Struct({
  hash: Schema.String,
  files: Schema.mutable(Schema.Array(Schema.String)),
}).pipe(withStatics((s) => ({ zod: zod(s) }))) // sonderr_change
export type Patch = typeof Patch.Type

// sonderr_change - retain the legacy Zod facade while sharing the canonical schema
export const FileDiff = Info.pipe(withStatics((s) => ({ zod: zod(s) })))
export type FileDiff = typeof FileDiff.Type

// sonderr_change start - lightweight FileDiff without heavy content (patch/before/after) for session summaries
...
const prune = "7.days"
const retention = 7 * 24 * 60 * 60 * 1000 // sonderr_change
...
export const MAX_DIFF_SIZE = 256 * 1024 // sonderr_change
...
export interface Interface {
  ...
  // sonderr_change start - pass prompt context and managed initialization policy
  readonly track: (opts?: {
    sessionID?: SessionID
    messageID?: MessageID
    snapshotInitialization?: SonderrSnapshotTrack.SnapshotInitialization
  ...
  readonly diffFile: (from: string, to: string, file: string) => Effect.Effect<FileDiff | undefined> // sonderr_change - authoritative full-content detail
}

export class Service extends Context.Service<Service, Interface>()("@sonderr/Snapshot") {}
...
export const layer: Layer.Layer<Service, never, Requirements> =
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const flock = yield* EffectFlock.Service // sonderr_change
      ...
      const state = yield* InstanceState.make<State>(
        Effect.fn("Snapshot.state")(function* (ctx) {
          ...
          // sonderr_change start - restoration must fail if deletion fails
          const remove = (file: string) => fs.remove(file, { force: true }).pipe(Effect.orDie)
          // sonderr_change end
          // sonderr_change start - serialize snapshot repositories across CLI and extension processes
          ...
          const enabled = Effect.fnUntraced(function* () {
            ...
            if (Flag.SONDERR_CLIENT === "acp") return false // sonderr_change - ACP clients do not support snapshots
          ...
          const add = Effect.fnUntraced(function* (opts?: { env?: Record<string, string>; root?: boolean }) {
            ...
            // sonderr_change start - initial seeded writes stay protected by the source pin
            yield* stage(
          ...
          const materialize = Effect.fnUntraced(function* () {
            yield* locked(SonderrSnapshotMaterialize.run({ gitdir: state.gitdir, git, fs }).pipe(Effect.orDie)).pipe(
          ...
          const cleanup = Effect.fnUntraced(function* () {
            return yield* locked(
              Effect.gen(function* () {
                ...
                // sonderr_change start - retain snapshots for the same seven-day window as object pruning
                yield* SonderrSnapshotMaterialize.prune({ gitdir: state.gitdir, git, fs }, Date.now() - retention)
                // sonderr_change end
          ...
          const track = Effect.fnUntraced(function* (opts?: Parameters<Interface["track"]>[0]) {
            ...
            return yield* locked(
              Effect.gen(function* () {
                ...
                const seeded: { value?: SonderrSnapshotSeed.Output } = {} // sonderr_change
                ...
                if (!existed) {
                  ...
                  // sonderr_change start - seed all eligible new snapshots from the worktree index
                  seeded.value = yield* SonderrSnapshotSeed.seed({
                    dir: state.directory,
                ...
                // sonderr_change start - pin every snapshot before background materialization
                ...
                if (
                  seed &&
                  !(yield* SonderrSnapshotMaterialize.localize({
                    gitdir: state.gitdir,
                    git,
                    fs,
                ...
                if (
                  seed &&
                  !(yield* SonderrSnapshotMaterialize.localizeTrees(
                    { gitdir: state.gitdir, git, fs, staging: seed.staging },
                    hash,
                  ))
                )
                  return
                if (!(yield* SonderrSnapshotMaterialize.pin({ gitdir: state.gitdir, git, fs }, hash))) return
            ...
            return yield* locked(
              Effect.gen(function* () {
                ...
                if (result.code === 0) {
                  ...
                  return yield* Effect.die(new Error(`Failed to restore snapshot ${snapshot}`)) // sonderr_change
                }
                yield* Effect.logError("failed to restore snapshot", {
                  snapshot,
                  exitCode: result.code,
          ...
          const revert = Effect.fnUntraced(function* (patches: Patch[]) {
            return yield* locked(
              Effect.gen(function* () {
                // sonderr_change start - validate every checkpoint before mutating workspace files
                ...
                const single = Effect.fnUntraced(function* (op: (typeof ops)[number]) {
                  ...
                  // sonderr_change start - never report success for a file that Git could not restore
                  if (tree.code !== 0) {
          ...
          const diffFull = Effect.fnUntraced(function* (from: string, to: string) {
            return yield* locked(
              Effect.gen(function* () {
                ...
                // sonderr_change start - use git patches to avoid blocking the event loop on large diffs
          ...
          yield* materialize() // sonderr_change - resume interrupted snapshot object materialization
          ...
          // sonderr_change start - authoritative full-content detail for editor diff tabs
          ...
          return { cleanup, track, patch, restore, revert, diff, diffFull, diffFile } // sonderr_change - diffFile
        }),
      )

      // sonderr_change start - service-local state and cache avoid leaking across Snapshot layer instances
      const trackState = SonderrSnapshotTrack.makeStates()
      ...
      return Service.of({
        ...
        // sonderr_change start - isolate turn-facing snapshot work from poisoned locks
        track: Effect.fn("Snapshot.track")(function* (opts) {
          const ctx = yield* InstanceState.context
          const guard = trackState(ctx.worktree)
          return yield* SonderrSnapshotTrack.protect({
            inner: SonderrSnapshotTrack.wrap({
        ...
        diffFull: Effect.fn("Snapshot.diffFull")(function* (from: string, to: string) {
          // sonderr_change start - cache full diffs at the service boundary
          if (from === to) return []
...
export const node = LayerNode.make({
  service: Service,
  layer,
  deps: [FSUtil.node, AppProcess.node, Config.node, EffectFlock.node], // sonderr_change
})
