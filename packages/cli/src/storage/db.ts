...
import { DbPreflight } from "@sonderr/core/sonderr/db-preflight" // sonderr_change
...
declare const SONDERR_MIGRATIONS: { sql: string; timestamp: number; name: string }[] | undefined
...
export function getChannelPath(flags: Pick<DatabaseFlags, "disableChannelDb"> = readRuntimeFlags()) {
  if (["latest", "beta", "prod"].includes(InstallationChannel) || flags.disableChannelDb)
    return path.join(Global.Path.data, "sonderr.db") // sonderr_change
  const safe = InstallationChannel.replace(/[^a-zA-Z0-9._-]/g, "-")
  const next = path.join(Global.Path.data, `sonderr-${safe}.db`) // sonderr_change
  const prev = path.join(Global.Path.data, `sonderr-${safe}.db`) // sonderr_change
  if (!existsSync(next) && existsSync(prev)) return prev // sonderr_change
  return next // sonderr_change
}

export const getPath = (flags?: Pick<DatabaseFlags, "disableChannelDb">) => {
  if (Flag.SONDERR_DB) {
    if (Flag.SONDERR_DB === ":memory:" || path.isAbsolute(Flag.SONDERR_DB)) return Flag.SONDERR_DB
    return path.join(Global.Path.data, Flag.SONDERR_DB)
  }
...
export const Client = Object.assign(
  (flags: DatabaseFlags = readRuntimeFlags()): Client => {
    ...
    DbPreflight.assertWritable(dbPath) // sonderr_change - actionable error (and self-heal for sonderr-owned files) instead of an opaque wal_checkpoint crash on read-only db files
    ...
    const entries =
      typeof SONDERR_MIGRATIONS !== "undefined"
        ? SONDERR_MIGRATIONS
        : migrations(path.join(import.meta.dirname, "../../migration"))
    if (entries.length > 0) {
      log.info("applying migrations", {
        count: entries.length,
        mode: typeof SONDERR_MIGRATIONS !== "undefined" ? "bundled" : "dev",
      })
