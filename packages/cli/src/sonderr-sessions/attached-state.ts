// sonderr_change - extracted state machine that separates presence-owned
...
export namespace AttachedState {
  ...
  // sonderr_change - collision-safe union key. The historical "|" join
  ...
  export function create(options: Options): Interface {
    ...
    // sonderr_change - K1 W1: tombstones for ids that have been remotely
    ...
    // sonderr_change - in-flight dedup. Concurrent announce(id) callers
    ...
    // sonderr_change - in-flight detach dedup, mirrors `inflight` for the
    ...
    // sonderr_change end
    let lastSentKey = ""
    // sonderr_change - lifecycle generation. Incremented on reset() so a
    ...
    return {
      setPresence(ids) {
        ...
        // sonderr_change - K1 W1: suppression tombstone. Any id in `next`
      ...
      async announce(id) {
        ...
        // sonderr_change - join a same-kind in-flight announce so concurrent
        ...
        // sonderr_change - K1 W1: if a detach is in flight for this id, we
        ...
        const owned = (async () => {
          ...
          try {
            ...
            // sonderr_change - K1 W1: if reset() ran while this heartbeat was
      ...
      // sonderr_change - K1 W1: session-detach semantics.
      async detach(id) {
        // sonderr_change - join a same-kind in-flight detach so concurrent
        ...
        // sonderr_change - K1 W1: if an announce is in flight for this id we
      ...
      reset() {
        ...
        pending.clear()
        inflight.clear()
        // sonderr_change - K1 W1: also drop the in-flight detach map and
        ...
        suppressed.clear()
        lastSentKey = ""
        // sonderr_change - bump the lifecycle generation so any in-flight
