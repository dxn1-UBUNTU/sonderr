...
import { IngestQueue } from "@/sonderr-sessions/ingest-queue"
import { IngestDrain } from "@/sonderr-sessions/ingest-drain"
import { clearInFlightCache, withInFlightCache } from "@/sonderr-sessions/inflight-cache"
import type * as SDK from "@sonderr/sdk/v2"
import z from "zod"
...
import { RemoteWS } from "@/sonderr-sessions/remote-ws"
import { RemoteSender } from "@/sonderr-sessions/remote-sender"
import { RemoteProtocol } from "@/sonderr-sessions/remote-protocol"
import { buildInstanceAdvertisement } from "@/sonderr-sessions/instance-advertisement"
import { detectPrLink, readPrLinkOverride } from "@/sonderr-sessions/pr-link"
import type { PrLink } from "@/sonderr-sessions/pr-link"
import { AttachedState } from "@/sonderr-sessions/attached-state"
import {
  ...
  markAutoTitle,
  markRenameAdopted,
} from "@/sonderr-sessions/rename-adoptions"
import { SessionStatus } from "@/session/status"
import { Telemetry } from "@sonderr/sonderr-telemetry"
import { Question } from "@/question"
import { Permission } from "@/permission"
import { withTimeout } from "@/util/timeout"
import { Snapshot } from "@/snapshot"
import { cumulativeSessionDiff } from "@/sonderr/session-portability/cumulative-diff"
...
async function provide<R>(input: { directory: string; fn: () => R }): Promise<R> {
  const { provide } = await import("@/sonderr/instance")
  return provide(input)
}

export namespace SonderrSessions {
  export const Event = {
    RemoteStatusChanged: BusEvent.define(
      "sonderr-sessions.remote-status-changed",
      Schema.Struct({
  ...
  export class Service extends Context.Service<Service, Interface>()("@sonderr/SonderrSessions") {}

  const log = Log.create({ service: "sonderr-sessions" })
  ...
  const tokenValidKeyTemplate = "sonderr-sessions:token-valid:"
  let tokenValidKey = tokenValidKeyTemplate + "unknown"

  const tokenKey = "sonderr-sessions:token"
  const orgKey = "sonderr-sessions:org"
  const clientKey = "sonderr-sessions:client"
  const gitUrlKeyPrefix = "sonderr-sessions:git-url:"
  ...
  function agentNotificationTimeoutMs(): number {
    const value = process.env["SONDERR_AGENT_NOTIFICATION_TIMEOUT_MS"]
  ...
  async function authValid(token: string) {
    ...
    return withInFlightCache(tokenValidKey, 15 * 60_000, async () => {
      const response = await fetch(`${SONDERR_API_BASE}/api/user`, {
        headers: {
  ...
  async function sonderrToken() {
    return withInFlightCache(tokenKey, ttlMs, async () => {
      const auth = await runtime.runPromise((svc) => svc.get("sonderr"))
      ...
      const key = process.env["SONDERR_API_KEY"]?.trim()
      if (key) return key
      return undefined
    })
  }
  ...
  async function getClient(): Promise<Client | undefined> {
    return withInFlightCache(clientKey, ttlMs, async () => {
      const token = await sonderrToken()
      if (!token) return undefined

      const valid = await authValid(token)
      if (!valid) return undefined

      const base = process.env["SONDERR_SESSION_INGEST_URL"] ?? "https://ingest.kilosessions.ai"
  ...
  const shareDisabled = process.env["SONDERR_DISABLE_SHARE"] === "true" || process.env["SONDERR_DISABLE_SHARE"] === "1"
  const ingestDisabled =
    process.env["SONDERR_DISABLE_SESSION_INGEST"] === "true" || process.env["SONDERR_DISABLE_SESSION_INGEST"] === "1"
  const debugIngest =
    process.env["SONDERR_DEBUG_SESSION_INGEST"] === "true" || process.env["SONDERR_DEBUG_SESSION_INGEST"] === "1"
  ...
  SonderrShutdown.register(drainIngest)
  ...
  const remoteEnabled = process.env["SONDERR_REMOTE"] === "1"
  let remote: { conn: RemoteWS.Connection; sender: RemoteSender.Sender } | undefined
  let enabling: Promise<void> | undefined
  let remoteSeq = 0
  // sonderr_change - K1 W1: module-level instance advertisement flag.
  // `enableRemote` can be triggered either by the explicit `sonderr remote` command
  // or by bootstrap auto-enable (`SONDERR_REMOTE=1` / `remote_control` config); it
  ...
  // sonderr_change - PR link advertise (plan 8.2/8.4): resolve the worktree PR
  ...
  export const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const reportSessionTitle = Effect.fn("SonderrSessions.reportSessionTitle")(function* (
        sessionID: string,
        title: string,
        opts: { generated: boolean },
      ) {
      ...
      const state = yield* InstanceState.make(
        Effect.fn("SonderrSessions.state")(function* (ctx) {
          ...
          watch(Session.Event.Updated, async (evt) => {
            ...
            try {
              await ingest.sync(sessionID, [
                { type: "sonderr_meta", data: await meta(sessionID, session) },
      ...
      const init = Effect.fn("SonderrSessions.init")(function* () {
        yield* InstanceState.get(state)
      })

      const sendAgentNotification = Effect.fn("SonderrSessions.sendAgentNotification")(function* (
        sessionID: string,
        input: { id: string; message: string },
      ) {
        if (ingestDisabled) {
  ...
  // sonderr_change - DEF-1: default advertisement for every successful
  ...
  export async function enableRemote() {
    ...
    enabling = (async () => {
      const token = await sonderrToken()
      if (!token) {
        throw new Error("Unable to enable remote: no Sonderr credentials found. Run `sonderr auth login`.")
      }

      const valid = await authValid(token)
      if (valid === false) {
        throw new Error("Unable to enable remote: invalid or expired Sonderr credentials. Run `sonderr auth login`.")
      }
      if (valid === undefined) throw new Error("Unable to enable remote: failed to verify Sonderr credentials.")

      const url = (process.env["SONDERR_SESSION_INGEST_URL"] ?? "https://ingest.kilosessions.ai")
        .replace(/^https:\/\//, "wss://")
        .replace(/^http:\/\//, "ws://")

      const [{ RemoteWS }, { RemoteSender }] = await Promise.all([
        import("@/sonderr-sessions/remote-ws"),
        import("@/sonderr-sessions/remote-sender"),
      ])
      ...
      // sonderr_change - K1 W1: capture module-level advertisement so each
      ...
      const getSessions = async (): Promise<RemoteProtocol.Heartbeat> => {
              ...
              [...ids].map((id) =>
                svc.get(SessionID.make(id)).pipe(
                  Effect.map((session) => ({
                    ...
                    gitUrl,
                    gitBranch,
                    // sonderr_change - K1 W1: per-session platform, mirrors
                    // meta()'s resolution order so the live value always agrees
                    // with the session's stored created_on_platform.
                    platform: SonderrSession.resolvePlatform(id) || process.env["SONDERR_PLATFORM"] || "cli",
                  })),
                  Effect.orElseSucceed(() => undefined),
                ),
              ),
        ...
        // sonderr_change - PR link advertise (plan 8.2): resolve once
      ...
      const conn = RemoteWS.connect({
        ...
        onOpen: () => {
          ...
          // sonderr_change - K1 W1: on reconnect, a headless `sonderr remote` host
      ...
      const sender = RemoteSender.create({
        ...
        log,
        // sonderr_change - K1 W1: in-process attach/detach/ownership seams
        // back to SonderrSessions. The sender does NOT spawn a process per
        // session — concurrent remote sessions share this CLI process with
        // per-directory InstanceRef isolation.
        attachSession: (id) => SonderrSessions.attachRemoteSession(id),
        detachSession: (id) => SonderrSessions.detachRemoteSession(id),
        hasSession: (id) => SonderrSessions.hasRemoteSession(id),
        ownedCount: () => SonderrSessions.ownedRemoteSessionCount(),
        cancelPrompt: async (id) => {
          // sonderr_change - K1 W1: dynamic import breaks the module-load cycle
          // (@/session/prompt reads SonderrSessionPrompt at eval; a static edge here
        ...
        // sonderr_change - K1 W1 clone: import a cloud session in-process. The
  ...
  // sonderr_change - K1 W1: instance advertisement setter.
  ...
  // explicit `sonderr remote` command — `enableRemote` itself is coalescing), we
  ...
  // (once a process runs `sonderr remote`, it keeps advertising for its whole
  ...
  // sonderr_change - K1 W1: session-detach semantics. The exit_cli handler
  ...
  // sonderr_change - K1 W1: ownership probe used by the exit_cli handler
  ...
  // sonderr_change - K1 W1: count of "owned" sessions (presence ∪ pending).
  ...
  // zero remaining + no callback (sonderr remote) => keep host alive. Sessions
  ...
  export async function share(sessionId: string) {
    if (ingestDisabled) {
      throw new Error("Session ingest is disabled (SONDERR_DISABLE_SESSION_INGEST=1)")
    }

    if (shareDisabled) {
      throw new Error("Sharing is disabled (SONDERR_DISABLE_SHARE=1)")
    }

    const client = await getClient()
    if (!client) {
      throw new Error("Unable to share session: no Sonderr credentials found. Run `sonderr auth login`.")
    }
  ...
  export async function unshare(sessionId: string) {
    ...
    if (shareDisabled) {
      throw new Error("Unshare is disabled (SONDERR_DISABLE_SHARE=1)")
    }

    const client = await getClient()
    if (!client) {
      throw new Error("Unable to unshare session: no Sonderr credentials found. Run `sonderr auth login`.")
    }
  ...
  async function meta(sessionId?: string, info?: Session.Info | null) {
    const override = sessionId ? SonderrSession.resolvePlatform(sessionId) : undefined
    const platform = override || process.env["SONDERR_PLATFORM"] || "cli"
  ...
  async function getOrgId(sessionId?: string, info?: Session.Info | null): Promise<Uuid | undefined> {
    ...
    const env = process.env["SONDERR_ORG_ID"]
