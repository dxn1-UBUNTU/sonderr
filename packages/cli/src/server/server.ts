...
import { serverUrls } from "@/sonderr/cli/server-urls" // sonderr_change
...
import type { CorsOptions } from "@sonderr/server/cors"
import { lazy } from "@/util/lazy"
import * as SonderrListener from "@/sonderr/server/listener" // sonderr_change
...
export async function listen(opts: ListenOptions): Promise<Listener> {
  ...
  return {
    ...
    port: listener.port,
    url: listener.url,
    urls: listener.urls, // sonderr_change
...
const listenEffect: (opts: ListenOptions) => Effect.Effect<EffectListener, unknown> = Effect.fn("Server.listen")(
  function* (opts: ListenOptions) {
    ...
    return {
      ...
      port: address.port,
      url: listenerUrl,
      urls: serverUrls(opts.hostname, address.port), // sonderr_change
...
function startListener(opts: ListenOptions, port: number) {
  ...
  return SonderrListener.build(listenerLayer(opts, port), scope).pipe(
    // sonderr_change
  ...
  return Effect.gen(function* () {
    ...
    return (close?: boolean) =>
      Effect.gen(function* () {
        ...
        if (close) yield* Effect.promise(() => Pty.shutdown()) // sonderr_change
