...
import { matchLegacySonderrOpenApi } from "@/sonderr/server/httpapi/public"
import * as SonderrServer from "@/sonderr/server/server"
// sonderr_change end
import { SonderrHttpApi } from "./api"
...
const QueryParameterSchemas: Record<string, OpenApiSchema> = {
  ...
  "GET /sonderr/cloud-sessions cursor": { type: "string" },
  "GET /sonderr/cloud-sessions limit": { type: "number" },
  // sonderr_change end
  ...
  "DELETE /session/{sessionID}/message/{messageID} queued": QueryBooleanOpenApi, // sonderr_change
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for the sonderr serve command", // sonderr_change
...
function matchLegacyOpenApi(input: Record<string, unknown>) {
  ...
  restoreAgentManagerNulls(spec) // sonderr_change
  ...
  matchLegacySonderrOpenApi(input) // sonderr_change
  return input
}
...
function applyLegacySchemaOverrides(spec: OpenApiSpec) {
  ...
  // sonderr_change start - preserve model reset sentinels in indexing config patches
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(SonderrHttpApi)
...
// sonderr_change start - preserve nullable Agent Manager move targets in generated SDK schemas
...
function normalizeParameter(param: OpenApiParameter, route: string) {
  ...
  if (param.in === "path") {
    param.schema = pathParameterSchema(route, param.name) ?? stripOptionalNull(param.schema) // sonderr_change
    return
  }
  if (param.in === "query") {
...
export const PublicApi = SonderrHttpApi.annotateMerge(
  OpenApi.annotations({
    title: SonderrServer.DOC_TITLE, // sonderr_change
    version: "1.0.0",
    description: SonderrServer.DOC_DESCRIPTION, // sonderr_change
