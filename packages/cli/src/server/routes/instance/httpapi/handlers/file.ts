...
import { WorkspaceRef } from "@/effect/instance-ref" // sonderr_change - preserve the shared location key shape
import { FileSystem } from "@sonderr/core/filesystem"
import { LocationServiceMap } from "@sonderr/core/location-services" // sonderr_change - reuse the server location map
...
import { Effect, Option } from "effect" // sonderr_change - location map is provided by the server
...
export const fileHandlers = HttpApiBuilder.group(InstanceHttpApi, "file", (handlers) =>
  Effect.gen(function* () {
    ...
    const filesystem = Effect.fnUntraced(function* <A, E, R>(effect: Effect.Effect<A, E, R>) {
      // sonderr_change start - preserve the shared location key shape
    ...
    const findText = Effect.fn("FileHttpApi.findText")(function* (ctx: { query: { pattern: string } }) {
      // sonderr_change start - preserve the released HTTP response shape while Core retains search metadata.
...
) // sonderr_change - reuse the server location map
