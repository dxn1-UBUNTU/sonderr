...
import { EffectBridge } from "@/effect/bridge" // sonderr_change
...
import { Filesystem } from "@/util/filesystem" // sonderr_change
import { Review } from "@/sonderr/review/review" // sonderr_change
import { WorktreeDiff } from "@/sonderr/review/worktree-diff" // sonderr_change
import { WorktreeFamily } from "@/sonderr/worktree-family" // sonderr_change
import { Worktree } from "@/worktree"
import { Effect, Option } from "effect"
import * as HttpServerResponse from "effect/unstable/http/HttpServerResponse"
import { HttpApiBuilder, HttpApiError } from "effect/unstable/httpapi"
import * as Log from "@sonderr/core/util/log" // sonderr_change
import path from "path" // sonderr_change
  ...
  Effect.gen(function* () {
    ...
    const tool = Effect.fn("ExperimentalHttpApi.tool")(function* (ctx: { query: typeof ToolListQuery.Type }) {
      ...
      const list = yield* registry.tools({
        ...
        modelID: model ? ModelV2.ID.make(model.api.id) : ctx.query.model, // sonderr_change
        family: model?.family, // sonderr_change
    ...
    // sonderr_change start - discover Agent Manager and external git worktrees
    ...
    // sonderr_change start - worktree diff endpoints for agent manager
    ...
    const session = Effect.fn("ExperimentalHttpApi.session")(function* (ctx: { query: typeof SessionListQuery.Type }) {
      ...
      // sonderr_change end
      if (roots && directory && !current) return HttpServerResponse.jsonUnsafe([]) // sonderr_change
      const all = yield* sessions.listGlobal({
        projectID, // sonderr_change
        directory: ctx.query.worktrees ? undefined : ctx.query.directory, // sonderr_change
        directories: roots, // sonderr_change
        currentDirectory: directory, // sonderr_change
      ...
      // sonderr_change start - resolve worktree folder name for each session
      const result = sorted
        ? all.map((session) => {
      ...
      return HttpServerResponse.jsonUnsafe(list, {
        headers:
          result.length > limit && list.length > 0 // sonderr_change
