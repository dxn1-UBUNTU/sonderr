import { Image } from "@/image/image" // sonderr_change - classify user image validation defects
import { busyMessage, isBusy } from "@/sonderr/database/sqlite-error" // sonderr_change
import { SonderrSessionHttpApi } from "@/sonderr/server/httpapi/session-fork" // sonderr_change
...
import {
  CommandPayload,
  DeleteMessageQuery, // sonderr_change
  DiffQuery,
  ForkPayload,
  InitPayload,
  ListQuery,
...
export const sessionHandlers = HttpApiBuilder.group(InstanceHttpApi, "session", (handlers) =>
  Effect.gen(function* () {
    ...
    const viewers = yield* SonderrViewers.Service // sonderr_change
    ...
    const diff = Effect.fn("SessionHttpApi.diff")(function* (ctx: {
      ...
      // sonderr_change start - pass full-content detail query fields through to the summary service
    ...
    const forkRaw = SonderrSessionHttpApi.forkRaw(fork) // sonderr_change - carry upstream bodyless full-session fork support
    ...
    const prompt = Effect.fn("SessionHttpApi.prompt")(function* (ctx: {
      ...
      const message = yield* promptSvc
        .prompt({ ...ctx.payload, sessionID: ctx.params.sessionID } as unknown as SessionPrompt.PromptInput) // sonderr_change
        .pipe(
          // sonderr_change start - reject only typed user image validation defects as request errors
    ...
    const promptAsync = Effect.fn("SessionHttpApi.promptAsync")(function* (ctx: {
      ...
      yield* promptSvc
        ...
        .pipe(
          Effect.catchCause((cause) => {
            if (Cause.hasInterruptsOnly(cause)) return Effect.void // sonderr_change - Stop is not an error
            return Effect.gen(function* () {
              const error = Cause.squash(cause)
              // sonderr_change start - keep SQLite lock failures out of local CLI logs
              ...
              yield* events.publish(Session.Event.Error, {
                sessionID: ctx.params.sessionID,
                error: busy // sonderr_change
                    ? new NamedError.Unknown({ message: busyMessage }).toObject() // sonderr_change
                    : new NamedError.Unknown({ message: Cause.pretty(cause) }).toObject(), // sonderr_change
              })
            })
          }),
    ...
    const deleteMessage = Effect.fn("SessionHttpApi.deleteMessage")(function* (ctx: {
      params: { sessionID: SessionID; messageID: MessageID }
      query: typeof DeleteMessageQuery.Type // sonderr_change
    }) {
      yield* requireSession(ctx.params.sessionID)
      // sonderr_change start - allow deleting prompts that are queued behind the active turn
      const remove = yield* ctx.query.queued === true
        ? SonderrSessionPromptQueue.drop(ctx.params.sessionID, ctx.params.messageID)
    ...
    return handlers
      ...
      .handleRaw("fork", forkRaw) // sonderr_change - carry upstream bodyless full-session fork support
      .handle("abort", abort)
      .handle("init", init)
      .handle("share", share)
      ...
      .handle("viewed", viewed) // sonderr_change
  }),
)
