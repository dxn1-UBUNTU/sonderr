...
import { forkTargetDirectory } from "@/sonderr/server/routes/fork-routing" // sonderr_change - fork honors explicit target directory
...
>()("@sonderr/ExperimentalHttpApiWorkspaceRouteContext") {}
...
>()("@sonderr/ExperimentalHttpApiWorkspaceRouting") {}
...
function configuredWorkspaceID(): WorkspaceV2.ID | undefined {
  return Flag.SONDERR_WORKSPACE_ID ? WorkspaceV2.ID.make(Flag.SONDERR_WORKSPACE_ID) : undefined
}
...
function defaultDirectory(request: HttpServerRequest.HttpServerRequest, url: URL): string {
  return url.searchParams.get("directory") || request.headers["x-sonderr-directory"] || process.cwd()
}
...
function planRequest(
  ...
  return Effect.gen(function* () {
    ...
    // sonderr_change start - a fork targeting an explicit directory (e.g. a worktree) must not inherit the source session's directory
