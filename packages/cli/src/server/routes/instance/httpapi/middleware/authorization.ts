...
} from "@sonderr/server/middleware/authorization"

const AUTH_TOKEN_QUERY = "auth_token"
const UNAUTHORIZED = 401
const WWW_AUTHENTICATE = 'Basic realm="Secure Area"'
// sonderr_change start - require auth for high-risk permission toggles even when global auth is optional
const REQUIRED_AUTH_PATHS = new Set(["/permission/allow-everything", "/sonderr/snapshot/remove"])
// sonderr_change end
...
export class Authorization extends HttpApiMiddleware.Service<Authorization>()(
  "@sonderr/ExperimentalHttpApiAuthorization",
  {
...
export class PtyConnectAuthorization extends HttpApiMiddleware.Service<PtyConnectAuthorization>()(
  "@sonderr/ExperimentalHttpApiPtyConnectAuthorization",
  {
    error: HttpApiError.UnauthorizedNoContent,
  },
) {}
...
function validateCredential<A, E, R>(
  ...
  force = ServerAuth.required(config), // sonderr_change - allow endpoint-specific required auth
) {
  return Effect.gen(function* () {
    if (!force) return yield* effect // sonderr_change
...
// sonderr_change start - fail closed for high-risk unauthenticated endpoints
...
export const authorizationLayer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    return Authorization.of((effect) =>
      Effect.gen(function* () {
        const request = yield* HttpServerRequest.HttpServerRequest
        const url = new URL(request.url, "http://localhost") // sonderr_change - inspect endpoint-specific auth policy
        if (!guarded(url, config)) return yield* effect // sonderr_change
        return yield* credentialFromRequest(request).pipe(
          Effect.flatMap((credential) => validateCredential(effect, credential, config, true)), // sonderr_change
        )
      }),
    )
  }),
