...
import type { AssistantMessage, Message, SonderrClient, SessionMessageResponse } from "@sonderr/sdk/v2"
...
export const AuthMethodID = "sonderr-login" // sonderr_change
...
export class Service extends Context.Service<Service, Interface>()("@sonderr/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `sonderr auth login` in the terminal",
      name: "Login with Sonderr",
      id: AuthMethodID,
    }
    // sonderr_change end
...
function makeDirectoryService(sdk: SonderrClient) {
...
function makeUsageService(sdk: SonderrClient) {
...
async function loadDirectorySnapshot(sdk: SonderrClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, sonderr provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const sonderrProvider = providers[ProviderV2.ID.make("sonderr")] // sonderr_change
  const sonderrModel = sonderrProvider ? Provider.sort(Object.values(sonderrProvider.models))[0] : undefined // sonderr_change
  if (sonderrProvider && sonderrModel) return { providerID: sonderrProvider.id, modelID: sonderrModel.id } // sonderr_change
...
function promptErrorMessage(error: AssistantError) {
  ...
  return "Sonderr prompt failed" // sonderr_change - user-visible ACP error
}
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Sonderr service failure", service })
}
