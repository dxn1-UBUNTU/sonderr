...
import type { Event, SonderrClient } from "@sonderr/sdk/v2"
import { applyPatch } from "diff"
import { exists, readText } from "@/util/filesystem"
import type { ACPSession } from "./session"
import { pendingToolCall, toLocations, type ToolInput } from "./tool"
import { Effect } from "effect"
import { SkillShellPrompt } from "@/sonderr/acp/permission" // sonderr_change
...
export class Handler {
  ...
  private async process(event: PermissionEvent) {
    ...
    const skillShell = SkillShellPrompt.is(permission.metadata) // sonderr_change - skill batches list commands and never persist
    const temporary = skillShell || permission.metadata?.["sandboxEscalation"] === true // sonderr_change
    const title = skillShell
      ? SkillShellPrompt.title
      : temporary
        ? "Allow Git operation outside the sandbox"
        : undefined // sonderr_change
    const result = await this.input.connection
      .requestPermission({
        ...
        toolCall: await permissionToolCall({
          ...
          title, // sonderr_change
        }),
        options: temporary ? SkillShellPrompt.options : permissionOptions, // sonderr_change
      })
      .catch(async () => {
        await this.reply(permission.id, "reject", session.cwd)
        return undefined
    ...
    await this.reply(permission.id, reply, session.cwd, true) // sonderr_change - human selected via requestPermission
  }

  private async reply(requestID: string, reply: Reply, directory: string, interactive = false) {
    // sonderr_change - interactive param
    await this.input.sdk.permission.reply({
      requestID,
      reply,
      directory,
      interactive, // sonderr_change
    })
  }
...
async function permissionToolCall(input: {
  ...
  readonly title?: string // sonderr_change - skill-shell batches title the prompt themselves
  ...
  const toolCall = pendingToolCall({
    ...
    state: {
      input: input.input,
      title: input.title ?? permissionTitle(input.toolName, input.input), // sonderr_change
    },
  })
