...
import { context as instanceContext } from "@/project/instance-context" // sonderr_change
...
export function attach<A, E, R>(effect: Effect.Effect<A, E, R>): Effect.Effect<A, E, R> {
  ...
  // sonderr_change start - bridge legacy AsyncLocalStorage instance context into Effect runtimes
...
export function makeRuntime<I, S, E>(service: Context.Service<I, S>, layer: Layer.Layer<I, E>) {
  ...
  return {
    ...
    // sonderr_change start - allow Sonderr-owned service runtimes to release persistent resources
    dispose: async () => {
      const current = rt
      rt = undefined
