...
import { EventV2 } from "@sonderr/core/event" // sonderr_change
import { ProjectV2 } from "@sonderr/core/project" // sonderr_change
import { ProjectCopy } from "@sonderr/core/project/copy" // sonderr_change
import { MoveSession } from "@sonderr/core/control-plane/move-session" // sonderr_change
import { PtyTicket } from "@sonderr/core/pty/ticket" // sonderr_change
import { Pty } from "@sonderr/core/pty" // sonderr_change

// sonderr_change start - retain Sonderr runtime services in the upstream node graph
const memory = LayerNode.make({ service: MemoryService.Service, layer: MemoryService.layer, deps: [] })
const sonderr = LayerNode.group([Credential.node, ModelCache.node, AgentManager.node, Notebook.node, memory])
// sonderr_change end
...
export const AppLayer = AppNodeBuilderV1.build(
  LayerNode.group([
    ...
    ShareNext.node,
    SessionShare.node,
    // sonderr_change start - the app runtime must provide these; the v2 handlers resolve them
