...
import { SonderrRunner } from "@/sonderr/effect/runner" // sonderr_change
...
export const make = <A, E = never>(
  ...
  // sonderr_change start - do not let work publish busy before the Running state is committed
  const startRun = (work: Effect.Effect<A, E>, done: Deferred.Deferred<A, E | Cancelled>) => {
    const id = next()
    return SonderrRunner.start({
      work,
      scope,
  ...
  // sonderr_change start - open work only after the Running state is committed
    ...
    SynchronizedRef.modifyEffect(
      ...
      Effect.fnUntraced(function* (st) {
        ...
        if (st._tag === "ShellThenRun" && st.shell.id === id) {
          return yield* SonderrRunner.commit(startRun(st.run.work, st.run.done), Effect.void)
        }
      ...
      Effect.fnUntraced(function* (st) {
        switch (st._tag) {
          ...
          case "Idle": {
            ...
            return yield* SonderrRunner.commit(startRun(work, done), awaitDone(done))
          }
        }
      }),
    ).pipe(Effect.flatten)
