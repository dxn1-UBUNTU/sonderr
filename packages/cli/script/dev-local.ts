...
// Launch the sonderr CLI dev build against a locally running cloud dev server.
//   bun dev:local <project-dir> [--cloud <dir>] [--no-ingest] [--no-events] [--print] [-- <sonderr args>]
//
// Reads ports from <cloud>/dev/logs/manifest.json (+ .dev-port), probes the web
// server, and points the CLI at it (SONDERR_API_URL / SONDERR_SESSION_INGEST_URL /
// EVENT_SERVICE_URL).
// Auth/config/state/cache are isolated under ~/.sonderr-dev so it can't clash with
// your main sonderr install; real HOME is kept so git/ssh still work.
...
const sonderr = path.resolve(import.meta.dir, "../../..")
const home = path.join(os.homedir(), ".sonderr-dev")
...
async function main() {
  ...
  env.SONDERR_API_URL = `http://localhost:${webPort}`
  env.SONDERR_DEV_CWD = project
  env.SONDERR_DISABLE_AUTOUPDATE = "1"
  if (ingestPort) env.SONDERR_SESSION_INGEST_URL = `http://localhost:${ingestPort}`
  else env.SONDERR_DISABLE_SESSION_INGEST = "1"
  if (eventsPort) {
    env.EVENT_SERVICE_URL = `ws://localhost:${eventsPort}`
    delete env.SONDERR_DISABLE_PRESENCE
    delete env.SONDERR_EVENT_SERVICE_URL
  } else env.SONDERR_DISABLE_PRESENCE = "1"
  ...
  process.exit(await Bun.spawn({ cmd: ["bun", "run", "--cwd", "packages/cli", "--conditions=browser", "src/index.ts", ...pass], cwd: sonderr, env, stdio: ["inherit", "inherit", "inherit"] }).exited)
}
