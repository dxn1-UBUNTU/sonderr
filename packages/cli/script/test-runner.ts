...
import { TestProfile } from "./sonderr/test-profile"
import { TestShard } from "./sonderr/test-shard"
import { TestCli } from "./sonderr/test-cli"
import { remove } from "../test/sonderr/cleanup"
...
if (argv.includes("--help") || argv.includes("-h")) {
  console.log(
    [
      ...
      "  --concurrency <N>    Max parallel processes (default: min(4, CPU count), env: SONDERR_TEST_CONCURRENCY)",
      "  --timeout <ms>       Per-test timeout passed to bun test (default: 60000)",
      "  --file-timeout <ms>  Per-file process timeout (default: 300000, env: SONDERR_TEST_FILE_TIMEOUT)",
      "  --retries <N>        Extra attempts for failing files (default: 1)",
      "  --profile <name>     Run a curated test profile (env: SONDERR_TEST_PROFILE)",
      "  --shard <N/M>        Run one balanced file shard (env: SONDERR_TEST_SHARD)",
      "  --update-durations   After a full run, refresh script/sonderr/test-durations.json",
...
const updateDurations = argv.includes("--update-durations") // sonderr_change
const verbose = argv.includes("--verbose")
const dots = !verbose && (ci || argv.includes("--dots"))
// Cap concurrency at 4 even on bigger runners: the bottleneck is shared
// resources (ports, global filesystem like ~/.local/share/sonderr), not CPU.
// Eight parallel processes was triggering port/FS races, not going faster.
// sonderr_change start - allow CI to lower concurrency via env. On the 4-vCPU
...
// `SONDERR_TEST_CONCURRENCY` lets the workflow throttle Windows without affecting the
// local default. An explicit `--concurrency` flag wins.
const concurrencyEnv = (() => {
  const raw = process.env.SONDERR_TEST_CONCURRENCY?.trim()
  if (!raw) return undefined
  const value = Number(raw)
  if (!Number.isSafeInteger(value) || value < 1) {
    console.error(`Invalid SONDERR_TEST_CONCURRENCY "${raw}"; expected a positive integer`)
    process.exit(2)
  }
  return value
})()
...
// sonderr_change start - allow CI to raise the per-file kill deadline via env. On Windows,
...
const fileTimeoutEnv = (() => {
  const raw = process.env.SONDERR_TEST_FILE_TIMEOUT?.trim()
  if (!raw) return undefined
  const value = Number(raw)
  if (!Number.isSafeInteger(value) || value < 1) {
    console.error(`Invalid SONDERR_TEST_FILE_TIMEOUT "${raw}"; expected a positive integer (ms)`)
    process.exit(2)
  }
  return value
})()
...
const env = process.env.SONDERR_TEST_PROFILE?.trim() || undefined
if (flag && env && flag !== env) {
  console.error(`Conflicting test profiles: --profile=${flag}, SONDERR_TEST_PROFILE=${env}`)
  process.exit(2)
}
const profile = flag ?? env
const shardFlag = text("shard")
const shardEnv = process.env.SONDERR_TEST_SHARD?.trim() || undefined
if (shardFlag && shardEnv && shardFlag !== shardEnv) {
  console.error(`Conflicting test shards: --shard=${shardFlag}, SONDERR_TEST_SHARD=${shardEnv}`)
  process.exit(2)
}
...
const matched =
  patterns.length > 0
    ...
    : selected
const candidates = patterns.length > 0 && !profile ? matched : matched.filter((file) => !skipped.has(file)) // sonderr_change
...
// sonderr_change start - shard by estimated DURATION, not file size. File size is a poor
...
const DURATION_HINTS: Record<string, number> = {
  "sonderr/server/config-overlay.test.ts": 270_000,
  ...
  "sonderr/background-process.test.ts": 94_000,
  "provider/provider.test.ts": 90_000,
  "sonderr/indexing-startup.test.ts": 88_000,
  "sonderr/daemon.test.ts": 65_000,
...
const measuredDurations: Record<string, number> = await Bun.file(
  path.join(root, "script", "sonderr", "test-durations.json"),
)
  .json()
  .catch(() => ({}))
...
// sonderr_change start - fast tier: run isolation-safe test files in ONE shared `bun test`
...
const FAST_TIERS: Record<string, string[]> = {
  ...
  "fast-tier-sonderr": [
    "sonderr/config/",
    "sonderr/memory/",
    // sonderr/permission/ stays per-file: permission-origins asserts the merged config has
  ...
  "fast-tier-sonderr-sessions": ["sonderr/session-export/", "sonderr/session/", "sonderr/sessions/"],
  "fast-tier-sonderr-tools": ["sonderr/anaconda-desktop/", "sonderr/cloud/", "sonderr/tool/"],
  "fast-tier-cli": ["cli/"],
...
const BATCH_EXCLUDES = [
  ...
  "sonderr/background-process.test.ts",
  "sonderr/daemon.test.ts", // spawns real daemon subprocesses, races wall-clock deadlines
  "sonderr/instance-vcs-watcher.test.ts",
  "sonderr/issue-8656-stall.test.ts",
...
const SONDERR_ROOT_TIERS = 8
const sonderrRootTier = (file: string) => {
  if (!file.startsWith("sonderr/")) return undefined
  if (file.slice("sonderr/".length).includes("/")) return undefined
  let hash = 0
  for (let i = 0; i < file.length; i++) hash = (hash * 31 + file.charCodeAt(i)) | 0
  return `fast-tier-sonderr-root-${Math.abs(hash) % SONDERR_ROOT_TIERS}`
}
...
type Result = {
  ...
  duration: number
  timedout: boolean
  deadline: number // sonderr_change - the kill deadline actually applied (batches get a roomier one)
  attempts: number
}
...
const xmldir = ci ? path.join(os.tmpdir(), `sonderr-junit-${process.pid}`) : ""
...
async function run(file: string): Promise<Result> {
  // sonderr_change start - a fast-tier pseudo-file expands to all of its batch members in
  ...
  const fileDeadline = members ? Math.max(deadline, 600_000) : deadline // sonderr_change
  ...
  const code = await Promise.race([
    ...
    Bun.sleep(fileDeadline).then(() => ({ timedout: true, value: -1 })), // sonderr_change
  ]).then(async (result) => {
    if (result.timedout) {
      killed.value = true
  ...
  return {
    ...
    deadline: fileDeadline, // sonderr_change
    attempts: 1,
  }
}
...
// sonderr_change start - report the batches so shard logs stay interpretable
for (const [name, members] of batches) {
...
// after everything else finished. Heaviest-first keeps the tail short. sonderr_change
const queue = TestShard.order(files, shardWeight)

// sonderr_change start - a flaky batch names only its pseudo-file; pull the members that
...
const workers = Array.from({ length: Math.min(concurrency, files.length) }, async () => {
  while (queue.length > 0 && !stopped.value) {
    ...
    while (!result.passed && !result.timedout && result.attempts <= retries && !stopped.value) {
      const members = batches.get(file) // sonderr_change
      if (members) flakyMembers.set(file, failedMembersOf(result.stdout, members)) // sonderr_change
...
if (flaky.length > 0) {
  ...
  if (process.env.GITHUB_ACTIONS === "true") {
    for (const r of sorted) {
      // sonderr_change start - annotate a flaky batch's failing members, not the pseudo-file
      if (batches.has(r.file)) {
        for (const member of flakyMembers.get(r.file) ?? []) {
          console.log(
            `::warning file=packages/cli/test/${member},title=Flaky test file (in ${r.file})::passed on attempt ${r.attempts} of ${retries + 1}`,
          )
        }
        continue
      }
      // sonderr_change end
      const repo = `packages/cli/test/${r.file}`
...
// sonderr_change start - refresh the measured shard weights from this run. Only a full,
...
if (updateDurations) {
  if (patterns.length > 0 || profile || shard) {
    ...
    console.log(
      `\nUpdated script/sonderr/test-durations.json: ${Object.keys(fresh).length} re-measured, ${Object.keys(merged).length} total`,
    )
  }
}
// sonderr_change end
...
async function cleanup(pid: number) {
  const dir = path.join(os.tmpdir(), `sonderr-test-data-${pid}`)
