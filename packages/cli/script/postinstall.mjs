...
// sonderr_change start - variant detection matching bin/sonderr logic
const platformMap = {
  darwin: "darwin",
  linux: "linux",
  win32: "windows",
...
const sourceBinary = platform === "windows" ? "sonderr.exe" : "sonderr"
const targetBinary = path.join(__dirname, "bin", ".sonderr")
...
// sonderr_change start - copy runtime resources next to cached binary
function copyResources(source) {
  ...
  const worker = path.join(path.dirname(source), "sonderr-sandbox-mutation-worker.js")
  if (fs.existsSync(worker)) fs.copyFileSync(worker, path.join(__dirname, "bin", "sonderr-sandbox-mutation-worker.js"))
}
...
function main() {
  ...
  for (const name of packageNames()) {
    try {
      ...
      const temp = fs.mkdtempSync(path.join(os.tmpdir(), "sonderr-install-"))
      try {
  ...
  throw new Error(
    `It seems your package manager failed to install the right Sonderr CLI package. Try manually installing ${packageNames()
...
try {
  ...
  console.error("Failed to setup sonderr binary:", error.message)
  process.exit(1)
}
