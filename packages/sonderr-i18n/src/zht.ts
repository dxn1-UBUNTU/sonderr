export const dict = {
  ...
  "provider.connect.sonderrGateway.byok.prefix": "如需更多使用統計資訊，請",
  "provider.connect.sonderrGateway.byok.link": "透過 Sonderr's Gateway 進行 BYOK",
  ...
  "marketplace.install.about.mcp": "MCP 伺服器為 Sonderr 提供用於處理外部服務或本機程式的額外工具。",
  "marketplace.install.about.agent": "智能體會新增一個具有專屬指示和權限的可重複使用角色。",
  "marketplace.install.about.skill": "技能會新增特定任務的指示和資源，Sonderr 可在需要時載入它們。",
  "marketplace.install.mcp.warning":
    "MCP 伺服器可以執行本機命令或連線至外部服務。除非你的權限允許自動使用，否則 Sonderr 會在使用其工具前請求許可。",
  ...
  "snapshot.slowRepo.answer.disable.description":
    "關閉本專案的 Sonderr 快照。你將失去對 Sonderr 變更的撤銷/重做，但 git 仍會追蹤所有內容。",
