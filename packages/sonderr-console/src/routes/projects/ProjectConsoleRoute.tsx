...
import { Badge } from "@sonderr/sonderr-web-ui/badge"
import { Button } from "@sonderr/sonderr-web-ui/button"
import { Card } from "@sonderr/sonderr-web-ui/card"
import { Icon } from "@sonderr/sonderr-web-ui/icon"
import { ResizeHandle } from "@sonderr/sonderr-web-ui/resize-handle"
import { Spinner } from "@sonderr/sonderr-web-ui/spinner"
import { File } from "@sonderr/sonderr-web-ui/file"
import { FileComponentProvider } from "@sonderr/sonderr-web-ui/context/file"
import { SessionReview, type SessionReviewDiffStyle } from "@sonderr/sonderr-web-ui/session-review"
...
export function ProjectConsoleRoute() {
  ...
  const [selected, setSelected] = createSignal(window.localStorage.getItem(`sonderr.console.${params.project}.dir`) ?? "")
  const [active, setActive] = createSignal(window.localStorage.getItem(`sonderr.console.${params.project}.pty`) ?? "")
  ...
  function labelKey(dir: string) {
    return `sonderr.console.${project()}.worktree.${encodeURIComponent(dir)}.label`
  }
  ...
  function remember(dir: string, pty?: string) {
    window.localStorage.setItem(`sonderr.console.${project()}.dir`, dir)
    if (pty) {
      window.localStorage.setItem(`sonderr.console.${project()}.pty`, pty)
      return
    }
    window.localStorage.removeItem(`sonderr.console.${project()}.pty`)
  }
  ...
  function submitEditor() {
    ...
    run("Creating worktree", async () => {
      ...
      window.localStorage.setItem(`sonderr.console.${project()}.dir`, next.directory)
    })
  }
  ...
  function addSession(item = current()) {
    ...
    const label = `Sonderr ${terminalsFor(item.dir).length + 1}`
      ...
      <main class="project-console-main">
        ...
        <Show when={!terminal() && !snap.loading && !snap.error && !failure()}>
          <div class="project-terminal-empty">
            ...
            <span>Use + next to a worktree to start Sonderr CLI.</span>
          </div>
        </Show>
      </main>
