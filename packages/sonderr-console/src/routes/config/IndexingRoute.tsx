...
import type { IndexingConfig } from "@sonderr/sdk/v2/client"
...
export function IndexingRoute() {
  ...
  const sonderrModels = createMemo<SelectOption<string>[]>(() => {
    const models = catalog()?.models ?? []
    if (models.length === 0) return [{ value: "", label: "No Sonderr embedding models available", disabled: true }]
    return models.map((model) => ({
      value: model.id,
      label: `${model.name} (${model.note ? `${model.note}, ` : ""}${model.dimension}d)`,
    }))
  })
  const sonderrModel = createMemo(() => {
    const data = catalog()
    if (!data) return ""
        ...
        <section class="builder-form agent-builder-stack">
          ...
          <Card class="ui-card agent-builder-card" padding={0}>
            ...
            <div class="ui-form agent-builder-form">
              <FieldCard
                label="Provider"
                description="Automatic uses Sonderr when signed in, otherwise the provider runtime default."
                actions={
                  <SourceBadge
              ...
              <FieldCard
                ...
                description={
                  provider() === "sonderr"
                    ? "Select a Sonderr-hosted embedding model."
              ...
              >
                <input
                  ...
                  disabled={Boolean(ctx.saving()) || provider() === "sonderr"}
                  onInput={(event) => number("dimension", event.currentTarget.value)}
                />
              </FieldCard>

              <Show when={provider() === "sonderr"}>
                <div class="indexing-note">
                  Sonderr embeddings use the account currently signed in to this Sonderr server. Model dimensions are supplied
                  by the catalog.
                </div>
              </Show>
                ...
                fallback={
                  ...
                  >
                    <input
                      ...
                      placeholder="Default Sonderr state directory"
                      disabled={Boolean(ctx.saving())}
                      onInput={(event) =>
