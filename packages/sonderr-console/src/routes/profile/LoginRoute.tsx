...
import { completeSonderrLogin, loadSonderrProfile, startSonderrLogin, type ProjectQuery } from "../../client"
  ...
  function start(input: ProjectQuery | undefined = server.query()) {
    ...
    void startSonderrLogin(input)
      .then((info) => {
        ...
        return completeSonderrLogin(input, ctl.signal).then(() => true)
      })
      .then((ok) => {
        if (!ok || attempt() !== rev) return
        setAuth({ status: "success" })
        void loadSonderrProfile(input)
    ...
    <section class="route-empty">
      <div class="profile-login-page">
        <header class="profile-login-header">
          <p class="eyebrow">Sonderr Login</p>
          <h1>Login to Sonderr</h1>
          <p>Authorize this Sonderr Console through the same device auth flow used by the editor clients.</p>
        </header>
        ...
        <Show when={!server.query() && !server.discoverable()}>
          <Card class="profile-login-card" variant="warning">
            ...
            <p>Start a local Sonderr server or pass a server URL with ?server=.</p>
