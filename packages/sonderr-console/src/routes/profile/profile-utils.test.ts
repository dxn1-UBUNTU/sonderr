...
test("detects Sonderr auth failures", () => {
  expect(authError(new Error("Sonderr profile: Unauthorized"))).toBe(true)
  expect(authError({ status: 401 })).toBe(true)
  expect(authError(new Error("Sonderr profile: temporary network failure"))).toBe(false)
})
...
test("builds local links while preserving server", () => {
  ...
  expect(page(params, "/sonderr/login", { return: "/profile" })).toBe(
    "/sonderr/login?server=http%3A%2F%2F127.0.0.1%3A4097&return=%2Fprofile",
  )
})
