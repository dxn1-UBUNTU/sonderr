...
import { loadSonderrProfile, logoutSonderr, setSonderrOrganization, type SonderrProfileData, type ProjectQuery } from "../../client"
...
type Org = NonNullable<SonderrProfileData["profile"]["organizations"]>[number]
type State = { kind: "connected"; data: SonderrProfileData } | { kind: "disconnected" }

async function load(input: ProjectQuery): Promise<State> {
  if (wasDisconnected()) return { kind: "disconnected" }
  try {
    const data = await loadSonderrProfile(input)
...
export function ProfileRoute() {
  ...
  const login = () => page(params(), "/sonderr/login")
  ...
  function choose(id: string | null) {
    ...
    setSaving(next)
    setError("")
    void setSonderrOrganization(current, id)
        ...
        <div class="profile-page">
          <Show when={!disconnected()}>
            <header class="profile-header">
              <div>
                <p class="eyebrow">Sonderr Account</p>
                <h1>Your Profile</h1>
                <p>Manage your Sonderr identity, account context, credits, and billing shortcuts.</p>
              </div>
          ...
          <Show when={disconnected()}>
            <Card class="profile-connect-card">
              ...
              <div class="profile-connect-copy">
                <p class="eyebrow">Sonderr Account</p>
                <h1>Connect your Sonderr account</h1>
                <p>Sign in to view your credits, organizations, and account details in Sonderr Console.</p>
              </div>
              <A
                class="profile-primary-link"
                data-component="button"
          ...
          <Show when={!server.query() && !server.discoverable()}>
            <Card class="profile-banner" variant="warning">
              <strong>Sonderr server not found</strong>
              <span>Start Sonderr Console from a running Sonderr server or pass a server URL with ?server=.</span>
            </Card>
          </Show>
                  ...
                  <div class="profile-org-list">
                    ...
                    >
                      ...
                      <span class="profile-org-body">
                        ...
                        <span>Your personal Sonderr credits and settings</span>
                      </span>
