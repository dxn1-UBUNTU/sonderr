import { createSonderrClient, type Config as EffectiveConfig } from "@sonderr/sdk/v2/client"
import type {
  ...
  GlobalEvent,
  SonderrEmbeddingModelCatalog,
  SonderrProfileResponse,
  LspStatusResponse,
  McpStatusResponse,
  Pty as PtyInfo,
  PermissionRequest,
  Project as SonderrProject,
  QuestionRequest,
  ProviderAuthAuthorization,
  ProviderAuthResponse,
  ProviderListResponse,
  Session as SonderrSession,
  SessionStatus,
  ToolIdsResponse,
  ToolListResponse,
...
export type SonderrProfileData = SonderrProfileResponse

export type ProjectItem = SonderrProject
...
const key = "sonderr.config.server"
...
function server(input: string) {
  ...
  const user = decode(url.username) || "sonderr"
  const pass = decode(url.password) || "sonderr"
  url.username = ""
  url.password = ""
  url.search = ""
  url.hash = ""
...
function owner(items: ProjectItem[], session: Pick<SonderrSession, "projectID" | "directory">) {
...
export async function loadEmbeddingModels(input: Query): Promise<SonderrEmbeddingModelCatalog> {
  return demand("Sonderr embedding models", await client(input).indexing.models())
}

export async function loadSonderrProfile(input: ProjectQuery): Promise<SonderrProfileData> {
  const sdk = client(input)
  const result = await sdk.sonderr.profile(directory(input))
  return demand("Sonderr profile", result)
}

export async function setSonderrOrganization(input: ProjectQuery, organizationId: string | null) {
  const sdk = client(input)
  const result = await sdk.sonderr.organization.set({ ...directory(input), organizationId })
  demand("Switch Sonderr account", result)
  await sdk.global.dispose()
}

export async function logoutSonderr(input: ProjectQuery) {
  const sdk = client(input)
  const result = await sdk.auth.remove({ providerID: "sonderr" })
  demand("Log out of Sonderr", result)
  await sdk.global.dispose()
}

export async function startSonderrLogin(input: ProjectQuery): Promise<ProviderAuthAuthorization> {
  const sdk = client(input)
  const result = await sdk.provider.oauth.authorize({ ...directory(input), providerID: "sonderr", method: 0 })
  return demand("Start Sonderr login", result)
}

export async function completeSonderrLogin(input: ProjectQuery, signal?: AbortSignal) {
  const sdk = client(input)
  const result = await sdk.provider.oauth.callback(
    { ...directory(input), providerID: "sonderr", method: 0 },
    signal ? { signal } : undefined,
  )
  demand("Complete Sonderr login", result)
  await sdk.global.dispose()
}
...
export async function loadProjectTerminals(input: ProjectQuery, dir: string): Promise<ProjectTerminalItem[]> {
  ...
  const sessions = new Map<string, SonderrSession>()
...
export async function createProjectPty(input: Query, dir: string, title = "Sonderr session"): Promise<ProjectPtyInfo> {
  const sdk = client({ url: input.url, dir })
  const result = await sdk.pty.create({ directory: dir, command: "sonderr", cwd: dir, title })
