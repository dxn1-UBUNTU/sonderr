 ...
 * the package does not hard-code the sonderr global directory; defaults to XDG-style data storage. */
export namespace MemoryPaths {
  ...
  let host: () => Host = () => ({
    data: path.join(process.env.XDG_DATA_HOME ?? path.join(homedir(), ".local", "share"), "sonderr"),
  })
