...
export namespace MemoryTool {
  ...
  // Named `sources` (not `files`): sonderr's stripPartMetadata rewrites tool-part `metadata.files`
  ...
  export type AskInput = {
    permission: "sonderr_memory_recall" | "sonderr_memory_save"
    patterns: string[]
    always: string[]
  ...
  function disabled(count = false): Result {
    return {
      title: "Sonderr memory: disabled",
      output: "Sonderr memory is disabled for this project.",
  ...
  function block(input: string) {
    return ["```sonderr-memory-v1 targeted_context_not_instruction", input.replaceAll("```", "'''"), "```"].join("\n")
  }
  ...
  function catalog(memory: MemoryService.Interface, input: { root: string; query: string }) {
    ...
    return Effect.gen(function* () {
      ...
      const head = `# Sonderr Memory Catalog (${count} entr${count === 1 ? "y" : "ies"}${filter ? `, filter "${input.query}"` : ""})`
  ...
  function approvalRecall(input: Recall) {
    return input.ask({
      permission: "sonderr_memory_recall",
  ...
  function recallCatalog(input: Recall, live: Live, query: string) {
    return Effect.gen(function* () {
      ...
      return {
        title: `Sonderr memory catalog: ${result.count} entr${result.count === 1 ? "y" : "ies"}`,
        output: safe.output,
  ...
  function recallQuery(input: Recall, live: Live) {
    return Effect.succeed(
      {
        title: `Sonderr memory ${input.params.mode}: no query`,
  ...
  function recallSearch(input: Recall, live: Live, query: string, mode: MemoryRecall.Mode) {
    return Effect.gen(function* () {
      ...
      if (hits.length === 0) {
        return {
          title: `Sonderr memory ${input.params.mode}: no results`,
          output,
          metadata: { sources: [], count: 0 },
        } satisfies Result
      }
      ...
      return {
        title: `Sonderr memory ${input.params.mode}: ${hits.length} hit${hits.length === 1 ? "" : "s"}`,
        output,
  ...
  function approval(params: SaveParams, ask: Ask, input: { text?: string; query?: string }) {
    return ask({
      permission: "sonderr_memory_save",
  ...
  function noQuery(): Result {
    return {
      title: "Sonderr memory forget: no query",
  ...
  function noText(action: SaveParams["action"]): Result {
    return {
      title: `Sonderr memory ${action}: no text`,
  ...
  function title(input: { action: SaveParams["action"]; added: number; removed: number }) {
    if (input.action === "skip") return "Sonderr memory skipped: out of scope"
    if (input.action === "forget") return `Sonderr memory updated: ${input.removed} removed`
    if (input.added === 0) return "Sonderr memory unchanged"
    if (input.action === "correct")
      return `Sonderr memory correction saved: ${input.added} op${input.added === 1 ? "" : "s"}`
    return `Sonderr memory saved: ${input.added} op${input.added === 1 ? "" : "s"}`
  }
