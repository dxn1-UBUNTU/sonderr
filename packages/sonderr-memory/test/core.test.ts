...
import { SonderrMemory } from "../src/effect/index"

async function tmp() {
  const dir = await mkdtemp(path.join(os.tmpdir(), "sonderr-memory-"))
  return {
    dir,
...
describe("memory core package", () => {
  ...
  test("prepare removes legacy decisions once from owned memory roots", async () => {
    await use(async (t) => {
      ...
      await SonderrMemory.status({ root: t.root })
      ...
      await SonderrMemory.status({ root: other })
  ...
  test("prepare ignores legacy decisions cleanup failures", async () => {
    ...
    try {
      await use(async (t) => {
        ...
        const status = await SonderrMemory.status({ root: t.root })
  ...
  test("prepare ignores corrupt manifests during legacy cleanup", async () => {
    ...
    try {
      await use(async (t) => {
        ...
        await writeFile(paths.manifest, '{"kind":"sonderr-memory","version":1}\n')
  ...
  test("uses unicode-safe project and memory identifiers", async () => {
    await use(async (t) => {
      ...
      await Memory.remember({
        root: t.root,
        key: "設定_é",
        text: "日本語の設定は packages/sonderr-vscode に保存します。",
      })
      const shown = await Memory.show({ root: t.root })

      expect(id.display).toBe("proyecto_ñ_日本")
      expect(id.folder).toContain("proyecto_ñ_日本-")
      expect(shown.sources.project).toContain("- 設定_é :: 日本語の設定は packages/sonderr-vscode に保存します。")
  ...
  test("apply upserts, removes, dedupes, and skips secrets without aborting the batch", async () => {
    await use(async (t) => {
      ...
      await Memory.apply({
        root: t.root,
        ops: [
          { action: "add", key: "repo_tests", text: "Run CLI memory tests from packages/cli." },
          { action: "add", key: "repo_tests_copy", text: "Run CLI memory tests from packages/cli." },
  ...
  test("targeted recall dedupes lower-value session hits when typed memory answers", async () => {
    await use(async (t) => {
      ...
      await Memory.remember({
        root: t.root,
        key: "memory_tests",
        text: "Run memory tests from packages/cli with bun test ./test/sonderr/memory.",
      })
      await Memory.recordSession({
        root: t.root,
        sessionID: "ses_memory_tests",
        summary: "Discussed running memory tests from packages/cli.",
        time: Date.UTC(2026, 0, 1),
      })

      const result = await MemoryRecall.search({ root: t.root, query: "memory tests packages/cli", limit: 5 })
  ...
  test("non-English stored text remains searchable", async () => {
    await use(async (t) => {
      ...
      await Memory.apply({
        root: t.root,
        ops: [
          { action: "add", key: "pruebas_cli", text: "Ejecuta las pruebas CLI desde packages/cli." },
          { action: "add", key: "memoire", text: "Les corrections de mémoire restent dans corrections.md." },
          { action: "add", key: "設定", text: "日本語の設定は packages/sonderr-vscode に保存します。" },
        ],
      })
    ...
    await use(async (t) => {
      ...
      await MemoryFiles.writeSource(
        ...
        [
          ...
          "- vscode_unit_tests :: Run VS Code memory unit tests from packages/sonderr-vscode with bun test tests/unit/memory-command.test.ts.",
          ...Array.from(
            { length: 40 },
            (_, idx) =>
  ...
  test("index suppresses bulk session digests covered by typed memory", async () => {
    await use(async (t) => {
        ...
        ops: [
          {
            ...
            text: "Small model call sites are selected in the Sonderr adapter during memory capture.",
          },
        ],
      })
      await Memory.recordSession({
        ...
        summary: "Small model call sites are selected in the Sonderr adapter during memory capture.",
        time: Date.UTC(2026, 0, 1, 0, 0),
      })
      await Memory.recordSession({
        root: t.root,
      ...
      // ...but a compact pointer keeps its id targetable via sonderr_memory_recall mode=digest.
