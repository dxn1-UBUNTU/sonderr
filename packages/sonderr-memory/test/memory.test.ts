...
describe("memory facade", () => {
  test("enables, writes, indexes, and recalls project memory", async () => {
    ...
    try {
      ...
      await Memory.remember({
        ...
        file: "environment.md",
        section: "Commands",
        text: "Run CLI tests from packages/cli.",
      })

      const ctx = await Memory.context({ root: t.root, record: false })
      const recall = await Memory.recall({ root: t.root, query: "CLI tests packages sonderr" })

      expect(ctx.blocks[0]?.text).toContain("packages/cli")
      expect(recall.result?.block).toContain("packages/cli")
    } finally {
      await t.done()
    }
  })
  ...
  test("keeps Unicode keys and non-English text searchable", async () => {
    ...
    try {
      ...
      const recall = await Memory.recall({ root: t.root, query: "日本語 設定 sonderr-vscode" })
