...
async function tmp() {
  const dir = await mkdtemp(path.join(os.tmpdir(), "sonderr-memory-recall-"))
  return {
    dir,
...
describe("memory recall lexical fixtures", () => {
  ...
  test("expected hit: phrasing mismatch works when anchor terms overlap", async () => {
    await use(async (t) => {
      ...
      const result = await MemoryRecall.search({ root: t.root, query: "which packages/cli command checks CLI?" })
  ...
  test("expected hit: path and tool query finds environment memory", async () => {
    await use(async (t) => {
      ...
      await Memory.remember({
        ...
        file: "environment.md",
        section: "Commands",
        key: "sonderr_memory_tests",
        text: "Run bun test ./test/sonderr/memory from packages/cli.",
      })

      const result = await MemoryRecall.search({ root: t.root, query: "bun packages/cli memory" })

      expect(result?.hits[0]?.source).toBe("environment.md")
      expect(result?.block).toContain("sonderr_memory_tests")
    })
  })
  ...
  test("expected miss: ubiquitous-term overlap does not leak unrelated memory", async () => {
    await use(async (t) => {
      ...
      await Memory.remember({ root: t.root, key: "the_docs", text: "The docs live in packages/sonderr-docs." })
  ...
  test("expected hit: a topic word repeated in a small corpus is not dropped", async () => {
    await use(async (t) => {
      ...
      await Memory.remember({ root: t.root, key: "test_rule", text: "Tests run from packages/cli." })
  ...
  test("expected miss: corpus-derived filtering drops non-English function words", async () => {
    await use(async (t) => {
      ...
      await Memory.remember({ root: t.root, key: "docs_es", text: "La ruta de docs vive en packages/sonderr-docs." })
