...
async function tmp() {
  const dir = await mkdtemp(path.join(os.tmpdir(), "sonderr-memory-effect-"))
  return {
...
function view(over: Partial<MemoryPorts.TurnView> = {}): MemoryPorts.TurnView {
  return {
    ...
    assistant: "Use bun install, then bun test ./test from packages/cli.",
...
describe("MemoryCapture (fake ports)", () => {
  test("turn-close typed LLM saves environment memory", async () => {
    const t = await tmp()
    try {
      await SonderrMemory.enable({ root: t.root })
      await SonderrMemory.configure({ root: t.root, settings: { autoConsolidate: true } })
      ...
      const result = await run({
        ...
        model: model({
          ...
          typed:
            '{"operations":[{"op":"upsert_environment_fact","section":"Commands","key":"cli_memory_tests","value":"Run bun test ./test from packages/cli."}],"skipped":[]}',
        }),
      })
      ...
      const shown = await SonderrMemory.show({ root: t.root })
  ...
  test("auto-consolidate off skips digest and typed model writes", async () => {
    ...
    try {
      await SonderrMemory.enable({ root: t.root })
      await SonderrMemory.configure({ root: t.root, settings: { autoConsolidate: false } })
...
describe("MemoryTimers signal ref-counting", () => {
  test("shares one controller per root and drops it once the last capture releases", () => {
    const root = "/sonderr-memory/ref-count-root"
