...
description: "Guidelines for writing Sonderr documentation"
---

# Documentation Style Guide

This guide covers writing, formatting, and structuring documentation for the Sonderr docs site.

## Voice and Tone

Sonderr documentation should be:
...
- Use present tense
- Be specific: "Run `sonderr run` to execute a task" not "You can run sonderr run"
...
```markdown
{% callout type="tip" %}
**The easiest way to configure Sonderr is to ask the agent.** Just tell the agent what you want.
{% /callout %}
```
...
- Use `sonderr run` for CLI examples, not hypothetical commands

````markdown
```bash
sonderr run "create a utils.py file with a function that adds two numbers"
```
````
...
- `sonderr.jsonc` for configuration files
...
- Use generated screenshots from `packages/sonderr-docs/public/img/screenshot-tests/` when available
...
| Term | Use for |
|---|---|
| Sonderr | The product name |
| sonderr CLI | The command-line interface |
| VS Code extension | The VS Code extension |
| JetBrains plugin | The JetBrains IDE plugin |
| `sonderr serve` | The local HTTP server |
| `sonderr run` | The headless execution command |
