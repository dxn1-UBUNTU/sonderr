---
title: "Using GCP Vertex AI with Sonderr"
description: "Connect Google Cloud Vertex AI to Sonderr to use Claude, Gemini, and other models through your GCP account."
sidebar_label: GCP Vertex AI
---

# Using GCP Vertex AI With Sonderr

Sonderr supports accessing models through Google Cloud Platform's Vertex AI, a managed machine learning platform that provides access to various foundation models, including Anthropic's Claude family.
...
The extension stores the service account JSON in Sonderr's credential store, not in `sonderr.json`. The JSON is visible while you edit it so you can verify the value before connecting.
...
- **Prompt caching:** Claude models served through Vertex AI support Sonderr prompt caching. Sonderr applies Anthropic cache controls and tracks cache write/read tokens when Vertex reports them. Native Vertex Gemini models use Google's implicit server-side caching; no extra Sonderr configuration is required, and Gemini may not report cache write tokens.
