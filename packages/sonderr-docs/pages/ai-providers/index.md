---
title: "AI Providers"
description: "Configure and connect different AI model providers to Sonderr"
---

# AI Providers

Sonderr supports a wide variety of AI providers, giving you flexibility in how you power your AI-assisted development workflow. Choose from cloud providers, local models, or AI gateways based on your needs.

## Getting Started

The fastest way to get started is with **Sonderr's built-in provider**, which requires no configuration. Simply sign in and start coding.
...
- **[Atomic Chat](/docs/ai-providers/atomic-chat)** - Local models with TurboQuant inference and auto-discovery in Sonderr
...
|---|---|
| Ease of use | [Sonderr (built-in)](/docs/ai-providers/sonderr) |
...
You can prevent specific providers from loading using `disabled_providers` in your `sonderr.json` (or `sonderr.jsonc`). This is useful to hide models from built-in or detected providers that you don't intend to use.
...
Both fields accept provider IDs — the lowercase identifier used in the `provider/model` format (e.g. `sonderr`, `anthropic`, `openai`, `google`, `groq`).

## Next Steps

- **New to Sonderr?** Start with the [Sonderr provider](/docs/ai-providers/sonderr) - no setup required
