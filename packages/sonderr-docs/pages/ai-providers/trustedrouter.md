---
title: "Using TrustedRouter with Sonderr | Attested AI Router"
description: "Configure TrustedRouter in Sonderr through its OpenAI-compatible API for attested routing, zero-data-retention routing, and encrypted model routes."
sidebar_label: TrustedRouter
---

# Using TrustedRouter With Sonderr

TrustedRouter exposes an OpenAI-compatible API at `https://api.trustedrouter.com/v1`.
You can use it from Sonderr with the built-in TrustedRouter provider.
...
Run `sonderr`, then use the `/connect` command, select **TrustedRouter**, and paste your API key when prompted:
...
Then configure the built-in TrustedRouter provider in your `sonderr.json` file:
...
- To use a specific model instead of a router alias, add it under `provider.trustedrouter.models` and select it in Sonderr.
