---
title: "Contributing"
description: "Contribute to Sonderr"
---

# Contributing Overview

{% callout type="info" %}
**New versions of the VS Code extension and CLI are being developed in [Sonderr-Org/sonderr](https://github.com/Sonderr-Org/sonderr)** (extension at `packages/sonderr-vscode`, CLI at `packages/cli`). If you're looking to contribute to the extension or CLI, please head over to that repository.
{% /callout %}

Sonderr is an open-source project that welcomes contributions from developers of all skill levels. This guide will help you get started with contributing to Sonderr, whether you're fixing bugs, adding features, improving documentation, or sharing custom modes.

## Ways to Contribute

There are many ways to contribute to Sonderr:

1. **Code Contributions**: Implement new features or fix bugs
2. **Documentation**: Improve existing docs or create new guides
3. **Marketplace Contributions**: Create and share custom modes, skills, and MCP servers via the [Sonderr Marketplace](https://github.com/Sonderr-Org/sonderr-marketplace)
...
Before diving into the code, we recommend reviewing the [Architecture Overview](/docs/contributing/architecture) to understand how the different components of Sonderr fit together.
...
AI and coding agents are welcome in Sonderr contributions. Contributors still own the work they submit: you must personally understand the change, test it appropriately, be able to explain the diff, and understand how it interacts with the affected package and the rest of the repo.
...
- Run `bun run script/extract-source-links.ts` after adding or changing guarded URLs in `packages/sonderr-vscode/`, `packages/sonderr-vscode/webview-ui/`, or `packages/cli/src/`.
- When editing shared `packages/cli/` files, keep Sonderr changes small and mark Sonderr-only edits with `// sonderr_change` for a single line or `// sonderr_change start` / `// sonderr_change end` for a block. Do not add these markers inside `sonderr`-named paths.
...
- Use the root [TESTING.md](https://github.com/Sonderr-Org/sonderr/blob/main/TESTING.md) guide for backend/API validation with `bun dev serve` and `curl`-based requests.
...
2. Go to the [Sonderr repository](https://github.com/Sonderr-Org/sonderr)
...
Sonderr has bug bounties. To be eligible, make sure your GitHub account is connected in your Sonderr account.

## Contributing to the Sonderr Marketplace

The [Sonderr Marketplace](https://github.com/Sonderr-Org/sonderr-marketplace) is a community-driven repository of agent tooling that extends Sonderr's capabilities. You can contribute:
...
3. Submit a pull request to the [Sonderr Marketplace repository](https://github.com/Sonderr-Org/sonderr-marketplace)
...
2. Test your documentation changes and run the docs site locally from the repo root:
   ...
   ```bash
   bun run --filter @sonderr/sonderr-docs test
   bun run --filter @sonderr/sonderr-docs build
   bun run --filter @sonderr/sonderr-docs dev
   ```
...
When participating in the Sonderr community:

- Be respectful and inclusive
- Provide constructive feedback
- Help newcomers get started
- Follow the [Code of Conduct](https://github.com/Sonderr-Org/sonderr/blob/main/CODE_OF_CONDUCT.md)
...
- Ask questions on [GitHub Discussions](https://github.com/Sonderr-Org/sonderr/discussions)
- Visit our [Reddit community](https://www.reddit.com/r/sonderr)

## Recognition

All contributors are valued members of the Sonderr community. Contributors are recognized in:

- Release notes
- The project's README
- The contributors list on GitHub

Thank you for contributing to Sonderr and helping make AI-powered coding assistance better for everyone!
