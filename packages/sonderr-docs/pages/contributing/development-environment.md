...
{% callout type="info" %}
**New versions of the VS Code extension and CLI are being developed in [Sonderr-Org/sonderr](https://github.com/Sonderr-Org/sonderr)** (extension at `packages/sonderr-vscode`, CLI at `packages/cli`). For extension and CLI development, please head over to that repository.
{% /callout %}
...
4. **Java 21** - Required only when running JetBrains plugin checks or repo-level checks that include `@sonderr/sonderr-jetbrains`
...
1. **Fork and Clone the Repository**:
   - **Fork the Repository**:
     - Visit the [Sonderr GitHub repository](https://github.com/Sonderr-Org/sonderr)
     - Click the "Fork" button in the top-right corner to create your own copy.
   - **Clone Your Fork**:
     ```bash
     git clone https://github.com/[YOUR-USERNAME]/sonderr.git
     cd sonderr
     ```
...
- **`packages/cli/`** - Sonderr CLI, agent runtime, local HTTP server, session management, and TUI
- **`packages/sonderr-vscode/`** - VS Code extension, webview UI, Agent Manager, and extension packaging
- **`packages/sdk/js/`** - Generated TypeScript SDK for the local server API
- **`packages/sonderr-docs/`** - Documentation site
- **`packages/sonderr-jetbrains/`** - JetBrains plugin
...
`bun dev` and `bun run dev` are equivalent. Both run the local source in `packages/cli/`; they do not use a globally installed `sonderr` binary.

### Backend/API Validation

For backend and API validation, use the root [TESTING.md](https://github.com/Sonderr-Org/sonderr/blob/main/TESTING.md) guide. It covers starting the local backend with:
...
If you change server endpoints in `packages/cli/src/server/`, regenerate the SDK from the repo root:
...
`extension:isolated` launches a separate VS Code process with user data and extensions under `.sonderr-dev/vscode/`, and points Sonderr at isolated XDG storage under `.sonderr-dev/{data,config,state,cache}`. Re-running it keeps the same isolated environment, so installed extensions, VS Code settings, Sonderr auth, sessions, config, state, and cache persist.

`extension:isolated:clean` deletes `.sonderr-dev/` before launch. Use it to simulate a fresh VS Code + Sonderr install without affecting your main VS Code profile or real Sonderr config.
...
When the Extension Development Host opens, check the Sonderr output channel and the Developer Tools console for startup or webview errors.

### Building the Extension

From `packages/sonderr-vscode/`:
...
Sonderr uses several types of tests to ensure quality:
...
Use the root [TESTING.md](https://github.com/Sonderr-Org/sonderr/blob/main/TESTING.md) guide for backend/API checks that require `bun dev serve` and `curl`-based requests.
...
- After adding or changing guarded URLs in `packages/sonderr-vscode/`, `packages/sonderr-vscode/webview-ui/`, or `packages/cli/src/`, run `bun run script/extract-source-links.ts` from the repo root.
- When editing shared `packages/cli/` files, keep changes small and mark Sonderr-only edits with `// sonderr_change` for a single line or `// sonderr_change start` / `// sonderr_change end` for a block. Do not add these markers inside `sonderr`-named paths.
...
- Check the Output panel in VSCode (View > Output) and select "Sonderr" from the dropdown
