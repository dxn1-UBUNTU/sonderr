...
description: "Architecture of the Sonderr JetBrains split-mode plugin"
---

# JetBrains Plugin Architecture

The JetBrains plugin (`packages/sonderr-jetbrains/`) is a split-mode Swing client of [Sonderr CLI runtime](/docs/contributing/architecture/cli-runtime). Frontend module renders IDE UI. Backend module owns project-local logic and one bundled `sonderr serve` server. Shared module defines cross-process RPC contracts and serializable payloads.
...
flowchart LR
  ...
  subgraph backend ["JetBrains backend"]
    ...
    conn["SonderrConnectionService"]
    workspaces["Directory workspace cache"]
    cli["Extracted sonderr serve --port 0"]
  end
...
|---|---|---|
| `SonderrAppRpcApi` | Application | Connect, state flow, health, retry, restart, reinstall, model state, profile, login, telemetry |
| `SonderrWorkspaceRpcApi` | Directory | Resolve real backend project directory, workspace state flow, reload, file lookup, open file |
| `SonderrSessionRpcApi` | Session and directory | Create/list sessions, prompt, stream events, permission and question replies, config update |
| `SonderrMigrationRpcApi` | Legacy migration | Detect, run, and observe migration state |
...
```text
<PathManager.getSystemPath()>/sonderr/bin/sonderr
<PathManager.getSystemPath()>/sonderr/bin/sonderr.exe   # Windows
```

It chooses platform resource by OS and CPU architecture, reuses extracted binary when resource size matches, and can force re-extraction during reinstall flow. This editor-owned child is separate from detached local daemon managed by `sonderr daemon`.

| Area | Behavior |
|---|---|
| Spawn | Runs extracted binary as `sonderr serve --port 0` |
| Port | CLI server prefers `4096`, then asks OS for free port; backend reads listening line from stdout |
| Authentication | Generates random 32-byte hex password and passes `SONDERR_SERVER_PASSWORD`; username defaults to `sonderr` |
...
This mirrors CLI `InstanceStore`: directory remains isolation key while one editor-owned `sonderr serve` process serves multiple workspace contexts.
...
| Check | Command from `packages/sonderr-jetbrains/` |
|---|---|
...
For plugin icon creation, modification, or review, use the `icon-jetbrains` skill at `.sonderr/skills/icon-jetbrains/SKILL.md` together with `packages/sonderr-jetbrains/AGENTS.md`. The skill covers IntelliJ New UI SVG roles, sizing, palette, dark variants, and icon validation guidance.
...
| Concern | Source path |
|---|---|
| Split modules | `packages/sonderr-jetbrains/settings.gradle.kts` and module XML descriptors |
| Contributor constraints | `packages/sonderr-jetbrains/AGENTS.md` |
| CLI lifecycle | `packages/sonderr-jetbrains/backend/src/main/kotlin/ai/sonderr/backend/cli/SonderrBackendCliManager.kt` |
| Connection recovery | `packages/sonderr-jetbrains/backend/src/main/kotlin/ai/sonderr/backend/app/SonderrBackendConnectionService.kt` |
| Workspace cache | `packages/sonderr-jetbrains/backend/src/main/kotlin/ai/sonderr/backend/workspace/SonderrBackendWorkspaceManager.kt` |
| Kotlin client generation | `packages/sonderr-jetbrains/backend/build.gradle.kts` |
| RPC contracts | `packages/sonderr-jetbrains/shared/src/main/kotlin/ai/sonderr/rpc/` |
