...
description: "Architecture of Sonderr Cloud automation services that dispatch scoped work"
---
...
This page describes cloud automation boundaries present in `Sonderr-Org/cloud`. Static source shows supported code paths, Worker bindings, and deployable surfaces. It does not prove live production enablement or rollout policy.
{% /callout %}
...
|---|---|---|---|---|
| Sonderr Bot | GitHub, Slack, or Linear command ingress | Web control plane bot libraries | Cloud Agent for requested repository work | Command response and coding-session result |
...
| Auto Fix | `sonderr-auto-fix` label or dispatch rule | `auto-fix-infra` Durable Object per fix ticket | Cloud Agent branch and pull-request work | Pull request and lifecycle status callback |
...
| Webhook Agent Ingest | HTTP webhook or scheduled alarm | `webhook-agent-ingest` queue and `TriggerDO` | Cloud Agent or Sonderr Chat destination | Destination delivery and queue retry behavior |

## Sonderr Bot ingress and source-control targets

Sonderr Bot command ingress and repository target support are separate concerns.

| Concern | Current static-source statement |
|---|---|
| Command ingress | GitHub, Slack, and Linear are current Sonderr Bot command surfaces |
...
| GitLab command ingress | GitLab repository support does not imply note or comment-triggered Sonderr Bot commands |

Do not document GitLab issue notes or merge-request comments as current Sonderr Bot trigger surfaces unless source adds explicit ingress handling.
...
| Output | High-confidence classification applies labels such as `sonderr-auto-fix` for downstream Auto Fix |
...
| Destination | `sonderrclaw_chat` | Posts to user-scoped Sonderr Chat destination through Sonderr Chat service binding |
...
|---|---|
| Sonderr Bot | `apps/web/src/lib/bot/`{% linebreak /%}`apps/web/src/lib/bots/`{% linebreak /%}`apps/web/src/lib/slack-bot/` |
