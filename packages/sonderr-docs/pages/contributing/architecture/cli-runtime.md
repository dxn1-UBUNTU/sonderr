...
description: "Architecture of the Sonderr CLI runtime, daemon, server, persistence, SDK, and indexing"
---

# CLI Runtime Architecture

The CLI (`packages/cli/`) is Sonderr's local agent engine. It owns agent execution, tools, sessions, provider integration, configuration, local persistence, directory routing, and HTTP surfaces used by editor clients and Sonderr Console.
...
| Term | Meaning |
|---|---|
| Sonderr CLI runtime | Local agent engine in `packages/cli/` |
...
| Process-shared state | Runtime service state shared by every directory context in one Sonderr CLI process |
| Modes | Configurable agent presets for tools, prompts, restrictions, and behavior |
| MCP | Protocol for extending agent tools |

One `sonderr serve` process can host several local runtime instances. Directory-keyed state stays isolated. Process-shared service state does not.

## Command entry points

{% callout type="warning" title="Sonderr Console is deprecated" %}
The `sonderr console` command and the browser interface described on this page are deprecated and will be removed in an upcoming release.
{% /callout %}

| Entry point | Command or caller | Runtime model |
|---|---|---|
| Interactive TUI | `sonderr` | Attaches to local daemon when available; otherwise starts Bun worker and sends SDK-shaped requests over RPC |
| Headless run | `sonderr run` | Uses daemon attach when available, then embedded server fetch fallback |
| Attached run | `sonderr run --attach <url>` | Targets explicit running `sonderr serve` server |
| Explicit API server | `sonderr serve` | Starts HTTP + SSE server for external local clients |
| Local daemon | `sonderr daemon start` | Starts detached `sonderr serve` child for reuse |
| Browser console | `sonderr console` | Starts or reuses local daemon and opens daemon-served `/console` UI |
| Editor-spawned server | VS Code or JetBrains client | Starts bundled `sonderr serve --port 0` child owned by editor client, not local daemon manager |

```mermaid
flowchart LR
  run["sonderr run"]
  tui["sonderr TUI"]
  daemon["Detached daemon: sonderr serve"]
  worker["Bun worker"]
  rpc["RPC-backed fetch and global events"]
  embedded["Embedded Server.Default().app.fetch"]
  serve["Explicit sonderr serve"]
  editors["VS Code or JetBrains"]
  editorServer["Editor-owned sonderr serve --port 0"]
...
Each running editor host starts one editor-owned `sonderr serve` server. That server can handle coding sessions for workspace root and additional worktree directories at same time. It does not start separate server process for each directory.

```mermaid
flowchart LR
  views["Editor views<br/>Workspace root and worktrees"]
  server["One editor-owned<br/>sonderr serve server"]
  store["InstanceStore"]
...
|---|---|---|
| Local `sonderr serve` access | HTTP, SSE, and selected WebSocket access to local server | Sonderr CLI server and spawning local client |
| Outbound provider authentication | Model provider, Sonderr Gateway, catalog, and indexing access | Sonderr CLI provider router and auth stores |
| Remote MCP OAuth | Browser authorization and credentials for remote MCP server | Sonderr CLI MCP runtime |

### Local `sonderr serve` access

Server Basic Auth is optional. It becomes required when `SONDERR_SERVER_PASSWORD` is non-empty. Default username is `sonderr`; `SONDERR_SERVER_USERNAME` can override it.
...
| PTY shell child | Removes `SONDERR_SERVER_PASSWORD` and `SONDERR_SERVER_USERNAME` from spawned user-shell environment |
...
Provider auth records use `api`, `oauth`, or `wellknown` variants in `${Global.Path.data}/auth.json`, written with mode `0600`. `SONDERR_AUTH_CONTENT` can supply process-local auth JSON. Separate v2 multi-account auth store also exists for account-oriented flows.

| Path | Behavior |
|---|---|
| Direct providers | Use provider-specific keys, OAuth records, environment values, and configured endpoints |
| Sonderr Gateway | Resolves Sonderr model access and model catalog through gateway client |
| Anonymous Sonderr | If no Sonderr key exists, provider loader sets API key value `anonymous`; gateway model catalog can fall back to public unauthenticated endpoint |
| Organization catalog | Sonderr model fetch includes organization ID when resolved from config, auth, or environment |
...
| Indexing auth | Resolves indexing-specific Sonderr config first, then provider config, auth record, provider options, and `SONDERR_API_KEY` / `SONDERR_ORG_ID` environment values |
...
2. `x-sonderr-directory` request header.
3. Server process cwd.

Local routing workspace selection is separate. Session workspace, `workspace` query parameter, and `SONDERR_WORKSPACE_ID` can select workspace context. Configured `SONDERR_WORKSPACE_ID` keeps requests local to current workspace runtime. Other selected workspaces resolve through workspace-routing adapter to local directory or remote target.
...
| Tool registry | Loads built-in, Sonderr-specific, MCP, and readiness-gated semantic search tools |
...
| Provider router | Resolves direct providers, Sonderr Gateway, custom endpoints, and credentials |
| HTTP server | Publishes REST, WebSocket, and SSE surfaces |

## Daemon lifecycle

`sonderr daemon start|status|stop|restart` manage a detached local `sonderr serve` child, with bare `sonderr daemon` equivalent to `sonderr daemon start`. `sonderr console` calls the same start path, so it reuses a healthy daemon instead of spawning a second process, while `sonderr console stop` aliases `sonderr daemon stop`.
...
| Child process | Detached `sonderr serve --hostname <host> --port <port>` process |
...
| Opt-out | `SONDERR_NO_DAEMON` disables automatic attach by clients; explicit daemon commands still manage daemon |

Daemon credentials differ from editor-spawned server credentials. Current daemon source stores username `sonderr`, password `sonderr`, and base64 Basic token in `daemon.json`. File permissions protect this local credential record. Editor clients generate random passwords per spawned server.
...
| Area | Behavior |
|---|---|
| Default database | `${Global.Path.data}/sonderr.db` |
| Override | `SONDERR_DB`; relative paths resolve under data directory; `:memory:` is accepted |
...
Snapshot implementation state is directory-keyed through `InstanceState`. One `Snapshot.Service` also owns process-shared slow-snapshot guard state outside directory cache. This distinction matters when multiple Agent Manager worktrees use same `sonderr serve` process.
...
1. Effect `HttpApi` groups under `packages/cli/src/server/routes/instance/httpapi/` define routes.
2. `packages/cli/src/server/routes/instance/httpapi/public.ts` normalizes public OpenAPI to legacy-compatible request and response shapes.
3. Sonderr-specific API groups and handlers live under `packages/cli/src/sonderr/server/httpapi/` and enter shared API through narrow injection seams.
4. `packages/sdk/js/script/build.ts` generates TypeScript v2 client from CLI OpenAPI.
5. `packages/sdk/js/src/v2/client.ts` adds `createSonderrClient()` wrapper for directory and workspace routing, Electron and Node fetch compatibility, and clearer empty-response errors.
...
| Order | Source |
|---|---|
| 1 | Legacy Sonderr migrations |
| 2 | Organization modes |
| 3 | Auth-record `.well-known/sonderr` remote config |
| 4 | Global config files |
| 5 | Explicit `SONDERR_CONFIG` file |
| 6 | Project `sonderr.json[c]` and `sonderr.json[c]` files plus discovered config directories |
| 7 | `SONDERR_CONFIG_DIR` directory |
| 8 | `SONDERR_CONFIG_CONTENT` |
| 9 | Active Sonderr Cloud organization config |
...
Global config files load from `${Global.Path.config}`. Project updates prefer existing config files found in ancestor `.sonderr` or legacy `.sonderr` directories, then existing project root config files, then create `.sonderr/sonderr.json`. Global indexing settings can carry provider and storage defaults, but global `indexing.enabled` is stripped so project enablement remains local in effective instance config.
...
Runtime config loading is separate from editor-facing JSON Schema publication. Cloud-served schema improves validation and completion for `sonderr.json` and `sonderr.jsonc`; it does not load, apply, or override effective runtime config. When adding or changing config key, follow [CLI Config Schema](/docs/contributing/architecture/config-schema) so CLI source and cloud overlay stay aligned.
...
**Deprecated.** The Sonderr Console browser interface and its `sonderr console` launcher will be removed in a future release.

`sonderr console` starts or reuses daemon, opens `/console`, and prints Console launch URL. Browser launch URL embeds daemon Basic credentials so initial request authenticates.

| Area | Behavior |
|---|---|
| Frontend | Solid/Vite app in `packages/sonderr-console/` |
...
| SDK | Console calls generated JavaScript SDK through `createSonderrClient()` |
...
`packages/sonderr-indexing/` owns indexing engine. CLI bridge injects indexing plugin by default unless default plugins are disabled, then starts indexing asynchronously per normalized directory during instance bootstrap.

| Area | Behavior |
|---|---|
| Bootstrap | `SonderrBootstrap` forks indexing initialization so instance startup is not blocked |
...
| Worktrees | Agent Manager `.sonderr/worktrees/` and legacy `.sonderr/worktrees/` paths return disabled status |
| Empty VS Code window | Extension sets `SONDERR_DISABLE_CODEBASE_INDEXING=vscode-no-workspace`; bridge reports disabled status |
| Embeddings | Supports Sonderr, OpenAI, Ollama, OpenAI-compatible, Gemini, Mistral, Vercel AI Gateway, Bedrock, OpenRouter, and Voyage configuration |
...
|---|---|
| CLI entry points | `packages/cli/src/cli/cmd/` |
| Daemon | `packages/cli/src/sonderr/daemon/` |
| HTTP server | `packages/cli/src/server/` |
| Directory and workspace routing | `packages/cli/src/server/routes/instance/httpapi/middleware/workspace-routing.ts` |
| SQLite | `packages/cli/src/storage/db.ts` |
| Snapshots | `packages/cli/src/snapshot/index.ts`{% linebreak /%}`packages/cli/src/sonderr/snapshot/track.ts` |
| SDK | `packages/sdk/js/`{% linebreak /%}`script/generate.ts` |
| Console | `packages/sonderr-console/`{% linebreak /%}`packages/cli/src/sonderr/console/` |
| Indexing | `packages/sonderr-indexing/`{% linebreak /%}`packages/cli/src/sonderr/indexing.ts` |
