...
How much you spend with Sonderr is shaped by several factors working together:
...
This page covers the controls currently available in Sonderr. For a direct overview of Auto Model tiers and token optimization tips, see [Cost Efficiency & Model Selection](/docs/getting-started/rate-limits-and-costs).
...
When the agent enters a repeated failure cycle — attempting the same action multiple times without making progress — Sonderr pauses and asks for permission before continuing. This is controlled by the `doom_loop` permission, which defaults to `ask`.

**Where to configure:** Settings → Auto Approve (VS Code) or the `permission.doom_loop` key in `sonderr.jsonc` (CLI).
...
Every action Sonderr takes — reading files, editing code, running shell commands, launching sub-agents — is governed by the permission system. Each tool can be set to `allow`, `ask`, or `deny`. When set to `ask`, Sonderr pauses before executing and you can approve or reject that specific action.

**Where to configure:** Settings → Auto Approve (VS Code) or the `permission` section in `sonderr.jsonc` (CLI). See [Auto-Approving Actions](/docs/getting-started/settings/auto-approving-actions) for the full list of available permissions.
...
A shield button in the prompt controls lets you toggle auto-approve on and off at runtime without opening Settings. When enabled, pending permission prompts are approved automatically. The state stays synced across the sidebar and open Sonderr tabs.
...
Requests to free models (`sonderr-auto/free` and other free-tier models) are rate-limited to **200 requests per hour**. If you exceed this, requests return HTTP 429 and you must wait before continuing.
...
Sonderr automatically skips a set of directories including `node_modules`, `dist`, `build`, `.git`, `__pycache__`, `.cache`, and `vendor`. You can add additional paths using **permission deny rules** in `sonderr.jsonc` or using a `.sonderrignore` file at your workspace root.
...
**Where to configure:** `sonderr.jsonc` (VS Code / CLI). See [.sonderrignore](/docs/customize/context/sonderrignore) for full details.

### Compact long conversations

When a conversation grows long, use `/compact` in the chat (also searchable as `smol` or `condense`) to summarize the history and free up context space. Sonderr replaces older conversation turns with an anchored summary that captures your goal, constraints, progress, and next steps.

Auto-compaction is **enabled by default** — Sonderr automatically compacts when approaching the context window limit so you do not need to intervene manually.

**Where to configure:** Toggle auto-compaction in **Settings → Context** (VS Code) or set `compaction.auto` in `sonderr.jsonc`. Configure the trigger threshold with `compaction.threshold_percent` (e.g. `80` to compact at 80% of the model's context window).
...
**Where to configure:** Model settings in the Sonderr UI, or the `limit.output` key in custom model configuration.
...
Sonderr automatically applies prompt caching on supported providers. Repeated context — your system prompt, stable file contents, and tool definitions — is reused from cache at a discounted rate. No configuration is required to benefit from this.
...
Sonderr has auto-models that can help you control costs; more information is available in [Auto Model](/docs/code-with-ai/agents/auto-model).

### Practical examples by task type

| Task type | Suggested approach |
|---|---|
| Quick questions, syntax lookups, simple formatting | `sonderr-auto/efficient` or a lightweight model |
| Routine edits, test generation, straightforward refactors | `sonderr-auto/efficient` or a mid-tier model |
| Complex debugging, tracing unexpected behavior | `sonderr-auto/frontier` or a strong reasoning model; Debug mode |
| Architecture planning, design decisions | `sonderr-auto/frontier`; Architect mode |
| Repository-wide analysis or search | A model with a large context window (256K+); Architect mode |
| Code review and summarization | `sonderr-auto/efficient` or a cost-effective model |
| Automated background tasks (CI, scripting) | `sonderr-auto/efficient` or `sonderr-auto/free` |

### Manually selecting a model

Use the **model selector dropdown** in the Sonderr chat interface to switch models for the current session. In the CLI, pass the `--model` flag to `sonderr run` or use the model picker in the TUI (`Ctrl+X m` or `/models`).
...
- **VS Code:** Settings → Models → Model per Mode, or edit `sonderr.jsonc` directly.
- **CLI:** Set `agent.<name>.model` in `sonderr.jsonc`.

```jsonc
{
  "agent": {
    "code": {
      "model": "sonderr-auto/efficient"
    },
    "architect": {
      "model": "sonderr-auto/frontier"
    }
  }
}
```
...
- Use `sonderr-auto/efficient` as the default model
- Switch to `sonderr-auto/free` for low-stakes questions and exploration
...
- Assign `sonderr-auto/efficient` to Code and Debug agents for everyday work
- Assign `sonderr-auto/frontier` to Architect (or Plan) agent for planning tasks
- Set `sonderr-auto/efficient` as the compaction model for all agents
- If on an Enterprise plan, use Providers & Models to block high-cost models that are not needed for your team's typical work

```jsonc
{
  "agent": {
    "code": { "model": "sonderr-auto/efficient" },
    "debug": { "model": "sonderr-auto/efficient" },
    "architect": { "model": "sonderr-auto/frontier" },
...
The snippet below is a ready-to-copy `sonderr.jsonc` that turns on every available cost-control knob at its most restrictive setting. Drop it into your project root (or your global `~/.config/sonderr/sonderr.jsonc`) and adjust individual values upward as you get comfortable with how each one behaves.

{% callout type="tip" %}
This configuration uses only `sonderr-auto/efficient`.
{% /callout %}
...
```jsonc
{
  ...
  // ── Model selection ──────────────────────────────────────────────────────
  // Route all requests through the two lowest-cost Sonderr Auto tiers.
  // sonderr-auto/efficient: lowest-cost paid tier (classifies each request by
  //   difficulty and routes to the cheapest benchmark-proven model).
  "model": "sonderr-auto/efficient",
  "subagent_model": "sonderr-auto/efficient", // default model for Task-tool subagents
  ...
  "agent": {
    ...
    "compaction": { "model": "sonderr-auto/free" },
    "title":      { "model": "sonderr-auto/free" },
    "summary":    { "model": "sonderr-auto/free" }
  },
  ...
  // "ask" means Sonderr pauses and requires your approval before executing.
