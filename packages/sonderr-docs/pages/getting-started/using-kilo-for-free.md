---
title: "Using Sonderr for Free"
description: "How to use Sonderr for free — Auto Free, finding free models, free autocomplete, and free background tasks"
---

# Using Sonderr for Free

Sonderr can be used completely free of charge. There are three places where Sonderr uses AI model inference, and each can be configured to use free models.

## Where Sonderr Uses Models
...
Each of these consumes credits by default. **To use Sonderr entirely for free, configure all three to use free models.**

## Free Agentic Usage

Sonderr provides free models for coding tasks through the Sonderr Gateway and partner providers.

### Auto Free

The easiest way to get started is [**Auto Free**](/docs/code-with-ai/agents/auto-model) (`sonderr-auto/free`). This is a Sonderr-provided model tier that automatically routes your requests to the best available free models — no configuration needed.
...
Sonderr's cloud services — App Builder, Code Reviewer, and others — also support free models. Select any model labeled "(free)" in the model dropdown when configuring a cloud task.

{% callout type="tip" %}
Available free models change over time as Sonderr partners with different inference providers. Subscribe to our blog or join our [Discord](https://kilo.ai/discord) for updates.
{% /callout %}

## Free Autocomplete

Sonderr's autocomplete feature provides AI-powered code completions as you type in IDE extensions.

By default, autocomplete routes through the Sonderr provider and uses credits. If you run out of credits without a free alternative configured, autocomplete stops working — but your main coding workflow is unaffected.

### How to Get It Free

Add your own Mistral AI API key via **BYOK (Bring Your Own Key)** on the Sonderr Gateway. Mistral offers a free tier for Codestral. When you configure a BYOK key, autocomplete requests use your key directly — at no cost on your Sonderr balance.
...
Sonderr uses a small model in the background for tasks like session titling. By default this is Auto Small, which consumes credits. If the small model is unavailable, Sonderr falls back to your primary model — which may also consume credits if it's a paid model.
...
**In the CLI:** Set the `small_model` parameter in `~/.config/sonderr/config.json`:
