...
description: "Guide for migrating to Sonderr from other AI coding tools"
---

# Migrating from Cursor or Windsurf

Quickly migrate your custom rules from Cursor or Windsurf to Sonderr. The process typically takes just a few minutes per project.

{% callout type="info" title="Two Workflow Approaches"%}

Sonderr supports **two complementary workflows**—choose the one that fits your style, or use both:
...
## Why Sonderr's Rules System?

Sonderr simplifies AI configuration while adding powerful new capabilities:
...
### What's Different in Sonderr

| Cursor | Sonderr | Key Difference |
|---|---|---|
| `.cursor/rules/*.mdc` with YAML frontmatter | `.sonderr/rules/*.md` plain Markdown | No YAML metadata required |
...
| Global rules in UI settings | `~/.sonderr/rules/*.md` files | Global rules stored as files in home folder |
...
**2. Create Sonderr directory:**

```bash
mkdir -p .sonderr/rules
```
...
for file in .cursor/rules/*.mdc; do
  basename="${file##*/}"
  cp "$file" ".sonderr/rules/${basename%.mdc}.md"
done
...
- Save to `~/.sonderr/rules/cursor-global.md`

**6. Handle legacy `.cursorrules`:**

```bash
cp .cursorrules .sonderr/rules/legacy-rules.md
```

### Converting Cursor's `globs` Patterns

Cursor's `globs` field specifies which files a rule applies to. Sonderr handles this through **mode-specific directories** instead.
...
**Sonderr approach (Option 1 - Mode-specific directory):**

```bash
mkdir -p .sonderr/rules-code
# Save TypeScript-specific rules here
```

**Sonderr approach (Option 2 - Custom mode):**

```yaml
# .sonderrmodes (at project root)
- slug: typescript
  name: TypeScript
...
Then place rules in `.sonderr/rules-typescript/`

### Flattening Nested Cursor Rules

Cursor supports nested `.cursor/rules/` directories. Sonderr uses flat structure with descriptive names:

```bash
# Cursor: .cursor/rules/backend/server/api-rules.mdc
# Sonderr: .sonderr/rules/backend-server-api-rules.md
```
...
| Windsurf | Sonderr | Key Difference |
|---|---|---|
| `.windsurf/rules/*.md` | `.sonderr/rules/*.md` | Same Markdown format |
| GUI configuration for activation modes | File location determines scope | Scope controlled by directory structure |
| "Always On" mode (GUI) | Place in `.sonderr/rules/` | Rules stored as files, not GUI settings |
...
| Global rules in `~/.codeium/windsurf/memories/global_rules.md` | `~/.sonderr/rules/*.md` | Global rules in home folder, multiple files |
...
```bash
cp .windsurf/rules/*.md .sonderr/rules/
```

**4. Migrate global rules:**

```bash
cp ~/.codeium/windsurf/memories/global_rules.md ~/.sonderr/rules/global-rules.md
```

**5. Handle legacy `.windsurfrules`:**

```bash
cp .windsurfrules .sonderr/rules/legacy-rules.md
```
...
# .sonderr/rules/api-conventions.md
# .sonderr/rules/testing-standards.md
# .sonderr/rules/code-style.md
```

### Converting Windsurf's Activation Modes

Windsurf configures activation through the GUI. In Sonderr, file organization replaces GUI configuration:

| Windsurf GUI Mode | Sonderr Equivalent |
|---|---|
| **Always On** | Place in `.sonderr/rules/` (default) |
...
Then place the rule in `.sonderr/rules-test/`

## AGENTS.md Support

All three tools support the `AGENTS.md` standard. If you have one, it works in Sonderr automatically:

```bash
# Verify it exists
ls -la AGENTS.md

# That's it - Sonderr loads it automatically (enabled by default)
```

**Important:** Use uppercase `AGENTS.md` (not `agents.md`). Sonderr also accepts `AGENT.md` (singular) as a fallback.

**Note:** Both `AGENTS.md` and `AGENT.md` are write-protected files in Sonderr and require user approval to modify.

## Understanding Mode-Specific Rules

This is Sonderr's unique feature that replaces both Cursor's `globs` and Windsurf's activation modes.

### Directory Structure

```bash
.sonderr/rules/              # Apply to ALL modes
.sonderr/rules-code/         # Only in Code mode
.sonderr/rules-debug/        # Only in Debug mode
.sonderr/rules-ask/          # Only in Ask mode
.sonderr/rules-{custom}/     # Only in your custom mode
```
...
mkdir -p .sonderr/rules-test

# 2. Save rule as plain Markdown
cat > .sonderr/rules-test/testing-standards.md << 'EOF'
# Testing Rules
- Write tests for all features
- Maintain >80% coverage
EOF
...
- [ ] **Verify rules loaded:** Click law icon (⚖️) in Sonderr panel
- [ ] **Test rule application:** Ask Sonderr to perform tasks following your rules
- [ ] **Organize rules:** Split large files, use clear names
- [ ] **Set up mode-specific rules:** Create directories for specialized workflows
- [ ] **Update team docs:** Document new `.sonderr/rules/` location
- [ ] **Commit to version control:** `git add .sonderr/`
...
```bash
ls -la .sonderr/rules/      # Project rules
ls -la ~/.sonderr/rules/    # Global rules
```
...
- **For always-on rules:** Place in `.sonderr/rules/`
...
- **After migrating:** Organize files accordingly in Sonderr

### Nested Rules Flattened

Cursor's nested directories don't map to Sonderr. Flatten with descriptive names:

```bash
# Bad: .cursor/rules/backend/api/rules.mdc
# Good: .sonderr/rules/backend-api-rules.md
```
...
- **Check setting:** Verify "Use Agent Rules" is enabled in Sonderr settings (enabled by default)
- **Reload:** Restart VS Code if needed

### Choosing Your Workflow

Sonderr supports **both autocomplete and chat-driven workflows**. Choose the approach that fits your coding style, or combine them:
...
```bash
mkdir -p .sonderr/rules-review
mkdir -p .sonderr/rules-docs
```

**Note:** `.sonderrmodes` can be in YAML (preferred) or JSON format. For global modes, edit the `custom_modes.yaml` file via Settings > Edit Global Modes.
