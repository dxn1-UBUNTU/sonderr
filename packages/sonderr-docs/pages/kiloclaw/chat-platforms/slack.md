---
title: "Slack"
description: "Using SonderrClaw with Slack"
---
...
This page covers everything you need to use SonderrClaw with Slack: connecting your bot, controlling who can DM it, and adding it to channels.

## Connecting SonderrClaw to Slack

{% youtube url="https://youtu.be/Q5bt-qH-_pY" title="Slack Setup Guide" caption="How to connect your SonderrClaw agent to Slack" /%}

Create a Slack app from the OpenClaw manifest and link it to your SonderrClaw dashboard.
...
4. Customize the manifest before creating:
   ...
   - Update the slash command if desired (e.g., `/sonderrclaw`)
5. Click **Create**
...
### Step 3: Connect Slack to SonderrClaw

1. In the [SonderrClaw UI](https://app.kilo.ai/claw), find the Slack integration section (may show "not configured")
2. Enter both tokens:
   - The `xapp-` app-level token
   - The `xoxb-` bot user OAuth token
3. Click **Save**
4. Scroll to the top of the SonderrClaw UI and click **Redeploy**. Wait for the instance to come back up

### Step 4: Pair Slack with SonderrClaw
...
By default, SonderrClaw can respond to any DMs and will not respond in Slack channels, even if added.

## Making SonderrClaw DM-Only (from you)

By default, SonderrClaw will respond to DMs from any user in Slack.
...
## Adding SonderrClaw to a Slack Channel

By default, SonderrClaw will not respond in Slack channels, even if added. To have SonderrClaw participate in a Slack channel:
...
Tell your SonderrClaw agent (via DM):
