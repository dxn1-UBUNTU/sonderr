---
title: "SonderrClaw Dashboard Reference"
description: "Managing your SonderrClaw instance from the dashboard"
---

# SonderrClaw Dashboard

{% partial file="sonderrclaw-eol.md" /%}

This page covers everything you can do from the SonderrClaw dashboard. For getting started, see [SonderrClaw Overview](/docs/sonderrclaw/overview).

{% image src="/docs/img/sonderrclaw/dashboard.png" alt="Connect account screen" width="800" caption="The SonderrClaw Dashboard" /%}

## Personal and Organization Instances

The dashboard controls are the same for personal and organization-scoped SonderrClaw instances. Organization instances are selected from the organization context and are listed separately from your **Personal** instance. Availability depends on your organization membership and SonderrClaw entitlement.
...
For access to the full catalog of 335+ models, use the `/model` and `/models` commands in the [Control UI Chat](/docs/sonderrclaw/control-ui#changing-models).

### Channels

Sonderr Chat is always available as SonderrClaw's first-party channel and does not need a token. You can also connect Telegram, Discord, and Slack by entering bot tokens in the Settings tab. See [Connecting Chat Platforms](/docs/sonderrclaw/chat-platforms) for setup instructions.
...
See [Version Pinning](/docs/sonderrclaw/control-ui/version-pinning) for details.
...
> 💡 **Tip**
> If your instance is in a crash loop and you can't access the Control UI, try **Restore Config** from the SonderrClaw dashboard first before redeploying.
...
When your instance is running you can access the [OpenClaw Control UI](/docs/sonderrclaw/control-ui) — a browser-based dashboard for managing your agent, channels, sessions, exec approvals, and more:

1. Click **Open** to launch the OpenClaw web interface in a new tab

See the [Control UI reference](/docs/sonderrclaw/control-ui) for a full overview of its capabilities.

{% callout type="warning" %}
Do not use the **Update** feature in the OpenClaw Control UI to update SonderrClaw. Use **Redeploy** from the SonderrClaw Dashboard instead. Updating via the Control UI will not apply the correct SonderrClaw platform image and may break your instance.
{% /callout %}
...
You need to **approve** each request before the user or device can interact with your agent. See [Pairing Requests](/docs/sonderrclaw/chat-platforms#pairing-requests) for details.

## Changelog

The dashboard shows recent SonderrClaw platform updates. Each entry is tagged as a **feature** or **bugfix**, and some include a deploy hint:
...
- [OpenClaw Control UI](/docs/sonderrclaw/control-ui)
