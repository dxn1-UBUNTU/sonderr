---
title: "Custom Rules"
description: "Define custom rules for Sonderr behavior"
---

# Custom Rules

Custom rules provide a powerful way to define project-specific and global behaviors and constraints for the Sonderr AI agent. With custom rules, you can ensure consistent formatting, restrict access to sensitive files, enforce coding standards, and customize the AI's behavior for your specific project needs or across all projects.
...
Sonderr supports two types of custom rules:
...
Project rules are configured via the `instructions` key in your project's `sonderr.jsonc` file. You can edit this file directly or use the **Settings** webview to manage the `instructions` configuration. Each entry points to a file path or glob pattern:

```jsonc
// sonderr.jsonc
{
  "instructions": [".sonderr/rules/formatting.md", ".sonderr/rules/*.md"],
}
```

You can also place rule files in the **`.sonderr/`** directory structure:
...
Global rules are configured via the `instructions` key in your global `sonderr.jsonc` config file (typically at `~/.config/sonderr/sonderr.jsonc`).

{% callout type="note" title="Migration" %}
The extension is backward compatible with `.sonderr/rules/` directories. Existing rules will continue to work, but migrating to `sonderr.jsonc` is recommended.
{% /callout %}
...
Project rules are configured via the `instructions` key in your project's `sonderr.jsonc` file. Each entry points to a file path or glob pattern:
...
{% callout type="note" title="Migration" %}
The CLI is backward compatible with `.sonderr/rules/` directories. Existing rules will continue to work, but migrating to `sonderr.jsonc` is recommended.
{% /callout %}
...
Rules are managed by editing the `instructions` array in your `sonderr.jsonc` config file. You can also use the **Settings** webview in VS Code to edit the configuration.

- **Add a rule**: Add a file path or glob pattern to the `instructions` array
- **Remove a rule**: Remove the entry from the array
- **Disable a rule temporarily**: Comment out the line in `sonderr.jsonc` (JSONC supports `//` comments)

```jsonc
// sonderr.jsonc
{
  "instructions": [
    ".sonderr/rules/formatting.md",
    // ".sonderr/rules/experimental.md"  -- temporarily disabled
    ".sonderr/rules/naming_conventions.md",
  ],
}
```
...
Rules are managed by editing the `instructions` array in your `sonderr.jsonc` config file directly.
...
Rules are loaded in the order they appear in the `instructions` array in `sonderr.jsonc`:

1. **Global instructions** from the global `sonderr.jsonc` config
2. **Project instructions** from the project's `sonderr.jsonc`
...
If `.sonderr/rules/` directories exist in your project, their contents are automatically included for backward compatibility. To fully migrate, move your rule files and reference them in `sonderr.jsonc`.
{% /callout %}
...
1. Create a `sonderr.jsonc` file in your project root (if it doesn't exist)
2. Create a `.sonderr/rules/` directory (or any directory you prefer)
3. Write your rule as a Markdown file in that directory
4. Add the file path or a glob pattern to the `instructions` array in `sonderr.jsonc`

```jsonc
// sonderr.jsonc
{
  "instructions": [".sonderr/rules/my-new-rule.md"],
}
```

Rules are applied on the next interaction. You can also edit `sonderr.jsonc` through the **Settings** webview in VS Code.
...
{% image src="/docs/img/custom-rules/custom-rules.png" alt="Sonderr ignores request to read sensitive file" width="600" /%}
