---
title: "Skills"
description: "Extend Sonderr capabilities with skills"
---

# Skills

Sonderr implements [Agent Skills](https://agentskills.io/home), a lightweight, open format for extending AI agent capabilities with specialized knowledge and workflows.
...
1. **Discovery**: Skills are scanned from designated directories when Sonderr initializes. Only the metadata (name, description, and file path) is read at this stage—not the full instructions.
...
Global skills are located in the `.sonderr` directory within your Home directory:

- Mac and Linux: `~/.sonderr/skills/`
- Windows: `\Users\<yourUser>\.sonderr\skills\`
...
Located in `.sonderr/skills/` within your project:
...
For interoperability with other tools, Sonderr also loads skills from:
...
You can configure extra skill locations and remote skill URLs in your `sonderr.jsonc` config (project or global):
...
When you change a remote skill's contents or file list, also change its `version`. On the next skill rediscovery (`/reload` or a new session), Sonderr downloads the complete new version before atomically replacing the cached directory. If any download fails, Sonderr keeps the previous cached version.
...
When multiple skills share the same name, project-level skills (`.sonderr/skills/`) take precedence over global skills (`~/.sonderr/skills/`). Skills from compatibility directories (`.claude/skills/`, `.agents/skills/`) and additional configured paths are loaded alongside project and global skills.
...
- In the **CLI**: Skills are loaded when you start a new session or run `sonderr run`
...
In Sonderr, the `name` field **must match** the parent directory name:
...
- **Trusted skills only** — commands execute only in skills from trusted locations: global skills (such as `~/.sonderr/skills/`, `~/.agents/skills/`, and `~/.claude/skills/`), skills built into Sonderr, and absolute skill paths declared in global config. Project skills (`.sonderr/skills/` in a repository) and skills fetched from remote URLs never execute commands; their placeholders are replaced with a marker noting the skill is untrusted.
...
- **Kill switch** — set the `SONDERR_DISABLE_SKILL_SHELL` environment variable to disable embedded command execution entirely.
...
1. Create the skill directory:
   ...
   ```bash
   mkdir -p ~/.sonderr/skills/api-design
   ```
...
- **[Sonderr Marketplace repository](https://github.com/Sonderr-Org/sonderr-marketplace)** — Browse community skills on GitHub and manually download them into your skills directory
...
3. **Check file location**: Ensure `SKILL.md` is directly inside the skill directory (e.g., `.sonderr/skills/my-skill/SKILL.md`), not nested further.

4. **Check config paths**: If using `skills.paths` or `skills.urls`, verify the paths and URLs are correct in your `sonderr.jsonc`.
...
Have you created a skill that others might find useful? Share it with the community by contributing to the [Sonderr Marketplace](https://github.com/Sonderr-Org/sonderr-marketplace)!

{% tabs %}
{% tab label="VSCode" %}

While the new platform does not yet have a built-in marketplace UI, skills from the [Sonderr Marketplace repository](https://github.com/Sonderr-Org/sonderr-marketplace) can be manually downloaded into your `.sonderr/skills/` directory or loaded via `skills.urls` in config.
...
3. **Fork the marketplace repository**: Visit [github.com/Sonderr-Org/sonderr-marketplace](https://github.com/Sonderr-Org/sonderr-marketplace) and create a fork
...
- Follow the [contribution guidelines](https://github.com/Sonderr-Org/sonderr-marketplace/blob/main/CONTRIBUTING.md) in the marketplace repository

For more details on contributing to Sonderr, see the [Contributing Guide](/docs/contributing).
