...
Sonderr includes two built-in subagents:
...
Add agents to the `agent` section of your `sonderr.jsonc` config file. Any key that doesn't match a built-in agent name creates a new custom agent.
...
- **Global**: `~/.config/sonderr/agents/`
- **Project-specific**: `.sonderr/agents/`

The **filename** (without `.md`) becomes the agent name.

If `.sonderr/agents/` is a symlink to a directory outside the project, allow that exact source in your global `~/.config/sonderr/sonderr.jsonc`:
...
2. **Global config** (`~/.config/sonderr/config.json`)
3. **Project config** (`sonderr.jsonc` in the project root)
4. **Global agent markdown files** (`~/.config/sonderr/agents/*.md`)
5. **Project agent markdown files** (`.sonderr/agents/*.md`)
