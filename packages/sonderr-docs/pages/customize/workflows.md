---
title: "Workflows"
description: "Create automated workflows with Sonderr"
platform: new
---

# Workflows

Workflows (also called **slash commands** in the new extension) automate repetitive tasks by defining step-by-step instructions for Sonderr to execute.

{% image src="/docs/img/screenshot-tests/sonderr-vscode/visual-regression/settings/agent-behaviour-workflows-chromium-linux.png" alt="Workflows tab in Sonderr" width="420" caption="Workflows tab in Sonderr" /%}

## Creating Workflows

Workflows are Markdown files stored as **slash commands** in `.sonderr/commands/`:

- **Global commands**: `~/.config/sonderr/commands/` (available in all projects)
- **Project commands**: `[project]/.sonderr/commands/` (project-specific)

If `.sonderr/commands/` is a symlink to a directory outside the project, allow that exact source in your global `~/.config/sonderr/sonderr.jsonc`:
...
For example, a file at `.sonderr/commands/submit-pr.md` is invoked with `/submit-pr`.
...
You can also set `model` and `variant` in the command's frontmatter or in the `command` section of `sonderr.jsonc`.  A variant only applies when the selected model supports it — picking a different model clears a variant the new model does not offer.
...
The new extension automatically migrates legacy workflows from `.sonderr/workflows/` to the new command format on startup. You can also manually move files and remove the `.md` extension from invocations.
...
Create a file called `submit-pr.md` in your `.sonderr/commands` directory:
