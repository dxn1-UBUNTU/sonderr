...
description: Learn how Sonderr's tools automate your development workflow
---

# Tool Use Overview

Sonderr implements a sophisticated tool system that allows AI models to interact with your development environment in a controlled and secure manner. This document explains how tools work, when they're called, and how they're managed.
...
| **Browser Group** | Web browser automation | `sonderr-playwright_*` (via built-in Playwright MCP) | Browser testing and interaction |
...
These tools help Sonderr understand your code and project:
...
These tools help Sonderr make changes to your code:
...
For multiple replacements in one file, Sonderr uses repeated `edit` calls or a patch-style edit when the model supports it.

### Execute Tools

These tools help Sonderr run commands:

- `bash` - Runs shell commands with configurable timeout and working directory

### Web Tools

These tools help Sonderr access web content:
...
`websearch` is available automatically with the Sonderr provider. For models from other providers it is off by default; enable it for all providers by setting `web_search` in `sonderr.jsonc`:
...
In the VS Code extension, the same option lives under **Settings → Web Tools → Web Search → Enable for All Providers**. The `SONDERR_ENABLE_EXA` and `SONDERR_ENABLE_PARALLEL` environment flags also enable it.

#### Web Search Providers

`websearch` routes through the Exa or Parallel search providers. When the Exa provider is used and you are signed into Sonderr, requests go through the Sonderr proxy automatically — no separate Exa API key is required. Setting `EXA_API_KEY` uses your own Exa key instead. Exa searches return at most 10 results.

Set the `SONDERR_WEBSEARCH_PROVIDER` environment variable to force a provider:

| Value | Behavior |
|---|---|
| `exa` | Use Exa — through the Sonderr proxy when signed in, through `EXA_API_KEY` when set |
| `parallel` | Use Parallel |
| `sonderr-exa` | Always route Exa searches through the Sonderr proxy (requires Sonderr sign-in) |

### Browser Tools

The VS Code extension has a built-in browser automation tool powered by [Playwright MCP](https://www.npmjs.com/package/@playwright/mcp). Enable it in Settings → Browser Automation. When enabled, it registers an MCP server named `sonderr-playwright` and exposes tools such as:

- `sonderr-playwright_browser_navigate` - Navigate to a URL
- `sonderr-playwright_browser_click` - Click an element
- `sonderr-playwright_browser_type` - Type text into an element
- `sonderr-playwright_browser_screenshot` - Capture a screenshot
- `sonderr-playwright_browser_snapshot` - Capture an accessibility snapshot
