---
title: "Using MCP in Sonderr"
description: "How to use MCP servers in Sonderr"
---

# Using MCP in Sonderr

Model Context Protocol (MCP) extends Sonderr's capabilities by connecting to external tools and services. This guide covers everything you need to know about using MCP with Sonderr.
...
MCP server configurations are stored inside the main Sonderr config file. There are two levels:

1. **Global Configuration**: `~/.config/sonderr/sonderr.jsonc` — applies to all projects.
2. **Project-level Configuration**: `sonderr.jsonc` in your project root, or `.sonderr/sonderr.jsonc` for a cleaner setup.

**Precedence**: Project-level configuration takes precedence over global configuration.

### Editing MCP Settings

You can edit MCP settings from the Sonderr settings UI:
...
If the UI cannot add a server, edit a Sonderr config file directly and add the server under the top-level `mcp` key. For project-specific servers, edit `./sonderr.json` or `./sonderr.jsonc` if your project already has one; otherwise use `./.sonderr/sonderr.json` or `./.sonderr/sonderr.jsonc` for a cleaner setup. For servers you want in every workspace, use `~/.config/sonderr/sonderr.json` or `~/.config/sonderr/sonderr.jsonc`.

### Config Format

MCP servers are configured under the `mcp` key in `sonderr.jsonc`:
...
Remote servers support OAuth 2.0 authentication. If the server supports it, Sonderr will automatically start the OAuth flow when you connect. You can also disable OAuth with `"oauth": false`.

{% /tab %}
{% tab label="CLI" %}

The CLI accepts several config filenames. The recommended file is `sonderr.json`:

| Scope | Recommended Path | Also supported |
|---|---|---|
| **Global** | `~/.config/sonderr/sonderr.json` | `sonderr.jsonc`, `sonderr.json`, `sonderr.jsonc`, `config.json` |
| **Project** | `./sonderr.json` or `./.sonderr/sonderr.json` | `sonderr.jsonc`, `sonderr.jsonc`, `sonderr.json` |
...
- **Remote (HTTP/SSE)**: Servers hosted over HTTP/HTTPS. Sonderr tries `StreamableHTTP` first, then falls back to `SSE` automatically.
...
| Command | Description |
|---|---|
| `sonderr mcp list` | List all configured MCP servers |
| `sonderr mcp add` | Add an MCP server |
| `sonderr mcp auth` | Authenticate with an MCP server |
| `sonderr mcp logout` | Log out from an MCP server |
| `sonderr mcp debug` | Debug an MCP server connection |
...
Run `sonderr mcp list` to verify the server status.
...
**In your config file:** Add the tool name (or a wildcard pattern) to the `permission` key in `sonderr.jsonc`:
...
Sonderr does not come with any pre-installed MCP servers. You'll need to find and install them separately.

- **Sonderr Marketplace:** Browse community-contributed MCP server configurations and agent skills in the [Sonderr Marketplace](https://github.com/Sonderr-Org/sonderr-marketplace). The marketplace includes ready-to-use configs for popular tools like Figma, Sentry, and more.
- **Community Repositories:** Check for community-maintained lists of MCP servers on GitHub
- **Ask Sonderr:** You can ask Sonderr to help you find or even create MCP servers
- **Build Your Own:** Create custom MCP servers using the SDK to extend Sonderr with your own tools

For full SDK documentation, visit the [MCP GitHub repository](https://github.com/modelcontextprotocol/).

## Using MCP Tools in Your Workflow

After configuring an MCP server, Sonderr will automatically detect available tools and resources. To use them:

1. Type your request in the Sonderr chat interface
2. Sonderr will identify when an MCP tool can help with your task
...
When a connected MCP server provides instructions, Sonderr adds them to the model context so the agent can follow the server's usage guidance. Sonderr omits those instructions when every tool from that server is denied.

Resource-capable servers also make the `list_mcp_resources`, `list_mcp_resource_templates`, and `read_mcp_resource` tools available to the agent. Resource templates describe parameterized URIs; the agent fills in a template, then reads the resulting resource URI. Resource listing and reads use Sonderr's normal read approval flow.
...
- **Server Not Responding:** Check if the server process is running. Use `sonderr mcp debug <server-name>` to inspect the connection.
- **Permission Errors:** Ensure API keys and credentials are set in your `sonderr.jsonc` config or via `{env:VARIABLE_NAME}` references.
