...
Model Context Protocol (MCP) supports two primary transport mechanisms for communication between Sonderr and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (Sonderr) spawns an MCP server as a child process
...
- **Locality**: Runs on the same machine as Sonderr
...
- Single-client scenarios (one Sonderr instance per server)
...
1. The client (Sonderr) connects to the server's SSE endpoint via HTTP GET request
...
- **Remote Access**: Can be hosted on a different machine from Sonderr
...
STDIO servers run locally on the same machine as Sonderr, which has several important implications:
...
- **Execution**: Starts and stops with Sonderr (child process lifecycle)
...
- Start when needed by Sonderr
- Not require network configuration
- Need to be installed alongside Sonderr or via a package manager
...
## Configuring Transports in Sonderr

For detailed information on configuring STDIO and SSE transports in Sonderr, including example configurations, see the [Understanding Transport Types](using-in-sonderr-code#understanding-transport-types) section in the Using MCP in Sonderr guide.
