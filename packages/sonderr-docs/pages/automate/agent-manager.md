...
The Agent Manager is a control panel for running and orchestrating multiple Sonderr agents, with support for parallel worktree-isolated sessions.

The Agent Manager is a **full-panel editor tab** built directly into the extension. It uses the extension's embedded runtime, so no separate Sonderr CLI installation or CLI authentication setup is required. It supports:
...
- Command Palette: "Sonderr: Open Agent Manager"
...
Agent Manager worktree defaults belong to a repository. Open a project's settings button, then select the **Agent Manager** tab in Sonderr Settings. The repository selector controls which project's default base branch and setup script you edit.
...
Managed worktrees are created under `.sonderr/worktrees/` in your project. Sonderr also stores Agent Manager UI state in `.sonderr/agent-manager.json`.
...
The panel header also provides **Copy PR link**, **Open in browser**, and **Close**. Sending a comment gives it to Sonderr as review context. It does not post a reply to GitHub. The panel intentionally has no reply composer; write replies in GitHub.

### Creating a New Worktree Session

1. Click **New Worktree** or press `Cmd+N` (macOS) / `Ctrl+N` (Windows/Linux) to open the new worktree dialog
2. Enter a branch name (or let Sonderr generate one)
3. Type your first message, then create the worktree

Sonderr creates the worktree from the selected project's configured default base branch. In a multi-project workspace, the selected project determines this setting. An explicit base branch selected in the dialog takes precedence. If no default is configured, Sonderr falls back to automatic detection of the repository's remote default branch. The agent works in isolation, so your main branch is unaffected.
...
3. Sonderr creates one worktree + session per version and runs them in parallel
...
File mentions, clickable file links, review-comment file links, file-link validation, and native VS Code opening resolve against the referenced session's directory or worktree. If a session ID is present in multiple projects, Sonderr rejects the unqualified reference rather than choosing an arbitrary project.
...
Sonderr can start Agent Manager sessions from chat with the `agent_manager` tool. It is available by default only in the VS Code extension because Agent Manager is an extension feature.
...
When you use `@terminal` in an Agent Manager prompt, Sonderr captures the focused terminal for the selected session or worktree. This includes embedded **Run** and **Setup** tabs. Terminal context is limited to 500 lines or 50,000 characters; longer output is truncated.
...
The dropdown choice is remembered per panel and becomes the default for new panels. You can also set the default directly with the `sonderr-code.new.agentManager.terminalButtonDestination` setting (`vscode` or `agentManager`). The `Cmd+/` (macOS) / `Ctrl+/` (Windows/Linux) shortcut follows the same destination.
...
Create a script file in `.sonderr/` using the appropriate filename for your platform:
...
Sonderr runs the script automatically whenever a new worktree is created. It uses `sh` for POSIX scripts, PowerShell for `.ps1`, and `cmd.exe` for `.cmd` / `.bat`, so executable permissions are not required.
...
# Copy a nested env file that Sonderr does not auto-copy.
...
Before the setup script runs, Sonderr automatically copies root-level `.env` files from the main repo into the new worktree.
...
- Files that already exist in the worktree, because Sonderr never overwrites them

Use `.sonderr/setup-script` for anything outside the automatic copy rules, including nested env files, ignored local config, local certificates, local database files, generated config directories, or tool-specific files required to run the project.
...
For example, on macOS / Linux create `.sonderr/run-script`:
...
Agent Manager state is persisted in `.sonderr/agent-manager.json`. It stores worktrees, sections, session tabs, ordering, collapsed state, diff preferences, and cached PR metadata. Git branches and worktree directories remain on disk separately.

Closing a managed worktree removes it from Agent Manager, deletes its `.sonderr/worktrees/` directory, and deletes the local branch. Closing an imported external worktree removes the Agent Manager entry but leaves the external directory and branch untouched.
...
- **A configured base branch is missing** — Sonderr clears the stale project setting and uses automatic remote-default detection for the new worktree. Select the project and configure a new default if needed.
