---
title: "Plugins"
description: "Extend the Sonderr CLI with custom hooks, tools, auth providers, and more"
platform: new
---

# Plugins

Plugins extend Sonderr by hooking into events, adding custom tools, registering auth or model providers, and customizing runtime behavior. They are TypeScript or JavaScript modules loaded at startup, and work in both the Sonderr CLI and the VS Code extension.
...
- Global: `~/.config/sonderr/plugin/`
- Project: `.sonderr/plugin/` or legacy `.sonderr/plugin/`
...
### From the `sonderr plugin` command

Install an npm plugin and patch your config in one step:

```bash
# Install into the current project's config
sonderr plugin my-plugin

# Install into your global config
sonderr plugin my-plugin --global

# Replace an existing entry
sonderr plugin my-plugin --force
```

The command resolves the package, reads its `package.json` for plugin entrypoints, and writes the entry into the appropriate config file (`.sonderr/sonderr.jsonc` / `.sonderr/tui.jsonc` for local installs, or `~/.config/sonderr/sonderr.jsonc` / `~/.config/sonderr/tui.jsonc` for `--global`) while preserving JSONC comments.

### How plugins are installed

- **npm plugins** are installed automatically at startup using Bun. Packages and their dependencies are cached under `packages/` in the current CLI XDG cache directory (`~/.cache/sonderr/packages/` by default, or `$XDG_CACHE_HOME/sonderr/packages/` when `XDG_CACHE_HOME` is set).
...
- **Install scripts are disabled** for npm plugins. Sonderr installs packages with lifecycle scripts such as `install` and `postinstall` blocked.
- **Local plugins** are loaded directly from the plugin directory. If your plugin imports external packages, add a `package.json` to your config directory (see [Dependencies](#dependencies)) — Sonderr runs `bun install` on startup so imports resolve.

### Load order

Plugins from all sources run on every session. They load in this order:

1. Internal built-ins (Sonderr Gateway auth, Codex auth, Copilot auth, Cloudflare, etc.)
2. Global config plugin array (`~/.config/sonderr/sonderr.json`)
3. Global plugin directory (`~/.config/sonderr/plugin/`)
4. Project config plugin array (`sonderr.json` / `sonderr.json`)
5. Project plugin directory (`.sonderr/plugin/` and friends)
...
Set the `SONDERR_PURE=1` environment variable to skip all external plugins — only built-in plugins will load. Useful for reproducible CI runs or debugging.
...
```ts
// .sonderr/plugin/hello.ts
import type { Plugin } from "@sonderr/plugin"
...
| `client` | A Sonderr SDK client (`@sonderr/sdk`) for calling the local server. |
| `$` | [Bun's shell API](https://bun.com/docs/runtime/shell). |
| `serverUrl` | URL of the local Sonderr server. |
...
Workspace adaptors let plugins add custom workspace targets to Sonderr's workspace creation flow. This API is experimental and may change.
...
const WorkspacePlugin: Plugin = async ({ experimental_workspace }) => {
  experimental_workspace.register("folder", {
    ...
    configure(config) {
      return { ...config, directory: `/tmp/sonderr-${Date.now()}` }
    },
    async create(config) {
...
Published npm plugins should declare separate package entrypoints for each runtime they support. Sonderr detects install targets from `package.json`:
...
```bash
bun add -d @sonderr/plugin
```

```ts
import type { Plugin } from "@sonderr/plugin"
import { tool } from "@sonderr/plugin/tool"
```

Sonderr automatically creates a `package.json` in config directories that contain a `plugin/` folder and installs `@sonderr/plugin` so types resolve out of the box.
...
Sonderr runs `bun install` at startup so your plugins can import the packages:

```ts
// .sonderr/plugin/escape-bash.ts
...
| `shell.env` | Inject environment variables into every shell command Sonderr runs. |
...
Sonderr fills provider/model IDs from the returned catalog and uses the returned models in the picker and provider router.
...
The `event` hook fires for every event on Sonderr's internal bus. Common event types include:
...
For tools that don't need the full plugin context, drop them in a `tool/` or `tools/` folder inside any config directory — for example `.sonderr/tool/database.ts` or `~/.config/sonderr/tool/database.ts`. The filename becomes the tool name, and each file exports a `tool()` definition directly. The layout is identical to the [Sonderr custom tools guide](https://sonderr.ai/docs/custom-tools); use `.sonderr/`, or legacy `.sonderr/`, instead of `.sonderr/`.
...
Prefer `client.app.log()` over `console.log` so entries land in Sonderr's log pipeline:
...
By default, Sonderr sends a synthetic "continue" turn after compaction so the agent resumes the interrupted task. Use `experimental.compaction.autocontinue` to disable that turn for specific sessions or providers:
...
Plugins can also target the Sonderr TUI itself — registering slash commands, routes, slots, dialogs, and keybinds. TUI plugins are SolidJS modules exported from `"./tui"` in your plugin package, or theme-only packages declared with `oc-themes`.

TUI plugins live in a separate module namespace (`@sonderr/plugin/tui`) and have their own API surface (`TuiPluginApi`). Because the TUI API is larger and still evolving, this guide doesn't cover it exhaustively — use the types in `@sonderr/plugin/tui` as the reference, and look at the built-in TUI plugins under `packages/cli/src/cli/cmd/tui/feature-plugins/` for working examples.
...
- **Plugin failed to load** — check the CLI logs with `sonderr --print-logs --log-level DEBUG`. Load failures are also surfaced as session errors in the TUI and VS Code extension.
...
- **Plugin loads in dev but not in CI** — verify `SONDERR_PURE` is not set, and that npm-installed plugins are cached under `packages/` in the current CLI XDG cache directory (`~/.cache/sonderr/packages/` by default, or `$XDG_CACHE_HOME/sonderr/packages/` when `XDG_CACHE_HOME` is set). Run with `--log-level DEBUG` to see install output.
- **Reset the plugin cache** — delete the plugin package folder under the CLI's `packages/` cache directory (or the `node_modules` cache under your config directory) and restart Sonderr.

---

## Reference

- Types: [`@sonderr/plugin`](https://github.com/Sonderr-Org/sonderr/tree/main/packages/plugin) — `Plugin`, `Hooks`, `PluginInput`, `ToolDefinition`, `AuthHook`, `ProviderHook`.
- Example plugin: [`packages/plugin/src/example.ts`](https://github.com/Sonderr-Org/sonderr/blob/main/packages/plugin/src/example.ts)
- CLI command: [`sonderr plugin`](/docs/code-with-ai/platforms/cli-reference#sonderr-plugin)
- Upstream docs (behavior is identical to Sonderr): [sonderr.ai/docs/plugins](https://sonderr.ai/docs/plugins) and [sonderr.ai/docs/custom-tools](https://sonderr.ai/docs/custom-tools)
