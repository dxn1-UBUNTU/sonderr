...
- **Run script** — create `.sonderr/run-script` (or `.ps1` / `.cmd` / `.bat` on Windows) and trigger it with `Cmd+E` / `Ctrl+E`. Runs in whichever worktree is selected. Gets `WORKTREE_PATH` and `REPO_PATH` in the environment.
...
export COMPOSE_PROJECT_NAME="sonderr_${name}"
...
Use `.sonderr/setup-script` to make new worktrees runnable without manual setup. It runs after Sonderr copies root-level `.env` and `.env.*` files, and before the agent starts in the new worktree.

Sonderr's env copy is intentionally narrow:
...
Put the remaining project-specific setup in `.sonderr/setup-script`, for example copying `apps/web/.env.local`, creating a per-worktree database, or installing dependencies. The setup script receives `WORKTREE_PATH` and `REPO_PATH` in the environment.
...
- **`sonderr review` in CI** — automated PR review. See [Code Reviews](/docs/automate/code-reviews/overview) for the setup.
