...
Instead of maintaining a static list that's perpetually behind, we built something better — a real-time leaderboard showing which models Sonderr users are actually having success with right now.
...
- For non-interactive use, pass `--model` flag to `sonderr run` (e.g., `sonderr run --model claude-sonnet-4-20250514`).
- Set the global default with the `model` key in `sonderr.jsonc`, or configure per-agent models in the `agent` section.
...
You don't need a paid API key to use Sonderr productively. For the lowest cost on paid work, [Auto Efficient](/docs/code-with-ai/agents/auto-model#tiers) (`sonderr-auto/efficient`) routes each request to the cheapest model proven accurate enough for that task. The fastest way to start for free is [Auto Model Free](/docs/code-with-ai/agents/auto-model) (`sonderr-auto/free`), which routes to the best available free models automatically. See [Using Sonderr for Free](/docs/getting-started/using-sonderr-for-free) for the full zero-cost setup.
...
- **Via config file** — edit `sonderr.jsonc`:
...
```
vscode://sonderr.sonderr-code/sonderr/switch?model=<modelID>
vscode://sonderr.sonderr-code/sonderr/switch?agent=<agentName>
vscode://sonderr.sonderr-code/sonderr/switch?model=<modelID>&agent=<agentName>
```

Replace `<modelID>` with a Sonderr Gateway model ID such as `sonderr-auto/free`. Replace `<agentName>` with a visible primary agent ID such as `code` or `plan`, rather than its display name.

### Example: Auto Free

To open Sonderr and switch to the [Auto Free](/docs/code-with-ai/agents/auto-model) tier (`sonderr-auto/free`), use:

```
vscode://sonderr.sonderr-code/sonderr/switch?model=sonderr-auto%2Ffree
```

To switch only to Plan and use its normal model selection, specify the agent without a model:

```
vscode://sonderr.sonderr-code/sonderr/switch?agent=plan
```

To select both at the same time, include both parameters:

```
vscode://sonderr.sonderr-code/sonderr/switch?model=sonderr-auto%2Ffree&agent=plan
```
...
- **VS Code open**: the Sonderr sidebar is focused and the linked selection is applied to the active session immediately.
- **VS Code closed**: VS Code launches, then applies the selection once the extension is ready.
- When `model` is provided, it must identify a model in the current Sonderr Gateway catalog. Invalid or unavailable models cause the deep link to be ignored.
...
```html
<a href="vscode://sonderr.sonderr-code/sonderr/switch?model=sonderr-auto%2Ffree&amp;agent=plan">
  Open Sonderr with Auto Free in Plan
</a>
```
