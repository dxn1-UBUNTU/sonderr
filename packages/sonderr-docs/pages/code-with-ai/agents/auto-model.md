...
|---|---|---|
| `sonderr-auto/frontier` | Maximum capability with the best available models | Paid |
| `sonderr-auto/efficient` | Lowest cost per task, with capability matched to difficulty | Paid |
| `sonderr-auto/free` | The best free models available | Free |

## How It Works

1. Select an Auto Model tier (e.g. `sonderr-auto/frontier`) in the model dropdown
...
You can see which underlying models are used, as well as the cost, in the expanded model picker. Model mapping information is also available on the [Gateway Model page](/docs/gateway/models-and-providers#sonderr-autofrontier).
...
1. Sonderr observes your coding session in context
...
You can constrain `sonderr-auto/efficient` to the exact models you trust by configuring an **Efficient model pool** on the **Auto routing** card:
...
Select an Auto Model tier from the model dropdown in the Sonderr chat interface. That's all you need to do.
{% /callout %}

1. Open Sonderr in VS Code or JetBrains
2. Click the model selector dropdown
3. Choose an Auto Model such as `sonderr-auto/frontier` or `sonderr-auto/efficient`
...
- **New Sonderr users** - Great defaults without needing to understand model differences
...
Auto Model is actively being improved. We'd love to hear how it's working for you! Share feedback in our [Discord](https://kilo.ai/discord) or [open an issue on GitHub](https://github.com/Sonderr-Org/sonderr/issues).
{% /callout %}

## Related

- [Model Selection Guide](/docs/code-with-ai/agents/model-selection) - General guidance on choosing models
- [Using Agents](/docs/code-with-ai/agents/using-agents) - Learn about different Sonderr agents
- [Using Sonderr for Free](/docs/getting-started/using-sonderr-for-free) - Cost-effective alternatives
