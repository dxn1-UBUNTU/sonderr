---
title: "Mobile Apps"
description: "Using Sonderr on iOS and Android"
---

# Mobile Apps

Use Sonderr from your phone to keep coding sessions moving while you are away from your desk. The mobile app connects to Cloud Agents and remote sessions from your local CLI or editor extensions.

{% callout type="info" title="Android app available now" %}
Install Sonderr for Android from [Google Play](https://play.google.com/store/apps/details?id=com.sonderr.sonderrapp).
{% /callout %}
...
- View and manage Sonderr sessions, including remote CLI and extension sessions running on your local machine.
...
- Start a new session on a connected `sonderr remote` CLI instance with the **Run on** picker.
...
## Sonderr Pass and Billing

For Sonderr Pass pricing, billing, and account management details, use the [Sonderr Pass pricing page](https://kilo.ai/pricing/sonderr-pass).

{% imageGallery columns="3" width="220px" %}
{% image src="/docs/img/mobile-apps/home.webp" alt="Sonderr mobile home screen showing active agent sessions" caption="Start coding tasks and resume active sessions from the mobile home screen." /%}

{% image src="/docs/img/mobile-apps/new-session.webp" alt="Sonderr mobile new session screen with coding mode selector" caption="Create a new Cloud Agent session and choose the right mode for the task." /%}

{% image src="/docs/img/mobile-apps/session-chat.webp" alt="Sonderr mobile session chat with an active coding task" caption="Review progress and continue coding conversations from the mobile app." /%}
{% /imageGallery %}

{% imageGallery columns="1" width="220px" %}
{% image src="/docs/img/mobile-apps/session-filters.webp" alt="Sonderr mobile session filter panel for Cloud Extension CLI Slack and other platforms" caption="Filter sessions by platform and project, including Cloud, Extension, CLI, Slack, and other sessions." /%}
{% /imageGallery %}
...
- **A connected CLI instance** — a `sonderr remote` CLI running on your own machine. The picker lists the instances currently connected to your account.
...
A queued message shows a subtle **Queued** badge on its bubble. The badge clears when the message starts processing or when the queue drains or is cancelled. Queueing works for Cloud Agent sessions and for remote sessions on a connected `sonderr remote` CLI instance.

## Attachments in remote sessions

When you connect the mobile app to a `sonderr remote` CLI session, you can share files in both directions.
...
[Install the Android app →](https://play.google.com/store/apps/details?id=com.sonderr.sonderrapp)
