---
title: "Cloud Agent"
description: "Using Sonderr in the browser"
---

# {% $markdoc.frontmatter.title %}

Cloud Agents let you run Sonderr in the cloud from any device, without relying on your local machine. They provide a remote development environment that can read and modify your GitHub and GitLab repositories, run commands, and auto-commit changes as work progresses.

## What Cloud Agents Enable

- Run Sonderr remotely from a browser
...
Cloud Agent compute is billed per second while the container is awake. Compute and model inference draw from the same Sonderr credit balance, but they are charged separately.
...
See [Sonderr pricing](https://kilo.ai/pricing) for current rates and pricing for other cloud products.
...
4. **Start chatting with Sonderr.**

Your work is always pushed to GitHub, ensuring nothing is lost.

## Starting Tasks from the CLI

Use the `sonderr cloud` command to run Cloud Agent tasks without opening the browser:

```bash
sonderr cloud start --prompt "Fix the flaky login test" --repo Sonderr-Org/sonderr
```

`sonderr cloud` can start tasks, send follow-up prompts, and check task status and results. Repository, branch, model, mode, and organization are inferred from your local checkout and CLI defaults unless you pass the matching flags. Add `--stream` to `sonderr cloud start` to print task events as JSONL until the task completes. See the [CLI reference](/docs/code-with-ai/platforms/cli-reference#sonderr-cloud) for all commands and options.
...
### Setup Commands vs `.sonderr/setup-script`

- Cloud Agent executes **Setup Commands** configured in the Cloud UI/profile.
- Cloud Agent does **not** automatically discover or run `.sonderr/setup-script`.
- If you want to use `.sonderr/setup-script` in Cloud Agent, call it explicitly from Setup Commands, for example: `bash .sonderr/setup-script`.
- If both are present, execution order is:
  1. Setup Commands (in the order you define them)
  2. Anything those commands invoke (such as `.sonderr/setup-script`)

## Skills

Cloud Agents support project-level [skills](/docs/code-with-ai/platforms/cli#skills) stored in your repository. When your repo is cloned, any skills in `.sonderr/skills/` (or the legacy `.sonderr/skills/`) are automatically available. Skill folders are uploaded as `.zip` archives, with up to 40 companion files per skill.

{% callout type="note" %}
Global skills (`~/.sonderr/skills/`) are not available in Cloud Agents since there is no persistent user home directory.
{% /callout %}
...
- Same Sonderr account on both CLI and Cloud Agent
...
- **Context is persistent across messages.**
  Sonderr remembers previous turns within the same session.
