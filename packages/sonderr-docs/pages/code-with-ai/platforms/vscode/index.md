---
title: "Sonderr for VS Code: Free Open-Source AI Coding Extension"
description: "Using Sonderr in Visual Studio Code"
---

# Sonderr for VS Code

The Sonderr VS Code extension uses the shared Sonderr agent runtime while integrating chat, agents, and automation into the editor.

## Installation

1. Open VS Code
2. Go to Extensions (`Ctrl+Shift+X` / `Cmd+Shift+X`)
3. Search for "Sonderr"
4. Click the dropdown arrow next to **Install** and select **Install Pre-Release Version**

The extension includes its own embedded runtime. No separate Sonderr CLI installation is required.
...
- **[JSONC config files](/docs/getting-started/settings)** — Portable settings in `sonderr.jsonc` instead of VS Code settings
- **[Granular permissions](/docs/getting-started/settings/auto-approving-actions)** — Per-tool permission rules with glob patterns
- **[Agents](/docs/code-with-ai/agents/using-agents)** — Customizable agents (`.sonderr/agents/*.md`) replacing the modes system
...
When Sonderr starts an agent in the background, a collapsible status strip appears in the chat header. The strip stays visible while the transcript scrolls, so you can check background work without searching the conversation. Click the strip to expand it and see each agent.
...
Settings apply across extension surfaces, including the sidebar and Agent Manager. The standalone CLI uses the same `~/.config/sonderr/sonderr.jsonc` (global) and `./sonderr.jsonc` (project) files when used directly.

## Interface Language

The extension UI follows VS Code's display language by default. Override it with the `sonderr-code.new.language` setting (for example `en`, `de`, `ja`, or `fa`). Right-to-left languages such as Arabic and Persian switch the layout direction automatically.

## Proxy and Certificate Troubleshooting

Sonderr for VS Code starts its embedded runtime from the extension and applies the relevant VS Code network settings to that runtime. On managed networks, configure proxy and certificate trust in VS Code settings rather than in a separate CLI install.
...
- Leave `http.proxySupport` enabled unless you intentionally want VS Code and Sonderr to ignore proxy settings.
...
- If the operating system trust store is not enough, set `sonderr-code.new.extraCaCerts` to the absolute path of a PEM file that contains the additional certificate authority certificates.
...
```json
{
  ...
  "sonderr-code.new.extraCaCerts": "/absolute/path/to/corporate-ca.pem"
}
```
