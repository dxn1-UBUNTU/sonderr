---
title: "Linear"
description: "Using Sonderr in Linear"
---

# Sonderr for Linear

**Sonderr for Linear** connects Sonderr to your Linear workspace, so you can trigger implementations, debug issues, and investigate bugs directly from your project management tool. Mention `@sonderr` on any issue and the bot gets to work.
...
{% image src="/docs/img/connect/linear/linear-fix-issue.png" alt="Asking @sonderr to fix an issue in Linear" width="800" /%}
...
{% image src="/docs/img/connect/linear/linear-multi-repo.png" alt="Asking @sonderr to apply changes across multiple repositories from Linear" width="800" /%}
...
```
@sonderr what could be the cause of this issue?
```

The bot examines the issue context and searches the connected codebase to surface likely causes.

{% image src="/docs/img/connect/linear/linear-understand-issue.png" alt="Asking @sonderr to analyze the cause of a Linear issue" width="800" /%}

---

## How It Works

1. **Mention `@sonderr`** in a comment on a Linear issue
2. **Sonderr reads the issue context** — title, description, labels, and comment thread
3. **A Cloud Agent spins up** to process the request (you'll see a thinking animation in Linear while this happens)
4. **Sonderr responds** with an answer or opens a pull request with the implementation

When Sonderr creates a pull request, it links back to the Linear issue so everything stays connected.
...
3. Authorize Sonderr to access your Linear workspace

Once connected, `@sonderr` is available as a mention in any issue across your Linear workspace.
