...
Popular choices:
- **Sonderr Auto Frontier** — highest quality models, best results (recommended)
- **Sonderr Auto Efficient** — cheapest model proven accurate enough for each task, with capability matched to difficulty (minimum for Gas Town)
...
Adding a GitHub PAT ensures that all commits, branches, and PRs created by your agents appear as **you** in git history. Without it, activity shows up under the Sonderr GitHub App bot account.
{% /callout %}
...
- PRs show as created by the Sonderr bot
- Some `gh` CLI operations may not work

### GitHub App Installation

The [Sonderr GitHub App](https://github.com/apps/sonderr-code) provides base-level repository access. It's installed per-organization or per-repository and gives agents read/write access to code, PRs, and issues.
