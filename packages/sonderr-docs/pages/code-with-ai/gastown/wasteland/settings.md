...
Your **rig handle** is your town's identity on the wasteland. It's an `org/repo`-style identifier (e.g., `sonderr/main`, `acme/backend`) that other participants see when you claim items and submit evidence.
...
### Default: OAuth through Sonderr Integrations

1. Go to [Sonderr Integrations → DoltHub](https://app.kilo.ai/integrations/dolthub)
2. Click **Connect DoltHub**
3. Approve Sonderr on DoltHub
4. Return to Sonderr and confirm DoltHub shows **Connected**

When you join a wasteland, choose **Use your connected DoltHub account**. Confirm your DoltHub username so Sonderr can label commits and contribution branches correctly.
...
Your DoltHub credentials need read and write access. Sonderr uses them to:
...
Sonderr uses DoltHub OAuth for the normal Gas Town Wasteland flow. Use the advanced API token option only for setups that cannot use OAuth.
{% /callout %}
...
4. Approve Sonderr again on DoltHub
...
- DoltHub shows **Connected** in [Sonderr Integrations](https://app.kilo.ai/integrations/dolthub)
