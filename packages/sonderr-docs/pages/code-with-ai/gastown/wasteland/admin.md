...
4. Authorize DoltHub — OAuth through Sonderr Integrations is the default. The advanced API token option is also supported, but it must have write access to the upstream repo.
...
### Sonderr Cloud hosted option

<!-- TODO: verify — confirm whether Sonderr Cloud offers a hosted wasteland creation flow or if self-serve is CLI/dashboard only -->

Sonderr Cloud provides the managed infrastructure (Cloudflare containers, encryption, billing) but the upstream DoltHub database is always user-owned. There is no fully-hosted wasteland where Sonderr manages the DoltHub repo on your behalf — you always create and own the upstream. The wasteland container runs on Sonderr's infrastructure, but data lives in your DoltHub account.
...
1. Click **Add member**
2. Enter the user's Sonderr user ID
...
The **Test admin access** button checks whether Sonderr can write to a scratch branch on the upstream. If it works, you see a green "Admin access verified" badge. If it fails, DoltHub OAuth may be disconnected, the API token may be invalid, the wrong org may be connected, or write access may be missing.

Use this after reconnecting DoltHub OAuth, updating an API token, or toggling the admin checkbox. It confirms Sonderr can push before you run admin actions.
