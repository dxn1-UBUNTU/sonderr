...
This guide walks you through setting up Mistral's Codestral model for free autocomplete in Sonderr. Mistral offers a free tier that's perfect for getting started with AI-powered code completions.

## Prerequisites

- A [Sonderr account](https://app.kilo.ai) (free to create)
...
## Step 4: Add Your Key via BYOK in Sonderr

1. Log into the [Sonderr platform](https://app.kilo.ai).
...
Once your BYOK key is saved, Sonderr's autocomplete will automatically use your Codestral key through the Sonderr Gateway. No additional configuration is needed in the extension.

1. Open VS Code with the Sonderr extension installed.
...
The autocomplete status bar in VS Code shows the current provider ("Sonderr Gateway") and tracks cumulative cost. With BYOK, requests are billed directly by Mistral at their rates (Codestral has a free tier) and show as $0.00 on your Sonderr balance.
...
```
Your Editor → Sonderr Gateway (with your key) → Mistral
```

- The Sonderr Gateway detects your BYOK key and routes autocomplete requests using it.
- You are billed directly by Mistral — Sonderr does not add any markup.
- If your BYOK key is invalid, the request will fail (it does not fall back to Sonderr's keys).

## Troubleshooting

- **Autocomplete not appearing?** Check that autocomplete is enabled in Sonderr settings (it is on by default). Also verify you are signed into Sonderr in the extension.
...
- **Seeing charges on your Sonderr balance?** If you haven't configured BYOK, autocomplete defaults to using your Sonderr credits. Add your Codestral key via BYOK to route requests through your own Mistral account.
