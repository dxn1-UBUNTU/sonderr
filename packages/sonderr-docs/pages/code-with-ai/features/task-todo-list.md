...
{% image src="/docs/img/screenshot-tests/sonderr-vscode/visual-regression/composite-webview/todo-write-docs-overview-chromium-linux.png" alt="Task Todo List overview showing interactive checklist in Sonderr" width="420" /%}
...
- Ask Sonderr to "use the [`todowrite` tool](/docs/automate/tools)"
- Say "create a todo list"

**The bottom line**: Sonderr decides what goes in the list, but you can provide feedback during approval dialogs.
...
**Now**: Sonderr creates structured checklists that update automatically as work progresses. You see exactly where you are and what's coming up.
...
{% image src="/docs/img/screenshot-tests/sonderr-vscode/visual-regression/chat/task-header-with-todos-chromium-linux.png" alt="Task header summary showing todo list progress" width="420" /%}
...
- Edit descriptions when Sonderr asks for approval
- Stage changes using the "Edit" button

**3. Environment Details**
Background "REMINDERS" table that keeps Sonderr informed about current progress
...
When Sonderr proposes a todo list update, you can edit the list before approving. Use the "Edit" button in the tool block to update item text, add or remove steps, or adjust status. Once approved, Sonderr continues with the updated list.

## Common questions

**"Can I create my own todo lists?"**
Yes, just ask Sonderr to use the `todowrite` tool. Sonderr stays in control of the content and workflow.

**"What about simple tasks?"**
Sonderr typically skips todo lists for simple tasks. The overhead isn't worth it.

**"Why can't I directly edit the list?"**
Design choice. Sonderr maintains authority over task management to ensure consistent progress tracking. You provide input, Sonderr executes.

---

## Settings

You can disable todo lists in Settings -> Advanced -> **Enable todo list tool**. When disabled, Sonderr won't create or update todo lists, and the REMINDERS table won't appear in Environment Details.
