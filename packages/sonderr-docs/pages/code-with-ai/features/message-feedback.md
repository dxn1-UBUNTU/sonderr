...
description: "Rate assistant responses with thumbs up/down to help Sonderr improve"
---

# Message Feedback

You can give a thumbs up or thumbs down to any response Sonderr gives you. It's a quick way to tell us when something worked well, or when it didn't.
...
When Sonderr finishes a response, you'll see thumbs-up and thumbs-down buttons next to the **Copy response** button. Click one to send your rating. Click the same button again to take it back, or click the other one to change your mind.
...
Your feedback is the most direct way to tell us what's working and what isn't. We use it to tune the prompts behind Sonderr's behavior, pick better default models, and find and fix problems faster than we could on our own.
...
- For responses that came through Sonderr Gateway, the request ID so we can match the rating back to the session.

No further information is included in the rating feedback. For more information on telemetry, see [PRIVACY.md](https://github.com/Sonderr-Org/sonderr/blob/main/PRIVACY.md).
...
Set `SONDERR_TELEMETRY_LEVEL=off` in your environment before starting Sonderr:

```bash
SONDERR_TELEMETRY_LEVEL=off sonderr
```
