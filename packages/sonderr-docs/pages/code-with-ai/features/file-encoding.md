---
title: "File Encoding"
description: "How Sonderr handles text file encodings when reading and editing files"
---

# File Encoding

Sonderr automatically detects the text encoding of each file it reads and preserves that encoding when writing changes back. You can work with source files in any supported encoding without worrying about Sonderr corrupting them or showing the model garbled text.
...
New files Sonderr creates are always UTF-8 without a BOM. Encoding detection only runs when Sonderr reads or overwrites an existing file.
...
If Sonderr displays a file as garbled text, or writes it back in a different encoding than it was saved in, please open an issue at [github.com/Sonderr-Org/sonderr/issues](https://github.com/Sonderr-Org/sonderr/issues) and include all of the following:
...
- **The model and provider** you were using when the issue occurred, for example `claude-sonnet-4.5` via Sonderr Gateway.
- **The exact Sonderr version** you are running. For the CLI, run `sonderr --version`. For the VS Code extension, open the Extensions view and check the version next to "Sonderr".
