...
Sonderr Enterprise lets your organization securely manage access using **Single Sign-On (SSO)**. With SSO enabled, team members can sign in to Sonderr using your company's existing identity provider, such as Okta, Github, Google Workspace, etc.

{% callout type="warning" %}
**IDP-initiated logins are not currently supported.** Users must navigate to the [Sonderr Web App](https://app.kilo.ai) to log in. Logging in directly from your identity provider's dashboard is not supported at this time.
{% /callout %}
...
- Admin or Owner permissions for your Sonderr organization.
...
Once the Sonderr team has enabled SSO for your organization, your named admin will get an email from WorkOS to configure SSO.

{% callout type="warning" %}
**Save domain policy for last.**

If you configure domain policy before setting up SSO, you may lock users out of Sonderr.
{% /callout %}
