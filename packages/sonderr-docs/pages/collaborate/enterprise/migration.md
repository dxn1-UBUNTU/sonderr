---
title: "Migration"
description: "Migrate your team to Sonderr Enterprise"
---

# Migration

Switch to **Sonderr Teams** or **Sonderr Enterprise** from other AI coding tools and experience transparent pricing, no vendor lock-in, and superior team management capabilities.

## Why Teams Switch to Sonderr
...
**Sonderr Teams** and **Sonderr Enterprise** show you what each AI request costs. Model inference is charged at provider rates with no markup, and credit purchases have a disclosed 5% payment-processing fee.

### No Rate Limiting

**Other tools** slow you down with rate limits and model switching when you need AI most.

**Sonderr Teams** and **Sonderr Enterprise** never limit your usage - pay for what you use, use what you need.

### True Team Management

**Other solutions** offer basic user management with limited visibility.

**Sonderr Teams** provides comprehensive team analytics, role-based permissions, and detailed usage insights, while **Sonderr Enterprise** adds advanced governance, audit logging, and enterprise-level security controls.
...
### What You Gain with Sonderr Teams or Sonderr Enterprise
...
3. **Calculate current costs** (if visible) vs. Sonderr pricing
4. **Plan migration timeline** to minimize disruption

**Step 2: Sonderr Setup**
...
1. **Invite team members** to Sonderr
2. **Install Sonderr extension** alongside Cursor initially
3. **Migrate projects gradually** starting with non-critical work
4. **Train team** on Sonderr features and workflows

**Step 4: Full Transition**

1. **Monitor usage patterns** in Sonderr dashboard
...
| Cursor Feature | Sonderr Equivalent |
|---|---|
...
2. **Install Sonderr** extension for team members
3. **Start with simple tasks** in Sonderr
4. **Compare results** and team satisfaction

**Phase 2: Gradual Transition (Week 3-4)**

1. **Use Sonderr** for new projects
...
Sonderr uses a compatible rules system that supports Cursor and Windsurf patterns. Migrating your custom rules and configurations is straightforward and typically takes 5-10 minutes per project.

**Quick Overview:**

- **Project rules**: `.cursor/rules/*.mdc` → `.sonderr/rules/*.md` (remove YAML frontmatter, keep Markdown content)
- **Legacy rules**: `.cursorrules` → `.sonderr/rules/legacy-rules.md`
- **AGENTS.md**: Works identically in Sonderr (no conversion needed)
- **Global rules**: Recreate in `~/.sonderr/rules/*.md` directory

Sonderr also supports mode-specific rules (`.sonderr/rules-{mode}/`), which Cursor and Windsurf don't have. This allows different rules for different workflows (e.g., Code mode vs Debug mode).
...
- Sonderr extension installation and setup
