...
1. Sign in to the [Sonderr Web App](https://app.kilo.ai).
...
| **Credits** | Credit balances, acquired and used credits, expirations, auto top-up status, Sonderr Pass allocation, recent spend, and estimated runway. |
...
Use **Credits** to compare financial state across sub-organizations. Sonderr Pass allocation represents pooled credit capacity for an organization, not passes assigned to individual people. If a future allocation differs from the current allocation, the table shows the transition separately.

Use **Distribute funds** when you need to transfer credit balance. Sonderr Pass capacity and credit balance are separate quantities and are managed separately.

{% image src="/docs/img/enterprise/sub-organizations-credits.webp" alt="Sub-organization Credits section comparing balances, expirations, Sonderr Pass allocation, spend, and runway" width="100%" caption="Compare credit state and Sonderr Pass allocation across sub-organizations." /%}
