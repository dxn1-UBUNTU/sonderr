...
> current private `@sonderr/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@sonderr/ai`
...
- Secondary audience: Sonderr and other durable agent runtimes
...
import { LLM } from "@sonderr/ai"
import { OpenAI } from "@sonderr/ai/providers/openai"
...
```ts
import { Message } from "@sonderr/ai"
...
import { LLM, Tool } from "@sonderr/ai"
...
Sonderr and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@sonderr/ai/promise"
import { OpenAI } from "@sonderr/ai/providers/openai"
...
```ts
import { LLMSchema } from "@sonderr/ai/schema"
...
```ts
import { Provider, Protocol } from "@sonderr/ai/provider"
...
@sonderr/ai/providers/openai
@sonderr/ai/providers/anthropic
@sonderr/ai/providers/google
...
```
...
Sonderr should migrate to `generateTurn` / `streamTurn`, preserving its durable
