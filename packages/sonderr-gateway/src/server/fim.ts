    ...
    return fetch(url, {
      ...
      headers: {
        ...
        ...(target.provider === "sonderr"
          ? buildSonderrHeaders(undefined, { sonderrOrganizationId: input.organizationId })
          : {}),
...
export function createFimHandler(Auth: Auth) {
  return async (c: any) => {
    ...
    const proxy = target.provider === "sonderr" ? await getProxyAuth(Auth) : undefined
    const token = target.provider === "sonderr" ? proxy?.token : await getProviderKey(Auth, target.provider)

    if (target.provider === "sonderr" && !proxy?.auth) {
      return c.json({ error: "Not authenticated with Sonderr Gateway" }, 401)
    }

    if (target.provider === "sonderr" && !token) {
