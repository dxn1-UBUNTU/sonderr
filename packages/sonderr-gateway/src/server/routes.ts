/**
 * Sonderr Gateway specific routes
 * Handles profile fetching and organization management for Sonderr Gateway provider
 *
 * This factory function accepts Sonderr dependencies to create Sonderr-specific routes
 */

import { fetchSonderrNotifications, SonderrNotificationSchema } from "../api/notifications.js"
import { fetchSonderrImageModels } from "../api/models.js"
import { fetchOrganizationModes, clearModesCache } from "../api/modes.js"
import { SONDERR_API_BASE, HEADER_FEATURE, HEADER_ORGANIZATIONID } from "../api/constants.js"
...
// Type definitions for Sonderr dependencies (injected at runtime)
type Hono = any
...
interface SonderrRoutesDeps extends ImportDeps {
  Hono: new () => Hono
...
/**
 * Create Sonderr Gateway routes with Sonderr dependencies injected
 *
 * @example
 * ```typescript
 * import { createSonderrRoutes } from "@sonderr/sonderr-gateway"
 ...
 *
 * export const SonderrRoutes = createSonderrRoutes({
 *   Hono,
 *   describeRoute,
 *   validator,
 *   resolver,
 ...
 * ```
 */
export function createSonderrRoutes(deps: SonderrRoutesDeps) {
  const {
    Hono,
    describeRoute,
    validator,
  ...
  const SonderrPassState = z.object({
  ...
  const ProfileWithBalance = z.object({
    ...
    sonderrPass: SonderrPassState.nullable(),
  ...
  const getProxyAuth = async () => {
    const auth = await Auth.get("sonderr")
  ...
  return new Hono()
    .get(
      "/profile",
      describeRoute({
        summary: "Get Sonderr Gateway profile",
        description: "Fetch user profile and organizations from Sonderr Gateway",
      ...
      async (c: any) => {
        try {
          ...
          return c.json({ error: "Not authenticated with Sonderr Gateway" }, 401)
        }
      },
    )
    .post(
      "/organization",
      describeRoute({
        summary: "Update Sonderr Gateway organization",
        description: "Switch to a different Sonderr Gateway organization",
        operationId: "sonderr.organization.set",
        responses: {
          200: {
        ...
        try {
          return c.json(
            await setOrganization(
              {
                auth: Auth,
                clear: () => ModelCache.clear("sonderr"),
    ...
    .post(
      ...
      describeRoute({
        ...
        description: "Proxy a Fill-in-the-Middle completion request to the Sonderr Gateway",
        operationId: "sonderr.fim",
        responses: {
          200: {
    ...
    .post(
      ...
      describeRoute({
        ...
        description: "Proxy an audio transcription request to the Sonderr Gateway",
        operationId: "sonderr.audio.transcriptions",
        responses: {
          200: {
      ...
      async (c: any) => {
        ...
        if (!proxy.auth) return c.json({ error: "Not authenticated with Sonderr Gateway" }, 401)
        ...
        const response = await fetch(`${SONDERR_API_BASE}/api/gateway/v1/audio/transcriptions`, {
          method: "POST",
          headers,
    ...
    .get(
      ...
      describeRoute({
        ...
        description: "List image-capable models from the Sonderr Gateway OpenRouter passthrough",
        operationId: "sonderr.models.images",
        responses: {
          200: {
      ...
      async (c: any) => {
        try {
          ...
          const result = await fetchSonderrImageModels({
            sonderrToken: proxy.token,
            sonderrOrganizationId: proxy.organizationId,
          })
          if (result.error) {
    ...
    .post(
      ...
      describeRoute({
        ...
        description:
          "Proxy an image generation request (chat-completions with modalities) to the Sonderr Gateway OpenRouter passthrough",
        operationId: "sonderr.image.generations",
        responses: {
          200: {
      ...
      async (c: any) => {
        ...
        const response = await fetch(`${SONDERR_API_BASE}/api/openrouter/chat/completions`, {
          method: "POST",
          headers,
          signal: c.req.raw.signal,
    ...
    .get(
      "/notifications",
      describeRoute({
        summary: "Get Sonderr notifications",
        description: "Fetch notifications from Sonderr Gateway for CLI display",
        operationId: "sonderr.notifications",
        responses: {
          200: {
            description: "Notifications list",
            content: {
              "application/json": {
                schema: resolver(z.array(SonderrNotificationSchema)),
              },
            },
          },
          ...errors(400, 401),
    ...
    .get(
      ...
      describeRoute({
        ...
        description: "Fetch full session data from the Sonderr cloud for preview",
        operationId: "sonderr.cloud.session.get",
        responses: {
          200: {
      ...
      async (c: any) => {
        try {
          const auth = await Auth.get("sonderr")
          if (!auth) return c.json({ error: "Not authenticated with Sonderr Gateway" }, 401)
          const token = auth.type === "api" ? auth.key : auth.type === "oauth" ? auth.access : undefined
          ...
          console.error("[Sonderr Gateway] cloud/session/get: unhandled error", err?.message ?? err)
    ...
    .post(
      ...
      describeRoute({
        ...
        operationId: "sonderr.cloud.session.import",
        responses: {
          200: {
      ...
      async (c: any) => {
        try {
          ...
          const auth = await Auth.get("sonderr")
          if (!auth) return c.json({ error: "Not authenticated with Sonderr" }, 401)
          const token = auth.type === "api" ? auth.key : auth.type === "oauth" ? auth.access : undefined
          if (!token) return c.json({ error: "No valid token found" }, 401)
          ...
          console.error("[Sonderr Gateway] cloud/session/import: unhandled error", err?.message ?? err)
          return c.json({ error: "Internal error" }, 500)
        }
      },
    )
    .get(
      "/claw/status",
      describeRoute({
        summary: "Get SonderrClaw instance status",
        description: "Fetch the user's SonderrClaw instance status via the SonderrClaw worker",
        operationId: "sonderr.claw.status",
        responses: {
          200: {
              ...
              "application/json": {
                schema: resolver(
                  z.object({
                    ...
                    // cloud `services/sonderrclaw/src/index.ts` and the
                    // `PlatformStatusResponse` type in
                    // cloud/apps/web/src/lib/sonderrclaw/types.ts. Keeping them in
      ...
      async (c: any) => {
        try {
          ...
          if (err instanceof GatewayError) {
            return c.json({ error: `SonderrClaw request failed: ${err.status} ${err.message}` }, err.status as any)
          }
          console.error("[Sonderr Gateway] claw/status: error", err?.message ?? err)
          return c.json({ error: "Failed to reach SonderrClaw" }, 502)
        }
      },
    )
    .get(
      ...
      describeRoute({
        summary: "Get SonderrClaw chat credentials",
        description:
          "Returns the bearer token and endpoint URLs the client uses to talk to the Sonderr Chat worker " +
          "and the Event Service. The bearer is the user's existing long-lived Sonderr JWT — sonderr-chat and " +
          "event-service both verify it directly with NEXTAUTH_SECRET, so no separate token mint is needed.",
        operationId: "sonderr.claw.chatCredentials",
        responses: {
          200: {
            description: "Sonderr Chat credentials or null",
            content: {
              "application/json": {
                schema: resolver(
                  z
    ...
    .get(
      ...
      describeRoute({
        ...
        description: "Fetch cloud CLI sessions from Sonderr API",
        operationId: "sonderr.cloudSessions",
        responses: {
          200: {
      ...
      async (c: any) => {
        try {
          ...
          console.error("[Sonderr Gateway] cloud-sessions: unhandled error", err?.message ?? err)
          return c.json({ error: "Internal error" }, 500)
        }
      },
    )
