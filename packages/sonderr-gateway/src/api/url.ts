...
function base(options: UrlOptions): string {
  return getSonderrUrlFromToken(options.baseURL ?? SONDERR_API_BASE, options.token ?? "")
}

export function resolveSonderrGatewayBaseUrl(options: UrlOptions = {}): string {
  return route(base(options), "gateway")
}

export function resolveSonderrOpenRouterBaseUrl(options: UrlOptions = {}): string {
