...
import type { SonderrProfile, Organization, SonderrBalance } from "../types.js"
import { SONDERR_API_BASE, DEFAULT_MODEL, DEFAULT_FREE_MODEL } from "./constants.js"

/**
 * Fetch user profile from Sonderr API
 */
export async function fetchProfile(token: string): Promise<SonderrProfile> {
  const response = await fetch(`${SONDERR_API_BASE}/api/profile`, {
    headers: {
  ...
  // Transform to flat SonderrProfile structure
  return {
 ...
 */
export const getSonderrProfile = fetchProfile
 ...
 */
export function defaultOrganizationId(profile: SonderrProfile): string | undefined {
...
/**
 * Fetch user balance from Sonderr API
 * @param token - Authentication token
 * @param organizationId - Optional organization ID for team balance
 */
export async function fetchBalance(token: string, organizationId?: string): Promise<SonderrBalance | null> {
  try {
    ...
    if (organizationId) {
      headers["x-sonderr-organizationid"] = organizationId
    }

    const response = await fetch(`${SONDERR_API_BASE}/api/profile/balance`, { headers })
 ...
 * Alias for compatibility with existing code
 */
export const getSonderrBalance = fetchBalance
...
export async function fetchDefaultModel(token?: string, organizationId?: string): Promise<string> {
  ...
  const url = `${SONDERR_API_BASE}${path}`
 ...
 * Alias for compatibility with existing code
 */
export const getSonderrDefaultModel = fetchDefaultModel
