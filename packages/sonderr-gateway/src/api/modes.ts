import { z } from "zod"
import { SONDERR_API_BASE, MODELS_FETCH_TIMEOUT_MS } from "./constants.js"
...
/**
 * Fetch custom modes for an organization from the Sonderr Cloud API.
 *
...
export async function fetchOrganizationModes(token: string, organizationId: string): Promise<OrganizationMode[]> {
  ...
  try {
    const url = `${SONDERR_API_BASE}/api/organizations/${encodeURIComponent(organizationId)}/modes`
    ...
    console.warn("[Sonderr Gateway] Error fetching organization modes:", err)
    return []
  }
}
