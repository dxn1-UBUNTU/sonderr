...
const SONDERR_FIM_URL = SONDERR_API_BASE + "/api/fim/completions"
const INCEPTION_FIM_URL = "https://api.inceptionlabs.ai/v1/fim/completions"

function sonderrTarget(model?: string): FimTarget {
  return { provider: "sonderr", model: model ?? "mistralai/codestral-2501", url: SONDERR_FIM_URL }
}

export function resolveFimTarget(provider?: string, model?: string): FimTarget {
  if (!provider || provider === "sonderr") return sonderrTarget(model)
