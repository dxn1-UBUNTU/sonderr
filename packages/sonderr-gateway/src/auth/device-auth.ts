...
import { getSonderrProfile, getSonderrDefaultModel, promptOrganizationSelection } from "../api/profile.js"
...
export async function authenticateWithDeviceAuth(): Promise<DeviceAuthResult> {
  ...
  const profileData = await getSonderrProfile(token)
  ...
  const model = await getSonderrDefaultModel(token, organizationId)
