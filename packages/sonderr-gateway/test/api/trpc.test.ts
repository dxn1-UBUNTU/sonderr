...
describe("Cloud tRPC client", () => {
  test("uses unbatched GET queries without an organization header", async () => {
    const fn = mock(() =>
      Promise.resolve(
        result([
          {
            ...subscription,
            routeLabel: "MiniMax via Sonderr Gateway",
    ...
    expect(new Headers(call[1].headers).has("x-sonderr-organizationid")).toBe(false)
  ...
  test("decodes procedure errors even when HTTP is successful", async () => {
    ...
    expect(error).toMatchObject({ kind: "procedure", message: "Sonderr Cloud data is temporarily unavailable." })
