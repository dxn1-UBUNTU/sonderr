...
import { EMPTY_SONDERR_EMBEDDING_MODEL_CATALOG, fetchSonderrEmbeddingModelCatalog } from "../../src/api/embedding-models"
...
describe("fetchSonderrEmbeddingModelCatalog", () => {
  test("fetches catalog from Sonderr Gateway", async () => {
    const prev = global.fetch
    const fn = mock(() => Promise.resolve(response())) as unknown as typeof fetch
    global.fetch = fn

    try {
      const catalog = await fetchSonderrEmbeddingModelCatalog({ baseURL: "https://example.test" })
  ...
  test("bounds caller-controlled retry attempts", async () => {
    ...
    try {
      await fetchSonderrEmbeddingModelCatalog({ baseURL: "https://example.test", attempts: Number.POSITIVE_INFINITY })
  ...
  test("reports a final failure without writing to the console", async () => {
    ...
    try {
      await expect(
        fetchSonderrEmbeddingModelCatalog({ baseURL: "https://example.test", attempts: 1, onError: issue }),
      ).resolves.toEqual(EMPTY_SONDERR_EMBEDDING_MODEL_CATALOG)
      expect(issue).toHaveBeenCalledWith({
        code: "http",
        message: "Unable to load Sonderr embedding models (HTTP 500).",
        status: 500,
      })
  ...
  test("fallback catalog is empty so Cloud owns model metadata", () => {
    expect(EMPTY_SONDERR_EMBEDDING_MODEL_CATALOG).toEqual({
      defaultModel: "",
      models: [],
      aliases: {},
    })
