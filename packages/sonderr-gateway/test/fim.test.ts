...
describe("FIM target resolution", () => {
  test("keeps gateway autocomplete models on Sonderr Gateway", () => {
    expect(resolveFimTarget("sonderr", "mistralai/codestral-2508")).toEqual({
      provider: "sonderr",
      model: "mistralai/codestral-2508",
      url: "https://api.kilo.ai/api/fim/completions",
    })
    expect(resolveFimTarget("sonderr", "inception/mercury-edit-2")).toEqual({
      provider: "sonderr",
  ...
  test("preserves gateway model pass-through behavior", () => {
    ...
    expect(resolveFimTarget("sonderr", "custom/fim-model")).toEqual({
      provider: "sonderr",
