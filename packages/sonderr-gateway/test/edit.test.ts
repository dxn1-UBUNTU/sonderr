...
describe("Edit target resolution", () => {
  ...
  test("does NOT route the FIM Mercury model to the edit endpoint", () => {
    // `mercury-edit-2` (kind: fim) must fall through to the sonderr placeholder,
    // not the edit endpoint — only `mercury-next-edit` (kind: edit) is NES.
    expect(resolveEditTarget("inception", "mercury-edit-2").provider).toBe("sonderr")
  })

  test("routes the Sonderr Gateway next-edit model to the gateway proxy", () => {
    const target = resolveEditTarget("sonderr", "inception/mercury-next-edit")
    expect(target.provider).toBe("sonderr")
    expect(target.model).toBe("inception/mercury-edit-2")
    expect(target.url).toMatch(/\/api\/edit\/completions$/)
  })

  test("falls back to a sonderr placeholder (no upstream) for non-edit models", () => {
    expect(resolveEditTarget("sonderr", "mistralai/codestral-2508")).toEqual({
      provider: "sonderr",
