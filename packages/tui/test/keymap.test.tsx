...
import { getSonderrModeStack, SONDERR_BASE_MODE, SonderrKeymapProvider, registerSonderrKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerSonderrKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when sonderr mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getSonderrModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getSonderrModeStack(keymap).push("autocomplete")
