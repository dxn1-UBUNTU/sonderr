...
import { createDefaultOptions, styleVariables } from "@sonderr/ui/pierre"
import { getWorkerPool } from "@sonderr/ui/pierre/worker"
...
const codeStyle = {
  ...styleVariables,
  "--diffs-line-height": "var(--sonderr-font-size-24)",
}

function lineHeight() {
  if (typeof window === "undefined") return codeMetrics.lineHeight
  const value = Number.parseFloat(getComputedStyle(document.documentElement).getPropertyValue("--sonderr-font-size-24"))
...
export function Code<T>(props: CodeProps<T>) {
  ...
  const clearHighlightFind = () => {
    ...
    if (!api) return
    api.delete("sonderr-find")
    api.delete("sonderr-find-current")
  }
  ...
  const setHighlights = (ranges: Range[], index: number) => {
    ...
    if (active) api.set("sonderr-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== index)
    if (rest.length > 0) api.set("sonderr-find", new Highlight(...rest))
    return true
  }
