...
export interface SonderrPassMeterProps extends Omit<ComponentProps<"div">, "children"> {
  used: number
  paid: number
  bonus: number
  label: JSX.Element
...
export function SonderrPassMeter(props: SonderrPassMeterProps) {
  ...
  return (
    <div
      {...rest}
      data-component="sonderr-pass-meter"
      role="meter"
      aria-valuemin={0}
    ...
    >
      <div data-slot="sonderr-pass-meter-header">
        <span>{local.label}</span>
        <strong>
          {local.format(model().used)} / {local.format(model().total)}
        </strong>
      </div>
      <div data-slot="sonderr-pass-meter-track" aria-hidden="true">
        <div data-slot="sonderr-pass-meter-paid-background" style={{ width: `${model().boundary}%` }} />
        <div
          data-slot="sonderr-pass-meter-bonus-background"
          hidden={model().bonus <= 0}
          style={{ left: `${model().boundary}%`, width: `${100 - model().boundary}%` }}
        />
        <div data-slot="sonderr-pass-meter-paid-fill" style={{ width: `${model().paidFill}%` }} />
        <div
          data-slot="sonderr-pass-meter-bonus-fill"
          hidden={model().bonusFill <= 0}
          style={{ left: `${model().boundary}%`, width: `${model().bonusFill}%` }}
        />
        <div
          data-slot="sonderr-pass-meter-boundary"
          hidden={model().bonus <= 0}
          style={{ left: `${model().boundary}%` }}
        />
      </div>
      <div data-slot="sonderr-pass-meter-amounts" aria-hidden="true">
        <span
        ...
        <span data-slot="sonderr-pass-meter-bonus-amount" hidden={model().bonus <= 0}>
          {local.format(model().bonus)}
        </span>
      </div>
      <div data-slot="sonderr-pass-meter-legend">
        <span>
          <i data-slot="sonderr-pass-meter-paid-dot" aria-hidden="true" />
          {local.paidLabel}
        </span>
        <span hidden={model().bonus <= 0}>
          <i data-slot="sonderr-pass-meter-bonus-dot" aria-hidden="true" />
          {local.bonusLabel}
        </span>
      </div>
    </div>
