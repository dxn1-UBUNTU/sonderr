export const INDEXING_PLUGIN_NAMES = ["sonderr-indexing", "@sonderr/sonderr-indexing"] as const
...
function isPathSpecifier(value: string): boolean {
  ...
  if (normalized.includes("/.sonderr/") || normalized.includes("/.sonderr/") || normalized.includes("/.sonderr/")) {
    return true
  }
...
function normalizePath(value: string): string {
  ...
  const scoped = parts.findIndex((part, i) => part === "@sonderr" && parts[i + 1] === "sonderr-indexing")
  if (scoped >= 0) return "@sonderr/sonderr-indexing"

  const workspace = parts.findIndex((part, i) => part === "packages" && parts[i + 1] === "sonderr-indexing")
  if (workspace >= 0) return "@sonderr/sonderr-indexing"
