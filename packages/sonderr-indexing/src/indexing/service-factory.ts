...
import { SonderrEmbedder } from "./embedders/sonderr"
...
export class CodeIndexServiceFactory {
  ...
  public createEmbedder(): IEmbedder {
    ...
    if (provider === "sonderr") {
      if (!config.sonderrOptions?.apiKey) throw new Error("Sonderr API key is required for embedding.")
      if (!config.modelId) throw new Error("Sonderr embedding model is required.")
      return new SonderrEmbedder({
        apiKey: config.sonderrOptions.apiKey,
        baseUrl: config.sonderrOptions.baseUrl,
        organizationId: config.sonderrOptions.organizationId,
