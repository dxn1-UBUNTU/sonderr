...
export interface IndexingConfigInput {
  ...
  sonderrApiKey?: string
  sonderrBaseUrl?: string
...
export class CodeIndexConfigManager {
  ...
  private sonderrOptions?: { apiKey: string; baseUrl?: string; organizationId?: string }
  ...
  private applyInput(input: IndexingConfigInput): void {
    ...
    this.sonderrOptions = input.sonderrApiKey
      ? { apiKey: input.sonderrApiKey, baseUrl: input.sonderrBaseUrl, organizationId: input.sonderrOrganizationId }
      : undefined
  ...
  private captureSnapshot(): PreviousConfigSnapshot {
    return {
      ...
      sonderrApiKey: this.sonderrOptions?.apiKey ?? "",
      sonderrBaseUrl: this.sonderrOptions?.baseUrl ?? "",
      sonderrOrganizationId: this.sonderrOptions?.organizationId ?? "",
  ...
  public isConfigured(): boolean {
    ...
    if (provider === "sonderr")
      return !!(this.sonderrOptions?.apiKey && this.modelId && this.currentModelDimension && hasStore)
  ...
  doesConfigChangeRequireRestart(prev: PreviousConfigSnapshot): boolean {
    ...
    // Auth changes
    if ((prev.sonderrApiKey ?? "") !== (this.sonderrOptions?.apiKey ?? "")) return true
    if ((prev.sonderrBaseUrl ?? "") !== (this.sonderrOptions?.baseUrl ?? "")) return true
    if ((prev.sonderrOrganizationId ?? "") !== (this.sonderrOptions?.organizationId ?? "")) return true
  ...
  public getConfig(): CodeIndexConfig {
    return {
      ...
      modelId: this.modelId,
      modelDimension: this.modelDimension,
      sonderrOptions: this.sonderrOptions,
