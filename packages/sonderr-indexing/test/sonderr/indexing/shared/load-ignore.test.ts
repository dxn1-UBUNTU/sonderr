...
describe("loadIgnore", () => {
  ...
  test("loads root .sonderrignore in addition to root .gitignore", async () => {
    await writeFile(path.join(root, ".gitignore"), "dist/\n")
    await writeFile(path.join(root, ".sonderrignore"), "secret/\n")
  ...
  test("preserves existing .gitignore-only behavior when .sonderrignore is absent", async () => {
  ...
  test("loads nested .sonderrignore relative to its directory", async () => {
    await mkdir(path.join(root, "pkg", "sub"), { recursive: true })
    await writeFile(path.join(root, "pkg", ".sonderrignore"), "secret.ts\n")
  ...
  test("allows descendants when a parent directory is re-included", async () => {
    await mkdir(path.join(root, "pkg"), { recursive: true })
    await writeFile(path.join(root, ".gitignore"), "pkg/\n")
    await writeFile(path.join(root, ".sonderrignore"), "!pkg/\n")
  ...
  test("keeps descendants ignored when only a child directory is re-included", async () => {
    await mkdir(path.join(root, "pkg", "sub"), { recursive: true })
    await writeFile(path.join(root, ".gitignore"), "pkg/\n")
    await writeFile(path.join(root, ".sonderrignore"), "!pkg/sub/\n")
  ...
  test("applies .sonderrignore after .gitignore in the same directory", async () => {
    await writeFile(path.join(root, ".gitignore"), "*.ts\n")
    await writeFile(path.join(root, ".sonderrignore"), "!keep.ts\n")
  ...
  test("ignores the ignore files themselves", async () => {
    ...
    expect(ig.ignores(".sonderrignore")).toBe(true)
  })

  test("keeps ignore files ignored after negation rules", async () => {
    await writeFile(path.join(root, ".sonderrignore"), "!.gitignore\n")
  ...
  test("ignores ignore file names even when absent during loading", async () => {
    ...
    expect(ig.ignores(".gitignore")).toBe(true)
    expect(ig.ignores("pkg/.sonderrignore")).toBe(true)
  })
