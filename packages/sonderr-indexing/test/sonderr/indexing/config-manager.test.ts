...
describe("CodeIndexConfigManager", () => {
  ...
  test("configures Sonderr with hosted auth options and explicit model metadata", () => {
    const cfg = new CodeIndexConfigManager(
      createInput({
        embedderProvider: "sonderr",
        openAiKey: undefined,
        sonderrApiKey: "sonderr-token",
        sonderrBaseUrl: "https://example.test/api/gateway/",
    ...
    expect(cfg.isFeatureConfigured).toBe(true)
    expect(cfg.getConfig().sonderrOptions).toEqual({
      apiKey: "sonderr-token",
  ...
  test("requires Sonderr model metadata from Cloud config", () => {
    const cfg = new CodeIndexConfigManager(
      createInput({
        embedderProvider: "sonderr",
        openAiKey: undefined,
  ...
  test("uses configured dimension for Sonderr models outside the fallback catalog", () => {
    const cfg = new CodeIndexConfigManager(
      createInput({
        embedderProvider: "sonderr",
        openAiKey: undefined,
  ...
  describe("loadConfiguration restart checks", () => {
    ...
    test("requires restart when Sonderr auth changes", () => {
      const cfg = new CodeIndexConfigManager(
        createInput({
          embedderProvider: "sonderr",
          openAiKey: undefined,
          sonderrApiKey: "old-token",
          modelId: "mistralai/mistral-embed-2312",
          modelDimension: 1024,
        }),
      )
