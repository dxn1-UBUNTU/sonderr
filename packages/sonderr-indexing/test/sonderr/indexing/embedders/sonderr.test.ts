...
import { SonderrEmbedder, SONDERR_INDEXING_FEATURE } from "../../../../src/indexing/embedders/sonderr"

describe("SonderrEmbedder", () => {
  beforeEach(() => {
    mockEmbeddingsCreate.mockReset()
    setOpenAIConstructorHook(undefined)
  })

  test("uses Sonderr Gateway headers and configured embedding model", async () => {
    const seen: unknown[] = []
    ...
    const embedder = new SonderrEmbedder({
    ...
    expect(seen[0]).toEqual({
      ...
      apiKey: "sonderr-token",
      defaultHeaders: {
        "X-SONDERR-FEATURE": SONDERR_INDEXING_FEATURE,
        "X-SONDERR-ORGANIZATIONID": "org_123",
      },
    })
