...
export interface Interface {
   ...
   *
   * This exists while the old sonderr project service and this core project
...
export class Service extends Context.Service<Service, Interface>()("@sonderr/ProjectV2") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const cached = Effect.fnUntraced(function* (dir: string) {
      return yield* fs.readFileString(path.join(dir, "sonderr")).pipe( // sonderr_change
    ...
    const commit = Effect.fn("Project.commit")(function* (input: { store: AbsolutePath; id: ID }) {
      yield* fs.writeFileString(path.join(input.store, "sonderr"), input.id).pipe(Effect.ignore) // sonderr_change
    })
