...
import { makeGlobalNode, makeLocationNode } from "./effect/app-node" // sonderr_change
import { Context, Effect, Layer, Schema, Types } from "effect"
import { Pty } from "@sonderr/schema/pty"
import { Config } from "./config"
import { EventV2 } from "./event"
import { Location } from "./location"
import { PtyID } from "./pty/schema"
import { SessionSchema } from "./session/schema" // sonderr_change
import { Shell } from "./shell"
import { lazy } from "./util/lazy"
import { SonderrPtySelfCommand } from "./sonderr/pty-self-command" // sonderr_change
import * as SonderrPtyRegistry from "./sonderr/pty/registry" // sonderr_change
import type { Active, Subscriber } from "./sonderr/pty/registry" // sonderr_change
...
// sonderr_change - the Sonderr `sessionID` field now lives on the canonical shared schema (see
...
export const UpdateInput = Schema.Struct({
  ...
  sessionID: Schema.optional(Schema.NullOr(SessionSchema.ID)), // sonderr_change
})

export type UpdateInput = Types.DeepMutable<typeof UpdateInput.Type>

// sonderr_change - the shared events already carry Sonderr's extended Info (see packages/schema/src/pty.ts),
...
export type AttachInput = {
  ...
  readonly allowExited?: boolean // sonderr_change
}
...
export interface Interface {
  ...
  readonly removeDirectory: (location: Location.Ref) => Effect.Effect<void> // sonderr_change
...
export class Service extends Context.Service<Service, Interface>()("@sonderr/v2/Pty") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const sessions = SonderrPtyRegistry.sessions // sonderr_change
    ...
    const requireSession = Effect.fn("Pty.requireSession")(function* (id: PtyID) {
      ...
      if (!session || !SonderrPtyRegistry.sameLocation(session.location, owner))
        return yield* new NotFoundError({ ptyID: id })
      return session
    })

    const removeSession = Effect.fnUntraced(function* (id: PtyID) {
      // sonderr_change start - removal and its deleted event are one uninterruptible lifecycle transition.
      const session = sessions.get(id)
      if (!session || !SonderrPtyRegistry.claimRemoval(id)) return
      yield* Effect.uninterruptible(
        Effect.gen(function* () {
          yield* Effect.logInfo("removing session", { id })
          yield* Effect.promise(() => SonderrPtyRegistry.teardown(session))
          sessions.delete(id)
          SonderrPtyRegistry.removeExited(session)
          yield* events
        ...
        }).pipe(Effect.ensuring(Effect.sync(() => SonderrPtyRegistry.releaseRemoval(id)))),
      )
      // sonderr_change end
    })
    ...
    const removeDirectory = Effect.fn("Pty.removeDirectory")(function* (target: Location.Ref) {
      const owned = Array.from(sessions.values()).filter(
        (session) =>
          SonderrPtyRegistry.sameDirectory(session.location.directory, target.directory) &&
    ...
    const list = Effect.fn("Pty.list")(function* () {
      ...
      return Array.from(sessions.values())
        .filter((session) => SonderrPtyRegistry.sameLocation(session.location, owner))
    ...
    const createBody = Effect.fn("Pty.createBody")(function* (input: CreateInput, owner: Location.Ref) {
      ...
      // sonderr_change start - resolve Sonderr self-commands to the real binary, arguments, and project cwd
      const resolved = SonderrPtySelfCommand.resolve({
      ...
      const env = {
        ...
        TERM: "xterm-256color",
        SONDERR_TERMINAL: "1",
        SONDERR_PTY_ID: id, // sonderr_change - let nested Sonderr processes identify their parent terminal
      } as Record<string, string>
      // sonderr_change start - do not expose local server credentials to user terminals.
      // node-pty inherits parent values for omitted keys, so empty tombstones are required.
      env.SONDERR_SERVER_PASSWORD = ""
      env.SONDERR_SERVER_USERNAME = ""
      // sonderr_change end
      ...
      session.listeners.push(
        ...
        proc.onExit(({ exitCode }) => {
          ...
          SonderrPtyRegistry.markExited(session)
          runFork(
            Effect.gen(function* () {
              ...
              while (SonderrPtyRegistry.exitedCount(session.location) > EXITED_LIMIT) {
                const oldest = SonderrPtyRegistry.oldestExited(session.location)
                if (!oldest) break
                yield* removeSession(oldest)
                if (sessions.has(oldest)) break
                SonderrPtyRegistry.removeExitedID(session.location, oldest)
              }
            }),
          )
        }),
    ...
    const create = Effect.fn("Pty.create")(function* (input: CreateInput) {
      const owner = Location.Ref.make({ directory: location.directory, workspaceID: location.workspaceID })
      const release = SonderrPtyRegistry.beginCreate(owner)
    ...
    const update = Effect.fn("Pty.update")(function* (id: PtyID, input: UpdateInput) {
      ...
      // sonderr_change start - associate nested Sonderr TUI terminals with the viewed session
    ...
    const attach = Effect.fn("Pty.attach")(function* (id: PtyID, input: AttachInput) {
      const session = yield* requireSession(id)
      if (session.info.status !== "running" && !input.allowExited) return yield* new ExitedError({ ptyID: id }) // sonderr_change
      ...
      const subscriber: Subscriber = {
        ...
        detached: false,
        pending: [],
        end: session.info.status === "exited" ? { exitCode: session.info.exitCode } : undefined, // sonderr_change
      }
    ...
    return Service.of({ list, get, create, update, remove, removeDirectory, write, attach }) // sonderr_change
  }),
)

export const locationLayer = layer.pipe(Layer.provide(Config.locationLayer))

export const shutdown = SonderrPtyRegistry.shutdown // sonderr_change
export const terminateDirectory = SonderrPtyRegistry.terminateDirectory // sonderr_change

export const shutdownNode = makeGlobalNode({
  name: "pty-shutdown",
  layer: Layer.effectDiscard(
    Effect.gen(function* () {
      const release = yield* Effect.promise(() => SonderrPtyRegistry.acquireOwner())
