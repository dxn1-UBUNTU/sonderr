export * as SonderrOauthCallbackPage from "./page"

import { OauthCallbackPage, type CallbackPageOptions } from "../../oauth/page"

const SONDERR_MARK = `<svg class="wordmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" fill="none" aria-label="Sonderr" role="img">
...
function brand(page: string) {
  return page.replace(/<svg class="wordmark"[\s\S]*?<\/svg>/, SONDERR_MARK).replaceAll("Sonderr", "Sonderr")
}
