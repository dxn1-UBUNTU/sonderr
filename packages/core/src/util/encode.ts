export function base64Encode(value: string) {
  const normalized = value.replace(/\\/g, "/") // sonderr_change - normalize Windows paths
  const bytes = new TextEncoder().encode(normalized) // sonderr_change - encode as UTF-8 bytes
