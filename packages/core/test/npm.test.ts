...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@sonderr/acme")).toBe("@sonderr/acme")
    expect(Npm.sanitize("@sonderr/acme@1.0.0")).toBe("@sonderr/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/sonderr/acme.git"
    const expected = win ? "acme@git+https_//github.com/sonderr/acme.git" : spec
