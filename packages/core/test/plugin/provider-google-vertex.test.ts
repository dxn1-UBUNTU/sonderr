...
import { GoogleVertexPlugin } from "@sonderr/core/plugin/provider/google-vertex"
      ...
      yield* catalog.transform((catalog) =>
        catalog.provider.update(ProviderV2.ID.sonderr, (provider) => {
          provider.api = {
            ...
            url: "https://sonderr.ai/zen/v1",
          }
        }),
      )
      yield* addPlugin()

      const provider = required(yield* catalog.provider.get(ProviderV2.ID.sonderr))
