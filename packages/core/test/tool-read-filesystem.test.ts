...
describe("ReadToolFileSystem", () => {
  it.effect("fails with a typed filesystem error when a resolved file disappears", () =>
    Effect.gen(function* () {
      ...
      const resolved = yield* target(fs, file)
      yield* files.remove(file)

      const error = yield* ReadToolFileSystem.read(fs, resolved, "missing.txt").pipe(Effect.flip)
  ...
  it.effect("fails when a file becomes the wrong path kind", () =>
    Effect.gen(function* () {
      ...
      const error = yield* ReadToolFileSystem.read(fs, yield* target(fs, directory), "folder").pipe(Effect.flip)
  ...
  it.effect("fails with a typed filesystem error when directory listing fails", () =>
    Effect.gen(function* () {
      ...
      const error = yield* ReadToolFileSystem.list(fs, yield* target(fs, file)).pipe(Effect.flip)
  ...
  it.effect("reports binary and malformed UTF-8 content as typed errors", () =>
    Effect.gen(function* () {
      ...
      const binaryError = yield* ReadToolFileSystem.read(fs, yield* target(fs, binary), "archive.dat").pipe(Effect.flip)
      const malformedError = yield* ReadToolFileSystem.read(fs, yield* target(fs, malformed), "malformed.txt").pipe(Effect.flip)
  ...
  it.effect("reports out-of-range pagination as a typed error", () =>
    Effect.gen(function* () {
      ...
      const error = yield* ReadToolFileSystem.read(fs, yield* target(fs, file), "short.txt", { offset: 2 }).pipe(Effect.flip)
  ...
  it.effect("stops reading after the requested page is complete", () =>
    Effect.gen(function* () {
      ...
      for (const [name, trailing] of [
        ...
        const result = yield* ReadToolFileSystem.read(fs, yield* target(fs, file), name, { limit: 1 })
  ...
  it.effect("preserves the media ingestion limit message", () =>
    Effect.gen(function* () {
      ...
      const error = yield* ReadToolFileSystem.read(fs, yield* target(fs, file), "oversized.png").pipe(Effect.flip)
