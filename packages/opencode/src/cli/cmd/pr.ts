...
export const PrCheckoutCommand = effectCmd({
  ...
  describe: "fetch and checkout a GitHub PR branch, then run sonderr", // kilocode_change
  builder: (yargs) =>
  ...
  handler: Effect.fn("Cli.pr.checkout")(function* (args) {
    ...
    UI.println()
    UI.println("Starting sonderr...") // kilocode_change
    UI.println()
    ...
    if (code !== 0) return yield* Effect.die(new Error(`sonderr exited with code ${code}`)) // kilocode_change
  }),
})
