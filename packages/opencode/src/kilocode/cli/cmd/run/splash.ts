...
//
// Renders the full Sonderr entry logo and a compact [O] exit badge, plus // sonderr_change
...
function build(input: SplashWriterInput, kind: "entry" | "exit", ctx: ScrollbackRenderContext): ScrollbackSnapshot {
  ...
  if (kind === "entry") {
    ...
    const text = "SONDERR"
    const gap = 2
    const totalWidth = (mark[0]?.length ?? 0) + gap + text.length
    const startLeft = Math.max(0, Math.floor((width - totalWidth) / 2))
    ...
    push(lines, startLeft + (mark[0]?.length ?? 0) + gap, top, text, right, undefined, TextAttributes.BOLD)
    if (input.detail) {
      push(
        lines,
        startLeft + (mark[0]?.length ?? 0) + gap,
        top + 1,
        Locale.truncateMiddle(input.detail, Math.max(1, width - startLeft - (mark[0]?.length ?? 0) - gap)),
        left,
        undefined,
      )
    }
  ...
  if (kind === "exit") {
    ...
    push(
      ...
      top + 1,
       `sonderr run -i -s ${meta.session_id}`, // sonderr_change
      right,
      undefined,
      TextAttributes.BOLD,
    )
