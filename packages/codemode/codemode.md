...
This is the living design and status document for `@sonderr/codemode` and its existing V2 Sonderr adapter.
...
`packages/cli/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
