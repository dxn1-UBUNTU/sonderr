...
beforeEach(() => {
  profileCalls = 0
  dir = mkdtempSync(path.join(tmpdir(), "sonderr-telemetry-identity-"))
  Identity.reset()
...
describe("Identity.updateFromSonderrAuth profile cache", () => {
  test("fetches profile and writes cache keyed by token hash", async () => {
    await Identity.updateFromSonderrAuth("token-a")
  ...
  test("refetches when the token changes", async () => {
    await Identity.updateFromSonderrAuth("token-a")
    Identity.reset()
    await Identity.updateFromSonderrAuth("token-b")
  ...
  test("clears identity when token is null", async () => {
    await Identity.updateFromSonderrAuth("token-a")
    Identity.reset()
    await Identity.updateFromSonderrAuth(null)
