# Sonderr V2 Promise Plugin API

The Promise plugin API is the async/await equivalent of `@sonderr/plugin/v2/effect`. It grants plugins the same two in-process capabilities:

- `hook` installs behavior at an Sonderr extension point.
...
```ts
import { define } from "@sonderr/plugin/v2/promise"
